...
} from "../prescribed-path-analysis/B1CompleteCycleProbeProtocol.mjs";

export const EVOLVED_FAMILY_C_US_PROTOCOL_SCHEMA =
  "eom-analysis/evolved-family-c-us-protocol.v1";
export const EVOLVED_FAMILY_C_US_RESULT_SCHEMA =
  "eom-analysis/evolved-family-c-us-result.v1";
...
export function validateEvolvedFamilyCUsProtocol(raw) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    throw new TypeError("evolved Family-C U-S protocol must be an object.");
  }
  if (raw.schema !== EVOLVED_FAMILY_C_US_PROTOCOL_SCHEMA) {
    throw new TypeError(
      `protocol.schema must be ${EVOLVED_FAMILY_C_US_PROTOCOL_SCHEMA}.`,
    );
  }
  if (raw.familyId !== "C") {
    throw new TypeError("protocol.familyId must be C.");
  }
  ...
  return Object.freeze({
    ...
    familyId: "C",
    memberId: concreteString(raw.memberId, "protocol.memberId"),
    fieldSpeed: 1,
...
function validateRecord(record, protocol) {
  if (record?.schema !== ASSEMBLY_VIEW_RECORD_SCHEMA) {
    throw new TypeError(
      `evolved Family-C U-S analysis requires ${ASSEMBLY_VIEW_RECORD_SCHEMA}.`,
    );
  }
  ...
  if (dataset.worldlines.length !== 12) {
    throw new RangeError(
      `Family C requires exactly 12 evolved worldlines; received ${dataset.worldlines.length}.`,
    );
  }
  ...
  if (Math.abs(netCharge) >
    ...
    throw new RangeError(
      `Family-C U-S analysis requires a neutral source; net charge is ${netCharge}.`,
    );
  }
...
export function evaluateEvolvedFamilyCUsRelationship({ record, protocol: rawProtocol }) {
  const protocol = validateEvolvedFamilyCUsProtocol(rawProtocol);
  ...
  return {
    schema: EVOLVED_FAMILY_C_US_RESULT_SCHEMA,
    protocol: {
    ...
    claimBoundary:
      "This instrument measures one direct EOM evolved record. A Family-C " +
