...
import {
  COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_SPACING_SENSITIVITY_ADAPTER,
  createCoincidentAxisAndTwoComponentCircularPilotInventory,
} from "./CoincidentAxisAndTwoComponentCircularPilot.mjs";

export const COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CAMPAIGN_REDUCER_VERSION =
  "prescribed-record-analytics/coincident-axis-and-two-component-circular-pilot-reducer/v3";
export const COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CAMPAIGN_ID =
  "coincident-axis-and-two-component-circular-prescribed-path-pilot.v3";
...
function augmentCoincidentAxisAndTwoComponentCircularPacket(packet, sourceRecord) {
  ...
  const generalized = {
    schema: "prescribed-path-analysis/coincident-axis-and-two-component-circular-reduction.v3",
    reducerVersion: COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CAMPAIGN_REDUCER_VERSION,
  ...
  modified.reducer.coincidentAxisAndTwoComponentCircularReducer =
    COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CAMPAIGN_REDUCER_VERSION;
  modified.diagnosticReductions.coincidentAxisAndTwoComponentCircular = generalized;
  modified.convergenceComparisons.coincidentAxisAndTwoComponentCircularResiduals = convergence;
  ...
  else modified.reducedMeasures.coincidentAxisAndTwoComponentCircular = generalized;
  modified.methodologyBoundary.coincidentAxisAndTwoComponentCircular =
...
function compactMeasures(packet) {
  const generalized = packet.diagnosticReductions.coincidentAxisAndTwoComponentCircular;
  return {
...
export function buildCoincidentAxisAndTwoComponentCircularPilotCampaign({
  outputDirectory,
  protocolPath,
  seed = 20210102,
  ...
  const inventory = createCoincidentAxisAndTwoComponentCircularPilotInventory({
    seed,
  ...
  for (let index = 0; index < inventory.length; index += 1) {
    ...
    const packet = augmentCoincidentAxisAndTwoComponentCircularPacket(
      evaluateCompleteCycleCandidate({
        candidateId: row.spec.specId,
        ...
        onRawPacket,
        sensitivityAdapter: COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_SPACING_SENSITIVITY_ADAPTER,
        onProgress(progress) {
          onProgress?.({
            ...progress,
            completedCandidates: index,
    ...
    const packetRelativePath = `packets/${row.spec.specId}.complete-cycle-result.v3.json`;
    const sourceRelativePath = `exact-sources/${row.spec.specId}.exact-source-record.v1.json`;
    const specRelativePath = `specs/${row.spec.specId}.coincident-axis-and-two-component-circular-configuration-spec.v3.json`;
    ...
    const caseRow = {
      ...
      geometryKind: row.sourceRecord.configuration.geometryKind,
      configurationId: row.sourceRecord.configuration.configurationId,
      seed: row.seed ?? null,
    ...
    manifestCases.push({
      ...
      geometryKind: caseRow.geometryKind,
      configurationId: caseRow.configurationId,
  ...
  const manifest = {
    ...
    campaignId: COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CAMPAIGN_ID,
    campaignStage: "coincident-axis-and-two-component-circular-complete-cycle-prescribed-path-pilot",
    registry: {
    ...
    catalog: {
      catalogId: "coincident-axis-and-two-component-circular-pilot.v3",
    ...
    reducer: {
      version: COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CAMPAIGN_REDUCER_VERSION,
    ...
    outputs: {
      ...
      summaryFilename: "coincident-axis-and-two-component-circular-pilot.summary.v3.json",
    },
  };
  ...
  const summaryWithoutHash = {
    ...
    status: {
      ...
      severity: "ok",
      message:
        "every bounded coincident-axis and two-component circular pilot row received complete-cycle prescribed-path analysis",
    },
    claimBoundary: {
  ...
  const manifestPath = path.join(
    absoluteOutput,
    "coincident-axis-and-two-component-circular-pilot.manifest.v3.json",
  );
  ...
  return {
    ...
    report: compareCoincidentAxisAndTwoComponentCircularPilotCases(cases),
  };
}

function dominates(left, right) {
  const leftVector = [
    left.configurationMeasures.residuals.axial.maximumReceiverRms,
    left.configurationMeasures.residuals.radial.maximumReceiverRms,
    left.configurationMeasures.residuals.tangential.maximumReceiverRms,
    left.configurationMeasures.exterior.eta_ext,
    left.configurationMeasures.exterior.wakeFluxEta,
  ];
  const rightVector = [
    right.configurationMeasures.residuals.axial.maximumReceiverRms,
    right.configurationMeasures.residuals.radial.maximumReceiverRms,
    right.configurationMeasures.residuals.tangential.maximumReceiverRms,
    right.configurationMeasures.exterior.eta_ext,
    right.configurationMeasures.exterior.wakeFluxEta,
  ];
...
function matchedAccessoryBenefit(cases) {
  const withoutAccessory = cases.find(
    (row) => row.caseId ===
      "counter-rotating-two-component-circular-central-envelope-no-accessory",
  );
  ...
  const measures = [
    ["axialResidualRms",
      (row) => row.configurationMeasures.residuals.axial.maximumReceiverRms],
    ["radialResidualRms",
      (row) => row.configurationMeasures.residuals.radial.maximumReceiverRms],
    ["tangentialResidualRms",
      (row) => row.configurationMeasures.residuals.tangential.maximumReceiverRms],
    ["minimumRootTransversalityMargin",
      (row) => row.configurationMeasures.minimumRootTransversalityMargin],
    ["exteriorEta", (row) => row.configurationMeasures.exterior.eta_ext],
    ["wakeFluxEta", (row) => row.configurationMeasures.exterior.wakeFluxEta],
  ];
  return {
    definition:
...
export function compareCoincidentAxisAndTwoComponentCircularPilotCases(cases) {
  ...
  return {
    schema: "prescribed-path-analysis/coincident-axis-and-two-component-circular-pilot-report.v3",
    claimGrade: "measured",
    ...
    cases: cases.map((row) => ({
      ...
      objectiveVector: {
        rootMargin: row.configurationMeasures.minimumRootTransversalityMargin,
        axialResidualRms: row.configurationMeasures.residuals.axial.maximumReceiverRms,
        radialResidualRms: row.configurationMeasures.residuals.radial.maximumReceiverRms,
        tangentialResidualRms:
          row.configurationMeasures.residuals.tangential.maximumReceiverRms,
        exteriorEta: row.configurationMeasures.exterior.eta_ext,
        wakeFluxEta: row.configurationMeasures.exterior.wakeFluxEta,
      },
    })),
    falsifiers: {
...
export function serializeCoincidentAxisAndTwoComponentCircularPilotReport(report) {
