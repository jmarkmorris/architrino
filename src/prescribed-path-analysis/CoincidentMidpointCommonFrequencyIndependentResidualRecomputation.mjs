...
function phaseFromToken(token) {
  ...
  throw new TypeError(`unsupported coincident-midpoint common-frequency configuration phase token ${token}.`);
}
...
function radiusForBinary(binary, alpha1, alpha3) {
  ...
  throw new TypeError(`unsupported coincident-midpoint common-frequency configuration radius parameter ${binary.radiusParameter}.`);
}
...
export function recomputeCoincidentMidpointCommonFrequencySquaredCausalResidual({
  protocol,
  receiver,
  transmitter,
  alpha1,
  ...
  const receiverBinary = protocol.sourceConfiguration.binaries[receiver.binaryIndex - 1];
  const transmitterBinary = protocol.sourceConfiguration.binaries[transmitter.binaryIndex - 1];
  ...
  return {
    schema: "prescribed-path-analysis/coincident-midpoint-common-frequency-independent-residual-row.v1",
    evaluatorId: "coincident-midpoint-common-frequency-squared-causal-residual-direct-coordinate-recomputation.v1",
