...
export const COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CONFIGURATION_SPEC_SCHEMA =
  "prescribed-path-analysis/coincident-axis-and-two-component-circular-configuration-spec.v3";
export const COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_SOURCE_EMITTER_VERSION =
  "prescribed-path-analysis/coincident-axis-and-two-component-circular-source-emitter/v3";

const TWO_PI = 2 * Math.PI;
const HALF = Math.PI / TWO_PI;
...
export function validateCoincidentAxisAndTwoComponentCircularConfigurationSpec(rawSpec) {
  if (!rawSpec || rawSpec.schema !== COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CONFIGURATION_SPEC_SCHEMA) {
    fail(
      "coincident-axis and two-component circular configuration requires " +
      `schema ${COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_CONFIGURATION_SPEC_SCHEMA}.`,
    );
  }
...
export function createCoincidentAxisAndTwoComponentCircularExactSourceRecord(rawSpec) {
  const spec = validateCoincidentAxisAndTwoComponentCircularConfigurationSpec(rawSpec);
  ...
  const modelRevisionSha256 = sha256Canonical(spec);
  const recordWithoutHash = {
    schema: EXACT_PRESCRIBED_SOURCE_RECORD_SCHEMA,
    assemblyId: `asm-${modelRevisionSha256.slice(
      0,
      modelRevisionSha256.length * HALF,
    )}`,
    modelRevisionSha256,
    ...
    generatingSpec: {
      schema: spec.schema,
      specId: spec.specId,
      specHash: modelRevisionSha256,
    },
    engineId: "prescribed-geometry",
    engineVersion: COINCIDENT_AXIS_AND_TWO_COMPONENT_CIRCULAR_SOURCE_EMITTER_VERSION,
    claimGrade: spec.claimGrade,
    evidenceStatus: spec.evidenceStatus,
    configuration: {
      geometryKind: spec.geometryClass === "six-architrino"
        ? "coincident-axis-three-binary"
        : "two-component-circular",
      configurationId: configurationId(spec),
      status: "prescribed-path-candidate",
      geometryBoundary:
        spec.geometryClass === "six-architrino"
          ? "contains the common-midpoint coincident-axis three-binary locus"
          : "does not require decomposition into two coincident-axis three-binary components",
    },
    parameterVector: {
      coordinateDefinition:
        "coincident-axis-and-two-component-circular-configuration.v3",
      ...
      additionalWorldlineDefinition: spec.geometryClass === "twelve-architrino"
        ? "six declared worldlines form one Accessory Configuration outside the twelve defining two-component circular worldlines"
...
function configurationId(spec) {
  if (spec.geometryClass === "six-architrino") {
    return spec.additionalWorldlines.length === 0
      ? "coincident-axis-three-binary"
      : "coincident-axis-three-binary-plus-three-worldline-scaling-control";
  }
  const senses = spec.architrinoWorldlines.map((row) => row.circulationSense);
  if (senses.every((sense) => sense === senses[0])) {
    return "two-component-circular-co-rotating";
  }
  ...
  if (subsetSenses.length === 2 &&
    ...
    return "two-component-circular-counter-rotating";
  }
  fail(
    "twelve-architrino circulation senses must be uniformly co-rotating or counter-rotating between the two declared index subsets.",
  );
}
