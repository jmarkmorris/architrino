...
const EVENT_SCHEMA = "braid-program/subfield-circular-root-adapter-event.v1";
const ROW_SCHEMA = "braid-program/subfield-circular-root-row.v1";
 ...
 */
export async function runSubfieldCircularPhaseProcess(options) {
  ...
  const receipt = { schema: "braid-program/subfield-circular-phase-process.v1", status: "incomplete",
  ...
  try {
    ...
    heartbeat = setInterval(() => {
      ...
      process.stderr.write(`${JSON.stringify({ schema: "braid-program/subfield-circular-phase-process-heartbeat.v1", pid: child.pid,
