...
export const A11_INTERVAL_PROTOCOL_SCHEMA =
  "prescribed-path-analysis/a1-1-continuous-root-inventory-protocol.v1";
export const A11_INTERVAL_RESULT_SCHEMA =
  "prescribed-path-analysis/a1-1-continuous-root-inventory-result.v1";
export const A11_INTERVAL_SUMMARY_SCHEMA =
  "prescribed-path-analysis/a1-1-continuous-root-inventory-summary.v1";
...
const EXPECTED_BINARY_ROWS = Object.freeze([
  Object.freeze({
    binaryId: "a1-1-binary-1",
    worldlineIds: Object.freeze([
      "a1-1-binary-1-endpoint-1",
      "a1-1-binary-1-endpoint-2",
    ]),
    radiusParameter: "alpha1",
    polarityAssignment: 1,
  }),
  Object.freeze({
    binaryId: "a1-1-binary-2",
    worldlineIds: Object.freeze([
      "a1-1-binary-2-endpoint-1",
      "a1-1-binary-2-endpoint-2",
    ]),
    radiusParameter: "alpha2",
    polarityAssignment: -1,
  }),
  Object.freeze({
    binaryId: "a1-1-binary-3",
    worldlineIds: Object.freeze([
      "a1-1-binary-3-endpoint-1",
      "a1-1-binary-3-endpoint-2",
    ]),
...
export function canonicalA11IntervalJson(value) {
  return JSON.stringify(canonicalize(value));
}

export function sha256A11Interval(value) {
  return createHash("sha256")
    .update(canonicalA11IntervalJson(value))
    .digest("hex");
}
...
function validateSourceFamily(protocol) {
  const sourceFamily = protocol.sourceFamily;
  if (sourceFamily?.taxonomy?.familyId !== "A" ||
      sourceFamily?.taxonomy?.memberId !== "A1.1") {
    throw new TypeError("protocol source family must be A1.1.");
  }
  if (sourceFamily.fieldSpeed !== 1 ||
      sourceFamily.commonAngularFrequency !== 1 ||
      sourceFamily.commonCirculationSense !== 1) {
    throw new TypeError(
      "A1.1 interval inventory requires normalized c_f=1, omega=1, and common circulation +1.",
    );
  }
  exactArray(sourceFamily.groupVelocity, [0, 0, 0], "protocol.sourceFamily.groupVelocity");
  exactArray(sourceFamily.phaseBaseline, EXPECTED_PHASES, "protocol.sourceFamily.phaseBaseline");
  if (sourceFamily.displaySourceProvenance?.path !== EXPECTED_DISPLAY_SOURCE.path ||
      sourceFamily.displaySourceProvenance?.sha256 !== EXPECTED_DISPLAY_SOURCE.sha256) {
    throw new TypeError("protocol source provenance must bind the reviewed A1.1 display source.");
  }
  if (!Array.isArray(sourceFamily.binaries) || sourceFamily.binaries.length !== 3) {
    throw new TypeError("protocol.sourceFamily.binaries must contain three binaries.");
  }
  const worldlineIds = new Set();
  sourceFamily.binaries.forEach((binary, index) => {
...
export function validateA11ContinuousRootInventoryProtocol(rawProtocol) {
  if (!rawProtocol || typeof rawProtocol !== "object" || Array.isArray(rawProtocol)) {
    throw new TypeError("A1.1 interval protocol must be an object.");
  }
  if (rawProtocol.schema !== A11_INTERVAL_PROTOCOL_SCHEMA) {
    throw new TypeError(`A1.1 interval protocol requires schema ${A11_INTERVAL_PROTOCOL_SCHEMA}.`);
  }
  ...
  if (rawProtocol.claimGrade !== "diagnostic" ||
    ...
    throw new TypeError("A1.1 interval protocol must preserve its diagnostic-only claim boundary.");
  }
  validateSourceFamily(rawProtocol);
  ...
  if (policy?.rules !== 9 ||
    ...
    throw new TypeError("A1.1 interval protocol must preserve the frozen nine-rule 36-channel map.");
  }
  for (const field of [
    "rootResidualFloor",
    "rootSeparationFloor",
  ...
  if (partition?.boundaryOwnership?.ratio !==
    ...
    throw new TypeError("A1.1 interval protocol requires explicit partition-boundary ownership.");
  }
  if (rawProtocol.foldSchema?.unresolvedDisposition !== "drawn-not-evaluated" ||
      rawProtocol.foldSchema?.score !== null) {
    throw new TypeError("A1.1 fold schema must fail closed with null score.");
  }
...
function buildWorldlines(protocol) {
  return protocol.sourceFamily.binaries.flatMap((binary) =>
...
export function buildA11OrderedChannelInventory(rawProtocol) {
...
export function evaluateA11ContinuousRootInventory({
  ...
  const resultWithoutHash = {
    ...
    source: {
      ...
      levelIdentity: protocol.sourceFamily.levelIdentity,
      taxonomy: protocol.sourceFamily.taxonomy,
      displaySourceProvenance: protocol.sourceFamily.displaySourceProvenance,
      commonCirculationSense: protocol.sourceFamily.commonCirculationSense,
      phaseBaseline: protocol.sourceFamily.phaseBaseline,
      endpointCount: 6,
    },
    protocolHash: sha256A11Interval(rawProtocol),
    executionProtocolHash: sha256A11Interval(protocol),
...
export function summarizeA11ContinuousRootInventory(result) {
  if (!result || result.schema !== A11_INTERVAL_RESULT_SCHEMA) {
    throw new TypeError(`result must use schema ${A11_INTERVAL_RESULT_SCHEMA}.`);
  }
  ...
  const summaryWithoutHash = {
    schema: A11_INTERVAL_SUMMARY_SCHEMA,
    ...
    unresolvedPartitionLedger: {
      ...
      completeLedgerHash: sha256A11Interval(result.unresolvedPartitions),
