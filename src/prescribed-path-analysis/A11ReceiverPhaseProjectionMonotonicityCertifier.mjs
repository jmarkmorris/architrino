...
export const A11_PROJECTION_MONOTONICITY_PROTOCOL_SCHEMA =
  "prescribed-path-analysis/a1-1-receiver-phase-projection-monotonicity-protocol.v1";
export const A11_PROJECTION_MONOTONICITY_RESULT_SCHEMA =
  "prescribed-path-analysis/a1-1-receiver-phase-projection-monotonicity-result.v1";
export const A11_PROJECTION_MONOTONICITY_SUMMARY_SCHEMA =
  "prescribed-path-analysis/a1-1-receiver-phase-projection-monotonicity-summary.v1";
...
const EXPECTED_STRUCTURAL_RAW_HASH =
  "e28835c1df045bfbbbbc3f419b7a31b1474d0bc51e99381a0bd54e75fa4be4b4";
...
export function validateA11ProjectionMonotonicityProtocol(rawProtocol) {
  if (!rawProtocol || typeof rawProtocol !== "object" ||
      Array.isArray(rawProtocol)) {
    throw new TypeError("A1.1 projection-monotonicity protocol must be an object.");
  }
  if (rawProtocol.schema !== A11_PROJECTION_MONOTONICITY_PROTOCOL_SCHEMA) {
    throw new TypeError(
      `A1.1 projection-monotonicity protocol requires schema ` +
      `${A11_PROJECTION_MONOTONICITY_PROTOCOL_SCHEMA}.`,
    );
  }
  if (rawProtocol.claimGrade !== "diagnostic" ||
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity protocol must remain prescribed-only and null-score.",
    );
  }
  if (rawProtocol.sealedInputs?.baseProtocol?.sha256 !== EXPECTED_BASE_HASH ||
    ...
    throw new TypeError("A1.1 projection-monotonicity sealed input identity drifted.");
  }
  exactArray(
  ...
  if (rawProtocol.relativePhaseLock?.varyingCoordinate !==
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity relative-phase lock drifted.",
    );
  }
  exactArray(
  ...
  if (rawProtocol.certificate?.boxDispositionRule !==
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity certificate identity drifted.",
    );
  }
  ...
  if (analytic?.method !==
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity analytic interval bounds drifted.",
    );
  }
  if (rawProtocol.rootSheetParameterization?.dependentVariable !==
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity root-sheet parameterization drifted.",
    );
  }
  ...
  if (interval?.squaredResidualExclusionFloor !== 1e-10 ||
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity interval resources or floors drifted.",
    );
  }
  if (rawProtocol.controls?.sealed36ChannelAccountingRequired !== true ||
    ...
    throw new TypeError(
      "A1.1 projection-monotonicity fail-closed controls drifted.",
    );
  }
...
function sealedInputControls({
  ...
  const baseHash = sha256A11Interval(baseProtocol);
  const complete36 =
  ...
  return {
    ...
    structuralIndependentResidualAndProvenance: {
      id: "a1-1-projection-sealed-structural-ledger-control.v1",
    ...
    fixedRelativePhases: {
      id: "a1-1-projection-fixed-relative-phase-control.v1",
      expected: EXPECTED_PHASES,
      observed: baseProtocol.sourceFamily?.phaseBaseline,
...
function syntheticControls(protocol) {
  ...
  return {
    id: "a1-1-projection-classifier-positive-negative-controls.v1",
    rows,
...
function endpointInversionControl({
  ...
  return {
    id: "a1-1-projection-endpoint-inversion-control.v1",
    comparisonCount,
...
function phaseSeamControl({
  ...
  return {
    id: "a1-1-projection-emission-phase-seam-control.v1",
    replayCount,
    maximumEnclosureEndpointDifference: maximumDifference,
    tolerance: 1e-12,
    passed: maximumDifference <= 1e-12,
...
function independentWitnessControl({
  ...
  return {
    id: "a1-1-projection-independent-witness-recomputation.v1",
...
function compactRepresentative(row) {
  return {
    ...
    rawLedgerHash: sha256A11Interval(row.ledger),
  };
}
...
export function evaluateA11ReceiverPhaseProjectionMonotonicity({
  ...
  const protocol =
    validateA11ProjectionMonotonicityProtocol(rawProjectionProtocol);
  const baseProtocol =
  ...
  if (maximumBoxesPerRepresentative >
    ...
    throw new RangeError(
      "A1.1 projection execution limits may not exceed declared ceilings.",
    );
  }
  ...
  const resourceExhaustion = {
    id: "a1-1-projection-resource-exhaustion-negative-control.v1",
  ...
  const resultWithoutHash = {
    schema: A11_PROJECTION_MONOTONICITY_RESULT_SCHEMA,
    evaluator: {
      id: "a1-1-receiver-phase-projection-monotonicity-certifier",
      version: 1,
    ...
    projectionProtocolHash: sha256A11Interval(rawProjectionProtocol),
...
export function summarizeA11ReceiverPhaseProjectionMonotonicity(result) {
  if (!result ||
      result.schema !== A11_PROJECTION_MONOTONICITY_RESULT_SCHEMA) {
    throw new TypeError(
      `result must use schema ${A11_PROJECTION_MONOTONICITY_RESULT_SCHEMA}.`,
    );
  }
  const summaryWithoutHash = {
    schema: A11_PROJECTION_MONOTONICITY_SUMMARY_SCHEMA,
