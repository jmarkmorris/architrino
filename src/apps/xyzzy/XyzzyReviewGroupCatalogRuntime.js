...
export function normalizeXyzzyReviewGroupCatalog(rawCatalog) {
  return {
...
export async function loadXyzzyReviewGroupCatalog({
  fetchImpl = globalThis.fetch?.bind(globalThis),
  specUrl,
} = {}) {
  if (typeof fetchImpl !== "function") {
    throw new Error("Xyzzy review group loading requires fetch().");
  }
  const response = await fetchImpl(specUrl, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`Failed to load Xyzzy review groups: ${response.status} ${response.statusText}`);
  }
  const rawCatalog = await response.json();
  return normalizeXyzzyReviewGroupCatalog(rawCatalog);
}
