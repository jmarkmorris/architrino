...
export function normalizeXyzzyDocument(rawDocument = {}) {
  return {
...
export function getXyzzyAssemblyDisplayTileKeys(assembly = {}) {
  return normalizeTileKeys(assembly?.tiles);
}

export function getXyzzyDocumentAssemblyRows(document = {}) {
  const normalizedDocument = normalizeXyzzyDocument(document);
  return normalizedDocument.assemblies.map((assembly) => ({
    id: assembly.id,
    x: assembly.x,
    y: assembly.y,
    role: assembly.role,
    tiles: getXyzzyAssemblyDisplayTileKeys(assembly),
  }));
}

export function validateXyzzyDocumentTilePayload(document = {}, catalog = {}) {
  const errors = [];
  ...
  normalizeXyzzyDocument(document).assemblies.forEach((assembly, assemblyIndex) => {
    const tiles = getXyzzyAssemblyDisplayTileKeys(assembly);
    if (tiles.length !== 4) {
