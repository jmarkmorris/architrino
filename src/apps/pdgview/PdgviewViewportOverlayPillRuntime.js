export function createPdgviewViewportOverlayPillRuntime(options = {}) {
  ...
  let pdgviewPathPointInfoPill = null;
  ...
  function formatPdgviewCoordinatePillValue(value) {
  ...
  function syncPdgviewPathPointInfoPillCoordinateInputs(point, { force = false } = {}) {
    if (!pdgviewPathPointInfoPill?.inputs || !isVector3(point)) {
      return;
    }
    ["x", "y", "z"].forEach((axis) => {
      const input = pdgviewPathPointInfoPill.inputs[axis];
      ...
      input.value = formatPdgviewCoordinatePillValue(point[axis]);
    });
  }

  function commitPdgviewPathPointCoordinateInput(axis, rawValue) {
  ...
  function clearPdgviewSelectedPoint(options = {}) {
  ...
  function ensurePdgviewPathPointInfoPill() {
    const pdgviewViewportOverlays = getViewportOverlays();
    if (!pdgviewViewportOverlays || !documentLike) {
      return null;
    }
    if (pdgviewPathPointInfoPill?.element?.isConnected) {
      return pdgviewPathPointInfoPill;
    }
    const element = documentLike.createElement("div");
    element.className = "pdgview-path-point-pill";
    element.setAttribute("aria-hidden", "true");

    const grid = documentLike.createElement("div");
    grid.className = "pdgview-path-point-pill-grid";
    ...
    ["t", "x", "y", "z"].forEach((key) => {
      ...
      field.className = "pdgview-path-point-pill-field";
      const label = documentLike.createElement("div");
      label.className = "pdgview-path-point-pill-label";
      label.textContent = key.toUpperCase();
      let value = null;
      if (key === "t") {
        value = documentLike.createElement("div");
        value.className = "pdgview-path-point-pill-value";
        value.textContent = "0";
        timeValue = value;
      } else {
        ...
        value.className = "pdgview-path-point-pill-input";
        ...
        value.addEventListener("input", () => {
          commitPdgviewPathPointCoordinateInput(key, value.value);
        });
        value.addEventListener("blur", () => {
          ...
          if (!Number.isFinite(Number(value.value))) {
            value.value = formatPdgviewCoordinatePillValue(point[key]);
            return;
          }
          value.value = formatPdgviewCoordinatePillValue(point[key]);
        });
    ...
    pdgviewViewportOverlays.appendChild(element);
    pdgviewPathPointInfoPill = { element, inputs, timeValue };
    return pdgviewPathPointInfoPill;
  }

  function hidePdgviewPathPointInfoPill() {
    const pill = pdgviewPathPointInfoPill?.element;
    if (!pill) {
      return;
    }
  ...
  function projectPdgviewLocalPointToViewport(localPoint) {
    const pdgviewCanvasWrap = getCanvasWrap();
    const pdgviewCamera = getCamera();
    const pdgviewFrameGroup = getFrameGroup();
    if (!pdgviewCanvasWrap || !pdgviewCamera || !pdgviewFrameGroup || !isVector3(localPoint)) {
      return null;
    }
    const worldPoint = pdgviewFrameGroup.localToWorld(localPoint.clone());
    const projected = worldPoint.project(pdgviewCamera);
    if (projected.z < -1 || projected.z > 1) {
      return null;
    }
    const rect = pdgviewCanvasWrap.getBoundingClientRect();
    return {
  ...
  function getPdgviewPathPointNormalizedTime(pointIndex) {
    const pathState = getPathState();
    if (!Number.isInteger(pointIndex) || pointIndex < 0 || pointIndex >= pathState.points.length) {
      return 0;
    }
  ...
  function updatePdgviewPathPointInfoPill() {
    const pill = ensurePdgviewPathPointInfoPill();
    const pdgviewOverlay = getOverlay();
    const pdgviewCameraFlightState = getCameraFlightState();
    const pdgviewViewportModeState = getViewportModeState();
    if (
      !pill ||
      !pdgviewOverlay?.classList.contains("is-open") ||
      pdgviewCameraFlightState.preview ||
      pdgviewViewportModeState.cameraSource === "authored"
    ) {
      hidePdgviewPathPointInfoPill();
      return;
    }
    ...
    const projected = projectPdgviewLocalPointToViewport(point);
    if (!projected) {
      hidePdgviewPathPointInfoPill();
      return;
    }
    const normalizedT = getPdgviewPathPointNormalizedTime(pointIndex);
    const pdgviewCurrentDocument = getCurrentDocument();
    const absoluteTime = pdgviewCurrentDocument
      ? getPlaybackTimeForMotionProgress(pdgviewCurrentDocument, normalizedT)
      : 0;

    if (pill.timeValue) {
      pill.timeValue.textContent = formatTimeLabel(absoluteTime);
    }
    syncPdgviewPathPointInfoPillCoordinateInputs(point);
