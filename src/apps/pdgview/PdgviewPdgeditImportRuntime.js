...
import {
  ...
  sanitizePdgviewId,
} from "./PdgviewDraftScaffoldRuntime.js";

export const PDGVIEW_PDGEDIT_STAGING_SCHEMA = "pdgview-staging/v1";
export const PDGVIEW_PDGEDIT_PREVIEW_SCHEMA = "pdgview-preview/v1";
export const PDGVIEW_PDGEDIT_EXPORT_SCHEMA = "pdgview-export/v1";
...
function getUniqueEntityId(rawId, fallbackId, usedIds) {
  const baseId = sanitizePdgviewEntityId(rawId, fallbackId);
...
function getPdgviewPositionForPdgeditGridObject(object = {}, bounds = {}) {
...
function buildPdgviewMembersFromPdgeditObject(object = {}) {
...
function buildPdgviewAssemblyDraftsFromPdgeditDocument(pdgeditDocument = {}) {
  ...
  const drafts = objects.map((object, index) => {
    ...
    return {
      ...
      position: getPdgviewPositionForPdgeditGridObject(object, bounds),
      members: buildPdgviewMembersFromPdgeditObject(object),
      metadata: {
...
function buildPdgviewTransfersFromPdgeditDocument({
  pdgeditDocument = {},
  ...
  return links
    .map((link, index) => {
      ...
      const transferId = sanitizePdgviewEntityId(link?.id, `transfer_${index + 1}`);
      return {
        id: transferId,
        source: {
...
function assertAcceptedPdgeditDocument(pdgeditDocument = {}) {
  if (normalizeText(pdgeditDocument?.schema) !== PDGEDIT_DOCUMENT_SCHEMA) {
    throw new Error(`pdgview pdgedit import requires ${PDGEDIT_DOCUMENT_SCHEMA}.`);
  }
}
...
export function createPdgviewPdgeditImportRuntime(options = {}) {
  ...
  function buildPdgviewStagingContractFromPdgeditDocument(pdgeditDocument = {}, runtimeOptions = {}) {
    ...
    return {
      ...
      export: {
        ...
        defaultFileName: `${draftState.id}.pdgview-scene.v1.json`,
        sceneDocument,
      },
    };
  }
  ...
  return {
    buildPdgviewDraftStateFromPdgeditDocument,
    buildPdgviewStagingContractFromPdgeditDocument,
  };
}

export function buildPdgviewStagingContractFromPdgeditDocument(pdgeditDocument = {}, options = {}) {
  return createPdgviewPdgeditImportRuntime(options)
    .buildPdgviewStagingContractFromPdgeditDocument(pdgeditDocument, options);
}
