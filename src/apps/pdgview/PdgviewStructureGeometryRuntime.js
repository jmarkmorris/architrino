export function createPdgviewStructureGeometryRuntime(options = {}) {
  ...
  function resolvePdgviewGraphicTargetContactPosition(
    target,
    overlayCenter,
  ...
  function getPdgviewProxyMemberOffset(memberIndex, memberCount, baseRadius) {
  ...
  function clearPdgviewMemberAnchors() {
    memberAnchors = new Map();
    return memberAnchors;
  }

  function setPdgviewMemberAnchor(assemblyId, memberId, anchor) {
  ...
  function getPdgviewOrbitBasis(motion) {
  ...
  function getPdgviewOrbitOffsetAtTime(motion, chargeType, timeSeconds) {
  ...
  function resolvePdgviewMemberAnchorPosition(anchor, assemblyCenter, timeSeconds) {
    ...
    if (anchor.type === "orbit" && anchor.motion?.type === "orbit.circular") {
      return assemblyCenter
        .clone()
        .add(getPdgviewOrbitOffsetAtTime(anchor.motion, anchor.chargeType, timeSeconds));
    }
    return assemblyCenter.clone();
  }

  function resolvePdgviewTransferEndpointPosition(endpoint, assemblyCenters, timeSeconds) {
    ...
    if (memberId) {
      ...
      const memberPosition = resolvePdgviewMemberAnchorPosition(anchor, assemblyCenter, timeSeconds);
      if (memberPosition) {
  ...
  function findPdgviewCoreMemberId(members, chargeType, binaryIndex) {
  ...
  function getPdgviewPersonalityRingRadius(assembly) {
  ...
  function getPdgviewAssemblyWorldCenterById(assemblyId) {
    if (!assemblyId) {
      return new THREE.Vector3();
    }
  ...
  function shiftPdgviewPointTriplets(points, delta) {
  ...
  function rebasePdgviewAssemblyParentFrame(assembly, nextParentId = "") {
    if (!assembly?.id) {
      return;
    }
    const previousParentCenter = getPdgviewAssemblyWorldCenterById(assembly.parentId);
    const nextParentCenter = getPdgviewAssemblyWorldCenterById(nextParentId);
    ...
    updateAssemblyDraftByIdState(assembly.id, (currentAssembly) => ({
      ...
      pathPoints: shiftPdgviewPointTriplets(currentAssembly.pathPoints, delta),
    }));
  }

  function computePdgviewAssemblyBasePosition(assembly, index, count, pathById) {
  ...
  function samplePdgviewPointAt(points, normalizedT, options = {}) {
  ...
  function samplePdgviewCurvePoints(points, segments = 80) {
  ...
  function getPdgviewViewportAutoscaleTargetSpheres(documentData, assemblyCenters, framingState) {
    const assemblies = Array.isArray(documentData?.assemblies) ? documentData.assemblies : [];
  ...
  function getPdgviewAutoscaledCameraState(cameraState, documentData, assemblyCenters, framingState) {
    const camera = getCamera();
    ...
    const autoscaleMode = String(framingState?.framing?.autoscale ?? "")
      .trim()
      .toLowerCase();
    const targetSpheres = getPdgviewViewportAutoscaleTargetSpheres(
      documentData,
      assemblyCenters,
      framingState
    );
  ...
  return {
    ...
    resolvePdgviewMemberAnchorPosition,
    ...
    getPdgviewViewportAutoscaleTargetSpheres,
