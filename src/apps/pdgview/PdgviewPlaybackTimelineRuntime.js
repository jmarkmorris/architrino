export function createPdgviewPlaybackTimelineRuntime(options = {}) {
  ...
  function clearPdgviewEditorPreviewState() {
  ...
  function setPdgviewViewportCameraSource(source = "design") {
  ...
  function getPdgviewSortedMarkers(documentData) {
  ...
  function syncPdgviewMarkerNavigation(documentData, timeSeconds) {
    const markers = getPdgviewSortedMarkers(documentData);
  ...
  function renderPdgviewTimeline(documentData) {
  ...
  function updatePdgviewTimelinePlayhead(timeSeconds, documentData) {
    if (!documentData) {
      return;
    }
  ...
  function setPdgviewPlaybackPlayhead(timeSeconds, runtimeOptions = {}) {
    ...
    updatePdgviewTimelinePlayhead(playbackState.playheadSeconds, documentData);
  }

  function startPdgviewPlayback(timeSeconds, runtimeOptions = {}) {
    const documentData = runtimeOptions.documentData ?? getCurrentDocument();
    if (!documentData) {
      return;
    }
  ...
  function togglePdgviewPlayback() {
    ...
    if (playbackState.playing) {
      setPdgviewPlaybackPlayhead(playbackState.playheadSeconds, { documentData, playing: false });
      return;
    }
  ...
  function restartPdgviewPlayback() {
    const documentData = getCurrentDocument();
    if (!documentData) {
      return;
    }
    const timeWindow = getSceneTimeWindow(documentData);
    setPdgviewPlaybackPlayhead(timeWindow.start, { documentData, playing: false });
  }

  function jumpToPdgviewMarker(markerId, runtimeOptions = {}) {
    const documentData = getCurrentDocument();
    if (!documentData || !markerId) {
      return;
    }
    const marker = getPdgviewSortedMarkers(documentData).find((entry) => entry.id === markerId);
    if (!marker) {
      return;
    }
    setPdgviewPlaybackPlayhead(marker.t, { documentData, playing: runtimeOptions.playing });
  }

  function jumpPdgviewMarkerByOffset(direction) {
    const documentData = getCurrentDocument();
    if (!documentData) {
      return;
    }
    ...
    jumpToPdgviewMarker(target.id, { playing: false });
  }

  function scrubPdgviewPlayback(fraction, runtimeOptions = {}) {
    const documentData = getCurrentDocument();
    if (!documentData) {
      return;
    }
    ...
    setPdgviewPlaybackPlayhead(nextTime, { documentData, playing: runtimeOptions.playing });
  }

  function updatePdgviewPlaybackState(now) {
    const documentData = getCurrentDocument();
    const playbackState = getPlaybackState();
