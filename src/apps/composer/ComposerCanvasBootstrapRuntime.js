export function createComposerCanvasBootstrapRuntime(options = {}) {
  ...
  function initComposerCanvas() {
    if (!dom.composerCanvas || getRenderer()) {
      return;
    }
    const renderer = new THREE.WebGLRenderer({
      canvas: dom.composerCanvas,
      antialias: true,
      alpha: true,
    });
    ...
    wireCanvasUiListeners({
      composerCanvas: dom.composerCanvas,
      onComposerPointerDown: operations.onPointerDown,
      onComposerPointerMove: operations.onPointerMove,
      onComposerPointerUp: operations.onPointerUp,
      onComposerWheel: operations.onWheel,
      onComposerContextMenu: operations.onContextMenu,
    });
    ...
    if (dom.overlay && !dom.overlay.dataset.assemblyMenuWired) {
      ...
      dom.overlay.addEventListener(
        ...
        (event) => {
          ...
          if (event.target === dom.composerCanvas && event.button === 2) {
            return;
          }
          operations.closeAssemblyMenu?.();
        },
