...
export function sanitizeComposerId(raw) {
  if (!raw) {
...
export function sanitizeComposerEntityId(raw, fallback = "item_1") {
  if (!raw) {
    return fallback;
  }
  const cleaned = String(raw)
...
export function createComposerDefaultPathPoints(anchor = [0, 0, 0]) {
...
export function createComposerDefaultCoreSpec(assemblyId, options = {}) {
...
export function createDefaultComposerAssemblyDraft(index = 0) {
