import {
  getComboXyzzyAssemblyRecipe,
  getComboXyzzyOperatorRecipe,
  normalizeComboXyzzyRecipeCatalog,
} from "./ComboXyzzyRecipeCatalogRuntime.js";
...
function getUnitEndpointIds(unit, portId, catalog) {
  ...
  const recipe = getComboXyzzyAssemblyRecipe(unit.recipeId, catalog);
  if (!recipe) {
    throw new Error(`Unknown Combo Xyzzy assembly recipe: ${unit.recipeId}`);
  }
...
function buildOperatorRecords(graph, catalog) {
  ...
  return graph.units
    ...
    .map((unit) => {
      const recipe = getComboXyzzyOperatorRecipe(unit.recipeId, catalog);
      if (!recipe) {
        throw new Error(`Unknown Combo Xyzzy operator recipe: ${unit.recipeId}`);
      }
...
export function buildXyzzyDocumentFromComboPublicationGraph(graph = {}, recipeCatalog = null) {
  const normalizedGraph = normalizePublicationGraph(graph);
  const normalizedCatalog = normalizeComboXyzzyRecipeCatalog(recipeCatalog ?? undefined);
  return {
    schema: "xyzzy/v1",
...
export function buildComboXyzzyPackage({
  ...
  const xyzzyDocument = buildXyzzyDocumentFromComboPublicationGraph(graph, recipeCatalog);
