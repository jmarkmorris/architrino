...
function allRecordsUseAdmittedAssemblies(records = []) {
  return (Array.isArray(records) ? records : []).every((record) => isAdmittedAssemblyId(record?.assemblyId));
}
...
function buildPrimitiveImbalanceResult(request = {}) {
  ...
  const family = makeFamily({
    ...
    score: {
      ...
      dissociationCount: 0,
      ambiguityPenalty: 1,
      tieBreakKey: "none|none|lane2:pass-thru,pass-thru,pass-thru|lane4:none|lane3:pro_down_quark,pro_down_quark,pro_up_quark|rho:primitive_imbalance",
    },
    laneInventories: {
      lane1: countOccurrences(lane1),
      lane3: countOccurrences(lane3),
...
export function classifyPdgsolveRequestScenario(request = {}) {
  ...
  if (
    ...
    reactantKey &&
    allRecordsUseAdmittedAssemblies(normalizedRequest.reactants) &&
    allRecordsUseAdmittedAssemblies(normalizedRequest.products)
  ) {
