import { borgCircularOrbit, sameBorgCircularOrbit, describeBorgOrbitSharing } from "./BorgOrbitGeometry.mjs";
...
function sharedArc(path, peers) {
  ...
  return { mode: "shared-arc", duration: Math.min(...lags)/path.omega, fade: true,
    reason: "Co-rotating members share this source circle: each trail ends at the preceding member's phase." };
}
...
export function describeBorgOrbitTrails(dataset) {
  ...
  const dedicated = new Set(describeBorgOrbitSharing(dataset).groups.filter((group) => group.length === 1).flat());
  ...
  return new Map(rows.map(({line,source}, index) => {
    ...
    if (path) {
      ...
      if (!peers.length && sources.length===rows.length && circles.every(Boolean)) {
        ...
        policy = sharedArc(path, peers.map(j => circles[j]));
      }
    } else if (op?.kind === "f6c-harmonic-member.v1" && op.phase?.modulationAmplitude===0 && Number.isFinite(op.phase.rate) && Math.abs(op.phase.rate)>0 && dedicated.has(line.id)) {
    ...
    } else if (op?.kind === "f5-phase-varying-member.v1" && Number.isFinite(op.resultantAngularFrequency) && op.resultantAngularFrequency>0 && dedicated.has(line.id)) {
