...
import {
  createBorgArchitrinoPointTexture as createArchitrinoPointTexture,
  createBorgParticleStyles,
  getBorgParticleStyle as getParticleStyle,
} from "./BorgRuntimeVisuals.js";
import {
  calculateBorgOrthographicFrustum,
  clampBorgValue as clamp,
  formatBorgNumber as formatNumber,
  formatBorgTimelineTime,
  getBorgBufferedPlaybackAdvance,
  getBorgPlaybackReanchor,
  getBorgTimelineRangePresentation,
  getBorgVectorLength as vectorLength,
  getBorgVelocityRayLength,
  interpolateBorgFrameSetInto,
} from "./BorgRuntimeTimeline.js";
import {
  createBorgLiveRunBudgetMeasurement as createLiveRunBudgetMeasurement,
  createEmptyBorgLiveRunBudget as createEmptyLiveRunBudget,
  readBorgLiveRunBudgetSnapshot as readLiveRunBudgetSnapshot,
} from "./BorgRuntimeRunBudget.js";
import {
  appendBorgRadioChoice,
  formatBorgDiagnosticPercentagePoints as formatDiagnosticPercentagePoints,
  formatBorgDiagnosticPercent as formatDiagnosticPercent,
  formatBorgFieldValue as formatValue,
  getBorgRadioGroupValue,
  isBorgEditableTarget as isEditableTarget,
  queryRequiredBorgElement as queryRequiredElement,
  setBorgRadioGroupValue,
  setBorgStatusTone as setTone,
} from "./BorgRuntimePresentation.js";
import {
  createBorgDeploymentFieldRows,
  createBorgDiagnosticFieldRows,
  createBorgEnvelopeFieldRows,
  createBorgInitialConditionFieldRows,
  createBorgSourceFieldRows,
} from "./BorgRuntimePanelRows.js";
import {
  DEFAULT_RUN_CONTROL_PRESET_ID,
  RUN_CONTROL_PRESETS,
  boundedBorgControlInteger as boundedInteger,
  createDefaultBorgEomRecordReplayOptions as createDefaultEomRecordReplayOptions,
  createDefaultEomShadowRunnerOptions,
  formatBorgRunDurationLabel as formatRunDurationLabel,
  formatBorgRunTargetDuration as formatRunTargetDuration,
  isBorgForeverRunPreset as isForeverRunPreset,
  normalizeBorgEomRunGrade as normalizeEomRunGrade,
  positiveBorgControlNumber as positiveControlNumber,
} from "./BorgRuntimeRunSession.js";
...
export function mountBorgApp(options = {}) {
  ...
  const dom = {
    ...
    recordSummary: documentLike.querySelector?.("#borg-record-summary") ?? null,
    recordSummaryTitle: documentLike.querySelector?.("#borg-record-summary-title") ?? null,
    ...
    obstructionScope: documentLike.querySelector?.("#borg-obstruction-scope") ?? null,
    obstructionLedger: documentLike.querySelector?.("#borg-obstruction-ledger") ?? null,
    ...
    replayPlatonicRelationships: queryRequiredElement(documentLike, "#borg-platonic-relationships"),
  ...
  let frameSetByIndex = new Map(
    frameSets.map((frameSet) => [frameSet.frameIndex, frameSet]),
  );
  let frameSetPositionByIndex = new Map(
    frameSets.map((frameSet, index) => [frameSet.frameIndex, index]),
  );
  let frameRowsByFrameIndex = new Map(
    frameSets.map((frameSet) => [
      frameSet.frameIndex,
      new Map(frameSet.frames.map((frame) => [frame.pathKey, frame])),
    ]),
  );
  ...
  function renderDiagnosticsPanel() {
    ...
    renderFieldRows(
      dom.authorityFields,
      [
        ["frameAuthority", replayActive
          ? activeReplayEntry?.dataset.provenance.engineId === "prescribed-geometry"
  ...
  function updatePolarityDiagnostics() {
    const rawFrameSet = frameSetByIndex.get(state.activeFrameIndex) ??
      frameSets.at(-1);
    if (!rawFrameSet) {
      state.polarityDiagnostics = null;
      state.polarityDiagnosticFrameIndex = null;
  ...
  function renderDeploymentFields() {
    renderFieldRows(dom.deploymentFields, createBorgDeploymentFieldRows({
      manifest,
      state,
      retentionPolicy: BORG_LIVE_RUN_RETENTION_POLICY_V1,
    }));
  }
  ...
  function renderSourceFields() {
    ...
    renderFieldRows(dom.sourceFields, createBorgSourceFieldRows({
      state,
      manifest,
      activePreset,
      placement,
      isEomSimulationActive: isEomSimulationActive(),
      formatRunDurationLabel,
      formatRunTargetDuration,
      formatRealtimeRate: formatBorgRealtimeRate,
  ...
  function renderEnvelopeFields() {
    ...
    renderFieldRows(dom.envelopeFields, createBorgEnvelopeFieldRows({
      manifest,
      coupling: state.eomCoupling,
      envelopeRadius: activeEnvelopeRadius(),
      sampleInterval: activeSampleInterval(),
      populationCount: runtimePopulationCount,
  ...
  function renderInitialConditionFields() {
    ...
    renderFieldRows(dom.initialConditionFields, createBorgInitialConditionFieldRows({
      state,
      manifest,
      certifiedBudget,
      placement,
  ...
  function renderDiagnosticFields() {
    ...
    renderFieldRows(dom.diagnosticsFields, createBorgDiagnosticFieldRows({
      diagnostics,
      manifest,
      formatPercent: formatDiagnosticPercent,
      formatPercentagePoints: formatDiagnosticPercentagePoints,
    }));
  }
  ...
  function renderFieldRows(container, rows) {
    ...
    rows.forEach(([label, value]) => {
      ...
      renderInlineMathText(valueElement, formatValue(value, formatNumber), {
        documentLike,
        windowLike,
      });
  ...
  function configureStartingGeometryControl() {
    ...
    listen(dom.startingGeometry, "change", (event) => {
  ...
  function configureAssemblyViewControls() {
    ...
    assemblyViewControls = createBorgAssemblyViewControls({
      ...
      dom: {
        ...
        platonicRelationships: dom.replayPlatonicRelationships,
  ...
  function renderRecordSummary() {
    if (!dom.recordSummary) {
      return;
    }
    dom.recordSummary.hidden = !replayActive;
    if (!replayActive || !activeReplayEntry) {
      return;
    }
    const rawRecord = activeReplayEntry.rawRecord;
    const supplied = options.eomRecordReplay?.librarySummary;
    const exactSummary = supplied?.assemblyId === rawRecord.assemblyId &&
      supplied?.modelRevisionSha256 === rawRecord.modelRevisionSha256
      ? supplied
      : null;
    const prescribedGeometry = activeReplayEntry.dataset.provenance?.prescribedGeometry;
    const obstruction = prescribedGeometry?.coordinates?.geometry?.prescribedBalanceObstruction;
    dom.recordSummaryTitle.textContent = exactSummary?.label ??
      rawRecord.title ??
      prescribedGeometry?.coordinates?.identity?.displayLabel ??
      "Selected assembly";
    dom.recordSummaryIdentity.textContent = rawRecord.assemblyId;
    dom.recordSummaryDescription.textContent = `Recorded description: ${
      exactSummary?.description ?? prescribedGeometry?.description ?? "Sealed assembly record."
    }`;
    if (dom.obstructionBadge && dom.obstructionDetail) {
      const visible = obstruction?.disposition === "excluded-prescribed-balance";
      dom.obstructionBadge.hidden = !visible;
      dom.obstructionDetail.hidden = !visible;
      if (visible) {
        dom.obstructionReadout.textContent = `${obstruction.receiverLabel} receiver: required x acceleration = ${obstruction.requiredXAcceleration}; certified x acceleration is ${obstruction.certifiedXLowerBound} to ${obstruction.certifiedXUpperBound}.`;
        dom.obstructionScope.textContent = `Scope: ${obstruction.parameterDomain}. Amber marks the certified mismatch; polarity remains red and blue.`;
        dom.obstructionLedger.textContent = `Root ledger: ${obstruction.rootLedger}`;
      }
    }
    dom.recordSummaryFacets.textContent = "";
    (exactSummary?.facets ?? []).forEach((facet) => {
      const chip = documentLike.createElement("span");
      chip.textContent = `${facet.label}: ${facet.value}`;
      dom.recordSummaryFacets.append(chip);
    });
  }
  ...
  async function switchStartingGeometry(geometryId) {
    ...
    try {
      ...
      setInitialConditionFeedback(
        `Prescribed geometry could not be loaded: ${dom.prescribedBranchFeedback.value}`,
        "bad",
      );
      setBorgRadioGroupValue(dom.startingGeometry, activeStartingGeometryId);
    } finally {
  ...
  function captureSimulationWorkspace() {
    return Object.freeze({
      ...
      state: createBorgWorkspaceSnapshotState(state),
  ...
  function restoreSimulationWorkspace(geometryId, snapshot) {
    ...
    rebuildFrameSetLookups();
    Object.assign(
      state,
      restoreBorgWorkspaceState(state, snapshot.state),
    );
    resetPolarityDiagnosticsHistory();
    assemblyViewScene.setRecord(activeReplayEntry);
  ...
  function enterPrescribedReplay(geometryId, session) {
    ...
    renderStaticPanels();
    describePrescribedAnalysisProvider();
    const firstReplayChunk = startDynamicRunner();
  ...
  function applyFrameSetAtCurrentTime() {
    const frameSet = frameSetByIndex.get(state.activeFrameIndex);
    if (frameSet) {
  ...
  function updateEomControlPresentation() {
    ...
    const displayStatus = ({
      ...
      "completed-eom-run": "complete",
      "eom-shadow-stopped": "stopped",
      "eom-run-error": "failed",
  ...
  function bindEvents() {
    ...
    listen(dom.playButton, "click", () => {
      togglePlayback();
    });
    listen(dom.startButton, "click", goToStartFrame);
    listen(dom.newDistributionButton, "click", () => {
      startNewDistributionRun({ advanceSeed: true, autoStart: false });
    });
    listen(dom.prescribedBranchStart, "click", () => {
      startPrescribedDisplayBranch();
    });
    listen(dom.initialConditionForm, "submit", (event) => {
      event.preventDefault();
    });
    listen(dom.initialConditionForm, "input", () => {
      markInitialConditionControlsPending();
      scheduleInitialConditionReset();
    });
    listen(dom.initialConditionForm, "change", applyInitialConditionResetNow);
    listen(dom.eomRunGrade, "change", () => {
      markInitialConditionControlsPending();
      applyInitialConditionResetNow();
    });
    listen(dom.certifiedBudget, "change", () => {
    ...
    listen(dom.runDurationButton, "change", () => {
    ...
    listen(dom.historyDuration, "input", applyPathTrailDuration);
    listen(dom.historyDuration, "change", applyPathTrailDuration);
    listen(dom.resetButton, "click", resetView);
    listen(dom.fitButton, "click", fitView);
    listen(dom.focusButton, "click", focusSelected);
    ...
    listen(windowLike, "keydown", handleKeyDown);
  }

  function listen(target, type, handler, options) {
    target.addEventListener(type, handler, options);
    boundEventListeners.push(() =>
      target.removeEventListener?.(type, handler, options));
  }

  function rebuildFrameSetLookups() {
    frameSetByIndex = new Map(
      frameSets.map((frameSet) => [frameSet.frameIndex, frameSet]),
    );
    frameSetPositionByIndex = new Map(
      frameSets.map((frameSet, index) => [frameSet.frameIndex, index]),
    );
    frameRowsByFrameIndex = new Map(
      frameSets.map((frameSet) => [
        frameSet.frameIndex,
        new Map(frameSet.frames.map((frame) => [frame.pathKey, frame])),
      ]),
  ...
  function updateFrame(frameIndex) {
    const frameSet = frameSetByIndex.get(frameIndex) ?? frameSets.at(-1);
    if (!frameSet) {
      return;
    }
    applyFrameSet(frameSet, {
  ...
  function stepPlayback(now) {
    ...
    const msPerFrameInterval = getPlaybackMsPerFrameInterval();
    const rawProgress = state.playbackSegmentProgress +
      (now - state.playbackSegmentStartedAt) / msPerFrameInterval;
    ...
    if (rawProgress >= 1) {
      ...
      if (nextFromSetIndex >= frameSets.length - 1) {
        if (canExtendDynamicRun()) {
          ...
          if (newestSetIndex > state.playbackFromSetIndex) {
            ...
            applyFrameSet(newestFrameSet, {
              outputLabel: formatActiveTimelineLabel(newestFrameSet.time),
              rangeValue: newestFrameSet.frameIndex,
            });
            state.playbackFromSetIndex = newestSetIndex;
            state.playbackToSetIndex = newestSetIndex;
          }
          state.playbackSegmentStartedAt = null;
          state.playbackSegmentProgress = 0;
          ensureDynamicFramesAhead();
        ...
        applyFrameSet(frameSets.at(-1), {
          outputLabel: formatActiveTimelineLabel(frameSets.at(-1).time),
    ...
    const displayFrameSet = interpolateBorgFrameSetInto(
      fromFrameSet,
      toFrameSet,
      progress,
      interpolatedFrameSetScratch,
      frameRowsByFrameIndex.get(toFrameSet.frameIndex),
    );
    const currentFrameIndex = progress < 0.5 ? fromFrameSet.frameIndex : toFrameSet.frameIndex;
    applyFrameSet(displayFrameSet, {
      outputLabel: formatActiveTimelineLabel(displayFrameSet.time),
      rangeValue: currentFrameIndex,
    });
    if (progress >= 1) {
      applyFrameSet(toFrameSet, {
        outputLabel: formatActiveTimelineLabel(toFrameSet.time),
  ...
  function updateSelectedTag() {
    ...
    dom.selectedTag.textContent = `${style.polarity} ${state.selectedPathKey} | speed ${formatNumber(speed)} | ${keyframeCount} frame sets`;
  }
  ...
  function dispose() {
    ...
    disposeBorgWorkspaceSnapshotRunners(simulationWorkspaceSnapshots, {
  ...
  function activeFrameTime() {
    return frameSetByIndex.get(state.activeFrameIndex)?.time ??
  ...
  function getFrameSetIndex(frameIndex) {
    return frameSetPositionByIndex.get(frameIndex) ?? 0;
  }
  ...
  function getPlaybackMsPerFrameInterval() {
  ...
  function startDynamicRunner() {
    ...
    if (!runnerOptions) {
      state.dynamicRunnerStatus = "eom-run-error";
  ...
  function startRunAndPlayback() {
    ...
    setPlayButtonPresentation(true);
    const generation = state.dynamicRunGeneration;
    const firstChunk = startDynamicRunner();
    if (!firstChunk) {
      startPlayback();
      if (frameSets.length < 2) {
        stopPlayback();
  ...
  function ensureDynamicFramesAhead({
    ...
    state.dynamicChunkPromise = state.dynamicRunner
      ...
      .then((chunk) => {
        ...
        const compactedFrameSets = eomSimulation
        ...
        syncRunDurationButton();
        if (!replayActive &&
            (replaceCurrentFrames || state.liveRunRetention?.compactedThisPass)) {
      ...
      .catch((error) => {
        ...
        state.dynamicRunnerMessage = error?.message ?? (
          eomSimulation ? "EOM runner failed" : "record replay failed"
        );
  ...
  function rebuildPathTrails({ recreateMaterials = false } = {}) {
    ...
    const activeFrameSet = frameSetByIndex.get(state.activeFrameIndex);
    pathTrails.setVisibleWindow({
  ...
  function resetDynamicRunState() {
    ...
    state.dynamicRunnerMessage = activeEomRunnerOptions
      ? activePrescribedDisplayBranch
        ...
        : "exact polynomial initial history ready; forward EOM evolution pending"
      : "no EOM data source is configured";
