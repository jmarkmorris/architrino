...
Try `Assembly radii: Iso-radii` for the equal-radius prescribed examples, including the circular-path and co-spherical configurations. `Braids in assembly: 2` plus `Hetero-radii` returns the qualifying two-braid records; combining two braids with spindle shape narrows that set further. The braid-count selector offers 1, 2, and 3; three currently returns no records.
...
- [BorgLibraryFindings.mjs](BorgLibraryFindings.mjs): validates the versioned [active-finding relation index](../../../../reference/priorities/app-borg/library-finding-relations.v1.json) and links only by allowed source-declared structure or exact configuration identity. The query counts finding-bearing configurations once, not finding rows.
...
This provider covers the 144 prescribed records in the [configuration display catalog](../../../../reference/priorities/braid-program/configuration-display-catalog.md), organized as 45 current braid entries, not the entire scientific taxonomy. The initial relation index links 114 exact configurations across 15 entries to active source-owned findings; `No active findings indexed yet` is not evidence that no finding exists. A full record-byte hash pins a selection and is verified again when opening it in Borg; it is not a semantic model identity or causal-state identity. Changed records are rejected, not silently substituted.

Facet version `borg-record-facets.v11` counts persistent worldlines and source-declared component braids, computes assembly span and per-component braid dimensionality from complete retained cubic paths, assigns circle occupancy only from complete supported circular carriers, derives assembly-radius equality from source formulas, carries the separately versioned source-derived variant-set descriptor, and attaches the separately versioned finding relations. Iso-radii compares every architrino about the one source-declared whole-assembly center throughout the record window; Hetero-radii has an unequal-time-slice witness. Common breathing can be Iso-radii. Scientific classifications and finding relations remain source-bound, and missing information remains unassigned or unindexed rather than false.
