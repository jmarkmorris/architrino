...
export function createReactionAnchorStateRuntime(options = {}) {
  const nodeKeysConflictFn =
    typeof options.nodeKeysConflict === "function"
      ? options.nodeKeysConflict
  ...
  function isSingleMappingRole(role, nodeKey, anchorInstanceIndex = null) {
    const descriptor = { role, nodeKey, anchorInstanceIndex };
    const structuredResult = isSingleMappingAnchorRole(descriptor);
    if (structuredResult === true) {
      return true;
    }
    if (structuredResult === false) {
      return isSingleMappingAnchorRole(role, nodeKey, anchorInstanceIndex) === true;
    }
    return Boolean(structuredResult);
  }

  function canTargetRole(role, nodeKey, anchorInstanceIndex = null) {
    const descriptor = { role, nodeKey, anchorInstanceIndex };
    const structuredResult = canTargetMappingRole(descriptor);
    if (structuredResult === true) {
      return true;
    }
    if (structuredResult === false) {
      return canTargetMappingRole(role, nodeKey, anchorInstanceIndex) === true;
    }
    return Boolean(structuredResult);
  }
  ...
  function getConflictingMappings(nodeKey, role, anchorInstanceIndex = null) {
    return getMappings().filter((mapping) => {
      ...
      return isSingleMappingRole(role, nodeKey, anchorInstanceIndex) && mappedKey
  ...
  function getAnchorAvailability(role, nodeKey, anchorInstanceIndex = null) {
    ...
    if (existingMappings.length && isSingleMappingRole(role, nodeKey, anchorInstanceIndex)) {
      return { disabled: false, reason: "" };
    }
    const hasConflict =
      isSingleMappingRole(role, nodeKey, anchorInstanceIndex) &&
    ...
    if (canTargetRole(role, nodeKey, anchorInstanceIndex) && pendingSourceKey) {
