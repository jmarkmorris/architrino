...
import {
  createReactionCanvasDocumentRuntime as defaultCreateCanvasDocumentRuntime,
} from "./ReactionCanvasDocumentRuntime.js";
import {
  addReactionLedgers,
  createEmptyReactionLedger,
  formatReactionLedger,
  getReactionLedgerSignature,
  hasReactionLedger,
...
import {
  createReactionOperatorLedgerRuntime as defaultCreateOperatorLedgerRuntime,
} from "./ReactionOperatorLedgerRuntime.js";
...
export function createReactionCanvasUiRuntime(deps = {}) {
  const {
    ...
    createOperatorLedgerRuntime = defaultCreateOperatorLedgerRuntime,
  ...
  let syncOperatorFan = () => {};
  const canvasDocumentRuntime = createCanvasDocumentRuntime({
    state,
    cloneSerializableValue,
    clearDragState,
    clearAllRecentRouteState: (...args) => clearAllRecentRouteState(...args),
    closeMenu,
    render,
  ...
  const operatorLedgerRuntime = createOperatorLedgerRuntime({
    findParticipantById,
    getMappings: () => state.mappings,
    getNodeLedger,
    parseNodeKey,
    resolveOperatorOutputLedger: getOperatorOutputLedgerForAnchor,
  });
  const {
    getOperatorLedgerSummary,
  } = operatorLedgerRuntime;
  ...
  function getLedgerFromBinaryAssignment(assignment = {}, nodes = []) {
    return nodes.reduce(
      (ledger, node) => addReactionLedgers(ledger, getBinaryChoiceInventory(assignment?.[node?.id])),
      createEmptyReactionLedger()
    );
  }

  function getOperatorOutputLedgerForAnchor(participantOrId, operatorSummary = null, anchorInstanceIndex = null) {
    const baseLedger = hasReactionLedger(operatorSummary?.outputLedger)
  ...
  function cycleFreeArchitrinoPreset(participantId, nodeId) {
    ...
    validAssignments.forEach((assignment) => {
      const signature = getReactionLedgerSignature(getLedgerFromBinaryAssignment(assignment, nodes));
    ...
    const currentLedgerSignature = getReactionLedgerSignature(
  ...
  function syncHeaderActionButtons() {
    ...
    if (solveButton instanceof HTMLButtonElement) {
      const canSolve = state.active && !state.isSolving && typeof solveSnapshot === "function";
  ...
  function getParticipantSurfaceRowIndex(participant, fallbackIndex = 0) {
    return normalizeSurfaceRowStartIndex(
      participant?.surfaceRowIndex ?? (isOperatorParticipant(participant) ? participant?.operatorSlotIndex : null),
      getParticipantSurfaceRowSpan(participant),
      fallbackIndex
    );
  }
