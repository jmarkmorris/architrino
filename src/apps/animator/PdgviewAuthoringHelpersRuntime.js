...
export function formatPdgviewTransferList(transfers = []) {
  return transfers
    .map((transfer) => {
...
export function formatPdgviewTransferEndpointLabel(endpoint) {
...
export function describePdgviewTransferProvenance(transfer, refLabel = "") {
  if (!transfer) {
    return null;
  }
  const prefix = refLabel ? `${refLabel}: ` : "";
  return `${prefix}${formatPdgviewTransferEndpointLabel(transfer.source)} -> ${formatPdgviewTransferEndpointLabel(transfer.target)}`;
}

export function sanitizePdgviewGraphicTarget(rawTarget, fallbackAssemblyId = "") {
  ...
  if (type === "assembly") {
    const assemblyId = sanitizePdgviewEntityId(rawTarget.assemblyId, "");
...
export function createPdgviewPersonalityMembers(states = []) {
...
export function createPdgviewGenIFermionPersonalityMembers() {
  return createPdgviewPersonalityMembers();
}

export function getPdgviewBuiltInPersonalityStates(templateId) {
