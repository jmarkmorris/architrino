...
function wrapPdgviewOverlayText(context, text, maxWidth) {
...
export function createPdgviewRenderAssetsRuntime(options = {}) {
  const THREE = options.THREE;
  const documentLike = options.documentLike ?? globalThis.document;
  if (!THREE) {
    throw new Error("createPdgviewRenderAssetsRuntime requires THREE");
  }

  function createPdgviewLozengeTexture(text, options = {}) {
    const {
      isActive = false,
  ...
  function createPdgviewPointLabelTexture(text, isActive = false) {
  ...
  function createPdgviewMemberLabelTexture(text, color = "#ffd894") {
  ...
  function createPdgviewGraphicOverlayTextTexture(text, radius = 0.42) {
    const canvas = documentLike.createElement("canvas");
    canvas.width = 320;
    canvas.height = 256;
    const context = canvas.getContext("2d");
    if (context) {
      ...
      context.textBaseline = "middle";
      const padding = 42;
      const lines = wrapPdgviewOverlayText(context, text, canvas.width - padding * 2);
      const lineHeight = 34;
  ...
  function createPdgviewGraphicOverlayTextSprite(text, radius = 0.42) {
    const { texture, scale } = createPdgviewGraphicOverlayTextTexture(text, radius);
  ...
  function updatePdgviewGraphicOverlayTextSprite(sprite, text, radius = 0.42) {
    if (!sprite?.material) {
      return;
    }
  ...
  function updatePdgviewPointLabelSprite(sprite, text, isActive = false) {
    if (!sprite) {
      return;
    }
    const previousMap = sprite.material?.map ?? null;
    const nextTexture = createPdgviewPointLabelTexture(text, isActive);
    sprite.material.map = nextTexture;
    sprite.material.needsUpdate = true;
    previousMap?.dispose?.();
  }

  function createPdgviewPointLabelSprite(text) {
    const material = new THREE.SpriteMaterial({
      map: createPdgviewPointLabelTexture(text, false),
      transparent: true,
      depthTest: false,
      depthWrite: false,
    });
  ...
  function createPdgviewCameraWaypointLabelTexture(text, isActive = false) {
    const canvas = documentLike.createElement("canvas");
    canvas.width = 96;
    canvas.height = 96;
    const context = canvas.getContext("2d");
  ...
  function updatePdgviewCameraWaypointLabelSprite(sprite, text, isActive = false) {
    if (!sprite) {
      return;
    }
    const previousMap = sprite.material?.map ?? null;
    const nextTexture = createPdgviewCameraWaypointLabelTexture(text, isActive);
    sprite.material.map = nextTexture;
    sprite.material.needsUpdate = true;
    previousMap?.dispose?.();
  }

  function createPdgviewCameraWaypointLabelSprite(text) {
    const material = new THREE.SpriteMaterial({
      map: createPdgviewCameraWaypointLabelTexture(text, false),
      transparent: true,
      depthTest: false,
      depthWrite: false,
    });
  ...
  function createPdgviewMemberLabelSprite(text, color = "#ffd894") {
    const material = new THREE.SpriteMaterial({
      map: createPdgviewMemberLabelTexture(text, color),
      transparent: true,
      depthTest: false,
      depthWrite: false,
    });
  ...
  function createPdgviewAssemblyBadgeTexture(title, subtitle = "") {
    const canvas = documentLike.createElement("canvas");
    canvas.width = 176;
    canvas.height = 88;
    const context = canvas.getContext("2d");
  ...
  function createPdgviewAssemblyBadgeSprite(title, subtitle = "") {
    const material = new THREE.SpriteMaterial({
      map: createPdgviewAssemblyBadgeTexture(title, subtitle),
      transparent: true,
      depthTest: false,
      depthWrite: false,
    });
  ...
  function createPdgviewChildBadgeSprite(title, subtitle = "") {
    const sprite = createPdgviewAssemblyBadgeSprite(title, subtitle);
