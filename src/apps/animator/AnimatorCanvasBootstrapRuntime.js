export function createAnimatorCanvasBootstrapRuntime(options = {}) {
  ...
  function initAnimatorCanvas() {
    if (!dom.animatorCanvas || getRenderer()) {
      return;
    }
    const renderer = new THREE.WebGLRenderer({
      canvas: dom.animatorCanvas,
      antialias: true,
      alpha: true,
    });
    ...
    wireCanvasUiListeners({
      animatorCanvas: dom.animatorCanvas,
      onAnimatorPointerDown: operations.onPointerDown,
      onAnimatorPointerMove: operations.onPointerMove,
      onAnimatorPointerUp: operations.onPointerUp,
      onAnimatorWheel: operations.onWheel,
      onAnimatorContextMenu: operations.onContextMenu,
    });
    ...
    if (dom.overlay && !dom.overlay.dataset.assemblyMenuWired) {
      ...
      dom.overlay.addEventListener(
        ...
        (event) => {
          ...
          if (event.target === dom.animatorCanvas && event.button === 2) {
            return;
          }
          operations.closeAssemblyMenu?.();
        },
