...
export function sanitizePdgviewId(raw) {
  if (!raw) {
    return "pdgview_scene";
  }
  const cleaned = String(raw)
    ...
    .replace(/\s+/g, "_")
    .replace(/[^a-z0-9_-]/g, "");
  return cleaned || "pdgview_scene";
}

export function sanitizePdgviewEntityId(raw, fallback = "item_1") {
  if (!raw) {
    return fallback;
  }
  const cleaned = String(raw)
...
export function createPdgviewDefaultPathPoints(anchor = [0, 0, 0]) {
...
export function createPdgviewDefaultCoreSpec(assemblyId, options = {}) {
...
export function createDefaultPdgviewAssemblyDraft(index = 0) {
  ...
  return {
    ...
    members:
      ordinal === 1
        ...
        : [],
    pathPoints: ordinal === 1 ? createPdgviewDefaultPathPoints(defaultPosition) : [],
    pathInterpolate: "spline",
    pathClosed: false,
    historyTraceEnabled: false,
    envelopeEnabled: false,
    core: ordinal === 1 ? createPdgviewDefaultCoreSpec(assemblyId) : undefined,
  };
}
