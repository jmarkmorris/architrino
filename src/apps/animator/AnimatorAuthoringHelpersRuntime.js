...
export function formatAnimatorTransferList(transfers = []) {
  return transfers
    .map((transfer) => {
...
export function formatAnimatorTransferEndpointLabel(endpoint) {
...
export function describeAnimatorTransferProvenance(transfer, refLabel = "") {
  if (!transfer) {
    return null;
  }
  const prefix = refLabel ? `${refLabel}: ` : "";
  return `${prefix}${formatAnimatorTransferEndpointLabel(transfer.source)} -> ${formatAnimatorTransferEndpointLabel(transfer.target)}`;
}

export function sanitizeAnimatorGraphicTarget(rawTarget, fallbackAssemblyId = "") {
  ...
  if (type === "assembly") {
    const assemblyId = sanitizeAnimatorEntityId(rawTarget.assemblyId, "");
...
export function createAnimatorPersonalityMembers(states = []) {
...
export function createAnimatorGenIFermionPersonalityMembers() {
  return createAnimatorPersonalityMembers();
}

export function getAnimatorBuiltInPersonalityStates(templateId) {
