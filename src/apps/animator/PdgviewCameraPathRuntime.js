export function createPdgviewCameraPathRuntime(options = {}) {
  ...
  function setPdgviewFrameDefaults() {
  ...
  function getPdgviewOrbitTargetWorld() {
  ...
  function updatePdgviewOrbitFromPosition(position) {
    const orbitState = getCameraOrbitState();
    const target = getPdgviewOrbitTargetWorld();
  ...
  function syncPdgviewCameraRadiusInput() {
  ...
  function setPdgviewCameraDefaults() {
    ...
    updatePdgviewOrbitFromPosition(cameraState?.position ?? getCameraState().position);
    syncPdgviewCameraRadiusInput();
  }

  function updatePdgviewWaypointCount() {
  ...
  function updatePdgviewCameraWaypointMaterials(activeIndex = null) {
  ...
  function updatePdgviewCameraPoiStatus() {
  ...
  function applyPdgviewCameraRadiusInput() {
    if (!dom.cameraRadiusInput) {
      return;
    }
    const orbitState = getCameraOrbitState();
  ...
  function addPdgviewCameraWaypoint(position = null) {
    const frameGroup = getFrameGroup();
    ...
    const localPos = Array.isArray(position)
      ...
      : position instanceof THREE.Vector3
        ...
        : camera
          ...
          : new THREE.Vector3();
    const localLookAt = getPdgviewPoiLocal();
  ...
  function clearPdgviewCameraWaypoints() {
    const cameraFlightState = getCameraFlightState();
  ...
  function resetPdgviewPathPoints() {
  ...
  function addPdgviewPathPoint(position = null, options = {}) {
    const selectedAssembly = getSelectedAssembly();
    const pathState = getPathState();
    if (!selectedAssembly) {
      return;
  ...
  function updatePdgviewPointMaterials(activeIndex = null) {
  ...
  function updatePdgviewPathMarkerScales() {
    const camera = getCamera();
  ...
  function samplePdgviewCameraWaypointState(waypoints, normalizedT) {
  ...
  function getPdgviewCameraWaypointDisplayPosition(waypoint) {
  ...
  function startPdgviewCameraFlightPreview() {
    const cameraFlightState = getCameraFlightState();
  ...
  function stopPdgviewCameraFlightPreview() {
    const cameraFlightState = getCameraFlightState();
    if (!cameraFlightState.preview) {
      return;
    }
    ...
    if (camera) {
      ...
      updatePdgviewOrbitFromPosition(camera.position);
      syncPdgviewCameraRadiusInput();
      updateCamera();
    }
    if (dom.cameraFlightToggle) {
