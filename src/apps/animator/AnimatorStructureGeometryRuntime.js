export function createAnimatorStructureGeometryRuntime(options = {}) {
  ...
  function resolveAnimatorGraphicTargetContactPosition(
    target,
    overlayCenter,
  ...
  function getAnimatorProxyMemberOffset(memberIndex, memberCount, baseRadius) {
  ...
  function clearAnimatorMemberAnchors() {
    memberAnchors = new Map();
    return memberAnchors;
  }

  function setAnimatorMemberAnchor(assemblyId, memberId, anchor) {
  ...
  function getAnimatorOrbitBasis(motion) {
  ...
  function getAnimatorOrbitOffsetAtTime(motion, chargeType, timeSeconds) {
  ...
  function resolveAnimatorMemberAnchorPosition(anchor, assemblyCenter, timeSeconds) {
    ...
    if (anchor.type === "orbit" && anchor.motion?.type === "orbit.circular") {
      return assemblyCenter
        .clone()
        .add(getAnimatorOrbitOffsetAtTime(anchor.motion, anchor.chargeType, timeSeconds));
    }
    return assemblyCenter.clone();
  }

  function resolveAnimatorTransferEndpointPosition(endpoint, assemblyCenters, timeSeconds) {
    ...
    if (memberId) {
      ...
      const memberPosition = resolveAnimatorMemberAnchorPosition(anchor, assemblyCenter, timeSeconds);
      if (memberPosition) {
  ...
  function findAnimatorCoreMemberId(members, chargeType, binaryIndex) {
  ...
  function getAnimatorPersonalityRingRadius(assembly) {
  ...
  function getAnimatorCameraPlaneBasisInFrame() {
  ...
  function getAnimatorPersonalitySlotLocalOffset(assembly, slotIndex) {
    const radius = getAnimatorPersonalityRingRadius(assembly);
    const angle = Math.max(0, Number(slotIndex) || 0) * (Math.PI / 3);
    const { right, up } = getAnimatorCameraPlaneBasisInFrame();
    return right
      .clone()
      .multiplyScalar(Math.cos(angle) * radius)
  ...
  function getAnimatorAssemblyWorldCenterById(assemblyId) {
    if (!assemblyId) {
      return new THREE.Vector3();
    }
  ...
  function shiftAnimatorPointTriplets(points, delta) {
  ...
  function rebaseAnimatorAssemblyParentFrame(assembly, nextParentId = "") {
    if (!assembly?.id) {
      return;
    }
    const previousParentCenter = getAnimatorAssemblyWorldCenterById(assembly.parentId);
    const nextParentCenter = getAnimatorAssemblyWorldCenterById(nextParentId);
    ...
    updateAssemblyDraftByIdState(assembly.id, (currentAssembly) => ({
      ...
      pathPoints: shiftAnimatorPointTriplets(currentAssembly.pathPoints, delta),
    }));
  }

  function computeAnimatorAssemblyBasePosition(assembly, index, count, pathById) {
  ...
  function sampleAnimatorPointAt(points, normalizedT, options = {}) {
  ...
  function sampleAnimatorCurvePoints(points, segments = 80) {
  ...
  function getAnimatorViewportAutoscaleTargetSpheres(documentData, assemblyCenters, framingState) {
    const assemblies = Array.isArray(documentData?.assemblies) ? documentData.assemblies : [];
  ...
  function getAnimatorAutoscaledCameraState(cameraState, documentData, assemblyCenters, framingState) {
    const camera = getCamera();
    ...
    const autoscaleMode = String(framingState?.framing?.autoscale ?? "")
      .trim()
      .toLowerCase();
    const targetSpheres = getAnimatorViewportAutoscaleTargetSpheres(
      documentData,
      assemblyCenters,
      framingState
    );
  ...
  return {
    ...
    resolveAnimatorMemberAnchorPosition,
    resolveAnimatorTransferEndpointPosition,
    findAnimatorCoreMemberId,
    getAnimatorPersonalityRingRadius,
    getAnimatorCameraPlaneBasisInFrame,
    ...
    getAnimatorViewportAutoscaleTargetSpheres,
