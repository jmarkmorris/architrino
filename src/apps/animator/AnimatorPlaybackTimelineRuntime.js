...
export function createAnimatorPlaybackTimelineRuntime(options = {}) {
  ...
  function clearAnimatorEditorPreviewState() {
  ...
  function syncAnimatorMarkerNavigation(documentData, timeSeconds) {
    const markers = getAnimatorSortedMarkers(documentData);
  ...
  function renderAnimatorTimeline(documentData) {
  ...
  function updateAnimatorTimelinePlayhead(timeSeconds, documentData) {
    if (!documentData) {
      return;
    }
  ...
  function setAnimatorPlaybackPlayhead(timeSeconds, runtimeOptions = {}) {
    ...
    updateAnimatorTimelinePlayhead(playbackState.playheadSeconds, documentData);
  }

  function startAnimatorPlayback(timeSeconds, runtimeOptions = {}) {
    const documentData = runtimeOptions.documentData ?? getCurrentDocument();
    if (!documentData) {
      return;
    }
  ...
  function toggleAnimatorPlayback() {
    ...
    if (playbackState.playing) {
      setAnimatorPlaybackPlayhead(playbackState.playheadSeconds, { documentData, playing: false });
      return;
    }
  ...
  function restartAnimatorPlayback() {
    const documentData = getCurrentDocument();
    if (!documentData) {
      return;
    }
    const timeWindow = getSceneTimeWindow(documentData);
    setAnimatorPlaybackPlayhead(timeWindow.start, { documentData, playing: false });
  }

  function jumpToAnimatorMarker(markerId, runtimeOptions = {}) {
    const documentData = getCurrentDocument();
    if (!documentData || !markerId) {
      return;
    }
    const marker = getAnimatorSortedMarkers(documentData).find((entry) => entry.id === markerId);
    if (!marker) {
      return;
    }
    setAnimatorPlaybackPlayhead(marker.t, { documentData, playing: runtimeOptions.playing });
  }

  function jumpAnimatorMarkerByOffset(direction) {
    const documentData = getCurrentDocument();
    if (!documentData) {
      return;
    }
    ...
    jumpToAnimatorMarker(target.id, { playing: false });
  }

  function scrubAnimatorPlayback(fraction, runtimeOptions = {}) {
    const documentData = getCurrentDocument();
    if (!documentData) {
      return;
    }
    ...
    setAnimatorPlaybackPlayhead(nextTime, { documentData, playing: runtimeOptions.playing });
  }

  function updateAnimatorPlaybackState(now) {
    const documentData = getCurrentDocument();
    const playbackState = getPlaybackState();
