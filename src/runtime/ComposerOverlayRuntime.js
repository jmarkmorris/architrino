export function getComposerGraphicDefaultTarget({
  selectedAssemblyId = "",
  fallbackAssemblyId = "",
...
export function encodeComposerGraphicTargetValue(target) {
  if (!target?.type) {
    return "";
  }
...
export function decodeComposerGraphicTargetValue(rawValue, { sanitizeTarget } = {}) {
...
export function getComposerGraphicTargetEntries({
  assemblyDrafts = [],
  getAssemblyLetter,
...
export function sanitizeComposerMediaSource(rawSource, kind = "image", { supportedExtensions = {} } = {}) {
...
export function normalizeComposerGraphicOverlayDraft(
  ...
  if (kind === "image" || kind === "video" || kind === "audio") {
    const source = sanitizeComposerMediaSource(overlay?.source, kind, {
      supportedExtensions,
    });
    const fallbackDirectory = mediaAssetDirectories[kind] ?? "";
    const rect = normalizeComposerMediaRect(overlay?.rect, kind, { clampFn });
...
export function normalizeComposerGraphicOverlayList(overlays = [], duration = 24, options = {}) {
  return (Array.isArray(overlays) ? overlays : []).map((overlay, index) =>
    normalizeComposerGraphicOverlayDraft(overlay, index, duration, options)
  );
}

export function getComposerGraphicOverlayDraftIndexById(overlayDrafts = [], overlayId) {
  return overlayDrafts.findIndex((overlay) => overlay?.id === overlayId);
}

export function getNextComposerGraphicOverlayId(overlayDrafts = []) {
  let suffix = 1;
...
export function getComposerGraphicTimelineOverlays(documentData) {
  return (Array.isArray(documentData?.overlays) ? documentData.overlays : []).filter(
    (overlay) => overlay?.kind === "graphic"
  );
}

export function getComposerViewportMediaTimelineOverlays(documentData) {
  return (Array.isArray(documentData?.overlays) ? documentData.overlays : []).filter(
