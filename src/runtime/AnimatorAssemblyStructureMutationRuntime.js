import { buildAnimatorAssemblyStructure } from "./AnimatorAssemblyStructureBridgeRuntime.js";
...
function getCanonicalSubassemblyMemberIds(root, assemblyId, subassemblyId) {
  ...
  const targetNode = children.find(
    (node) => node?.id === targetNodeId && node?.species === "animator_subassembly"
  );
  if (!targetNode) {
    return [];
  }
...
export function splitAnimatorAssemblyGroup(assembly = null, subassemblyId = "") {
  if (!assembly?.id) {
    return null;
  }
