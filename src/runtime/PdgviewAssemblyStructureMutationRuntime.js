import { buildPdgviewAssemblyStructure } from "./PdgviewAssemblyStructureBridgeRuntime.js";
...
function getCanonicalSubassemblyMemberIds(root, assemblyId, subassemblyId) {
  ...
  const targetNode = children.find(
    (node) => node?.id === targetNodeId && node?.species === "pdgview_subassembly"
  );
  if (!targetNode) {
    return [];
  }
...
export function splitPdgviewAssemblyGroup(assembly = null, subassemblyId = "") {
  if (!assembly?.id) {
    return null;
  }
