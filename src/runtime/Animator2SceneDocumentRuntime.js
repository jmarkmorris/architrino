...
function normalizeAnimatorPaths(rawPaths, rawAssemblies) {
...
function normalizeReactions(rawReactions, transfers, start, end) {
  ...
  return rawReactions
    .map((reaction, index) => {
      const minimumSpan = Math.min(animatorMinimumTimelineItemDurationSeconds, Math.max(0, end - start));
...
const animatorMinimumTimelineItemDurationSeconds = 2;
...
function normalizePauses(rawPauses, start, end) {
  ...
  return rawPauses
    .map((pause, index) => {
      ...
      const minimumDuration = Math.min(animatorMinimumTimelineItemDurationSeconds, maxDuration);
...
export function normalizeAnimatorSceneDocument(rawDocument = {}) {
  ...
  return {
    ...
    scene: {
      id: normalizeString(rawScene.id, "animator_scene"),
      type: rawScene.type ?? "Scene-Composed-Animation",
      kind: rawScene.kind ?? "composed_animation",
      name: normalizeString(rawScene.name, "animator scene"),
...
export function createAnimatorSceneDocument(input = {}, options = {}) {
  ...
  const authoredProvenance = transferList
    ...
    .filter(Boolean);
  return normalizeAnimatorSceneDocument({
    schemaVersion: "0.1.0",
    scene: {
...
export function buildAnimatorPreviewSceneData(document, options = {}) {
  const normalized = normalizeAnimatorSceneDocument(document);
...
import { normalizeAnimatorCameraShots } from "./AnimatorViewportFramingRuntime.js";
