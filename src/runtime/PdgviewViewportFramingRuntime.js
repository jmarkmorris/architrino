...
export function normalizePdgviewViewportFraming(rawFraming) {
...
export function resolvePdgviewShotInterval(shot, timeWindow) {
...
export function normalizePdgviewCameraShots(rawCameraShots, cameraPathId, start, end) {
  ...
  return source.map((shot, index) => ({
    ...
    framing: normalizePdgviewViewportFraming(shot?.framing),
  }));
}

export function getPdgviewActiveCameraShot(documentData, timeSeconds, timeWindow) {
  ...
  return (
    shots.find((shot) => {
      const interval = resolvePdgviewShotInterval(shot, resolvedWindow);
      return timeSeconds >= interval.start && timeSeconds <= interval.end;
    }) ?? null
  );
}

export function getPdgviewActiveCameraPathId(documentData, timeSeconds, timeWindow) {
...
export function getPdgviewAssemblyViewportPolicy(assembly) {
  const viewport =
...
export function resolvePdgviewViewportFramingState(documentData, timeSeconds, timeWindow) {
  const assemblies = Array.isArray(documentData?.assemblies) ? documentData.assemblies : [];
  const activeShot = getPdgviewActiveCameraShot(documentData, timeSeconds, timeWindow);
  const framing = normalizePdgviewViewportFraming(activeShot?.framing);
  ...
  assemblies.forEach((assembly) => {
    ...
    if (getPdgviewAssemblyViewportPolicy(assembly) === "required") {
  ...
  return {
    shot: activeShot,
    cameraPathId: getPdgviewActiveCameraPathId(documentData, timeSeconds, timeWindow),
    framing,
...
export function getPdgviewViewportAutoscaleTargetIds(state, allAssemblyIds = []) {
...
export function computePdgviewViewportAutoscaleCameraState({
  cameraState,
  targetSpheres,
...
export function formatPdgviewViewportFramingSummary(state) {
