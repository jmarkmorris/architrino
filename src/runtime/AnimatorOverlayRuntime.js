export function getAnimatorGraphicDefaultTarget({
  selectedAssemblyId = "",
  fallbackAssemblyId = "",
...
export function encodeAnimatorGraphicTargetValue(target) {
  if (!target?.type) {
    return "";
  }
...
export function decodeAnimatorGraphicTargetValue(rawValue, { sanitizeTarget } = {}) {
...
export function getAnimatorGraphicTargetEntries({
  assemblyDrafts = [],
  getAssemblyLetter,
  ...
  assemblyDrafts.forEach((assembly, index) => {
    ...
    entries.push({
      value: encodeAnimatorGraphicTargetValue({
        type: "assembly",
...
export function getAnimatorGraphicDefaultOffset(size = 0.42) {
...
export function sanitizeAnimatorMediaSource(rawSource, kind = "image", { supportedExtensions = {} } = {}) {
...
export function normalizeAnimatorGraphicOverlayDraft(
  ...
  const kind = getAnimatorOverlayKind(overlay);
  ...
  if (kind === "image" || kind === "video" || kind === "audio") {
    const source = sanitizeAnimatorMediaSource(overlay?.source, kind, {
      supportedExtensions,
    });
    const fallbackDirectory = mediaAssetDirectories[kind] ?? "";
    const rect = normalizeAnimatorMediaRect(overlay?.rect, kind, { clampFn });
    const label = getAnimatorMediaOverlayLabel({
      ...overlay,
      kind,
  ...
  const offsetSource =
    Array.isArray(overlay?.offset) && overlay.offset.length >= 3
      ? overlay.offset
      : getAnimatorGraphicDefaultOffset(size);
  const label =
...
export function normalizeAnimatorGraphicOverlayList(overlays = [], duration = 24, options = {}) {
  return (Array.isArray(overlays) ? overlays : []).map((overlay, index) =>
    normalizeAnimatorGraphicOverlayDraft(overlay, index, duration, options)
  );
}

export function getAnimatorGraphicOverlayDraftIndexById(overlayDrafts = [], overlayId) {
  return overlayDrafts.findIndex((overlay) => overlay?.id === overlayId);
}

export function getNextAnimatorGraphicOverlayId(overlayDrafts = []) {
  let suffix = 1;
...
export function getAnimatorGraphicTimelineOverlays(documentData) {
  return (Array.isArray(documentData?.overlays) ? documentData.overlays : []).filter(
    (overlay) => overlay?.kind === "graphic"
  );
}

export function getAnimatorViewportMediaTimelineOverlays(documentData) {
  return (Array.isArray(documentData?.overlays) ? documentData.overlays : []).filter(
