export function openAnimatorMemberMenu(config) {
  const {
    menu,
    clientX,
    clientY,
  ...
  appendMenuNote(
    menu,
    "Internal particle structure is read-only in the animator. Provenance-changing internal edits are not available here."
  );
...
export function openAnimatorPersonalitySlotMenu(config) {
  const {
    menu,
    clientX,
    clientY,
  ...
  appendMenuNote(
    menu,
    currentState === "unset"
      ? "Unset polar site. The animator now treats axial-site assignment as read-only."
      : currentState === "electrino"
        ? "Electrino assigned. Axial-site assignment is read-only in the animator."
        : "Positrino assigned. Axial-site assignment is read-only in the animator."
  );
  appendMenuNote(
    menu,
    "Provenance-changing internal edits are not available in the animator."
  );
...
export function openAnimatorSubassemblyMenu(config) {
  const {
    menu,
    clientX,
    clientY,
  ...
  appendMenuNote(
    menu,
    "Group splitting and regrouping are not available in the animator. This menu is inspection-only."
  );
...
export function openAnimatorAssemblyTemplateMenu(config) {
  const {
    menu,
    event,
    localPoint,
  ...
  addLabel.className = "animator-assembly-menu-subtitle";
  ...
  const [frameEditButton] = appendMenuButtonRow(menu, [
    {
      text: animatorFrameEditModeRef.get() ? "Stop Edit" : "Edit",
      onClick: () => {
        const nextValue = !animatorFrameEditModeRef.get();
        animatorFrameEditModeRef.set(nextValue);
        if (frameEditButton) {
    ...
    },
    {
      ...
      onClick: () => {
        ...
        if (frameScaleControl) {
          ...
          frameScaleControl.output.textContent = formatScaleLabel(animatorFrameState.scale);
        }
        renderJsonPreview();
      },
    },
  ...
  frameScaleControl = appendMenuRangeControl(menu, {
    ...
    max: 3,
    step: 0.1,
    value: Math.log10(animatorFrameState.scale || 1).toFixed(2),
    valueLabel: formatScaleLabel(animatorFrameState.scale),
    onInput: (nextValue) => {
      animatorFrameState.scale = Math.pow(10, nextValue);
      updateAnimatorFrame();
      renderJsonPreview();
      return formatScaleLabel(animatorFrameState.scale);
    },
  });
  ...
  appendMenuButtonRow(menu, [
    {
      ...
      onClick: () => {
        ...
        if (orbitSpeedControl) {
          ...
          orbitSpeedControl.output.textContent = formatScaleLabel(animatorCameraState.speed);
        }
        if (orbitRadiusControl) {
          orbitRadiusControl.input.value = Math.log10(animatorCameraOrbitState.radius || 1).toFixed(2);
          orbitRadiusControl.output.textContent = formatScaleLabel(animatorCameraOrbitState.radius);
        }
        renderJsonPreview();
      },
    },
  ...
  orbitSpeedControl = appendMenuRangeControl(menu, {
    ...
    max: 2,
    step: 0.1,
    value: Math.log10(animatorCameraState.speed || 1).toFixed(2),
    valueLabel: formatScaleLabel(animatorCameraState.speed),
    onInput: (nextValue) => {
      animatorCameraState.speed = Math.pow(10, nextValue);
      renderJsonPreview();
      return formatScaleLabel(animatorCameraState.speed);
    },
  });
  ...
  orbitRadiusControl = appendMenuRangeControl(menu, {
    ...
    max: 3,
    step: 0.1,
    value: Math.log10(animatorCameraOrbitState.radius || 1).toFixed(2),
    valueLabel: formatScaleLabel(animatorCameraOrbitState.radius),
    onInput: (nextValue) => {
      animatorCameraOrbitState.radius = Math.pow(10, nextValue);
      updateAnimatorCamera();
      renderJsonPreview();
      return formatScaleLabel(animatorCameraOrbitState.radius);
    },
  });
...
export function openAnimatorAssemblyPropertiesMenu(config) {
  const {
    menu,
    clientX,
    clientY,
  ...
  appendMenuNote(
    menu,
    "Internal particle structure is now treated as read-only in the animator. Provenance-changing internal edits are not available here."
  );

  const structureActions = document.createElement("div");
  structureActions.className = "animator-button-row";
  ...
  if (pendingTransferSource?.assemblyId) {
    ...
    appendMenuNote(
      menu,
      `A previous canvas transfer draft from ${pendingTransferSource.assemblyId}.${pendingTransferSource.memberId} is still present. New transfer drafting is being retired from the animator.`
    );
  ...
  removeButton.className = "animator-assembly-menu-danger";
...
export function openAnimatorPathPointMenu(config) {
  const {
    menu,
    clientX,
    clientY,
