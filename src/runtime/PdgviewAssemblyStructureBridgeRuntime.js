          ...
          ? [
              {
                ...
                traits: { slot: slotName, variant: "pdgview_bridge" },
                children: [
                  ...(hasBinary
...
export function buildPdgviewAssemblyStructure(assembly = null) {
  if (!assembly?.id) {
...
export function summarizePdgviewAssemblyStructure(root = null, validation = null) {
  if (!root) {
    return {
      nodeCount: 0,
      maxDepth: 0,
...
export function formatPdgviewAssemblyStructureSummary(summary = {}) {
...
export function formatPdgviewAssemblyStructureStatus(summary = {}, assembly = null) {
  const slotCount = Math.max(0, Number(summary?.slotCount ?? 0) || 0);
  const binarySlotCount = Math.max(0, Number(summary?.binarySlotCount ?? 0) || 0);
