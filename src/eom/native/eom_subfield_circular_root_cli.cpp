...
constexpr std::string_view kOutputRoot = ".local-data/braid-analysis/subfield-circular-root-pilot-20260827-v1";
...
constexpr std::array<std::string_view, 16> kCandidates{
 ...
 "coincident-center-two-component-circular-co-rotating","coincident-center-two-component-circular-counter-rotating","coaxial-separated-two-component-circular-co-rotating","coaxial-separated-two-component-circular-counter-rotating","coaxial-separated-two-planar-braid-co-rotating","coaxial-separated-two-planar-braid-counter-rotating"
};
...
class ExclusiveOutput {
 ...
 ExclusiveOutput(const fs::path& requested, const fs::path& repository) {
   ...
   if (!within(parent, admitted) || !within(admitted, repository))
     throw std::runtime_error("output must be under the ignored sub-field circular pilot lane");
...
struct Progress {
 ...
 void emit_locked(std::string_view event, const std::string& detail = "") const {
   Object out; out.string("schema", "braid-program/subfield-circular-root-adapter-event.v1");
...
void usage() {
 std::cout << "Prescribed sub-field circular carrier; no evolution or H3 acceptance.\n"
...
void check_conformance(const std::string& bytes, const Options& o, const std::string& manifest_hash,
 ...
 if (proof.get<std::string>("schema") != "braid-program/subfield-circular-history-conformance.v1" ||
...
int run(const Options& o) {
 ...
 try {
   ...
   const auto id = "subfield-circular-history/v1:" + o.candidate + ":T=" + reception;
   ...
   for (;;) {
     ...
     Object row; row.string("schema", "braid-program/subfield-circular-root-row.v1");
