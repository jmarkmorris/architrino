...
- geometry centralization inventory for Photon, Ideal Swarm, Animator, and the assembly-dynamics reference engine, with the removed `sim2` prototype and non-app legacy solver families kept out of migration scope;
...
- ordered migration parity reporting for Animator, Photon, then Ideal Swarm before app-local solver code is simplified.
...
`scripts/check-solver-geometry-inventory.mjs` writes `.tmp/solver-geometry-inventory/solver-geometry-inventory.json`. It verifies the known app-local geometry and solver surfaces for Photon, Ideal Swarm, Animator, and the assembly-dynamics reference engine, and records the central-solver ownership target for each surface. The inventory keeps the removed `sim2` prototype and non-app legacy solver families outside migration scope.

`scripts/check-solver-migration-parity.mjs` reads `.tmp/solver-baseline-sandbox/manifest.json`, verifies artifact hashes and the baseline-classification vocabulary, enforces the migration order Animator, Photon, then Ideal Swarm, and writes `.tmp/solver-migration-parity/solver-migration-parity-report.json`. The report includes aggregate app/case readiness counts plus explicit missing and blocking case lists, and it must stay green before app adapter migration or app-local solver simplification proceeds.
