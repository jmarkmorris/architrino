...
`solveMovingCircularSourceCausalRootsF64`, `solveMovingCircularSameSourceCausalRootsF64`, and `computeMovingCircularObserverFieldF64` expose the first bridge-level Photon absolute-history moving-circular facade. These methods run through the shared app bridge and worker protocol, return stable `solver-moving-circular-source-causal-roots-f64.v1`, `solver-moving-circular-same-source-causal-roots-f64.v1`, and `solver-moving-circular-observer-field-f64.v1` schemas, preserve source phase, receiver phase for same-source roots, source-normal Jacobian, receiver-normal branch factor, residual, scan diagnostics, and assemble receiver-normal observer-field branch sums from retained roots. They are JavaScript-backed migration facades rather than native C ABI rows; the later native/WASM migration can keep the method names and schemas while replacing the execution backend.
