...
`scripts/check-solver-geometry-inventory.mjs` writes `.tmp/solver-geometry-inventory/solver-geometry-inventory.json`. It verifies the known app bridge geometry and solver surfaces for Photon, Ideal Braid, and Animator, and records the zombie-solver ownership target for each surface.
...
`solver_analytic_smoke.cpp` adds a manufactured moving-source case with a closed-form causal root. It verifies the root time, delay, distance, residual, Jacobian, branch weight, receiver-normal factor, delayed-hit strength, unit direction, moving-circular source velocity, source phase-at-hit, and observer-field contribution row, so the compiled zombie-solver is checked against an analytic target in addition to app-facing baseline fixtures.
