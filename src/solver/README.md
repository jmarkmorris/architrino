...
API versions, schema versions, binary layout catalog, build metadata,
toolchain versions, build-host runtime capability probes, and packaging policy. It intentionally excludes
...
- app-runtime package manifest creation and check for the ES-module WebAssembly loader,
  WebAssembly binary, app bridge modules, worker bridge module, worker runtime
  module, adapters, declarations, schema, schema versions, binary layout
...
- geometry centralization inventory for Photon, Ideal Braid, and Animator app bridge surfaces;
...
- app-facing run manifests with model, envelope, error-budget, precision metadata, output, admission, provenance, buffer, stream, diagnostic, and status metadata;
...
- first app-facing request adapters for Photon causal-root and phase-diagnostics runs, path-history runs, Ideal Braid delayed-hit and shared-geometry runs, Animator motion runs, Animator playback runs, generic shared-geometry runs, validation-replay runs, path-history stream/readback requests, storage-lifecycle requests, path-history work-packet plans, and emission-shell query/packet/merge requests;
...
- app-facing path-history stream metadata for precision path, numeric type, numeric chart, stream value authority, app-buffer authority, claim level, units, coordinate frame, scale normalization, interpolation rule, provenance, diagnostics, and per-chunk checksums;
...
- high-offset causal-root solving with returned distance, residual, source-normal Jacobian, receiver-normal branch weight, and receiver-normal diagnostics computed from the high-precision root evaluation state;
- deterministic causal-root solving over linear source and receiver path segments;
- native invariant checks for finite root and delayed-hit rows, time ordering, delay and distance consistency, residual tolerance, receiver-normal branch weights, and unit-direction normalization;
...
- isolated baseline sandbox artifacts for Animator, Animator worker-owned solver-bridge runs, Photon causal-root, Photon app-owned WebAssembly client roots/hits/ledger paths, Photon circular-source roots/hits/ledger, Photon direct and runSimulation normalized circular-source roots/hits/ledger, Photon phase-diagnostics, Animator path-history, Animator motion dynamic replay, Ideal Braid app-owned WebAssembly client geometry paths, and Ideal Braid smoke cases.
- ordered migration parity reporting for Animator, Photon, then Ideal Braid central bridge fixtures.
- manufactured analytic causal-root validation for moving-source and moving-receiver delayed-hit cases with known root, source-normal Jacobian, receiver-normal branch weight, and unit direction.
...
- manufactured analytic moving-circular source and observer-field validation, with native/WASM row production for source velocity, phase-at-hit, receiver-normal branch weight, and field contribution rows.

## First ABI Surface

The first checked ABI is intentionally narrow:

- `architrino_solver_admit_simulation_envelope_f64`
...
- `architrino_solver_sample_linear_path_history_f64`
...
- fixed-layout moving-circular source history, root, observer-field branch, observer-field contribution, and observer-field summary rows for Photon absolute-history source-history experiments;
...
- fixed-layout precision-solve option and summary rows for selected path, numeric type, numeric chart, root controls, replay flags, escalation records, residual summary, and dominant status;
- fixed-layout admission rows that report model/envelope/capability admission decisions, stress summaries, and status records;
...
- fixed-layout delayed-potential geometry rows that compute Ideal Braid flight-time and potential diagnostics;
- fixed-layout circular self-hit span rows that compute Ideal Braid super-field self-hit geometry;
...
- app bridge methods `diagnosePrecisionF64`, `solveCausalRootsPrecisionF64`, `solveRootsAndHitsPrecisionF64`, `propagateErrorBudgetF64`, `checkRootHitInvariantsF64`, `classifyRootLedgerTransitionsF64`, `prepareWorkPacketHeader`, `orderWorkPacketResults`, `planPathHistoryWorkPackets`, `solveCausalRootsF64`, `solveCircularSourceCausalRootsF64`, `solveCircularSourceRootsHitsLedgerF64`, `solveCircularSourceRootsHitsLedgerNormalizedF64`, `solveMovingCircularSourceCausalRootsF64`, `solveMovingCircularSameSourceCausalRootsF64`, `computeMovingCircularObserverFieldF64`, `solveMovingCircularAbsoluteHistoryRunF64`, `solveCausalRootsNormalizedF64`, `buildRootLedgerDetailF64`, `solveCausalRootBatchF64`, `solveRootsAndHitsF64`, `sampleLinearMotionF64`, `integrateConstantAccelerationMotionF64`, `computePhaseAtHitF64`, `summarizePhaseAtHitsF64`, `computeSharedGeometryF64`, `detectAssemblyMembershipEventsF64`, `buildAssemblyGraphDatasetF64`, `createAssemblyGraphStoreF64`, `describeAssemblyGraphStoreF64`, `readAssemblyGraphStoreRangeF64`, `createPathHistoryStreamF64`, `validatePathHistoryDynamicReplayF64`, `planPathHistoryStorageLifecycleF64`, `applyPathHistoryStorageLifecycleF64`, `buildSpaceTimeIndexF64`, `buildPathHistoryStreamSpaceTimeIndexF64`, `querySpaceTimeIndexF64`, `queryEmissionShellCandidatesF64`, `queryEmissionShellCandidatePacketF64`, `queryEmissionShellCandidatePacketsF64`, and `refineEmissionShellCandidateRootsF64`.

## First Shared Geometry API

`computeSharedGeometryF64` exposes the first solver-owned geometry calculations through the app bridge. The first operations batch path-segment bounds, point-on-sphere checks, delayed-potential flight-time rows, and circular self-hit spans through the WebAssembly C ABI. This keeps broad-phase bounds, spherical-emission checks, Ideal Braid delayed-potential diagnostics, and circular self-hit span logic in the central solver surface instead of duplicating them across apps.

## First Admission API

`admitSimulationEnvelope` validates a model contract, error budget, and simulation envelope before a run starts. The app bridge routes admission through `architrino_solver_admit_simulation_envelope_f64` when the WebAssembly module is available, with a JavaScript fallback for non-Wasm clients. It reports whether the request should be admitted, simplified through an explicit reduced model, batched, precision-escalated, or rejected, with structured status records and a stress summary for entity count, interaction graph, memory, storage, time-step, output-detail, and precision pressure.

## First Run API

`runSimulation` now supports the first `causalRoots`, `phaseDiagnostics`, `pathHistory`, `delayedHits`, `sharedGeometry`, `validationReplay`, `appPlayback`, and `motionSimulation` run shapes through the shared app bridge. `causalRoots` and `delayedHits` runs accept exactly one of `rootRequest`, `normalizedRootRequest`, `circularSourceRootRequest`, or `normalizedCircularSourceRootRequest`. Normalized runs compute root and hit rows in authoritative local coordinates and attach a diagnostic that records `coordinateFrame: "origin-normalized"`, `coordinateOrigin`, and `absolutePointAuthority: "display-only"`. Circular-source causal-root and delayed-hit runs compute native circular-source root, delayed-hit, and detailed `root_ledger_detail.v1` rows through the run-handle, manifest, dense-buffer, and stream-readback surface. Normalized circular-source runs use the same run-handle, manifest, dense-buffer, and stream-readback surface, while preserving local root, hit, and detailed ledger rows as authoritative and attaching optional absolute-display root metadata for app rendering. The circular-source ledger rows include native root brackets and iteration counts. `motionSimulation` accepts exactly one of a linear sample request or a constant-acceleration integration request, and both publish `frame_buffer.v1` output through the same manifest and buffer contract. When `motionSimulation` requests `pathStream`, the bridge also emits a run-scoped `path_segment.v1` stream from native solver rows: linear motion uses the exact declared segment window, and constant-acceleration motion uses chord segments with an interpolation error bound. The direct bridge admits the request, resolves the effective precision path from the caller request, admission result, and claim-level minimum, and rejects the run if either an explicit requested path or the selected path is outside the model contract's `compatiblePrecisionPaths`. It then sends linear causal-root and delayed-hit runs through the native precision roots-and-hits path, runs the WebAssembly circular-source root/hit/ledger path, phase-diagnostics, path-history, shared-geometry, validation-replay, app-playback, sampled-motion, or integrated-motion path as requested, returns a run handle with an attached completed response, registers run-scoped streams when a run emits streams, and exposes the same stream readback path used by direct root/hit calls. Run manifests preserve the effective capability envelope used for admission, including memory, storage, precision, step-count, and entity limits, and they carry the effective threading plan used for the run. Each run manifest also carries `solver-run-precision-metadata.v1`, and each stream descriptor carries `solver-path-history-stream-metadata.v1`, so app-facing `f64` buffers produced from stricter precision paths are labeled with their stream value authority and app-buffer authority. Linear root-solving run responses, manifests, and run descriptions carry the native precision solve summary as structured metadata, including escalation records when the selected path is stricter than the prior path. Circular-source run responses and manifests carry a separate analytic `f64` precision summary with the requested/admission precision path, f64 output numeric type, applied root tolerance, residual, Jacobian, iteration, and subdivision metadata; the diagnostic records that the precision-path selector has been applied to circular-source controls while the row output remains governed by the f64 C ABI. Linear root-solving responses also publish `root_ledger_detail.v1` rows and buffers generated with the selected precision root controls, so validation and migration consumers can inspect active roots, inactive gaps, and first-failure rows through the bridge. `SolverAppWorkerBridge.mjs` adds the first worker request/response protocol over that same client surface, with structured responses, normalized errors, dense-buffer transfer collection, stream readback, cancellation, and disposal. The JSON schema now includes `solver-app-worker/v1` request, response, and error messages with the supported bridge method set. Later native and browser-worker implementations can preserve the handle and message shape while moving more execution off the immediate call path.

`SolverAppAdapters.mjs` defines the first app-facing request builders on top of the run, stream, work-packet, assembly graph, and emission-shell query shapes. The current adapters build Photon causal-root and phase-diagnostics requests, path-history requests, Ideal Braid delayed-hit and shared-geometry requests, Animator motion-simulation requests, Animator app-playback requests, generic shared-geometry requests, validation-replay requests, path-history stream, readback, and dynamic-replay validation requests, assembly graph dataset/store/describe/read requests, storage-lifecycle requests, path-history work-packet plan requests, and emission-shell query, packet, batch, and merge requests. Photon causal-root and Ideal Braid delayed-hit adapters enforce the same exact-one rule for ordinary `rootRequest`, precision-preserving `normalizedRootRequest`, or analytic `circularSourceRootRequest`. These builders centralize IDs, claim level, precision path, config version, output defaults, app labels, stream storage defaults, packet defaults, and dense-response preservation so migrating app code does not hand-roll solver request envelopes. `SolverAppBridgeClientResolver.mjs` is the shared app-side client resolver for Photon, Ideal Braid, and Animator; it accepts an existing client, app-supplied client factory, provided worker, app-supplied worker factory, constructible worker URL, or packaged WebAssembly module factory, performs bridge initialization when it creates the client, and disposes internally-owned clients after the run. `SolverAppWorkerRuntime.mjs` is the first packaged worker entry helper; it resolves the packaged WASM module factory, installs the worker bridge on a worker scope, and keeps app code on the same request/response protocol instead of requiring C++ or Emscripten handling in app modules. The WebAssembly loader is emitted as `architrino_solver_wasm_smoke.mjs` with Emscripten ES-module output, so module workers can import the packaged factory directly. `AnimatorSolverBridgeWorkerRuntime.js` gives the Animator worker a standard solver bridge option bundle with that packaged loader path plus deployment overrides for alternate package locations.

The bridge capabilities include `solver-app-bridge-capabilities.v1`, which reports the available app adapters, dense-data transport modes, precision-routing paths, normalized status taxonomy, stream-query helpers, worker fallback posture, and browser storage fallback status. The top-level `solver-binary-layout-catalog.v1` block gives apps the fixed row size, role, numeric type, storage authority, and streamability flag for each dense binary layout. The `solver-numeric-serialization.v1` block now names the chart families required by the precision contract, including `direction_log_magnitude`. The `solver-precision-routing-capabilities.v1` block lets apps distinguish the linear native precision-path route from the analytic circular-source route, whose precision-path selector adjusts solve controls while keeping row output on the `f64` C ABI. The `solver-status-taxonomy.v1` block gives apps the canonical status codes, categories, default severities, recoverability defaults, and stage hints used by normalized diagnostics. This lets apps negotiate bridge support before a run without knowing C++ exports, WebAssembly lifecycle details, or stream-file internals.

`solveMovingCircularSourceCausalRootsF64`, `solveMovingCircularSameSourceCausalRootsF64`, `computeMovingCircularObserverFieldF64`, and `solveMovingCircularAbsoluteHistoryRunF64` expose the first bridge-level Photon absolute-history moving-circular facade. These methods run through the shared app bridge and worker protocol, return stable `solver-moving-circular-source-causal-roots-f64.v1`, `solver-moving-circular-same-source-causal-roots-f64.v1`, `solver-moving-circular-observer-field-f64.v1`, and `solver-moving-circular-absolute-history-run-f64.v1` schemas, preserve source phase, receiver phase for same-source roots, source-normal Jacobian, receiver-normal branch factor, residual, scan diagnostics, and assemble receiver-normal observer-field branch sums from retained roots. With a loaded WebAssembly module, moving-circular source-root rows and observer-field contribution rows are produced by the native C ABI; without a module, the bridge keeps the JavaScript reference facade for deterministic app tests. The absolute-history run facade accepts source-root requests plus an observer-field request, solves the retained source-to-receiver roots, and reconstructs observer-field rows in one central bridge call while preserving app-supplied source-history provider and solver-boundary metadata. Same-source helical roots remain on the JavaScript reference facade until their native/WASM row shape is promoted.

`solveCausalRootsNormalizedF64` is the first app-facing precision-preservation helper for large absolute coordinate regimes. The caller supplies a `coordinateOrigin` separately from a local `causalRootsF64Request`; the solver computes in the local normalized frame, returns authoritative local roots, and can attach best-effort absolute display points with `absolutePointAuthority: "display-only"`. `solveCircularSourceRootsHitsLedgerNormalizedF64` applies the same local-frame policy to circular-source roots, delayed-hit rows, detailed root-ledger rows, buffers, and streams. This avoids losing small source/receiver separations that cannot survive if they are first encoded as `1e18 + 1` style JavaScript numbers.

`PrecisionPathSolver.hpp` is the first native precision-path execution wrapper. It combines precision diagnostics, claim-level minimums, caller-requested precision paths, automatic escalation, path-specific causal-root controls, and optional validation replay. It can reject a run when the caller forbids escalation from a weaker path to the required path. `architrino_solver_solve_causal_roots_precision_f64` exposes precision causal roots through a fixed C ABI summary row, `architrino_solver_solve_roots_and_hits_precision_f64` exposes the matching roots-and-delayed-hits solve, and `architrino_solver_solve_roots_hits_ledger_precision_f64` exposes roots, delayed hits, and detailed root-ledger rows from the same selected precision solve. `solveCausalRootsPrecisionF64` and `solveRootsAndHitsPrecisionF64` expose those calls through the shared app bridge and worker protocol. App `runSimulation` sends linear and normalized `causalRoots` and `delayedHits` runs through the native precision roots-and-hits path, so apps receive the selected precision path, root controls, numeric type, replay flags, root rows, hit rows, detailed root-ledger rows, buffers, and transient stream through one central solver authority. Circular-source runs use the native circular-source roots/hits/ledger path, with normalized support for large absolute coordinate regimes, precision-path-controlled root tolerance and search controls, and explicit analytic `f64` precision metadata in run manifests.

`planThreadingPolicy` is the first app-facing threading diagnostic. It reports the requested worker count, active worker count, scheduling mode, backend, deterministic-reduction flag, thread capability flags, fallback reason, planned chunk count, planned chunk size, queue depth, determinism status, contention diagnostics, chunk timing summary, per-stage speedup summary, and status. Work split and queue depth are deterministic plan-time values. Chunk timing and speedup are reported as `not-measured` unless the caller supplies observed timing data from a real run. The current WebAssembly bridge reports a recoverable sequential fallback when a caller requests fixed threads but browser or WASM thread support is unavailable; native C++ still owns the first actual task-pool execution path.

`cancelRun` accepts a `runId` or `requestId` and returns the normalized `cancelled` status. When the bridge has a matching stored run, cancellation updates the run response, summary, diagnostics, and manifest status, then regenerates the manifest hash so later `describeRun` calls see the cancellation state. Cancellation is non-destructive by default; `cancelRun({ releaseStreams: true })` also releases the run's registered stream handles while keeping the run description available for diagnostics.

`describeRun` returns metadata for a completed or cancelled run already registered with the bridge. It includes the run manifest, summary, diagnostics, buffer descriptors without dense payloads, and stream descriptors with available ranges. Apps can use it to decide whether to render, range-read, export, validate, or inspect a cancelled dataset without keeping all dense buffers in application memory.

`createPathHistoryStreamF64` publishes path-history rows as chunked `path_segment.v1` stream data through the same `openStream` and `readStreamRange` path used by root and delayed-hit streams. It supports transient `caller-buffer` storage and the first durable `native-file` storage path for Node/native runs. The native-file path writes one binary file per chunk, a compact binary `stream_index.v1` sidecar with 64-byte index rows, and a JSON stream manifest under `.tmp/solver-app-streams/` by default. The manifest records file paths and checksums in chunk descriptors, persists `solver-stream-index.v1` path-index rows plus the sidecar descriptor, and loads only selected chunks during readback, lifecycle planning, and emission-shell scans. Range metadata is checked before file-backed chunk payloads are read, so time, frame, byte, and chunk-index filters can reject excluded chunks without checksum or dense-buffer IO. A fresh bridge can reopen a durable native-file stream with `openStream({ manifestPath, purpose })`, validate the chunk files and binary index sidecar, load the persisted path index when present, and return the stream storage policy plus metadata on the handle. Apps can also call `describeStream({ manifestPath })` and `readStreamRange({ manifestPath, ...selection })` directly when a durable stream does not need a separate open handle. Native package callers can use `architrino_solver_write_path_history_stream_f64`, `architrino_solver_read_path_history_stream_index`, `architrino_solver_read_path_history_stream_chunks`, `architrino_solver_query_path_history_stream_index`, and `architrino_solver_read_path_history_stream_query_f64` to write the same file-backed stream format and read it back with optional chunk-checksum verification. `closeRun({ releaseStreams: true })` removes the closed run's stream handles and deletes its native-file stream directory; `dispose()` releases all remaining stream handles and native-file directories. Browser OPFS storage remains a later backend; the bridge reports OPFS and native-file capability separately so apps can choose a visible fallback. The stream metadata records precision path, units, coordinate frame, scale normalization, interpolation rule, lifecycle metadata when applied, dynamic replay metadata when available, provenance, diagnostics, per-chunk checksums, and per-chunk spacetime bounds derived from segment endpoints plus error bounds. For `path_segment.v1` streams, `readStreamRange` can restrict reads by chunk index and compact matching rows by path key, time range, and frame range inside a larger chunk, preserving filtered time, frame, byte, and bounds metadata for high-speed readers.

`describeStream` returns stream metadata without dense payloads. For `path_segment.v1` streams, it includes stream ranges with time, frame, byte, and spacetime-bounds metadata, plus `solver-stream-index.v1` path-index rows with path key, chunk index, row offset, row count, time range, frame range, and byte range. This gives apps a cheap way to decide which path-history ranges to request before reading buffers.

`validatePathHistoryDynamicReplayF64` regenerates path-history rows from a stream's dynamic replay metadata, reads the registered stream through the normal range-selection path, and returns a structured match or mismatch report. It works against live caller-buffer streams and durable native-file streams supplied directly as `validatePathHistoryDynamicReplayF64({ manifestPath, tolerance })`, so apps can verify that a motion path stream is still replayable without keeping the original dense rows in application memory.

`planPathHistoryStorageLifecycleF64`, `planPathHistoryWorkPackets`, `buildPathHistoryStreamSpaceTimeIndexF64`, `queryEmissionShellCandidatesF64`, `queryEmissionShellCandidatePacketF64`, `queryEmissionShellCandidatePacketsF64`, and `refineEmissionShellCandidateRootsF64` can consume either a registered `streamId` or a durable native-file `manifestPath`. `queryEmissionShellCandidatesF64` is the first app-facing broad-phase query over path-history streams. It uses path keys, time bounds, signal speed, and path-segment bounding boxes to find source/receiver segment pairs whose distance interval can overlap an emitted-shell radius interval. The bridge scans the stream once and applies path-key/time filters before materializing full rows, so selective source/receiver queries do not allocate every path row. The same candidate filter is exposed through `architrino_solver_query_emission_shell_broad_phase_f64`; callers can opt into the interval-time-slab / spatial-hash / emission-shell-annulus v0 index by passing `indexOptions.strategy = "emission_shell_broad_phase_v0"`, which routes WebAssembly-backed calls through `architrino_solver_query_emission_shell_broad_phase_indexed_v0_f64` and reports `native_c_abi_indexed_v0` plus an index summary. The sampled narrow-phase estimator is exposed through `architrino_solver_estimate_emission_shell_narrow_phase_f64`, so WebAssembly-backed apps can run candidate rejection and false-positive estimation in the C++ solver core while retaining the JavaScript bridge fallback. It reports checked pairs, rejected pairs, rejection and candidate rates, a sampled narrow-phase false-positive estimate, object rows for simple app consumers, and dense `emission_shell_candidate.v1` plus `emission_shell_narrow_phase.v1` buffers for high-volume worker and index consumers. `refineEmissionShellCandidateRootsF64` consumes those candidate rows, reloads only the referenced path-history chunks, and uses the C++ roots-and-hits batch ABI to solve exact fixed-hit causal roots at sampled-hit times. It emits native delayed-hit rows and accepts `workerCount` so larger candidate sets can use the native batch worker path where available. When a refinement request carries a work packet, the bridge validates candidate chunk ownership against the packet source/receiver ranges, scopes root and hit buffers by `packetId`, and returns `packetResult` metadata so exact-root refinement can merge through the same deterministic work-packet path as broad-phase candidate scans.

The first `validationReplay` run shape compares a baseline response against a candidate response with the migration vocabulary `baseline_within_tolerance`, `baseline_refined_result`, `baseline_model_boundary_difference`, and `baseline_investigation_required_mismatch`. It returns the comparison as part of the run response and records the same result in the run manifest and diagnostics so replay checks can flow through the normal app bridge instead of a separate side channel. The baseline comparison helper also supports shared-geometry payloads so bridge geometry responses can be compared against solver-owned geometry rows.
...
`RootLedger.hpp` defines fixed-layout `root_ledger_detail.v1` rows for validation and replay, exposed through `architrino_solver_build_root_ledger_detail_f64` and `buildRootLedgerDetailF64`. The existing `root_ledger.v1` ABI remains the compact app-facing root row. The detailed ledger adds stable source, receiver, root, and ledger keys; root support intervals; bracket bounds; root kind; status code; Jacobian sign stratum; sequence index; iteration count; state flags; active-root rows, inactive search-gap rows, retained-history tail-boundary rows, and first-failure rows. The app bridge also derives required root-solve precision forensics on readback: residual scale, absolute residual, normalized residual, selected root tolerance, and first-failure code. This lets validation consumers distinguish an active retained root from a search interval where no root was bracketed, a finite-history boundary, or a solver halt such as insufficient history depth without changing the fixed 192-byte `root_ledger_detail.v1` row.

## First Motion Frames

`sample_linear_motion` in C++ and `sampleLinearMotionF64` in the app bridge sample a linear path segment into fixed-time frames through the WebAssembly C ABI. `sample_linear_path_history` and `architrino_solver_sample_linear_path_history_f64` emit solver-owned `path_segment.v1` rows for a declared linear sample window. `integrate_constant_acceleration_motion` and `integrateConstantAccelerationMotionF64` add the first deterministic native motion-integration path for a declared constant-acceleration model. `integrate_constant_acceleration_path_history` and `architrino_solver_integrate_constant_acceleration_path_history_f64` emit solver-owned `path_segment.v1` rows for that model, using endpoint chords over each integration step and adding the analytic constant-acceleration interpolation bound to the row error budget. Motion-generated path streams attach `solver-path-history-dynamic-replay.v1` metadata with the compact linear or constant-acceleration replay request that produced the stream. Both frame routes publish the same `frame_buffer.v1` row shape: path key, frame index, time, position, velocity, error bound, and state flags. This gives apps a stable frame-buffer shape for playback, path-history inspection, and the first narrow integration fixture before richer force-law integration is connected.

## First Work Packets

`WorkPacket.hpp` defines the first transport-ready packet header for solver work. A packet owns explicit source, receiver, path, and time ranges; declares selected precision path, expected output layouts, binary input-buffer references, checksums, merge order, and merge key; and serializes to canonical JSON-compatible bytes. `deterministic_merge_order` gives thread, worker, process, service, and future GPU backends one stable result-merge rule before those backends are implemented. The app bridge exposes the same contract through `prepareWorkPacketHeader` and `orderWorkPacketResults`, so worker dispatch code can validate packet ranges, row sizes, checksums, canonical headers, and merge order without parsing native C++ internals. `planPathHistoryWorkPackets` creates deterministic source/receiver chunk-pair packets from an already-registered `path_segment.v1` stream, which is the first packetized broad-phase dispatch plan over path-history data. The plan response now includes source and receiver chunk-selection summaries plus a plan checksum derived from the selected chunks, pair counts, truncation state, and packet header checksums, giving later worker, native-thread, service, and GPU-style dispatchers one compact replay identity for the whole packet plan. The same packet can now carry the next narrow-phase operation: `refineEmissionShellCandidateRootsF64` validates that supplied candidates remain inside packet-owned source and receiver ranges and publishes packet-scoped `root_ledger.v1` and `delayed_hit_events.v1` output references.
...
`InvariantChecks.hpp` defines the first native validation pass over solved causal-root and delayed-hit rows. It checks finite fields, ordered times, delay and distance consistency, residual tolerance, receiver-normal branch weights, and delayed-hit unit-direction normalization. The first smoke target verifies that a valid analytic solve passes and that a deliberately corrupted delayed-hit direction fails.
...
`StorageLifecycle.hpp` defines the first deterministic planner for path-history chunk retention. Given a policy and chunk rows, it marks chunks to remain active, spill to warm storage, archive cold, build a deep index, delete, or stay blocked in active memory when a chunk is pinned unsafe. `planPathHistoryStorageLifecycleF64` exposes that planner through the WebAssembly C ABI and app bridge for either app-supplied chunk metadata or an already-registered `path_segment.v1` stream, including native-file backed streams. `applyPathHistoryStorageLifecycleF64` applies the same plan to a registered stream by recording lifecycle metadata on the stream and rewriting the native-file manifest when the stream is file-backed. Default apply is non-destructive. If `deleteStreamWhenAllChunksDeleted` is set, the bridge deletes and unregisters only a complete stream whose lifecycle plan marks every chunk for deletion; partial delete plans remain metadata-only and report `not_all_chunks_planned_delete`. The app response includes row decisions plus a summary with tier counts, action counts, byte totals by tier, unsafe-to-age-out chunk ids, the deep-index work queue, and cleanup status. When the plan queues deep-index work, apply also builds a compact `solver-path-history-deep-index.v1` metadata artifact from the stream-backed `spacetime_index.v1` builder and persists the artifact with the lifecycle metadata. Native-file stream directories are cleaned up by guarded lifecycle delete, `closeRun`, and `dispose`; automated partial tier movement, browser quota handling, and deeper user-visible deletion workflows will use lifecycle decisions in later storage slices.
...
`scripts/check-solver-baseline-sandbox.mjs` runs fixed app-labeled Animator, Photon, and Ideal Braid smoke cases through the shared bridge, including causal-root, the Photon causal-root facade, Photon app-owned WebAssembly client causal-root creation, the Photon circular-source roots/hits/ledger facade, Photon app-owned WebAssembly client circular-source roots/hits/ledger creation, direct and runSimulation normalized circular-source roots/hits/ledger paths, Photon phase diagnostics, Animator path-history, Animator motion dynamic replay, Animator worker-owned solver bridge client creation, Ideal Braid geometry artifacts, Ideal Braid app-owned WebAssembly client flight-time and circular self-hit creation, and the first Ideal Braid flight-time and circular self-hit app facades. It writes artifact-only JSON output under `.tmp/solver-baseline-sandbox`, records resource caps, fixed no-randomness seed policy, explicit tolerance policy, sandbox provenance, and SHA-256 artifact hashes, preserves each `solver-run-manifest.v1` manifest and manifest hash where a case uses `runSimulation`, and verifies each case classifies as `baseline_within_tolerance`.

`scripts/check-solver-geometry-inventory.mjs` writes `.tmp/solver-geometry-inventory/solver-geometry-inventory.json`. It verifies the known app bridge geometry and solver surfaces for Photon, Ideal Braid, and Animator, and records the central-solver ownership target for each surface.

`scripts/check-solver-migration-parity.mjs` reads `.tmp/solver-baseline-sandbox/manifest.json`, verifies artifact hashes and the baseline-classification vocabulary, enforces the migration order Animator, Photon, then Ideal Braid, and writes `.tmp/solver-migration-parity/solver-migration-parity-report.json`. The report includes aggregate app/case readiness counts plus explicit missing and blocking case lists, and it must stay green for central bridge fixtures.

## First Analytic Validation

`solver_analytic_smoke.cpp` adds a manufactured moving-source case with a closed-form causal root. It verifies the root time, delay, distance, residual, Jacobian, branch weight, receiver-normal factor, delayed-hit strength, unit direction, moving-circular source velocity, source phase-at-hit, and observer-field contribution row, so the native solver is checked against an analytic target in addition to app-facing baseline fixtures.
...
single-worker versus bounded-worker causal-root thread scaling, emission-shell
broad-phase scans, space-time index build/query behavior, and path-history plus
assembly graph store IO. It reports operation counts, observation counts,
elapsed milliseconds, operations per second, a checksum, strategy labels, and
named metrics for query and threading workloads such as rejection rate,
candidate rate, index row count, query matches, overflow entries, worker counts,
single-worker elapsed time, bounded-worker elapsed time, speedup ratio, and
checksum delta.
By default the runner writes `.tmp/solver-build/benchmark/solver-benchmark-report.json`
with schema `solver-benchmark-report.v1`. Use `--report <path>` to write the
same report somewhere else, or `--no-report` for console-only runs. The runner
parses its own benchmark output and fails if the final status or case count does
not match the structured report. The report includes Node runtime, platform,
CPU-count, memory, and `EM_CACHE` context. It checks result sanity but does not
enforce machine-dependent wall-clock thresholds.
