...
`validatePathHistoryDynamicReplayF64` regenerates path-history rows from a stream's dynamic replay metadata, reads the registered stream through the normal range-selection path, and returns a structured match or mismatch report. Apps can use it to verify that a motion path stream is still replayable without keeping the original dense rows in application memory.
