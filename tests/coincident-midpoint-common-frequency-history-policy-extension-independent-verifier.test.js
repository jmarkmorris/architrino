...
import {
  ...
  validateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentProtocol,
...
function evaluate({
  ...
  return evaluateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentVerifier({
    verifierProtocol,
    baseProtocol,
...
test("independent verifier freezes only the new history-extension slice", async () => {
  const validated =
    validateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentProtocol(protocol);
  ...
  const source = await readFile(
    "src/prescribed-path-analysis/" +
      "CoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentVerifier.mjs",
    "utf8",
  );
  assert.doesNotMatch(source, /CoincidentMidpointCommonFrequencyOuterRadiusBandExpansionDiagnostic/u);
});
...
test("independent verifier rejects geometry, reach, and gate drift", () => {
  ...
  assert.throws(() => validateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentProtocol(identityDrift), /exact coincident-midpoint common-frequency scientific identity/);
  const radiusDrift = structuredClone(protocol);
  radiusDrift.rootSheet.domain.alpha3[1] += 1 / 1024;
  assert.throws(
    () => validateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentProtocol(radiusDrift),
  ...
  assert.throws(
    () => validateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentProtocol(historyDrift),
  ...
  assert.throws(
    () => validateCoincidentMidpointCommonFrequencyHistoryPolicyExtensionIndependentProtocol(toleranceDrift),
