...
} from "../scripts/neutral-braid/octahedral-fold-aware-clock-length-chart-closure-proof.mjs";
...
test("fold-aware clock length chart closure proof CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-fold-aware-chart-closure-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-braid/octahedral-fold-aware-clock-length-chart-closure-proof.mjs",
      import.meta.url
    )
  );
