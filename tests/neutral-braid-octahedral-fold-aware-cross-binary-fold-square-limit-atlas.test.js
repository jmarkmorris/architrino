...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-fold-square-limit-atlas.mjs";
...
test("fold-aware fold square limit atlas CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "neutral-braid-fold-square-")
  );
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-fold-square-limit-atlas.mjs",
      import.meta.url
    )
  );
