...
import { lensingTransferEvidenceStatusForPath } from "../scripts/equation-mapping/eq21-lensing-transfer-evidence.mjs";

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const checkerPath = path.join(
  repoRoot,
  "scripts/equation-mapping/eq21-lensing-transfer-child-residual.mjs",
);
const acceptedPath =
  "scripts/equation-mapping/eq21-lensing-transfer-child-accepted.v1.json";
const evidencePath =
  "scripts/equation-mapping/eq21-lensing-transfer-child-evidence.v1.json";
const attemptPath =
  "scripts/equation-mapping/eq21-lensing-transfer-child-attempt.v1.json";
const priorityControlPath =
  "scripts/equation-mapping/eq21-lensing-transfer-child-priority-source-negative-control.v1.json";

function runLensingChild(inputPath, extraArgs = []) {
...
function writeTempFixture(fixture, name) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eq21-lensing-child-"));
...
function setAllSourcePaths(fixture, sourcePath) {
  ...
  fixture.packet.model.lensingKernel.sourcePath = sourcePath;
}

test("EQ-21 lensing transfer evidence is accepted", () => {
  const status = lensingTransferEvidenceStatusForPath(evidencePath, { repoRoot });
  ...
  assert.equal(status.model.derived.cmb_lensing_grid_normalized_residual, 0);
});

test("accepted EQ-21 lensing child populates CMB-lensing grid", () => {
  const report = runLensingChild(acceptedPath, ["--require-populated"]);
  ...
  assert.equal(report.summary.cmbLensingGridResidualPass, true);
  assert.equal(report.summary.noHiddenRetunePass, true);
  assert.equal(report.summary.sampleCount, 3);
  assert.equal(report.summary.cmbLensingGridNormalizedResidual, 0);
});

test("EQ-21 lensing attempt stays blocked at missing child row", () => {
  const report = runLensingChild(attemptPath);

  assert.equal(report.summary.status, "blocked_missing_rows");
  assert.equal(report.summary.nextBlocker, "missing_accepted_lensing_transfer_child");
  assert.equal(report.rows.lensing_transfer_child.reason, "row_not_accepted");
});

test("EQ-21 lensing priority-source control is rejected as non-evidence", () => {
  const report = runLensingChild(priorityControlPath);

  assert.equal(report.summary.status, "blocked_missing_rows");
  assert.equal(report.summary.nextBlocker, "missing_accepted_lensing_transfer_child");
  assert.equal(
    report.rows.lensing_transfer_child.reason,
    "lensing_coordination_source_path",
  );
  assert.equal(report.sourceEvidence.failureCount, 16);
});

test("accepted-looking EQ-21 lensing rows must cite lensing evidence", () => {
  ...
  assert.equal(
    report.rows.lensing_transfer_child.reason,
    "lensing_lensing_transfer_evidence_fields_missing",
  );
});

test("EQ-21 lensing child rejects hidden retune of inherited shared keys", () => {
  const fixture = loadAcceptedFixture();
  fixture.packet.sharedKeys.find((row) => row.key === "rho_A").value = 0.26;

  const report = runLensingChild(writeTempFixture(fixture, "hidden-retune.json"));
...
test("EQ-21 lensing child requires the accepted matter-power parent", () => {
  const fixture = loadAcceptedFixture();
...
test("EQ-21 lensing child rejects derived-grid mismatches", () => {
  const fixture = loadAcceptedFixture();
  fixture.packet.model.derived.samples[0].C_phi_phi = 0;

  const report = runLensingChild(
    writeTempFixture(fixture, "derived-mismatch.json"),
  );

  assert.equal(report.summary.status, "blocked_lensing_model_derived_mismatch");
  assert.equal(
    report.summary.nextBlocker,
    "lensing_model_derived_mismatch_samples.L40_z1_k0p02.C_phi_phi",
  );
