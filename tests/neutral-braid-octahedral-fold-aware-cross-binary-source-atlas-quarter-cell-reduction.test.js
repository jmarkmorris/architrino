...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-source-atlas-quarter-cell-reduction.mjs";
...
test("source-atlas quarter-cell reduction CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "neutral-braid-source-atlas-cell-")
  );
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-source-atlas-quarter-cell-reduction.mjs",
      import.meta.url
    )
  );
