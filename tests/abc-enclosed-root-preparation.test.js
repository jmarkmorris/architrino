...
import { commandTokens, compileInput, fileBinding, makeDependencies, parsePrepareAbcArgs,
  referenceSnapshot, requireSameBindings, resolvedInvocation, sourceSnapshot } from "../scripts/eom/prepare-abc-enclosed-root.mjs";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

test("ABC build preparation accepts only a fresh scoped directory argument", () => {
  assert.throws(() => parsePrepareAbcArgs([]), /Usage/u);
  assert.throws(() => parsePrepareAbcArgs(["--out", ".tmp/abc-build"]));
  assert.throws(() => parsePrepareAbcArgs(["--out", ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/../escape"]));
  assert.throws(() => parsePrepareAbcArgs(["--out", "x", "--run", "yes"]), /Usage/u);
  assert.equal(parsePrepareAbcArgs(["--out", ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/control"]),
    path.join(root, ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/control"));
});
...
test("observed source inventory binds full solver tree and reviewed adapter/CMake/supervisor", () => {
  ...
  for (const filename of ["src/eom/src/ExactPairBatch.cpp", "src/eom/src/History.cpp", "src/eom/src/Interval.cpp",
    ...
    "src/eom/CMakeLists.txt", "src/eom/native/eom_abc_enclosed_root_cli.cpp", "scripts/eom/prepare-f5-enclosed-root.mjs"]) {
...
test("reference inventory binds eight frozen references and sixteen exact candidate sources", () => {
  const records = referenceSnapshot();
  assert.equal(records.length, 24);
  assert.equal(records.filter((record) => record.id.startsWith("candidate-source:")).length, 16);
  assert.equal(new Set(records.map((record) => record.path)).size, 24);
});
