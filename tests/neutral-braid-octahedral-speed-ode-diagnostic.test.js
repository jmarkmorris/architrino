...
} from "../scripts/neutral-braid/octahedral-speed-ode-diagnostic.mjs";
...
test("octahedral speed-ODE diagnostic rejects the frozen fixed-ledger primitive", () => {
  ...
  assert.equal(artifact.schema, "neutral-braid-octahedral-speed-ode-diagnostic/v1");
...
test("octahedral speed-ODE CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-speed-ode-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-speed-ode-diagnostic.mjs", import.meta.url)
  );
