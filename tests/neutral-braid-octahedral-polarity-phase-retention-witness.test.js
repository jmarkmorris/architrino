...
} from "../scripts/neutral-braid/octahedral-polarity-phase-retention-witness.mjs";
...
test("octahedral polarity-phase retention witness rejects best-row improvement as retention", () => {
  ...
  assert.equal(artifact.schema, "neutral-braid-octahedral-polarity-phase-retention-witness/v1");
...
test("octahedral polarity-phase retention witness CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-polarity-phase-retention-witness-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-polarity-phase-retention-witness.mjs", import.meta.url)
  );
