...
test("camera shot normalization upgrades legacy framing strings into authored framing objects", () => {
  const shots = normalizePdgviewCameraShots(
    [
      {
        id: "shot_1",
...
test("active camera path follows the current shot before falling back to the scene default", () => {
  ...
  assert.equal(getPdgviewActiveCameraPathId(documentData, 2), "camera_wide");
  assert.equal(getPdgviewActiveCameraPathId(documentData, 5), "camera_close");
});
...
test("viewport framing state merges assembly keep-in-view defaults with shot overrides", () => {
  ...
  const state = resolvePdgviewViewportFramingState(documentData, 8, {
    start: 0,
    end: 24,
  });
  ...
  assert.equal(
    formatPdgviewViewportFramingSummary(state),
...
test("autoscale camera state recenters on the target set and only expands distance when needed", () => {
  const state = computePdgviewViewportAutoscaleCameraState({
    cameraState: {
