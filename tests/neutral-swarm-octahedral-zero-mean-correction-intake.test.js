...
const SOURCE_ARTIFACT_ID = "neutral_swarm_octahedral_zero_mean_correction_intake.frozen_constant_vector.v1";
...
test("octahedral zero-mean correction intake emits the constant six-vector obstruction", () => {
  ...
  assert.equal(artifact.schema, "neutral-swarm-octahedral-zero-mean-correction-intake/v1");
...
test("octahedral zero-mean correction intake CLI emits and validates JSON artifacts", () => {
  ...
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-zero-mean-correction-intake.mjs", import.meta.url)
  );
  fs.writeFileSync(
    candidatePath,
    `${JSON.stringify(
