...
} from "../src/apps/animator/AnimatorPdgeditImportRuntime.js";
...
test("animator builds the downstream staging contract from accepted pdgedit output", () => {
  const pdgeditDocument = createSyntheticPassThruPdgeditDocument();
  const stagingSchema = readJson("src/contracts/animator-staging/v1/schema.json");
  const builtStaging = buildAnimatorStagingContractFromPdgeditDocument(pdgeditDocument, stagingOptions);

  assert.deepEqual(validateAgainstSchema(builtStaging, stagingSchema), [], "animator staging schema drifted");
...
test("animator staging preserves observer framing from the accepted pdgedit assembly roles", () => {
  const builtStaging = buildAnimatorStagingContractFromPdgeditDocument(
    createSyntheticPassThruPdgeditDocument(),
    stagingOptions
  );
  const framingState = resolveAnimatorViewportFramingState(builtStaging.export.sceneDocument, 4, {
    start: 0,
    end: 24,
  });
...
test("animator staging still accepts pdgedit documents derived from publication graphs", () => {
...
test("animator pdgedit import runtime stays on the data side of the app boundary", () => {
  const runtimeSource = readText("src/apps/animator/AnimatorPdgeditImportRuntime.js");
