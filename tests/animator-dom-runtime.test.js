...
test("animator dom runtime collects animator shell elements and bindings", () => {
  ...
  elementMap.set("animator-recorded-output-diagnostics", { id: "recorded_output_diagnostics" });
  elementMap.set("animator-recorded-output-status", { id: "recorded_output_status" });
  ...
  assert.equal(dom.animatorRecordedOutputDiagnostics?.id, "recorded_output_diagnostics");
  assert.equal(dom.animatorRecordedOutputStatus?.id, "recorded_output_status");
  ...
  assert.equal(dom.animatorHudViewportToggleBindings[6]?.label, "Recorded Motion");
