...
test("binary UI removes no-op contours and preserves accessible shared transport", () => {
  ...
  assert.match(html, /name="topo-binary-background" value="purple" checked/u);
  assert.match(html, /name="topo-binary-background" value="white"/u);
  ...
  assert.match(runtime, /binaryWhite[\s\S]*\? "#ffffff"/u);
  assert.match(runtime, /dom\.binaryBackgroundControl\.hidden = !binaryMode/u);
  ...
  assert.match(runtime, /\^\(INPUT\|SELECT\|TEXTAREA\|BUTTON\|A\)\$/u);
  assert.match(runtime, /event\.target\?\.isContentEditable/u);
