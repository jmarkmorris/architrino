...
import {
  ...
  validateA11RootSheetTreatmentProtocol,
...
test("A1.1 root-sheet protocol locks targets, charts, resources, and null score", () => {
  const validated = validateA11RootSheetTreatmentProtocol(treatmentProtocol);
...
test("A1.1 root-sheet protocol rejects chart and resource drift", () => {
  ...
  assert.throws(
    () => validateA11RootSheetTreatmentProtocol(driftedChart),
  ...
  assert.throws(
    () => validateA11RootSheetTreatmentProtocol(driftedResource),
    /may not relax/,
  );
});
