...
} from "../scripts/neutral-swarm/octahedral-root-force-closure-witness.mjs";
...
test("octahedral root force closure witness rejects resolved positive root rows as sufficient closure", () => {
  ...
  assert.equal(artifact.schema, "neutral-swarm-octahedral-root-force-closure-witness/v1");
...
test("octahedral root force closure witness CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-root-force-closure-witness-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-root-force-closure-witness.mjs", import.meta.url)
  );
