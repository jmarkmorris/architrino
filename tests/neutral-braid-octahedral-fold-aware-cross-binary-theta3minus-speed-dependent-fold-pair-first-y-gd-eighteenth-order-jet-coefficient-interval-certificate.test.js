...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-eighteenth-order-jet-coefficient-interval-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-eighteenth-order-jet-coefficient-interval-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D eighteenth-order jet coefficient interval certificate validates", () => {
...
test("first-y eighteenth-order jet certificate imposes no fixed speed band", () => {
  const packet = artifact();

  assert.equal(
    packet.eighteenth_order_jet_parameters.speed_constraint,
    NO_SPEED_WINDOW
  );
  assert.deepEqual(packet.eighteenth_order_jet_parameters.speed_ratio_enclosure, [
    3.02156,
    3.02157,
  ]);
  assert.equal(packet.artifact_claim.assumes_fixed_speed_window, false);
  assert.equal(packet.eighteenth_order_jet_parameters.speed_band, undefined);
  assert.equal(packet.eighteenth_order_jet_parameters.speed_window, undefined);
  assert.equal(packet.eighteenth_order_jet_parameters.speed_min, undefined);
  assert.equal(packet.eighteenth_order_jet_parameters.speed_max, undefined);
});

test("first-y eighteenth-order jet intervals certify h18 and Q_G/Q_D y18 coefficients", () => {
  const packet = artifact();
  const summary = packet.eighteenth_order_jet_summary;
  const firstRow = packet.eighteenth_order_jet_rows[0];
  ...
  assert.ok(Number(summary.max_abs_source_equation_coeff_y0_to_y22_interval) < 1442425243);
  assert.ok(Number(summary.min_h18_solve_slope_clearance) > 0.79);
  ...
  assert.ok(Number(summary.max_abs_Q_D_y18_coefficient_interval) < 9105262932);
  ...
  assert.ok(Number(summary.max_abs_Q_D_plus_19Q_G_y18_coefficient_interval) < 18051118676);
  assert.ok(firstRow.Q_D_plus_19Q_G_y18_coefficient_interval[0] <= 0);
  assert.ok(firstRow.Q_D_plus_19Q_G_y18_coefficient_interval[1] >= 0);
  assert.equal(
    firstRow.row_status,
...
test("first-y eighteenth-order jet certificate keeps finite remainder closure open", () => {
  const packet = artifact();
...
test("first-y eighteenth-order jet validator rejects invalid controls and overclaims", () => {
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdEighteenthOrderJetCoefficientIntervalCertificate({
        speedCellCount: 2,
  ...
  packet.eighteenth_order_jet_parameters.speed_band = "forbidden";
...
test("first-y eighteenth-order jet CLI writes and validates artifact JSON", () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "theta3minus-first-y-eighteenth-order-")
  );
