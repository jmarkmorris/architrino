...
} from "../scripts/neutral-braid/octahedral-same-source-witness.mjs";
...
test("octahedral same-source witness excludes ordinary positive-delay roots", () => {
  ...
  assert.equal(artifact.schema, "neutral-braid-octahedral-same-source-witness/v1");
...
test("octahedral same-source witness CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-same-source-witness-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-same-source-witness.mjs", import.meta.url)
  );
