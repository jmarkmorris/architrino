...
const self=readFileSync("scripts/eom/launch-f6c-cached-root-cover-full.mjs");
const entryPath="scripts/eom/run-f6c-cached-root-cover-full.mjs";
...
function entrySource(mode) {
  ...
  return Buffer.from([
    ...
    "export async function initializeSourceBindings(){}", // Synthetic transport fixture only.
    ...
    " if(job.kind==='finalize'){"+(mode==="late-publication"?"const until=Date.now()+2500;while(Date.now()<until){};":"")+"return U.writeNew(path.join(job.output,'full-admission.json'),job.record);}",
    " if(job.kind==='admit'){",
...
async function runFixture(mode) {
  ...
  try {
    ...
    const operation=module.launchCaptured({root:dir,options:{output:path.join(lane,"attempt"),plan:path.join(dir,"fake-plan"),planSha256:"1".repeat(64),launcherSha256:digest(self),sourceMapSha256:"2".repeat(64)},
      self:{path:path.join(root,"scripts/eom/launch-f6c-cached-root-cover-full.mjs"),sha256:digest(self),bytes:self.length,data:self},
    ...
    if(["child-failure","lost-monitor","log-failure","startup-interruption"].includes(mode)) {
      await assert.rejects(operation);const reject=JSON.parse(readFileSync(path.join(dir,lane,"attempt/full-rejection.json")));
