...
test("the JS Xyzzy tile renderer stays aligned with representative committed reference SVG artifacts", () => {
  ...
  sampleTileKeys.forEach((tileKey) => {
    ...
    const referenceSemanticTree = toComparableSvgTree(parseSvgTree(readSvgFixture(`xyzzy-tile-${tileKey}.svg`)), {
      isRoot: true,
    });
...
test("the JS tile renderer can reconstruct representative committed Xyzzy review-group SVG semantics", () => {
  const catalog = normalizeXyzzyTileCatalog(readJson("src/apps/xyzzy/xyzzy-tiles.json"));
  ...
  sampleGroupKeys.forEach((groupKey) => {
    ...
    const referenceSemanticTree = toComparableSvgTree(parseSvgTree(readSvgFixture(`xyzzy-group-${groupKey}.svg`)), {
      isRoot: true,
    });
