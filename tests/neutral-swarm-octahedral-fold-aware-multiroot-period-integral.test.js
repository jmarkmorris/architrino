...
} from "../scripts/neutral-swarm/octahedral-fold-aware-multiroot-period-integral.mjs";
...
test("fold-aware multiroot integral CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-integral-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fold-aware-multiroot-period-integral.mjs", import.meta.url)
  );
