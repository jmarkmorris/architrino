...
test("Borg renders one source-carried axis row per binary and light-purple chart curves", () => {
  ...
  assert.equal(axisGroup.children.length, 3);
  assert.deepEqual(
    axisGroup.children.map((line) => line.userData.sourceBinaryId),
    ["I", "M", "O"],
  );
  assert.equal(axisGroup.children.every((line) => line.material.isLineDashedMaterial), true);
