...
test("pdgview timeline overlay runtime parses authored transfers", () => {
  const runtime = createPdgviewTimelineOverlayRuntime({
  ...
  const parsed = runtime.parsePdgviewTransfers(`
...
test("pdgview timeline overlay runtime reads timing state from pause and warp inputs", () => {
  const runtime = createPdgviewTimelineOverlayRuntime({
    dom: {
  ...
  const timing = runtime.readPdgviewTimingState();
...
test("pdgview timeline overlay runtime updates diagnostics using document markers", () => {
  ...
  const statusModels = runtime.updatePdgviewTimingDiagnostics(
    {
      scene: {
        markers: [{ id: "m1" }, { id: "m2" }],
        pauses: [{ id: "p1" }],
...
test("pdgview timeline overlay runtime resolves graphic targets and target radius", () => {
  const runtime = createPdgviewTimelineOverlayRuntime({
  ...
  assert.deepEqual(
    runtime.resolvePdgviewGraphicTargetPosition(
  ...
  const pathPoint = runtime.resolvePdgviewGraphicTargetPosition(
  ...
  assert.equal(
    runtime.getPdgviewAssemblyGraphicTargetRadius({
      members: [{ id: "a" }, { id: "b" }],
