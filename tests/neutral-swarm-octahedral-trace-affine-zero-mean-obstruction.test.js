...
} from "../scripts/neutral-swarm/octahedral-trace-affine-zero-mean-obstruction.mjs";
...
test("trace affine obstruction CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-trace-affine-obstruction-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-trace-affine-zero-mean-obstruction.mjs", import.meta.url)
  );
