...
test("b meson authored templates build explicit meson assemblies with bottom-quark occupancy", () => {
  ...
  const antiB0 = buildReactionParticipantStructure("bB0", {
    id: "bB0_structure",
...
test("b meson descriptor trees preserve explicit bottom-quark labels", () => {
  const descriptorTree = buildReactionStructureDescriptorTree(
    buildReactionParticipantStructure("dB0", {
      id: "dB0_structure",
...
test("reaction-flow export keeps neutral b meson identities distinct on the shared contract path", () => {
  ...
  assert.equal(document.participants[0]?.structureKey, "dB0");
  assert.equal(document.participants[1]?.structureKey, "bB0");
  assert(document.participants[0]?.tags.includes("dB0"));
  assert(document.participants[1]?.tags.includes("bB0"));
