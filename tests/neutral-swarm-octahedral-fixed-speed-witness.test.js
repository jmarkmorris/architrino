...
} from "../scripts/neutral-swarm/octahedral-fixed-speed-witness.mjs";
...
test("octahedral fixed-speed witness captures the receiver 1+ theta=0 residual", () => {
  ...
  assert.equal(artifact.schema, "neutral-swarm-octahedral-fixed-speed-witness/v1");
...
test("octahedral fixed-speed witness CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fixed-speed-witness-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fixed-speed-witness.mjs", import.meta.url)
  );
