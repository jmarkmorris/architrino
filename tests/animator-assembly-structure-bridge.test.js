...
} from "../src/runtime/AnimatorAssemblyStructureBridgeRuntime.js";

test("animator assembly bridge builds a canonical structure summary for a core assembly", () => {
  const assembly = {
    id: "assembly_1",
    name: "Assembly 1",
    core: {
  ...
  const structure = buildAnimatorAssemblyStructure(assembly);
...
test("animator assembly bridge formats a compact canonical summary string", () => {
  assert.equal(
    formatAnimatorAssemblyStructureSummary({
      nodeCount: 7,
      slotCount: 3,
...
test("animator assembly bridge formats a human-readable core status string", () => {
  assert.equal(
    formatAnimatorAssemblyStructureStatus({
      slotCount: 3,
      binarySlotCount: 3,
...
test("animator assembly bridge formats non-core and warning statuses readably", () => {
  assert.equal(
    formatAnimatorAssemblyStructureStatus({
      valid: true,
    }, {
...
test("animator assembly bridge includes shallow subassemblies and root members", () => {
  const assembly = {
    id: "assembly_2",
    name: "Assembly 2",
