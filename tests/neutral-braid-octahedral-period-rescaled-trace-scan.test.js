...
} from "../scripts/neutral-braid/octahedral-period-rescaled-trace-scan.mjs";
...
test("period-rescaled trace scan CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-period-rescaled-trace-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-period-rescaled-trace-scan.mjs", import.meta.url)
  );
