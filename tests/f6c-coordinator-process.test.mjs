...
test('known literal synthetic hook accepts only its declared completion',async()=>{
 ...
 const dir=mkdtempSync(path.join(lane,'closure-known-')),stdout=path.join(dir,'stdout.log');
...
test('current serial coordinator closes through the independently selected observer',async t=>{
 const lane=path.join(root,'.local-data/braid-analysis');mkdirSync(lane,{recursive:true});
 const dir=mkdtempSync(path.join(lane,'current-closure-'));
 ...
 const plan={schema:'braid-program/f6c-bounded-operation-plan.v1',root,
  ...
  sources:C.sourceUnion([bind(path.join(root,C.CONTROLS)),node,bind('/bin/ps'),bind('/usr/bin/memory_pressure'),supervisor]),
  hookModule:bind(hookPath),hookControls:bind(path.join(root,'tests/f6c-coordinator-process.test.mjs')),
 ...
 const args=['--control','--plan',planPath,'--plan-sha256',bind(planPath).sha256,
  ...
  '--out-directory',out,'--owner-task',process.env.CODEX_SESSION_ID??'current-closure-controls'];
 ...
 assert.deepEqual(receipt.invocation.coordinator,coordinator);
 assert(!existsSync(path.join(root,LOCK)));
 for(const flag of ['--coordinator-sha256']){
