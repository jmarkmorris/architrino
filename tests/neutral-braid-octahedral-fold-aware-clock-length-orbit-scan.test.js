...
} from "../scripts/neutral-braid/octahedral-fold-aware-clock-length-orbit-scan.mjs";
...
test("fold-aware clock length orbit scan CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-fold-aware-clock-orbit-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-fold-aware-clock-length-orbit-scan.mjs", import.meta.url)
  );
