...
} from "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-quarter-profile-certificate.mjs";
...
test("fold-aware cross-binary quarter profile CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-cross-quarter-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-quarter-profile-certificate.mjs",
      import.meta.url
    )
  );
