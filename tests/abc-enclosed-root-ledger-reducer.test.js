...
import {ABC_CLI_PATH,abcApiSpeedBox,abcExactDecimal,abcOriginalJson,assertABCPhaseSequence,
  inspectABCRowForTests,reduceABCPhaseSnapshot,inspectABCRoundingForTests,inspectABCNDJSONForTests,abcSha256} from "../src/prescribed-path-analysis/ABCEnclosedRootLedgerReducer.mjs";
...
test("exact decimal comparisons preserve distinctions below binary64 precision",()=>{
  const a=abcExactDecimal("1.00000000000000000000000000000000000000001"),b=abcExactDecimal("1");
  assert.equal(fractionCompare(a,b),1);assert.equal(fractionCompare(abcExactDecimal("1e-8"),abcExactDecimal("0.00000001")),0);
  for(const token of [NaN,1,"NaN","Infinity","1e9999","bad"])assert.throws(()=>abcExactDecimal(token));
});
test("independent API speed enclosure covers a stationary polynomial and positive error radii",()=>{
  const result=abcApiSpeedBox(segment());assert.equal(result.strictlySubField,true);
...
test("constant 3-4-5 tangent control has the independently known half-unit speed",()=>{
  const result=abcApiSpeedBox(segment(["0.3","0.4","0"]));assert.equal(result.strictlySubField,true);
...
test("strict unit-speed gate includes the actual positive velocity error boxes",()=>{
  assert.equal(abcApiSpeedBox(segment(["1","0","0"])).strictlySubField,false);
  assert.equal(abcApiSpeedBox(segment(["2","0","0"])).strictlySubField,false);
});
test("cubic derivative bound uses local time, not global absolute time",()=>{
  const row=segment();row.coefficients[0]=["0","0","1","0"];
  const result=abcApiSpeedBox(row);assert.ok(Number(result.normUpper)>0.004);assert.ok(Number(result.normUpper)<0.004002);
});
test("dyadic lifts near zero remain outward enclosures, never silently exact fixed-point points",()=>{
  const row=segment(["1e-300","0","0"]);const result=abcApiSpeedBox(row);
...
test("invalid or overflowed carrier inputs fail closed",()=>{
  for(const token of ["Infinity","1e400","1e-400"]){const row=segment([token,"0","0"]);assert.throws(()=>abcApiSpeedBox(row));}
  const row=segment();row.velocityErrors[1]="0";assert.throws(()=>abcApiSpeedBox(row),/cubic\/error/u);
});
test("outward fixed-point operations enclose actual binary64 cancellation, products and subnormals",()=>{
  ...
  for(const [a,b] of [["0.1","0.2"],["-0.1","0.2"],["1","-1"],["1e-300","-1e-300"],["0","0"]])for(const operation of ["add","subtract","multiply"]){
    const result=inspectABCRoundingForTests(a,b,operation),x=Number(a),y=Number(b),z=operation==="add"?x+y:operation==="subtract"?x-y:x*y;
    assert.equal(result.accepted,false);assert.ok(fractionCompare(abcExactDecimal(result.interval[0]),ratio(adjacent(z,-1)))<=0);assert.ok(fractionCompare(abcExactDecimal(result.interval[1]),ratio(adjacent(z,1)))>=0);
  }
});
test("original JSON rejects duplicate keys, invalid UTF8 and excessive nesting",()=>{
  assert.throws(()=>abcOriginalJson(Buffer.from('{"a":1,"a":2}')),/duplicate/u);
  assert.throws(()=>abcOriginalJson(Buffer.from([0xff])));
  assert.throws(()=>abcOriginalJson(Buffer.from("[".repeat(34)+"0"+"]".repeat(34))),/nesting/u);
});
...
function control(self=false,bits=53){
  const manifestId="abc-circular-history/v1:a1-1:T=4",members=[0,1].map(index=>({index,constituentId:`c${index}`,worldlineId:`w${index}`,polarity:index? -1:1,historyId:`${manifestId}/w${index}`,historyFingerprint:`f${index}`,segments:[{tStart:"2",tEnd:"4"}]}));
  ...
  const row={schema:"braid-program/abc-enclosed-root-row.v1",candidateId:"a1-1",rung:2,phase:0,manifestId,historyManifestSha256:H,conformanceSha256:H,sourceBinding:{path:"not-an-actual-source",sha256:H},receiverIndex:0,transmitterIndex:index,
  ...
  const context={manifest:{candidateId:"a1-1",members,sourceBinding:row.sourceBinding},proof:{manifestSha256:H},sourceRow:{vUpper:"0.5"},speedBounds:members.map(()=>({normUpper:"0.6"})),options:{conformanceSha256:H},rung:2,phase:0,reception:"4",lower:"2",manifestId};
  return {row,context,index};
}
const inspect=fixture=>inspectABCRowForTests(fixture.row,fixture.context,fixture.index);
...
test("ordinary cached imports cannot produce accepted phase receipts",async()=>{
  await assert.rejects(reduceABCPhaseSnapshot({reducerBytes:Buffer.from("not actual code")}),/captured executing/u);
});

function phaseHeaders(){return [0,1].map(phase=>({candidateId:"a1-1",rung:2,phase,receptionTime:String(4+2*phase),historyManifest:{sha256:H},members:[],sourceBinding:{path:"control",sha256:H},rows:[]}));}
test("summary scopes reject missing/duplicate phases and do not invent the cohort",()=>{
  assert.equal(assertABCPhaseSequence(phaseHeaders(),"candidate-rung"),undefined);
  assert.throws(()=>assertABCPhaseSequence(phaseHeaders().slice(0,1),"candidate-rung"),/census/u);
  assert.throws(()=>assertABCPhaseSequence(phaseHeaders(),"pilot"),/census/u);
  assert.throws(()=>assertABCPhaseSequence(phaseHeaders(),"candidate-ladder"),/census/u);
});
test("all repeated receptions across a genuine ladder require identical carriers and overlapping roots",()=>{
  const receipts=[8,32,128].flatMap(rung=>Array.from({length:rung},(_,phase)=>({candidateId:"a1-1",rung,phase,receptionTime:String(4+4*phase/rung),historyManifest:{sha256:H},members:[],sourceBinding:{path:"control",sha256:H},rows:[{receiverIndex:0,transmitterIndex:1,roots:[{lower:"3",upper:"3.000000001"}]}]})));
  assert.equal(assertABCPhaseSequence(receipts,"candidate-ladder"),undefined);
  receipts[40].rows[0].roots[0]={lower:"3.1",upper:"3.2"};assert.throws(()=>assertABCPhaseSequence(receipts,"candidate-ladder"),/disjoint/u);
});
test("CLI requires authenticated receipt hashes and never overwrites existing output",()=>{
  const directory=mkdtempSync(path.join(tmpdir(),"abc-ledger-negative-")),output=path.join(directory,"existing.json");writeFileSync(output,"preserve\n");
  const command=[path.join(ROOT,ABC_CLI_PATH),"--repo-root",ROOT,"--history-manifest",path.join(directory,"missing"),"--conformance",path.join(directory,"proof"),"--conformance-sha256",H,"--rows",path.join(directory,"rows"),"--build-receipt",path.join(directory,"build"),"--build-receipt-sha256",H,"--out",output];
  ...
  const missing=spawnSync(process.execPath,[path.join(ROOT,ABC_CLI_PATH),"--out",path.join(directory,"new.json")],{encoding:"utf8",timeout:15000});assert.equal(missing.status,1);assert.match(missing.stderr,/needs/u);
});
test("NDJSON byte hash spans chunks without dropping large cell arrays",async()=>{
  const directory=mkdtempSync(path.join(tmpdir(),"abc-ledger-stream-")),filename=path.join(directory,"rows.ndjson");
  ...
  writeFileSync(filename,bytes);const result=await inspectABCNDJSONForTests(filename);assert.equal(result.accepted,false);assert.equal(result.raw.sha256,abcSha256(bytes));assert.equal(result.raw.bytes,bytes.length);assert.equal(result.raw.rowCount,2);
});
test("NDJSON partial lines and empty rows cannot count as flushed scientific rows",async()=>{
  const directory=mkdtempSync(path.join(tmpdir(),"abc-ledger-partial-"));
  for(const [index,text]of ["{}","{}\n\n"].entries()){const filename=path.join(directory,`${index}.ndjson`);writeFileSync(filename,text);await assert.rejects(inspectABCNDJSONForTests(filename));}
});
