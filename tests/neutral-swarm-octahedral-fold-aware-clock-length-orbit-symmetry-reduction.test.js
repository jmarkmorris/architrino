...
} from "../scripts/neutral-swarm/octahedral-fold-aware-clock-length-orbit-symmetry-reduction.mjs";
...
test("fold-aware clock length orbit symmetry reduction CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-clock-orbit-symmetry-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-swarm/octahedral-fold-aware-clock-length-orbit-symmetry-reduction.mjs",
      import.meta.url
    )
  );
