...
import {
  EVOLVED_FAMILY_C_US_PROTOCOL_SCHEMA,
  computeSameRowUsMoments,
  evaluateEvolvedFamilyCUsRelationship,
} from "../src/eom-analysis/EvolvedFamilyCUsRelationship.mjs";
import {
...
import {
  EVOLVED_FAMILY_C_US_STUDY_SCHEMA,
  evaluateEvolvedFamilyCUsStudy,
} from "../scripts/eom/analyze-evolved-family-c-us-relationship.mjs";
...
function recordFixture(overrides = {}) {
  const worldlines = Array.from({ length: 12 }, (_, index) => {
    ...
    return {
      ...
      historyFingerprint: `evolved-family-c-fixture-${index + 1}`,
  ...
  return {
    ...
    provenance: {
      engineId: "eom-solver",
      engineVersion: "eom-family-c-test-build",
      runId: "evolved-family-c-test-run",
      claimGrade: "evolved-record",
      evidenceStatus: "executable_architecture_evidence",
      generatingSpec:
        "reference/priorities/braid-program/campaigns/future-family-c.md",
      date: "2026-07-24",
    },
    window: {
      start: 0,
...
function protocolFixture(overrides = {}) {
  return {
    schema: EVOLVED_FAMILY_C_US_PROTOCOL_SCHEMA,
    protocolId: "evolved-family-c-us-test-v1",
    familyId: "C",
    memberId: "C-test",
    fieldSpeed: 1,
...
test("evolved Family-C evaluator computes exact finite and far ratios with refinement", () => {
  const result = evaluateEvolvedFamilyCUsRelationship({
    record: recordFixture(),
    protocol: protocolFixture(),
  });

  assert.equal(result.schema, "eom-analysis/evolved-family-c-us-result.v1");
...
test("evolved Family-C evaluator fails closed on ineligible record authority and geometry", () => {
...
test("evolved Family-C evaluator rejects field-speed and pre-clearance roots", () => {
...
test("study binds a complete collapse/refinement cohort and pools same-row results", () => {
  const directory = mkdtempSync(path.join(os.tmpdir(), "evolved-family-c-us-"));
  ...
  for (const prehistoryId of ["P0", "P1", "P2"]) {
    for (const refinementId of ["R1", "R2"]) {
      ...
      record.provenance.runId = `family-c-${prehistoryId}-${refinementId}`;
  ...
  const study = {
    schema: EVOLVED_FAMILY_C_US_STUDY_SCHEMA,
    studyId: "family-c-us-study-test",
    branchId: "family-c-test-branch",
    protocol: protocolFixture({
      rowCount: 1,
      radii: [1],
      quadrature: {
  ...
  const result = evaluateEvolvedFamilyCUsStudy({
    study,
