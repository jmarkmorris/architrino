...
} from "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-octic-jet-coefficient-interval-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-octic-jet-coefficient-interval-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D octic jet coefficient interval certificate validates", () => {
...
test("first-y octic jet certificate imposes no fixed speed band", () => {
  const packet = artifact();

  assert.equal(packet.octic_jet_parameters.speed_constraint, NO_SPEED_WINDOW);
  assert.deepEqual(packet.octic_jet_parameters.speed_ratio_enclosure, [
    3.02156,
    3.02157,
  ]);
  assert.equal(packet.artifact_claim.assumes_fixed_speed_window, false);
  assert.equal(packet.octic_jet_parameters.speed_band, undefined);
  assert.equal(packet.octic_jet_parameters.speed_window, undefined);
  assert.equal(packet.octic_jet_parameters.speed_min, undefined);
  assert.equal(packet.octic_jet_parameters.speed_max, undefined);
});

test("first-y octic jet intervals certify h8 and octic Q_G/Q_D coefficients", () => {
  const packet = artifact();
  const summary = packet.octic_jet_summary;
  const firstRow = packet.octic_jet_rows[0];
  ...
  assert.equal(summary.all_source_equation_coefficients_contain_zero_y0_to_y12, true);
  assert.ok(Number(summary.max_abs_source_equation_coeff_y0_to_y12_interval) < 26);
  ...
  assert.ok(Number(summary.max_Q_G_octic_loss_on_first_y_cell) < 9e-20);
  assert.ok(Number(summary.max_Q_D_octic_loss_on_first_y_cell) < 7e-19);
  ...
  assert.ok(Number(summary.min_Q_G_remaining_ninth_order_tail_budget) > 4e23);
  assert.ok(Number(summary.min_Q_D_remaining_ninth_order_tail_budget) > 4e23);
  ...
  assert.equal(firstRow.row_status, "directed-rounded-first-y-GD-octic-jet-coefficient-enclosed");
});

test("first-y octic jet certificate keeps finite remainder closure open", () => {
  const packet = artifact();
...
test("first-y octic jet validator rejects invalid controls and overclaims", () => {
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdOcticJetCoefficientIntervalCertificate({
        speedCellCount: 2,
  ...
  packet.octic_jet_parameters.speed_band = [0.5, 1.5];
...
test("first-y octic jet CLI writes and validates artifact JSON", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "theta3minus-first-y-octic-"));
