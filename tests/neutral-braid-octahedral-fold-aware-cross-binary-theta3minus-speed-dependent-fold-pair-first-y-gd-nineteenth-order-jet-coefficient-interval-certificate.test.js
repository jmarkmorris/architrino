...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-nineteenth-order-jet-coefficient-interval-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-nineteenth-order-jet-coefficient-interval-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D nineteenth-order jet coefficient interval certificate validates", () => {
...
test("first-y nineteenth-order jet certificate imposes no fixed speed band", () => {
  const packet = artifact();

  assert.equal(
    packet.nineteenth_order_jet_parameters.speed_constraint,
    NO_SPEED_WINDOW
  );
  assert.deepEqual(packet.nineteenth_order_jet_parameters.speed_ratio_enclosure, [
    3.02156,
    3.02157,
  ]);
  assert.equal(packet.artifact_claim.assumes_fixed_speed_window, false);
  assert.equal(packet.nineteenth_order_jet_parameters.speed_band, undefined);
  assert.equal(packet.nineteenth_order_jet_parameters.speed_window, undefined);
  assert.equal(packet.nineteenth_order_jet_parameters.speed_min, undefined);
  assert.equal(packet.nineteenth_order_jet_parameters.speed_max, undefined);
});

test("first-y nineteenth-order jet intervals certify h19 and Q_G/Q_D y19 coefficients", () => {
  const packet = artifact();
  const summary = packet.nineteenth_order_jet_summary;
  const firstRow = packet.nineteenth_order_jet_rows[0];
  ...
  assert.ok(Number(summary.max_abs_source_equation_coeff_y0_to_y23_interval) < 8607321786);
  assert.ok(Number(summary.min_h19_solve_slope_clearance) > 0.79);
  ...
  assert.ok(Number(summary.max_abs_Q_D_y18_coefficient_interval) < 9105262933);
  assert.ok(Number(summary.max_abs_Q_G_y19_coefficient_interval) < 2834624758);
  assert.ok(Number(summary.max_abs_Q_D_y19_coefficient_interval) < 56692495150);
  ...
  assert.ok(Number(summary.max_abs_Q_D_plus_19Q_G_y18_coefficient_interval) < 18051118679);
  assert.ok(Number(summary.max_abs_Q_D_plus_20Q_G_y19_coefficient_interval) < 113384990299);
  assert.ok(firstRow.Q_D_plus_20Q_G_y19_coefficient_interval[0] <= 0);
  assert.ok(firstRow.Q_D_plus_20Q_G_y19_coefficient_interval[1] >= 0);
  assert.equal(
    firstRow.row_status,
...
test("first-y nineteenth-order jet certificate keeps finite remainder closure open", () => {
  const packet = artifact();
...
test("first-y nineteenth-order jet validator rejects invalid controls and overclaims", () => {
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdNineteenthOrderJetCoefficientIntervalCertificate({
        speedCellCount: 2,
  ...
  packet.nineteenth_order_jet_parameters.speed_band = "forbidden";
...
test("first-y nineteenth-order jet CLI writes and validates artifact JSON", () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "theta3minus-first-y-nineteenth-order-")
  );
