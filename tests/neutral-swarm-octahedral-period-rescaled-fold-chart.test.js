...
} from "../scripts/neutral-swarm/octahedral-period-rescaled-fold-chart.mjs";
...
test("period-rescaled fold chart CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-period-fold-chart-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-period-rescaled-fold-chart.mjs", import.meta.url)
  );
