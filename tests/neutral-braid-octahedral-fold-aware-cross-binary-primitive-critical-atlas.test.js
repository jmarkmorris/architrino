...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-primitive-critical-atlas.mjs";
...
test("fold-aware primitive critical atlas CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "neutral-braid-critical-atlas-")
  );
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-primitive-critical-atlas.mjs",
      import.meta.url
    )
  );
