...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-eighth-order-post-u-successor-coefficient-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-eighth-order-post-u-successor-coefficient-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D thirty-eighth-order post-U successor coefficient certificate validates", () => {
...
test("thirty-eighth-order post-U successor coefficient certificate imposes no fixed speed band", () => {
  const packet = artifact();
  const params =
    packet.thirty_eighth_order_post_u_successor_coefficient_parameters;
...
test("thirty-eighth-order post-U successor coefficient intervals certify h38 and Q_G/Q_D y38 coefficients", () => {
  const packet = artifact();
  const summary =
    packet.thirty_eighth_order_post_u_successor_coefficient_summary;
  const firstRow =
  ...
  assert.equal(summary.predecessor_h37_artifact_valid, true);
  assert.equal(summary.all_rows_certified, true);
  assert.equal(
    summary.all_source_equation_coefficients_contain_zero_y0_to_y42,
    true
  );
  assert.ok(
    Number(summary.max_abs_source_equation_coeff_y0_to_y42_interval) < 4.9e24
  );
  assert.ok(Number(summary.min_h38_solve_slope_clearance) > 0.79);
  assert.deepEqual(summary.h38_interval_hull, [
    -1.85548531397e24,
    1.85548531397e24,
  ]);
  assert.deepEqual(summary.Q_G_y38_coefficient_interval_hull, [
    -1.58063981796e24,
    1.58063984386e24,
  ]);
  assert.deepEqual(summary.Q_D_y38_coefficient_interval_hull, [
    -6.16449539105e25,
    6.16449529005e25,
  ]);
  assert.ok(Number(summary.max_abs_h38_interval) < 1.9e24);
  assert.ok(Number(summary.max_abs_Q_G_y38_coefficient_interval) < 1.6e24);
  assert.ok(Number(summary.max_abs_Q_D_y38_coefficient_interval) < 6.2e25);
  assert.ok(Number(summary.max_Q_G_y38_loss_on_first_y_cell) < 8e-81);
  assert.ok(Number(summary.max_Q_D_y38_loss_on_first_y_cell) < 3e-79);
  assert.equal(
    summary.all_QD_QG_coefficient_identity_intervals_contain_zero,
    true
  );
  assert.ok(
    Number(summary.max_abs_Q_D_plus_39Q_G_y38_coefficient_interval) < 1.3e26
  );
  assert.ok(firstRow.Q_D_plus_39Q_G_y38_coefficient_interval[0] <= 0);
  assert.ok(firstRow.Q_D_plus_39Q_G_y38_coefficient_interval[1] >= 0);
  assert.ok(
    Number(summary.min_Q_G_remaining_thirty_ninth_order_tail_budget) > 1e106
  );
  assert.ok(
    Number(summary.min_Q_D_remaining_thirty_ninth_order_tail_budget) > 1e106
  );
  assert.equal(
    firstRow.row_status,
...
test("thirty-eighth-order post-U successor coefficient validator rejects speed-band fields and overclaims", () => {
  const packet = clone(artifact());
  packet.thirty_eighth_order_post_u_successor_coefficient_parameters.speed_band =
    "forbidden";
...
test("thirty-eighth-order post-U successor coefficient CLI writes and validates artifact JSON", () => {
