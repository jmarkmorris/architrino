...
test('captured entry rejects invalid profile and promptly closes startup guard before any workload',async()=>{
 ...
 try {
  ...
  const child=spawn(process.execPath,[entry,'--profile',profile,'--profile-sha256',sha(bytes),'--out','.local-data/braid-analysis/subfield-circular-root-pilot-20260827-v1/invalid-control'],{stdio:['ignore','pipe','pipe']});
