...
function staticSixSourceRecord() {
  ...
  return {
    ...
    recordId: "b1-streaming-independent-static-six-source",
    ...
    taxonomy: { familyId: "B", memberId: "B1" },
...
test("independent static closed form fixes exposure, wake norm, monopole power, and radial exponents", () => {
  ...
  const radialRows = computeB1RadialScalingRows(surfaces);
...
test("streaming acceptance independently fails closed on every event-level validity obligation", () => {
  ...
  for (const defect of defects) {
    assert.throws(() => evaluateB1StreamingSurfaceReductions({
      sourceRecord,
      completeCycleProtocol: protocol,
      evaluate(request) {
