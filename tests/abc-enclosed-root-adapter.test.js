...
import { inspectAbcHistoryStructureForTests } from "../scripts/eom/verify-abc-circular-history.mjs";
...
const binary = process.env.AAA_ABC_ADAPTER_BIN && path.resolve(process.env.AAA_ABC_ADAPTER_BIN);
const lane = path.join(root, ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1");
...
function structure(bytes, rung, phase) {
  const result = inspectAbcHistoryStructureForTests(bytes, rung, phase);
...
test("ABC subject help and syntax controls require a fresh explicitly selected binary", gate, () => {
  assert.ok(statSync(binary).mtimeMs >= statSync(path.join(root, "src/eom/native/eom_abc_enclosed_root_cli.cpp")).mtimeMs);
  mkdirSync(lane, { recursive: true });
  directory = mkdtempSync(path.join(lane, "adapter-controls."));
  assert.equal(run(["--help"]).status, 0);
  assert.notEqual(run([]).status, 0);
  assert.notEqual(run(args("manifest", "a1-1", 16, 0, path.join(directory, "bad-rung.json"))).status, 0);
  assert.notEqual(run([...args("manifest", "a1-1", 2, 0, path.join(directory, "caller-hash.json")), "--source-hash", "invented"]).status, 0);
});

test("six-member actual manifest preserves exact grid, source order, errors, endpoint bits and EOM fingerprints", gate, () => {
  firstPath = path.join(directory, "a1-1-T4.json");
  const result = run(args("manifest", "a1-1", 2, 0, firstPath));
  ...
  assert.equal(manifest.manifestId, "abc-circular-history/v1:a1-1:T=4");
...
test("repeated reception across rungs has identical original manifest bytes", gate, () => {
  const output = path.join(directory, "a1-1-T4-rung128.json");
  const result = run(args("manifest", "a1-1", 128, 0, output));
  assert.equal(result.status, 0, result.stderr);
...
test("twelve-member arbitrary-vector candidate preserves final pilot reception and full coverage", gate, () => {
  const output = path.join(directory, "c3-T7.96875.json");
  const result = run(args("manifest", "c3", 128, 127, output));
  assert.equal(result.status, 0, result.stderr);
...
test("existing output cannot be overwritten and semantic-only manifest equality is insufficient", gate, () => {
  assert.notEqual(run(args("manifest", "a1-1", 2, 0, firstPath)).status, 0);
  ...
  const result = run([...args("serve", "a1-1", 2, 0, path.join(directory, "reformatted-rejected.ndjson")),
...
test("unaccepted conformance receipt cannot open the root execution path", gate, () => {
  ...
  writeFileSync(proof, JSON.stringify({ schema: "braid-program/abc-circular-history-conformance.v1", accepted: false }), { flag: "wx" });
  const result = run([...args("serve", "a1-1", 2, 0, path.join(directory, "proof-rejected.ndjson")),
...
test("FIFO manifest without a writer rejects without entering a blocking read or root call", gate, () => {
  ...
  const result = run([...args("serve", "a1-1", 2, 0, path.join(directory, "fifo-rejected.ndjson")),
