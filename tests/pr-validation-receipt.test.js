...
test("publication dispatch runs ordinary validation without a migration trial", async () => {
  const { runValidationCommands, VALIDATION_COMMANDS } = await import('../scripts/pr-validation-receipt.mjs');
  const invoked = [];
  runValidationCommands({ spawn: (_node, args) => {
    invoked.push(args[0]);
    return { status: 0 };
  } });
  assert.deepEqual(invoked, ['scripts/prepare-runtime-assets.mjs', 'scripts/check-foundational-impact.mjs', 'scripts/check-content-integrity.mjs', 'scripts/check-animator-runtime-wiring.mjs']);
  assert.equal(VALIDATION_COMMANDS.some(c => c.reportOnly), false);
  assert.throws(() => runValidationCommands({ spawn: () => ({ status: 1 }) }), /Prepare ignored runtime assets failed/);
});
