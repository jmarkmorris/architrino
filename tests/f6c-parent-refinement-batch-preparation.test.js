...
function fixture(){
 ...
 const operationalBindings=[B.SELF,B.CONTROL,...B.OPERATIONAL_PATHS,realpathSync(process.execPath),'/bin/ps','/usr/bin/memory_pressure'].map(p=>bind(path.resolve(root,p)));
 const originalBindings=Object.fromEntries(Object.entries({...B.ORIGINAL,...Object.fromEntries(Object.entries(B.ORIGINAL_PATHS).map(([k,p])=>[k,[p]]))}).map(([k,[p,h,n]])=>[k,originalBind(p,h,n)]));
 const template={schema:'braid-program/f6c-parent-emission-refinement-launch.v1',scope:B.parentScope(2),parentIndex:2,...Object.fromEntries(Object.entries(B.NAMED).map(([k,p])=>[k,bind(p,'a'.repeat(64))])),dependencies:Object.fromEntries(Object.entries(B.DEPENDENCIES).map(([k,p])=>[k,bind(p)])),originalBindings,acceptanceOwner:bind('reference/priorities/braid-program/evidence/2026-08-27-braid-search-launch-readiness.md'),priorCoverClosure:{authority:'versioned-acceptance-owner-declaration-not-fresh-observation',originalCallerSession:'13512',finalCompletionChunk:'c21aa7',exitCode:0,elapsedSeconds:'862.951823625',processesClosed:true,independentAuditAccepted:true},runtimeBindings,operationalBindings,limits:structuredClone(B.LIMITS)};
 const c={schema:'braid-program/f6c-parent-refinement-batch-preparation.v3',root,template:bind('/synthetic/template.json'),coordinator:bind(path.join(root,P.COORDINATOR)),composition:operationalBindings[0],compositionControls:operationalBindings[1],preparationControls:bind(path.join(root,P.CONTROLS)),observer:bind(path.join(root,P.OBSERVER)),closureChecker:bind(path.join(root,P.CLOSURE_CHECKER)),closureControls:bind(path.join(root,P.CLOSURE_CONTROLS)),ownerTask:'synthetic-owner',sources:[],sourceIdentities:{},runtimeBindings,operationalBindings,acceptanceOwner:abs(template.acceptanceOwner),pythonCommand,git,operationDirectory:path.join(root,'.local-data/braid-analysis/synthetic-batch-operation'),parents:[3,4,5].map(parentIndex=>({parentIndex,output:path.join(root,B.LANE,'pilot-parent-'+parentIndex+'-synthetic-preparation'),producerMaximumBytes:8388608,comparisonMaximumBytes:4194304})),closureReserveBytes:4194304};
 ...
 c.sources=C.sourceUnion([bind(path.join(root,P.SUPERVISOR)),c.template,c.coordinator,c.composition,c.compositionControls,c.preparationControls,c.observer,c.closureChecker,c.closureControls,c.acceptanceOwner,...named,...dependencies,...originals,...runtimeBindings,...operationalBindings,...B.historicalPhysicalSources(historical,c),...logs]);
...
test('exact CLI flags and hashes; captured imports do not execute CLI',()=>{
 const a=['--configuration','/a','--configuration-sha256','a'.repeat(64),'--self-sha256','b'.repeat(64),'--out-directory','/b','--coordinator-sha256','c'.repeat(64)];assert.equal(P.parseArguments(a).outDirectory,'/b');
...
test('pure plan preparation yields original3/4/5 tokens and40MiB commitment',()=>{
 ...
 assert.equal(r.batch.configuration.completionContract,'braid-program/f6c-bounded-operation-external-closure.v2');
 assert.equal(r.invocation[1],f.configuration.observer.path);
 assert.equal(r.invocation[r.invocation.indexOf('--checker-sha256')+1],f.configuration.closureChecker.sha256);
 assert.equal(r.invocation[r.invocation.indexOf('--owner-task')+1],f.configuration.ownerTask);
...
test('closed source/configuration/identity/schema/census mutations reject',()=>{
 const mutations=[f=>f.configuration.sources=f.configuration.sources.filter(b=>b.path!==path.join(root,P.SUPERVISOR)),f=>f.configuration.extra=1,f=>f.configuration.parents.reverse(),f=>f.configuration.parents[0].parentIndex=true,f=>delete f.configuration.sourceIdentities[f.configuration.sources[0].path],f=>f.configuration.sourceIdentities[f.configuration.sources[0].path]='1:2',f=>f.configuration.coordinator.sha256='b'.repeat(64),f=>f.configuration.sources.pop(),f=>f.admission.sourceBindings.pop(),f=>f.configuration.parents[0].producerMaximumBytes=67108864,f=>f.configuration.runtimeBindings.push(bind('/undeclared')),f=>f.template.proposalReference.path='foreign.py'];
