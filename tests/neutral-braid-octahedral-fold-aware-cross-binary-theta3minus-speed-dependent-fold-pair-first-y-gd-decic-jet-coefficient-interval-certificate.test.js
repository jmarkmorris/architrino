...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-decic-jet-coefficient-interval-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-decic-jet-coefficient-interval-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D decic jet coefficient interval certificate validates", () => {
...
test("first-y decic jet certificate imposes no fixed speed band", () => {
  const packet = artifact();

  assert.equal(packet.decic_jet_parameters.speed_constraint, NO_SPEED_WINDOW);
  assert.deepEqual(packet.decic_jet_parameters.speed_ratio_enclosure, [
    3.02156,
    3.02157,
  ]);
  assert.equal(packet.artifact_claim.assumes_fixed_speed_window, false);
  assert.equal(packet.decic_jet_parameters.speed_band, undefined);
  assert.equal(packet.decic_jet_parameters.speed_window, undefined);
  assert.equal(packet.decic_jet_parameters.speed_min, undefined);
  assert.equal(packet.decic_jet_parameters.speed_max, undefined);
});

test("first-y decic jet intervals certify h10 and decic Q_G/Q_D coefficients", () => {
  const packet = artifact();
  const summary = packet.decic_jet_summary;
  const firstRow = packet.decic_jet_rows[0];
  ...
  assert.equal(summary.all_source_equation_coefficients_contain_zero_y0_to_y14, true);
  assert.ok(Number(summary.max_abs_source_equation_coeff_y0_to_y14_interval) < 900);
  ...
  assert.equal(firstRow.row_status, "directed-rounded-first-y-GD-decic-jet-coefficient-enclosed");
});

test("first-y decic jet certificate keeps finite remainder closure open", () => {
  const packet = artifact();
...
test("first-y decic jet validator rejects invalid controls and overclaims", () => {
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdDecicJetCoefficientIntervalCertificate({
        speedCellCount: 2,
  ...
  packet.decic_jet_parameters.speed_band = "forbidden";
...
test("first-y decic jet CLI writes and validates artifact JSON", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "theta3minus-first-y-decic-"));
