...
const directory = () => realpathSync(mkdtempSync(path.join(os.tmpdir(), "subfield-circular-pilot-control-")));
...
test("phase receipt plumbing demands original raw/proof/build hashes and exact pair census", () => {
  ...
  const receipt = { ...expected, ...bindings, schema: "braid-program/subfield-circular-root-phase-reduction.v1", accepted: true,
...
test("summary plumbing requires all32 authenticated ordered phase receipts", () => {
  ...
  const receipt = { schema: "braid-program/subfield-circular-root-summary-reduction.v1", accepted: true,
