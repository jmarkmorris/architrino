...
import { buildXyzzyDocumentValidation } from "../src/apps/xyzzy/XyzzyDocumentRuntime.js";
...
test("xyzzy example matches the xyzzy/v1 schema", () => {
  const schema = readJson("src/contracts/xyzzy/v1/schema.json");
  const example = readJson("content/contracts/examples/xyzzy/free_neutron_beta.v1.json");
  const errors = validateAgainstSchema(example, schema);

  assert.deepEqual(errors, []);
});

test("xyzzy example passes relational strip validation", () => {
  const example = readJson("content/contracts/examples/xyzzy/free_neutron_beta.v1.json");
  const validation = buildXyzzyDocumentValidation(example);
