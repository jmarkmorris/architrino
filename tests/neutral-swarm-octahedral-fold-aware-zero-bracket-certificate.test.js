...
} from "../scripts/neutral-swarm/octahedral-fold-aware-zero-bracket-certificate.mjs";
...
test("fold-aware zero bracket certificate CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-zero-certificate-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fold-aware-zero-bracket-certificate.mjs", import.meta.url)
  );
