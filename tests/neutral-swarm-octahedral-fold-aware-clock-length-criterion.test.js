...
} from "../scripts/neutral-swarm/octahedral-fold-aware-clock-length-criterion.mjs";
...
test("fold-aware clock length criterion CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-clock-length-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fold-aware-clock-length-criterion.mjs", import.meta.url)
  );
