...
} from "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-critical-value-atlas.mjs";
...
test("fold-aware critical value atlas CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "neutral-swarm-critical-values-")
  );
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-critical-value-atlas.mjs",
      import.meta.url
    )
  );
