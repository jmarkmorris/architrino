...
} from "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-forcing-derivative-atlas.mjs";
...
test("fold-aware forcing derivative atlas CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "neutral-swarm-forcing-derivative-")
  );
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-forcing-derivative-atlas.mjs",
      import.meta.url
    )
  );
