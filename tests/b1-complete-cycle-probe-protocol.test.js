...
test("declared B1 protocol covers one complete retained-history cycle and fixes event counts", () => {
  const protocol = validateB1CompleteCycleProbeProtocol(loadProtocol());
  const summary = summarizeB1CompleteCycleProbeProtocol(protocol);

  assert.equal(protocol.schema, B1_COMPLETE_CYCLE_PROBE_PROTOCOL_SCHEMA);
...
test("surface expansion is a valid canonical event protocol with exact declared coordinates", () => {
  const eventProtocol = buildB1SurfaceEventAnalysisProtocol(loadProtocol(), {
    radius: 1.5,
    resolution: "primary",
  });
...
test("fixed internal expansion retains the complete cycle and both probe polarities", () => {
  const eventProtocol = buildB1FixedInternalEventAnalysisProtocol(loadProtocol());
...
test("cap-angle stencils exactly differentiate a separately authored quadratic case", () => {
  ...
  for (const value of [0, 0.7, Math.PI / 2]) {
    const stencil = createB1CapAngleSensitivityStencil({ value, domain, step });
...
test("protocol validation does not advance on surface/history and resolution defects", () => {
  ...
  assert.throws(
    () => validateB1CompleteCycleProbeProtocol(insideEnvelope),
  ...
  assert.throws(
    () => validateB1CompleteCycleProbeProtocol(shortHistory),
