...
} from "../scripts/neutral-braid/octahedral-force-residual.mjs";
...
test("octahedral force residual rejects the rigid zero-offset fixed-speed row", () => {
  ...
  assert.equal(artifact.schema, "neutral-braid-octahedral-force-residual/v1");
...
test("octahedral force-residual CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-force-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(new URL("../scripts/neutral-braid/octahedral-force-residual.mjs", import.meta.url));
