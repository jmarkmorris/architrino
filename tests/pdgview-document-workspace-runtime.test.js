...
import { createPdgviewDocumentWorkspaceRuntime } from "../src/apps/animator/PdgviewDocumentWorkspaceRuntime.js";
...
test("pdgview document workspace runtime sorts browser library entries and refreshes the library ui", () => {
  const storage = {
    getItem() {
  ...
  const runtime = createPdgviewDocumentWorkspaceRuntime({
  ...
  const entries = runtime.getPdgviewSortedLibraryEntries();
  runtime.refreshPdgviewLibraryUi("newer");
...
test("pdgview document workspace runtime saves to browser storage and applies library loads", () => {
  ...
  runtime.savePdgviewSceneToLibrary();

  const storedEntries = JSON.parse(storageData.get("architrino.pdgview.library.v1"));
  ...
  runtime.loadPdgviewSceneFromLibrary("my_scene");
...
test("pdgview document workspace runtime renders json preview and falls back to timeline rendering", () => {
  let statusMessage = "";
  ...
  console.error = () => {};
  try {
    runtime.renderPdgviewJsonPreview();
  } finally {
