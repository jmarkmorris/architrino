...
} from "../scripts/neutral-braid/octahedral-affine-exposure-probe.mjs";
...
test("octahedral affine exposure probe CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-affine-exposure-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-affine-exposure-probe.mjs", import.meta.url)
  );
