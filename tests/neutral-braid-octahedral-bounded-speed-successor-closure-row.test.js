...
} from "../scripts/neutral-braid/octahedral-bounded-speed-successor-closure-row.mjs";
...
test("bounded-speed successor closure row CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-bounded-speed-successor-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-bounded-speed-successor-closure-row.mjs", import.meta.url)
  );
