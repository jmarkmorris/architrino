...
} from "../scripts/neutral-swarm/octahedral-fold-aware-dynamics-handoff.mjs";
...
test("fold-aware dynamics handoff CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-dynamics-handoff-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fold-aware-dynamics-handoff.mjs", import.meta.url)
  );
