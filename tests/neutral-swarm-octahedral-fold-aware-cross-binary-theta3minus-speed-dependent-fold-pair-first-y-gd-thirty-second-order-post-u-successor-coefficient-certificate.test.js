...
} from "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-second-order-post-u-successor-coefficient-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-second-order-post-u-successor-coefficient-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D thirty-second-order post-U successor coefficient certificate validates", () => {
...
test("thirty-second-order post-U successor coefficient certificate imposes no fixed speed band", () => {
  const packet = artifact();
  const params =
    packet.thirty_second_order_post_u_successor_coefficient_parameters;
...
test("thirty-second-order post-U successor coefficient intervals certify h32 and Q_G/Q_D y32 coefficients", () => {
  const packet = artifact();
  const summary =
    packet.thirty_second_order_post_u_successor_coefficient_summary;
  const firstRow =
    packet.thirty_second_order_post_u_successor_coefficient_rows[0];

  assert.equal(summary.speed_cell_count, 128);
  assert.equal(summary.branch_cell_count, 256);
  assert.equal(summary.predecessor_h31_artifact_valid, true);
  assert.equal(summary.all_rows_certified, true);
  assert.equal(
    summary.all_source_equation_coefficients_contain_zero_y0_to_y36,
    true
  );
  assert.ok(
    Number(summary.max_abs_source_equation_coeff_y0_to_y36_interval) < 1.1e20
  );
  assert.ok(Number(summary.min_h32_solve_slope_clearance) > 0.79);
  assert.deepEqual(summary.h32_interval_hull, [
    -41053391146300000000,
    41053391146300000000,
  ]);
  assert.deepEqual(summary.Q_G_y32_coefficient_interval_hull, [
    -34972235669500000000,
    34972226860900000000,
  ]);
  assert.deepEqual(summary.Q_D_y32_coefficient_interval_hull, [
    -1.15408348641e21,
    1.15408377709e21,
  ]);
  assert.ok(Number(summary.max_abs_h32_interval) < 4.2e19);
  assert.ok(Number(summary.max_abs_Q_G_y32_coefficient_interval) < 3.6e19);
  assert.ok(Number(summary.max_abs_Q_D_y32_coefficient_interval) < 1.2e21);
  assert.ok(Number(summary.max_Q_G_y32_loss_on_first_y_cell) < 5e-69);
  assert.ok(Number(summary.max_Q_D_y32_loss_on_first_y_cell) < 1.7e-67);
  assert.equal(
    summary.all_QD_QG_coefficient_identity_intervals_contain_zero,
    true
  );
  assert.ok(
    Number(summary.max_abs_Q_D_plus_33Q_G_y32_coefficient_interval) < 2.4e21
  );
  assert.ok(firstRow.Q_D_plus_33Q_G_y32_coefficient_interval[0] <= 0);
  assert.ok(firstRow.Q_D_plus_33Q_G_y32_coefficient_interval[1] >= 0);
  assert.ok(
    Number(summary.min_Q_G_remaining_thirty_third_order_tail_budget) > 3e89
  );
  assert.ok(
    Number(summary.min_Q_D_remaining_thirty_third_order_tail_budget) > 3e89
  );
  assert.equal(
    firstRow.row_status,
...
test("thirty-second-order post-U successor coefficient certificate keeps tube, continuous tail, and retention open", () => {
  const packet = artifact();
...
test("thirty-second-order post-U successor coefficient validator rejects speed-band fields and overclaims", () => {
  const packet = clone(artifact());
  packet.thirty_second_order_post_u_successor_coefficient_parameters.speed_band =
    "forbidden";
...
test("thirty-second-order post-U successor coefficient CLI writes and validates artifact JSON", () => {
