...
} from "../scripts/neutral-braid/octahedral-phase-offset-retention-witness.mjs";
...
test("octahedral phase-offset retention witness rejects the sampled-improvement overread", () => {
  ...
  assert.equal(artifact.schema, "neutral-braid-octahedral-phase-offset-retention-witness/v1");
...
test("octahedral phase-offset retention witness CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-phase-retention-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-braid/octahedral-phase-offset-retention-witness.mjs", import.meta.url)
  );
