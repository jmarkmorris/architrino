...
} from "../scripts/neutral-swarm/octahedral-root-ledger.mjs";

test("octahedral root ledger samples all ordered distinct neutral-swarm pairs", () => {
  ...
  assert.equal(artifact.schema, "neutral-swarm-octahedral-root-ledger/v1");
  ...
  assert.equal(artifact.result.root_ledger_diagnostic, "sampled_passed");
  assert.equal(artifact.result.retention, "not_retained");
  assert.equal(artifact.result.retained_branch, false);
  assert.equal(artifact.result.first_failure_status, "support-complete-root-ledger-open");
});
...
test("octahedral root-ledger CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-octahedral-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(new URL("../scripts/neutral-swarm/octahedral-root-ledger.mjs", import.meta.url));
  ...
  assert.equal(validation.result.root_ledger_diagnostic, "sampled_passed");
