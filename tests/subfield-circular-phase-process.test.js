...
import { runSubfieldCircularPhaseProcess } from "../src/prescribed-path-analysis/SubfieldCircularPhaseProcess.mjs";
...
function emit(event, detail) {
  process.stderr.write(JSON.stringify({schema:'braid-program/subfield-circular-root-adapter-event.v1',event,stage:event,
...
if (config.mode==='descendant') {
  ...
  if(config.mode==='malformed-event') {process.stderr.write('{"bad":\n');setInterval(()=>{},1000);}
    ...
    rl.on('line',(line)=>{
      ...
      const row={schema:'braid-program/subfield-circular-root-row.v1',candidateId:identity.candidateId,rung:identity.rung,phase:identity.phase,
...
function fixture(mode = "success") {
  const directory = realpathSync(mkdtempSync(path.join(os.tmpdir(), "subfield-circular-phase-process-")));
...
async function failed(options) {
  try { await runSubfieldCircularPhaseProcess(options); assert.fail("synthetic failure must reject"); }
  catch (error) {
...
test("successful synthetic sequence waits for each asynchronous check and retains profiler diagnostics", async () => {
  ...
  const running = runSubfieldCircularPhaseProcess(options);
  await entered;
...
test("output reuse and duplicate pair schedules reject before any child starts", async () => {
  ...
  await assert.rejects(runSubfieldCircularPhaseProcess(options), /already exists/u);
  assert.equal(readFileSync(config.raw, "utf8"), "preserved");
  const other = fixture(); other.options.pairs = [[0, 0], [0, 0]];
  await assert.rejects(runSubfieldCircularPhaseProcess(other.options), /unique/u);
