...
const lane='.local-data/braid-analysis/f6c-continuous-reception-acceleration-20260827';
...
function pythonSource(){return Buffer.from([
 ...
 ' (ops/"synthetic-target-ready").write_text("ready")',
  ...
  'else:',
  ' assert (data/"range.json").is_file();target=ops/"comparison.json"',
  'raw=(json.dumps({"syntheticOnly":True,"accepted":False})+"\\n").encode()',
  'with target.open("xb") as stream:stream.write(raw)',
  'if mode=="checker-failure" and stage=="comparison":sys.exit(8)',
  'print(json.dumps({"completed":True,"accepted":False,"syntheticOnly":True,"output":{"path":str(target),"sha256":hashlib.sha256(raw).hexdigest(),"bytes":len(raw)}}),flush=True)',
].join('\n'));}
function entrySource(mode,program,digest){return Buffer.from([
  ...
  " if(job.kind==='admit'){",
  "  const completion=JSON.parse(readFileSync(job.stdout.path,'utf8'));if(!completion.syntheticOnly)throw Error('not synthetic');U.checkBindings([completion.output]);",
  ...
  "  return{accepted:true,h3EvidenceEligible:false,mathematicalAuthority:'synthetic process plumbing only',completion,outputs:[completion.output],",
  ...
  ' await U.runSingleStage({command:'+JSON.stringify(python)+",args:['-I','-B','-c',U.PYTHON_BOOTSTRAP,"+JSON.stringify(program)+",get('--out'),get('--stage'),"+JSON.stringify(mode)+']});',
  " console.error(JSON.stringify({kind:'f6c-range-entry-process-resources',resourceUsage:process.resourceUsage()}));}",
].join('\n'));}
async function runFixture(mode){
  const dir=realpathSync(mkdtempSync(path.join(tmpdir(),'f6c-range-registered-control-')));
  ...
  try{
    ...
    if(!['pass','slow-publication'].includes(mode)){
      await assert.rejects(operation);const rejection=JSON.parse(readFileSync(path.join(ops,'pilot-rejection.json')));assert.equal(rejection.accepted,false);
    ...
    assert.equal(receipt.stages[0].admission.outputs[0].path,path.join(output,'range.json'));assert.equal(receipt.stages[1].admission.outputs[0].path,path.join(ops,'comparison.json'));
...
test('real unread diagnostic pipe fails final drain then closes without a success completion',async()=>{
  const source=pathToFileURL(path.join(root,'scripts/eom/launch-f6c-acceleration-pilot.mjs')).href;
