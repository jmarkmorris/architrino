...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-finite-shift27-u-root-tail-tube-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-finite-shift27-u-root-tail-tube-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y finite Shift27 U root-tail tube certificate validates", () => {
...
test("finite Shift27 U root-tail tube certificate imposes no fixed speed band", () => {
  const packet = artifact();
  const params = packet.finite_shift27_u_root_tail_tube_parameters;
...
test("finite Shift27 U root-tail tube certifies endpoint signs, monotonicity, and predecessor E containment", () => {
  const packet = artifact();
  const summary = packet.finite_shift27_u_root_tail_tube_summary;
  const firstRow = packet.finite_shift27_u_root_tail_tube_rows[0];
  ...
  assert.equal(summary.finite_shift27_row_count, 3840);
  assert.equal(summary.shift_power, 27);
  assert.equal(summary.shift_series_order, 52);
  assert.equal(summary.u_seed_artifact_valid, true);
  assert.equal(summary.all_rows_certified, true);
  assert.equal(
    summary.all_lower_shift_coefficients_contain_zero_y0_to_y26,
    true
  );
  assert.equal(summary.all_shift27_endpoint_signs_certified, true);
  assert.equal(summary.all_partial_U_R_shift27_signs_certified, true);
  assert.equal(
    summary.all_E_images_from_U_tube_inside_predecessor_tube,
    true
  );
  assert.equal(summary.all_rows_use_coefficient_shift, true);
  assert.equal(summary.all_rows_avoid_raw_y_inverse_division, true);
  assert.equal(summary.all_finite_shift27_U_root_tail_tubes_certified, true);
  assert.ok(Number(summary.min_shift27_endpoint_clearance) > 7.9e15);
  assert.ok(Number(summary.min_partial_U_R_shift27_clearance) > 0.79);
  assert.ok(Number(summary.max_abs_shift27_trig_remainder) < 1e-100);
  assert.ok(Number(summary.max_E_image_from_U_tube_abs_upper) < 2e11);
  assert.ok(Number(summary.max_abs_lower_shift_residual_y0_to_y26) > 1e16);
  assert.ok(Number(summary.max_abs_lower_shift_residual_y0_to_y26) < 2e16);
  assert.deepEqual(summary.R_shift27_left_endpoint_interval_hull, [
    -7942457489110000,
    7934091580040000,
  ]);
  assert.deepEqual(summary.R_shift27_right_endpoint_interval_hull, [
    -7934091580560000,
    7942457481750000,
  ]);
  assert.deepEqual(summary.inherited_h23_interval_hull, [
    -4218031991090,
    4218902780690,
  ]);
  assert.deepEqual(summary.inherited_Q_G_y23_coefficient_interval_hull, [
    -3594155902090,
    3594155902090,
  ]);
  assert.deepEqual(summary.inherited_Q_D_y23_coefficient_interval_hull, [
    -86259741650100,
    86259741650100,
  ]);
  assert.ok(
  ...
  assert.equal(firstRow.shift27_coefficient_shift_used, true);
  assert.equal(firstRow.shift27_raw_y_inverse_division_used, false);
  assert.equal(firstRow.zero_touching_y_division_used, false);
  assert.equal(firstRow.E_image_from_U_tube_inside_predecessor_tube, true);
  assert.equal(firstRow.row_status, "finite-shift27-U-root-tail-tube-certified");
});

test("finite Shift27 U root-tail tube closes only the U tube row", () => {
  const packet = artifact();

  assert.equal(
    packet.artifact_claim.certifies_shift_27_finite_y_residual_u_evaluator,
    true
  );
  assert.equal(
  ...
  assert.equal(packet.artifact_claim.certifies_first_y_zero_endpoint_U_seed, true);
  assert.equal(
    packet.artifact_claim
...
test("finite Shift27 U root-tail tube validator rejects invalid controls and overclaims", () => {
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdFiniteShift27URootTailTubeCertificate(
        { ySubcellCount: 2 }
  ...
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdFiniteShift27URootTailTubeCertificate(
        { ePredecessorTubePadding: -1 }
      ),
    /ePredecessorTubePadding/
  );

  const packet = clone(artifact());
  packet.finite_shift27_u_root_tail_tube_parameters.speed_band = "forbidden";
  packet.artifact_claim.certifies_directed_rounded_first_y_GD_continuous_U_tail_bound = true;
...
test("finite Shift27 U root-tail tube CLI writes and validates artifact JSON", () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "theta3minus-finite-shift27-u-root-tail-tube-")
  );
