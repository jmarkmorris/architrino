...
} from "../scripts/neutral-swarm/octahedral-fold-aware-speed-primitive-existence.mjs";
...
test("fold-aware speed primitive existence CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-speed-primitive-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fold-aware-speed-primitive-existence.mjs", import.meta.url)
  );
