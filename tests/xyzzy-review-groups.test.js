...
test("Xyzzy review groups reference only tiles present in the shared catalog", () => {
...
test("Xyzzy binary generator includes bare core-row glyph variants for composite review groups", () => {
  const tileCatalog = normalizeXyzzyTileCatalog(readJson("src/apps/xyzzy/xyzzy-tiles.json"));
  const tileKeys = new Set(tileCatalog.tiles.map((tile) => tile.key));
...
test("Xyzzy review group catalog covers the requested single-row and composite families", () => {
  const reviewGroups = normalizeXyzzyReviewGroupCatalog(readJson("src/apps/xyzzy/xyzzy-review-groups.json"));
