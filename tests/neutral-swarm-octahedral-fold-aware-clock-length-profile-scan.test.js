...
} from "../scripts/neutral-swarm/octahedral-fold-aware-clock-length-profile-scan.mjs";
...
test("fold-aware clock length profile scan CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-fold-aware-clock-profile-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-fold-aware-clock-length-profile-scan.mjs", import.meta.url)
  );
