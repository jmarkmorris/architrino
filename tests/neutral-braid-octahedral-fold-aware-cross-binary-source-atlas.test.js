...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-source-atlas.mjs";
...
test("fold-aware cross-binary source atlas CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-cross-source-atlas-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-source-atlas.mjs",
      import.meta.url
    )
  );
