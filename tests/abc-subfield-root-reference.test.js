...
import { Q, decimalToken, floorDivide, multiply, rootLo, rootHi, sinCos } from "../scripts/eom/derive-abc-subfield-root-reference.mjs";
...
test("reference output cannot overwrite existing evidence", () => {
  const directory = mkdtempSync(path.join(tmpdir(), "abc-reference-test-"));
  try {
    const output = path.join(directory, "preserve.json");
    writeFileSync(output, "preserve\n");
    const result = spawnSync(process.execPath, ["scripts/eom/derive-abc-subfield-root-reference.mjs", "--out", output], { encoding: "utf8" });
