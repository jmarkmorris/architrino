...
class RecursiveBlockExclusionTests(unittest.TestCase):
    ...
    def test_accelerating_piecewise_cubic_histories_have_complete_accounting(self) -> None:
        first = self.run_case("traversal", "accelerating_sparse", 128)
        second = self.run_case(
            "traversal", "accelerating_sparse", 128, threads=4
        )
        for row in (first, second):
            self.assertEqual(row["status"], "certified_complete")
            self.assertEqual(row["logical_pairs"], 128 * 128)
    ...
    def test_accelerating_dense_population_falls_back_or_fails_preflight(self) -> None:
        bounded = self.run_case("traversal", "accelerating_dense", 128)
        self.assertEqual(bounded["status"], "certified_complete")
        self.assertEqual(bounded["excluded_pairs"], 0)
        self.assertEqual(
            bounded["exact_fallback_pairs"], bounded["logical_pairs"]
    ...
    def test_accelerating_recursive_path_matches_exhaustive_exact_control(self) -> None:
        traversal = self.run_case("traversal", "accelerating_sparse", 128)
        recursive = self.run_case("recursive", "accelerating_sparse", 128)
        exhaustive = self.run_case("exhaustive", "accelerating_sparse", 128)
        self.assertEqual(traversal["status"], "certified_complete")
        self.assertEqual(recursive["status"], "certified_complete")
        self.assertEqual(exhaustive["status"], "certified_complete")
        self.assertEqual(traversal["logical_pairs"], exhaustive["logical_pairs"])
