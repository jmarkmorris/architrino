...
import { createAnimatorDocumentWorkspaceRuntime } from "../src/apps/animator/AnimatorDocumentWorkspaceRuntime.js";
...
test("animator document workspace runtime sorts browser library entries and refreshes the library ui", () => {
  const storage = {
    getItem() {
  ...
  const runtime = createAnimatorDocumentWorkspaceRuntime({
  ...
  const entries = runtime.getAnimatorSortedLibraryEntries();
  runtime.refreshAnimatorLibraryUi("newer");
...
test("animator document workspace runtime saves to browser storage and applies library loads", () => {
  ...
  runtime.saveAnimatorSceneToLibrary();

  const storedEntries = JSON.parse(storageData.get("architrino.animator.library.v1"));
  ...
  runtime.loadAnimatorSceneFromLibrary("my_scene");
...
test("animator document workspace runtime renders json preview and falls back to timeline rendering", () => {
  let statusMessage = "";
  ...
  console.error = () => {};
  try {
    runtime.renderAnimatorJsonPreview();
  } finally {
