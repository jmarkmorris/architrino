...
} from "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-forcing-topology-atlas.mjs";
...
test("fold-aware forcing topology atlas CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "neutral-braid-forcing-topology-")
  );
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL(
      "../scripts/neutral-braid/octahedral-fold-aware-cross-binary-forcing-topology-atlas.mjs",
      import.meta.url
    )
  );
