...
} from "../scripts/neutral-swarm/octahedral-coordinate-exposure-matrix.mjs";
...
test("octahedral coordinate exposure matrix CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-coordinate-exposure-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-coordinate-exposure-matrix.mjs", import.meta.url)
  );
