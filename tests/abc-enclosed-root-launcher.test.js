...
import { ABC_GATE_SOURCE, classifyPilotGateRoles, currentOwnedGroup, descendantRecords, parseLauncherArgs, parseOwnedProcessTable, processTable,
  outerWorkerOperation, superviseRegisteredPilot } from "../scripts/eom/launch-abc-enclosed-root-pilot.mjs";
...
function fixture(mode = "normal") {
  const root = realpathSync(mkdtempSync(path.join(os.tmpdir(), "abc-outer-control-")));
  ...
  return { root, marker, options: { root, entry: "actor.mjs", args: [], sources: [{ path: "actor.mjs", bytes, sha256: sha(bytes) }],
    output: path.join(root, "run"), limitMs: 3000, heartbeatMs: 200, graceMs: 100,
...
test("launcher arguments require reviewed bytes and fresh scoped output", () => {
  const good = ["--out", ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/new", "--launcher-sha256", "a".repeat(64)];
...
test("registered normal child preserves output, original arguments, clean exit and measured gate overhead", async () => {
  ...
  assert.equal(gate.actualGateSourceSha256, sha(ABC_GATE_SOURCE)); assert.equal(gate.requestedCommand, process.execPath);
...
test("blocked runner and detached target are stopped, while an unrelated process survives", async () => {
  ...
  const { marker, options } = fixture("blocked-parent"); options.limitMs = 700; options.heartbeatMs = 150;
  try {
    const receipt = await rejection(options); assert.match(receipt.failure, /deadline/u);
...
test("process inspection failure after STOP kills the validated frozen set and returns bounded unverified closure", async () => {
  const { marker, options } = fixture("blocked-parent"); options.limitMs = 700; options.heartbeatMs = 150;
  let injected = false;
  ...
  assert.ok(receipt.signals.some(record => record.signal === "SIGKILL"));
  gone(receipt.runner.pid); gone(Number(readFileSync(marker, "utf8")));
});

test("inspection failure after CONT still cancels the gate's own group and preserves an unrelated process", async () => {
  const unrelated = spawn(process.execPath, ["-e", "setInterval(()=>{},1000)"], { stdio: "ignore" });
  const { marker, options } = fixture("blocked-parent"); options.limitMs = 700; options.heartbeatMs = 150;
...
test("inspection failure before STOP loses parent control and cancels the started detached target", async () => {
  const { marker, options } = fixture("blocked-parent"); options.limitMs = 700; options.heartbeatMs = 150;
  let injected = false;
  options.inspectProcesses = async () => {
...
test("deadline covers an unresolved final admission callback after all child processes exit", async () => {
  const { options } = fixture(); options.limitMs = 600; options.admit = async () => new Promise(() => {});
  const receipt = await rejection(options); assert.match(receipt.failure, /deadline/u); assert.equal(receipt.processesClosed, true);
});
...
test("final publication rechecks actual log bytes and refuses an already expired deadline", async () => {
  const root = realpathSync(mkdtempSync(path.join(os.tmpdir(), "abc-outer-publication-")));
...
test("failure publication preserves rejected operational evidence without granting acceptance", async () => {
  const root = realpathSync(mkdtempSync(path.join(os.tmpdir(), "abc-outer-failure-")));
