...
function entrySource(mode){
  const program=mode==='child-failure'?'process.exit(7)': ['lost-monitor','log-failure','host-failure'].includes(mode)?"process.on('SIGTERM',()=>{});require('node:fs').writeFileSync('synthetic-target-ready','ready');setInterval(()=>{},1000)":"process.stdout.write(JSON.stringify({completed:true,accepted:false,synthetic:true,elapsedSeconds:0})+'\\n')";
  return Buffer.from([
...
async function runFixture(mode){
  ...
  fs.writeSync=function(fd,bytes,...args){
    if(mode==='log-failure'&&existsSync(path.join(dir,'synthetic-target-ready'))&&Buffer.isBuffer(bytes)&&bytes.toString().includes('"kind":"aggregate-rss"'))throw Error('synthetic ENOSPC monitor log');
