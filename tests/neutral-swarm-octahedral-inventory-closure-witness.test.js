...
} from "../scripts/neutral-swarm/octahedral-inventory-closure-witness.mjs";
...
test("octahedral inventory closure witness rejects inventory bias as sufficient fixed-speed closure", () => {
  ...
  assert.equal(artifact.schema, "neutral-swarm-octahedral-inventory-closure-witness/v1");
...
test("octahedral inventory closure witness CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-inventory-closure-witness-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-inventory-closure-witness.mjs", import.meta.url)
  );
