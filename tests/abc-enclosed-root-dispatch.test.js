...
import { abcDispatchFileOperation, checkABCRungGateCensus, classifyABCCandidateFailure, parseABCDispatchArgs,
  abcWholeRungClock, parseABCResourceObservation, recordABCResourceObservation, runABCBoundedCandidatePool, sourceBytes,
  watchedABCDispatchOperation } from "../scripts/eom/dispatch-abc-enclosed-root-ladder.mjs";
import { ABC_BUILD_PATH, ABC_BUILD_SHA, ABC_IDS, ABC_RESOURCE_OBSERVATION, ABC_RUNG_PATH, authenticateABCPriorPhases, rungSha } from "../scripts/eom/run-abc-enclosed-root-rung.mjs";

// Synthetic process/scheduling/file controls only. No EOM or history generation.
const SELF="scripts/eom/dispatch-abc-enclosed-root-ladder.mjs", bytes=readFileSync(SELF), hash=rungSha(bytes);
const temp=()=>realpathSync(mkdtempSync(path.join(os.tmpdir(),"abc-dispatch-control-")));
const delay=ms=>new Promise(resolve=>setTimeout(resolve,ms));

test("dispatcher CLI requires exact external plan and code hashes plus fresh scoped output",()=>{
  const argv=["--plan","plan.json","--plan-sha256",hash,"--out",".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/new","--dispatcher-sha256",hash];
  assert.equal(parseABCDispatchArgs(argv)["--plan-sha256"],hash);
  for(const x of [argv.slice(0,-2),[...argv,"--plan","other"],argv.map(v=>v.endsWith("/new")?v+"/../escape":v)])assert.throws(()=>parseABCDispatchArgs(x));
});

test("resource minima allow exact contact and reject strictly below or ambiguous observations",()=>{
  const p=ABC_RESOURCE_OBSERVATION;
  assert.equal(parseABCResourceObservation("diagnostic\nSystem-wide memory free percentage: 20%\n",String(p.minimumFreeDiskBytesAtLaunch),p,true).accepted,true);
  assert.equal(parseABCResourceObservation("System-wide memory free percentage: 20%",String(p.minimumFreeDiskBytesDuringRun),p).accepted,true);
  for(const text of ["", "System-wide memory free percentage: NaN%", "System-wide memory free percentage: 101%",
    ...
    assert.throws(()=>parseABCResourceObservation(text,String(p.minimumFreeDiskBytesAtLaunch),p,true));
  assert.throws(()=>parseABCResourceObservation("System-wide memory free percentage: 90%",String(p.minimumFreeDiskBytesAtLaunch-1),p,true));
  assert.throws(()=>parseABCResourceObservation("System-wide memory free percentage: 90%",String(p.minimumFreeDiskBytesDuringRun-1),p));
});

test("shared byte drift and observation failure stop dispatch; geometry row failure stays candidate-local",()=>{
  assert.equal(classifyABCCandidateFailure("CANDIDATE_LOCAL_FAILURE",true),"candidate-stop");
  assert.equal(classifyABCCandidateFailure("REPEATED_PHASE_MISMATCH",true),"candidate-stop");
  assert.equal(classifyABCCandidateFailure("CANDIDATE_RESOURCE_LIMIT",true),"candidate-stop");
  assert.equal(classifyABCCandidateFailure("CANDIDATE_LOCAL_FAILURE",false),"shared-dispatch-stop");
  assert.equal(classifyABCCandidateFailure("SHARED_BINDING_DRIFT",true),"shared-dispatch-stop");
  assert.equal(classifyABCCandidateFailure("RESOURCE_OBSERVATION_STOP",true),"resource-dispatch-stop");
});
...
test("four-slot pool runs each candidate ladder serially and never exceeds reviewed concurrency",async()=>{
  ...
  await runABCBoundedCandidatePool({candidates:Array.from({length:9},(_,i)=>i),limit:4,shouldStop:()=>false,onFatal:()=>assert.fail("unexpected failure"),
...
test("startup fatal cancellation waits another active worker's cleanup before pool rejection",async()=>{
  ...
  await assert.rejects(runABCBoundedCandidatePool({candidates:[0,1,2],limit:2,shouldStop:()=>stopped,onFatal:()=>{stopped=true;},
...
test("candidate-local rejected rung ends that ladder but allows independent candidates",async()=>{
  const visits=[];
  await runABCBoundedCandidatePool({candidates:[0,1,2],limit:2,shouldStop:()=>false,onFatal:()=>assert.fail("unexpected global failure"),
...
test("gate census assigns exact output roles and counts shared rung summary once",()=>{
  const f=gateFixture(),cost=checkABCRungGateCensus(f.process,f.record);assert.equal(cost.gateCount,33);assert.equal(cost.notAggregateSimultaneousMemory,true);
  for(const mutate of [x=>x.process.gates.pop(),x=>x.process.gates.push(x.process.gates[0]),x=>x.process.gates[0].measurement.code=1,
    x=>x.process.gates[0].requestedArgs.push("--out","/extra")]){const x=gateFixture();mutate(x);assert.throws(()=>checkABCRungGateCensus(x.process,x.record));}
  f.record.ladderSummary={path:"/run/ladder-ledger.json"};assert.throws(()=>checkABCRungGateCensus(f.process,f.record));
});
...
test("captured file worker verifies exact bytes and rejects changed generations",async()=>{
  ...
  const result=await watchedABCDispatchOperation({kind:"read",root,files:[{path:filename,sha256:inputHash,json:true}]},{bytes,sha256:hash,limitMs:2000});
  assert.equal(result[0].value.synthetic,true);writeFileSync(filename,'{"synthetic":false}\n');
  await assert.rejects(watchedABCDispatchOperation({kind:"read",root,files:[{path:filename,sha256:inputHash}]},{bytes,sha256:hash,limitMs:2000}),/changed/u);
  await assert.rejects(watchedABCDispatchOperation({kind:"read",root,files:[]},{bytes,sha256:"f".repeat(64),limitMs:2000}),/captured/u);
});
...
test("regular-file worker rejects FIFO without waiting for a writer",async()=>{
  ...
  await assert.rejects(watchedABCDispatchOperation({kind:"read",root,files:[{path:fifo,json:true}]},{bytes,sha256:hash,limitMs:2000}),/regular/u);
});
...
test("cancelled and expired worker operations are terminated before returning",async()=>{
  ...
  await assert.rejects(watchedABCDispatchOperation({kind:"read",root:temp(),files:[]},{bytes,sha256:hash,limitMs:2000,signal}),/synthetic stop/u);
  await assert.rejects(watchedABCDispatchOperation({kind:"read",root:temp(),files:[]},{bytes,sha256:hash,limitMs:1}),/deadline/u);
});

test("admission rejects an unaccepted rung before any mathematical or build claim",async()=>{
  const root=temp();writeFileSync(path.join(root,"rung-process.json"),JSON.stringify({schema:"braid-program/abc-candidate-rung-process.v1",accepted:false}));
  const rungBytes=readFileSync(ABC_RUNG_PATH);
  await assert.rejects(abcDispatchFileOperation({kind:"admit",root,runOutput:root,rungBytes,rungSha256:rungSha(rungBytes),candidateId:"a1-1",rung:8,wallLimitSeconds:1800}),/authority/u);
});

test("successful synthetic rung admission carries authenticated plan into the next rung's prior chain",async()=>{
  const root=temp(),rungBytes=readFileSync(ABC_RUNG_PATH);
  ...
  const phases=Array.from({length:8},(_,phase)=>{const directory=path.join(root,`phase-${phase}`);mkdirSync(directory);
    ...
    return{candidateId:"a1-1",rung:8,phase,accepted:true,directory,historyManifest:binding("history-manifest.json"),conformance:binding("history-conformance.json"),
  ...
  const phaseReceipts=phases.map(p=>p.phaseReceipt),summary=put("rung-ledger.json",{schema:"braid-program/abc-enclosed-root-summary-reduction.v1",accepted:true,
    h3EvidenceEligible:false,rootExecutionAuthorized:false,authority:"authenticated-phase-summary-chain-only",scope:"candidate-rung",candidateIds:["a1-1"],phaseCount:8,
    rowCount:288,ordinaryRootCount:240,selfEndpointCount:48,phaseReceipts:phaseReceipts.map((p,phase)=>({...p,candidateId:"a1-1",rung:8,phase})),
    phaseReceiptChainSha256:rungSha(phaseReceipts.map(p=>p.sha256).join("\n")+"\n")});
  const record={schema:"braid-program/abc-candidate-rung-process.v1",accepted:true,h3EvidenceEligible:false,rootExecutionAuthorized:false,candidateId:"a1-1",rung:8,
    ...
    buildReceipt:{path:path.join(root,ABC_BUILD_PATH),sha256:ABC_BUILD_SHA},buildBefore:before,buildAfter:after,resources:{complete:true,measuredCPUSeconds:1,maximumPrecisionBits:53}};
  ...
  const job={kind:"admit",root,runOutput:root,rungBytes,rungSha256:rungSha(rungBytes),candidateId:"a1-1",rung:8,wallLimitSeconds:1800,plan,processReceipt,bindings:[]};
  const admission=await abcDispatchFileOperation(job);assert.deepEqual(admission.plan,plan);assert.equal(admission.h3EvidenceEligible,false);
  ...
  const pilotPhases=ABC_IDS.flatMap(candidateId=>[0,1].map(phase=>({candidateId,rung:2,phase,path:`/${candidateId}/${phase}`,sha256:hash})));
  authenticateABCPriorPhases({candidateId:"a1-1",rung:32,prior:{rungAdmissions:[{}]},planBinding:plan,
    pilotSummary:{accepted:true,h3EvidenceEligible:false,scope:"pilot",phaseCount:32,rowCount:2448,phaseReceipts:pilotPhases},
    phaseBindings:[...pilotPhases.filter(p=>p.candidateId==="a1-1"),...phaseReceipts],
    admissions:[{value:{accepted:true,h3EvidenceEligible:false,processesClosed:true,admission}}]});
  await assert.rejects(abcDispatchFileOperation({...job,plan:{...plan,sha256:"f".repeat(64)}}),/authenticated plan/u);
});
...
test("whole-rung deadline precedes prior preparation and terminates an overlong preparation worker",async()=>{
  ...
  assert.notEqual(rungSha(changed),hash);const clock=abcWholeRungClock(30);
  try {await assert.rejects(watchedABCDispatchOperation({kind:"prior",root,filename,value:{}},
...
test("whole-rung admission checks elapsed synchronous time and aborts observation/publication waits",async()=>{
  const first=abcWholeRungClock(10);
  ...
  const second=abcWholeRungClock(15);
  try {await assert.rejects(second.wait(new Promise(()=>{})),/deadline/u);}finally{second.close();}
  const third=abcWholeRungClock(1000);third.cancel(new Error("shared resource interruption"));
  try {await assert.rejects(watchedABCDispatchOperation({kind:"read",root:temp(),files:[]},
...
test("periodic resource log failure stops scheduling and waits an active sibling cleanup",async()=>{
  ...
  await runABCBoundedCandidatePool({candidates:[0,1,2],limit:2,shouldStop:()=>stopped,onFatal:()=>{stopped=true;},
    work:async id=>{started.push(id);if(id===0){await delay(5);
      await recordABCResourceObservation({query:async()=>({accepted:true}),append:()=>{throw Object.assign(Error("synthetic disk full"),{code:"ENOSPC"});},
...
test("initial resource-log reservation failure leaves no heartbeat, signal handlers or live process",()=>{
  ...
  const child=`import assert from 'node:assert/strict';import fs from 'node:fs';import path from 'node:path';
    ...
    const root=process.argv[1],source=process.argv[2],base='.local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/';
    ...
    await assert.rejects(m.runABCMeasuredDispatch({root,args:{'--out':base+'startup'},selfBytes:Buffer.alloc(0),rungBytes:Buffer.alloc(0),runtime:{}}),/reservation ENOSPC/);
