...
} from "../scripts/neutral-swarm/octahedral-affine-force-mean-derivative.mjs";
...
test("octahedral affine force-mean derivative CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-swarm-affine-force-mean-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(
    new URL("../scripts/neutral-swarm/octahedral-affine-force-mean-derivative.mjs", import.meta.url)
  );
