...
} from "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-third-order-u-seed-coefficient-certificate.mjs";
...
function scriptPath() {
  return path.resolve(
    ...
    "../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-third-order-u-seed-coefficient-certificate.mjs"
  );
}

test("theta3minus fold-pair first-y G/D twenty-third-order U-seed coefficient certificate validates", () => {
...
test("first-y twenty-third-order U-seed coefficient certificate imposes no fixed speed band", () => {
  const packet = artifact();
  const params = packet.twenty_third_order_u_seed_coefficient_parameters;
...
test("first-y U-seed coefficient intervals certify h23 and Q_G/Q_D y23 coefficients", () => {
  const packet = artifact();
  const summary = packet.twenty_third_order_u_seed_coefficient_summary;
  const firstRow = packet.twenty_third_order_u_seed_coefficient_rows[0];
  ...
  assert.equal(
    summary.all_source_equation_coefficients_contain_zero_y0_to_y27,
    true
  );
  assert.ok(
    Number(summary.max_abs_source_equation_coeff_y0_to_y27_interval) < 2e13
  );
  assert.ok(Number(summary.min_h23_solve_slope_clearance) > 0.79);
  assert.deepEqual(summary.h23_interval_hull, [
    -4218031991090,
    4218902780690,
  ]);
  assert.deepEqual(summary.Q_G_y23_coefficient_interval_hull, [
    -3594155902090,
    3594155902090,
  ]);
  assert.deepEqual(summary.Q_D_y23_coefficient_interval_hull, [
    -86259741650100,
    86259741650100,
  ]);
  assert.ok(Number(summary.max_abs_h23_interval) < 5e12);
  assert.ok(Number(summary.max_abs_Q_G_y23_coefficient_interval) < 4e12);
  assert.ok(Number(summary.max_abs_Q_D_y23_coefficient_interval) < 9e13);
  assert.ok(
    Number(summary.max_Q_G_twenty_third_order_loss_on_first_y_cell) <
      2.6e-51
  );
  assert.ok(
    Number(summary.max_Q_D_twenty_third_order_loss_on_first_y_cell) <
      6.2e-50
  );
  ...
  assert.ok(firstRow.Q_D_plus_24Q_G_y23_coefficient_interval[0] <= 0);
  assert.ok(firstRow.Q_D_plus_24Q_G_y23_coefficient_interval[1] >= 0);
  assert.ok(
...
test("first-y U-seed coefficient certificate keeps finite tube and quotient closure open", () => {
  const packet = artifact();
...
test("first-y U-seed coefficient validator rejects invalid controls and overclaims", () => {
  assert.throws(
    () =>
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyThirdOrderUSeedCoefficientCertificate({
        speedCellCount: 2,
  ...
  packet.twenty_third_order_u_seed_coefficient_parameters.speed_band =
    "forbidden";
...
test("first-y U-seed coefficient CLI writes and validates artifact JSON", () => {
