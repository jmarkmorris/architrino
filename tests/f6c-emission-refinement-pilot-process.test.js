...
const Admission=await loadProductionTestModule(import.meta.url,"scripts/eom/run-f6c-emission-refinement-pilot.mjs");
...
const lane='.local-data/braid-analysis/f6c-emission-refinement-20260827';
...
function pythonSource(){return Buffer.from([
 ...
 'names=("queries.ndjson","rows.ndjson","pieces.ndjson","cover-manifest.json")',
 'if stage=="producer":',
...
function entrySource(mode,program,digest){return Buffer.from([
  ...
  " const envelope=U.stageSpec({stage:'producer',plan:{},planBinding:{path:'/synthetic-plan',sha256:'a'.repeat(64)},root:realpathSync(process.cwd()),output:get('--out'),python:'/explicit/python',git:'/usr/bin/git',budget:'5'}).args[6];",
  ...
  try{
    ...
    if(!['pass','slow-publication'].includes(mode)){
      ...
      const expectedFailure={'child-failure':/runner did not exit cleanly/,'checker-failure':/runner did not exit cleanly/,'publication-failure':/synthetic late publication/,
        'startup-interruption':/interrupted/,'lost-monitor':/^(synthetic monitor lost|outer operator interruption or original work deadline)$/,'log-failure':/interruption/,'wrong-stdout':/fresh supervisor logs differ/,'broken-diagnostic':/interruption|runner did not exit/};
    ...
    const receipt=JSON.parse(readFileSync(result.receipt.path));assert.deepEqual(receipt.stages.map(s=>s.stage),['producer','comparison']);
    ...
    assert.deepEqual(receipt.stages[0].admission.outputs.map(b=>b.path),['queries.ndjson','rows.ndjson','pieces.ndjson','cover-manifest.json'].map(p=>path.join(output,p)));assert.equal(receipt.census.queries,3584);assert.equal(receipt.accelerationEvaluated,false);assert.equal(receipt.rootsEvaluated,undefined);assert.equal(receipt.stages[1].admission.outputs[0].path,path.join(ops,'comparison.json'));
...
test('real unread diagnostic pipe fails final drain then closes without a success completion',async()=>{
  const source=pathToFileURL(path.join(root,'scripts/eom/launch-f6c-emission-refinement-pilot.mjs')).href;
