...
} from "../scripts/neutral-braid/octahedral-phase-offset-scan.mjs";
...
test("octahedral phase-offset scan emits a ranked sampled rigid-phase family", () => {
  ...
  assert.equal(artifact.schema, "neutral-braid-octahedral-phase-offset-scan/v1");
...
test("octahedral phase-offset scan CLI emits and validates JSON artifacts", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "neutral-braid-phase-"));
  const artifactPath = path.join(tempDir, "artifact.json");
  const scriptPath = fileURLToPath(new URL("../scripts/neutral-braid/octahedral-phase-offset-scan.mjs", import.meta.url));
