...
- The **zombie-solver** — the quarantined former central solver, a prescribed-orbit evaluator with no integrator (audit 2026-07-12) — was **deleted on 2026-07-16** after consumer-by-consumer migration. Its history lives in git; the audit, quarantine, and claims-triage records remain under `reference/priorities/app-solver/`. Do not reintroduce it, port its kernels forward as evidence-bearing code, or cite its outputs as evidence.
- Stored zombie-solver outputs that remain in the tree (for example the Borg compatibility fixture in `src/apps/borg/`) are frozen, explicitly quarantine-labeled (`canonicalEomEvidence: false`) compatibility data. They may be displayed only under those labels, can no longer be regenerated, and never count as evidence.
