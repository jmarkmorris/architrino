...
- Runtime payloads declared in [scripts/config/generated-runtime-assets.json](scripts/config/generated-runtime-assets.json) are reproducible build outputs. Local setup, tests, and Pages builds may prepare them with `node scripts/prepare-runtime-assets.mjs --write`; this command must not edit authored sources. During the operator-approved Pages proof phase, retain the exact file set listed by `runtimeTransition` in the [retention registry](reference/op/machine-artifact-retention-registry.v1.json), within its bounded budget, until the Actions deployment is verified live. Remove those tracked copies only in the second migration PR; afterward never force-add them. The [machine-artifact retention policy](reference/op/machine-artifact-retention.md) owns this transition and file, collection, and branch growth limits.
