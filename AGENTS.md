...
- Ordinary development does not include Git publication. No agent stages, commits, pushes, creates or updates a pull request, merges, deletes a branch, or rolls over to a successor except as the runner the operator has explicitly designated under [reference/op/git/codex-pr-branch.md](reference/op/git/codex-pr-branch.md), which owns that contract and its stop conditions, or under another procedure the operator explicitly invokes for that purpose. The operator initiates the procedure and remains the sole merge gate. Reading, linking to, or citing a publication procedure does not invoke it. This routing states who may publish; the Git read/write split above states which commands each agent may run at all.
- Git inspection and Git writes are separate permissions. Claude and Codex may both run inspection commands, including `git status`, `git diff`, `git log`, `git show`, and `git branch --list`, and may read `.git` to observe repository state. Repository-changing Git belongs exclusively to Codex: Claude must not run any command that changes refs, the object store, the working tree, or remote state, including `add`, `commit`, `push`, `checkout`, `switch`, `merge`, `rebase`, `reset`, `stash`, `restore`, `cherry-pick`, `tag`, `branch` creation or deletion, `gh pr` mutations, and Git configuration changes. When an inspection command has a mutating form, use the inspection form. Keep the write set scoped to the files the current task owns.
- `git status` is permitted even though it is not strictly read-only: it refreshes the index and takes `.git/index.lock` to do so. In a sandbox or container that cannot unlink files under `.git`, that lock can survive the command and block every later Git operation until someone removes it by hand. Prefer `git --no-optional-locks status`, which does not take the lock. Note the placement: `--no-optional-locks` is a top-level Git option and must precede the subcommand, because `git status --no-optional-locks` is rejected as an unknown option. Report a lock left behind rather than leaving it for the next session to discover; only the operator can remove one that a restricted environment refuses to unlink. Report unrelated dirty files only when they block the task, create overwrite risk, affect validation, or matter for staging, committing, pushing, or PR publication.
...
- Outside an explicit regeneration/fix-drift request or the final branch/PR process, if a generator `--check` reports generated drift, record the drift and the exact `--write` command needed instead of running it. Drift in a generated target whose fingerprinted source the current task edited is an expected consequence of that edit and is resolved by the branch/PR process; leave it for that procedure rather than raising it with the operator. Raise drift with the operator only when it blocks the current task or its source was not edited by this task.
