...
- The accepted [EOM production-host decision](reference/architectural-decisions/eom-cpp-production-host.md) owns solver policy, and the live [EOM evolution contract](reference/priorities/app-solver/evolution-contract-v1.md) owns the current versioned requirements. EOM under `src/eom` is the sole forward production solver target; its output gains authority only through declared acceptance gates and independent oracles.
