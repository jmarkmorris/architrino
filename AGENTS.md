...
- Git read and Git write are separate permissions. Claude and Codex may both run read-only Git commands, such as `git status`, `git diff`, `git log`, `git show`, and `git branch --list`, and may read `.git` to observe repository state. Git writes belong exclusively to Codex: Claude must not run any command that changes refs, the index, the object store, the working tree, or remote state, including `add`, `commit`, `push`, `checkout`, `switch`, `merge`, `rebase`, `reset`, `stash`, `restore`, `cherry-pick`, `tag`, `branch` creation or deletion, `gh pr` mutations, and Git configuration changes. When a read-only command has a mutating form, use the read-only form. Keep the write set scoped to the files the current task owns. Report unrelated dirty files only when they block the task, create overwrite risk, affect validation, or matter for staging, committing, pushing, or PR publication.
