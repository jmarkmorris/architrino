...
- [ ] 2026-07-14: `scripts/borg/write-fixture-data.mjs --check` fails for a pre-existing reason unrelated to any current task: the generator emits `canonicalEomEvidence: true` for central-solver output while the committed artifact correctly carries `false`. Fix the generator's claim labels so `--check` can go green without re-upgrading a quarantined claim. Do not run `--write` until then.
