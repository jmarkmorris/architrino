...
### 9. Add Pion Authoring And Review Support

Status: `pending`

Goal:

- let Reaction author, inspect, and review pion participants cleanly so solver-visible pion support can be exercised from the app instead of only through PDG ingest fixtures.

Why it matters:

- once pion support lands in the solver, Reaction needs to expose the same particle vocabulary cleanly or the new coverage will remain awkward to author and review;
- charged and neutral pion channels are common enough that they should not feel like second-class exceptions in the Reaction surface;
- and `pi0` needs a clear authored representation for its two allowed quark-content options without pretending the UI is modeling a quantum superposition object.

Required particle content:

- `pi+` as `u + anti-d`;
- `pi-` as `d + anti-u`;
- and `pi0` with two allowed authored decompositions, `u + anti-u` and `d + anti-d`, while keeping those forms solver-equivalent.

Next steps:

- add pion templates, labels, and review vocabulary to the Reaction-owned authoring surface;
- make neutral-pion option selection explicit wherever a participant is opened, inspected, or exported;
- keep the Reaction-side contract seam stable while extending its participant vocabulary;
- and add focused Reaction export and review tests once pion participants can be authored through the normal surface.
