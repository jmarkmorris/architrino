...
const OUTPUT_JSON = `higher_fold_layer_same_packet_candidate_live_higher_fold_constants_accepted_status_source_packet_rule_derivation_proof_object_contract_target_packet.${PACKET_ID}.proof-interval-v6.lambda0305.json`;
const OUTPUT_REPORT = `higher_fold_layer_same_packet_candidate_live_higher_fold_constants_accepted_status_source_packet_rule_derivation_proof_object_contract_target_packet_report.${PACKET_ID}.proof-interval-v6.lambda0305.md`;
...
const DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS = [
...
function printHelp() {
  console.log(`Usage: node scripts/proof-programs/fresh-v10-higher-fold-layer-same-packet-candidate-live-higher-fold-constants-accepted-interval-certified-status-source-packet-acceptance-rule-derivation-proof-object-contract-target-packet.mjs [options]
...
function validateCurrentPoolAbsence(currentPoolAbsence) {
  ...
  expectEqual(s.retained_source_data_readiness_direct_source_hash_checks_passed, 1, "retained source-data locks");
  expectEqual(
  ...
  expectEqual(s.accepted_status_lane_non_fail_closed_json_files, 0, "accepted-status lane non-fail-closed count");
  expectEqual(
    s.current_pool_source_packet_acceptance_rule_derivation_proof_object_files_found,
    0,
    "compatible derivation-proof object files",
  );
  expectEqual(
    s.current_pool_compatible_source_packet_acceptance_rule_derivation_proof_object_refs,
    0,
    "compatible derivation-proof object refs",
  );
  ...
  expectEqual(s.source_packet_acceptance_rule_derivation_proof_object_slots, 124, "proof object slots");
  expectEqual(
    s.source_packet_acceptance_rule_derivation_proof_object_slots_satisfied,
    0,
    "proof object slots satisfied",
  );
...
function contractRecord(profile) {
  return {
    compatible_proof_object_role: COMPATIBLE_PROOF_OBJECT_ROLE,
    derivation_proof_target: DERIVATION_PROOF_TARGET,
    derivation_proof_object_contract_target_declared: true,
    derivation_proof_object_contract_target_satisfied: false,
    required_contract_fields: DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS,
    contract_field_slots: DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.length,
    contract_field_slots_satisfied: 0,
    source_data_ready: profile.derivation_proof_source_data_records_ready === 1,
...
function buildSeparatorProfiles(currentPoolAbsence) {
  return currentPoolAbsence.separator_source_packet_acceptance_rule_derivation_proof_object_current_pool_absence_profiles
    .map((profile) => {
      const contract = contractRecord(profile);
      return {
        ...
        source_packet_acceptance_rule_derivation_proof_object_contract_field_slots:
          DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.length,
        source_packet_acceptance_rule_derivation_proof_object_contract_field_slots_satisfied: 0,
        source_packet_acceptance_rule_derivation_proof_object_contract_target_record: contract,
        current_pool_derivation_proof_object_present: false,
        ...
        first_contract_blocker: CONTRACT_BLOCKER,
        classification:
          "separator_source_packet_acceptance_rule_derivation_proof_object_contract_target_declared_object_absent_fail_closed",
      };
    })
    .sort((left, right) => separatorSortKey(left.separator_event) - separatorSortKey(right.separator_event));
}

function buildRowProfiles(currentPoolAbsence) {
  return currentPoolAbsence.row_source_packet_acceptance_rule_derivation_proof_object_current_pool_absence_profiles
    .map((profile) => {
      const contract = contractRecord(profile);
      return {
        ...
        first_contract_blocker: CONTRACT_BLOCKER,
        classification:
          "row_source_packet_acceptance_rule_derivation_proof_object_contract_target_declared_object_absent_fail_closed",
      };
    })
    .sort((left, right) => rowSortKey(left).localeCompare(rowSortKey(right)));
}

function buildPacket(paths, currentPoolAbsence, sourceDataReadiness) {
  ...
  const source = currentPoolAbsence.summary;
  const separatorProfiles = buildSeparatorProfiles(currentPoolAbsence);
  const rowProfiles = buildRowProfiles(currentPoolAbsence);
  const rowsBySeparator = sortedObjectBySeparator(countBy(rowProfiles, (row) => row.separator_event));
  const contractTargetSlots = separatorProfiles.length + rowProfiles.length;
  const contractFieldSlots =
    contractTargetSlots * DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.length;
  ...
  const summary = {
    ...
    retained_current_pool_absence_direct_source_hash_checks_passed: source.direct_source_hash_checks_passed,
    ...
    retained_current_pool_absence_source_data_readiness_direct_source_hash_checks_passed:
      source.retained_source_data_readiness_direct_source_hash_checks_passed,
    ...
    retained_current_pool_json_files_scanned: source.current_pool_json_files_scanned,
    retained_accepted_status_lane_json_files_scanned: source.accepted_status_lane_json_files_scanned,
    retained_accepted_status_lane_fail_closed_json_files:
      source.accepted_status_lane_fail_closed_json_files,
    retained_current_pool_derivation_proof_object_files_found:
      source.current_pool_source_packet_acceptance_rule_derivation_proof_object_files_found,
    ...
    separator_derivation_proof_object_contract_target_profiles: separatorProfiles.length,
    row_derivation_proof_object_contract_target_profiles: rowProfiles.length,
    ...
    derivation_proof_source_data_records_missing: source.derivation_proof_source_data_records_missing,
    ...
    source_packet_acceptance_rule_derivation_proof_object_slots: source.source_packet_acceptance_rule_derivation_proof_object_slots,
    source_packet_acceptance_rule_derivation_proof_object_slots_satisfied:
      source.source_packet_acceptance_rule_derivation_proof_object_slots_satisfied,
    source_packet_acceptance_rule_derivation_proof_object_contract_target_slots: contractTargetSlots,
    ...
    source_packet_acceptance_rule_derivation_proof_object_contract_fields:
      DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.length,
    source_packet_acceptance_rule_derivation_proof_object_contract_field_slots: contractFieldSlots,
    source_packet_acceptance_rule_derivation_proof_object_contract_field_slots_satisfied: 0,
    ...
    current_pool_source_packet_acceptance_rule_derivation_proof_object_files_found:
      source.current_pool_source_packet_acceptance_rule_derivation_proof_object_files_found,
    current_pool_compatible_source_packet_acceptance_rule_derivation_proof_object_refs:
      source.current_pool_compatible_source_packet_acceptance_rule_derivation_proof_object_refs,
    contract_target_packet_as_derivation_proof_object_rejections: contractTargetSlots,
    current_pool_absence_classifier_as_derivation_proof_object_rejections: contractTargetSlots,
    source_data_readiness_classifier_as_derivation_proof_object_rejections:
      source.source_data_readiness_classifier_as_derivation_proof_object_rejections,
    derivation_proof_source_data_record_as_derivation_proof_object_rejections:
      source.derivation_proof_source_data_record_as_derivation_proof_object_rejections,
  ...
  const packet = {
    schema:
      "architrino.proof_programs.accepted_interval_certified_status_source_packet_acceptance_rule_derivation_proof_object_contract_target_packet.v0",
    packet_id: PACKET_ID,
    route:
      "fresh-v10 higher-fold null-coordinate preledger accepted interval-certified status source-packet acceptance rule derivation-proof object contract target",
    status: STATUS,
    theorem_target:
      "candidate-live higher-fold constants accepted interval-certified constants status source-packet acceptance rule derivation proof object contract target",
    claim_level:
      "priority-only source-packet acceptance rule derivation-proof object contract target packet; imports the current-pool absence classifier, declares the exact non-reinterpretation contract fields a future source_packet_acceptance_rule_derivation_proof object must satisfy, and keeps proof-rule, source-packet acceptance rule, accepted source packet, accepted status, row-consumption, live-ledger, and branch-chart decisions absent",
    preledger_pass: false,
    updates_live_ledger: false,
    branch_chart_authorized: false,
    ...
    compatible_proof_object_role: COMPATIBLE_PROOF_OBJECT_ROLE,
    derivation_proof_object_contract_fields: DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS,
    source_packet_acceptance_rule_derivation_proof_object_contract_target: {
      target: COMPATIBLE_PROOF_OBJECT_ROLE,
      derivation_proof_target: DERIVATION_PROOF_TARGET,
      contract_target_slots_declared: contractTargetSlots,
      contract_target_slots_satisfied: 0,
      contract_fields_per_slot: DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.length,
      contract_field_slots: contractFieldSlots,
      contract_field_slots_satisfied: 0,
      first_contract_blocker: CONTRACT_BLOCKER,
      current_pool_derivation_proof_object_absent: true,
    },
    separator_source_packet_acceptance_rule_derivation_proof_object_contract_target_profiles:
      separatorProfiles,
    row_source_packet_acceptance_rule_derivation_proof_object_contract_target_profiles: rowProfiles,
    summary,
    next_certificate_handoff: {
      handoff_class: "source_packet_acceptance_rule_derivation_proof_object_contract_target",
      sharpened_blocker:
        "The current pool has no compatible derivation-proof object, and the exact proof-object contract target is now declared with 124 slots and 992 contract-field slots, all unsatisfied.",
      current_pool_closure_state:
        "source data ready; current-pool derivation-proof object absent; contract target declared but unsatisfied",
      first_rule_blocker: RULE_BLOCKER,
      first_derivation_proof_blocker: DERIVATION_PROOF_BLOCKER,
      first_current_pool_derivation_proof_object_absence_blocker:
        CURRENT_POOL_DERIVATION_PROOF_OBJECT_BLOCKER,
      ...
      downstream_inputs_not_actionable_until_derivation_proof_contract_satisfied: [
    ...
    capture_decision:
      "Priority-only. This packet declares the contract target for a future derivation-proof object; it does not construct, accept, or apply that proof object.",
  };
...
function assertPacketInvariants(packet) {
  ...
  const checks = [
    ...
    s.retained_current_pool_absence_source_data_readiness_direct_source_hash_checks_passed === 1,
    ...
    s.source_packet_acceptance_rule_derivation_proof_object_contract_target_slots_declared === 124,
    s.source_packet_acceptance_rule_derivation_proof_object_contract_target_slots_satisfied === 0,
    s.source_packet_acceptance_rule_derivation_proof_object_contract_fields ===
      DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.length,
    ...
    s.contract_target_packet_as_derivation_proof_object_rejections === 124,
    s.current_pool_absence_classifier_as_derivation_proof_object_rejections === 124,
  ...
  if (!checks.every(Boolean)) {
    throw new Error("Source-packet acceptance rule derivation-proof object contract target invariant failure.");
  }
}
...
function renderReport(packet) {
  ...
  const contractRows = DERIVATION_PROOF_OBJECT_CONTRACT_FIELDS.map((field, index) => [
    String(index + 1),
    `\`${field}\``,
  ]);
  ...
  return `# Candidate-Live Higher-Fold Constants Accepted-Status Source-Packet Acceptance Rule Derivation-Proof Object Contract Target Packet
...
This packet declares the contract target for a future
\`${COMPATIBLE_PROOF_OBJECT_ROLE}\`. It does not construct or accept that
object. A compatible proof object must satisfy every field below for the live
same-packet separator aggregate family.
...
- contract field slots satisfied: ${s.source_packet_acceptance_rule_derivation_proof_object_contract_field_slots_satisfied} / ${s.source_packet_acceptance_rule_derivation_proof_object_contract_field_slots}
- first contract blocker: \`${s.first_derivation_proof_object_contract_blocker}\`
...
## Retained Absence State

- retained certificate JSON files scanned: ${s.retained_current_pool_json_files_scanned}
- retained accepted-status lane files scanned: ${s.retained_accepted_status_lane_json_files_scanned}
- retained accepted-status lane fail-closed files: ${s.retained_accepted_status_lane_fail_closed_json_files}
- retained compatible derivation-proof object files found: ${s.retained_current_pool_derivation_proof_object_files_found}
...
- contract target packet as derivation-proof object rejections: ${s.contract_target_packet_as_derivation_proof_object_rejections}
- current-pool absence classifier as derivation-proof object rejections: ${s.current_pool_absence_classifier_as_derivation_proof_object_rejections}
...
This packet does not construct a derivation proof, proof rule, source-packet
...
The blocker is now \`${s.first_derivation_proof_object_contract_blocker}\`:
the future \`${COMPATIBLE_PROOF_OBJECT_ROLE}\` must instantiate all declared
contract fields. Until then, the soundness proof, endpoint-application proof,
...
function main() {
  ...
  const packet = buildPacket(paths, currentPoolAbsence, sourceDataReadiness);
