...
const OUTPUT_JSON = `higher_fold_layer_same_packet_candidate_live_higher_fold_constants_accepted_interval_certified_status_proof_grade_derivation_ref_evidence_absence_classifier.${PACKET_ID}.proof-interval-v6.lambda0305.json`;
const OUTPUT_REPORT = `higher_fold_layer_same_packet_candidate_live_higher_fold_constants_accepted_interval_certified_status_proof_grade_derivation_ref_evidence_absence_classifier_report.${PACKET_ID}.proof-interval-v6.lambda0305.md`;
...
function printHelp() {
  console.log(`Usage: node scripts/proof-programs/fresh-v10-higher-fold-layer-same-packet-candidate-live-higher-fold-constants-accepted-interval-certified-status-proof-grade-derivation-ref-evidence-absence-classifier.mjs [options]
...
function validateTargetProfiles(target) {
  for (const profile of target.separator_proof_grade_derivation_ref_evidence_target_profiles) {
    if (profile.proof_grade_derivation_ref_evidence_target_field !== TARGET_FIELD) {
      throw new Error(`Unexpected separator target field: ${profile.separator_event}`);
    }
    if (profile.proof_grade_derivation_ref_evidence_target_satisfied !== false) {
      throw new Error(`Separator target should be unsatisfied: ${profile.separator_event}`);
    }
    if (profile.first_target_blocker !== TARGET_BLOCKER) {
      throw new Error(`Unexpected separator target blocker: ${profile.first_target_blocker}`);
    }
    if (!Array.isArray(profile.compatible_evidence_refs) || profile.compatible_evidence_refs.length !== 0) {
      throw new Error(`Separator target should have no compatible evidence refs: ${profile.separator_event}`);
    }
  }
  for (const profile of target.row_proof_grade_derivation_ref_evidence_target_profiles) {
    if (profile.proof_grade_derivation_ref_evidence_target_field !== TARGET_FIELD) {
      throw new Error(`Unexpected row target field: ${profile.row_id}`);
    }
    if (profile.proof_grade_derivation_ref_evidence_target_satisfied !== false) {
      throw new Error(`Row target should be unsatisfied: ${profile.row_id}`);
    }
    if (profile.first_target_blocker !== TARGET_BLOCKER) {
      throw new Error(`Unexpected row target blocker: ${profile.first_target_blocker}`);
    }
    if (!Array.isArray(profile.compatible_evidence_refs) || profile.compatible_evidence_refs.length !== 0) {
      throw new Error(`Row target should have no compatible evidence refs: ${profile.row_id}`);
    }
  }
}
...
function validateInputs(inputs) {
  ...
  const expected = [
    ...
    [target.total_proof_grade_derivation_ref_evidence_target_slots, 124, "derivation-ref target slots"],
    [target.total_proof_grade_derivation_ref_evidence_target_slots_satisfied, 0, "derivation-ref target slots satisfied"],
    [target.compatible_proof_grade_derivation_ref_evidence_refs, 0, "derivation-ref target compatible refs"],
    [first.direct_source_hash_checks_passed, 5, "first-blocker source-hash locks"],
  ...
  validateTargetProfiles(inputs.derivationRefTarget);
}

function buildSeparatorAbsenceProfiles(target) {
  return target.separator_proof_grade_derivation_ref_evidence_target_profiles.map((profile) => ({
    ...
    target_packet_is_derivation_ref_evidence: false,
    evidence_dependency_record_is_derivation_ref_evidence: false,
    compatible_evidence_refs: [],
    derivation_ref_evidence_object_present: false,
    first_absence_blocker: ABSENCE_BLOCKER,
    ...
    classification: "separator_derivation_ref_target_declared_derivation_ref_evidence_object_absent",
  }));
}

function buildRowAbsenceProfiles(target) {
  return target.row_proof_grade_derivation_ref_evidence_target_profiles.map((profile) => ({
    ...
    classification: "row_derivation_ref_target_declared_derivation_ref_evidence_object_absent",
  }));
}
...
function buildPacket(paths, inputs) {
  ...
  const first = inputs.firstBlocker.summary;
  const evidence = inputs.proofGradeEvidence.summary;
  const separatorAbsenceProfiles = buildSeparatorAbsenceProfiles(inputs.derivationRefTarget);
  const rowAbsenceProfiles = buildRowAbsenceProfiles(inputs.derivationRefTarget);
  const absenceSlots = separatorAbsenceProfiles.length + rowAbsenceProfiles.length;
  const summary = {
    ...
    candidate_higher_fold_constants_artifacts: target.candidate_higher_fold_constants_artifacts,
    candidate_separator_constants: target.candidate_separator_constants,
    candidate_row_constant_associations: target.candidate_row_constant_associations,
    rows_by_separator_count: target.rows_by_separator_count,
    evidence_pool_json_files_scanned_upstream: evidence.evidence_pool_json_files_scanned,
    evidence_pool_compatible_proof_grade_status_evidence_files_upstream:
    ...
    separator_proof_grade_derivation_ref_evidence_absence_profiles: separatorAbsenceProfiles.length,
    row_proof_grade_derivation_ref_evidence_absence_profiles: rowAbsenceProfiles.length,
    total_proof_grade_derivation_ref_evidence_absence_slots: absenceSlots,
    total_proof_grade_derivation_ref_evidence_absence_slots_with_evidence_object: 0,
    total_proof_grade_derivation_ref_evidence_absence_slots_missing_evidence_object: absenceSlots,
    derivation_ref_evidence_absence_classifications_declared: 1,
    derivation_ref_evidence_absence_classifications_satisfied: 0,
    target_packet_as_derivation_ref_evidence_rejections: absenceSlots,
    evidence_dependency_record_as_derivation_ref_evidence_rejections: absenceSlots,
    compatible_proof_grade_derivation_ref_evidence_refs: 0,
    derivation_ref_evidence_objects_found: 0,
    ...
    updates_live_ledger: false,
    branch_chart_authorized: false,
    first_proof_grade_derivation_ref_evidence_absence_blocker: ABSENCE_BLOCKER,
  ...
  const invariant =
    ...
    summary.evidence_pool_compatible_proof_grade_status_evidence_files_upstream === 0 &&
    ...
    summary.separator_proof_grade_derivation_ref_evidence_absence_profiles === 12 &&
    summary.row_proof_grade_derivation_ref_evidence_absence_profiles === 112 &&
    ...
    summary.total_proof_grade_derivation_ref_evidence_absence_slots_missing_evidence_object === 124 &&
    summary.target_packet_as_derivation_ref_evidence_rejections === 124 &&
    summary.evidence_dependency_record_as_derivation_ref_evidence_rejections === 124 &&
  ...
  if (!invariant) {
    throw new Error("Proof-grade derivation-ref evidence absence invariant failed.");
  }

  return {
    schema:
      "breather-higher-fold-layer-same-packet-candidate-live-higher-fold-constants-accepted-interval-certified-status-proof-grade-derivation-ref-evidence-absence-classifier-v1",
    packet_id: PACKET_ID,
    route: "fresh-v10 higher-fold null-coordinate preledger closure",
    status: STATUS,
    theorem_target: "Null-Coordinate Causal Pre-Ledger",
    claim_level:
      "priority-only proof-grade derivation-ref evidence absence classifier; proves that the declared derivation-ref evidence target slots are still target obligations, not evidence objects, and that the imported proof-grade evidence dependency pool supplies no compatible derivation-ref evidence over the 12-separator and 112-row scope",
    preledger_pass: false,
    updates_live_ledger: false,
    branch_chart_authorized: false,
    ...
    derivation_ref_evidence_absence_rule:
      "A proof-grade derivation-ref evidence target is not itself proof-grade derivation-ref evidence. A proof-grade evidence dependency record with empty compatible evidence refs is not proof-grade derivation-ref evidence. This classifier records the absence of a separate compatible derivation-ref evidence object for each live separator and row target slot.",
    separator_proof_grade_derivation_ref_evidence_absence_profiles: separatorAbsenceProfiles,
    row_proof_grade_derivation_ref_evidence_absence_profiles: rowAbsenceProfiles,
    summary,
    next_certificate_handoff: {
      sharpened_blocker:
        "The proof-grade branch now requires a separate compatible accepted_interval_certified_constants_status_proof_grade_derivation_ref evidence object; the target packet and upstream dependency records cannot be promoted into that evidence.",
      continuation_class:
        "mechanical only after a proof-grade derivation-ref evidence object is supplied; this classifier does not derive that object or select a proof rule",
      fail_closed_stop_conditions: [
        "Do not treat a derivation-ref evidence target as derivation-ref evidence.",
        "Do not treat an empty proof-grade evidence dependency record as derivation-ref evidence.",
        "Do not infer a status derivation, status rule, soundness proof, endpoint application, accepted constants conformance derivation, source-packet acceptance rule, primitive acceptance, or route decision from this classifier.",
    ...
    capture_decision:
      "Captured as a priority-only certificate-side proof-grade derivation-ref evidence absence classifier under reference/priorities/proof-programs; no promotion to content/markdown/aaa.",
  };
}
...
function renderReport(packet) {
  ...
  return `# Higher-Fold Layer Same-Packet Candidate-Live Higher-Fold Constants Accepted Interval-Certified Status Proof-Grade Derivation-Ref Evidence Absence Classifier
...
declaring \`${TARGET_FIELD}\` as a target does not construct proof-grade
...
- ${s.evidence_pool_json_files_scanned_upstream} upstream certificate JSON files scanned by the imported evidence dependency classifier;
- ${s.evidence_pool_compatible_proof_grade_status_evidence_files_upstream} upstream compatible proof-grade status evidence files;
- ${s.separator_proof_grade_derivation_ref_evidence_absence_profiles} separator absence profiles;
- ${s.row_proof_grade_derivation_ref_evidence_absence_profiles} row absence profiles;
- ${s.total_proof_grade_derivation_ref_evidence_absence_slots} derivation-ref evidence absence slots;
- ${s.total_proof_grade_derivation_ref_evidence_absence_slots_with_evidence_object} slots with a compatible derivation-ref evidence object;
- ${s.compatible_proof_grade_derivation_ref_evidence_refs} compatible derivation-ref evidence refs;
- ${s.target_packet_as_derivation_ref_evidence_rejections} target-packet-as-evidence rejections;
- ${s.evidence_dependency_record_as_derivation_ref_evidence_rejections} dependency-record-as-evidence rejections.

The first derivation-ref evidence absence blocker is
\`${s.first_proof_grade_derivation_ref_evidence_absence_blocker}\`.
...
\`${TARGET_FIELD}\` evidence object. The target packet and empty proof-grade
