...
import {
  getAllXyzzyReviewGroups,
  loadXyzzyReviewGroupCatalogFromFile,
  loadXyzzyTileCatalogFromFile,
  renderXyzzyGroupReferenceSvg,
  renderXyzzyTileReferenceSvg,
...
function formatUsage() {
  return [
    "Usage: node scripts/xyzzy/render-reference-svg.mjs [options]",
    "",
    "Batch options:",
    ...
    "  --output-prefix text       Tile filename prefix. Default: xyzzy-tile-",
    "  --group-output-prefix text Group filename prefix. Default: xyzzy-group-",
    "",
...
function parseArgs(argv) {
  const parsed = {
    specJsonPath: path.join(repoRootPath, "src", "apps", "xyzzy", "xyzzy-tiles.json"),
    groupSpecJsonPath: path.join(repoRootPath, "src", "apps", "xyzzy", "xyzzy-review-groups.json"),
    outputDirPath: path.join(repoRootPath, "scripts", "glyphs"),
    outputPrefix: "xyzzy-tile-",
    groupOutputPrefix: "xyzzy-group-",
    tileKey: "",
    groupKey: "",
    outputPath: "",
    writeStdout: false,
...
function main() {
  ...
  const catalog = loadXyzzyTileCatalogFromFile(options.specJsonPath);
  const groupCatalog = loadXyzzyReviewGroupCatalogFromFile(options.groupSpecJsonPath);
  const tileByKey = new Map(catalog.tiles.map((tile) => [tile.key, tile]));
  const groupByKey = new Map(getAllXyzzyReviewGroups(groupCatalog).map((group) => [group.key, group]));
  const sampleCounts = {
  ...
  if (options.tileKey || options.groupKey) {
    ...
    if (options.tileKey) {
      ...
      if (!tile) {
        throw new Error(`Unknown Xyzzy tile key: ${options.tileKey}`);
      }
      svgText = renderXyzzyTileReferenceSvg(tile, catalog, { sampleCounts });
      outputPath ||= path.join(options.outputDirPath, `${options.outputPrefix}${tile.key}.svg`);
    } else {
      const group = groupByKey.get(options.groupKey);
      if (!group) {
        throw new Error(`Unknown Xyzzy review group key: ${options.groupKey}`);
      }
      svgText = renderXyzzyGroupReferenceSvg(group, tileByKey, catalog, { sampleCounts });
  ...
  catalog.tiles.forEach((tile) => {
    ...
    writeOutputFile(outputPath, renderXyzzyTileReferenceSvg(tile, catalog, { sampleCounts }));
    writtenPaths.push(outputPath);
  });
  getAllXyzzyReviewGroups(groupCatalog).forEach((group) => {
    const outputPath = path.join(options.outputDirPath, `${options.groupOutputPrefix}${group.key}.svg`);
    writeOutputFile(outputPath, renderXyzzyGroupReferenceSvg(group, tileByKey, catalog, { sampleCounts }));
    writtenPaths.push(outputPath);
  });
