...
export const DEFAULT_XYZZY_REVIEW_BROWSER_PATH =
  "/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge";
export const DEFAULT_XYZZY_REVIEW_PAGE_PATH = "xyzzy-review.html";
export const DEFAULT_XYZZY_REVIEW_WIDTH = 1600;
export const DEFAULT_XYZZY_REVIEW_HEIGHT = 9000;
export const DEFAULT_XYZZY_REVIEW_BUDGET_MS = 12000;
const DEFAULT_XYZZY_REVIEW_EXPORT_TIMEOUT_MS = 30000;
...
function normalizePagePath(pagePath) {
  ...
  return cleaned || DEFAULT_XYZZY_REVIEW_PAGE_PATH;
}

export function formatXyzzyReviewExportUsage() {
  return [
    "Usage: node scripts/export-xyzzy-review.mjs [options]",
    "",
    "Options:",
    ...
    "  --page path         Review page path to render. Default: xyzzy-review.html",
...
export function parseXyzzyReviewExportArgs(argv, options = {}) {
  const cwd = path.resolve(options.cwd ?? process.cwd());
  const defaultOutputDirPath = path.resolve(
    options.outputDirPath ?? path.join(os.tmpdir(), "xyzzy-review-export")
  );

  const parsed = {
    browserPath: options.browserPath ?? DEFAULT_XYZZY_REVIEW_BROWSER_PATH,
    host: options.host ?? "127.0.0.1",
    port: options.port ?? 0,
    width: options.width ?? DEFAULT_XYZZY_REVIEW_WIDTH,
    height: options.height ?? DEFAULT_XYZZY_REVIEW_HEIGHT,
    virtualTimeBudgetMs: options.virtualTimeBudgetMs ?? DEFAULT_XYZZY_REVIEW_BUDGET_MS,
    outputDirPath: defaultOutputDirPath,
    pagePath: DEFAULT_XYZZY_REVIEW_PAGE_PATH,
    query: "",
    pngOutputPath: null,
    pdfOutputPath: null,
    help: false,
  ...
  if (parsed.pngOutputPath != null) {
    parsed.pngOutputPath = resolveOutputPath(parsed.pngOutputPath, {
      ...
      defaultFilename: "xyzzy-review.png",
    });
  }
  if (parsed.pdfOutputPath != null) {
    parsed.pdfOutputPath = resolveOutputPath(parsed.pdfOutputPath, {
      cwd,
      outputDirPath: parsed.outputDirPath,
      defaultFilename: "xyzzy-review.pdf",
    });
  }
...
export function createXyzzyReviewExportUrl({ origin, pagePath, query }) {
...
function resolveStaticRequestPath(rootDir, requestUrl = "/") {
  ...
  const relativePath = pathname === "/" ? DEFAULT_XYZZY_REVIEW_PAGE_PATH : pathname.replace(/^\/+/, "");
...
export async function startXyzzyReviewStaticServer({
  rootDir = process.cwd(),
  host = "127.0.0.1",
  port = 0,
} = {}) {
...
async function runEdgeCommand(browserPath, args, outputPath) {
  ...
  try {
    await Promise.race([
      waitForStableFile(outputPath, DEFAULT_XYZZY_REVIEW_EXPORT_TIMEOUT_MS),
...
export async function exportXyzzyReviewPageWithEdge({
  browserPath = DEFAULT_XYZZY_REVIEW_BROWSER_PATH,
  pageUrl,
  pngOutputPath = null,
  pdfOutputPath = null,
  width = DEFAULT_XYZZY_REVIEW_WIDTH,
  height = DEFAULT_XYZZY_REVIEW_HEIGHT,
  virtualTimeBudgetMs = DEFAULT_XYZZY_REVIEW_BUDGET_MS,
  tempDirPath = fs.mkdtempSync(path.join(os.tmpdir(), "xyzzy-review-edge-")),
} = {}) {
  if (!pageUrl) {
