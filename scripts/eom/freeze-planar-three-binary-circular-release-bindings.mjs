...
// Freeze the exact files reviewed for one planar common-center three-binary constraint launch. This does not authorize
...
function main(args=process.argv.slice(2)){
  ...
  const bytes=Buffer.from(JSON.stringify(manifest,null,2)+'\n');admission.check();writeFileSync(out,bytes,{flag:'wx',mode:0o600});
  admission.check();
