...
} from "./analyze-compact-configuration-sweep.mjs";
import {
...
export const TARGET_CONFIGURATIONS = Object.freeze([
  Object.freeze({
    ...
    modelRevisionSha256:
      "d8bb2d00210eba7ab7c052b8e8b37d218fd23e5593052d5749e799b4abfb1ec7",
  }),
  Object.freeze({
...
const DEFAULT_PACKET = null;
const DEFAULT_SWEEP_INPUT =
  ".local-data/braid-analysis/compact-monte-carlo/configuration-sweep-v2";
const DEFAULT_RECEIPT =
  `${DEFAULT_SWEEP_INPUT}/final-sweep-analyzer-receipt.v2.json`;
const DEFAULT_OUTPUT =
  ".local-data/braid-analysis/resolution-calibration/coincident-midpoint-4-2-1-frequency-and-coaxial-separated-two-planar-braid-co-rotating-v2.json";
...
export function assertProtocolPacketContract(packetText) {
  if (RECEIPT_INSTANTIATION_STATUS !== "instantiated" ||
      !SHA256_PATTERN.test(RECEIPT_SHA256 ?? "")) {
    fail(`facts-first v2 calibration is uninstantiated: ${RECEIPT_UNINSTANTIATED_REASON}.`);
  }
...
function frozenConfigurationIdentity(rows) {
  return rows.map(({
    assemblyId,
    modelRevisionSha256,
    sourceSlug,
    ...
    compareText(left.assemblyId, right.assemblyId) ||
    compareText(left.modelRevisionSha256, right.modelRevisionSha256) ||
    compareText(left.sourceSlug, right.sourceSlug) ||
    compareText(left.candidateId, right.candidateId));
}

function validateExactIdentityPair(value, label) {
  if (!value || typeof value !== "object" || Array.isArray(value) ||
      typeof value.assemblyId !== "string" ||
      !SHA256_PATTERN.test(value.modelRevisionSha256 ?? "") ||
      value.assemblyId !==
        `asm-${value.modelRevisionSha256.slice(0, 32)}`) {
    fail(`${label} must contain a consistent assemblyId and modelRevisionSha256.`);
  }
  return value;
}

export function validateTargetConfigurationIdentity(row) {
  const target = TARGET_CONFIGURATIONS.find(
    (configuration) => configuration.sourceSlug === row.sourceSlug,
  );
  if (!target) return null;
  const reference = validateExactIdentityPair(
    row.referenceConfigurationIdentity,
    `target ${row.sourceSlug} referenceConfigurationIdentity`,
  );
  if (reference.assemblyId !== target.assemblyId ||
      reference.modelRevisionSha256 !== target.modelRevisionSha256) {
    fail(`target ${row.sourceSlug} reference exact identity drifted.`);
  }
  return target;
}
...
export function verifyReceiptAndPacket({
  ...
  if (packetPath === null) {
    fail("facts-first v2 calibration requires an explicit factual packet path.");
  }
  ...
  if (receipt.skippedFiles?.length !== 0 ||
      ...
      receipt.drawCounts?.evaluated +
        receipt.drawCounts?.notEvaluated !== EXPECTED.draws ||
      receipt.drawCounts?.nullScoreRows !==
        receipt.drawCounts?.notEvaluated) {
  ...
  if (fileKinds.shard !== EXPECTED.shardFiles ||
      fileKinds.configuration !== EXPECTED.configurationFiles) {
    fail("receipt shard/configuration file census differs.");
  }
  ...
  if (!Array.isArray(receipt.frozenIdentitySet?.configurations) ||
      receipt.frozenIdentitySet.configurations.length <
        EXPECTED.configurationFiles) {
    fail("receipt does not contain its exact sampled configuration identities.");
  }
  for (const configuration of receipt.frozenIdentitySet.configurations) {
    validateExactIdentityPair(configuration, "receipt sampled configuration");
  }
  if (receipt.perConfigurationCounts?.length !==
      EXPECTED.configurationFiles) {
    fail("receipt does not contain all factual source-configuration counts.");
  }
  for (const configuration of receipt.perConfigurationCounts ?? []) {
    if (configuration.expectedDrawCount !== EXPECTED.perConfiguration ||
        configuration.actualDrawCount !== EXPECTED.perConfiguration) {
      fail(
        `receipt configuration ${configuration.sourceSlug} does not contain 33 draws.`,
      );
    }
  }
  for (const target of TARGET_CONFIGURATIONS) {
    const configuration = receipt.perConfigurationCounts.find(
      (row) => row.sourceSlug === target.sourceSlug,
    );
    if (configuration?.actualDrawCount !== EXPECTED.perConfiguration) {
      fail(`receipt target ${target.sourceSlug} does not contain 33 draws.`);
    }
  }
  const freshReceipt = analyzeCompactAssemblySweep({
    inputDirectory: path.resolve(sweepInput),
    throughWave: 4,
    expectedPilotShardCount: 6,
    expectedWaveShardCount: 6,
    expectedWave3ConfigurationCount: EXPECTED.configurationFiles,
  });
  ...
  const registryConfigurations = frozenConfigurationIdentity(
    loaded.candidates.map((candidate) => candidate.declaration),
  );
  assertEqual(
    registryConfigurations.map((configuration) =>
      configuration.sourceSlug).sort(compareText),
    receipt.perConfigurationCounts.map((configuration) =>
      configuration.sourceSlug).sort(compareText),
  ...
  if (loadCampaignRows) {
    ...
    for (const { compactRow } of census) {
      validateExactIdentityPair(compactRow, `case ${compactRow.caseId}`);
      const candidate = loaded.candidates.find(
        (row) => row.declaration.sourceSlug === compactRow.sourceSlug,
      );
      if (!candidate) {
        fail(`case ${compactRow.caseId} has an unregistered sourceSlug.`);
      }
      const reference = validateExactIdentityPair(
        compactRow.referenceConfigurationIdentity,
        `case ${compactRow.caseId} referenceConfigurationIdentity`,
      );
      if (reference.assemblyId !== candidate.declaration.assemblyId ||
          reference.modelRevisionSha256 !==
            candidate.declaration.modelRevisionSha256) {
        fail(`case ${compactRow.caseId} reference exact identity drifted.`);
      }
      const derived = deriveAssemblyScientificIdentity(
        compactRow.exactRerunInstruction.sampledSpec,
      );
      if (derived.assemblyId !== compactRow.assemblyId ||
          derived.modelRevisionSha256 !== compactRow.modelRevisionSha256) {
        fail(`case ${compactRow.caseId} sampled identity preimage drifted.`);
      }
      validateTargetConfigurationIdentity(compactRow);
    }
    const targetCounts = countBy(census
      .filter(({ compactRow }) =>
        validateTargetConfigurationIdentity(compactRow) !== null)
      .map(({ compactRow }) => compactRow.sourceSlug));
    for (const target of TARGET_CONFIGURATIONS) {
      if (targetCounts[target.sourceSlug] !== EXPECTED.perConfiguration) {
        fail(`loaded target ${target.sourceSlug} count differs from the receipt.`);
      }
    }
  }
  return {
...
function exactSourceForRow(candidate, row) {
  ...
  const sampledIdentity = deriveAssemblyScientificIdentity(sampledSpec);
  ...
  try {
    for (const resolution of ["primary", "refined"]) {
      for (const radius of r0r1Protocol.enclosingSurfaces.radii) {
        const r0Full = buildCoincidentAxisThreeBinarySurfaceEventAnalysisProtocol(
          r0r1Protocol,
          { radius, resolution },
        );
        const r1Full = buildCoincidentAxisThreeBinarySurfaceEventAnalysisProtocol(
          r1r2Protocol,
          { radius, resolution },
        );
        const timeCount =
...
function evaluateSurfaceLevel({
  ...
  try {
    const result = evaluateCoincidentAxisThreeBinaryStreamingSurfaceReductions({
      sourceRecord: exactSource,
...
function evaluateTargetTask(task, context, progress) {
  ...
  return {
    ...
    referenceConfigurationIdentity: clone(
      row.referenceConfigurationIdentity,
    ),
...
function gateInventory(rows) {
  ...
  for (const row of rows) {
    for (const comparison of row.gateComparisons) {
      ...
      if (comparison.disagreed) {
        ...
        gate.sourceSlugs.add(row.compactRow.sourceSlug);
      }
    }
  }
  return [...gates.values()].map((gate) => ({
    ...
    disagreementSourceSlugs: [...gate.sourceSlugs].sort(compareText),
...
function buildBoundaryStress(rows) {
  ...
  const sourceSlugs = [...new Set(
    rows.map((row) => row.compactRow.sourceSlug),
  )].sort(compareText);
  for (const sourceSlug of sourceSlugs) {
    const configurationRows = rows.filter(
      (row) => row.compactRow.sourceSlug === sourceSlug &&
        drawState(row.compactRow) !== "not-evaluated",
    ).map((row) => ({
      row,
    ...
    for (const state of ["pass", "reject"]) {
      const chosen = configurationRows.filter((entry) => entry.state === state)
        .sort((left, right) =>
  ...
  return {
    ...
    selectionRule:
      "per factual source configuration, two passing and two rejecting evaluated compact rows with smallest normalized gate margin; ascending case hash tie-break",
    selected,
    missing,
...
function configurationInventories(rows) {
  const sourceSlugs = [...new Set(rows.map(
    (row) => row.compactRow.sourceSlug,
  ))].sort(compareText);
  return sourceSlugs.map((sourceSlug) => {
    const configurationRows = rows.filter(
      (row) => row.compactRow.sourceSlug === sourceSlug,
    );
    const inventory = confusionInventory(configurationRows);
    return {
      sourceSlug,
      referenceConfigurationIdentity: clone(
        configurationRows[0].compactRow.referenceConfigurationIdentity,
      ),
      ...inventory,
    };
  });
...
function macroDrawState(configurationInventories) {
  const average = (selector) =>
    configurationInventories.reduce((sum, row) => sum + selector(row), 0) /
    configurationInventories.length;
  return {
    sourceConfigurationCount: configurationInventories.length,
    equalSourceConfigurationWeight: true,
...
function groupFailures(rows, side) {
  ...
  for (const row of rows) {
    ...
    groups.get(key).rows.push({
      ...
      sourceSlug: row.compactRow.sourceSlug,
...
function addQueueReason(queue, row, reason) {
  ...
  if (!queue.has(key)) {
    queue.set(key, {
      ...
      assemblyId: row.compactRow.assemblyId,
      modelRevisionSha256: row.compactRow.modelRevisionSha256,
      sourceSlug: row.compactRow.sourceSlug,
      referenceConfigurationIdentity: clone(
        row.compactRow.referenceConfigurationIdentity,
      ),
      sampleOrdinal: row.compactRow.sampleOrdinal,
      reasons: [],
    });
...
function sufficiencyAssessment({
  ...
  const sourceSlugs = [...new Set(fullPassRows.map(
    (row) => row.compactRow.sourceSlug,
  ))].sort(compareText);
  const targetSourceSlugs = TARGET_CONFIGURATIONS.map(
    (configuration) => configuration.sourceSlug,
  );
  const conditions = [
    {
    ...
    {
      id: "target-source-configuration-full-pass-breadth",
      passed: targetSourceSlugs.every((sourceSlug) =>
        sourceSlugs.includes(sourceSlug)),
      observed: sourceSlugs.filter((sourceSlug) =>
        targetSourceSlugs.includes(sourceSlug)),
      required: targetSourceSlugs,
    },
    {
      id: "full-pass-source-configuration-breadth",
      passed: sourceSlugs.length >= 11,
      observed: sourceSlugs.length,
      required: 11,
    },
    {
    ...
    },
    {
      ...
      passed: gates.every((gate) =>
        ...
        gate.disagreementSourceSlugs.length < 3),
      observedFailures: gates.filter((gate) =>
        (gate.disagreementRate ?? 0) > 0.10 ||
        gate.disagreementSourceSlugs.length >= 3),
      required:
        "no gate above 10 percent or across three factual source configurations before full adjudication",
    },
  ];
  return {
    status: conditions.every((condition) => condition.passed)
      ...
      : "calibration-insufficient",
    fullPassSourceSlugs: sourceSlugs,
    conditions,
...
function journalHeader(binding, outputPath) {
  return {
    schema: "prescribed-path-analysis/resolution-calibration-journal.v2",
...
function assembleArtifact({
  ...
  const configurations = configurationInventories(rows);
  ...
  const body = {
    ...
    artifactId:
      "coincident-midpoint-4-2-1-frequency-and-coaxial-separated-two-planar-braid-co-rotating-resolution-coverage-v2",
    ...
    sourceCensus: {
      ...
      configurationFileCount: EXPECTED.configurationFiles,
      evaluatedCompactCount: binding.receipt.drawCounts.evaluated,
      retainedCompactNullScoreCount:
        binding.receipt.drawCounts.notEvaluated,
      manifest: binding.receipt.campaignAndFileManifest,
      syntheticCampaignIdentityCreated: false,
    },
    compactVersusFull: {
      inventory,
      perConfiguration: configurations,
      equalConfigurationMacroDrawState: macroDrawState(configurations),
      gateDisagreements: gates,
      boundaryStress,
      sufficiency,
    },
    targetedResolutionLadder: {
      targetConfigurations: clone(TARGET_CONFIGURATIONS),
      expectedRowsPerConfiguration: EXPECTED.perConfiguration,
...
export function verifyCalibrationArtifact({
  ...
  if (artifact.rows?.length !== EXPECTED.draws ||
      artifact.targetedResolutionLadder?.rowCount !==
        EXPECTED.perConfiguration * TARGET_CONFIGURATIONS.length ||
      Object.values(
...
async function runCalibration(options) {
  ...
  const targetJobs = binding.census.filter(
    ({ compactRow }) => validateTargetConfigurationIdentity(compactRow) !== null,
  ).map((item) => ({
    kind: "target",
  ...
  await runStage(
    targetJobs,
    "coincident-midpoint-4-2-1-and-coaxial-planar-co-rotating-resolution-ladder",
  );
