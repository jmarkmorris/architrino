...
function captureSource(filename,expected) {
  ...
  try {
    ...
    check(sameFile(before,fstatSync(fd))&&sameFile(before,lstatSync(filename))&&(!expected||sha(bytes)===expected),"composition generation differs");
    return {path:filename,sha256:sha(bytes),bytes:bytes.length,data:bytes};
...
export async function runFileWorker(job,bytes,remainingMs,signal) {
  ...
  const worker=new Worker(`const{parentPort,workerData}=require('node:worker_threads');
    ...
    .then(async m=>{return m.fileOperation(workerData.job);}).then(value=>parentPort.postMessage({value}))
...
export function assertNoCompetingPilot(table,ownPid) {
  ...
  check(!table.some(r=>!own.has(r.pid)&&/reduce-prescribed-acceleration-response\.py|(?:run|launch)-f6c(?:-cached)?-root-cover-(?:pilot|full)\.mjs|prepare-f6c(?:-cached)?-continuous-reception-root-cover\.py/u.test(r.command)),"competing response/root-cover attempt observed");
}
  ...
  try {
    ...
    for(let end;(end=state.pending.indexOf("\n"))>=0;){
      ...
      if(row&&Object.hasOwn(row,"completedRows")) {
        check(Number.isSafeInteger(row.completedRows)&&row.completedRows>=state.completedRows&&row.completedRows<=10240,"invalid observed row progress");
...
export async function launchCaptured({root,options,self,entry,outerBytes,began,deadlineNanoseconds}) {
  ...
  check(path.dirname(output)===lane&&!existsSync(output)&&realpathSync(lane)===lane,"fresh direct child of canonical ignored root-cover lane required");
  ...
  const live=()=>{check(!failure&&!abort.signal.aborted, failure?.message??"full cover interrupted");check(remaining()>0,"inclusive full-cover deadline");};
  const fail=error=>{failure??=error;abort.abort(failure);if(activeOuter)process.emit("SIGTERM");};
  const interrupt=()=>{if(!abort.signal.aborted)fail(new Error("operator interrupted full cover"));};
  ...
  const logBytes=()=>{const size=stdioTotal.bytes+monitorTotal.bytes+monitoredFiles().reduce((n,p)=>n+statRegular(p,LOG_LIMIT),0)+statRegular(path.join(output,"full-rejection.json"),LOG_LIMIT);
  ...
  const worker=job=>runFileWorker({...job,root,deadlineNanoseconds},entry.data,remaining(),abort.signal);
  ...
  try {
    ...
    deadlineTimer=setTimeout(()=>fail(new Error("inclusive full-cover wall deadline")),Math.max(1,remaining()));
    ...
    timer=setInterval(()=>{
      try {
        ...
        if(performance.now()>=nextHeartbeat){nextHeartbeat=performance.now()+15000;log({kind:"f6c-full-heartbeat",stage:currentStage,elapsedSeconds:elapsed(began),
    ...
    for(const stage of ["consumer","comparison"]) {
      ...
      if(stage==="consumer")completedRows=10240;else comparedRows=10240;
    ...
    const record={schema:"braid-program/f6c-cached-root-cover-full-admission.v1",accepted:true,scope:"full",
      authority:"source-bound full 160-cell conditional reconstructed-family coverage with separate frozen comparison",
    ...
    completion={completed:true,accepted:true,scope:"full",receipt:publication,logs,
    ...
    if(reserved)try{
      const rejection={completed:false,accepted:false,scope:"full",failure:String(error.message).slice(0,4096),stage:currentStage,
      ...
      C.writeNew(path.join(output,"full-rejection.json"),rejection,remainingLog);
...
async function main() {
  ...
  const C=await import(sourceURL(entry.data));
  const outer=captureSource(path.join(root,OUTER));
