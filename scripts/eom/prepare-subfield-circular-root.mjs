...
import { SUBFIELD_CIRCULAR_REFERENCES } from "../../src/prescribed-path-analysis/SubfieldCircularRootLedgerReducer.mjs";
...
export function referenceSnapshot() {
  const records = SUBFIELD_CIRCULAR_REFERENCES.map(({id,path:filename,sha256}) => {
    const record=fileBinding(filename);
    if (record.sha256 !== sha256) throw new Error(`reference changed: ${filename}`);
    return {id,...record};
  });
...
export function parsePrepareSubfieldCircularArgs(argv) {
  if (argv.length !== 2 || argv[0] !== "--out") throw Error("usage: --out NEW-BUILD-DIRECTORY");
  const value = argv[1];
  if (!value.startsWith(BASE) || value === BASE || path.isAbsolute(value) || value.split("/").some(p => !p || p === "." || p === "..")) throw Error("fresh relative circular build directory required");
...
export async function prepareSubfieldCircular(argv, captured) {
  ...
  const receipt = { schema: "braid-program/subfield-circular-root-build.v1", status: "incomplete",
    ...
    sourcesBefore: sourceSnapshot(admission), referencesBefore: referenceSnapshot(admission), stages: [] };
