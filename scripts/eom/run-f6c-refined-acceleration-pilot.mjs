...
export const ENTRY='scripts/eom/run-f6c-refined-acceleration-pilot.mjs';
export const LAUNCHER='scripts/eom/launch-f6c-refined-acceleration-pilot.mjs';
...
export const CHECKER='scripts/eom/verify-f6c-refined-acceleration.py';
...
export const PURE_FLAGS=Object.freeze('accepted source_authenticated reference_generation_authenticated root_coverage_established premise_truth_authenticated subject_membership_established historical_trajectory_identity_established execution_authorized eom_executed metrics_available score_authorized h3_evidence_eligible equilibrium_established retention_established physical_realization_established'.split(' '));
export const ANALYSIS=Object.freeze({conditional_projection_conformant:true,conditional_ranges_conformant:true,cells:1,members:8,pair_rows:64,ordinary_pairs:56,self_zeros:8,piece_records:112,compared_pair_components:192,compared_member_intervals:80,...Object.fromEntries(PURE_FLAGS.map(k=>[k,false]))});
...
export const NAMED=Object.freeze({
 ...
 comparisonCoreControls:'tests/test_f6c_refined_acceleration_conformance.py',
 rangeReference:'scripts/eom/oracle/continuous_reception_acceleration.py',
 rangeReferenceControls:'tests/test_eom_continuous_reception_acceleration.py',
...
export function decode(bytes,limit=FILE_LIMIT,documentClass='data') {
  check(documentClass==='data'||documentClass==='operational-receipt','closed metadata document class');const stringLimit=documentClass==='operational-receipt'?131072:8192;
  ...
  function string(){const start=at++;while(at<text.length){const c=text[at++];if(c==='"'){const value=JSON.parse(text.slice(start,at));check(value.length<=stringLimit,'JSON string bound');return value;}if(c==='\\')at++;}throw Error('unterminated string');}
  function item(depth){check(depth<=24&&++nodes<=1000000,'JSON structure limit');space();const c=text[at];if(c==='"')return string();
    if(c==='{'||c==='['){at++;const object=c==='{',result=object?Object.create(null):[],seen=new Set();space();if(text[at]===(object?'}':']')){at++;return result;}
      while(true){space();let key;if(object){check(text[at]==='"','JSON key');key=string();check(key.length<=4096&&seen.size<10000,'JSON key/object bound');check(!seen.has(key),'duplicate JSON key');seen.add(key);space();check(text[at++]===':','JSON colon');}
        const v=item(depth+1);if(object)result[key]=v;else{check(result.length<20000,'JSON array bound');result.push(v);}space();const end=text[at++];if(end===(object?'}':']'))return result;check(end===',','JSON delimiter');}}
...
function bindings(rows){check(Array.isArray(rows)&&rows.length>0&&rows.length<=512,'bounded binding list');rows.forEach(binding);check(new Set(rows.map(b=>b.path)).size===rows.length,'duplicate binding path');}
...
export function validatePlan(plan,root,launcherSha,entrySha,python,git) {
  ...
  closed(plan,['schema','scope',...Object.keys(NAMED),'runtimeBindings','operationalBindings','limits','priorRefinementClosure'],'plan');
  check(plan.schema==='braid-program/f6c-refined-acceleration-launch.v1'&&plan.scope===SCOPE&&equal(plan.limits,LIMITS),'fixed scope/limits');
  for(const [key,p]of Object.entries(NAMED)){binding(plan[key]);check(plan[key].path===p,'reviewed source/control generation');}
  ...
  check(equal(plan.operationalBindings.map(b=>b.path).sort(),ops.sort()),'exact nine operational bindings');
  check(equal((({ownerSha256,...rest})=>rest)(plan.priorRefinementClosure),PRIOR_CLOSURE),'refined external closure premise');
  return plan;
}
export function mergeBindings(rows){
 const map=new Map();
 for(const row of rows){const old=map.get(row.path);check(!old||(old.sha256===row.sha256&&(old.bytes===undefined||row.bytes===undefined||old.bytes===row.bytes)),'conflicting binding');map.set(row.path,{...old,...row});}
 return [...map.values()];
}
export function priorAdmissionBindings(admission,generation,root){
 check(generation==='broad'||generation==='refined','known prior generation');bindings(admission.sourceBindings);
 if(generation==='refined')check(admission.sourceBindings.length===202,'complete original202 source bindings');
 check(admission.accepted===true&&admission.processesClosed===true&&admission.stages?.length===2,'prior accepted closed generation');
 const rows=admission.sourceBindings.map(b=>absolute(b,root)),labels=generation==='broad'?['consumer','comparison']:['producer','comparison'];
 admission.stages.forEach((stage,i)=>{
  check(stage.stage===labels[i]&&stage.process.accepted===true&&stage.process.processesClosed===true&&stage.process.exit?.code===0&&stage.process.exit.signal===null,'original closed stage order');
  for(const key of ['stdoutLog','stderrLog']){binding(stage.process[key]);rows.push(absolute(stage.process[key],root));}
 });return mergeBindings(rows);
}
export function priorSourceBindings(root,live=()=>{}){
 // Pinned admission bytes authenticate the original source-generation lists.
 // No mathematical data is imported/evaluated, and no prior cost is transferred.
 const rows=[];
 for(const [generation,items]of [['broad',FIXED],['refined',REFINED]]){
  const [,p,h]=items.find(([role])=>role==='admission'),file=readBound(path.join(root,p),h,true,FILE_LIMIT,live),admission=decode(file.data,FILE_LIMIT,'operational-receipt');
  rows.push(...priorAdmissionBindings(admission,generation,root),clean(file));
 }
 return mergeBindings(rows);
}
export function planBindings(plan,root,live=()=>{}) {

 const rows=[...Object.values({...SOURCE_PATHS,...REFINED_PATHS}).map(p=>({path:path.join(root,p)})),...[...FIXED,...REFINED,...PRIOR_OPERATIONS].map(([,p,h,bytes])=>({path:path.join(root,p),sha256:h,...(bytes?{bytes}:{})})),
  ...Object.keys(NAMED).filter(k=>k!=='declaration').map(k=>absolute(plan[k],root)),...plan.runtimeBindings.map(b=>absolute(b,root)),
  ...plan.operationalBindings.map(b=>absolute(b,root)),...priorSourceBindings(root,live)];
 return mergeBindings(rows);
}
...
print(_json.dumps({'kind':'f6c-refined-range-python-process-resources','userSeconds':_self.ru_utime,'systemSeconds':_self.ru_stime,'waitedChildUserSeconds':_children.ru_utime,'waitedChildSystemSeconds':_children.ru_stime,'maximumIndividualResidentBytes':_self.ru_maxrss if sys.platform=='darwin' else _self.ru_maxrss*1024}),file=sys.stderr,flush=True)
`;
...
print(json.dumps({'schema':'braid-program/f6c-refined-acceleration-python-runtime-inventory.v1','scientificDataLoaded':False,'scientificModulesExecuted':False,'pythonInvocation':sys.executable,'pythonRealPath':str(pathlib.Path(sys.executable).resolve()),'files':[str(p) for p in sorted(files)]}))
`;
export function stageSpec({stage,plan,planBinding,root,output,python,git,candidate,budget}) {
  ...
  const paths=outputPaths(root,output),source=stage==='consumer'?CONSUMER:CHECKER,digest=stage==='consumer'?plan.consumer.sha256:plan.verifier.sha256;
  const args=['-I','-B','-c',PYTHON_BOOTSTRAP,path.join(root,source),'--plan',planBinding.path,'--plan-sha256',planBinding.sha256,
...
function sourceMap(job,items=[...FIXED,...Object.entries(SOURCE_PATHS)]){const byPath=new Map(job.sources.map(b=>[b.path,b]));return Object.fromEntries(items.map(([role,p])=>{const b=byPath.get(path.join(job.root,p));check(b,'missing fixed source');return [role,b];}));}
...
export function admitStage(job) {
  ...
  const record=decode(readBound(completion.output.path,completion.output.sha256,true).data),fixed=sourceMap(job),refined=sourceMap(job,[...REFINED,...Object.entries(REFINED_PATHS)]),priorOperations=sourceMap(job,PRIOR_OPERATIONS);
  const sources=Object.fromEntries(Object.keys(NAMED).map(k=>[k,absolute(plan[k],root)]));
  if(stage==='consumer'){
    closed(completion,'completed accepted scope output conditionalCells pairRows ordinaryPairs selfZeros members elapsedSeconds processUserSeconds processSystemSeconds maximumIndividualProcessResidentBytes independentComparisonRequired externalInclusiveDeadlineAndProcessClosureRequired metricsAvailable scoreAuthorized h3EvidenceEligible eomExecuted rootsEvaluated accelerationEvaluated'.split(' '),'producer completion');
    ...
    check(completion.eomExecuted===false&&completion.rootsEvaluated===false&&completion.accelerationEvaluated===true,'actual range-only computation scope');
    ...
    closed(record,CANDIDATE_KEYS,'range candidate');
    check(record.schema==='braid-program/f6c-refined-acceleration-candidate.v1'&&record.scope===SCOPE&&record.accepted===false&&record.status==='conditional-range-candidate'&&equal(record.census,CENSUS),'complete unpromoted candidate');
    check(equal(record.launchPlan,planBinding)&&equal(record.ancestryBindings,fixed)&&equal(record.refinementBindings,refined)&&equal(record.sourceBindings,sources)&&equal(record.consumer,sources.consumer)&&equal(record.declaration,sources.declaration)&&equal(record.verifier,sources.verifier)&&equal(record.priorRefinementClosure,plan.priorRefinementClosure)&&equal(record.runtimeBindings,plan.runtimeBindings)&&equal(record.operationalBindings,plan.operationalBindings)&&record.publicationRequires===CANDIDATE_PUBLICATION,'candidate source chain');
    ...
    closed(record,'schema scope accepted authority candidate launchPlan verifier sourceBindings ancestryBindings refinementBindings priorOperationalBindings priorOperationalObservations executionBindings priorRefinementClosure analysis referenceClaims candidateClaims publicationRequires elapsedSecondsBeforePublication'.split(' '),'checker report');
    check(record.schema==='braid-program/f6c-refined-acceleration-conformance.v1'&&record.accepted===true&&record.scope===SCOPE&&record.authority==='source-bound independent refined projection and conditional rational range containment only'&&equal(record.analysis,ANALYSIS),'conditional range comparison');
    check(equal(record.candidate,job.candidate)&&equal(record.launchPlan,planBinding)&&equal(record.verifier,sources.verifier)&&equal(record.sourceBindings,sources)&&equal(record.ancestryBindings,fixed)&&equal(record.refinementBindings,refined)&&equal(record.priorOperationalBindings,priorOperations)&&equal(record.executionBindings,[...plan.runtimeBindings,...plan.operationalBindings].map(b=>absolute(b,root)))&&equal(record.priorRefinementClosure,plan.priorRefinementClosure),'checker source/candidate chain');
    check(equal(record.priorOperationalObservations,{prepublicationHostObservations:19,finalHostObservations:20,prepublicationRssSamples:954,finalRssSamples:955,finalLogBytes:499829,wholeCompletionSource:'external declaration observation, not these logs'}),'prior operational observations, not new cost');
...
export function fileOperation(job) {
  ...
  if(job.kind==='preflight'){
    ...
    const sources=checkBindings([...planBindings(plan,job.root,live),clean(captured)],live);return {plan,planBinding:clean(captured),sources};}
...
export const SOURCE_PATHS=Object.freeze({
  "priorPlan": "reference/priorities/braid-program/evidence/2026-08-27-f6c-cached-root-cover-pilot-launch.v1.json",
  "reference": "scripts/eom/oracle/continuous_reception_acceleration.py",
  "referenceControls": "tests/test_eom_continuous_reception_acceleration.py",
});
...
export const REFINED_PATHS=Object.freeze({
  "plan": "reference/priorities/braid-program/evidence/2026-08-27-f6c-emission-refinement-launch.v2.json"
});
export const REFINED=Object.freeze([
  [
    "queries",
    ".local-data/braid-analysis/f6c-emission-refinement-20260827/pilot-cell-0-v2/queries.ndjson",
    "44d59ae62f8d7d9a9e7afd1d684e8ee15b8aeadf4dc92d489a787e5e224029fa"
  ],
  [
    "rows",
    ".local-data/braid-analysis/f6c-emission-refinement-20260827/pilot-cell-0-v2/rows.ndjson",
    "b6309b3c90f75590ba8270ea6cea1644be46727692a19be6aa364163e108035f"
  ],
  [
    "pieces",
    ".local-data/braid-analysis/f6c-emission-refinement-20260827/pilot-cell-0-v2/pieces.ndjson",
    "075007966aa14b3d0c9ff896d5cb752d06e410f1002dc2bc39a8e7d8db55340a"
  ],
  [
    "manifest",
    ".local-data/braid-analysis/f6c-emission-refinement-20260827/pilot-cell-0-v2/cover-manifest.json",
    "d4ec0d60631dd46cf2872ace941677dfd980af6f8cdbe540347b5252d336ebb6"
  ],
  [
    "comparison",
    ".local-data/braid-analysis/f6c-emission-refinement-20260827/pilot-cell-0-v2-outer/comparison.json",
    "eed41550c3c743df419efab8d0f9ad6094b43fa1efe3f4949365e731c1f7c63e"
  ],
  [
    "admission",
    ".local-data/braid-analysis/f6c-emission-refinement-20260827/pilot-cell-0-v2-outer/pilot-admission.json",
    "51f0b3774bfb489bbab4fddd7f7612c6d4132f2654a36aa4091e5445eca9b51c"
  ]
].map(Object.freeze));
...
export const PRIOR_CLOSURE=Object.freeze({authority:'externally-reviewed-caller-observation',admissionSha256:REFINED.find(([k])=>k==='admission')[2],matchingFreshCompletionObserved:true,exitCode:0,elapsedSeconds:'238.116677375',processesClosed:true,independentAuditAccepted:true});
