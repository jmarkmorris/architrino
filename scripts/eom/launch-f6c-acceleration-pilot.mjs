...
export function captureBootstrapSource(filename,expected){
  ...
  try{const before=fstatSync(fd,{bigint:true});check(before.isFile()&&before.size>0n&&before.size<=1024n**2n,'bounded source');
    ...
    check(identity(before)===identity(fstatSync(fd,{bigint:true}))&&identity(before)===identity(lstatSync(filename,{bigint:true}))&&(!expected||sha(data)===expected),'source generation differs');return {path:filename,sha256:sha(data),bytes:data.length,data};
  }finally{closeSync(fd);}
}
export async function reviewedHelpers(bytes){return import(url(bytes));}
...
export async function launchCaptured({root,options,self,entry,outerBytes,helperBytes,began,deadlineNanoseconds}){
  ...
  const H=await reviewedHelpers(helperBytes),outer=await import(url(outerBytes));
  ...
  const worker=job=>H.runFileWorker({...job,root,deadlineNanoseconds},entry.data,remaining(),abort.signal);
...
async function main(began){const deadlineNanoseconds=String(process.hrtime.bigint()+1800000000000n),options=parseArgs(process.argv.slice(2)),root=realpathSync(process.cwd());
  ...
  const helpers=captureBootstrapSource(path.join(root,HELPERS)),outer=captureBootstrapSource(path.join(root,OUTER));
