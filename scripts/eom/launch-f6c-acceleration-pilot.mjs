...
const SELF='scripts/eom/launch-f6c-acceleration-pilot.mjs',ENTRY='scripts/eom/run-f6c-acceleration-pilot.mjs';
...
export async function ignoredOutputs(git,root,output){
  check(path.dirname(output)===path.join(root,'.local-data/braid-analysis/f6c-continuous-reception-acceleration-20260827'),'scoped ignored output');
...
export async function launchCaptured({root,options,self,entry,outerBytes,helperBytes,began,deadlineNanoseconds}){
  ...
  const poll=()=>{size(paths.candidate,FILE_LIMIT);size(paths.comparison,FILE_LIMIT);privateCandidates(output,'.range-private-');privateCandidates(paths.operations,'.range-comparison-private-');logBytes();};
  ...
  try{
    ...
    interval=setInterval(()=>{try{
      ...
      if(performance.now()>=nextHeartbeat){nextHeartbeat=performance.now()+15000;log({kind:'f6c-acceleration-pilot-heartbeat',stage,elapsedSeconds:elapsed(began),closedAdmittedStages:stages.filter(s=>s.admission?.accepted).map(s=>s.stage),maximumSampledAggregateRSSBytes:rss.maximumSampledRSSBytes,accepted:false});}
      poll();
    ...
    for(const name of ['consumer','comparison']){
      ...
      const args=['--plan',planBinding.path,'--plan-sha256',planBinding.sha256,'--entry-sha256',entry.sha256,'--launcher-sha256',self.sha256,
        '--stage',stage,'--out',output,'--deadline-ns',deadlineNanoseconds,'--candidate-sha256',candidate?.sha256??'none','--python',options.python,'--git-binary',options.git,'--source-map-sha256',options.sourceMapSha256];
      ...
      try{receipt=await outer.superviseRegisteredPilot({root,entry:ENTRY,args,sources:[{path:ENTRY,sha256:entry.sha256,bytes:entry.data}],
        ...
        inspectProcesses:H.startupAbortInspection(inspect,abort.signal),admit:async({receipt:processReceipt,signal})=>{
          live();return H.runFileWorker({kind:'admit',root,sourceMapSha256:options.sourceMapSha256,output,stage,plan,planBinding,sources,python:options.python,git:options.git,candidate,
    ...
    const record={schema:'braid-program/f6c-acceleration-pilot-admission.v1',accepted:true,scope:C.SCOPE,
      authority:'source-bound one-cell conditional acceleration/residual-range output conformance only',processesClosed:true,stages,plan:planBinding,
      ...
      eomExecuted:false,rootsEvaluated:false,h3EvidenceEligible:false,metricsAvailable:false,scoreAuthorized:false,historicalTrajectoryIdentityEstablished:false,fullRunAuthorized:false};
    ...
    completion={completed:true,accepted:true,scope:C.SCOPE,receipt:publication,logs,rawOutputBytes:stages.flatMap(s=>s.admission.outputs).reduce((n,b)=>n+b.bytes,0),
      ...
      eomExecuted:false,rootsEvaluated:false,h3EvidenceEligible:false,metricsAvailable:false,scoreAuthorized:false,historicalTrajectoryIdentityEstablished:false,fullRunAuthorized:false};return completion;
  }catch(error){
    fail(error);await stopMonitors();if(reserved)try{const record={completed:false,accepted:false,scope:C.SCOPE,stage,failure:String(error.message).slice(0,4096),invalidatesPublication:publication??null,
    ...
    }catch(preservation){originalError('Range rejection preservation failed: '+preservation.message);}throw error;
  }finally{
    await stopMonitors();if(lock)try{H.releaseLock(lock);}catch(error){failure??=error;originalError(error.message);}
...
export function parseArgs(argv){const v={};for(let i=0;i<argv.length;i+=2){check(argv[i+1]&&!v[argv[i]],'unique paired arguments');v[argv[i]]=argv[i+1];}
  check(Object.keys(v).sort().join('|')===['--out','--plan','--plan-sha256','--launcher-sha256','--entry-sha256','--python','--git-binary','--source-map-sha256'].sort().join('|'),'closed launcher arguments');
  for(const key of ['--plan-sha256','--launcher-sha256','--entry-sha256','--source-map-sha256'])check(/^[a-f0-9]{64}$/u.test(v[key]),'reviewed SHA required');
...
async function main(began){const deadlineNanoseconds=String(process.hrtime.bigint()+1800000000000n),options=parseArgs(process.argv.slice(2)),root=realpathSync(process.cwd());
  ...
  const captured=await import(url(self.data)),result=await captured.launchCaptured({root,options,self,entry,outerBytes:outer.data,helperBytes:helpers.data,began,deadlineNanoseconds});
  ...
  await drainDiagnostics({began,lastSampleStartedMs:result.lastRSSObservationStartedAtMs});result.elapsedSeconds=elapsed(began);
...
if(import.meta.url.startsWith('file:')&&process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  const began=performance.now();main(began).catch(error=>failedCLICompletion(error,{began}));
}
