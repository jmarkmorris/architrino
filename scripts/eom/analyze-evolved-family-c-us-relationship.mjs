...
import {
  EVOLVED_FAMILY_C_US_RESULT_SCHEMA,
  evaluateEvolvedFamilyCUsRelationship,
  summarizeEvolvedUsRows,
  validateEvolvedFamilyCUsProtocol,
} from "../../src/eom-analysis/EvolvedFamilyCUsRelationship.mjs";

export const EVOLVED_FAMILY_C_US_STUDY_SCHEMA =
  "eom-analysis/evolved-family-c-us-study.v1";
export const EVOLVED_FAMILY_C_US_STUDY_RESULT_SCHEMA =
  "eom-analysis/evolved-family-c-us-study-result.v1";
...
export function evaluateEvolvedFamilyCUsStudy({
  study,
  baseDirectory,
}) {
  if (!study || typeof study !== "object" || Array.isArray(study)) {
    throw new TypeError("evolved Family-C U-S study must be an object.");
  }
  if (study.schema !== EVOLVED_FAMILY_C_US_STUDY_SCHEMA) {
    throw new TypeError(
      `study.schema must be ${EVOLVED_FAMILY_C_US_STUDY_SCHEMA}.`,
    );
  }
  ...
  const protocol = validateEvolvedFamilyCUsProtocol(study.protocol);
  ...
  const results = study.runs.map((run, index) => {
    ...
    const result = evaluateEvolvedFamilyCUsRelationship({ record, protocol });
    if (result.schema !== EVOLVED_FAMILY_C_US_RESULT_SCHEMA) {
  ...
  if (generatingSpecs.size !== 1) {
    throw new Error(
      "all study runs must bind the same Family-C campaign generatingSpec.",
    );
  }
  ...
  const resultWithoutHash = {
    schema: EVOLVED_FAMILY_C_US_STUDY_RESULT_SCHEMA,
    studyId,
    branchId,
...
function parseArguments(argv) {
  ...
  if (!options.studyPath || !options.outputPath) {
    throw new TypeError(
      "usage: analyze-evolved-family-c-us-relationship.mjs " +
...
function main(argv) {
  ...
  process.stdout.write(
    `evolved Family-C U-S study written: ${outputPath}\n` +
