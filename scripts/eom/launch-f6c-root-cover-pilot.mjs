...
const SELF="scripts/eom/launch-f6c-root-cover-pilot.mjs",ENTRY="scripts/eom/run-f6c-root-cover-pilot.mjs";
...
export function assertNoCompetingPilot(table,ownPid) {
  ...
  check(!table.some(r=>!own.has(r.pid)&&/reduce-prescribed-acceleration-response\.py|(?:run|launch)-f6c-root-cover-pilot\.mjs|prepare-f6c-continuous-reception-root-cover\.py/u.test(r.command)),"competing response/root-cover pilot observed");
}
...
export async function launchCaptured({root,options,self,entry,outerBytes,began,deadlineNanoseconds}) {
  ...
  try {
    ...
    const record={schema:"braid-program/f6c-root-cover-pilot-admission.v1",accepted:true,scope:"pilot-cell-0",
