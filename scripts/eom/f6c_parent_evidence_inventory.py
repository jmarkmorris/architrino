...
def path(value):
    require((type(value) is str) and (1 < len(value) <= 2048) and (value.startswith('/')) and (not value.startswith('//')) and ('\0' not in value) and ('\\' not in value) and (all(p not in ('', '.', '..') for p in value.split('/')[1:])), 'canonical absolute path')
    return value
...
def decode(raw, expected):
    binding(expected)
    require((type(raw) is bytes) and (0 < len(raw) <= MAX_RAW) and (len(raw) == expected['bytes']) and (hashlib.sha256(raw).hexdigest() == expected['sha256']), 'external raw binding')
    try:
