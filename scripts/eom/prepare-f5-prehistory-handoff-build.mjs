...
function fileBinding(filename) {
  ...
  return {path:full.startsWith(ROOT+"/")?path.relative(ROOT,full):full,realPath,sha256:f5Crypto.createHash("sha256").update(raw).digest("hex"),bytes:raw.length};
}
function scopedPath(value,prefix) {
  if(typeof value!=="string"||!value.startsWith(prefix)||value===prefix||path.isAbsolute(value)||value.includes("\\")||value.split("/").some(p=>!p||p==="."||p===".."))throw Error("fresh scoped F5 build path required");
  const full=path.join(ROOT,value);let parent=full;
  while(!existsSync(parent))parent=path.dirname(parent);
  if(realpathSync(parent)!==parent)throw Error("symlinked build path");
  return full;
}
...
const SOURCE_PATHS = ["src/eom/src/History.cpp","src/eom/src/Interval.cpp","src/eom/include/architrino/eom/Decimal.hpp","src/eom/include/architrino/eom/History.hpp","src/eom/src/CoupledEvolution.cpp","tests/test_f5_prehistory_handoff_producer.py","scripts/eom/verify-f5-prehistory-handoff.py","tests/test_f5_prehistory_handoff.py","src/eom/CMakeLists.txt","scripts/eom/prepare-f5-prehistory-handoff.py","src/eom/native/eom_f5_prehistory_inspector.cpp"];
...
export async function prepare(argv) {
  ...
  try {
    ...
    receipt.toolsBefore = [...new Set([process.execPath, python, cmake, xcrun, swVers, compiler, ar, ranlib, linker, otool])].sort().map(capabilityPath);
