...
SOURCE_PATHS = {'reference': REFERENCE, 'referenceControls': 'tests/test_eom_continuous_reception_acceleration.py'}
IDS = ('0+', '0-', '1+', '1-', '2+', '2-', '3+', '3-')
...
NAMED = {
    ...
    'verifier': 'scripts/eom/verify-f6c-refined-acceleration.py',
    'verifierControls': 'tests/test_f6c_refined_acceleration.py',
...
def _parse_document(raw, *, metadata, string_cap):
    ...
    def number_decimal(text):
        ...
        value=Decimal(text)
        require((value.is_finite()) and (len(value.as_tuple().digits)<=1024) and (abs(value.as_tuple().exponent)<=1000),'metadata decimal bound')
        return value
    def pairs(items):
...
def token(value):
    ...
    value = Decimal(value)
    require((value.is_finite()) and (len(value.as_tuple().digits) <= 1024) and (abs(value.as_tuple().exponent) <= 1000), 'finite bounded decimal required')
...
def project_cell(export, manifest, rows, pieces, reference, range_bindings):
    ...
    require((manifest['schema'] == 'braid-program/f6c-emission-refinement-cover.v1') and (manifest['scope'] == 'pilot-cell-0-emission-refinement') and (manifest['status'] == 'conditional_complete') and (manifest['accepted'] is False), 'complete unpromoted pilot manifest required')
    ...
    for i in range(8):
        for j in range(8):
            ...
            require((type(r['receiverIndex']) is int) and (type(r['transmitterIndex']) is int) and ((r['receiverIndex'],r['transmitterIndex'],r['receiverId'],r['transmitterId'])
    ...
    require((interval(manifest['retainedDomain']) == (Fraction(-8),Fraction(13,100))) and (interval(manifest['receptionDomain']) == (Fraction(0),Fraction(1,1000))), 'fixed cell/domain differs')
    ...
    for n,frame in enumerate(frames):
        ...
        if n < 80:
            ...
            require((type(edge['leftFrameIndex']) is int) and (type(edge['rightFrameIndex']) is int) and ((edge['leftFrameIndex'],edge['rightFrameIndex']) == (n,n+1)) and (edge['startTime'] == frame['time']) and (edge['endTime'] == frames[n+1]['time']), 'exact frame edge differs')
    require((frames[0]['time'] == '0') and (frames[-1]['time'] == '0.13') and (frames[1]['time'] == '0.002'), 'cell zero outside exact first frame')
    def bounds(record):
    ...
    for n,row in enumerate(rows):
        ...
        i,j=divmod(n,8)
        require((integer(row['rowIndex'],0,63) == n) and (integer(row['cellIndex'],0,0) == 0) and (integer(row['receiverIndex'],0,7) == i) and (integer(row['transmitterIndex'],0,7) == j) and (row['receiverId'] == IDS[i]) and (row['transmitterId'] == IDS[j]), 'root row ownership/order differs')
        ...
        coverage=[]
        if i == j:
            require((type(row['ordinaryRootsPerReception']) is int) and (row['ordinaryRootsPerReception'] == 0) and (row['coincidentEndpointExcluded'] is True) and (all(row[key] is None for key in geometry_fields)), 'self row geometry/coverage fabricated')
            coverage=[None,None]
        else:
            require((type(row['ordinaryRootsPerReception']) is int) and (row['ordinaryRootsPerReception'] == 1) and (row['coincidentEndpointExcluded'] is False), 'ordinary root count differs')
            ...
            require((interval(row['oldestResidual'])[1] < 0) and (interval(row['lowerFaceResidual'])[1] < 0) and (interval(row['upperFaceResidual'])[0] > 0), 'strict faces failed')
            # Oldest A=-8 and refined lower L are copied separately, never equated.
            require((interval(row['distance'])[0] > 0) and (interval(row['transmitterFactor'])[0] >= Fraction(1,10**24)) and (interval(row['receiverFactor'])[0] > 0), 'positive denominators/factors failed')
            for role,index in (('receiver',i),('transmitter',j)):
                ...
                require((integer(piece['recordIndex'],0,111) == piece_index) and (integer(piece['rowIndex'],0,63) == n) and (piece['role'] == role) and (piece['memberId'] == IDS[index]) and (piece['historyDigest'] == mapping[index]['historyDigest']), 'piece ownership/digest differs')
                ...
                require((first <= last) and (integer(piece['touchedPieceCount'],1,1760) == last-first+1) and (type(piece['contiguousIndexRange']) is list) and (len(piece['contiguousIndexRange']) == 2) and (all(type(x) is int for x in piece['contiguousIndexRange'])) and (piece['contiguousIndexRange'] == [first,last]), 'piece census differs')
...
class PinnedInput:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.expected=digest if self.expected is None else self.expected;require(digest==self.expected,'source hash differs: '+str(self.path))
    ...
    def scan(self,capture=False):
        ...
        require((size==expected_size) and (self.identity(os.fstat(self.fd))==self.identity(self.initial)) and (self.identity(os.stat(self.path,follow_symlinks=False))==self.identity(self.initial)),
...
def captured_reference(filename,raw):
    pass
    name='_f6c_acceleration_'+sha(raw); suffix=0
    while name in sys.modules:
        suffix+=1; name='_f6c_acceleration_'+sha(raw)+'_'+str(suffix)
    module=ModuleType(name); module.__file__=str(filename); module.__package__=''
    sys.modules[name]=module
    try:
        exec(compile(raw, str(str(filename)), 'exec', dont_inherit=True, optimize=sys.flags.optimize), module.__dict__)
        yield module
    finally:
...
def validate_plan(plan,own_sha):
    ...
    for role,path in NAMED.items():
        ...
        require((item['path']==path),'named source: '+role)
    ...
    require(equal({k:v for k,v in plan['priorRefinementClosure'].items() if k!='ownerSha256'},closure_premise()),'prior external whole-attempt closure')
    seen={}
...
def authenticate_receipts(export,manifest,comparison,admission,reconstruction,guards,prior_plan,bindings):
    ...
    require((comparison['schema']=='braid-program/f6c-continuous-reception-root-cover-conformance.v1') and (comparison['accepted'] is True) and (comparison['scope']=='pilot-cell-0'),'accepted comparison required')
    ...
    require((prior_plan['schema']=='braid-program/f6c-cached-root-cover-pilot-launch.v1') and (prior_plan['scope']=='pilot-cell-0'),'wrong prior plan generation')
    ...
    require((admission['schema']=='braid-program/f6c-cached-root-cover-pilot-admission.v1') and (admission['accepted'] is True) and (admission['scope']=='pilot-cell-0') and (admission['processesClosed'] is True),'prior admission differs')
    ...
    for item,stage in zip(admission['stages'],('consumer','comparison')):
        ...
        require((process['accepted'] is True) and (process['processesClosed'] is True) and (type(process['exit']['code']) is int) and (process['exit']['code']==0) and (process['exit']['signal'] is None),'prior stage process not cleanly closed')
...
def authenticate_refined(docs,refined,ancestry,root,read_bound):
    ...
    require((plan['schema']=='braid-program/f6c-emission-refinement-launch.v1') and (plan['scope']==scope) and (equal(plan['limits'],LIMITS)),'prior plan protocol')
    old_closure=dict(authority='externally-reviewed-caller-observation',
        admissionSha256=ancestry['admission']['sha256'],
        matchingFreshCompletionObserved=True,exitCode=0,elapsedSeconds='8.534247625',
        processesClosed=True,independentAuditAccepted=True)
    require(equal({k:v for k,v in plan['priorCoverClosure'].items() if k!='ownerSha256'},old_closure),'prior broad closure')
    for item in ancestry.values():read_bound(item)
    require((manifest['schema']=='braid-program/f6c-emission-refinement-cover.v1') and (manifest['scope']==scope) and (manifest['status']=='conditional_complete') and (manifest['accepted'] is False),'prior unpromoted cover')
    require((proof['schema']=='braid-program/f6c-emission-refinement-conformance.v1') and (proof['scope']==scope) and (proof['status']=='conditional-comparison-complete') and (proof['accepted'] is True),'prior independent conditional comparison')
    require((admission['schema']=='braid-program/f6c-emission-refinement-pilot-admission.v1') and (admission['scope']==scope) and (admission['accepted'] is True) and (admission['processesClosed'] is True),'closed prior admission')
    ...
    for role,record in ancestry.items():
        require(equal(manifest['fixedBindings'][role],record) and equal(proof['fixedBindings'][role],record),'prior scientific input differs')
    ...
    require((all(v is False for v in manifest['claims'].values())) and (equal(manifest['claims'],proof['candidateClaims'])),'prior copied authority')
    ...
    require((analysis['accepted'] is False) and (analysis['conditionalQueryReplayConformant'] is True) and (analysis['conditionalFinalCoverConformant'] is True),'conditional prior proof absent')
    ...
    for item,label in zip(admission['stages'],('producer','comparison')):
        ...
        require((item['stage']==label) and (process['accepted'] is True) and (process['processesClosed'] is True) and (equal(process['exit'],dict(code=0,signal=None))) and (stage['accepted'] is True) and (equal(process['admission'],stage)),'prior stage completion/closure')
        list_count(process['gates'],1);gate=process['gates'][0]
        require((gate['retired'] is True) and (gate['acknowledged'] is True) and (type(gate['measurement']['code']) is int) and (gate['measurement']['code']==0) and (gate['measurement']['signal'] is None),'prior registered gate')
        require((done['completed'] is True) and (done['accepted'] is (label=='comparison')) and (done['scope']==scope) and (done['h3EvidenceEligible'] is False) and (done['eomExecuted'] is False),'prior fresh completion')
        ...
        require((raw.endswith(b'\n')) and (len(raw.splitlines())==1) and (equal(decode_role(raw,'completion'),done)),'prior actual stdout record')
        if label=='producer':
            outputs=[refined[k] for k in ('queries','rows','pieces','manifest')]
            require((equal(done['outputs'],outputs)) and (equal(stage['outputs'],outputs)) and (equal(done['census'],census)) and (done['conditionalCoverPrepared'] is True) and (done['externalWholeAttemptAdmissionRequired'] is True),'prior producer outputs')
        else:
            require((equal(done['output'],refined['comparison'])) and (equal(stage['outputs'],[refined['comparison']])) and (equal(done['analysis'],analysis)),'prior comparison output')
    return analysis
...
def authenticate_observations(admission,logs):
    ...
    for h in hosts:
        ...
        require((type(h['availableDiskBytes']) is str) and (re.fullmatch(r'[0-9]{1,20}',h['availableDiskBytes'])) and (int(h['availableDiskBytes'])>=16*1024**3),'prior disk floor')
    require((hosts[0]['atLaunch'] is True) and (hosts[0]['freePercent']>=40) and (int(hosts[0]['availableDiskBytes'])>=64*1024**3),'prior host launch floor')
    require((all(x['accepted'] is False for x in beats)) and (all(x['h3EvidenceEligible'] is False and x['stopping'] is False for x in gates)),'prior heartbeat flags')
    for row in samples:
    ...
    require((type(summary['samples']) is int) and (summary['samples']==954) and (summary['maximumSampledRSSBytes']==max(x['aggregateResidentBytes'] for x in samples[:-1])) and (number(summary['maximumSampleGapMs'])==max(number(x['sampleGapMs']) for x in samples[:-1])),
            'prior RSS prefix')
    size=sum(map(len,logs.values()))+sum(binding(x['process'][k])['bytes']
        for x in admission['stages'] for k in ('stdoutLog','stderrLog'))
    require((type(admission['loggingBytesBeforePublication']) is int) and (0<=admission['loggingBytesBeforePublication']<=size<=16*1024**2),'prior final log size')
...
def mathematical_bindings(reference,ancestry,refined):
    selected=(('original_export',ancestry['export']), ('reconstruction_receipt',ancestry['reconstruction']), ('guards_receipt',ancestry['guards']), ('root_cover',refined['manifest']), ('root_cover_comparison',refined['comparison']))
...
def main(argv=None):
    ...
    try:
        with ExitStack() as stack:
            owned={}
            def capture(path,digest=None,*,data=False,limit=MAX_BYTES):
                live();key=str(Path(path).absolute())
                obj=owned.get(key)
                if obj is None:
                    obj=stack.enter_context(PinnedInput(key,digest,capture=data,limit=limit));owned[key]=obj
                else:
                    require((digest is None or obj.expected==digest) and (obj.initial.st_size<=limit),'conflicting capture')
            ...
            sources={k:(normalize(plan[k],root) if k=='declaration' else read_bound(plan[k])) for k in NAMED}
            fixed=({k:capture(root/p,h,data=True) for k,p,h in FIXED} | {k: capture(root/p,data=True) for k,p in SOURCE_PATHS.items()})
            ...
            require((git in runtime) and (Path(sys.executable).resolve() in runtime) and (Path(sys.executable).absolute().parent.parent/'pyvenv.cfg' in runtime),
