...
export const ABC_HISTORY_SCHEMA = "braid-program/abc-circular-history-manifest.v1";
export const ABC_PROOF_SCHEMA = "braid-program/abc-circular-history-conformance.v1";
export const ABC_FROZEN_BINDINGS = Object.freeze([
  { id: "circular-core", path: "src/prescribed-path-analysis/CircularHistoryConformance.mjs", sha256: "d5bf2aa286c28cc715d4903b35c85f5327966fd7a4fb5e7dd49e985298c600c9" },
  { id: "integer-primitive", path: "scripts/eom/derive-abc-subfield-root-reference.mjs", sha256: "2c0242d36ca47f5fc53077b0baa3db90aa5e37a97a4b6089bdaa4b86fcbfdbee" },
  { id: "root-reference", path: ".local-data/braid-analysis/parallel-agent-search/parallel-braid-prescribed-search-20260826-v1/abc-root-reference-20260827-v1.json", sha256: "c74bad1d7c5aeed4c9bb326ff711f87833ba43e39236bbc920afb2f375dc7e08" },
  { id: "budget-cli", path: "scripts/eom/derive-abc-circular-history-budget.mjs", sha256: "f20e16a1098706935df12be7e1b034fca16d64b927642f21b740465238ce0816" },
  { id: "construction-budget", path: ".local-data/braid-analysis/parallel-agent-search/parallel-braid-prescribed-search-20260826-v1/abc-circular-history-budget-20260827-v1.json", sha256: "df1b1254c867b928dac595eada4dc1f197fa2e23d2451efb114eb41de495c74a" },
  { id: "pilot-predeclaration", path: "reference/priorities/braid-program/evidence/2026-08-27-abc-h3-pilot-predeclaration.md", sha256: "886be366bf4051ecb0339930631ca81b0a85af016ed07a24f3bf0216854fef0c" },
  { id: "source-manifest", path: "reference/priorities/braid-program/campaigns/parallel-agent-braid-search.v1.json", sha256: "739eb4706ae1be9d427c1a643419c7e5d5455fe85a26ad5d6e490bb114d411ee" },
].map(Object.freeze));
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
const SELF = "scripts/eom/verify-abc-circular-history.mjs";
const SCALE = 10n ** 60n;
const SEGMENTS = 1000;
const DEADLINE_MS = 1800000;
const HEARTBEAT_MS = 15000;
const MAX_BYTES = 64 * 1024 * 1024;
const CANDIDATES = ["a1-1", "a1-2", "a1-4", "a2", "a3-1", "a3-2", "a3-4", "b1-1", "b1-2", "b1-3", "c1", "c2", "c3", "c4", "c5", "c6"];
const sha = (bytes) => createHash("sha256").update(bytes).digest("hex");
const reject = (message) => { throw new Error(`ABC circular history rejected: ${message}`); };
...
function captureProofSnapshot() {
  const sources = [
    ...ABC_FROZEN_BINDINGS.filter((binding) => ["circular-core", "integer-primitive"].includes(binding.id)),
...
function installSnapshotLoader(snapshot) {
  ...
  const marker = `?abc-proof-snapshot=${snapshot.nonce}`;
...
function validateSnapshot(snapshot) {
  ...
  const expected = [...ABC_FROZEN_BINDINGS.filter((binding) =>
...
const PROOF_SNAPSHOT = validateSnapshot(!isMainThread && workerData?.task === "abc-whole-manifest-proof"
...
function references() {
  const loaded = new Map(ABC_FROZEN_BINDINGS.map((binding) => [binding.id, readBound(binding)]));
...
function expectedSource(manifest, refs) {
  ...
  if (source.schema !== "prescribed-assembly-spec.v2" ||
...
function structure(bytes, rung, phase, refs, progress) {
  ...
  if (manifest.schema !== ABC_HISTORY_SCHEMA || manifest.normalizedFieldSpeed !== "1") reject("wrong manifest schema or field speed");
  const time = reception(rung, phase), timeToken = formatCircularBound(time), lower = time - 2n * SCALE;
  const manifestId = `abc-circular-history/v1:${manifest.candidateId}:T=${timeToken}`;
...
export function inspectAbcHistoryStructureForTests(bytes, rung, phase) {
  const checked = structure(bytes, rung, phase, references(), () => {});
  for (const binding of [...ABC_FROZEN_BINDINGS, checked.source.binding]) readBound(binding);
...
function prove(manifestPath, rung, phase, progress) {
  ...
  const bindings = [...ABC_FROZEN_BINDINGS, source.binding, selfBinding];
  ...
  checkDeadline();
  return { schema: ABC_PROOF_SCHEMA, accepted: true, actualCarrierValidated: true, h3EvidenceEligible: false,
...
function argumentsForCli(argv) {
  ...
  for (let at = 0; at < argv.length; at += 2) {
    ...
    if (!["--manifest", "--rung", "--phase", "--out"].includes(key) || argv[at + 1] === undefined || Object.hasOwn(options, key)) {
      reject("Usage: node scripts/eom/verify-abc-circular-history.mjs --manifest FILE --rung 2|8|32|128 --phase K --out NEW");
    }
...
async function cli() {
  ...
  try {
    ...
    worker = new Worker(new URL(`data:text/javascript;base64,${Buffer.from(WORKER_LOADER).toString("base64")}`),
      { workerData: { task: "abc-whole-manifest-proof", ...args, snapshot: PROOF_SNAPSHOT } });
    ...
    if (result.schema !== ABC_PROOF_SCHEMA || result.authority !== "source-bound-whole-manifest-analytic-conformance-only" ||
    ...
    const bindings = [...ABC_FROZEN_BINDINGS, checked.source.binding,
    ...
    result = { schema: ABC_PROOF_SCHEMA, accepted: false, actualCarrierValidated: false, h3EvidenceEligible: false,
...
if (!isMainThread && workerData?.task === "abc-whole-manifest-proof") {
  try {
