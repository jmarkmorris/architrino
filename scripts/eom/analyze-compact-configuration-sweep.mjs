...
function rowReference(shard, row) {
  return {
    ...
    configurationToken: shard.configurationToken,
    caseId: row.caseId,
...
function validateCaseRow(row, label) {
  ...
  requiredString(row.assemblyId, `${label}.assemblyId`);
  requiredSha256(
    row.modelRevisionSha256,
    `${label}.modelRevisionSha256`,
  );
  if (row.assemblyId !== `asm-${row.modelRevisionSha256.slice(0, 32)}`) {
    fail(`${label}.assemblyId does not match modelRevisionSha256.`);
  }
  requiredString(row.sourceSlug, `${label}.sourceSlug`);
...
function validateCampaign(campaign, file) {
  ...
  requiredPositiveInteger(
    campaign.sampling?.casesPerConfiguration,
    `${file}.sampling.casesPerConfiguration`,
  );
  requiredPositiveInteger(
    campaign.sampling?.configurationCount,
    `${file}.sampling.configurationCount`,
  );
  ...
  for (let index = 0; index < campaign.caseRows.length; index += 1) {
    ...
    configurationCounts.set(
      row.sourceSlug,
      (configurationCounts.get(row.sourceSlug) ?? 0) + 1,
    );
  }
  if (configurationCounts.size !== campaign.sampling.configurationCount ||
      [...configurationCounts.values()].some(
        (count) => count !== campaign.sampling.casesPerConfiguration,
      )) {
    fail(`${file} configuration counts do not match its sampling declaration.`);
  }
...
function isComparativelyRankable(row) {
  return isRankable(row) &&
    COMPARATIVE_RANKING_EXCLUSIONS[row.sourceSlug] === undefined;
}
...
function buildConfigurationCounts(shards) {
  const counts = new Map();
  for (const shard of shards) {
    const perConfiguration = shard.campaign.sampling.casesPerConfiguration;
    const configurations = new Set(shard.campaign.caseRows.map((row) => row.sourceSlug));
    for (const sourceSlug of configurations) {
      if (!counts.has(sourceSlug)) {
        counts.set(sourceSlug, {
          sourceSlug,
          expectedDrawCount: 0,
          actualDrawCount: 0,
          evaluatedCount: 0,
      ...
      counts.get(sourceSlug).expectedDrawCount += perConfiguration;
    }
    for (const row of shard.campaign.caseRows) {
      const count = counts.get(row.sourceSlug);
  ...
  return [...counts.values()].sort(
    (left, right) => compareText(left.sourceSlug, right.sourceSlug),
  );
}
...
function buildWaveSummary(shards) {
  ...
  for (const shard of shards) {
    ...
    wave.campaignFiles.push({
      ...
      campaignHash: shard.campaign.campaignHash,
      seed: shard.campaign.sampling.seed,
      casesPerConfiguration: shard.campaign.sampling.casesPerConfiguration,
      expectedDrawCount:
        shard.campaign.sampling.configurationCount *
        shard.campaign.sampling.casesPerConfiguration,
...
function buildFrozenIdentitySet(shards) {
  ...
  return {
    configurations: [...configurations.values()].sort((left, right) =>
...
function classifyCampaignFile(file) {
  ...
  const configuration = file.match(CONFIGURATION_FILE_PATTERN);
  if (configuration) {
    return {
      file,
      waveId: configuration[1],
      waveNumber: Number(configuration[2]),
      fileKind: "configuration",
      shardNumber: null,
      configurationToken: configuration[3],
    };
  }
...
function buildTerminalBoundary({
  ...
  expectedWave3ConfigurationCount,
  ...
  if (throughWave === null) {
    ...
    const requiredWaves = Array.from(
      ...
      (_, waveNumber) => {
        const fileKind = waveNumber === 3 ? "configuration" : "shard";
        return {
          waveId: waveNumber === 0
            ? "pilot"
    ...
    const referenceConfigurations = [...new Set(validShards
      .filter((shard) => shard.waveNumber === 0)
      .flatMap((shard) => shard.campaign.caseRows.map((row) => row.sourceSlug)))]
      .sort(compareText);
    for (const required of requiredWaves) {
      ...
      if (required.fileKind === "configuration") {
        const validConfigurationFiles = validShards.filter(
          (shard) => shard.waveNumber === required.waveNumber &&
            shard.fileKind === "configuration",
        );
        const fileConfigurations = validConfigurationFiles.map((shard) => {
          const configurations = [...new Set(
            shard.campaign.caseRows.map((row) => row.sourceSlug),
          )].sort(compareText);
          return {
            file: shard.file,
            configurationToken: shard.configurationToken,
        ...
        const invalidConfigurationFiles = fileConfigurations.filter(
          (row) => row.configurations.length !== 1 ||
            row.configurationToken !== row.configurations[0]?.replaceAll(".", "-"),
        );
        const observedConfigurations = fileConfigurations.flatMap((row) =>
          row.configurations.length === 1 ? row.configurations : []);
        const configurationCounts = new Map();
        for (const sourceSlug of observedConfigurations) {
          configurationCounts.set(sourceSlug, (configurationCounts.get(sourceSlug) ?? 0) + 1);
        }
        const duplicateConfigurations = [...configurationCounts.entries()]
          .filter(([, count]) => count > 1)
          .map(([sourceSlug, count]) => ({ sourceSlug, count }))
          .sort((left, right) => compareText(left.sourceSlug, right.sourceSlug));
        const observedConfigurationSet = [...configurationCounts.keys()].sort(compareText);
        const missingConfigurations = referenceConfigurations.filter(
          (sourceSlug) => !configurationCounts.has(sourceSlug),
        );
        const extraConfigurations = observedConfigurationSet.filter(
          (sourceSlug) => !referenceConfigurations.includes(sourceSlug),
        );
        if (discovered.length !== required.expectedFileCount ||
            invalidConfigurationFiles.length > 0 ||
            duplicateConfigurations.length > 0 ||
            referenceConfigurations.length !== required.expectedFileCount ||
            missingConfigurations.length > 0 ||
            extraConfigurations.length > 0) {
          defects.push({
            code: "boundary-configuration-inventory-mismatch",
            waveId: required.waveId,
            expectedConfigurationFileCount: required.expectedFileCount,
            discoveredConfigurationFileCount: discovered.length,
            referenceConfigurationCount: referenceConfigurations.length,
...
export function analyzeCompactAssemblySweep({
  ...
  expectedWave3ConfigurationCount = DEFAULT_EXPECTED_WAVE_3_CONFIGURATION_COUNT,
} = {}) {
  ...
  const expectedDrawCount = shards.reduce((sum, shard) =>
    sum +
      shard.campaign.sampling.configurationCount *
      shard.campaign.sampling.casesPerConfiguration, 0);
  const manifest = {
  ...
  const analysisBody = {
    ...
    inputPolicy: {
      ...
      configurationFilenamePattern: CONFIGURATION_FILE_PATTERN.source,
    ...
    perConfigurationCounts: buildConfigurationCounts(shards),
    ...
    metricLeaders: {
      ...
      exclusions: Object.entries(COMPARATIVE_RANKING_EXCLUSIONS).map(
        ([sourceSlug, reason]) => ({ sourceSlug, reason }),
      ),
...
function parseArguments(argv) {
  ...
  const expectedWave3ConfigurationCount = Number(
    values.get("--expected-wave-3-configurations") ??
      String(DEFAULT_EXPECTED_WAVE_3_CONFIGURATION_COUNT),
  );
  return {
    help: flags.has("--help"),
...
function help() {
  console.log([
    "Usage:",
    "  node scripts/eom/analyze-compact-configuration-sweep.mjs [options]",
    "",
    "Options:",
    ...
    "  --expected-wave-3-configurations N               default: 20",
    ...
    "deterministic receipt to stdout. Wave 3 uses per-configuration files; the other",
...
function main() {
  ...
  const result = analyzeCompactAssemblySweep(options);
