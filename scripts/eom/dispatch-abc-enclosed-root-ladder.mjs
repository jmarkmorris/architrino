...
const SELF = "scripts/eom/dispatch-abc-enclosed-root-ladder.mjs", RUNG = "scripts/eom/run-abc-enclosed-root-rung.mjs";
const BASE = ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/";
const OUTER = "scripts/eom/launch-abc-enclosed-root-pilot.mjs", OUTER_SHA = "5aa154b1579909cc63f01d81023e2e1412c2a0bb277663d9e1cd118999795baa";
export const ABC_MEMORY_COMMAND_SHA = "a1668e28505400a9e09ab9b2bd2558f04d038152dfdb05826576a0a0aa27fe56";
...
export function parseABCDispatchArgs(argv) {
...
export function classifyABCCandidateFailure(failureCode, sharedBytesMatch) {
...
export async function runABCBoundedCandidatePool({ candidates, limit, shouldStop, onFatal, work }) {
  let next = 0;
...
export function abcWholeRungClock(limitMs) {
...
export async function recordABCResourceObservation({ query, append, onFailure, atLaunch = false }) {
...
export function parseABCResourceObservation(stdout, diskBytes, policy, atLaunch = false) {
...
export function checkABCRungGateCensus(processReceipt, rungReceipt) {
...
export async function abcDispatchFileOperation(job) {
  ...
  if (job.kind === "shared") {
    ...
    const pilotBytes = sourceBytes(path.join(job.root, rung.ABC_RUNTIME_PATHS.pilot), rung.ABC_RUNTIME_HASHES.pilot);
  ...
  check(record.schema === "braid-program/abc-candidate-rung-process.v1" && record.accepted === true && record.h3EvidenceEligible === false &&
  ...
  check(record.buildReceipt?.sha256 === rung.ABC_BUILD_SHA && record.buildReceipt.path === path.join(job.root, rung.ABC_BUILD_PATH),
  ...
  rung.validateABCRungSummary(summary.value, { candidateId: job.candidateId, rung: job.rung }, record.phaseReceipts, "candidate-rung");
  ...
  if (job.rung === 128) {
    ...
    rung.validateABCRungSummary(ladder.value, { candidateId: job.candidateId, rung: job.rung }, [...old, ...record.phaseReceipts], "candidate-ladder");
  }
  ...
  const gates = checkABCRungGateCensus(job.processReceipt, record);
  ...
  const named = rung.summarizeABCNamedOutputs([...rung.abcRungNamedOutputBindings(record).map(binding =>
...
export async function watchedABCDispatchOperation(job, { bytes, sha256, limitMs, signal }) {
  ...
  const worker = new Worker(`const{parentPort,workerData}=require('node:worker_threads');(async()=>{
    ...
    parentPort.postMessage({result:await m.abcDispatchFileOperation(workerData.job)});
...
export async function runABCMeasuredDispatch({ root, args, selfBytes, rungBytes, runtime }) {
  ...
  const receipt = { schema: "braid-program/abc-candidate-ladder-dispatch.v1", accepted: false, h3EvidenceEligible: false,
  ...
  const operation = (job, limitMs = 1800000, signal) => watchedABCDispatchOperation({ root, rungBytes, rungSha256: sha(rungBytes), ...job },
  ...
  const observe = (policy, atLaunch = false) => {
    ...
    resourceInFlight = (async () => {
      try { return await recordABCResourceObservation({ atLaunch,
        ...
        query: async () => {
        await operation({ kind: "read", files: [{ path: "/usr/bin/memory_pressure", sha256: ABC_MEMORY_COMMAND_SHA }] }, 2000);
        ...
        return { ...memory, ...parseABCResourceObservation(memory.stdout, diskBytes, policy, atLaunch) };
  ...
  try {
    ...
    const plan = runtime.rung.validateABCResourcePlan(planBinding.value);
    ...
    bindings = [receipt.plan, plan.pilotAdmission, { path: SELF, sha256: sha(selfBytes) }, { path: RUNG, sha256: sha(rungBytes) },
      ...Object.entries(runtime.rung.ABC_RUNTIME_PATHS).map(([key, relative]) => ({ path: relative, sha256: runtime.rung.ABC_RUNTIME_HASHES[key] })),
      { path: "/usr/bin/memory_pressure", sha256: ABC_MEMORY_COMMAND_SHA },
    ...
    const work = async candidate => {
      ...
      for (const item of candidate.rungs) {
        ...
        const clock = abcWholeRungClock(wallLimitSeconds * 1000), { began, deadlineNanoseconds } = clock;
        ...
        try {
          ...
          item.resources = { ...processReceipt.admission.resources, ...runtime.rung.summarizeABCNamedOutputs(
          ...
          const disposition = classifyABCCandidateFailure(item.failureCode, sharedBytesMatch);
      ...
      candidate.resources = runtime.rung.summarizeABCNamedOutputs(candidate.rungs.flatMap(item => item.resources?.namedOutputs ?? []),
    ...
    pool = [runABCBoundedCandidatePool({ candidates: receipt.candidates, limit: plan.maximumConcurrentCandidates,
...
async function main() {
  const args = parseABCDispatchArgs(process.argv.slice(2)), root = realpathSync(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../.."));
  ...
  const result = await dispatcher.runABCMeasuredDispatch({ root, args, selfBytes, rungBytes, runtime });
