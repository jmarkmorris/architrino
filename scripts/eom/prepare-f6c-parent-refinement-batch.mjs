...
export function parseArguments(a){
 check(Array.isArray(a)&&a.length===10&&a[0]==='--configuration'&&a[2]==='--configuration-sha256'&&a[4]==='--self-sha256'&&a[6]==='--out-directory'&&a[8]==='--coordinator-sha256','usage: --configuration ABS --configuration-sha256 SHA --self-sha256 SHA --out-directory ABS --coordinator-sha256 SHA');
 absolute(a[1]);for(const i of[3,5,9])digest(a[i]);absolute(a[7]);return{configurationPath:a[1],configurationSha256:a[3],selfSha256:a[5],outDirectory:a[7],coordinatorSha256:a[9]};
}
...
export function validateConfiguration(c,C,root){
 keys(c,['schema','root','template','coordinator','composition','compositionControls','preparationControls','observer','closureChecker','closureControls','sources','sourceIdentities','runtimeBindings','operationalBindings','acceptanceOwner','pythonCommand','git','operationDirectory','parents','closureReserveBytes','ownerTask']);
 ...
 for(const k of ['template','coordinator','composition','compositionControls','preparationControls','observer','closureChecker','closureControls','acceptanceOwner'])C.binding(c[k]);
 check(c.preparationControls.path===path.join(root,CONTROLS),'preparation control/expectation');
 ...
 for(const k of ['template','coordinator','composition','compositionControls','preparationControls','observer','closureChecker','closureControls','acceptanceOwner'])check(sources.some(b=>canonical(b)===canonical(c[k])),'missing explicit '+k);
...
export function derivePlans({configuration:c,template,admission,exported,self,configurationBinding,outDirectory,C,B}){
 ...
 const plans=B.makeParentPlans({template,indices:c.parents.map(p=>p.parentIndex),sourceBindings:c.sources,runtimeBindings:c.runtimeBindings,operationalBindings:c.operationalBindings,acceptanceOwner:c.acceptanceOwner});
