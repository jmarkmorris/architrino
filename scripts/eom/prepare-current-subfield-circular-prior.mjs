...
async function main(){
 ...
 for(let i=2;i<process.argv.length;i+=2){check(['--review','--review-sha256','--accepted-review','--accepted-review-sha256','--out'].includes(process.argv[i])&&process.argv[i+1]&&!args[process.argv[i]],'exact prior-preparation arguments required');args[process.argv[i]]=process.argv[i+1];}
 const final=Boolean(args['--accepted-review']);check(Object.keys(args).length===3&&args['--out']&&(final?args['--accepted-review-sha256']:args['--review']&&args['--review-sha256']),'complete prior-preparation arguments required');
 return prepareCurrentCircularPrior({root,args});
}
export async function prepareCurrentCircularPrior({root,args}) {
 const final=Boolean(args["--accepted-review"]);
 const {captureCircularFile}=await import('../../src/prescribed-path-analysis/SubfieldCircularObservationOwner.mjs');
 ...
 if(final){const binding=capture(args['--accepted-review'],args['--accepted-review-sha256']),packet=decode(binding);
  ...
  ({reviewBytes:bytes}=buildCurrentCircularPriorPacket({jointBinding,reviewBinding,stdoutBinding,terminalProjection,priorPrefix,earlierAdmissions,planBinding:profile.plan,
   entrySha256:sha(readFileSync(path.join(root,'scripts/eom/run-current-subfield-circular-root-rung.mjs'))),memoryOwnerSha256:sha(readFileSync(path.join(root,'src/prescribed-path-analysis/SubfieldCircularMemoryOwner.mjs'))),memoryHelperSha256:sha(readFileSync(path.join(root,'scripts/eom/observe-subfield-circular-memory.py')))}));name=`next${review.rung===8?32:128}-review.pending.json`;
 }
