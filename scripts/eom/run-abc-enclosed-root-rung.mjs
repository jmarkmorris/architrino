...
export const ABC_RUNG_PATH = "scripts/eom/run-abc-enclosed-root-rung.mjs";
export const ABC_DISPATCH_PATH = "scripts/eom/dispatch-abc-enclosed-root-ladder.mjs";
export const ABC_RUN_BASE = ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/";
export const ABC_BUILD_PATH = `${ABC_RUN_BASE}recorded-build-20260827-v2/preparation.json`;
export const ABC_BUILD_SHA = "be6f2e43cc2c608a568d128c79535eacc628ea80cfc62cfe273af7c434243866";
export const ABC_RUNTIME_PATHS = Object.freeze({
  pilot: "scripts/eom/run-abc-enclosed-root-pilot.mjs", outer: "scripts/eom/launch-abc-enclosed-root-pilot.mjs",
  helper: "src/prescribed-path-analysis/ABCPhaseProcess.mjs", bridge: "src/prescribed-path-analysis/ABCPhaseLedgerWorker.mjs",
  watch: "scripts/eom/prepare-f5-enclosed-root.mjs", reducer: "src/prescribed-path-analysis/ABCEnclosedRootLedgerReducer.mjs",
  cli: "scripts/eom/reduce-abc-enclosed-root-ledger.mjs", proof: "scripts/eom/verify-abc-circular-history.mjs",
});
export const ABC_RUNTIME_HASHES = Object.freeze({
  pilot: "11dee965bfc2a859bd958dad2e349e17751a63d2d238ce1a4cc8027808f682e4",
  outer: "5aa154b1579909cc63f01d81023e2e1412c2a0bb277663d9e1cd118999795baa",
  helper: "8f63f16766b3c17935f81a8dcecae03614e0b247b9a921a749d8216bf6a77c98",
  bridge: "37ef2827d1542cd0b1092ca654f6217542649350909399fbc88a8b4d6619e567",
  watch: "ba154c0a8c63bd390ae1e16de005fd5d52000fedec352619b60b9465a2f813f5",
  reducer: "3619634d4c884fd96447976ace3396430fb4a8e4c134d93d47dfa9c5b443b361",
  cli: "6e5d63e73904f6e5871992430f4ed9a86e63050c5b59318eefdc48c3c26a988a",
  proof: "b213094bb38c73d291615042df27e8feac6711c8a7958b21688fc7414208630d",
});
export const ABC_IDS = Object.freeze(["a1-1", "a1-2", "a1-4", "a2", "a3-1", "a3-2", "a3-4", "b1-1", "b1-2", "b1-3", "c1", "c2", "c3", "c4", "c5", "c6"]);
export const ABC_RESOURCE_OBSERVATION = Object.freeze({ cadenceSeconds: 15, commandTimeoutMs: 2000,
...
export function validateABCResourcePlan(plan) {
  check(plan?.schema === "braid-program/abc-candidate-ladder-resource-plan.v1" && plan.resourceBudgetReviewed === true && plan.independentReviewStatus === "accepted" &&
  ...
  check(Array.isArray(plan.candidates) && plan.candidates.length > 0 && new Set(plan.candidates).size === plan.candidates.length &&
    plan.candidates.every(id => ABC_IDS.includes(id)) && plan.candidates.every((id, index) => index === 0 || ABC_IDS.indexOf(id) > ABC_IDS.indexOf(plan.candidates[index - 1])),
    "resource plan candidate census/order differs", "PLAN_REJECTED");
  check(plan.buildReceipt?.sha256 === ABC_BUILD_SHA && plan.buildReceipt.path === ABC_BUILD_PATH && digestToken(plan.pilotAdmission?.sha256) &&
  ...
  check(Object.entries(ABC_RESOURCE_OBSERVATION).every(([key, value]) => plan.resourceObservation?.[key] === value),
    "exact reviewed operational resource predicates required", "PLAN_REJECTED");
  check(Array.isArray(plan.resourceReturns) && plan.resourceReturns.every(id => ABC_IDS.includes(id) && !plan.candidates.includes(id)),
    "resource-return candidates must remain outside executable cohort", "PLAN_REJECTED");
  check(new Set(plan.resourceReturns).size === plan.resourceReturns.length && plan.candidates.length + plan.resourceReturns.length === ABC_IDS.length,
...
export function candidateRungSchedule(candidateId, rung) {
  check(ABC_IDS.includes(candidateId) && [8, 32, 128].includes(rung), "fixed candidate/rung required");
  const memberCount = ABC_IDS.indexOf(candidateId) < 10 ? 6 : 12;
...
export function summarizeABCNamedOutputs(bindings, maximumPrecisionBits = null) {
...
export function abcRungNamedOutputBindings(record) {
...
export function checkRepeatedABCPhase(current, prior, exactDecimal) {
...
export function authenticateABCPriorPhases({ candidateId, rung, prior, pilotSummary, admissions, phaseBindings, planBinding }) {
...
export async function watchedABCRepeatedPhases({ current, prior, sources, limitMs, signal }) {
  const runner = sources.find(row => row.path === ABC_RUNG_PATH), reducer = sources.find(row => row.path === ABC_RUNTIME_PATHS.reducer);
  ...
  const worker = new Worker(`const{parentPort,workerData:d}=require('node:worker_threads');(async()=>{
    ...
    for(const p of d.prior)r.checkRepeatedABCPhase(d.current,p,x.abcExactDecimal);
...
export function validateABCRungSummary(summary, expected, phaseReceipts, scope) {
  check(summary?.schema === "braid-program/abc-enclosed-root-summary-reduction.v1" && summary.accepted === true &&
...
export function parseABCRungArgs(argv) {
  ...
  check(names.every(name => result[name]) && [8, 32, 128].includes(Number(result["--rung"])) && ABC_IDS.includes(result["--candidate"]) &&
  ...
  check(out.startsWith(ABC_RUN_BASE) && out.length > ABC_RUN_BASE.length && !out.includes("\\") && out.split("/").every(part => part && part !== "." && part !== ".."), "fresh scoped output required", "PLAN_REJECTED");
  return result;
}

export async function runABCCandidateRung({ root, args, sources, runtime, began = performance.now() }) {
  ...
  const receipt = { schema: "braid-program/abc-candidate-rung-process.v1", accepted: false, h3EvidenceEligible: false,
    ...
    buildReceipt: { path: path.join(root, ABC_BUILD_PATH), sha256: ABC_BUILD_SHA }, plan: { path: path.resolve(args["--plan"]), sha256: args["--plan-sha256"] },
  ...
  const pilotSource = sources.find(row => row.path === ABC_RUNTIME_PATHS.pilot);
  ...
  const summary = async (scope, phases, filename) => {
    await stage(scope, process.execPath, [path.join(root, ABC_RUNTIME_PATHS.cli), "--repo-root", root, "--scope", scope,
      ...phases.flatMap(record => ["--phase-receipt", record.path, "--phase-receipt-sha256", record.sha256]), "--out", filename], output);
    const [binding] = await inspect([{ path: filename, json: true }]); validateABCRungSummary(binding.value, { candidateId, rung }, phases, scope);
  ...
  try {
    ...
    const plan = validateABCResourcePlan(planBinding.value), prior = priorBinding.value;
    ...
    authenticateABCPriorPhases({ candidateId, rung, prior, pilotSummary: pilotSummary.value, admissions,
    ...
    shared = [...build.fastBindings, ...receipt.sourceBindings, ...runtimeBindings, receipt.plan, receipt.priorReceipts, plan.pilotAdmission,
      ...
      { path: ABC_DISPATCH_PATH, sha256: plan.dispatcherSha256 }, ...(cohort.resourceReturn ? [cohort.resourceReturn] : [])];
    receipt.runtimeBindings = runtimeBindings;
    const executable = path.resolve(root, build.built.executable.path), reducerSource = sources.find(record => record.path === ABC_RUNTIME_PATHS.reducer);
    for (const expected of schedule) {
      ...
      await stage("conformance", process.execPath, [path.join(root, ABC_RUNTIME_PATHS.proof), "--manifest", manifestPath, "--rung", String(rung), "--phase", String(expected.phase), "--out", proofPath], directory);
      ...
      await recheckShared();
      activeWorker = await runtime.bridge.openABCPhaseLedgerWorker({ reducerBytes: reducerSource.bytes, reducerSha256: reducerSource.sha256,
        options: { repoRoot: root, historyManifest: manifestPath, conformance: proofPath, conformanceSha256: proof.sha256,
          buildReceipt: receipt.buildReceipt.path, buildReceiptSha256: ABC_BUILD_SHA }, limitMs: remaining(), signal: abort.signal,
      ...
      try {
        ...
        try { phase.process = await runtime.helper.runABCPhaseProcess({ command: "/usr/bin/time",
      ...
      await stage("phase-ledger", process.execPath, [path.join(root, ABC_RUNTIME_PATHS.cli), "--repo-root", root, "--history-manifest", manifestPath,
        "--conformance", proofPath, "--conformance-sha256", proof.sha256, "--rows", rowsPath,
        "--build-receipt", receipt.buildReceipt.path, "--build-receipt-sha256", ABC_BUILD_SHA, "--out", ledgerPath], directory);
      ...
      runtime.pilot.validatePilotPhase(ledger.value, expected, { historyManifest: phase.historyManifest, conformance: proof,
        buildReceipt: receipt.buildReceipt, rawRows: raw, reducer: { path: ABC_RUNTIME_PATHS.reducer, sha256: ABC_RUNTIME_HASHES.reducer },
        cli: { path: ABC_RUNTIME_PATHS.cli, sha256: ABC_RUNTIME_HASHES.cli }, manifestId });
      await watchedABCRepeatedPhases({ current: ledger.value, prior: repeated.map(old => old.value), sources,
    ...
    try { Object.assign(receipt.resources, summarizeABCNamedOutputs(abcRungNamedOutputBindings(receipt),
...
export async function captureABCRungSources(root, runnerSha256) {
  return [{ path: ABC_RUNG_PATH, sha256: runnerSha256 }, ...Object.entries(ABC_RUNTIME_PATHS).map(([key, relative]) => ({ path: relative, sha256: ABC_RUNTIME_HASHES[key] }))]
    .map(binding => ({ ...binding, bytes: captureABCRungSource(path.join(root, binding.path), binding.sha256) }));
}

export function captureABCRungSource(filename, expected, io = { read: readSync, stat: fstatSync }) {
...
async function main() {
  const began = performance.now(), args = parseABCRungArgs(process.argv.slice(2));
  const root = realpathSync(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../.."));
  const sources = await captureABCRungSources(root, args["--runner-sha256"]), pilotSource = sources.find(row => row.path === ABC_RUNTIME_PATHS.pilot);
  ...
  try {
    const runner = await snapshot.import(ABC_RUNG_PATH), runtime = {};
    for (const key of ["pilot", "helper", "bridge", "watch", "reducer"]) runtime[key] = await snapshot.import(ABC_RUNTIME_PATHS[key]);
    const result = await runner.runABCCandidateRung({ root, args, sources, runtime, began });
