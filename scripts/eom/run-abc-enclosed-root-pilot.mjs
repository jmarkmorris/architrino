...
const HELPER = "src/prescribed-path-analysis/ABCPhaseProcess.mjs";
const BRIDGE = "src/prescribed-path-analysis/ABCPhaseLedgerWorker.mjs";
const WATCH = "scripts/eom/prepare-f5-enclosed-root.mjs";
const LEDGER = "src/prescribed-path-analysis/ABCEnclosedRootLedgerReducer.mjs";
const LEDGER_CLI = "scripts/eom/reduce-abc-enclosed-root-ledger.mjs";
const PROOF = "scripts/eom/verify-abc-circular-history.mjs";
const PINS = Object.freeze({
  ...
  [LEDGER]: "3619634d4c884fd96447976ace3396430fb4a8e4c134d93d47dfa9c5b443b361",
  [LEDGER_CLI]: "6e5d63e73904f6e5871992430f4ed9a86e63050c5b59318eefdc48c3c26a988a",
  [PROOF]: "b213094bb38c73d291615042df27e8feac6711c8a7958b21688fc7414208630d",
});
export const PILOT_CANDIDATES = Object.freeze(["a1-1", "a1-2", "a1-4", "a2", "a3-1", "a3-2", "a3-4", "b1-1", "b1-2", "b1-3", "c1", "c2", "c3", "c4", "c5", "c6"]);
...
export function parsePilotArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    demand(["--out", "--runner-sha256"].includes(flag) && value && !value.startsWith("--") && !result[flag],
      "usage: --out NEW-ABC-DIRECTORY --runner-sha256 REVIEWED-SHA256");
    result[flag] = value;
  }
  ...
  demand(output.startsWith(BASE) && output.length > BASE.length && !path.isAbsolute(output) && !output.includes("\\") &&
    output.split("/").every(part => part && part !== "." && part !== ".."), "output must be a new relative child of the ABC run lane");
...
export function validatePilotProof(proof, expected, manifest) {
  demand(proof.schema === "braid-program/abc-circular-history-conformance.v1" && proof.accepted === true &&
  ...
  for (const relative of [PROOF, "src/prescribed-path-analysis/CircularHistoryConformance.mjs", "scripts/eom/derive-abc-subfield-root-reference.mjs"]) {
...
export function validatePilotPhase(receipt, expected, bindings) {
  demand(receipt.schema === "braid-program/abc-enclosed-root-phase-reduction.v1" && receipt.accepted === true &&
...
export function validatePilotSummary(receipt, phaseReceipts) {
  demand(receipt.schema === "braid-program/abc-enclosed-root-summary-reduction.v1" && receipt.accepted === true &&
    receipt.h3EvidenceEligible === false && receipt.rootExecutionAuthorized === false &&
...
export function pilotFileOperation(job) {
  if (job.kind === "build") {
    ...
    demand(build.schema === "braid-program/abc-enclosed-root-build.v1" && build.status === "build-recorded-pending-independent-review" &&
...
export function installPilotSnapshot(sources, root) {
  const marker = `?abc-pilot-snapshot=${randomUUID()}`;
...
export async function runSerialABCPilot({ root, options, sources, runtime, startedAtMs = performance.now(), startedAt = new Date().toISOString() }) {
  ...
  const receipt = { schema: "braid-program/abc-serial-cost-pilot.v1", status: "incomplete", accepted: false,
  ...
  const heartbeat = setInterval(() => console.error(JSON.stringify({ schema: "braid-program/abc-serial-pilot-heartbeat.v1",
  ...
  try {
    ...
    for (let candidateIndex = 0; candidateIndex < PILOT_CANDIDATES.length; candidateIndex++) {
      ...
      try {
        for (const phase of [0, 1]) {
          ...
          activeWorker = await runtime.openABCPhaseLedgerWorker({ reducerBytes, reducerSha256: PINS[LEDGER],
          ...
          try {
            ...
            try {
              currentPhase.process = await runtime.runABCPhaseProcess({ command: "/usr/bin/time",
    ...
    try {
      ...
      const terminal = { schema: "braid-program/abc-serial-pilot-terminal.v1", accepted, h3EvidenceEligible: false,
...
async function main() {
  ...
  try {
    ...
    const result = await runner.runSerialABCPilot({ root, options, sources, startedAtMs, startedAt,
      runtime: { runABCPhaseProcess: helper.runABCPhaseProcess, openABCPhaseLedgerWorker: bridge.openABCPhaseLedgerWorker, runWatched: watcher.runWatched } });
