...
export async function outerAdmissionOperation(job) {
  ...
  const runner = await import("./run-subfield-circular-root-pilot.mjs");
...
async function runAdmissionWorker(job, sourceBytes, limitMs, signal) {
  const worker = new Worker(`const{parentPort,workerData}=require('node:worker_threads');
    (async()=>{const m=await import(workerData.moduleUrl);
...
export function parseLauncherArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    requireThat(["--out"].includes(key) && value && !values[key], "usage: --out NEW"); values[key] = value;
  }
  requireThat(values["--out"]?.startsWith(BASE) &&
...
export async function launchReviewedPilot({ began, deadlineNanoseconds, options, root, self, runner, originalBindings=[] }) {
  ...
  let receipt, failure;
  try {
    receipt = await superviseRegisteredPilot({ root, entry: RUNNER, args: ["--out", path.relative(root, pilotOutput)],
      ...
      admit: ({ receipt: processReceipt, remainingMs, signal }) => runAdmissionWorker({ root, pilotOutput,
        runnerBytes: runner.data, runnerSha256, gates: processReceipt.gates }, self.data, remainingMs, signal) });
  ...
  if (receipt.outputReserved) {
    ...
    if (remainingMs > 0) {
      ...
      try {
        const publication = await runAdmissionWorker({ kind: "publication", receipt, output, deadlineNanoseconds,root,
          sources: [] }, self.data, remainingMs, controller.signal);
...
async function main() {
  ...
  const self = readBound(path.join(root,SELF)), runner = readBound(path.join(root,RUNNER));
  const result = await launchReviewedPilot({ began, deadlineNanoseconds, options, root, self, runner });
