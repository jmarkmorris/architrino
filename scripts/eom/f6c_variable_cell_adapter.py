"""F6c variable-cell mapping with explicit geometry restriction.

open_adapter(repo_root, *, adapter_sha256, controls_sha256,
             closure_owner_sha256, deadline, parent_refinements=(),
             evidence_package=None, accepted_parent_evidence=()) is a context manager. Construction
...
parents1..159. Each needs separately bound independent acceptance. An explicit EvidencePackage may replace only the frozen accepted parent1/2
...
PACKAGE_SOURCES = {
 'reader': 'scripts/eom/f6c_evidence_package.py',
 'readerControls': 'tests/test_f6c_evidence_package.py',
}
PACKAGE_ARTIFACTS = {
...
PARENT_INVENTORY_SOURCES = {
 'parser': 'scripts/eom/f6c_parent_evidence_inventory.py',
 'parserControls': 'tests/test_f6c_parent_evidence_inventory.py',
}
...
FRESH_CLOSURE_SOURCES = {
 'instrument': '.local-data/braid-analysis/f6c-whole-history-20260828/numerical-review/independent_parent_batch_closure.py',
 'controls': '.local-data/braid-analysis/f6c-whole-history-20260828/numerical-review/independent_parent_batch_closure_controls.py',
}
...
SOURCES = {
 ...
 'mapping': 'scripts/eom/verify-f6c-continuous-reception-acceleration.py',
 'mappingControls': 'tests/test_f6c_continuous_reception_acceleration.py',
 'decoder': 'scripts/eom/oracle/f6c_refined_acceleration_conformance.py',
 'decoderControls': 'tests/test_f6c_refined_acceleration_conformance.py',
 'rootComparison': 'scripts/eom/verify-f6c-cached-continuous-reception-root-cover.py',
 'rootControls': 'tests/test_f6c_cached_continuous_reception_root_cover.py',
 'acceleration': 'scripts/eom/oracle/continuous_reception_acceleration.py',
 'accelerationControls': 'tests/test_eom_continuous_reception_acceleration.py',
 'integral': 'scripts/eom/oracle/f6c_residual_integral_supremum.py',
 'integralControls': 'tests/test_f6c_residual_integral_supremum.py',
 'correlated': 'scripts/eom/oracle/f6c_correlated_residual_enclosure.py',
 'correlatedControls': 'tests/test_f6c_correlated_residual_enclosure.py',
 'gk': 'scripts/eom/oracle/f6c_gk13_protocol.py',
 'gkControls': 'tests/test_f6c_gk13_protocol.py',
 'fullEntry': 'scripts/eom/run-f6c-cached-root-cover-full.mjs',
 'geometry': 'scripts/eom/f6c_reception_geometry_restriction.py',
 'geometryControls': 'tests/test_f6c_reception_geometry_restriction.py',
 'captureHelper': 'scripts/eom/prepare-f6c-cached-continuous-reception-root-cover.py',
 'captureHelperControls': 'tests/test_f6c_cached_continuous_reception_root_cover_preparation.py',
 'geometryHistory': 'scripts/eom/oracle/certified_history.py',
 'geometryRoots': 'scripts/eom/oracle/continuous_reception_roots_cached.py',
 'geometryRootsControls': 'tests/test_eom_continuous_reception_roots_cached.py',
 'geometryIntervals': 'scripts/eom/oracle/decimal_interval.py',
 'geometryIntervalControls': 'tests/test_eom_decimal_interval.py',
}
...
def _originals(export,*,actual):
    require((type(export)is dict) and (export['schema']=='braid-program/f6c-retained-history-export.v1') and (export['fieldSpeed']=='1') and (export['coupling']==COUPLING),'fixed export/model required')
    raw_histories=export['retainedHistories'];_seq(raw_histories,8)
    histories=[];common=None
    for i,h in enumerate(raw_histories):
        require((type(h)is dict) and (h['id']==LABELS[i]) and (type(h['pathKey'])is int) and (h['pathKey']==i+1) and (type(h['polarity'])is int) and (h['polarity']==(1 if i%2==0 else -1)),'original identities')
        charge=(''if i%2==0 else '-')+CHARGE
        require((h['charge']==charge) and (type(h['historyFingerprint'])is str) and (0<len(h['historyFingerprint'])<=256),'original literal charge/fingerprint')
    ...
    for n,frame in enumerate(frames):
        ...
        t=number(frame['time']);require((type(frame['frameIndex'])is int) and (frame['frameIndex']==n) and (last is None or last<t) and (t in common),'original frame ordering/knot')
        ...
        for i,member in enumerate(frame['members']):
            ...
            require((type(member['pathKey'])is int) and (member['pathKey']==i+1) and (type(member['stateFlags'])is int) and (member['stateFlags']==(1 if i%2==0 else 2)) and (number(member['positionErrorBound'])>=0),'frame member identity')
...
class Adapter:
    __slots__=('_a','_i','_c','_gk','_integral_sha256','histories','frames','parents','context','provenance','_closed','_pool','_actual','_issued','_evaluated','_clips','_residuals',
        '_geometry','_geometry_refs','_geometry_histories','_geometry_parents','_geometry_guards','_geometry_counts','_successful_counts','fresh_provenance')
    ...
    @property
    def integral_reference_sha256(self):
        self._live()
        digest = (self._pool.files[str(Path(self._i.__file__).resolve())].digest
                  if self._pool is not None else self._integral_sha256)
        require(type(digest) is str and re.fullmatch('[0-9a-f]{64}', digest), 'captured integral reference required')
        return digest
    @property
    ...
    def residual_for(self,evaluated,polynomial):
        self._live();require((type(evaluated)is Evaluation) and (evaluated._owner is self) and ((registered:=self._evaluated.get(id(evaluated.projection)))is not None) and (registered()is evaluated),'original evaluated projection required')
        ...
        require((key.context==self.context) and (key.frame_index==p.cell.frame_index) and (type(key.domain)is i.Bounds) and ((key.domain.lower,key.domain.upper)==(p.cell.reception.lower,p.cell.reception.upper)) and (type(key.label)is str) and (key.label in LABELS),'same polynomial context/member/frame/domain required')
...
def _build(a,i,c,export,parents,*,actual,provenance,pool=None,source_sha=None,gk=None,integral_reference_sha256=None,
           geometry=None,geometry_references=None,geometry_guards=None,
           _authenticated_refined_indices=frozenset({0})):
    ...
    if actual:
        ...
        require((type(_authenticated_refined_indices)is frozenset) and (0 in _authenticated_refined_indices) and (all(type(p)is int and 0<=p<160 for p in _authenticated_refined_indices)),'authenticated admitted parent indices')
    cursor=F(0);clips={}
    for n,parent in enumerate(parents):
        require((type(parent)is ParentCell) and (type(parent.index)is int) and (parent.index==n) and (type(parent.refined)is bool) and (type(parent.reception)is a.Bounds),'ordered immutable parent')
    ...
    for name,value in (('_a',a),('_i',i),('_c',c),('_gk',gk),('histories',histories),('frames',frames),('parents',parents),
        ...
        ('_integral_sha256',integral_reference_sha256),('provenance',provenance),('fresh_provenance',()),('_closed',False),('_pool',pool),('_actual',actual),('_issued',{}),('_evaluated',{}),('_clips',clips),('_residuals',[0]),('_successful_counts',[0,0]),
...
def _bootstrap(path,digest,live):
    ...
    try:
        ...
        def scan():
            ...
            raw=b''.join(chunks)
            require((_hash(raw)==digest) and (identity(os.fstat(fd))==identity(before)) and (identity(os.stat(path,follow_symlinks=False))==identity(before)),'bootstrap changed')
            live();return raw
        raw=scan()
        try:yield raw
        finally:scan()
...
class _Pool:
    ...
    def capture(self,path,digest,*,data=False,size=None):
        ...
        if found is None:
            ...
            require((digest is None or found.digest==digest),'conflicting captured source generation')
            if data and found.data is None:
                require(found.initial.st_size<=MAX_BYTES,'capture upgrade byte bound')
                found.data,observed=found.scan(True);require(observed==found.digest,'late capture changed')
...
def _packaged_pool(pool,selection,descriptors,deadline):
    ...
    required={r:(str(pool.root/p),h)for r,(p,h) in PACKAGE_ARTIFACTS.items()}
    ...
    source={r:pool.capture(p,None,data=r=='reader') for r,p in PACKAGE_SOURCES.items()}
    source.update({r:pool.capture(p,h,data=True) for r,(p,h) in PACKAGE_ARTIFACTS.items()})
...
def _fresh_selections(values,root):
    ...
    authority={str(root/p) for role,p in FRESH_CLOSURE_SOURCES.items() if role=='instrument'}
    ...
    for value in values:
        ...
        require(instrument['path'] in authority,'unreviewed fresh closure instrument')
        ...
        if value.package is not None:
            ...
            require((b['bytes']<=MAX_BYTES) and (b['path'].startswith(str(root/'.local-data/braid-analysis')+'/')) and (value.package not in packages),'bounded unique explicitly selected package')
        ...
        for member in members:
            ...
            for path in aliases:
                ...
                if prior is not None:
                    ...
                    require((member.role==prior_member.role=='acceptanceOwner') and (asdict(member)==asdict(prior_member)),'conflicting shared evidence route')
                    continue
...
def _fresh_evidence_pool(pool,selections,deadline,already_refined):
    ...
    source={role:pool.capture(path,None,data=role in ('reader','parser'))for role,path in
        {**PACKAGE_SOURCES,**PARENT_INVENTORY_SOURCES}.items()}
...
def _refinement_descriptors(values,root,owner_sha):
    ...
    for value in values:
        require((type(value)is ParentRefinement) and (type(value.parent_index)is int) and (previous<value.parent_index<160),'sorted unique explicit parents1..159; parent0 is implicit')
        ...
        require((closure.owner.path==str(root/OWNER)) and (closure.owner.sha256==owner_sha) and (closure.operation==value.operation),'parent closure bound to current acceptance owner and operation')
        require((type(closure.exit_code)is int) and (closure.exit_code==0) and (re.fullmatch(r'[0-9]{1,32}',closure.original_caller_session)) and (re.fullmatch(r'[a-zA-Z0-9_-]{1,128}',closure.final_completion_chunk)) and (0<number(closure.elapsed_seconds)<=1800) and (closure.processes_closed is True) and (closure.independent_audit_accepted is True) and (closure.authority=='attributed-versioned-acceptance-owner-not-fresh-process-observation'),
...
def _authenticate_parent(w,core,pool,descriptor,owner,ancestry,full,fdocs,export,original,a,reference,histories):
    ...
    named=('producer','producerControls','proposalReference','proposalReferenceControls',
    ...
    require((p['schema']=='braid-program/f6c-parent-emission-refinement-launch.v1') and (p['scope']==scope) and (type(p['parentIndex'])is int) and (p['parentIndex']==index) and (w.equal(p['limits'],w.LIMITS)),'parent plan scope/limits')
    ...
    require((m['schema']=='braid-program/f6c-parent-emission-refinement-cover.v1') and (m['scope']==scope) and (m['status']=='conditional_complete') and (m['accepted']is False),'parent manifest disposition')
    require((c['schema']=='braid-program/f6c-parent-emission-refinement-conformance.v1') and (c['scope']==scope) and (c['accepted']is True) and (c['analysis']['conditional_final_cover_conformant']is True) and (c['analysis']['conditional_query_replay_conformant']is True),'independent parent comparison required')
    require((op['schema']=='braid-program/f6c-parent-emission-refinement-operation.v1') and (op['scope']==f'operational-original-parent{index}-refinement-completion-only') and (op['accepted']is True) and (type(op['parentIndex'])is int) and (op['parentIndex']==index),'parent operation disposition')
    ...
    expected_original['fullEntry']=p['originalBindings']['fullEntry']
    expected_original.update(('full'+k[0].upper()+k[1:],v)for k,v in full.items())
    require((w.equal(w.source_map(p['originalBindings'].values(),pool.root),w.source_map(expected_original.values(),pool.root))) and (set(p['originalBindings'])==set(expected_original)),'parent exact original sources')
    ...
    for group,n in ((subjects,len(named)+len(p['dependencies'])),(p['runtimeBindings'],len(p['runtimeBindings'])),(p['operationalBindings'],len(p['operationalBindings']))):
    ...
    for key in ('producer','verifier'):require(w.equal(m[key],p[key]),'parent implementation role differs')
    ...
    for item,stage in zip(op['stages'],('producer','comparison')):
        ...
        require((item['stage']==stage) and (proc['accepted']is True) and (proc['processesClosed']is True) and (w.equal(proc['exit'],dict(code=0,signal=None))) and (ad['accepted']is True) and (ad['h3EvidenceEligible']is False) and (done['completed']is True) and (done['accepted']is(stage=='comparison')),
        ...
        require((gate['retired']is True) and (gate['acknowledged']is True) and (gate['measurement']['code']==0) and (gate['measurement']['signal']is None),'parent stage gate closure')
    ...
    for j,row in enumerate(rss):
        require((row['kind']=='aggregate-rss') and (0<=F(row['elapsedSeconds'])<closure) and (j==0 or F(rss[j-1]['elapsedSeconds'])<=F(row['elapsedSeconds'])),'parent resource time ordering')
        require((type(row['aggregateResidentBytes'])is int) and (0<=row['aggregateResidentBytes']<=2*1024**3) and (0<=F(row['sampleGapMs'])<=1000),'parent resource limits')
...
def _authenticate_fresh_parents(w,core,pool,owner,ancestry,full,fdocs,export,parents,a,reference,histories,integral):
    ...
    source={role:pool.capture(path,None,data=role=='instrument')for role,path in FRESH_CLOSURE_SOURCES.items()}
    require(set(source)=={'instrument','controls'},'complete independently reviewed fresh authority')
    ...
    original['fullEntry']=next(b for b in fdocs['admission']['sourceBindings'] if Path(b['path']).name=='run-f6c-cached-root-cover-full.mjs')
    ...
    for selection,inventory,admitted in pool.metadata:
        require((w.equal(inventory['family'],family)) and (w.equal(inventory['context'],context)) and (w.equal(inventory['numericalSettings'],numerical_settings)),'fresh original family/context/settings differ')
        ...
        for item in admitted:
            ...
            require((type(verified['parents'])is tuple) and (0<len(verified['parents'])<=159) and (type(verified['sourceBindings'])is tuple) and (len(verified['sourceBindings'])<=512),'bounded verified fresh result')
            ...
            for accepted in verified['parents']:
                ...
                require((w.equal(accepted['roles'],bound)) and (w.equal(accepted['acceptanceOwner'],objects[declared['acceptanceOwner']]['original'])),
...
def _full_chain(w,core,docs,bound,entry_raw,pool,owner_raw):
    ...
    require((contract['scope']=='full'),'full comparison contract')
    source_map=w.source_map(a['sourceBindings'],pool.root)
    ...
    for item,stage in zip(a['stages'],('consumer','comparison')):
        ...
        require((item['stage']==stage) and (proc['accepted']is True) and (proc['processesClosed']is True) and (w.equal(proc['exit'],dict(code=0,signal=None))) and (ad['accepted']is True) and (w.equal(proc['admission'],ad)),'full closed stage')
        _seq(proc['gates'],1);gate=proc['gates'][0]
    ...
    for j,x in enumerate(rss):
        require((type(x)is dict) and (x['kind']=='aggregate-rss') and (0<=F(x['elapsedSeconds'])<F('862.951823625')) and (j==0 or F(rss[j-1]['elapsedSeconds'])<=F(x['elapsedSeconds'])),'full RSS order')
        require((type(x['aggregateResidentBytes'])is int) and (0<=x['aggregateResidentBytes']<=2*1024**3) and (0<=F(x['sampleGapMs'])<=1000),'full RSS/gap limits')
    prefix=a['observationsBeforePublication'];require((prefix['samples']==3444) and (max(x['aggregateResidentBytes']for x in rss[:3444])==prefix['maximumSampledRSSBytes']),'full prepublication RSS prefix')
...
def _parents_from_raw(a,reference,rows,pieces,histories,bindings,*,cells,refined,original_indices=None):
    ...
    require((type(original_indices)is tuple) and (len(original_indices)==cells) and (all(type(n)is int and 0<=n<160 for n in original_indices)) and (tuple(sorted(set(original_indices)))==original_indices),'ordered original parent identities')
    ...
    for cell_index in range(cells):
        ...
        for local in range(64):
            ...
            i,j=divmod(local,8)
            require((type(raw['rowIndex'])is int) and (raw['rowIndex']==index) and (type(raw['cellIndex'])is int) and (raw['cellIndex']==original_indices[cell_index]) and (type(raw['receiverIndex'])is int) and (raw['receiverIndex']==i) and (type(raw['transmitterIndex'])is int) and (raw['transmitterIndex']==j) and (raw['receiverId']==LABELS[i]) and (raw['transmitterId']==LABELS[j]),'raw ordered row identity')
...
def open_adapter(repo_root,*,adapter_sha256,controls_sha256,closure_owner_sha256,deadline,parent_refinements=(),evidence_package=None,accepted_parent_evidence=()):
    ...
    """
    root=Path(repo_root)
    require((root.is_absolute()) and (root==root.resolve()),'executing canonical repository')
    ...
    adapter=None
    transport_path=SOURCES['transport'];transport_sha=None
    with _bootstrap(root/transport_path,transport_sha,live)as raw:
        with _module(raw,root/transport_path)as w,ExitStack()as stack:
            ...
            source={role:(pool if role in ('fullEntry','integralProof','correlatedProof','gkProof','refinedClosure') else physical_pool).capture(path,None,data=role in module_roles+('fullEntry','geometryHistory','geometryRoots','geometryIntervals'))for role,path in SOURCES.items()}
            ...
            selected=override+parents[1:];refined_indices=frozenset({0})
            for descriptor in parent_refinements:
                index=descriptor.parent_index
                parent=_authenticate_parent(w,core,pool,descriptor,owner,ancestry,full,fdocs,old['export'],parents[index],a,reference,histories)
                selected=selected[:index]+(parent,)+selected[index+1:]

                refined_indices=refined_indices|frozenset({index})
            if accepted_parent_evidence:
                with physical_pool.observe(fresh_consumed):
                    fresh=_authenticate_fresh_parents(w,core,pool,owner,ancestry,full,fdocs,old['export'],parents,a,reference,histories,modules['integral'])
                for parent in fresh:
            ...
            adapter=_build(a,modules['integral'],modules['correlated'],old['export'],selected,actual=True,
                ...
                _authenticated_refined_indices=refined_indices)
