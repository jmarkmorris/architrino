...
export function captureSource(filename,expected) {
  ...
  try {
    ...
    check(sameFile(before,fstatSync(fd))&&sameFile(before,lstatSync(filename))&&(expected===undefined||sha(bytes)===expected),"composition generation differs");
...
export async function runFileWorker(job,bytes,remainingMs,signal) {
  ...
  const worker=new Worker(`const{parentPort,workerData}=require('node:worker_threads');
    ...
    .then(async m=>{const job=workerData.job;
