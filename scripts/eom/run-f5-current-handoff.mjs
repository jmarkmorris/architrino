...
export async function run(argv) {
  ...
  for (let i=0; i<argv.length; i+=2) {
    check(['--plan','--plan-sha256','--out','--owner-task'].includes(argv[i]) && argv[i+1] && !args[argv[i]], 'named unique argument pairs required');
    args[argv[i]]=argv[i+1];
  }
  check(Object.keys(args).length === 4, 'plan, plan-sha256, bridge-sha256, out and owner-task required');
  const node=realpathSync(process.execPath),nodeIdentity=f5Fs.lstatSync(node,{bigint:true});
  const nodeBinding=readBound(node);
  const sourceIdentities={[node]:[nodeIdentity.dev,nodeIdentity.ino,nodeIdentity.size,nodeIdentity.mtimeNs,nodeIdentity.ctimeNs].join(':')};
  ...
  recheckNode();
  const {runOwned}=await import("../dev/owned-compute-supervisor.mjs");
  const {writeNew}=await import("../eom/launch-f5-prehistory-handoff-build.mjs");
  const plan = readBound(args['--plan'], args['--plan-sha256']);
  const bridge = readBound(BRIDGE);
  ...
  recheckNode();
  const result={nodeRuntime:nodeBinding,schema:'braid-program/f5-current-handoff-admission.v1',accepted:true,
