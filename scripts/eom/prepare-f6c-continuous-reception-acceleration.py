...
def project_cell(export, manifest, rows, pieces, reference, range_bindings):
    ...
    require((manifest['schema'] == 'braid-program/f6c-continuous-reception-root-cover.v1') and (manifest['scope'] == 'pilot-cell-0') and (manifest['status'] == 'conditional_complete') and (manifest['accepted'] is False), 'complete unpromoted pilot manifest required')
    ...
    require((frames[0]['time'] == '0') and (frames[-1]['time'] == '0.13') and (token(frames[1]['time']) >= Fraction(1,1000)), 'cell zero outside exact first frame')
    def bounds(record):
...
class PinnedInput:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.expected=digest if self.expected is None else self.expected;require(digest==self.expected,'source hash differs: '+str(self.path)); return self
...
def validate_plan(plan,own_sha):
    ...
    keys(premise,('authority','admissionSha256','matchingFreshCompletionObserved','exitCode',
                 'elapsedSeconds','processesClosed','independentAuditAccepted'))
    require((premise['authority']=='externally-reviewed-caller-observation') and (premise['admissionSha256']==dict((r,h) for r,_,h in FIXED)['admission']) and (premise['matchingFreshCompletionObserved'] is True) and (premise['processesClosed'] is True) and (premise['independentAuditAccepted'] is True) and (type(premise['exitCode']) is int) and (premise['exitCode']==0) and (premise['elapsedSeconds']=='8.534247625'),'prior external closure premise absent/different')
    return plan
...
def main(argv=None):
    ...
    try:
        with ExitStack() as stack:
            owned=[]
            def capture(path,digest=None,**kwargs):
            ...
            fixed=({role:capture(root/path,digest,capture=True) for role,path,digest in FIXED} | {role: capture(root/path,capture=True) for role,path in SOURCE_PATHS.items()})
