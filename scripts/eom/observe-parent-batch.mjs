...
import {capture,clean,verifyClosure,parseProcessTable,COORDINATOR_SHA,LOCK} from './verify-f6c-bounded-operation-closure.mjs';
...
function parse(argv){
 ...
 for(let i=0;i<argv.length;i++){
  ...
  check(['--plan','--plan-sha256','--self-sha256','--checker-sha256','--out-directory','--owner-task'].includes(k)&&!Object.hasOwn(result,k),'unique known observer flag');
  check(typeof argv[i+1]==='string'&&!argv[i+1].startsWith('--'),'observer flag value');result[k]=argv[++i];
 }
 for(const k of ['--plan','--plan-sha256','--self-sha256','--checker-sha256','--out-directory','--owner-task'])check(result[k],'required observer flag');
 return result;
}
...
export async function main(argv=process.argv.slice(2)){
 ...
 const coordinator=identify(path.join(root,'scripts/eom/f6c-bounded-operation.mjs'),COORDINATOR_SHA,1048576);
 ...
 if(!options.control)check(plan.hookModule?.path===path.join(root,'scripts/eom/run-f6c-parent-emission-refinement-pilot.mjs')&&plan.configuration?.schema==='braid-program/f6c-parent-emission-refinement-batch.v2'&&plan.configuration.completionContract==='braid-program/f6c-bounded-operation-external-closure.v1','current parent batch completion contract');
 const initial=[self,checker,node,supervisor,ps,coordinator,launch];
 ...
 const invocation={schema:'braid-program/observed-bounded-invocation.v1',root,coordinator:clean(coordinator),node:clean(node),plan:clean(launch),control:options.control};
 save('invocation.json',invocation);
 const maximum=options.control?120000:1800000,began=performance.now();let lease;
 const args=[coordinator.path,options.control?'--control-plan':'--plan',launch.path,'--plan-sha256',launch.sha256,'--self-sha256',coordinator.sha256];
 try{
  ...
  const receipt=verifyClosure({invocation,lease,elapsedMilliseconds:performance.now()-began,wire,operation,plan,observation});
