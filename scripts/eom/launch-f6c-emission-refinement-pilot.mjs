// Nonmathematical, one-cell serial producer/comparison composition. The reused
...
function privateProducerFiles(directory){if(!existsSync(directory))return;const names=readdirSync(directory).filter(name=>name.startsWith('.emission-private-'));check(names.length<=1,'unexpected repeated private publication');for(const name of names){const folder=path.join(directory,name);check(lstatSync(folder).isDirectory()&&realpathSync(folder)===folder,'private publication directory type');const files=readdirSync(folder);check(files.every(file=>['queries.ndjson','rows.ndjson','pieces.ndjson','cover-manifest.json'].includes(file)),'unexpected private output');for(const file of files)size(path.join(folder,file),FILE_LIMIT);}}
export async function ignoredOutputs(git,root,output){
  check(path.dirname(output)===path.join(root,'.local-data/braid-analysis/f6c-emission-refinement-20260827'),'scoped ignored output');
...
export async function launchCaptured({root,options,self,entry,outerBytes,helperBytes,began,deadlineNanoseconds,diagnostics}){
  ...
  const stageLogs=()=>['producer','comparison'].flatMap(name=>['runner-stdout.log','runner-stderr.log'].map(file=>path.join(paths.operations,name+'-process',file)));
  ...
  const poll=()=>{for(const file of paths.dataFiles)size(file,FILE_LIMIT);size(paths.comparison,FILE_LIMIT);privateProducerFiles(output);privateCandidates(paths.operations,'.emission-comparison-private-');logBytes();};
  ...
  try{
    ...
    interval=setInterval(()=>{try{
      ...
      if(performance.now()>=nextHeartbeat){nextHeartbeat=performance.now()+15000;log({kind:'f6c-emission-refinement-pilot-heartbeat',stage,elapsedSeconds:elapsed(began),closedAdmittedStages:stages.filter(s=>s.admission?.accepted).map(s=>s.stage),maximumSampledAggregateRSSBytes:rss.maximumSampledRSSBytes,accepted:false});}
      poll();
    ...
    const record={schema:'braid-program/f6c-emission-refinement-pilot-admission.v1',accepted:true,scope:C.SCOPE,
      authority:'source-bound one-cell conditional emission-refinement and complete-cover conformance only',processesClosed:true,stages,plan:planBinding,
      ...
      census:C.CENSUS,accelerationEvaluated:false,eomExecuted:false,h3EvidenceEligible:false,metricsAvailable:false,scoreAuthorized:false,historicalTrajectoryIdentityEstablished:false,fullRunAuthorized:false};
    ...
    completion={completed:true,accepted:true,scope:C.SCOPE,receipt:publication,logs,rawOutputBytes:stages.flatMap(s=>s.admission.outputs).reduce((n,b)=>n+b.bytes,0),
      ...
      census:C.CENSUS,accelerationEvaluated:false,eomExecuted:false,h3EvidenceEligible:false,metricsAvailable:false,scoreAuthorized:false,historicalTrajectoryIdentityEstablished:false,fullRunAuthorized:false};return completion;
  }catch(error){
    ...
    }catch(preservation){diagnostics.write(Buffer.from('Refinement rejection preservation failed: '+preservation.message+'\n'));}throw error;
  }finally{
