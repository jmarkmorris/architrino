...
export function parseSubfieldCircularDispatchArgs(argv) {
  const allowed = ["--plan", "--plan-sha256", "--out"], result = {};
  ...
  check(allowed.every(key => result[key]) && ["--plan-sha256"].every(key => /^[0-9a-f]{64}$/u.test(result[key])) &&
...
export async function subfieldCircularDispatchFileOperation(job) {
  ...
  const rung = await import("./run-subfield-circular-root-rung.mjs");
  if (job.kind === "shared") {
    for (const binding of job.bindings) readBound(path.resolve(job.root, binding.path), binding.sha256);
    const pilot = await import("./run-subfield-circular-root-pilot.mjs");
    pilot.pilotFileOperation({ kind: "build", root: job.root }); return { sharedBytesMatch: true };
  }
  if (job.kind === "publish") {
    const outer = await import("./launch-subfield-circular-root-pilot.mjs");
...
export async function watchedSubfieldCircularDispatchOperation(job, { bytes, sha256, limitMs, signal }) {
  ...
  { eval: true, execArgv: [], workerData: { moduleUrl: import.meta.url, job } });
  let timer, onAbort;
        ...
        try {
          ...
          processReceipt = await runtime.outer.superviseRegisteredPilot({ root, entry: RUNG,
            args: ["--plan", receipt.plan.path, "--plan-sha256", receipt.plan.sha256, "--candidate", candidateId, "--rung", String(rung),
              ...
              "--out", path.relative(root, runOutput)],
...
async function main() {
  ...
  const selfBytes = readFileSync(path.join(root, SELF));
  const plan = readBound(path.resolve(args["--plan"]), args["--plan-sha256"], true).value;
  const rungBytes = readFileSync(path.join(root, RUNG));

  const runtime = { rung: await import("./run-subfield-circular-root-rung.mjs"), outer: await import("./launch-subfield-circular-root-pilot.mjs") };
  const result = await runSubfieldCircularMeasuredDispatch({ root, args, selfBytes, rungBytes, runtime });
...
if (isMainThread && import.meta.url.startsWith("file:") && process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url))
