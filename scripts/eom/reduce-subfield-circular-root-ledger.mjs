...
export async function runSubfieldCircularLedger(snapshot,progress) {
  const module=await import("../../src/prescribed-path-analysis/SubfieldCircularRootLedgerReducer.mjs");
...
async function main() {
  ...
  const snapshot={repoRoot:args.repoRoot,scope:args.scope,phaseReceipts:args.phaseReceipts,rowsFile:args.rowsFile,
    ...
    cliFile:self,cliBytes,reducerBytes,cliSha256:sha(cliBytes),reducerSha256:sha(reducerBytes)};
  ...
  const worker=new Worker(`const{parentPort,workerData}=require("node:worker_threads");(async()=>{const cli=await import(require("node:url").pathToFileURL(workerData.cliFile).href);const result=await cli.runSubfieldCircularLedger(workerData,event=>parentPort.postMessage({event}));parentPort.postMessage({result});})().catch(error=>{parentPort.postMessage({failure:String(error.message)});process.exitCode=1;});`,{eval:true,workerData:snapshot});
...
if (isMainThread && import.meta.url.startsWith("file:")&&process.argv[1]&&realpathSync(process.argv[1])===realpathSync(fileURLToPath(import.meta.url)))main().catch(error=>{console.error(error.message);process.exitCode=1;});
