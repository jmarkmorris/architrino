...
const SELF = "scripts/eom/prepare-abc-enclosed-root.mjs";
const SUPERVISOR = "scripts/eom/prepare-f5-enclosed-root.mjs";
const SUBJECT = "src/eom/native/eom_abc_enclosed_root_cli.cpp";
const BASE = ".local-data/braid-analysis/abc-h3-root-pilot-20260827-v1/";
const PINNED = Object.freeze({
  [SUBJECT]: "58f62a47bde4688708e2e2c3b10de32a123aa24b69b9d563878e280191174f7e",
  "src/eom/CMakeLists.txt": "e4b3a8bdfc91c756eb00e4c37e872bcbebfe1f7b406a551e3aa630f8818d2bdd",
  [SUPERVISOR]: "ba154c0a8c63bd390ae1e16de005fd5d52000fedec352619b60b9465a2f813f5",
});
...
export function referenceSnapshot() {
  ...
  if (rows.length !== 8) throw new Error("frozen adapter reference table differs");
...
export function parsePrepareAbcArgs(argv) {
  if (argv.length !== 2 || argv[0] !== "--out" || !argv[1]) throw new Error("Usage: node scripts/eom/prepare-abc-enclosed-root.mjs --out <new-ABC-run-directory>");
...
export async function prepareAbc(argv) {
  const output = parsePrepareAbcArgs(argv);
  ...
  const receipt = { schema: "braid-program/abc-enclosed-root-build.v1", status: "incomplete",
  ...
  try {
    ...
    for (const [index, unit] of units.entries()) {
      ...
      await watched(`dependencies-${index}`, compiler,
        [...unit.args, "-M", "-MF", dep, "-MT", "abc_dependency", unit.source], unit.directory);
    ...
    const executable = path.join(build, "eom_abc_enclosed_root_cli"), library = path.join(build, "libeom_native.a");
    ...
    await watched("adapterlink", compiler, [...manualArgs, "-MD", "-MF", manualDependencyFile,
      "-MT", "abc_adapter", subject.source, library, ...externalPaths, "-pthread", `-fuse-ld=${linker}`, "-v", "-o", executable]);
    for (const [index, unit] of units.entries()) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  prepareAbc(process.argv.slice(2)).catch((error) => { console.error(error.message); process.exitCode = 1; });
}
