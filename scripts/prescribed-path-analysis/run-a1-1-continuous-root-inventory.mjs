...
import {
  ...
  summarizeA11ContinuousRootInventory,
} from "../../src/prescribed-path-analysis/A11ContinuousRootIntervalCertifier.mjs";
...
const provenancePath = path.resolve(
  repositoryRoot,
  protocol.sourceFamily.displaySourceProvenance.path,
);
...
if (provenanceHash !== protocol.sourceFamily.displaySourceProvenance.sha256) {
  throw new Error(
    "A1.1 display-source provenance hash changed; review the exact endpoint, " +
      "frame, polarity, and circulation declarations before execution.",
  );
}

const result = evaluateA11ContinuousRootInventory({ protocol });
const summary = summarizeA11ContinuousRootInventory(result);
