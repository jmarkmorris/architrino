...
export function supportRatios({ geo = CHAMPION, cTrans = 1.0, sea = null, soft = 0.02 } = {}) {
  ...
  for (const i of [0, 2, 4]) {
    ...
    // site at t=0: azimuth th; cylindrical radial unit vector
    ...
    layers.push({ layer: s.name, support: (kap * inward) / need, rhoCyl, speed: w * rhoCyl });
  }
...
export function searchSupport({ start = CHAMPION, sea = null, rounds = 3, soft = 0.02, stepScale = 1 } = {}) {
  ...
  let best = supportRatios({ geo: g, sea, soft });
    ...
    for (const k of Object.keys(steps)) {
      for (const sgn of [+1, -1]) {
        ...
        while (improved) {
          ...
          const t = supportRatios({ geo: trial, sea, soft });
