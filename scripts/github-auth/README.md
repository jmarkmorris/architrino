...
## Mandatory caller restriction

Each agent may request only its own tool/repository context. Codex must not select a Claude context, and Claude must not select a Codex context, including for diagnostic tests, unless the operator separately and explicitly authorizes that cross-context test. An unavailable or denied assigned route stops the operation; never borrow another token or fall back to a shared login. The operator accepts the shared-Mac access limitation: this rule governs agent behavior and does not authenticate the caller at the operating-system level.
...
Claude Code Local reports a completed operator-authorized commit-and-push run through `--context claude-architrino` at `170f0c63934c6563290fe135d75c522dab34271f`, with the exact-state gate and hooks passing and server ref/tree matching local. This task independently checked local HEAD against the reported commit; the remote comparisons are runner-reported evidence. This exercised Claude Git write access. In a subsequent handoff, Claude reports PR creation, check monitoring, merge verification, and guarded cleanup through this context for [the F5 recovery and credential-route PR](https://github.com/jmarkmorris/architrino/pull/262). Independent Git/API inspection confirmed the merge, reviewed-head ancestry, synchronized main, and branch retirement; attribution to the Claude credential for those operations remains runner-reported. Ordinary-command and desktop PR-status routing remain open.

## Renewal and operating ownership

Use the [credential operating procedure](../../reference/op/git/git-github-operating-guide.md#243-operating-routes-credential-ownership-and-remaining-verification) for the four route choices, desktop limitations, renewal and revocation ownership, and verification evidence. Installation refuses duplicate Keychain entries; it does not rotate an existing token. Never switch a shared CLI login to select an agent.
