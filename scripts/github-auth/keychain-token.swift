...
let mode = args[0]
let contexts = ["codex-architrino": ("codex", "architrino"), "codex-mylists": ("codex", "MyLists"),
    "claude-architrino": ("claude", "architrino"), "claude-mylists": ("claude", "MyLists")]
guard let (tool, repository) = contexts[args[1]] else { fail("Unknown context; no credential accessed.") }
guard args.count == (mode == "credential" ? 3 : 2) else { fail("Unexpected arguments; no credential accessed.") }
let displayTool = tool == "codex" ? "Codex" : "Claude"
let service = "com.architrino.github.\(tool).\(repository.lowercased())"
...
switch mode {
case "describe":
    // Public routing metadata only; never queries Keychain.
    print(service)
case "install":
    guard isatty(STDIN_FILENO) != 0 else { fail("Run installation yourself in an interactive terminal; token input is hidden.") }
    guard let input = getpass("Paste \(displayTool)–\(repository) fine-grained PAT (hidden), then press Return: ") else { fail("No input received.") }
    ...
    var item = query
    item[kSecAttrLabel as String] = "\(displayTool) \(repository) GitHub PAT"
    ...
    print("Saved \(displayTool)–\(repository) token in a separate Keychain entry. Existing logins unchanged.")
case "check":
    _ = loadToken()
