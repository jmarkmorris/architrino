...
guard args.count >= 2 else { fail("Usage: keychain-token <install|credential|check> codex-architrino [get|store|erase]") }
let mode = args[0]
guard args[1] == "codex-architrino" else { fail("Unknown context; no credential accessed.") }
let query: [String: Any] = [kSecClass as String: kSecClassGenericPassword,
    kSecAttrService as String: "com.architrino.github.codex.architrino",
...
case "install":
    ...
    guard let input = getpass("Paste Codex–Architrino fine-grained PAT (hidden), then press Return: ") else { fail("No input received.") }
    ...
    var item = query
    item[kSecAttrLabel as String] = "Codex Architrino GitHub PAT"
    ...
    print("Saved Codex–Architrino token in a separate Keychain entry. Existing logins unchanged.")
case "check":
    _ = loadToken()
...
case "credential":
    ...
    guard fields["protocol"] == "https", fields["host"] == "github.com",
          ["jmarkmorris/architrino", "jmarkmorris/architrino.git"].contains(fields["path"] ?? "") else {
        print("quit=true\n"); exit(0)
    }
