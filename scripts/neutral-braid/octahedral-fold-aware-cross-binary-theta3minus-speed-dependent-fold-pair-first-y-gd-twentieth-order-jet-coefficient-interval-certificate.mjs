...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTIETH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twentieth-order-jet-coefficient-interval-certificate/v1";
...
const TWENTIETH_ORDER_SPEED_CELL_COUNT = 128;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y24";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y24";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y24";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y24";
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y22: gSeries
        .slice(0, 23)
  ...
  const qG = Array.from({ length: 21 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 21 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y22: pairG.slice(0, 23).map(root.formatInterval),
    D_pair_coefficients_y0_to_y22: pairD.slice(0, 23).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qDRemainingTwentyFirstOrderTailBudget > 1e56
      ...
      : "first-y-GD-twentieth-order-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y24_interval: root.formatSmallNumber(
    ...
    all_QD_QG_coefficient_identity_intervals_contain_zero: rows.every(
      (row) =>
        ...
        containsZero(row.Q_D_plus_15Q_G_y14_coefficient_interval.map(Number)) &&
        containsZero(row.Q_D_plus_16Q_G_y15_coefficient_interval.map(Number)) &&
        containsZero(row.Q_D_plus_17Q_G_y16_coefficient_interval.map(Number)) &&
        containsZero(row.Q_D_plus_18Q_G_y17_coefficient_interval.map(Number)) &&
        containsZero(row.Q_D_plus_19Q_G_y18_coefficient_interval.map(Number)) &&
        containsZero(row.Q_D_plus_20Q_G_y19_coefficient_interval.map(Number)) &&
        containsZero(row.Q_D_plus_21Q_G_y20_coefficient_interval.map(Number))
    ),
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-twentieth-order-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentiethOrderJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? TWENTIETH_ORDER_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    twentieth_order_jet_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+h15_epsilon*y^18+h16_epsilon*y^19+h17_epsilon*y^20+h18_epsilon*y^21+h19_epsilon*y^22+h20_epsilon*y^23+O(y^24)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h20; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, eighteenth-order, nineteenth-order, and twentieth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "twentieth-order coefficient interval only; the O(y^21) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    twentieth_order_jet_rows: rows,
    twentieth_order_jet_summary: summary,
    closure_burndown: [
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, eighteenth-order, nineteenth-order, and twentieth-order first-y quotient coefficients Q_G0 through Q_G20 and Q_D0 through Q_D20 on the fold-pair h-root graph. It closes the twentieth-order coefficient row only; the finite twenty-first-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite twentieth-order-tail problem to a finite twenty-first-order-tail problem after the certified twentieth-order coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^21) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentiethOrderJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.twentieth_order_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTIETH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twentieth-order jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twentieth-order jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twentieth_order_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twentieth-order jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twentieth_order_jet_parameters?.speed_band === undefined &&
      artifact?.twentieth_order_jet_parameters?.speed_window === undefined &&
      artifact?.twentieth_order_jet_parameters?.speed_min === undefined &&
      artifact?.twentieth_order_jet_parameters?.speed_max === undefined,
    "first-y twentieth-order jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === TWENTIETH_ORDER_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * TWENTIETH_ORDER_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y24 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y24_interval) <
        1e11 &&
      ...
      Number(summary?.max_abs_h14_interval) < 1e7 &&
      Number(summary?.max_abs_h15_interval) < 1e9 &&
      Number(summary?.max_abs_h16_interval) < 1e12 &&
      Number(summary?.max_abs_h17_interval) < 1e8 &&
      Number(summary?.max_abs_h18_interval) < 6e8 &&
      Number(summary?.max_abs_h19_interval) < 1e10 &&
      Number(summary?.max_abs_h20_interval) < 1e11 &&
      ...
      Number(summary?.max_abs_Q_G_y14_coefficient_interval) < 1e8 &&
      Number(summary?.max_abs_Q_D_y14_coefficient_interval) < 2e9 &&
      Number(summary?.max_abs_Q_G_y15_coefficient_interval) < 1e10 &&
      Number(summary?.max_abs_Q_D_y15_coefficient_interval) < 2e11 &&
      Number(summary?.max_abs_Q_G_y16_coefficient_interval) < 2e7 &&
      Number(summary?.max_abs_Q_D_y16_coefficient_interval) < 3e8 &&
      Number(summary?.max_abs_Q_G_y17_coefficient_interval) < 8e7 &&
      Number(summary?.max_abs_Q_D_y17_coefficient_interval) < 1.5e9 &&
      Number(summary?.max_abs_Q_G_y18_coefficient_interval) < 5e8 &&
      Number(summary?.max_abs_Q_D_y18_coefficient_interval) < 1e10 &&
      Number(summary?.max_abs_Q_G_y19_coefficient_interval) < 5e9 &&
      Number(summary?.max_abs_Q_D_y19_coefficient_interval) < 1e11 &&
      Number(summary?.max_abs_Q_G_y20_coefficient_interval) < 5e10 &&
      Number(summary?.max_abs_Q_D_y20_coefficient_interval) < 2e12 &&
      ...
      Number(summary?.max_abs_Q_D_plus_14Q_G_y13_coefficient_interval) <
        6500000 &&
      Number(summary?.max_abs_Q_D_plus_15Q_G_y14_coefficient_interval) <
        2e9 &&
      Number(summary?.max_abs_Q_D_plus_16Q_G_y15_coefficient_interval) <
        2e11 &&
      Number(summary?.max_abs_Q_D_plus_17Q_G_y16_coefficient_interval) <
        5e8 &&
      Number(summary?.max_abs_Q_D_plus_18Q_G_y17_coefficient_interval) <
        3e9 &&
      Number(summary?.max_abs_Q_D_plus_19Q_G_y18_coefficient_interval) <
        2e10 &&
      Number(summary?.max_abs_Q_D_plus_20Q_G_y19_coefficient_interval) <
        2e11 &&
      Number(summary?.max_abs_Q_D_plus_21Q_G_y20_coefficient_interval) <
        2e12 &&
    ...
    "first-y twentieth-order jet rows must certify h solves, source coefficient containment, twentieth-order coefficients, and positive post-twentieth-order tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twentieth-order-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
