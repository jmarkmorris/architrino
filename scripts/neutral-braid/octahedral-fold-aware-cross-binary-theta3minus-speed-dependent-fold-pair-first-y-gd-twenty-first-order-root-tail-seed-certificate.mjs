...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_FIRST_ORDER_ROOT_TAIL_SEED_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-first-order-root-tail-seed-certificate/v1";
...
const TWENTY_FIRST_ORDER_ROOT_TAIL_SEED_SPEED_CELL_COUNT = 128;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y25";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y25";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y25";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y25";
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y23: gSeries
        .slice(0, 24)
  ...
  const qG = Array.from({ length: 22 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 22 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y23: pairG.slice(0, 24).map(root.formatInterval),
    D_pair_coefficients_y0_to_y23: pairD.slice(0, 24).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qDRemainingTwentySecondOrderTailBudget > 1e59
      ...
      : "first-y-GD-twenty-first-order-root-tail-seed-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y25_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-twenty-first-order-root-tail-seed-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyFirstOrderRootTailSeedCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? TWENTY_FIRST_ORDER_ROOT_TAIL_SEED_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    twenty_first_order_root_tail_seed_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+h15_epsilon*y^18+h16_epsilon*y^19+h17_epsilon*y^20+h18_epsilon*y^21+h19_epsilon*y^22+h20_epsilon*y^23+h21_epsilon*y^24+O(y^25)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h21; h21 is the E_epsilon(0,nu) root-tail seed coefficient",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, eighteenth-order, nineteenth-order, twentieth-order, and twenty-first-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "root-tail seed coefficient interval only; the finite E_epsilon(y,nu) tube and the O(y^22) quotient tail remain open before full first-y enclosure",
      root_tail_seed_equation:
        "Shift_25(F_epsilon(y,h_{<=20}+y^21*E,nu))=C_{21,epsilon}(nu)+S_{21,epsilon}(nu)*E+O(y), with E(0,nu)=h21_epsilon(nu)",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    twenty_first_order_root_tail_seed_rows: rows,
    twenty_first_order_root_tail_seed_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-twenty-second-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-twenty-second-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the twenty-first root-tail seed coefficient h21=E_epsilon(0,nu) and the induced twenty-first-order first-y quotient coefficients Q_G21 and Q_D21 on the fold-pair h-root graph. It closes the root-tail seed row only; the finite E_epsilon(y,nu) tube, O(y^22) quotient tail/enclosure, and downstream collar closure remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a missing root-tail seed to a finite E_epsilon tube and O(y^22) quotient-tail problem after the certified h21, Q_G21, and Q_D21 intervals. The remaining sign condition is now a directed-rounded bound on the continuous post-seed tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyFirstOrderRootTailSeedCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.twenty_first_order_root_tail_seed_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_FIRST_ORDER_ROOT_TAIL_SEED_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-first-order root-tail seed certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-first-order root-tail seed packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_first_order_root_tail_seed_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-first-order root-tail seed certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_first_order_root_tail_seed_parameters?.speed_band === undefined &&
      artifact?.twenty_first_order_root_tail_seed_parameters?.speed_window === undefined &&
      artifact?.twenty_first_order_root_tail_seed_parameters?.speed_min === undefined &&
      artifact?.twenty_first_order_root_tail_seed_parameters?.speed_max === undefined,
    "first-y twenty-first-order root-tail seed parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === TWENTY_FIRST_ORDER_ROOT_TAIL_SEED_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * TWENTY_FIRST_ORDER_ROOT_TAIL_SEED_SPEED_CELL_COUNT &&
      ...
      Number.isFinite(Number(summary?.max_abs_source_equation_coeff_y0_to_y25_interval)) &&
      Number.isFinite(Number(summary?.max_abs_h21_interval)) &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y21_coefficient_interval)) &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y21_coefficient_interval)) &&
      Number.isFinite(
    ...
    "first-y twenty-first-order root-tail seed rows must certify h21, source coefficient containment, Q_G21/Q_D21 coefficients, and positive post-seed tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-first-order-root-tail-seed-certificate.mjs [options]",
    "",
    "Options:",
