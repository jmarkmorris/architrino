...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SIXTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-sixth-order-post-u-successor-coefficient-certificate/v1";
...
const SERIES_ORDER = 30;
const H_COUNT = 27;
const PREDECESSOR_H_COUNT = 26;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y30";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y30";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y30";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y30";
...
function solveH26Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h25_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h25_interval: predecessorBranchRow.h25_interval,
      h26_interval: root.formatInterval(solve.hInterval),
      h26_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h26_solve_slope_sign: slope.sign,
      h26_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h26_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y28: gSeries
        .slice(0, 29)
  ...
  const qGRemainingTwentySeventhOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 27;
  const qDRemainingTwentySeventhOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 27;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y28: pairG.slice(0, 29).map(root.formatInterval),
    D_pair_coefficients_y0_to_y28: pairD.slice(0, 29).map(root.formatInterval),
    ...
    Q_G_y26_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[26]),
    Q_D_y26_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[26]),
    Q_G_remaining_twenty_seventh_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentySeventhOrderTailBudget
    ),
    Q_D_remaining_twenty_seventh_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentySeventhOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingTwentySeventhOrderTailBudget > 1e73 &&
    qDRemainingTwentySeventhOrderTailBudget > 1e73
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-twenty-sixth-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h25_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y30_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_27Q_G_y26_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_27Q_G_y26_coefficient_interval")
    ),
    ...
    h26_interval_hull: coefficientHull(
      branchRows.map((row) => row.h26_interval.map(Number))
    ),
    Q_G_y26_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y26_coefficient_interval.map(Number))
    ),
    Q_D_y26_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y26_coefficient_interval.map(Number))
    ),
    max_abs_h26_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h26_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y26_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 26, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y26_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 26, "max_abs_coefficient"))
    ),
    max_Q_G_y26_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y26_loss_on_first_y_cell")
    ),
    max_Q_D_y26_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y26_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_twenty_seventh_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_seventh_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_seventh_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_seventh_order_tail_budget")
    ),
    inherited_h25_interval_hull:
      predecessorArtifact.twenty_fifth_order_post_u_successor_coefficient_summary
        .h25_interval_hull,
    ...
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-twenty-sixth-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySixthOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyFifthOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.twenty_fifth_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    ...
    twenty_sixth_order_post_u_successor_coefficient_parameters: {
      ...
      predecessor_h25_artifact:
        "twenty-fifth-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h25_epsilon*y^28+h26_epsilon*y^29+O(y^30)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h26 after the h25 predecessor row",
      intervalized_quantity:
        "twenty-sixth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor finite root-tail tube and O(y^27) quotient tail remain open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_30(F_epsilon(y,h_{<=25}+y^26*Z,nu))=C_{26,epsilon}(nu)+S_{26,epsilon}(nu)*Z+O(y), with Z(0,nu)=h26_epsilon(nu)",
    },
    twenty_sixth_order_post_u_successor_coefficient_rows: rows,
    twenty_sixth_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
    ...
    artifact_claim: {
      ...
      inherits_directed_rounded_first_y_GD_twenty_fifth_order_post_u_successor_coefficient_enclosure:
        true,
      ...
      claim_level:
        "Directed-rounded interval certificate for h26 and the induced twenty-sixth-order first-y quotient coefficients Q_G26 and Q_D26 on the fold-pair h-root graph. It sharpens the zero-touching quotient burden to the O(y^27) successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h26, Q_G26, and Q_D26 are interval-certified with the preserved Q_D26+27Q_G26 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySixthOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.twenty_sixth_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SIXTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-sixth-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-sixth-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_sixth_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-sixth-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_sixth_order_post_u_successor_coefficient_parameters
  ...
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h25_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y30_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y30_interval) <
        5e18 &&
      Number.isFinite(Number(summary?.max_abs_h26_interval)) &&
      Number(summary?.max_abs_h26_interval) < 1e18 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y26_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y26_coefficient_interval) < 1e18 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y26_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y26_coefficient_interval) < 3e19 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_27Q_G_y26_coefficient_interval)
      ) &&
    ...
    "first-y twenty-sixth-order post-U successor coefficient rows must certify h26, source coefficient containment, Q_G26/Q_D26 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-sixth-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
