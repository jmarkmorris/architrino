...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_OCTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-octic-jet-coefficient-interval-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_octic_jet_coefficient_interval_certificate";
...
const OCTIC_SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-octic-jet-coefficient-interval-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-octic-jet-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y12";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y12";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y12";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y12";
...
function nilpotentPowers(series) {
  ...
  return { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12 };
}

function sinSeries(series) {
  const center = series[0];
  const { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12 } =
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y10: gSeries
        .slice(0, 11)
  ...
  const qG = Array.from({ length: 9 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 9 }, (_, index) => pairD[index + 2]);
  ...
  const qGRemainingNinthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 9;
  const qDRemainingNinthOrderTailBudget =
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y10: pairG.slice(0, 11).map(root.formatInterval),
    D_pair_coefficients_y0_to_y10: pairD.slice(0, 11).map(root.formatInterval),
    ...
    Q_G_remaining_ninth_order_tail_budget: root.formatSmallNumber(
      qGRemainingNinthOrderTailBudget
    ),
    Q_D_remaining_ninth_order_tail_budget: root.formatSmallNumber(
      qDRemainingNinthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingOcticTailBudget > 1e15 &&
    qDRemainingOcticTailBudget > 1e15 &&
    ...
    qGRemainingNinthOrderTailBudget > 1e23 &&
    qDRemainingNinthOrderTailBudget > 1e23
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-octic-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y12_interval: root.formatSmallNumber(
    ...
    min_Q_G_remaining_ninth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_ninth_order_tail_budget")
    ),
    min_Q_D_remaining_ninth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_ninth_order_tail_budget")
    ),
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-octic-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdOcticJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? OCTIC_SPEED_CELL_COUNT
  );
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_OCTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    ...
    octic_jet_parameters: {
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+O(y^12)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h8; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, and octic coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "octic coefficient interval only; the O(y^9) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    octic_jet_rows: rows,
    octic_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-ninth-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-ninth-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, and octic first-y quotient coefficients Q_G0 through Q_G8 and Q_D0 through Q_D8 on the fold-pair h-root graph. It closes the octic coefficient row only; the finite ninth-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-ninth-order-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite eighth-order-tail problem to a finite ninth-order-tail problem after the certified octic coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^9) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdOcticJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.octic_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_OCTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D octic jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D octic jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.octic_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y octic jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.octic_jet_parameters?.speed_band === undefined &&
      artifact?.octic_jet_parameters?.speed_window === undefined &&
      artifact?.octic_jet_parameters?.speed_min === undefined &&
      artifact?.octic_jet_parameters?.speed_max === undefined,
    "first-y octic jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === OCTIC_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * OCTIC_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y12 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y12_interval) <
        26 &&
      ...
      Number(summary?.min_Q_G_remaining_octic_tail_budget) > 1e15 &&
      Number(summary?.min_Q_D_remaining_octic_tail_budget) > 1e15 &&
      Number(summary?.min_Q_G_remaining_octic_tail_budget) > 1e18 &&
      Number(summary?.min_Q_D_remaining_octic_tail_budget) > 1e18 &&
      Number(summary?.min_Q_G_remaining_ninth_order_tail_budget) > 4e23 &&
      Number(summary?.min_Q_D_remaining_ninth_order_tail_budget) > 4e23 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y octic jet rows must certify h solves, source coefficient containment, octic coefficients, and positive post-octic tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-octic-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
...
function main() {
  ...
  if (options.validate) {
    ...
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdOcticJetCoefficientIntervalCertificate(
        artifact
      );
  ...
  try {
    const artifact =
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdOcticJetCoefficientIntervalCertificate(
        options
      );
