...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-third-order-post-u-successor-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_FOURTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-fourth-order-post-u-successor-coefficient-certificate/v1";
...
const SERIES_ORDER = 38;
const H_COUNT = 35;
const PREDECESSOR_H_COUNT = 34;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y38";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y38";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y38";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y38";
...
function solveH34Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h33_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h33_interval: predecessorBranchRow.h33_interval,
      h34_interval: root.formatInterval(solve.hInterval),
      h34_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h34_solve_slope_sign: slope.sign,
      h34_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h34_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y36: gSeries
        .slice(0, 37)
  ...
  const qGRemainingThirtyFifthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 35;
  const qDRemainingThirtyFifthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 35;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y36: pairG.slice(0, 37).map(root.formatInterval),
    D_pair_coefficients_y0_to_y36: pairD.slice(0, 37).map(root.formatInterval),
    ...
    Q_G_y34_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[34]),
    Q_D_y34_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[34]),
    Q_G_remaining_thirty_fifth_order_tail_budget: root.formatSmallNumber(
      qGRemainingThirtyFifthOrderTailBudget
    ),
    Q_D_remaining_thirty_fifth_order_tail_budget: root.formatSmallNumber(
      qDRemainingThirtyFifthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingThirtyFifthOrderTailBudget > 6e75 &&
    qDRemainingThirtyFifthOrderTailBudget > 6e75
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-thirty-fourth-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h33_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y38_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_35Q_G_y34_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_35Q_G_y34_coefficient_interval")
    ),
    ...
    h34_interval_hull: coefficientHull(
      branchRows.map((row) => row.h34_interval.map(Number))
    ),
    Q_G_y34_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y34_coefficient_interval.map(Number))
    ),
    Q_D_y34_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y34_coefficient_interval.map(Number))
    ),
    max_abs_h34_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h34_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y34_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 34, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y34_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 34, "max_abs_coefficient"))
    ),
    max_Q_G_y34_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y34_loss_on_first_y_cell")
    ),
    max_Q_D_y34_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y34_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_thirty_fifth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_thirty_fifth_order_tail_budget")
    ),
    min_Q_D_remaining_thirty_fifth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_thirty_fifth_order_tail_budget")
    ),
    inherited_h33_interval_hull:
      predecessorArtifact.thirty_third_order_post_u_successor_coefficient_summary
        .h33_interval_hull,
    inherited_Q_G_y33_coefficient_interval_hull:
      predecessorArtifact.thirty_third_order_post_u_successor_coefficient_summary
        .Q_G_y33_coefficient_interval_hull,
    inherited_Q_D_y33_coefficient_interval_hull:
      predecessorArtifact.thirty_third_order_post_u_successor_coefficient_summary
        .Q_D_y33_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-thirty-fourth-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyFourthOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyThirdOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.thirty_third_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    ...
    predecessor_packets: [
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-third-order-post-u-successor-coefficient-certificate.md",
    ],
    priority_packet:
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-fourth-order-post-u-successor-coefficient-certificate.md",
    thirty_fourth_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h33_artifact:
        "thirty-third-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h33_epsilon*y^36+h34_epsilon*y^37+O(y^38)",
      h_solve_slope_policy:
        "uses the inherited fold-null slope S_epsilon=epsilon*beta*F_eta_eta for h34 after the h33 predecessor row",
      intervalized_quantity:
        "thirty-fourth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^35) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_38(F_epsilon(y,h_{<=33}+y^34*X34,nu))=C_{34,epsilon}(nu)+S_{34,epsilon}(nu)*X34+O(y), with X34(0,nu)=h34_epsilon(nu)",
    },
    thirty_fourth_order_post_u_successor_coefficient_rows: rows,
    thirty_fourth_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
        row: "theta3minus.fold-pair-first-y-GD-thirty-third-order-post-U-successor-coefficient",
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-thirty-fifth-order-successor-tail-bound",
    ...
    artifact_claim: {
      ...
      inherits_directed_rounded_first_y_GD_thirty_third_order_post_u_successor_coefficient_enclosure:
        true,
      ...
      claim_level:
        "Directed-rounded interval certificate for h34 and the induced thirty-fourth-order first-y quotient coefficients Q_G34 and Q_D34 on the fold-pair h-root graph. It turns the thirty-fourth-order quotient-tail constant term into a coefficient row and sharpens the zero-touching quotient burden to the thirty-fifth-order successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h34, Q_G34, and Q_D34 are interval-certified with the preserved Q_D34+35Q_G34 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyFourthOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.thirty_fourth_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_FOURTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D thirty-fourth-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D thirty-fourth-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.thirty_fourth_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y thirty-fourth-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.thirty_fourth_order_post_u_successor_coefficient_parameters
  ...
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h33_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y38_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y38_interval) <
        3.9e21 &&
      Number.isFinite(Number(summary?.max_abs_h34_interval)) &&
      Number(summary?.max_abs_h34_interval) < 1.5e21 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y34_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y34_coefficient_interval) < 1.3e21 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y34_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y34_coefficient_interval) < 4.4e22 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_35Q_G_y34_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_35Q_G_y34_coefficient_interval) <
        8.8e22 &&
      Number(summary?.min_Q_G_remaining_thirty_fifth_order_tail_budget) >
        1e95 &&
      Number(summary?.min_Q_D_remaining_thirty_fifth_order_tail_budget) >
        1e95 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y thirty-fourth-order post-U successor coefficient rows must certify h34, source coefficient containment, Q_G34/Q_D34 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-fourth-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
