...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirtieth-order-post-u-successor-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_FIRST_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-first-order-post-u-successor-coefficient-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_thirty_first_order_post_u_successor_coefficient_certificate";
const PROMOTION_STATUS = "priority-only";
const NO_SPEED_WINDOW =
  "none; uses the certified positive speed-ratio zero enclosure only";
const SOURCE_COEFFICIENT = -1;
const SERIES_ORDER = 35;
const H_COUNT = 32;
const PREDECESSOR_H_COUNT = 31;
const TARGET_INDEX = 31;
const FIRST_Y_CELL_UPPER = 0.115 / 64;
const SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-thirty-first-order-post-U-successor-coefficient-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-thirty-first-order-post-U-successor-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y35";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y35";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y35";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y35";
...
function solveH31Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h30_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h30_interval: predecessorBranchRow.h30_interval,
      h31_interval: root.formatInterval(solve.hInterval),
      h31_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h31_solve_slope_sign: slope.sign,
      h31_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h31_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y33: gSeries
        .slice(0, 34)
  ...
  const qGRemainingThirtySecondOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 32;
  const qDRemainingThirtySecondOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 32;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y33: pairG.slice(0, 34).map(root.formatInterval),
    D_pair_coefficients_y0_to_y33: pairD.slice(0, 34).map(root.formatInterval),
    ...
    Q_G_y31_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[31]),
    Q_D_y31_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[31]),
    Q_G_remaining_thirty_second_order_tail_budget: root.formatSmallNumber(
      qGRemainingThirtySecondOrderTailBudget
    ),
    Q_D_remaining_thirty_second_order_tail_budget: root.formatSmallNumber(
      qDRemainingThirtySecondOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingThirtySecondOrderTailBudget > 6e75 &&
    qDRemainingThirtySecondOrderTailBudget > 6e75
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-thirty-first-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h30_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y35_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_32Q_G_y31_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_32Q_G_y31_coefficient_interval")
    ),
    ...
    h31_interval_hull: coefficientHull(
      branchRows.map((row) => row.h31_interval.map(Number))
    ),
    Q_G_y31_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y31_coefficient_interval.map(Number))
    ),
    Q_D_y31_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y31_coefficient_interval.map(Number))
    ),
    max_abs_h31_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h31_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y31_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 31, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y31_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 31, "max_abs_coefficient"))
    ),
    max_Q_G_y31_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y31_loss_on_first_y_cell")
    ),
    max_Q_D_y31_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y31_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_thirty_second_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_thirty_second_order_tail_budget")
    ),
    min_Q_D_remaining_thirty_second_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_thirty_second_order_tail_budget")
    ),
    inherited_h30_interval_hull:
      predecessorArtifact.thirtieth_order_post_u_successor_coefficient_summary
        .h30_interval_hull,
    inherited_Q_G_y30_coefficient_interval_hull:
      predecessorArtifact.thirtieth_order_post_u_successor_coefficient_summary
        .Q_G_y30_coefficient_interval_hull,
    inherited_Q_D_y30_coefficient_interval_hull:
      predecessorArtifact.thirtieth_order_post_u_successor_coefficient_summary
        .Q_D_y30_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-thirty-first-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyFirstOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtiethOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.thirtieth_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_FIRST_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    promotion_status: PROMOTION_STATUS,
    predecessor_packets: [
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirtieth-order-post-u-successor-coefficient-certificate.md",
    ],
    priority_packet:
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-first-order-post-u-successor-coefficient-certificate.md",
    thirty_first_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h30_artifact:
        "thirtieth-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h30_epsilon*y^33+h31_epsilon*y^34+O(y^35)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h31 after the h30 predecessor row",
      intervalized_quantity:
        "thirty-first-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^32) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_35(F_epsilon(y,h_{<=30}+y^31*X31,nu))=C_{31,epsilon}(nu)+S_{31,epsilon}(nu)*X31+O(y), with X31(0,nu)=h31_epsilon(nu)",
    },
    thirty_first_order_post_u_successor_coefficient_rows: rows,
    thirty_first_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
        row: "theta3minus.fold-pair-first-y-GD-thirtieth-order-post-U-successor-coefficient",
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-thirty-second-order-tail-bound",
        status: "directed-rounded-open",
      },
    ],
    artifact_claim: {
      assumes_fixed_speed_window: false,
      inherits_directed_rounded_first_y_GD_thirtieth_order_post_u_successor_coefficient_enclosure:
        true,
      certifies_directed_rounded_first_y_GD_thirty_first_order_post_u_successor_coefficient_enclosure:
        passed,
      ...
      claim_level:
        "Directed-rounded interval certificate for h31 and the induced thirty-first-order first-y quotient coefficients Q_G31 and Q_D31 on the fold-pair h-root graph. It turns the thirty-first-order quotient-tail constant term into a coefficient row and sharpens the zero-touching quotient burden to the thirty-second-order successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-thirty-second-order-successor-tail-bound-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h31, Q_G31, and Q_D31 are interval-certified with the preserved Q_D31+32Q_G31 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyFirstOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.thirty_first_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_FIRST_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D thirty-first-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D thirty-first-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.thirty_first_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y thirty-first-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.thirty_first_order_post_u_successor_coefficient_parameters
      ...
      artifact?.thirty_first_order_post_u_successor_coefficient_parameters
        ?.speed_max === undefined,
    "first-y thirty-first-order post-U successor coefficient parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h30_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y35_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y35_interval) <
        1.8e19 &&
      Number.isFinite(Number(summary?.max_abs_h31_interval)) &&
      Number(summary?.max_abs_h31_interval) < 6.9e18 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y31_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y31_coefficient_interval) < 5.9e18 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y31_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y31_coefficient_interval) < 1.9e20 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_32Q_G_y31_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_32Q_G_y31_coefficient_interval) <
        3.8e20 &&
      Number(summary?.min_Q_G_remaining_thirty_second_order_tail_budget) >
        6e86 &&
      Number(summary?.min_Q_D_remaining_thirty_second_order_tail_budget) >
        6e86 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y thirty-first-order post-U successor coefficient rows must certify h31, source coefficient containment, Q_G31/Q_D31 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-first-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
