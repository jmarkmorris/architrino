...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_NINETEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-nineteenth-order-jet-coefficient-interval-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_nineteenth_order_jet_coefficient_interval_certificate";
...
const NINETEENTH_ORDER_SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-nineteenth-order-jet-coefficient-interval-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-nineteenth-order-jet-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y23";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y23";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y23";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y23";
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y21: gSeries
        .slice(0, 22)
  ...
  const qG = Array.from({ length: 20 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 20 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y21: pairG.slice(0, 22).map(root.formatInterval),
    D_pair_coefficients_y0_to_y21: pairD.slice(0, 22).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qGRemainingNineteenthOrderTailBudget > 1e45 &&
    qDRemainingNineteenthOrderTailBudget > 1e45 &&
    qGRemainingNineteenthOrderTailBudget > 1e48 &&
    qDRemainingNineteenthOrderTailBudget > 1e48 &&
    ...
    qDRemainingTwentiethOrderTailBudget > 1e53
      ...
      : "first-y-GD-nineteenth-order-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y23_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-nineteenth-order-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdNineteenthOrderJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? NINETEENTH_ORDER_SPEED_CELL_COUNT
  );
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_NINETEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    ...
    nineteenth_order_jet_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h19; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, eighteenth-order, and nineteenth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "nineteenth-order coefficient interval only; the O(y^20) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    nineteenth_order_jet_rows: rows,
    nineteenth_order_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-twentieth-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-twentieth-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, eighteenth-order, and nineteenth-order first-y quotient coefficients Q_G0 through Q_G19 and Q_D0 through Q_D19 on the fold-pair h-root graph. It closes the nineteenth-order coefficient row only; the finite twentieth-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-twentieth-order-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite nineteenth-order-tail problem to a finite twentieth-order-tail problem after the certified nineteenth-order coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^20) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdNineteenthOrderJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.nineteenth_order_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_NINETEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D nineteenth-order jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D nineteenth-order jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.nineteenth_order_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y nineteenth-order jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.nineteenth_order_jet_parameters?.speed_band === undefined &&
      artifact?.nineteenth_order_jet_parameters?.speed_window === undefined &&
      artifact?.nineteenth_order_jet_parameters?.speed_min === undefined &&
      artifact?.nineteenth_order_jet_parameters?.speed_max === undefined,
    "first-y nineteenth-order jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === NINETEENTH_ORDER_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * NINETEENTH_ORDER_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y23 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y23_interval) <
        1e11 &&
    ...
    "first-y nineteenth-order jet rows must certify h solves, source coefficient containment, nineteenth-order coefficients, and positive post-twentieth-order tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-nineteenth-order-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
...
function main() {
  ...
  if (options.validate) {
    ...
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdNineteenthOrderJetCoefficientIntervalCertificate(
        artifact
      );
  ...
  try {
    const artifact =
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdNineteenthOrderJetCoefficientIntervalCertificate(
        options
      );
