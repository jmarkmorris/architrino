...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_CUBIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-cubic-jet-coefficient-interval-certificate/v1";
...
const CUBIC_SPEED_CELL_COUNT = 128;
...
function branchSeriesCoordinates({
  ...
  phi[6] = root.scaleInterval(h3Interval, -1);
...
function solveH3Interval({
  ...
  const slope = root.subtractIntervals(oneSeries[7], zeroSeries[7]);
  return {
    h3Interval: root.divideIntervals(root.scaleInterval(zeroSeries[7], -1), slope),
    h3SlopeInterval: slope,
    h3ResidualBeforeSolve: zeroSeries[7],
  };
}
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const h3Slope = root.intervalSignAndClearance(h3Solve.h3SlopeInterval);
    return {
      branch,
      h0Interval: h0Solve.h0Interval,
      h1Interval: h1Solve.h1Interval,
      ...
      row: {
        ...
        h3_interval: root.formatInterval(h3Solve.h3Interval),
        ...
        h3_solve_slope_interval: root.formatInterval(h3Solve.h3SlopeInterval),
        h3_solve_slope_sign: h3Slope.sign,
        h3_solve_slope_clearance: root.formatSmallNumber(h3Slope.clearance),
        h3_residual_before_solve: root.formatInterval(
          h3Solve.h3ResidualBeforeSolve
        ),
        source_equation_coefficients_y0_to_y7:
          sourceSeries.map(root.formatInterval),
        source_equation_coefficients_contain_zero_y0_to_y7:
          sourceSeries.every(containsZero),
        max_abs_source_equation_coeff_y0_to_y7: root.formatSmallNumber(
          Math.max(...sourceSeries.map(maxAbsInterval))
        ),
        G_branch_coefficients_y0_to_y5: gSeries
          .slice(0, 6)
  ...
  const gMinus = branchGSeries({
    ...
    h3Interval: branchByName["-"].h3Interval,
  });
  ...
  const qG3Abs = maxAbsInterval(qG3Interval);
  const qD3Abs = maxAbsInterval(qD3Interval);
  ...
  const qGCubicLoss = qG3Abs * FIRST_Y_CELL_UPPER ** 3;
  const qDCubicLoss = qD3Abs * FIRST_Y_CELL_UPPER ** 3;
  const qGRemainingMargin =
    qG0Sign.clearance - qGLinearLoss - qGQuadraticLoss - qGCubicLoss;
  const qDRemainingMargin =
    qD0Sign.clearance - qDLinearLoss - qDQuadraticLoss - qDCubicLoss;
  ...
  const qDPlus4Qg3Interval = root.addIntervals(
    qD3Interval,
    root.scaleInterval(qG3Interval, 4)
  );
  return {
    ...
    G_pair_coefficients_y0_to_y5: pairG.slice(0, 6).map(root.formatInterval),
    D_pair_coefficients_y0_to_y5: pairD.slice(0, 6).map(root.formatInterval),
    ...
    Q_G_y3_coefficient_interval: root.formatInterval(qG3Interval),
    ...
    Q_G_y3_max_abs_coefficient: root.formatSmallNumber(qG3Abs),
    ...
    Q_G_cubic_loss_on_first_y_cell: root.formatSmallNumber(qGCubicLoss),
    ...
    Q_D_y3_coefficient_interval: root.formatInterval(qD3Interval),
    ...
    Q_D_y3_max_abs_coefficient: root.formatSmallNumber(qD3Abs),
    ...
    Q_D_cubic_loss_on_first_y_cell: root.formatSmallNumber(qDCubicLoss),
    ...
    Q_D_plus_4Q_G_y3_coefficient_interval:
      root.formatInterval(qDPlus4Qg3Interval),
    row_status:
      branchRows.every((row) => row.h0_interval_inside_predecessor_tube) &&
      branchRows.every(
        (row) => row.source_equation_coefficients_contain_zero_y0_to_y7
      ) &&
      branchRows.every(
        (row) =>
          ...
          Number(row.h3_solve_slope_clearance) > 0.5
      ) &&
      ...
      finiteInterval(qG3Interval) &&
      finiteInterval(qD3Interval) &&
      containsZero(qDPlus2Qg1Interval) &&
      containsZero(qDPlus3Qg2Interval) &&
      containsZero(qDPlus4Qg3Interval) &&
      ...
      qDRemainingQuarticTailBudget > 1e9
        ? "directed-rounded-first-y-GD-cubic-jet-coefficient-enclosed"
        : "first-y-GD-cubic-jet-coefficient-enclosure-open",
  };
}
...
function summarizeRows(rows) {
  ...
  const maxQg3Abs = Math.max(
    ...rows.map((row) => Number(row.Q_G_y3_max_abs_coefficient))
  );
  const maxQd3Abs = Math.max(
    ...rows.map((row) => Number(row.Q_D_y3_max_abs_coefficient))
  );
  ...
  const minQgQuarticTailBudget = Math.min(
    ...rows.map((row) => Number(row.Q_G_remaining_quartic_tail_budget))
  );
  const minQdQuarticTailBudget = Math.min(
    ...rows.map((row) => Number(row.Q_D_remaining_quartic_tail_budget))
  );
  return {
    ...
    min_h3_solve_slope_clearance: root.formatSmallNumber(
      Math.min(...branchRows.map((row) => Number(row.h3_solve_slope_clearance)))
    ),
    all_source_equation_coefficients_contain_zero_y0_to_y7: branchRows.every(
      (row) => row.source_equation_coefficients_contain_zero_y0_to_y7
    ),
    max_abs_source_equation_coeff_y0_to_y7_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) =>
          Number(row.max_abs_source_equation_coeff_y0_to_y7)
        )
      )
    ),
    ...
    h3_interval_hull: coefficientHull(
      branchRows.map((row) => row.h3_interval.map(Number))
    ),
    max_abs_h3_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h3_interval.map(Number)))
      )
    ),
    ...
    Q_G_y3_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y3_coefficient_interval.map(Number))
    ),
    ...
    Q_D_y3_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y3_coefficient_interval.map(Number))
    ),
    ...
    max_abs_Q_G_y3_coefficient_interval: root.formatSmallNumber(maxQg3Abs),
    max_abs_Q_D_y3_coefficient_interval: root.formatSmallNumber(maxQd3Abs),
    ...
    max_Q_G_cubic_loss_on_first_y_cell: root.formatSmallNumber(
      maxQg3Abs * FIRST_Y_CELL_UPPER ** 3
    ),
    max_Q_D_cubic_loss_on_first_y_cell: root.formatSmallNumber(
      maxQd3Abs * FIRST_Y_CELL_UPPER ** 3
    ),
    ...
    min_Q_G_remaining_quartic_tail_budget:
      root.formatSmallNumber(minQgQuarticTailBudget),
    min_Q_D_remaining_quartic_tail_budget:
      root.formatSmallNumber(minQdQuarticTailBudget),
    ...
    max_abs_Q_D_plus_4Q_G_y3_coefficient_interval: root.formatSmallNumber(
      Math.max(
        ...rows.map((row) =>
          maxAbsInterval(row.Q_D_plus_4Q_G_y3_coefficient_interval.map(Number))
        )
      )
    ),
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-cubic-jet-coefficient-interval-open",
  };
}

export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdCubicJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? CUBIC_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    cubic_jet_parameters: {
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+O(y^7)",
      intervalized_quantity:
        "constant, linear, quadratic, and cubic coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "cubic coefficient interval only; the O(y^4) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    cubic_jet_rows: rows,
    cubic_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-quartic-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-quartic-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, and cubic first-y quotient coefficients Q_G0, Q_G1, Q_G2, Q_G3, Q_D0, Q_D1, Q_D2, and Q_D3 on the fold-pair h-root graph. It closes the cubic coefficient row only; the finite quartic quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-quartic-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite O(y^3) tail problem to a finite quartic-tail problem after the certified cubic coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^4) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdCubicJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_CUBIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D cubic jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D cubic jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.cubic_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y cubic jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.cubic_jet_parameters?.speed_band === undefined &&
      artifact?.cubic_jet_parameters?.speed_window === undefined &&
      artifact?.cubic_jet_parameters?.speed_min === undefined &&
      artifact?.cubic_jet_parameters?.speed_max === undefined,
    "first-y cubic jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
      artifact?.cubic_jet_summary?.status === CERTIFIED_STATUS &&
      artifact?.cubic_jet_summary?.speed_cell_count === CUBIC_SPEED_CELL_COUNT &&
      artifact?.cubic_jet_summary?.branch_cell_count ===
        2 * CUBIC_SPEED_CELL_COUNT &&
      artifact?.cubic_jet_summary?.all_rows_certified === true &&
      artifact?.cubic_jet_summary?.all_h0_intervals_inside_predecessor_tubes ===
        true &&
      Number(artifact?.cubic_jet_summary?.min_h0_solve_slope_clearance) >
        0.79 &&
      Number(artifact?.cubic_jet_summary?.min_h1_solve_slope_clearance) >
        0.79 &&
      Number(artifact?.cubic_jet_summary?.min_h2_solve_slope_clearance) >
        0.73 &&
      Number(artifact?.cubic_jet_summary?.min_h3_solve_slope_clearance) >
        0.5 &&
      artifact?.cubic_jet_summary
        ?.all_source_equation_coefficients_contain_zero_y0_to_y7 === true &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_source_equation_coeff_y0_to_y7_interval
      ) < 15 &&
      Number(artifact?.cubic_jet_summary?.max_abs_h1_interval) < 6 &&
      Number(artifact?.cubic_jet_summary?.max_abs_h2_interval) < 4 &&
      Number(artifact?.cubic_jet_summary?.max_abs_h3_interval) < 32 &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_Q_G_y1_coefficient_interval
      ) < 0.0034 &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_Q_D_y1_coefficient_interval
      ) < 0.0068 &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_Q_G_y2_coefficient_interval
      ) < 1 &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_Q_D_y2_coefficient_interval
      ) < 3 &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_Q_G_y3_coefficient_interval
      ) < 2.4 &&
      Number(
        artifact?.cubic_jet_summary?.max_abs_Q_D_y3_coefficient_interval
      ) < 9.4 &&
      Number(
        artifact?.cubic_jet_summary?.min_Q_G_remaining_first_order_tail_budget
      ) > 47 &&
      Number(
        artifact?.cubic_jet_summary?.min_Q_D_remaining_first_order_tail_budget
      ) > 47 &&
      Number(
        artifact?.cubic_jet_summary?.min_Q_G_remaining_cubic_tail_budget
      ) > 1e7 &&
      Number(
        artifact?.cubic_jet_summary?.min_Q_D_remaining_cubic_tail_budget
      ) > 1e7 &&
      Number(
        artifact?.cubic_jet_summary?.min_Q_G_remaining_quartic_tail_budget
      ) > 8e9 &&
      Number(
        artifact?.cubic_jet_summary?.min_Q_D_remaining_quartic_tail_budget
      ) > 8e9 &&
      artifact?.cubic_jet_summary
        ?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y cubic jet rows must certify h solves, source coefficient containment, cubic coefficients, and positive remainder budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-cubic-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
