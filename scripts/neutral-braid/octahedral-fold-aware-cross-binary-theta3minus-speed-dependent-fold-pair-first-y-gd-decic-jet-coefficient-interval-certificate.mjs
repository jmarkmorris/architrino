...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_DECIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-decic-jet-coefficient-interval-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_decic_jet_coefficient_interval_certificate";
...
const DECIC_SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-decic-jet-coefficient-interval-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-decic-jet-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y14";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y14";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y14";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y14";
...
function sinSeries(series) {
  ...
  const { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13, n14 } =
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y12: gSeries
        .slice(0, 13)
  ...
  const qG = Array.from({ length: 11 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 11 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y12: pairG.slice(0, 13).map(root.formatInterval),
    D_pair_coefficients_y0_to_y12: pairD.slice(0, 13).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qDRemainingEleventhOrderTailBudget > 1e28
      ...
      : "first-y-GD-decic-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y14_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-decic-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdDecicJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? DECIC_SPEED_CELL_COUNT
  );
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_DECIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    ...
    decic_jet_parameters: {
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+O(y^14)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h10; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, and decic coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "decic coefficient interval only; the O(y^11) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    decic_jet_rows: rows,
    decic_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-eleventh-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-eleventh-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, and decic first-y quotient coefficients Q_G0 through Q_G10 and Q_D0 through Q_D10 on the fold-pair h-root graph. It closes the decic coefficient row only; the finite eleventh-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-eleventh-order-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite tenth-order-tail problem to a finite eleventh-order-tail problem after the certified decic coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^11) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdDecicJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.decic_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_DECIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D decic jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D decic jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.decic_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y decic jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.decic_jet_parameters?.speed_band === undefined &&
      artifact?.decic_jet_parameters?.speed_window === undefined &&
      artifact?.decic_jet_parameters?.speed_min === undefined &&
      artifact?.decic_jet_parameters?.speed_max === undefined,
    "first-y decic jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === DECIC_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * DECIC_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y14 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y14_interval) <
        900 &&
    ...
    "first-y decic jet rows must certify h solves, source coefficient containment, decic coefficients, and positive post-decic tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-decic-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
...
function main() {
  ...
  if (options.validate) {
    ...
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdDecicJetCoefficientIntervalCertificate(
        artifact
      );
  ...
  try {
    const artifact =
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdDecicJetCoefficientIntervalCertificate(
        options
      );
