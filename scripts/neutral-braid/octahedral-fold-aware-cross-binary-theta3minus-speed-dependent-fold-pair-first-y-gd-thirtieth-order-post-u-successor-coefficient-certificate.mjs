...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-ninth-order-post-u-successor-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTIETH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirtieth-order-post-u-successor-coefficient-certificate/v1";
...
const SERIES_ORDER = 34;
const H_COUNT = 31;
const PREDECESSOR_H_COUNT = 30;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y34";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y34";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y34";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y34";
...
function solveH30Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h29_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h29_interval: predecessorBranchRow.h29_interval,
      h30_interval: root.formatInterval(solve.hInterval),
      h30_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h30_solve_slope_sign: slope.sign,
      h30_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h30_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y32: gSeries
        .slice(0, 33)
  ...
  const qGRemainingThirtyFirstOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 31;
  const qDRemainingThirtyFirstOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 31;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y32: pairG.slice(0, 33).map(root.formatInterval),
    D_pair_coefficients_y0_to_y32: pairD.slice(0, 33).map(root.formatInterval),
    ...
    Q_G_y30_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[30]),
    Q_D_y30_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[30]),
    Q_G_remaining_thirty_first_order_tail_budget: root.formatSmallNumber(
      qGRemainingThirtyFirstOrderTailBudget
    ),
    Q_D_remaining_thirty_first_order_tail_budget: root.formatSmallNumber(
      qDRemainingThirtyFirstOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingThirtyFirstOrderTailBudget > 6e75 &&
    qDRemainingThirtyFirstOrderTailBudget > 6e75
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-thirtieth-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h29_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y34_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_31Q_G_y30_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_31Q_G_y30_coefficient_interval")
    ),
    ...
    h30_interval_hull: coefficientHull(
      branchRows.map((row) => row.h30_interval.map(Number))
    ),
    Q_G_y30_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y30_coefficient_interval.map(Number))
    ),
    Q_D_y30_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y30_coefficient_interval.map(Number))
    ),
    max_abs_h30_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h30_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y30_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 30, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y30_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 30, "max_abs_coefficient"))
    ),
    max_Q_G_y30_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y30_loss_on_first_y_cell")
    ),
    max_Q_D_y30_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y30_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_thirty_first_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_thirty_first_order_tail_budget")
    ),
    min_Q_D_remaining_thirty_first_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_thirty_first_order_tail_budget")
    ),
    inherited_h29_interval_hull:
      predecessorArtifact.twenty_ninth_order_post_u_successor_coefficient_summary
        .h29_interval_hull,
    inherited_Q_G_y29_coefficient_interval_hull:
      predecessorArtifact.twenty_ninth_order_post_u_successor_coefficient_summary
        .Q_G_y29_coefficient_interval_hull,
    inherited_Q_D_y29_coefficient_interval_hull:
      predecessorArtifact.twenty_ninth_order_post_u_successor_coefficient_summary
        .Q_D_y29_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-thirtieth-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtiethOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyNinthOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.twenty_ninth_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    ...
    predecessor_packets: [
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-ninth-order-post-u-successor-coefficient-certificate.md",
    ],
    priority_packet:
    ...
    thirtieth_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h29_artifact:
        "twenty-ninth-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h29_epsilon*y^32+h30_epsilon*y^33+O(y^34)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h30 after the h29 predecessor row",
      intervalized_quantity:
        "thirtieth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^31) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_34(F_epsilon(y,h_{<=29}+y^30*X,nu))=C_{30,epsilon}(nu)+S_{30,epsilon}(nu)*X+O(y), with X(0,nu)=h30_epsilon(nu)",
    },
    thirtieth_order_post_u_successor_coefficient_rows: rows,
    thirtieth_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-thirty-first-order-tail-bound",
    ...
    artifact_claim: {
      ...
      inherits_directed_rounded_first_y_GD_twenty_ninth_order_post_u_successor_coefficient_enclosure:
        true,
      ...
      claim_level:
        "Directed-rounded interval certificate for h30 and the induced thirtieth-order first-y quotient coefficients Q_G30 and Q_D30 on the fold-pair h-root graph. It turns the thirtieth-order quotient-tail constant term into a coefficient row and sharpens the zero-touching quotient burden to the thirty-first-order successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h30, Q_G30, and Q_D30 are interval-certified with the preserved Q_D30+31Q_G30 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtiethOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.thirtieth_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTIETH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D thirtieth-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D thirtieth-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.thirtieth_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y thirtieth-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.thirtieth_order_post_u_successor_coefficient_parameters
  ...
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h29_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y34_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y34_interval) <
        3.1e18 &&
      Number.isFinite(Number(summary?.max_abs_h30_interval)) &&
      Number(summary?.max_abs_h30_interval) < 1.2e18 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y30_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y30_coefficient_interval) < 1e18 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y30_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y30_coefficient_interval) < 3.1e19 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_31Q_G_y30_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_31Q_G_y30_coefficient_interval) <
        6.2e19 &&
    ...
    "first-y thirtieth-order post-U successor coefficient rows must certify h30, source coefficient containment, Q_G30/Q_D30 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirtieth-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
