...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SEPTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-septic-jet-coefficient-interval-certificate/v1";
...
const SEPTIC_SPEED_CELL_COUNT = 128;
...
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-septic-jet-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y11";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y11";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y11";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y11";
const H_SLOPE_MINIMUMS = [0.79, 0.79, 0.79, 0.79, 0.79, 0.79, 0.79, 0.79];
...
function nilpotentPowers(series) {
  ...
  return { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11 };
}

function sinSeries(series) {
  const center = series[0];
  const { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11 } =
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y9: gSeries
        .slice(0, 10)
  ...
  const qG = Array.from({ length: 8 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 8 }, (_, index) => pairD[index + 2]);
  ...
  const qGRemainingEighthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 8;
  const qDRemainingEighthOrderTailBudget =
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y9: pairG.slice(0, 10).map(root.formatInterval),
    D_pair_coefficients_y0_to_y9: pairD.slice(0, 10).map(root.formatInterval),
    ...
    Q_G_remaining_eighth_order_tail_budget: root.formatSmallNumber(
      qGRemainingEighthOrderTailBudget
    ),
    Q_D_remaining_eighth_order_tail_budget: root.formatSmallNumber(
      qDRemainingEighthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingEighthOrderTailBudget > 1e20 &&
    qDRemainingEighthOrderTailBudget > 1e20
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-septic-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y11_interval: root.formatSmallNumber(
    ...
    min_Q_G_remaining_eighth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_eighth_order_tail_budget")
    ),
    min_Q_D_remaining_eighth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_eighth_order_tail_budget")
    ),
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-septic-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSepticJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? SEPTIC_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    septic_jet_parameters: {
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+O(y^11)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h7; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, and septic coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "septic coefficient interval only; the O(y^8) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    septic_jet_rows: rows,
    septic_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-eighth-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-eighth-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, and septic first-y quotient coefficients Q_G0 through Q_G7 and Q_D0 through Q_D7 on the fold-pair h-root graph. It closes the septic coefficient row only; the finite eighth-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-eighth-order-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite septic-tail problem to a finite eighth-order-tail problem after the certified septic coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^8) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSepticJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.septic_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SEPTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D septic jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D septic jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.septic_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y septic jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.septic_jet_parameters?.speed_band === undefined &&
      artifact?.septic_jet_parameters?.speed_window === undefined &&
      artifact?.septic_jet_parameters?.speed_min === undefined &&
      artifact?.septic_jet_parameters?.speed_max === undefined,
    "first-y septic jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === SEPTIC_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * SEPTIC_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y11 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y11_interval) <
        4.3 &&
      ...
      Number(summary?.min_Q_G_remaining_septic_tail_budget) > 1e15 &&
      Number(summary?.min_Q_D_remaining_septic_tail_budget) > 1e15 &&
      Number(summary?.min_Q_G_remaining_septic_tail_budget) > 1e18 &&
      Number(summary?.min_Q_D_remaining_septic_tail_budget) > 1e18 &&
      Number(summary?.min_Q_G_remaining_eighth_order_tail_budget) > 7e20 &&
      Number(summary?.min_Q_D_remaining_eighth_order_tail_budget) > 7e20 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y septic jet rows must certify h solves, source coefficient containment, septic coefficients, and positive post-septic tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-septic-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
