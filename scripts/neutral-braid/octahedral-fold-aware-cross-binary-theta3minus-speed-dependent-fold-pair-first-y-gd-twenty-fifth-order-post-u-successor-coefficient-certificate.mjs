...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-fourth-order-post-u-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_FIFTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-fifth-order-post-u-successor-coefficient-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_twenty_fifth_order_post_u_successor_coefficient_certificate";
const PROMOTION_STATUS = "priority-only";
const NO_SPEED_WINDOW =
  "none; uses the historical positive speed-ratio zero-enclosure diagnostic; receiver-normal restart required only";
const SOURCE_COEFFICIENT = -1;
const SERIES_ORDER = 29;
const H_COUNT = 26;
const PREDECESSOR_H_COUNT = 25;
const TARGET_INDEX = 25;
const FIRST_Y_CELL_UPPER = 0.115 / 64;
const SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-twenty-fifth-order-post-U-successor-coefficient-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-twenty-fifth-order-post-U-successor-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y29";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y29";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y29";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y29";
...
function solveH25Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h24_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h24_interval: predecessorBranchRow.h24_interval,
      h25_interval: root.formatInterval(solve.hInterval),
      h25_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h25_solve_slope_sign: slope.sign,
      h25_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h25_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y27: gSeries
        .slice(0, 28)
  ...
  const qGRemainingTwentySixthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 26;
  const qDRemainingTwentySixthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 26;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y27: pairG.slice(0, 28).map(root.formatInterval),
    D_pair_coefficients_y0_to_y27: pairD.slice(0, 28).map(root.formatInterval),
    ...
    Q_G_y25_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[25]),
    Q_D_y25_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[25]),
    Q_G_remaining_twenty_sixth_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentySixthOrderTailBudget
    ),
    Q_D_remaining_twenty_sixth_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentySixthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingTwentySixthOrderTailBudget > 1e70 &&
    qDRemainingTwentySixthOrderTailBudget > 1e70
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-twenty-fifth-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h24_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y29_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_26Q_G_y25_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_26Q_G_y25_coefficient_interval")
    ),
    ...
    h25_interval_hull: coefficientHull(
      branchRows.map((row) => row.h25_interval.map(Number))
    ),
    Q_G_y25_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y25_coefficient_interval.map(Number))
    ),
    Q_D_y25_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y25_coefficient_interval.map(Number))
    ),
    max_abs_h25_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h25_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y25_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 25, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y25_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 25, "max_abs_coefficient"))
    ),
    max_Q_G_y25_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y25_loss_on_first_y_cell")
    ),
    max_Q_D_y25_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y25_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_twenty_sixth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_sixth_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_sixth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_sixth_order_tail_budget")
    ),
    inherited_h24_interval_hull:
      predecessorArtifact.twenty_fourth_order_post_u_coefficient_summary
        .h24_interval_hull,
    inherited_Q_G_y24_coefficient_interval_hull:
      predecessorArtifact.twenty_fourth_order_post_u_coefficient_summary
        .Q_G_y24_coefficient_interval_hull,
    inherited_Q_D_y24_coefficient_interval_hull:
      predecessorArtifact.twenty_fourth_order_post_u_coefficient_summary
        .Q_D_y24_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-twenty-fifth-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyFifthOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyFourthOrderPostUCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.twenty_fourth_order_post_u_coefficient_rows.map(
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_FIFTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    promotion_status: PROMOTION_STATUS,
    predecessor_packets: [
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-fourth-order-post-u-coefficient-certificate.md",
    ],
    priority_packet:
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-fifth-order-post-u-successor-coefficient-certificate.md",
    twenty_fifth_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h24_artifact:
        "twenty-fourth-order post-U coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h24_epsilon*y^27+h25_epsilon*y^28+O(y^29)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h25 after the h24 predecessor row",
      intervalized_quantity:
        "twenty-fifth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor finite root-tail tube and O(y^26) quotient tail remain open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_29(F_epsilon(y,h_{<=24}+y^25*Z,nu))=C_{25,epsilon}(nu)+S_{25,epsilon}(nu)*Z+O(y), with Z(0,nu)=h25_epsilon(nu)",
    },
    twenty_fifth_order_post_u_successor_coefficient_rows: rows,
    twenty_fifth_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
    ...
    artifact_claim: {
      ...
      inherits_directed_rounded_first_y_GD_twenty_fourth_order_post_u_coefficient_enclosure:
        true,
      certifies_directed_rounded_first_y_GD_twenty_fifth_order_post_u_successor_coefficient_enclosure:
        passed,
      ...
      claim_level:
        "Directed-rounded interval certificate for h25 and the induced twenty-fifth-order first-y quotient coefficients Q_G25 and Q_D25 on the fold-pair h-root graph. It sharpens the zero-touching quotient burden to the O(y^26) successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-finite-shift29-successor-root-tail-tube-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h25, Q_G25, and Q_D25 are interval-certified with the preserved Q_D25+26Q_G25 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyFifthOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.twenty_fifth_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_FIFTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-fifth-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-fifth-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_fifth_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-fifth-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_fifth_order_post_u_successor_coefficient_parameters
      ...
      artifact?.twenty_fifth_order_post_u_successor_coefficient_parameters
        ?.speed_max === undefined,
    "first-y twenty-fifth-order post-U successor coefficient parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h24_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y29_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y29_interval) <
        5e17 &&
      Number.isFinite(Number(summary?.max_abs_h25_interval)) &&
      Number(summary?.max_abs_h25_interval) < 2e17 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y25_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y25_coefficient_interval) < 2e17 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y25_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y25_coefficient_interval) < 5e18 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_26Q_G_y25_coefficient_interval)
      ) &&
      Number(summary?.min_Q_G_remaining_twenty_sixth_order_tail_budget) >
        1e70 &&
      Number(summary?.min_Q_D_remaining_twenty_sixth_order_tail_budget) >
        1e70 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y twenty-fifth-order post-U successor coefficient rows must certify h25, source coefficient containment, Q_G25/Q_D25 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-fifth-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
