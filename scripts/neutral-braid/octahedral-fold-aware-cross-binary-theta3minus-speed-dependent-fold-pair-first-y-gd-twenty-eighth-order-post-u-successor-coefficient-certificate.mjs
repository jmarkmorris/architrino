...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-seventh-order-post-u-successor-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_EIGHTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-eighth-order-post-u-successor-coefficient-certificate/v1";
...
const SERIES_ORDER = 32;
const H_COUNT = 29;
const PREDECESSOR_H_COUNT = 28;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y32";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y32";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y32";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y32";
...
function solveH28Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h27_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h27_interval: predecessorBranchRow.h27_interval,
      h28_interval: root.formatInterval(solve.hInterval),
      h28_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h28_solve_slope_sign: slope.sign,
      h28_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h28_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y30: gSeries
        .slice(0, 31)
  ...
  const qGRemainingTwentyNinthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 29;
  const qDRemainingTwentyNinthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 29;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y30: pairG.slice(0, 31).map(root.formatInterval),
    D_pair_coefficients_y0_to_y30: pairD.slice(0, 31).map(root.formatInterval),
    ...
    Q_G_y28_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[28]),
    Q_D_y28_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[28]),
    Q_G_remaining_twenty_ninth_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentyNinthOrderTailBudget
    ),
    Q_D_remaining_twenty_ninth_order_tail_budget: root.formatSmallNumber(
  ...
  row.row_status =
    ...
    qDRemainingTwentyNinthOrderTailBudget > 6e75
      ...
      : "first-y-GD-twenty-eighth-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h27_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y32_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_29Q_G_y28_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_29Q_G_y28_coefficient_interval")
    ),
    ...
    h28_interval_hull: coefficientHull(
      branchRows.map((row) => row.h28_interval.map(Number))
    ),
    Q_G_y28_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y28_coefficient_interval.map(Number))
    ),
    Q_D_y28_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y28_coefficient_interval.map(Number))
    ),
    max_abs_h28_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h28_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y28_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 28, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y28_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 28, "max_abs_coefficient"))
    ),
    max_Q_G_y28_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y28_loss_on_first_y_cell")
    ),
    max_Q_D_y28_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y28_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_twenty_ninth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_ninth_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_ninth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_ninth_order_tail_budget")
    ),
    inherited_h27_interval_hull:
      predecessorArtifact.twenty_seventh_order_post_u_successor_coefficient_summary
        .h27_interval_hull,
    inherited_Q_G_y27_coefficient_interval_hull:
      predecessorArtifact.twenty_seventh_order_post_u_successor_coefficient_summary
        .Q_G_y27_coefficient_interval_hull,
    inherited_Q_D_y27_coefficient_interval_hull:
      predecessorArtifact.twenty_seventh_order_post_u_successor_coefficient_summary
        .Q_D_y27_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-twenty-eighth-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyEighthOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySeventhOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.twenty_seventh_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    ...
    priority_packet:
      "reference/priorities/geometry-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-eighth-order-post-u-successor-coefficient-certificate.md",
    twenty_eighth_order_post_u_successor_coefficient_parameters: {
      ...
      predecessor_h27_artifact:
        "twenty-seventh-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h27_epsilon*y^30+h28_epsilon*y^31+O(y^32)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h28 after the h27 predecessor row",
      intervalized_quantity:
        "twenty-eighth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^29) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_32(F_epsilon(y,h_{<=27}+y^28*X,nu))=C_{28,epsilon}(nu)+S_{28,epsilon}(nu)*X+O(y), with X(0,nu)=h28_epsilon(nu)",
    },
    twenty_eighth_order_post_u_successor_coefficient_rows: rows,
    twenty_eighth_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-twenty-ninth-order-tail-bound",
        status: "directed-rounded-open",
      },
    ],
    artifact_claim: {
      assumes_fixed_speed_window: false,
      inherits_directed_rounded_first_y_GD_twenty_seventh_order_post_u_successor_coefficient_enclosure:
        true,
      ...
      claim_level:
        "Directed-rounded interval certificate for h28 and the induced twenty-eighth-order first-y quotient coefficients Q_G28 and Q_D28 on the fold-pair h-root graph. It turns the twenty-eighth-order quotient-tail constant term into a coefficient row and sharpens the zero-touching quotient burden to the O(y^29) successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h28, Q_G28, and Q_D28 are interval-certified with the preserved Q_D28+29Q_G28 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyEighthOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.twenty_eighth_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_EIGHTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-eighth-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-eighth-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_eighth_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-eighth-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_eighth_order_post_u_successor_coefficient_parameters
  ...
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h27_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y32_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y32_interval) <
        1e17 &&
      Number.isFinite(Number(summary?.max_abs_h28_interval)) &&
      Number(summary?.max_abs_h28_interval) < 3.4e16 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y28_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y28_coefficient_interval) < 2.8e16 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y28_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y28_coefficient_interval) < 8.1e17 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_29Q_G_y28_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_29Q_G_y28_coefficient_interval) <
        1.7e18 &&
    ...
    "first-y twenty-eighth-order post-U successor coefficient rows must certify h28, source coefficient containment, Q_G28/Q_D28 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-eighth-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
