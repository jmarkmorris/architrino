...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SEXTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sextic-jet-coefficient-interval-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_sextic_jet_coefficient_interval_certificate";
...
const SEXTIC_SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-sextic-jet-coefficient-interval-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-sextic-jet-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y10";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y10";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y10";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y10";
const H_SLOPE_MINIMUMS = [0.79, 0.79, 0.79, 0.79, 0.79, 0.79, 0.79];
...
function nilpotentPowers(series) {
  ...
  return { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10 };
}

function sinSeries(series) {
  const center = series[0];
  const { nilpotent, n2, n3, n4, n5, n6, n7, n8, n9, n10 } =
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y8: gSeries
        .slice(0, 9)
  ...
  const qG = Array.from({ length: 7 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 7 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y8: pairG.slice(0, 9).map(root.formatInterval),
    D_pair_coefficients_y0_to_y8: pairD.slice(0, 9).map(root.formatInterval),
  ...
  ["linear", "quadratic", "cubic", "quartic", "quintic", "sextic"].forEach(
    (label, labelIndex) => {
  ...
  row.row_status =
    ...
    qDRemainingSepticTailBudget > 1e18
      ...
      : "first-y-GD-sextic-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y10_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-sextic-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSexticJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? SEXTIC_SPEED_CELL_COUNT
  );
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SEXTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    ...
    sextic_jet_parameters: {
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+O(y^10)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h6; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, and sextic coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "sextic coefficient interval only; the O(y^7) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    sextic_jet_rows: rows,
    sextic_jet_summary: summary,
    closure_burndown: [
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, and sextic first-y quotient coefficients Q_G0 through Q_G6 and Q_D0 through Q_D6 on the fold-pair h-root graph. It closes the sextic coefficient row only; the finite septic quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-septic-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite sextic-tail problem to a finite septic-tail problem after the certified sextic coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^7) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSexticJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.sextic_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SEXTIC_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D sextic jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D sextic jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.sextic_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y sextic jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.sextic_jet_parameters?.speed_band === undefined &&
      artifact?.sextic_jet_parameters?.speed_window === undefined &&
      artifact?.sextic_jet_parameters?.speed_min === undefined &&
      artifact?.sextic_jet_parameters?.speed_max === undefined,
    "first-y sextic jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === SEXTIC_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * SEXTIC_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y10 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y10_interval) <
        0.8 &&
    ...
    "first-y sextic jet rows must certify h solves, source coefficient containment, sextic coefficients, and positive post-sextic tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sextic-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
...
function main() {
  ...
  if (options.validate) {
    ...
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSexticJetCoefficientIntervalCertificate(
        artifact
      );
  ...
  try {
    const artifact =
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSexticJetCoefficientIntervalCertificate(
        options
      );
