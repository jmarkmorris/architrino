...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_FOURTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-fourteenth-order-jet-coefficient-interval-certificate/v1";
...
const FOURTEENTH_ORDER_SPEED_CELL_COUNT = 128;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y18";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y18";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y18";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y18";
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y16: gSeries
        .slice(0, 17)
  ...
  const qG = Array.from({ length: 15 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 15 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y16: pairG.slice(0, 17).map(root.formatInterval),
    D_pair_coefficients_y0_to_y16: pairD.slice(0, 17).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qDRemainingFifteenthOrderTailBudget > 1e40
      ...
      : "first-y-GD-fourteenth-order-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y18_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-fourteenth-order-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdFourteenthOrderJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? FOURTEENTH_ORDER_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    fourteenth_order_jet_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+O(y^18)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h14; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, and fourteenth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "fourteenth-order coefficient interval only; the O(y^15) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    fourteenth_order_jet_rows: rows,
    fourteenth_order_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-fifteenth-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-fifteenth-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, and fourteenth-order first-y quotient coefficients Q_G0 through Q_G14 and Q_D0 through Q_D14 on the fold-pair h-root graph. It closes the fourteenth-order coefficient row only; the finite fifteenth-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite fourteenth-order-tail problem to a finite fifteenth-order-tail problem after the certified fourteenth-order coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^15) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdFourteenthOrderJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.fourteenth_order_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_FOURTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D fourteenth-order jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D fourteenth-order jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.fourteenth_order_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y fourteenth-order jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.fourteenth_order_jet_parameters?.speed_band === undefined &&
      artifact?.fourteenth_order_jet_parameters?.speed_window === undefined &&
      artifact?.fourteenth_order_jet_parameters?.speed_min === undefined &&
      artifact?.fourteenth_order_jet_parameters?.speed_max === undefined,
    "first-y fourteenth-order jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === FOURTEENTH_ORDER_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * FOURTEENTH_ORDER_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y18 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y18_interval) <
        1e9 &&
    ...
    "first-y fourteenth-order jet rows must certify h solves, source coefficient containment, fourteenth-order coefficients, and positive post-fourteenth-order tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-fourteenth-order-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
