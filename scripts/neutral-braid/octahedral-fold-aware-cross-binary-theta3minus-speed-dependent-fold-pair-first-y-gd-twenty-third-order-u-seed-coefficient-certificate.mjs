...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-third-order-u-seed-coefficient-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_twenty_third_order_u_seed_coefficient_certificate";
...
const TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-twenty-third-order-u-seed-coefficient-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-twenty-third-order-u-seed-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y27";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y27";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y27";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y27";
...
function nilpotentPowers(series) {
  ...
  const n18 = multiply(n16, n2);
  const n19 = multiply(n18, nilpotent);
  const n20 = multiply(n18, n2);
  const n21 = multiply(n20, nilpotent);
  const n22 = multiply(n20, n2);
  const n23 = multiply(n22, nilpotent);
  const n24 = multiply(n22, n2);
  const n25 = multiply(n24, nilpotent);
  const n26 = multiply(n24, n2);
  return {
    nilpotent,
    n2,
    n3,
...
function sinSeries(series) {
  ...
  sinN = add(sinN, scale(n19, -1 / 121645100408832000));
  sinN = add(sinN, scale(n21, 1 / 51090942171709440000));
  sinN = add(sinN, scale(n23, -1 / 25852016738884976640000));
  sinN = add(sinN, scale(n25, 1 / 15511210043330985984000000));
  ...
  cosN = subtract(cosN, scale(n18, 1 / 6402373705728000));
  cosN = add(cosN, scale(n20, 1 / 2432902008176640000));
  cosN = subtract(cosN, scale(n22, 1 / 1124000727777607680000));
  cosN = add(cosN, scale(n24, 1 / 620448401733239439360000));
  cosN = subtract(cosN, scale(n26, 1 / 403291461126605635584000000));
  return add(
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y24: gSeries
        .slice(0, 26)
  ...
  const qG = Array.from({ length: 24 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 24 }, (_, index) => pairD[index + 2]);
  ...
  const qGRemainingFifteenthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 15;
  const qDRemainingFifteenthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 15;
  const qGRemainingSixteenthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 16;
  const qDRemainingSixteenthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 16;
  const qGRemainingSeventeenthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 17;
  const qDRemainingSeventeenthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 17;
  const qGRemainingEighteenthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 18;
  const qDRemainingEighteenthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 18;
  const qGRemainingNineteenthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 19;
  const qDRemainingNineteenthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 19;
  const qGRemainingTwentiethOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 20;
  const qDRemainingTwentiethOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 20;
  const qGRemainingTwentyFirstOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 21;
  const qDRemainingTwentyFirstOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 21;
  const qGRemainingTwentySecondOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 22;
  const qDRemainingTwentySecondOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 22;
  const qGRemainingTwentyThirdOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 23;
  const qDRemainingTwentyThirdOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 23;
  const qGRemainingTwentyFourthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 24;
  const qDRemainingTwentyFourthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 24;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y25: pairG.slice(0, 26).map(root.formatInterval),
    D_pair_coefficients_y0_to_y25: pairD.slice(0, 26).map(root.formatInterval),
    ...
    Q_G_remaining_fifteenth_order_tail_budget: root.formatSmallNumber(
      qGRemainingFifteenthOrderTailBudget
    ),
    Q_D_remaining_fifteenth_order_tail_budget: root.formatSmallNumber(
      qDRemainingFifteenthOrderTailBudget
    ),
    Q_G_remaining_sixteenth_order_tail_budget: root.formatSmallNumber(
      qGRemainingSixteenthOrderTailBudget
    ),
    Q_D_remaining_sixteenth_order_tail_budget: root.formatSmallNumber(
      qDRemainingSixteenthOrderTailBudget
    ),
    Q_G_remaining_seventeenth_order_tail_budget: root.formatSmallNumber(
      qGRemainingSeventeenthOrderTailBudget
    ),
    Q_D_remaining_seventeenth_order_tail_budget: root.formatSmallNumber(
      qDRemainingSeventeenthOrderTailBudget
    ),
    Q_G_remaining_eighteenth_order_tail_budget: root.formatSmallNumber(
      qGRemainingEighteenthOrderTailBudget
    ),
    Q_D_remaining_eighteenth_order_tail_budget: root.formatSmallNumber(
      qDRemainingEighteenthOrderTailBudget
    ),
    Q_G_remaining_nineteenth_order_tail_budget: root.formatSmallNumber(
      qGRemainingNineteenthOrderTailBudget
    ),
    Q_D_remaining_nineteenth_order_tail_budget: root.formatSmallNumber(
      qDRemainingNineteenthOrderTailBudget
    ),
    Q_G_remaining_twentieth_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentiethOrderTailBudget
    ),
    Q_D_remaining_twentieth_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentiethOrderTailBudget
    ),
    Q_G_remaining_twenty_first_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentyFirstOrderTailBudget
    ),
    Q_D_remaining_twenty_first_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentyFirstOrderTailBudget
    ),
    Q_G_remaining_twenty_second_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentySecondOrderTailBudget
    ),
    Q_D_remaining_twenty_second_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentySecondOrderTailBudget
    ),
    Q_G_remaining_twenty_third_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentyThirdOrderTailBudget
    ),
    Q_D_remaining_twenty_third_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentyThirdOrderTailBudget
    ),
    Q_G_remaining_twenty_fourth_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentyFourthOrderTailBudget
    ),
    Q_D_remaining_twenty_fourth_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentyFourthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingFifteenthOrderTailBudget > 1e37 &&
    qDRemainingFifteenthOrderTailBudget > 1e37 &&
    qGRemainingFifteenthOrderTailBudget > 1e40 &&
    qDRemainingFifteenthOrderTailBudget > 1e40 &&
    qGRemainingSixteenthOrderTailBudget > 1e42 &&
    qDRemainingSixteenthOrderTailBudget > 1e42 &&
    qGRemainingSeventeenthOrderTailBudget > 1e45 &&
    qDRemainingSeventeenthOrderTailBudget > 1e45 &&
    qGRemainingEighteenthOrderTailBudget > 1e48 &&
    qDRemainingEighteenthOrderTailBudget > 1e48 &&
    qGRemainingNineteenthOrderTailBudget > 1e51 &&
    qDRemainingNineteenthOrderTailBudget > 1e51 &&
    qGRemainingTwentiethOrderTailBudget > 1e53 &&
    qDRemainingTwentiethOrderTailBudget > 1e53 &&
    qGRemainingTwentyFirstOrderTailBudget > 1e56 &&
    qDRemainingTwentyFirstOrderTailBudget > 1e56 &&
    qGRemainingTwentySecondOrderTailBudget > 1e59 &&
    qDRemainingTwentySecondOrderTailBudget > 1e59 &&
    qGRemainingTwentyThirdOrderTailBudget > 1e62 &&
    qDRemainingTwentyThirdOrderTailBudget > 1e62 &&
    qGRemainingTwentyFourthOrderTailBudget > 1e64 &&
    qDRemainingTwentyFourthOrderTailBudget > 1e64
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-twenty-third-order-u-seed-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y27_interval: root.formatSmallNumber(
    ...
    max_abs_Q_D_plus_15Q_G_y14_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_15Q_G_y14_coefficient_interval")
    ),
    max_abs_Q_D_plus_16Q_G_y15_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_16Q_G_y15_coefficient_interval")
    ),
    max_abs_Q_D_plus_17Q_G_y16_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_17Q_G_y16_coefficient_interval")
    ),
    max_abs_Q_D_plus_18Q_G_y17_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_18Q_G_y17_coefficient_interval")
    ),
    max_abs_Q_D_plus_19Q_G_y18_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_19Q_G_y18_coefficient_interval")
    ),
    max_abs_Q_D_plus_20Q_G_y19_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_20Q_G_y19_coefficient_interval")
    ),
    max_abs_Q_D_plus_21Q_G_y20_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_21Q_G_y20_coefficient_interval")
    ),
    max_abs_Q_D_plus_22Q_G_y21_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_22Q_G_y21_coefficient_interval")
    ),
    max_abs_Q_D_plus_23Q_G_y22_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_23Q_G_y22_coefficient_interval")
    ),
    max_abs_Q_D_plus_24Q_G_y23_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_24Q_G_y23_coefficient_interval")
    ),
    ...
    min_Q_G_remaining_fifteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_fifteenth_order_tail_budget")
    ),
    min_Q_D_remaining_fifteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_fifteenth_order_tail_budget")
    ),
    min_Q_G_remaining_sixteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_sixteenth_order_tail_budget")
    ),
    min_Q_D_remaining_sixteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_sixteenth_order_tail_budget")
    ),
    min_Q_G_remaining_seventeenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_seventeenth_order_tail_budget")
    ),
    min_Q_D_remaining_seventeenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_seventeenth_order_tail_budget")
    ),
    min_Q_G_remaining_eighteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_eighteenth_order_tail_budget")
    ),
    min_Q_D_remaining_eighteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_eighteenth_order_tail_budget")
    ),
    min_Q_G_remaining_nineteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_nineteenth_order_tail_budget")
    ),
    min_Q_D_remaining_nineteenth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_nineteenth_order_tail_budget")
    ),
    min_Q_G_remaining_twentieth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twentieth_order_tail_budget")
    ),
    min_Q_D_remaining_twentieth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twentieth_order_tail_budget")
    ),
    min_Q_G_remaining_twenty_first_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_first_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_first_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_first_order_tail_budget")
    ),
    min_Q_G_remaining_twenty_second_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_second_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_second_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_second_order_tail_budget")
    ),
    min_Q_G_remaining_twenty_third_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_third_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_third_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_third_order_tail_budget")
    ),
    min_Q_G_remaining_twenty_fourth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_fourth_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_fourth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_fourth_order_tail_budget")
    ),
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-twenty-third-order-u-seed-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyThirdOrderUSeedCoefficientCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_SPEED_CELL_COUNT
  );
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    ...
    predecessor_packets: [
      ...
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-cubic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-quartic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-quintic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sextic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-septic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-octic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-nonic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-decic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-undecic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-duodecic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-tridecic-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-fourteenth-order-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-fifteenth-order-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sixteenth-order-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-seventeenth-order-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-eighteenth-order-jet-coefficient-interval-certificate.md",
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-nineteenth-order-jet-coefficient-interval-certificate.md",
    ...
    twenty_third_order_u_seed_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+h15_epsilon*y^18+h16_epsilon*y^19+h17_epsilon*y^20+h18_epsilon*y^21+h19_epsilon*y^22+h20_epsilon*y^23+h21_epsilon*y^24+h22_epsilon*y^25+h23_epsilon*y^26+O(y^27)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h23; h21 is the E_epsilon(0,nu) seed, h22 is the first post-seed coefficient, and h23 is the U_epsilon(0,nu) seed",
      intervalized_quantity:
        "constant through twenty-third-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "U-seed coefficient interval only; the finite U_epsilon(y,nu) tube and the O(y^24) quotient tail remain open before full first-y enclosure",
      post_seed_coefficient_equation:
        "Shift_27(F_epsilon(y,h_{<=22}+y^23*U,nu))=C_{23,epsilon}(nu)+S_{23,epsilon}(nu)*U+O(y), with U(0,nu)=h23_epsilon(nu)",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    twenty_third_order_u_seed_coefficient_rows: rows,
    twenty_third_order_u_seed_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-fourteenth-order-jet-coefficient",
        status: "directed-rounded-interval-certified",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-fifteenth-order-jet-coefficient",
        status: "directed-rounded-interval-certified",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-sixteenth-order-jet-coefficient",
        status: "directed-rounded-interval-certified",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-seventeenth-order-jet-coefficient",
        status: "directed-rounded-interval-certified",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-eighteenth-order-jet-coefficient",
        status: "directed-rounded-interval-certified",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-nineteenth-order-jet-coefficient",
        status: "directed-rounded-interval-certified",
      },
      {
      ...
      },
      {
        ...
        status: "zero-safe-symbolic-cancellation-certified",
      },
      {
    ...
    artifact_claim: {
      ...
      certifies_directed_rounded_first_y_GD_twenty_third_order_u_seed_coefficient_enclosure:
        passed,
      certifies_directed_rounded_first_y_GD_finite_root_tail_tube: false,
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the U-seed coefficient h23 and the induced twenty-third-order first-y quotient coefficients Q_G23 and Q_D23 on the fold-pair h-root graph. It closes the U-seed coefficient row only; the finite U_epsilon(y,nu) tube, O(y^24) quotient tail/enclosure, and downstream collar closure remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from the h22 post-seed row to the U-coordinate seed h23 and the Q_G23/Q_D23 coefficients. The remaining sign condition is now a directed-rounded bound on the continuous U-coordinate post-seed tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentyThirdOrderUSeedCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.twenty_third_order_u_seed_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-third-order U-seed coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-third-order U-seed coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_third_order_u_seed_coefficient_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-third-order U-seed coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_third_order_u_seed_coefficient_parameters?.speed_band === undefined &&
      artifact?.twenty_third_order_u_seed_coefficient_parameters?.speed_window === undefined &&
      artifact?.twenty_third_order_u_seed_coefficient_parameters?.speed_min === undefined &&
      artifact?.twenty_third_order_u_seed_coefficient_parameters?.speed_max === undefined,
    "first-y twenty-third-order U-seed coefficient parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * TWENTY_THIRD_ORDER_U_SEED_COEFFICIENT_SPEED_CELL_COUNT &&
      ...
      Number.isFinite(Number(summary?.max_abs_source_equation_coeff_y0_to_y27_interval)) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y27_interval) <
        2e13 &&
      Number.isFinite(Number(summary?.max_abs_h23_interval)) &&
      Number(summary?.max_abs_h23_interval) < 5e12 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y23_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y23_coefficient_interval) < 4e12 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y23_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y23_coefficient_interval) < 9e13 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_24Q_G_y23_coefficient_interval)
      ) &&
      ...
      Number(summary?.min_Q_G_remaining_fifteenth_order_tail_budget) > 1e40 &&
      Number(summary?.min_Q_D_remaining_fifteenth_order_tail_budget) > 1e40 &&
      Number(summary?.min_Q_G_remaining_sixteenth_order_tail_budget) > 1e42 &&
      Number(summary?.min_Q_D_remaining_sixteenth_order_tail_budget) > 1e42 &&
      Number(summary?.min_Q_G_remaining_seventeenth_order_tail_budget) > 1e45 &&
      Number(summary?.min_Q_D_remaining_seventeenth_order_tail_budget) > 1e45 &&
      Number(summary?.min_Q_G_remaining_eighteenth_order_tail_budget) > 1e48 &&
      Number(summary?.min_Q_D_remaining_eighteenth_order_tail_budget) > 1e48 &&
      Number(summary?.min_Q_G_remaining_nineteenth_order_tail_budget) > 1e51 &&
      Number(summary?.min_Q_D_remaining_nineteenth_order_tail_budget) > 1e51 &&
      Number(summary?.min_Q_G_remaining_twentieth_order_tail_budget) > 1e51 &&
      Number(summary?.min_Q_D_remaining_twentieth_order_tail_budget) > 1e51 &&
      Number(summary?.min_Q_G_remaining_twentieth_order_tail_budget) > 1e53 &&
      Number(summary?.min_Q_D_remaining_twentieth_order_tail_budget) > 1e53 &&
      ...
      Number(summary?.min_Q_D_remaining_twenty_third_order_tail_budget) >
        1e62 &&
      Number(summary?.min_Q_G_remaining_twenty_fourth_order_tail_budget) >
        1e64 &&
      Number(summary?.min_Q_D_remaining_twenty_fourth_order_tail_budget) >
        1e64 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y twenty-third-order U-seed coefficient rows must certify h23, source coefficient containment, Q_G23/Q_D23 coefficients, and positive U-tail budget",
    errors
  );
  assertField(
    artifact?.artifact_claim
      ...
      artifact?.artifact_claim
        ...
        true &&
      artifact?.artifact_claim
        ?.certifies_directed_rounded_first_y_GD_finite_root_tail_tube ===
        false &&
      artifact?.artifact_claim
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-third-order-u-seed-coefficient-certificate.mjs [options]",
    "",
    "Options:",
