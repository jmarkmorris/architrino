...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-fifth-order-post-u-successor-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_SIXTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-sixth-order-post-u-successor-coefficient-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_thirty_sixth_order_post_u_successor_coefficient_certificate";
const PROMOTION_STATUS = "priority-only";
const NO_SPEED_WINDOW =
  "none; uses the historical positive speed-ratio zero-enclosure diagnostic; receiver-normal restart required only";
const SOURCE_COEFFICIENT = -1;
const SERIES_ORDER = 40;
const H_COUNT = 37;
const PREDECESSOR_H_COUNT = 36;
const TARGET_INDEX = 36;
const FIRST_Y_CELL_UPPER = 0.115 / 64;
const SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-thirty-sixth-order-post-U-successor-coefficient-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-thirty-sixth-order-post-U-successor-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y40";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y40";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y40";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y40";
...
function solveH36Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h35_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h35_interval: predecessorBranchRow.h35_interval,
      h36_interval: root.formatInterval(solve.hInterval),
      h36_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h36_solve_slope_sign: slope.sign,
      h36_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h36_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y38: gSeries
        .slice(0, 39)
  ...
  const qGRemainingThirtySeventhOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 37;
  const qDRemainingThirtySeventhOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 37;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y38: pairG.slice(0, 39).map(root.formatInterval),
    D_pair_coefficients_y0_to_y38: pairD.slice(0, 39).map(root.formatInterval),
    ...
    Q_G_y36_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[36]),
    Q_D_y36_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[36]),
    Q_G_remaining_thirty_seventh_order_tail_budget: root.formatSmallNumber(
      qGRemainingThirtySeventhOrderTailBudget
    ),
    Q_D_remaining_thirty_seventh_order_tail_budget: root.formatSmallNumber(
      qDRemainingThirtySeventhOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingThirtySeventhOrderTailBudget > 6e75 &&
    qDRemainingThirtySeventhOrderTailBudget > 6e75
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-thirty-sixth-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h35_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y40_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_37Q_G_y36_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_37Q_G_y36_coefficient_interval")
    ),
    ...
    h36_interval_hull: coefficientHull(
      branchRows.map((row) => row.h36_interval.map(Number))
    ),
    Q_G_y36_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y36_coefficient_interval.map(Number))
    ),
    Q_D_y36_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y36_coefficient_interval.map(Number))
    ),
    max_abs_h36_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h36_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y36_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 36, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y36_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 36, "max_abs_coefficient"))
    ),
    max_Q_G_y36_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y36_loss_on_first_y_cell")
    ),
    max_Q_D_y36_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y36_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_thirty_seventh_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_thirty_seventh_order_tail_budget")
    ),
    min_Q_D_remaining_thirty_seventh_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_thirty_seventh_order_tail_budget")
    ),
    inherited_h35_interval_hull:
      predecessorArtifact.thirty_fifth_order_post_u_successor_coefficient_summary
        .h35_interval_hull,
    inherited_Q_G_y35_coefficient_interval_hull:
      predecessorArtifact.thirty_fifth_order_post_u_successor_coefficient_summary
        .Q_G_y35_coefficient_interval_hull,
    inherited_Q_D_y35_coefficient_interval_hull:
      predecessorArtifact.thirty_fifth_order_post_u_successor_coefficient_summary
        .Q_D_y35_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-thirty-sixth-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtySixthOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyFifthOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.thirty_fifth_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_SIXTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    promotion_status: PROMOTION_STATUS,
    predecessor_packets: [
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-fifth-order-post-u-successor-coefficient-certificate.md",
    ],
    priority_packet:
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-sixth-order-post-u-successor-coefficient-certificate.md",
    thirty_sixth_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h35_artifact:
        "thirty-fifth-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h35_epsilon*y^38+h36_epsilon*y^39+O(y^40)",
      h_solve_slope_policy:
        "uses the inherited fold-null slope S_epsilon=epsilon*beta*F_eta_eta for h36 after the h35 predecessor row",
      intervalized_quantity:
        "thirty-sixth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^37) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_40(F_epsilon(y,h_{<=35}+y^36*X36,nu))=C_{36,epsilon}(nu)+S_{36,epsilon}(nu)*X36+O(y), with X36(0,nu)=h36_epsilon(nu)",
    },
    thirty_sixth_order_post_u_successor_coefficient_rows: rows,
    thirty_sixth_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-thirty-seventh-order-successor-tail-bound",
    ...
    artifact_claim: {
      ...
      inherits_directed_rounded_first_y_GD_thirty_fifth_order_post_u_successor_coefficient_enclosure:
        true,
      certifies_directed_rounded_first_y_GD_thirty_sixth_order_post_u_successor_coefficient_enclosure:
        passed,
      ...
      claim_level:
        "Directed-rounded interval certificate for h36 and the induced thirty-sixth-order first-y quotient coefficients Q_G36 and Q_D36 on the fold-pair h-root graph. It turns the thirty-sixth-order quotient-tail constant term into a coefficient row and sharpens the zero-touching quotient burden to the thirty-seventh-order successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-thirty-seventh-order-successor-tail-bound-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h36, Q_G36, and Q_D36 are interval-certified with the preserved Q_D36+37Q_G36 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtySixthOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.thirty_sixth_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_SIXTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D thirty-sixth-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D thirty-sixth-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.thirty_sixth_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y thirty-sixth-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.thirty_sixth_order_post_u_successor_coefficient_parameters
      ...
      artifact?.thirty_sixth_order_post_u_successor_coefficient_parameters
        ?.speed_max === undefined,
    "first-y thirty-sixth-order post-U successor coefficient parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h35_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y40_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y40_interval) <
        1.4e23 &&
      Number.isFinite(Number(summary?.max_abs_h36_interval)) &&
      Number(summary?.max_abs_h36_interval) < 5.3e22 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y36_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y36_coefficient_interval) < 4.5e22 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y36_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y36_coefficient_interval) < 1.7e24 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_37Q_G_y36_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_37Q_G_y36_coefficient_interval) <
        3.4e24 &&
      Number(summary?.min_Q_G_remaining_thirty_seventh_order_tail_budget) >
        3e100 &&
      Number(summary?.min_Q_D_remaining_thirty_seventh_order_tail_budget) >
        3e100 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y thirty-sixth-order post-U successor coefficient rows must certify h36, source coefficient containment, Q_G36/Q_D36 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-sixth-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
