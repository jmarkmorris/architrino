...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SEVENTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-seventh-order-post-u-successor-coefficient-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_twenty_seventh_order_post_u_successor_coefficient_certificate";
const PROMOTION_STATUS = "priority-only";
const NO_SPEED_WINDOW =
  "none; uses the historical positive speed-ratio zero-enclosure diagnostic; receiver-normal restart required only";
const SOURCE_COEFFICIENT = -1;
const SERIES_ORDER = 31;
const H_COUNT = 28;
const PREDECESSOR_H_COUNT = 27;
const TARGET_INDEX = 27;
const FIRST_Y_CELL_UPPER = 0.115 / 64;
const SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-twenty-seventh-order-post-U-successor-coefficient-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-twenty-seventh-order-post-U-successor-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y31";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y31";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y31";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y31";
...
function solveH27Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h26_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h26_interval: predecessorBranchRow.h26_interval,
      h27_interval: root.formatInterval(solve.hInterval),
      h27_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h27_solve_slope_sign: slope.sign,
      h27_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h27_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y29: gSeries
        .slice(0, 30)
  ...
  const qGRemainingTwentyEighthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 28;
  const qDRemainingTwentyEighthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 28;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y29: pairG.slice(0, 30).map(root.formatInterval),
    D_pair_coefficients_y0_to_y29: pairD.slice(0, 30).map(root.formatInterval),
    ...
    Q_G_y27_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[27]),
    Q_D_y27_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[27]),
    Q_G_remaining_twenty_eighth_order_tail_budget: root.formatSmallNumber(
      qGRemainingTwentyEighthOrderTailBudget
    ),
    Q_D_remaining_twenty_eighth_order_tail_budget: root.formatSmallNumber(
      qDRemainingTwentyEighthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingTwentyEighthOrderTailBudget > 6e75 &&
    qDRemainingTwentyEighthOrderTailBudget > 6e75
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-twenty-seventh-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h26_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y31_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_28Q_G_y27_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_28Q_G_y27_coefficient_interval")
    ),
    ...
    h27_interval_hull: coefficientHull(
      branchRows.map((row) => row.h27_interval.map(Number))
    ),
    Q_G_y27_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y27_coefficient_interval.map(Number))
    ),
    Q_D_y27_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y27_coefficient_interval.map(Number))
    ),
    max_abs_h27_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h27_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y27_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 27, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y27_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 27, "max_abs_coefficient"))
    ),
    max_Q_G_y27_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y27_loss_on_first_y_cell")
    ),
    max_Q_D_y27_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y27_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_twenty_eighth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_twenty_eighth_order_tail_budget")
    ),
    min_Q_D_remaining_twenty_eighth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_twenty_eighth_order_tail_budget")
    ),
    ...
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-twenty-seventh-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySeventhOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySixthOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SEVENTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    promotion_status: PROMOTION_STATUS,
    predecessor_packets: [
      "reference/priorities/braid-archive/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-sixth-order-post-u-successor-coefficient-certificate.md",
    ],
    priority_packet:
    ...
    twenty_seventh_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h26_artifact:
        "twenty-sixth-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h26_epsilon*y^29+h27_epsilon*y^30+O(y^31)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h27 after the h26 predecessor row",
      intervalized_quantity:
        "twenty-seventh-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^28) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_31(F_epsilon(y,h_{<=26}+y^27*W,nu))=C_{27,epsilon}(nu)+S_{27,epsilon}(nu)*W+O(y), with W(0,nu)=h27_epsilon(nu)",
    },
    twenty_seventh_order_post_u_successor_coefficient_rows: rows,
    twenty_seventh_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-twenty-seventh-order-post-U-successor-coefficient",
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-twenty-eighth-order-tail-bound",
    ...
    artifact_claim: {
      ...
      inherits_directed_rounded_first_y_GD_twenty_sixth_order_post_u_successor_coefficient_enclosure:
        true,
      certifies_directed_rounded_first_y_GD_twenty_seventh_order_post_u_successor_coefficient_enclosure:
        passed,
      ...
      claim_level:
        "Directed-rounded interval certificate for h27 and the induced twenty-seventh-order first-y quotient coefficients Q_G27 and Q_D27 on the fold-pair h-root graph. It turns the y^29 quotient constant term into a coefficient row and sharpens the zero-touching quotient burden to the O(y^28) successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-twenty-eighth-order-successor-tail-bound-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h27, Q_G27, and Q_D27 are interval-certified with the preserved Q_D27+28Q_G27 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySeventhOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.twenty_seventh_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SEVENTH_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-seventh-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-seventh-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_seventh_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-seventh-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_seventh_order_post_u_successor_coefficient_parameters
      ...
      artifact?.twenty_seventh_order_post_u_successor_coefficient_parameters
        ?.speed_max === undefined,
    "first-y twenty-seventh-order post-U successor coefficient parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h26_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y31_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y31_interval) <
        2e16 &&
      Number.isFinite(Number(summary?.max_abs_h27_interval)) &&
      Number(summary?.max_abs_h27_interval) < 6e15 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y27_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y27_coefficient_interval) < 5e15 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y27_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y27_coefficient_interval) < 1.4e17 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_28Q_G_y27_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_28Q_G_y27_coefficient_interval) <
        2.7e17 &&
      Number(summary?.min_Q_G_remaining_twenty_eighth_order_tail_budget) >
        6e75 &&
      Number(summary?.min_Q_D_remaining_twenty_eighth_order_tail_budget) >
        6e75 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y twenty-seventh-order post-U successor coefficient rows must certify h27, source coefficient containment, Q_G27/Q_D27 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-seventh-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
...
function main() {
  ...
  if (options.validate) {
    ...
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySeventhOrderPostUSuccessorCoefficientCertificate(
        artifact
      );
  ...
  try {
    const artifact =
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySeventhOrderPostUSuccessorCoefficientCertificate();
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySeventhOrderPostUSuccessorCoefficientCertificate(
        artifact
      );
