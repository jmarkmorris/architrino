...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SIXTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sixteenth-order-jet-coefficient-interval-certificate/v1";
...
const SIXTEENTH_ORDER_SPEED_CELL_COUNT = 128;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y20";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y20";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y20";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y20";
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y18: gSeries
        .slice(0, 19)
  ...
  const qG = Array.from({ length: 17 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 17 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y18: pairG.slice(0, 19).map(root.formatInterval),
    D_pair_coefficients_y0_to_y18: pairD.slice(0, 19).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qGRemainingSixteenthOrderTailBudget > 1e40 &&
    qDRemainingSixteenthOrderTailBudget > 1e40 &&
    ...
    qDRemainingSeventeenthOrderTailBudget > 1e45
      ...
      : "first-y-GD-sixteenth-order-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y20_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-sixteenth-order-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSixteenthOrderJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? SIXTEENTH_ORDER_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    sixteenth_order_jet_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+h15_epsilon*y^18+h16_epsilon*y^19+O(y^20)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h16; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, and sixteenth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "sixteenth-order coefficient interval only; the O(y^17) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    sixteenth_order_jet_rows: rows,
    sixteenth_order_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-seventeenth-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-seventeenth-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, and sixteenth-order first-y quotient coefficients Q_G0 through Q_G16 and Q_D0 through Q_D16 on the fold-pair h-root graph. It closes the sixteenth-order coefficient row only; the finite seventeenth-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite sixteenth-order-tail problem to a finite seventeenth-order-tail problem after the certified sixteenth-order coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^17) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdSixteenthOrderJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.sixteenth_order_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_SIXTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D sixteenth-order jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D sixteenth-order jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.sixteenth_order_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y sixteenth-order jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.sixteenth_order_jet_parameters?.speed_band === undefined &&
      artifact?.sixteenth_order_jet_parameters?.speed_window === undefined &&
      artifact?.sixteenth_order_jet_parameters?.speed_min === undefined &&
      artifact?.sixteenth_order_jet_parameters?.speed_max === undefined,
    "first-y sixteenth-order jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === SIXTEENTH_ORDER_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * SIXTEENTH_ORDER_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y20 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y20_interval) <
        1e10 &&
    ...
    "first-y sixteenth-order jet rows must certify h solves, source coefficient containment, sixteenth-order coefficients, and positive post-sixteenth-order tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sixteenth-order-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
