...
} from "./octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-second-order-post-u-successor-coefficient-certificate.mjs";
import {
...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_THIRD_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-third-order-post-u-successor-coefficient-certificate/v1";
...
const SERIES_ORDER = 37;
const H_COUNT = 34;
const PREDECESSOR_H_COUNT = 33;
const TARGET_INDEX = 33;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y37";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y37";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y37";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y37";
...
function solveH33Interval({ cell, branchSign, branchRow, hIntervals }) {
  ...
  const slope = numericInterval(branchRow.h32_solve_slope_interval);
  return {
...
function intervalRowForPredecessorRow({ speedIndex, predecessorRow }) {
  ...
  const branchSolves = predecessorRow.branch_rows.map((predecessorBranchRow) => {
    ...
    const row = {
      branch,
      inherited_h32_interval: predecessorBranchRow.h32_interval,
      h33_interval: root.formatInterval(solve.hInterval),
      h33_solve_slope_interval: root.formatInterval(solve.hSlopeInterval),
      h33_solve_slope_sign: slope.sign,
      h33_solve_slope_clearance: root.formatSmallNumber(slope.clearance),
      h33_residual_before_solve: root.formatInterval(
      ...
      G_branch_coefficients_y0_to_y35: gSeries
        .slice(0, 36)
  ...
  const qGRemainingThirtyFourthOrderTailBudget =
    qGRemainingMargin / FIRST_Y_CELL_UPPER ** 34;
  const qDRemainingThirtyFourthOrderTailBudget =
    qDRemainingMargin / FIRST_Y_CELL_UPPER ** 34;
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y35: pairG.slice(0, 36).map(root.formatInterval),
    D_pair_coefficients_y0_to_y35: pairD.slice(0, 36).map(root.formatInterval),
    ...
    Q_G_y33_loss_on_first_y_cell: root.formatSmallNumber(qGLosses[33]),
    Q_D_y33_loss_on_first_y_cell: root.formatSmallNumber(qDLosses[33]),
    Q_G_remaining_thirty_fourth_order_tail_budget: root.formatSmallNumber(
      qGRemainingThirtyFourthOrderTailBudget
    ),
    Q_D_remaining_thirty_fourth_order_tail_budget: root.formatSmallNumber(
      qDRemainingThirtyFourthOrderTailBudget
    ),
  };
  ...
  row.row_status =
    ...
    qGRemainingThirtyFourthOrderTailBudget > 6e75 &&
    qDRemainingThirtyFourthOrderTailBudget > 6e75
      ? ROW_CERTIFIED_STATUS
      : "first-y-GD-thirty-third-order-post-U-successor-coefficient-enclosure-open";
...
function summarizeRows({ rows, predecessorArtifact }) {
  ...
  const summary = {
    ...
    predecessor_h32_artifact_valid: predecessorErrors.length === 0,
    all_rows_certified: allRowsCertified,
    [ALL_SOURCE_CONTAINS_ZERO_FIELD]: branchRows.every(
      (row) => row[SOURCE_CONTAINS_ZERO_FIELD]
    ),
    max_abs_source_equation_coeff_y0_to_y37_interval: root.formatSmallNumber(
      maxField(branchRows, MAX_SOURCE_FIELD)
    ),
    max_abs_Q_D_plus_34Q_G_y33_coefficient_interval: root.formatSmallNumber(
      maxIdentityAbs(rows, "Q_D_plus_34Q_G_y33_coefficient_interval")
    ),
    ...
    h33_interval_hull: coefficientHull(
      branchRows.map((row) => row.h33_interval.map(Number))
    ),
    Q_G_y33_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_G_y33_coefficient_interval.map(Number))
    ),
    Q_D_y33_coefficient_interval_hull: coefficientHull(
      rows.map((row) => row.Q_D_y33_coefficient_interval.map(Number))
    ),
    max_abs_h33_interval: root.formatSmallNumber(
      Math.max(
        ...branchRows.map((row) => maxAbsInterval(row.h33_interval.map(Number)))
      )
    ),
    max_abs_Q_G_y33_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_G", 33, "max_abs_coefficient"))
    ),
    max_abs_Q_D_y33_coefficient_interval: root.formatSmallNumber(
      maxField(rows, qFieldName("Q_D", 33, "max_abs_coefficient"))
    ),
    max_Q_G_y33_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_G_y33_loss_on_first_y_cell")
    ),
    max_Q_D_y33_loss_on_first_y_cell: root.formatSmallNumber(
      maxField(rows, "Q_D_y33_loss_on_first_y_cell")
    ),
    min_Q_G_remaining_thirty_fourth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_G_remaining_thirty_fourth_order_tail_budget")
    ),
    min_Q_D_remaining_thirty_fourth_order_tail_budget: root.formatSmallNumber(
      minField(rows, "Q_D_remaining_thirty_fourth_order_tail_budget")
    ),
    inherited_h32_interval_hull:
      predecessorArtifact.thirty_second_order_post_u_successor_coefficient_summary
        .h32_interval_hull,
    inherited_Q_G_y32_coefficient_interval_hull:
      predecessorArtifact.thirty_second_order_post_u_successor_coefficient_summary
        .Q_G_y32_coefficient_interval_hull,
    inherited_Q_D_y32_coefficient_interval_hull:
      predecessorArtifact.thirty_second_order_post_u_successor_coefficient_summary
        .Q_D_y32_coefficient_interval_hull,
    status:
      allRowsCertified && predecessorErrors.length === 0
        ? CERTIFIED_STATUS
        : "theta3minus-fold-pair-first-y-GD-thirty-third-order-post-U-successor-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyThirdOrderPostUSuccessorCoefficientCertificate(options = {}) {
  const predecessorArtifact =
    options.predecessorArtifact ??
    buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtySecondOrderPostUSuccessorCoefficientCertificate(options.predecessorOptions ?? options);
  const rows =
    predecessorArtifact.thirty_second_order_post_u_successor_coefficient_rows.map(
  ...
  return {
    ...
    predecessor_packets: [
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-second-order-post-u-successor-coefficient-certificate.md",
    ],
    priority_packet:
      "reference/priorities/braid-geometry-export-bridge/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-third-order-post-u-successor-coefficient-certificate.md",
    thirty_third_order_post_u_successor_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      predecessor_h32_artifact:
        "thirty-second-order post-U successor coefficient certificate",
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+...+h32_epsilon*y^35+h33_epsilon*y^36+O(y^37)",
      h_solve_slope_policy:
        "uses the inherited fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h33 after the h32 predecessor row",
      intervalized_quantity:
        "thirty-third-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "coefficient interval only; the successor O(y^34) quotient tail remains open before full first-y enclosure",
      successor_coefficient_equation:
        "Shift_37(F_epsilon(y,h_{<=32}+y^33*X33,nu))=C_{33,epsilon}(nu)+S_{33,epsilon}(nu)*X33+O(y), with X33(0,nu)=h33_epsilon(nu)",
    },
    thirty_third_order_post_u_successor_coefficient_rows: rows,
    thirty_third_order_post_u_successor_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-thirty-fourth-order-tail-bound",
        status: "directed-rounded-open",
      },
    ],
    artifact_claim: {
      assumes_fixed_speed_window: false,
      inherits_directed_rounded_first_y_GD_thirty_second_order_post_u_successor_coefficient_enclosure:
        true,
      ...
      claim_level:
        "Directed-rounded interval certificate for h33 and the induced thirty-third-order first-y quotient coefficients Q_G33 and Q_D33 on the fold-pair h-root graph. It turns the thirty-third-order quotient-tail constant term into a coefficient row and sharpens the zero-touching quotient burden to the thirty-fourth-order successor tail; full first-y enclosure, scaled remainder, I1 closure, quadrature, and retained branch status remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The zero-touching first-y quotient tail has been sharpened again: h33, Q_G33, and Q_D33 are interval-certified with the preserved Q_D33+34Q_G33 identity.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdThirtyThirdOrderPostUSuccessorCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary =
    artifact?.thirty_third_order_post_u_successor_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_THIRTY_THIRD_ORDER_POST_U_SUCCESSOR_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D thirty-third-order post-U successor coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D thirty-third-order post-U successor coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.thirty_third_order_post_u_successor_coefficient_parameters
      ?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y thirty-third-order post-U successor coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.thirty_third_order_post_u_successor_coefficient_parameters
  ...
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      ...
      summary?.predecessor_h32_artifact_valid === true &&
      ...
      Number.isFinite(
        Number(summary?.max_abs_source_equation_coeff_y0_to_y37_interval)
      ) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y37_interval) <
        6.5e20 &&
      Number.isFinite(Number(summary?.max_abs_h33_interval)) &&
      Number(summary?.max_abs_h33_interval) < 2.5e20 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y33_coefficient_interval)) &&
      Number(summary?.max_abs_Q_G_y33_coefficient_interval) < 2.1e20 &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y33_coefficient_interval)) &&
      Number(summary?.max_abs_Q_D_y33_coefficient_interval) < 7.2e21 &&
      Number.isFinite(
        Number(summary?.max_abs_Q_D_plus_34Q_G_y33_coefficient_interval)
      ) &&
      Number(summary?.max_abs_Q_D_plus_34Q_G_y33_coefficient_interval) <
        1.5e22 &&
      Number(summary?.min_Q_G_remaining_thirty_fourth_order_tail_budget) >
        1.8e92 &&
      Number(summary?.min_Q_D_remaining_thirty_fourth_order_tail_budget) >
        1.8e92 &&
      summary?.all_QD_QG_coefficient_identity_intervals_contain_zero === true,
    "first-y thirty-third-order post-U successor coefficient rows must certify h33, source coefficient containment, Q_G33/Q_D33 coefficients, and positive successor-tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-thirty-third-order-post-u-successor-coefficient-certificate.mjs [options]",
    "",
    "Options:",
