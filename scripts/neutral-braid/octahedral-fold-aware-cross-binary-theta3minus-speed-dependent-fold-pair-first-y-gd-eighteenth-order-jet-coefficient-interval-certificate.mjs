...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_EIGHTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-eighteenth-order-jet-coefficient-interval-certificate/v1";

const PACKET_ID =
  "octahedral_fold_aware_cross_binary_theta3minus_speed_dependent_fold_pair_first_y_gd_eighteenth_order_jet_coefficient_interval_certificate";
...
const EIGHTEENTH_ORDER_SPEED_CELL_COUNT = 128;
const CERTIFIED_STATUS =
  "directed-rounded-theta3minus-fold-pair-first-y-GD-eighteenth-order-jet-coefficient-interval-certified";
const ROW_CERTIFIED_STATUS =
  "directed-rounded-first-y-GD-eighteenth-order-jet-coefficient-enclosed";
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y22";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y22";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y22";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y22";
...
function intervalRowForSpeedCell({
  ...
  const branchSolves = ["-", "+"].map((branch) => {
    ...
    const row = {
      ...
      G_branch_coefficients_y0_to_y20: gSeries
        .slice(0, 21)
  ...
  const qG = Array.from({ length: 19 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 19 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y20: pairG.slice(0, 21).map(root.formatInterval),
    D_pair_coefficients_y0_to_y20: pairD.slice(0, 21).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qGRemainingEighteenthOrderTailBudget > 1e45 &&
    qDRemainingEighteenthOrderTailBudget > 1e45 &&
    ...
    qDRemainingNineteenthOrderTailBudget > 1e51
      ...
      : "first-y-GD-eighteenth-order-jet-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y22_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-eighteenth-order-jet-coefficient-interval-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdEighteenthOrderJetCoefficientIntervalCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? EIGHTEENTH_ORDER_SPEED_CELL_COUNT
  );
  ...
  return {
    schema:
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_EIGHTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    packet_id: PACKET_ID,
    ...
    eighteenth_order_jet_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+h15_epsilon*y^18+h16_epsilon*y^19+h17_epsilon*y^20+h18_epsilon*y^21+O(y^22)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h18; avoids uncorrelated subtraction of large source-coefficient intervals at high jet order",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, and eighteenth-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "eighteenth-order coefficient interval only; the O(y^19) quotient tail remains open and must be bounded before full first-y enclosure",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    eighteenth_order_jet_rows: rows,
    eighteenth_order_jet_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-nineteenth-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-nineteenth-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, and eighteenth-order first-y quotient coefficients Q_G0 through Q_G18 and Q_D0 through Q_D18 on the fold-pair h-root graph. It closes the eighteenth-order coefficient row only; the finite nineteenth-order quotient tail/enclosure and downstream collar closure remain open.",
    },
    result: {
      theory_status: summary.status,
      first_successor_row:
        "theta3minus.fold-pair-first-y-GD-nineteenth-order-tail-bound-directed-rounded-required",
      retention: "not_retained",
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a finite eighteenth-order-tail problem to a finite nineteenth-order-tail problem after the certified eighteenth-order coefficient. The remaining sign condition is now a directed-rounded bound on the O(y^19) quotient tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdEighteenthOrderJetCoefficientIntervalCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.eighteenth_order_jet_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_EIGHTEENTH_ORDER_JET_COEFFICIENT_INTERVAL_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D eighteenth-order jet coefficient interval certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D eighteenth-order jet packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.eighteenth_order_jet_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y eighteenth-order jet certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.eighteenth_order_jet_parameters?.speed_band === undefined &&
      artifact?.eighteenth_order_jet_parameters?.speed_window === undefined &&
      artifact?.eighteenth_order_jet_parameters?.speed_min === undefined &&
      artifact?.eighteenth_order_jet_parameters?.speed_max === undefined,
    "first-y eighteenth-order jet parameters must not contain speed-band fields",
    errors
  );
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === EIGHTEENTH_ORDER_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * EIGHTEENTH_ORDER_SPEED_CELL_COUNT &&
      ...
      summary?.all_source_equation_coefficients_contain_zero_y0_to_y22 ===
        true &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y22_interval) <
        1.5e9 &&
      ...
      Number(summary?.min_Q_G_remaining_eighteenth_order_tail_budget) > 1e45 &&
      Number(summary?.min_Q_D_remaining_eighteenth_order_tail_budget) > 1e45 &&
    ...
    "first-y eighteenth-order jet rows must certify h solves, source coefficient containment, eighteenth-order coefficients, and positive post-eighteenth-order tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-eighteenth-order-jet-coefficient-interval-certificate.mjs [options]",
    "",
    "Options:",
...
function main() {
  ...
  if (options.validate) {
    ...
    const errors =
      validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdEighteenthOrderJetCoefficientIntervalCertificate(
        artifact
      );
  ...
  try {
    const artifact =
      buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdEighteenthOrderJetCoefficientIntervalCertificate(
        options
      );
