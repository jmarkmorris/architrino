...
export const OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SECOND_ORDER_POST_SEED_COEFFICIENT_CERTIFICATE_SCHEMA =
  "neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-second-order-post-seed-coefficient-certificate/v1";
...
const TWENTY_SECOND_ORDER_POST_SEED_COEFFICIENT_SPEED_CELL_COUNT = 128;
...
const SOURCE_FIELD = "source_equation_coefficients_y0_to_y26";
const SOURCE_CONTAINS_ZERO_FIELD =
  "source_equation_coefficients_contain_zero_y0_to_y26";
const ALL_SOURCE_CONTAINS_ZERO_FIELD =
  "all_source_equation_coefficients_contain_zero_y0_to_y26";
const MAX_SOURCE_FIELD = "max_abs_source_equation_coeff_y0_to_y26";
...
function intervalRowForSpeedCell({
  ...
  const qG = Array.from({ length: 23 }, (_, index) => pairG[index + 2]);
  const qD = Array.from({ length: 23 }, (_, index) => pairD[index + 2]);
  ...
  const row = {
    ...
    G_pair_coefficients_y0_to_y24: pairG.slice(0, 25).map(root.formatInterval),
    D_pair_coefficients_y0_to_y24: pairD.slice(0, 25).map(root.formatInterval),
  ...
  row.row_status =
    ...
    qDRemainingTwentyThirdOrderTailBudget > 1e62
      ...
      : "first-y-GD-twenty-second-order-post-seed-coefficient-enclosure-open";
...
function summarizeRows(rows) {
  ...
  const summary = {
    ...
    max_abs_source_equation_coeff_y0_to_y26_interval: root.formatSmallNumber(
    ...
    status: allRowsCertified
      ? CERTIFIED_STATUS
      : "theta3minus-fold-pair-first-y-GD-twenty-second-order-post-seed-coefficient-open",
  };
...
export function buildOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySecondOrderPostSeedCoefficientCertificate(
  options = {}
) {
  const speedBreaks = root.makeSpeedBreaks(
    options.speedCellCount ?? TWENTY_SECOND_ORDER_POST_SEED_COEFFICIENT_SPEED_CELL_COUNT
  );
  ...
  return {
    ...
    twenty_second_order_post_seed_coefficient_parameters: {
      receiver_label: "1+",
      source_label: "3-",
      ...
      h_root_graph_chart:
        "delta_epsilon=delta_f+epsilon*beta*y+gamma*y^2+h0_epsilon*y^3+h1_epsilon*y^4+h2_epsilon*y^5+h3_epsilon*y^6+h4_epsilon*y^7+h5_epsilon*y^8+h6_epsilon*y^9+h7_epsilon*y^10+h8_epsilon*y^11+h9_epsilon*y^12+h10_epsilon*y^13+h11_epsilon*y^14+h12_epsilon*y^15+h13_epsilon*y^16+h14_epsilon*y^17+h15_epsilon*y^18+h16_epsilon*y^19+h17_epsilon*y^20+h18_epsilon*y^21+h19_epsilon*y^22+h20_epsilon*y^23+h21_epsilon*y^24+h22_epsilon*y^25+O(y^26)",
      h_solve_slope_policy:
        "uses the fold-local slope S_epsilon=epsilon*beta*F_delta_delta for h0 through h22; h21 is the E_epsilon(0,nu) seed and h22 is the first post-seed coefficient",
      intervalized_quantity:
        "constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, sixteenth-order, seventeenth-order, eighteenth-order, nineteenth-order, twentieth-order, twenty-first-order, and twenty-second-order coefficients of Q_G=(P-L)/y^2 and Q_D=(D_pair-L)/y^2 at y=0",
      finite_remainder_policy:
        "post-seed coefficient interval only; the finite E_epsilon(y,nu) tube and the O(y^23) quotient tail remain open before full first-y enclosure",
      post_seed_coefficient_equation:
        "Shift_26(F_epsilon(y,h_{<=21}+y^22*E_1,nu))=C_{22,epsilon}(nu)+S_{22,epsilon}(nu)*E_1+O(y), with E_1(0,nu)=h22_epsilon(nu)",
      root_subdivisions_for_fold_rows: rootSubdivisions,
    },
    twenty_second_order_post_seed_coefficient_rows: rows,
    twenty_second_order_post_seed_coefficient_summary: summary,
    closure_burndown: [
      {
      ...
      {
        row: "theta3minus.fold-pair-first-y-GD-twenty-third-order-tail-bound",
        status: "directed-rounded-open",
      },
      {
        row: "theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure",
        status: "blocked-by-twenty-third-order-tail-bound",
      },
      {
    ...
    artifact_claim: {
      ...
      claim_level:
        "Directed-rounded speed-cell interval certificate for the twenty-second-order post-seed coefficient h22 and the induced twenty-second-order first-y quotient coefficients Q_G22 and Q_D22 on the fold-pair h-root graph. It closes the post-seed coefficient row only; the finite E_epsilon(y,nu) tube, O(y^23) quotient tail/enclosure, and downstream collar closure remain open.",
    },
    result: {
      ...
      retained_branch: false,
      status_note:
        "The first-y blocker is narrowed from a missing post-seed coefficient to a finite E_epsilon tube and O(y^23) quotient-tail problem after the certified h22, Q_G22, and Q_D22 intervals. The remaining sign condition is now a directed-rounded bound on the continuous post-seed tail.",
    },
  };
}
...
export function validateOctahedralFoldAwareCrossBinaryTheta3minusSpeedDependentFoldPairFirstYGdTwentySecondOrderPostSeedCoefficientCertificate(
  artifact
) {
  const errors = [];
  const summary = artifact?.twenty_second_order_post_seed_coefficient_summary;
  assertField(
    artifact?.schema ===
      OCTAHEDRAL_FOLD_AWARE_CROSS_BINARY_THETA3MINUS_SPEED_DEPENDENT_FOLD_PAIR_FIRST_Y_GD_TWENTY_SECOND_ORDER_POST_SEED_COEFFICIENT_CERTIFICATE_SCHEMA,
    "schema must match theta3minus fold-pair first-y G/D twenty-second-order post-seed coefficient certificate schema",
    errors
  );
  assertField(
    artifact?.packet_id === PACKET_ID,
    "packet id must match theta3minus fold-pair first-y G/D twenty-second-order post-seed coefficient packet",
    errors
  );
  assertField(
  ...
  assertField(
    artifact?.twenty_second_order_post_seed_coefficient_parameters?.speed_constraint === NO_SPEED_WINDOW &&
      artifact?.artifact_claim?.assumes_fixed_speed_window === false,
    "first-y twenty-second-order post-seed coefficient certificate must not impose a fixed speed window",
    errors
  );
  assertField(
    artifact?.twenty_second_order_post_seed_coefficient_parameters?.speed_band === undefined &&
      artifact?.twenty_second_order_post_seed_coefficient_parameters?.speed_window === undefined &&
      artifact?.twenty_second_order_post_seed_coefficient_parameters?.speed_min === undefined &&
      artifact?.twenty_second_order_post_seed_coefficient_parameters?.speed_max === undefined,
  ...
  assertField(
    summary?.status === CERTIFIED_STATUS &&
      summary?.speed_cell_count === TWENTY_SECOND_ORDER_POST_SEED_COEFFICIENT_SPEED_CELL_COUNT &&
      summary?.branch_cell_count === 2 * TWENTY_SECOND_ORDER_POST_SEED_COEFFICIENT_SPEED_CELL_COUNT &&
      ...
      Number.isFinite(Number(summary?.max_abs_source_equation_coeff_y0_to_y26_interval)) &&
      Number(summary?.max_abs_source_equation_coeff_y0_to_y26_interval) <
        2e12 &&
      Number.isFinite(Number(summary?.max_abs_h22_interval)) &&
      Number(summary?.max_abs_h22_interval) < 8e11 &&
      Number.isFinite(Number(summary?.max_abs_Q_G_y22_coefficient_interval)) &&
      Number.isFinite(Number(summary?.max_abs_Q_D_y22_coefficient_interval)) &&
      Number.isFinite(
    ...
    "first-y twenty-second-order post-seed coefficient rows must certify h22, source coefficient containment, Q_G22/Q_D22 coefficients, and positive post-seed tail budget",
    errors
  );
  assertField(
...
function usage() {
  return [
    "Usage: node scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-twenty-second-order-post-seed-coefficient-certificate.mjs [options]",
    "",
    "Options:",
