...
import {
  EQ21_SHEAR_RSD_TRANSFER_KEYS,
  EQ21_SHEAR_RSD_TRANSFER_ROWS,
  evaluateShearRsdTransferModel,
  shearRsdTransferEvidenceStatusForPath,
} from "./eq21-shear-rsd-transfer-evidence.mjs";
...
const DEFAULT_INPUT_PATH = path.join(
  SCRIPT_DIR,
  "eq21-shear-rsd-transfer-child-attempt.v1.json",
);
const INPUT_SCHEMA = "aaa-equation-map-eq21-shear-rsd-transfer-child-input/v1";
const OUTPUT_SCHEMA = "aaa-equation-map-eq21-shear-rsd-transfer-child-check/v1";
...
const output = evaluateEq21ShearRsdTransferChild(input, inputPath);
...
function printHelp() {
  console.log(`Usage: node scripts/equation-mapping/eq21-shear-rsd-transfer-child-residual.mjs [options]

Options:
  --input PATH          EQ-21 shear/RSD transfer child input JSON.
...
This checker evaluates the score-neutral EQ-21 shear/RSD transfer child.
It consumes the accepted shared-observation parent, accepted f-sigma8 child,
accepted normalized P(k,z) child, and accepted CMB-lensing child; it does not
raise equation scores.`);
}
...
function evaluateEq21ShearRsdTransferChild(input, inputPath) {
  const tolerances = parseTolerances(input.tolerances ?? {});
  const packet = input.packet ?? input;
  const rows = packet.rows ?? {};
  const rowChecks = Object.fromEntries(
    EQ21_SHEAR_RSD_TRANSFER_ROWS.map((rowId) => [
      rowId,
      evaluateAcceptedRow(rows[rowId]),
    ]),
  );
  const missingRows = EQ21_SHEAR_RSD_TRANSFER_ROWS.filter(
  ...
  return {
    ...
    residual: {
      ...
      row: "EQ-21",
      childConsumer: "shear_rsd_transfer",
      claimLevel:
        "score-neutral shear/RSD transfer child; accepted shared-observation, f-sigma8, P(k,z), and CMB-lensing child evidence are required before score review",
    },
    tolerances,
    summary: {
      status,
      ...
      shearRsdGridResidualPass: model.shearRsdGridResidualPass,
      noHiddenRetunePass: model.noHiddenRetunePass,
      sampleCount: model.derived?.samples?.length ?? 0,
      shearRsdGridNormalizedResidual:
        model.derived?.shear_rsd_grid_normalized_residual ?? null,
    },
...
function summarizeOutput(output) {
  return {
    ...
    shearRsdModel: {
      computed: output.shearRsdModel.computed,
      derivedPass: output.shearRsdModel.derivedPass,
      shearRsdGridResidualPass: output.shearRsdModel.shearRsdGridResidualPass,
      noHiddenRetunePass: output.shearRsdModel.noHiddenRetunePass,
      derived: output.shearRsdModel.derived,
    },
  };
}
...
function evaluateSharedKeys(rawKeys, parent, growthChild, matterChild, lensingChild, tolerances) {
  const keyRows = new Map(
    (Array.isArray(rawKeys) ? rawKeys : []).map((row) => [row.key, row]),
  );
  const keys = Object.fromEntries(
    EQ21_SHEAR_RSD_TRANSFER_KEYS.map((key) => {
  ...
  const missingSharedKeys = EQ21_SHEAR_RSD_TRANSFER_KEYS.filter(
  ...
  return {
    ...
    allExpectedKeysDeclared: EQ21_SHEAR_RSD_TRANSFER_KEYS.every((key) =>
      keyRows.has(key),
    ),
    missingSharedKeys,
    expectedKeys: EQ21_SHEAR_RSD_TRANSFER_KEYS,
...
function evaluateModel(rawModel, matterChildRaw, lensingChildRaw, growthChildRaw, tolerances) {
  const base = evaluateShearRsdTransferModel(
    rawModel,
    matterChildRaw,
    lensingChildRaw,
    growthChildRaw,
  ...
  if (!base.computed) {
    return {
      ...base,
      derivedPass: false,
      shearRsdGridResidualPass: false,
  ...
  const residualActual = finiteNumberOrNull(
    rawModel.residualComponents?.shear_rsd_grid_normalized ??
      rawModel.shear_rsd_grid_normalized,
  );
  const residualExpected = base.derived.shear_rsd_grid_normalized_residual;
  const shearRsdGridResidualPass =
    residualActual !== null &&
  ...
  return {
    ...
    derivedMismatches,
    shearRsdGridResidualPass,
    shearRsdGridResidualActual: residualActual,
...
function evaluateSourceEvidence({ rows, sharedKeys, model }) {
  const rowEntries = EQ21_SHEAR_RSD_TRANSFER_ROWS.map((rowId) =>
  ...
  const keyEntries = EQ21_SHEAR_RSD_TRANSFER_KEYS.map((key) =>
    evaluateSourceEvidenceEntry({
      scope: "shared_key",
      id: key,
  ...
  const modelEntries = [
    evaluateSourceEvidenceEntry({
      scope: "model_input",
      id: "shearRsdKernel",
      status: model.shearRsdKernel?.status ?? null,
      sourcePath: model.shearRsdKernel?.sourcePath ?? model.shearRsdKernel?.source ?? null,
    }),
  ];
...
function evaluateSourceEvidenceEntry({ scope, id, status, sourcePath }) {
  ...
  const evidence = shearRsdTransferEvidenceStatusForPath(sourcePath, {
    repoRoot: process.cwd(),
  });
  return {
    scope,
...
function decideStatus({
  ...
  if (!model.computed) {
    return "blocked_shear_rsd_model_not_computed";
  }
  if (!model.derivedPass) {
    return "blocked_shear_rsd_model_derived_mismatch";
  }
  if (!model.shearRsdGridResidualPass) {
    return "blocked_shear_rsd_grid_residual";
  }
...
function firstBlocker({
  ...
  if (!sourceEvidence.passed) {
    return "accepted_without_shear_rsd_transfer_evidence";
  }
  if (!parent.accepted) {
  ...
  if (!model.computed) {
    return model.reason ?? "shear_rsd_model_not_computed";
  }
  if (!model.derivedPass) {
    return `shear_rsd_model_derived_mismatch_${model.derivedMismatches[0]?.key ?? "unknown"}`;
  }
  if (!model.shearRsdGridResidualPass) {
    return "shear_rsd_grid_residual_failed";
  }
  if (!model.noHiddenRetunePass) {
    return "shear_rsd_child_hidden_retune";
  }
  return status;
}
...
function firstBlockerDetails({
  ...
  if (nextBlocker === "accepted_without_shear_rsd_transfer_evidence") {
    return {
...
function evaluateAcceptedRow(row) {
  ...
  const evidence = shearRsdTransferEvidenceStatusForPath(
  ...
  if (!evidence.accepted) {
    return {
      accepted: false,
      reason: `shear_rsd_${evidence.reason}`,
      evidence,
    };
  }
