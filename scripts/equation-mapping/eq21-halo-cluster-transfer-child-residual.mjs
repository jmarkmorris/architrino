...
import {
  EQ21_HALO_CLUSTER_TRANSFER_KEYS,
  EQ21_HALO_CLUSTER_TRANSFER_ROWS,
  evaluateHaloClusterTransferModel,
  haloClusterTransferEvidenceStatusForPath,
} from "./eq21-halo-cluster-transfer-evidence.mjs";
...
const DEFAULT_INPUT_PATH = path.join(
  SCRIPT_DIR,
  "eq21-halo-cluster-transfer-child-attempt.v1.json",
);
const INPUT_SCHEMA = "aaa-equation-map-eq21-halo-cluster-transfer-child-input/v1";
const OUTPUT_SCHEMA = "aaa-equation-map-eq21-halo-cluster-transfer-child-check/v1";
...
const output = evaluateEq21HaloClusterTransferChild(input, inputPath);
...
function printHelp() {
  console.log(`Usage: node scripts/equation-mapping/eq21-halo-cluster-transfer-child-residual.mjs [options]

Options:
  --input PATH          EQ-21 halo/cluster transfer child input JSON.
...
This checker evaluates the score-neutral EQ-21 halo/cluster transfer child.
...
function evaluateEq21HaloClusterTransferChild(input, inputPath) {
  const tolerances = parseTolerances(input.tolerances ?? {});
  const packet = input.packet ?? input;
  const rows = packet.rows ?? {};
  const rowChecks = Object.fromEntries(
    EQ21_HALO_CLUSTER_TRANSFER_ROWS.map((rowId) => [
      rowId,
      evaluateAcceptedRow(rows[rowId]),
    ]),
  );
  const missingRows = EQ21_HALO_CLUSTER_TRANSFER_ROWS.filter(
  ...
  return {
    ...
    residual: {
      ...
      row: "EQ-21",
      childConsumer: "halo_cluster_transfer",
      claimLevel:
        "score-neutral halo/cluster transfer child; accepted shared-observation, growth, matter-power, CMB-lensing, and shear/RSD child evidence are required before score review",
    },
    tolerances,
    summary: {
      status,
      ...
      haloClusterGridResidualPass: model.haloClusterGridResidualPass,
      noHiddenRetunePass: model.noHiddenRetunePass,
      sampleCount: model.derived?.samples?.length ?? 0,
      haloClusterGridNormalizedResidual:
        model.derived?.halo_cluster_grid_normalized_residual ?? null,
    },
...
function summarizeOutput(output) {
  return {
    ...
    haloClusterModel: {
      computed: output.haloClusterModel.computed,
      derivedPass: output.haloClusterModel.derivedPass,
      haloClusterGridResidualPass: output.haloClusterModel.haloClusterGridResidualPass,
      noHiddenRetunePass: output.haloClusterModel.noHiddenRetunePass,
      derived: output.haloClusterModel.derived,
    },
  };
}
...
function evaluateSharedKeys(
  ...
  const keys = Object.fromEntries(
    EQ21_HALO_CLUSTER_TRANSFER_KEYS.map((key) => {
  ...
  const missingSharedKeys = EQ21_HALO_CLUSTER_TRANSFER_KEYS.filter(
  ...
  return {
    ...
    allExpectedKeysDeclared: EQ21_HALO_CLUSTER_TRANSFER_KEYS.every((key) =>
      keyRows.has(key),
    ),
    missingSharedKeys,
    expectedKeys: EQ21_HALO_CLUSTER_TRANSFER_KEYS,
...
function evaluateModel(rawModel, matterChildRaw, lensingChildRaw, shearRsdChildRaw, tolerances) {
  const base = evaluateHaloClusterTransferModel(
    rawModel,
    matterChildRaw,
    lensingChildRaw,
    shearRsdChildRaw,
  ...
  if (!base.computed) {
    return {
      ...base,
      derivedPass: false,
      haloClusterGridResidualPass: false,
  ...
  const residualActual = finiteNumberOrNull(
    rawModel.residualComponents?.halo_cluster_grid_normalized ??
      rawModel.halo_cluster_grid_normalized,
  );
  const residualExpected = base.derived.halo_cluster_grid_normalized_residual;
  const haloClusterGridResidualPass =
    residualActual !== null &&
  ...
  return {
    ...
    derivedMismatches,
    haloClusterGridResidualPass,
    haloClusterGridResidualActual: residualActual,
...
function compareDerived(actual, expected, tolerances, mismatches) {
  for (const [key, expectedValue] of Object.entries(expected)) {
    if (key === "samples" || key === "halo_cluster_grid_normalized_residual") {
      continue;
    }
  ...
  for (const expectedSample of expected.samples) {
    ...
    for (const [key, expectedValue] of Object.entries(expectedSample)) {
      if (key === "sampleId" || key === "halo_cluster_normalized_residual") {
        continue;
      }
...
function evaluateSourceEvidence({ rows, sharedKeys, model }) {
  const rowEntries = EQ21_HALO_CLUSTER_TRANSFER_ROWS.map((rowId) =>
  ...
  const keyEntries = EQ21_HALO_CLUSTER_TRANSFER_KEYS.map((key) =>
    evaluateSourceEvidenceEntry({
      scope: "shared_key",
      id: key,
  ...
  const modelEntries = [
    evaluateSourceEvidenceEntry({
      scope: "model_input",
      id: "haloClusterInversion",
      status: model.haloClusterInversion?.status ?? null,
      sourcePath:
        model.haloClusterInversion?.sourcePath ?? model.haloClusterInversion?.source ?? null,
    }),
  ];
...
function evaluateSourceEvidenceEntry({ scope, id, status, sourcePath }) {
  ...
  const evidence = haloClusterTransferEvidenceStatusForPath(sourcePath, {
    repoRoot: process.cwd(),
  });
  return {
    scope,
...
function decideStatus({
  ...
  if (!model.computed) {
    return "blocked_halo_cluster_model_not_computed";
  }
  if (!model.derivedPass) {
    return "blocked_halo_cluster_model_derived_mismatch";
  }
  if (!model.haloClusterGridResidualPass) {
    return "blocked_halo_cluster_grid_residual";
  }
...
function firstBlocker({
  ...
  if (!sourceEvidence.passed) {
    return "accepted_without_halo_cluster_transfer_evidence";
  }
  if (!parent.accepted) {
  ...
  if (!model.computed) {
    return model.reason ?? "halo_cluster_model_not_computed";
  }
  if (!model.derivedPass) {
    return `halo_cluster_model_derived_mismatch_${model.derivedMismatches[0]?.key ?? "unknown"}`;
  }
  if (!model.haloClusterGridResidualPass) {
    return "halo_cluster_grid_residual_failed";
  }
  if (!model.noHiddenRetunePass) {
    return "halo_cluster_child_hidden_retune";
  }
  return status;
}
...
function firstBlockerDetails({
  ...
  if (nextBlocker === "accepted_without_halo_cluster_transfer_evidence") {
    return {
...
function evaluateAcceptedRow(row) {
  ...
  const evidence = haloClusterTransferEvidenceStatusForPath(
  ...
  if (!evidence.accepted) {
    return {
      accepted: false,
      reason: `halo_cluster_${evidence.reason}`,
      evidence,
    };
  }
