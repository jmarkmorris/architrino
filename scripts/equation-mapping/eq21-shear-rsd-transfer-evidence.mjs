...
export const EQ21_SHEAR_RSD_TRANSFER_EVIDENCE_SCHEMA =
  "aaa-equation-map-eq21-shear-rsd-transfer-evidence/v1";

export const EQ21_SHEAR_RSD_TRANSFER_ROWS = Object.freeze([
...
export const EQ21_SHEAR_RSD_TRANSFER_KEYS = EQ21_MATTER_POWER_TRANSFER_KEYS;

const ACCEPTED_STATUSES = new Set(["accepted", "passed", "populated"]);
const DEFAULT_TOLERANCE = 1e-12;

export function shearRsdTransferEvidenceStatusForPath(value, { repoRoot }) {
  ...
  const objectStatus = evaluateShearRsdTransferEvidenceObject(raw, {
    repoRoot,
...
export function evaluateShearRsdTransferEvidenceObject(
  raw,
  { repoRoot, sourcePath } = {},
) {
  const missing = [];
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return {
      accepted: false,
      reason: "shear_rsd_transfer_evidence_not_json_object",
      missingOrRejectedFields: ["shear_rsd_transfer_evidence_object"],
    };
  }
  if (raw.schema !== EQ21_SHEAR_RSD_TRANSFER_EVIDENCE_SCHEMA) {
    missing.push("shear_rsd_transfer_evidence_schema");
  }
  const status = raw.evidenceStatus ?? raw.status ?? null;
  if (!ACCEPTED_STATUSES.has(status)) {
    missing.push("shear_rsd_transfer_evidence_status_accepted");
  }
  if (raw.authorization?.acceptedShearRsdTransferChild !== true) {
    missing.push("authorization.acceptedShearRsdTransferChild");
  }
  ...
  const rows = raw.rows ?? {};
  for (const row of EQ21_SHEAR_RSD_TRANSFER_ROWS) {
    requireAcceptedRow({
      row,
      value: rows[row],
      missing,
  ...
  const keyValues = {};
  for (const key of EQ21_SHEAR_RSD_TRANSFER_KEYS) {
  ...
  const model = evaluateShearRsdTransferModel(
    raw.model ?? {},
    matterChild,
    lensingChild,
    growthChild,
  ...
  if (!model.computed) {
    ...
    const residual = finiteNumberOrNull(raw.residualComponents?.shear_rsd_grid_normalized);
    if (residual === null || residual > (raw.tolerances?.residual ?? 1)) {
      missing.push("residualComponents.shear_rsd_grid_normalized.within_tolerance");
    }
  }
  ...
  return {
    ...
    reason:
      missing.length === 0
        ? "accepted"
        : "shear_rsd_transfer_evidence_fields_missing",
...
export function evaluateShearRsdTransferModel(rawModel, matterChild, lensingChild, growthChild) {
  ...
  const kernel = rawModel.shearRsdKernel ?? {};
  const aShear = finiteNumberOrNull(kernel.A_shear);
  const biasEff = finiteNumberOrNull(kernel.bias_eff);
  const sigma8Seed =
    finiteNumberOrNull(growthChild?.model?.sigma8Seed?.value) ??
    finiteNumberOrNull(matterChild?.model?.sigma8Seed?.value);
  if (aShear === null || aShear <= 0) {
    return { computed: false, reason: "model.shearRsdKernel.A_shear_positive" };
  }
  if (biasEff === null || biasEff <= 0) {
    return { computed: false, reason: "model.shearRsdKernel.bias_eff_positive" };
  }
  if (sigma8Seed === null || sigma8Seed <= 0) {
    return { computed: false, reason: "model.shearRsdKernel.sigma8_seed_positive" };
  }

  const samples = Array.isArray(rawModel.samples) ? rawModel.samples : [];
  const uniqueLensingSamples = new Set(samples.map((sample) => sample.lensingSampleId));
  if (
    samples.length < 3 ||
    uniqueLensingSamples.size < 3 ||
    uniqueLensingSamples.has(undefined)
  ) {
    return { computed: false, reason: "model.samples.shear_rsd_grid_minimum" };
  }
  ...
  for (const sample of samples) {
    ...
    const matterSampleId = concreteString(sample.matterSampleId)
      ...
      : null;
    if (!sampleId || !lensingSampleId) {
      return { computed: false, reason: "model.samples.valid_shear_rsd_sample" };
    }
    const lensingSample = lensingSamples.get(lensingSampleId);
    if (!lensingSample) {
    ...
    const growthRate = finiteNumberOrNull(matterSample.growth_rate_f);
    const growthFactor = finiteNumberOrNull(matterSample.growth_factor);
    if (
      ell === null ||
      ell <= 0 ||
      cPhiPhi === null ||
    ...
    const shearBandPower = aShear * ell * (ell + 1) * cPhiPhi;
    const fSigma8Z = growthRate * sigma8Seed * growthFactor;
    const betaRSD = growthRate / biasEff;
    const shearObserved = finiteNumberOrNull(sample.benchmark?.shear_band_power_observed);
    const shearSigma = finiteNumberOrNull(sample.benchmark?.shear_sigma);
    const fSigma8Observed = finiteNumberOrNull(sample.benchmark?.f_sigma8_z_observed);
    const fSigma8Sigma = finiteNumberOrNull(sample.benchmark?.f_sigma8_sigma);
    const betaObserved = finiteNumberOrNull(sample.benchmark?.beta_RSD_observed);
    const betaSigma = finiteNumberOrNull(sample.benchmark?.beta_sigma);
    const sampleResiduals = [];
    if (shearObserved !== null && shearSigma !== null && shearSigma > 0) {
      sampleResiduals.push(Math.abs(shearBandPower - shearObserved) / shearSigma);
    }
    if (fSigma8Observed !== null && fSigma8Sigma !== null && fSigma8Sigma > 0) {
      sampleResiduals.push(Math.abs(fSigma8Z - fSigma8Observed) / fSigma8Sigma);
    }
    if (betaObserved !== null && betaSigma !== null && betaSigma > 0) {
      sampleResiduals.push(Math.abs(betaRSD - betaObserved) / betaSigma);
    }
    if (sampleResiduals.length !== 3) {
      return { computed: false, reason: "model.samples.benchmark_residuals_declared" };
    }
    const normalizedResidual = Math.max(...sampleResiduals);
    ...
    derivedSamples.push({
      ...
      growth_rate_f: growthRate,
      growth_factor: growthFactor,
      f_sigma8_z: fSigma8Z,
      beta_RSD: betaRSD,
      shear_rsd_normalized_residual: normalizedResidual,
    });
  }
  ...
  return {
    ...
    derived: {
      ...
      bias_eff: biasEff,
      sigma8Seed,
      shear_rsd_grid_normalized_residual: Math.max(...residuals),
...
function compareDerived(actual, expected, tolerances, missing) {
  ...
  for (const [key, expectedValue] of Object.entries(expected)) {
    if (key === "samples" || key === "shear_rsd_grid_normalized_residual") {
      continue;
    }
  ...
  for (const expectedSample of expected.samples) {
    ...
    for (const [key, expectedValue] of Object.entries(expectedSample)) {
      if (key === "sampleId" || key === "shear_rsd_normalized_residual") {
        continue;
      }
