...
export const EQ21_HALO_CLUSTER_TRANSFER_EVIDENCE_SCHEMA =
  "aaa-equation-map-eq21-halo-cluster-transfer-evidence/v1";

export const EQ21_HALO_CLUSTER_TRANSFER_ROWS = Object.freeze([
  "halo_cluster_transfer_child",
  "halo_cluster_inversion_row",
...
export const EQ21_HALO_CLUSTER_TRANSFER_KEYS = EQ21_MATTER_POWER_TRANSFER_KEYS;

const ACCEPTED_STATUSES = new Set(["accepted", "passed", "populated"]);
const DEFAULT_TOLERANCE = 1e-12;

export function haloClusterTransferEvidenceStatusForPath(value, { repoRoot }) {
  ...
  const objectStatus = evaluateHaloClusterTransferEvidenceObject(raw, {
    repoRoot,
...
export function evaluateHaloClusterTransferEvidenceObject(
  raw,
  { repoRoot, sourcePath } = {},
) {
  const missing = [];
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return {
      accepted: false,
      reason: "halo_cluster_transfer_evidence_not_json_object",
      missingOrRejectedFields: ["halo_cluster_transfer_evidence_object"],
    };
  }
  if (raw.schema !== EQ21_HALO_CLUSTER_TRANSFER_EVIDENCE_SCHEMA) {
    missing.push("halo_cluster_transfer_evidence_schema");
  }
  const status = raw.evidenceStatus ?? raw.status ?? null;
  if (!ACCEPTED_STATUSES.has(status)) {
    missing.push("halo_cluster_transfer_evidence_status_accepted");
  }
  if (raw.authorization?.acceptedHaloClusterTransferChild !== true) {
    missing.push("authorization.acceptedHaloClusterTransferChild");
  }
  ...
  const rows = raw.rows ?? {};
  for (const row of EQ21_HALO_CLUSTER_TRANSFER_ROWS) {
    requireAcceptedRow({
      row,
      value: rows[row],
      missing,
  ...
  const keyValues = {};
  for (const key of EQ21_HALO_CLUSTER_TRANSFER_KEYS) {
  ...
  const model = evaluateHaloClusterTransferModel(
    raw.model ?? {},
    matterChild,
    lensingChild,
    shearRsdChild,
  ...
  if (!model.computed) {
    ...
    const residual = finiteNumberOrNull(raw.residualComponents?.halo_cluster_grid_normalized);
    if (residual === null || residual > (raw.tolerances?.residual ?? 1)) {
      missing.push("residualComponents.halo_cluster_grid_normalized.within_tolerance");
    }
  }
  ...
  return {
    ...
    reason:
      missing.length === 0
        ? "accepted"
        : "halo_cluster_transfer_evidence_fields_missing",
...
export function evaluateHaloClusterTransferModel(rawModel, matterChild, lensingChild, shearRsdChild) {
  ...
  const inversion = rawModel.haloClusterInversion ?? {};
  if (!ACCEPTED_STATUSES.has(inversion.status ?? null)) {
    return { computed: false, reason: "model.haloClusterInversion.status_accepted" };
  }
  const aLens =
  ...
  if (aLens === null || aLens <= 0) {
    return { computed: false, reason: "model.haloClusterInversion.A_lens_positive" };
  }
  if (aShear === null || aShear <= 0) {
    return { computed: false, reason: "model.haloClusterInversion.A_shear_positive" };
  }
  if (biasEff === null || biasEff <= 0) {
    return { computed: false, reason: "model.haloClusterInversion.bias_eff_positive" };
  }
  if (sigma8Seed === null || sigma8Seed <= 0) {
    return { computed: false, reason: "model.haloClusterInversion.sigma8_seed_positive" };
  }
  ...
  if (
    ...
    return { computed: false, reason: "model.samples.halo_cluster_grid_minimum" };
  }
  ...
  for (const sample of samples) {
    ...
    if (!sampleId || !shearRsdSampleId) {
      return { computed: false, reason: "model.samples.valid_halo_cluster_sample" };
    }
    ...
    derivedSamples.push({
      ...
      P_shear: pFromShear,
      P_RSD: pFromRsd,
      halo_cluster_normalized_residual: normalizedResidual,
    });
  }
  ...
  return {
    ...
    derived: {
      ...
      bias_eff: biasEff,
      sigma8Seed,
      halo_cluster_grid_normalized_residual: Math.max(...residuals),
