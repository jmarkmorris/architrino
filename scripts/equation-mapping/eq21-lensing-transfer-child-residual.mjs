...
import {
  EQ21_LENSING_TRANSFER_KEYS,
  EQ21_LENSING_TRANSFER_ROWS,
  evaluateLensingTransferModel,
  lensingTransferEvidenceStatusForPath,
} from "./eq21-lensing-transfer-evidence.mjs";
...
const DEFAULT_INPUT_PATH = path.join(
  SCRIPT_DIR,
  "eq21-lensing-transfer-child-attempt.v1.json",
);
const INPUT_SCHEMA = "aaa-equation-map-eq21-lensing-transfer-child-input/v1";
const OUTPUT_SCHEMA = "aaa-equation-map-eq21-lensing-transfer-child-check/v1";
...
const output = evaluateEq21LensingTransferChild(input, inputPath);
...
function printHelp() {
  console.log(`Usage: node scripts/equation-mapping/eq21-lensing-transfer-child-residual.mjs [options]

Options:
  --input PATH          EQ-21 CMB-lensing transfer child input JSON.
...
This checker evaluates the score-neutral EQ-21 CMB-lensing transfer child.
It consumes the accepted shared-observation parent, accepted f-sigma8 child,
and accepted normalized P(k,z) child; it does not raise equation scores.`);
}
...
function evaluateEq21LensingTransferChild(input, inputPath) {
  ...
  const model = evaluateModel(packet.model ?? {}, matterChild.raw, tolerances);
  ...
  return {
    ...
    residual: {
      ...
      row: "EQ-21",
      childConsumer: "cmb_lensing_transfer_C_phi_phi",
      claimLevel:
        "score-neutral CMB-lensing transfer child; accepted shared-observation, f-sigma8, and P(k,z) child evidence are required before score review",
    },
    tolerances,
    summary: {
      status,
      ...
      cmbLensingGridResidualPass: model.cmbLensingGridResidualPass,
      noHiddenRetunePass: model.noHiddenRetunePass,
      sampleCount: model.derived?.samples?.length ?? 0,
      cmbLensingGridNormalizedResidual:
        model.derived?.cmb_lensing_grid_normalized_residual ?? null,
    },
...
function summarizeOutput(output) {
  return {
    ...
    lensingModel: {
      computed: output.lensingModel.computed,
      derivedPass: output.lensingModel.derivedPass,
      cmbLensingGridResidualPass: output.lensingModel.cmbLensingGridResidualPass,
      noHiddenRetunePass: output.lensingModel.noHiddenRetunePass,
      derived: output.lensingModel.derived,
    },
  };
}
...
function evaluateSharedKeys(rawKeys, parent, growthChild, matterChild, tolerances) {
  const keyRows = new Map(
    (Array.isArray(rawKeys) ? rawKeys : []).map((row) => [row.key, row]),
  );
  const keys = Object.fromEntries(
    EQ21_LENSING_TRANSFER_KEYS.map((key) => {
      ...
      return [
        key,
        {
          ...
          growthChildValue,
          matterChildValue,
          mismatch: parentMismatch || growthChildMismatch || matterChildMismatch,
  ...
  const missingSharedKeys = EQ21_LENSING_TRANSFER_KEYS.filter(
  ...
  return {
    ...
    allExpectedKeysDeclared: EQ21_LENSING_TRANSFER_KEYS.every((key) =>
      keyRows.has(key),
    ),
    missingSharedKeys,
    expectedKeys: EQ21_LENSING_TRANSFER_KEYS,
...
function evaluateModel(rawModel, matterChildRaw, tolerances) {
  const base = evaluateLensingTransferModel(rawModel, matterChildRaw);
  if (!base.computed) {
    return {
      ...base,
      derivedPass: false,
      cmbLensingGridResidualPass: false,
  ...
  const residualActual = finiteNumberOrNull(
    rawModel.residualComponents?.cmb_lensing_grid_normalized ??
      rawModel.cmb_lensing_grid_normalized,
  );
  const residualExpected = base.derived.cmb_lensing_grid_normalized_residual;
  const cmbLensingGridResidualPass =
    residualActual !== null &&
  ...
  return {
    ...
    derivedMismatches,
    cmbLensingGridResidualPass,
    cmbLensingGridResidualActual: residualActual,
...
function evaluateSourceEvidence({ rows, sharedKeys, model }) {
  const rowEntries = EQ21_LENSING_TRANSFER_ROWS.map((rowId) =>
  ...
  const keyEntries = EQ21_LENSING_TRANSFER_KEYS.map((key) =>
    evaluateSourceEvidenceEntry({
      scope: "shared_key",
      id: key,
  ...
  const modelEntries = [
    evaluateSourceEvidenceEntry({
      scope: "model_input",
      id: "lensingKernel",
      status: model.lensingKernel?.status ?? null,
      sourcePath: model.lensingKernel?.sourcePath ?? model.lensingKernel?.source ?? null,
    }),
  ];
...
function evaluateSourceEvidenceEntry({ scope, id, status, sourcePath }) {
  ...
  const evidence = lensingTransferEvidenceStatusForPath(sourcePath, {
    repoRoot: process.cwd(),
  });
  return {
    scope,
...
function decideStatus({
  ...
  if (!model.computed) {
    return "blocked_lensing_model_not_computed";
  }
  if (!model.derivedPass) {
    return "blocked_lensing_model_derived_mismatch";
  }
  if (!model.cmbLensingGridResidualPass) {
    return "blocked_cmb_lensing_grid_residual";
  }
...
function firstBlocker({
  ...
  if (!sourceEvidence.passed) {
    return "accepted_without_lensing_transfer_evidence";
  }
  if (!parent.accepted) {
  ...
  if (!model.computed) {
    return model.reason ?? "lensing_model_not_computed";
  }
  if (!model.derivedPass) {
    return `lensing_model_derived_mismatch_${model.derivedMismatches[0]?.key ?? "unknown"}`;
  }
  if (!model.cmbLensingGridResidualPass) {
    return "cmb_lensing_grid_residual_failed";
  }
  if (!model.noHiddenRetunePass) {
    return "lensing_child_hidden_retune";
  }
  return status;
}
...
function firstBlockerDetails({
  ...
  if (nextBlocker === "accepted_without_lensing_transfer_evidence") {
    return {
...
function evaluateAcceptedRow(row) {
  ...
  const evidence = lensingTransferEvidenceStatusForPath(
  ...
  if (!evidence.accepted) {
    return {
      accepted: false,
      reason: `lensing_${evidence.reason}`,
      evidence,
    };
  }
