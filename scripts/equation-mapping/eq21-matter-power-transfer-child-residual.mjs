...
import {
  ...
  EQ21_MATTER_POWER_TRANSFER_ROWS,
  evaluateMatterPowerModel,
...
const DEFAULT_INPUT_PATH = path.join(
  SCRIPT_DIR,
  "eq21-matter-power-transfer-child-attempt.v1.json",
);
const INPUT_SCHEMA = "aaa-equation-map-eq21-matter-power-transfer-child-input/v1";
const OUTPUT_SCHEMA = "aaa-equation-map-eq21-matter-power-transfer-child-check/v1";
...
const output = evaluateEq21MatterPowerTransferChild(input, inputPath);
...
function printHelp() {
  console.log(`Usage: node scripts/equation-mapping/eq21-matter-power-transfer-child-residual.mjs [options]

Options:
  --input PATH          EQ-21 matter-power transfer child input JSON.
...
This checker evaluates the score-neutral EQ-21 matter-power transfer child.
It computes a multi-point P(k,z) grid from the accepted shared-observation
record and accepted f-sigma8 growth child; it does not raise equation scores.`);
}
...
function evaluateEq21MatterPowerTransferChild(input, inputPath) {
  const tolerances = parseTolerances(input.tolerances ?? {});
  const packet = input.packet ?? input;
  const rows = packet.rows ?? {};
  const rowChecks = Object.fromEntries(
    EQ21_MATTER_POWER_TRANSFER_ROWS.map((rowId) => [
      rowId,
      evaluateAcceptedRow(rows[rowId]),
    ]),
  );
  const missingRows = EQ21_MATTER_POWER_TRANSFER_ROWS.filter(
  ...
  return {
    ...
    residual: {
      ...
      row: "EQ-21",
      childConsumer: "matter_power_transfer_P_k_z",
      claimLevel:
        "score-neutral predictive matter-power transfer child; accepted shared-observation and f-sigma8 child evidence are required before score review",
    },
    tolerances,
    summary: {
      status,
      ...
      matterPowerGridResidualPass: model.matterPowerGridResidualPass,
      noHiddenRetunePass: model.noHiddenRetunePass,
      sampleCount: model.derived?.samples?.length ?? 0,
      kEq: model.derived?.k_eq ?? null,
      kDamp: model.derived?.k_damp ?? null,
      matterPowerGridNormalizedResidual:
        model.derived?.matter_power_grid_normalized_residual ?? null,
    },
...
function summarizeOutput(output) {
  return {
    ...
    matterPowerModel: {
      computed: output.matterPowerModel.computed,
      derivedPass: output.matterPowerModel.derivedPass,
      matterPowerGridResidualPass: output.matterPowerModel.matterPowerGridResidualPass,
      noHiddenRetunePass: output.matterPowerModel.noHiddenRetunePass,
      derived: output.matterPowerModel.derived,
    },
  };
}
...
function evaluateSharedKeys(rawKeys, parent, growthChild, tolerances) {
  const keyRows = new Map(
    (Array.isArray(rawKeys) ? rawKeys : []).map((row) => [row.key, row]),
  );
  const keys = Object.fromEntries(
    EQ21_MATTER_POWER_TRANSFER_KEYS.map((key) => {
      ...
      return [
        key,
        {
          ...
          growthChildValue,
          mismatch: parentMismatch || growthChildMismatch,
  ...
  const missingSharedKeys = EQ21_MATTER_POWER_TRANSFER_KEYS.filter(
  ...
  return {
    ...
    allExpectedKeysDeclared: EQ21_MATTER_POWER_TRANSFER_KEYS.every((key) =>
      keyRows.has(key),
    ),
    missingSharedKeys,
    expectedKeys: EQ21_MATTER_POWER_TRANSFER_KEYS,
...
function evaluateModel(rawModel, keyValues, growthChildRaw, tolerances) {
  const base = evaluateMatterPowerModel(rawModel, keyValues, growthChildRaw);
  if (!base.computed) {
    return {
      ...base,
      derivedPass: false,
      matterPowerGridResidualPass: false,
  ...
  const residualActual = finiteNumberOrNull(
    rawModel.residualComponents?.matter_power_grid_normalized ??
      rawModel.matter_power_grid_normalized,
  );
  const residualExpected = base.derived.matter_power_grid_normalized_residual;
  const matterPowerGridResidualPass =
    residualActual !== null &&
  ...
  return {
    ...
    derivedMismatches,
    matterPowerGridResidualPass,
    matterPowerGridResidualActual: residualActual,
...
function evaluateSourceEvidence({ rows, sharedKeys, model }) {
  const rowEntries = EQ21_MATTER_POWER_TRANSFER_ROWS.map((rowId) =>
  ...
  const keyEntries = EQ21_MATTER_POWER_TRANSFER_KEYS.map((key) =>
    evaluateSourceEvidenceEntry({
      scope: "shared_key",
      id: key,
  ...
  const modelEntries = [
    evaluateSourceEvidenceEntry({
      scope: "model_input",
      id: "sigma8Seed",
      status: model.sigma8Seed?.status ?? null,
      sourcePath: model.sigma8Seed?.sourcePath ?? model.sigma8Seed?.source ?? null,
    }),
    evaluateSourceEvidenceEntry({
      scope: "model_input",
      id: "seedSpectrum",
      status: model.seedSpectrum?.status ?? null,
      sourcePath: model.seedSpectrum?.sourcePath ?? model.seedSpectrum?.source ?? null,
    }),
    evaluateSourceEvidenceEntry({
      scope: "model_input",
      id: "transferKernel",
      status: model.transferKernel?.status ?? null,
      sourcePath: model.transferKernel?.sourcePath ?? model.transferKernel?.source ?? null,
    }),
  ];
...
function evaluateSourceEvidenceEntry({ scope, id, status, sourcePath }) {
  ...
  const evidence = matterPowerTransferEvidenceStatusForPath(sourcePath, {
    repoRoot: process.cwd(),
  });
  return {
    scope,
...
function decideStatus({
  ...
  if (!model.computed) {
    return "blocked_matter_power_model_not_computed";
  }
  if (!model.derivedPass) {
    return "blocked_matter_power_model_derived_mismatch";
  }
  if (!model.matterPowerGridResidualPass) {
    return "blocked_matter_power_grid_residual";
  }
...
function firstBlocker({
  ...
  if (!sourceEvidence.passed) {
    return "accepted_without_matter_power_transfer_evidence";
  }
  if (!parent.accepted) {
  ...
  if (!model.computed) {
    return model.reason ?? "matter_power_model_not_computed";
  }
  if (!model.derivedPass) {
    return `matter_power_model_derived_mismatch_${model.derivedMismatches[0]?.key ?? "unknown"}`;
  }
  if (!model.matterPowerGridResidualPass) {
    return "matter_power_grid_residual_failed";
  }
  if (!model.noHiddenRetunePass) {
    return "matter_power_child_hidden_retune";
  }
  return status;
}
...
function firstBlockerDetails({
  ...
  if (nextBlocker === "accepted_without_matter_power_transfer_evidence") {
    return {
  ...
  if (nextBlocker.startsWith("hidden_retune_")) {
    return {
      reason: "shared_key_value_differs_from_parent_growth_projection_or_growth_child",
...
function evaluateAcceptedRow(row) {
  ...
  const evidence = matterPowerTransferEvidenceStatusForPath(
  ...
  if (!evidence.accepted) {
    return {
      accepted: false,
      reason: `matter_power_${evidence.reason}`,
      evidence,
    };
  }
