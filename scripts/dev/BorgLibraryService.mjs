...
export function createBorgLibraryService({ repoRoot, catalog = BORG_BRAID_RECORD_CATALOG.entries } = {}) {
  const cache = new Map();
  ...
  async function load(entry) {
    ...
    const described = describeLibraryRecord(JSON.parse(bytes), entry, recordSha256);
  ...
  async function route(req, res, url) {
    ...
    try {
      ...
      for (const entry of catalog) {
        try { rows.push((await load(entry)).described.summary); }
      ...
      const snapshot = digest(JSON.stringify([rows.map((row) => row.recordSha256), canonical.toString()])).slice(0, 24);
      let offset = 0;
