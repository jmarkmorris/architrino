...
const schema = 'architrino-agent-dispatch/v1';
const fail = message => { throw new Error(message); };
export const sha256 = bytes => createHash('sha256').update(bytes).digest('hex');
export function requireHash(value) {
  if (typeof value !== 'string' || value.length !== 64 || !/^[0-9a-f]{64}$/.test(value)) fail('SHA-256 must be exactly 64 lowercase hexadecimal characters');
  return value;
}
...
function source(root, name) {
  ...
  if (!resolved.startsWith(root + path.sep)) fail('source escapes repository root');
  if (!statSync(resolved).isFile()) fail('source must be a regular file');
  const bytes = readFileSync(resolved);
  return { path: name, bytes: bytes.length, sha256: sha256(bytes) };
}
function messageFor(body, sources) {
...
export function prepareDispatch({ root, body, sources }) {
  ...
  const records = sources.map(item => {
    ...
    if (Object.hasOwn(item, 'expectedSha256') && record.sha256 !== item.expectedSha256) fail(`source hash mismatch: ${item.path}`);
    return record;
  });
  const message = messageFor(body, records);
  return { schema, root, body, sources: records, message, messageBytes: Buffer.byteLength(message), messageSha256: sha256(Buffer.from(message, 'utf8')) };
}
export function verifyDispatch(packet) {
  ...
  const fresh = prepareDispatch({ root: packet.root, body: packet.body, sources: packet.sources.map(row => ({ path: row.path, expectedSha256: row.sha256 })) });
...
export function compareReceived(packet, receivedBytes) {
  ...
  requireHash(packet.messageSha256);
  text(packet.message, 'recorded message');
