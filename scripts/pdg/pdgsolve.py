...
ASSEMBLY_DISPLAY = {
    "pro_noether_braid_I": {
        ...
        "pdgeditType": "pro-noether-braid-assembly",
        "tiles": [
            "pro-noether-braid",
            "binary-bare-br-none",
            "binary-bare-br-none",
    ...
    "anti_noether_braid_I": {
        "title": "Anti Noether Braid",
        "pdgeditType": "anti-noether-braid-assembly",
        "tiles": [
            "anti-noether-braid",
            "binary-bare-rb-none",
            "binary-bare-rb-none",
...
def get_noether_braid_for_assembly(assembly_id: str) -> str | None:
    generation = get_generation_suffix(assembly_id)
    if assembly_id.startswith("pro_"):
        return f"pro_noether_braid_{generation}"
    if assembly_id.startswith("anti_"):
        return f"anti_noether_braid_{generation}"
    return None


def get_reachable_noether_braid_ids_for_assembly(assembly_id: str) -> tuple[str, ...]:
    direct_core_assembly_id = get_noether_braid_for_assembly(assembly_id)
    ...
    while current_core_assembly_id:
        ...
        current_core_assembly_id = normalize_text(NOETHER_BRAID_SUCCESSOR.get(current_core_assembly_id))
...
def build_targeted_dissociation_residue_counts(
    ...
    if target_core_assembly_id not in get_reachable_noether_braid_ids_for_assembly(assembly_id):
        return None
...
def select_dissociation_target_core_assembly(
    ...
    reachable_core_ids = get_reachable_noether_braid_ids_for_assembly(assembly_id)
...
def plan_middle_supply_counts(
    ...
    middle_supply_counts = {assembly_id: 0 for assembly_id in NOETHER_BRAID_SUCCESSOR}
    remaining_core_demand = {
        assembly_id: max(0, product_core_counts[assembly_id] - reactant_core_counts[assembly_id])
        for assembly_id in NOETHER_BRAID_SUCCESSOR
    }
...
def build_standard_model_law_inventory() -> dict[str, Any]:
    ...
    for assembly_id, residue_counts in FERMION_RESIDUE_COUNTS.items():
        core_assembly_id = get_noether_braid_for_assembly(assembly_id)
    ...
    for assembly_id, successor_assembly_id in NOETHER_BRAID_SUCCESSOR.items():
...
def is_noether_braid_assembly(assembly_id: str) -> bool:
    return normalize_text(assembly_id) in NOETHER_BRAID_SUCCESSOR


def build_core_support_occurrence(charge: str, generation: str, index: int) -> dict[str, Any]:
    assembly_id = f"{charge}_noether_braid_{generation}"
    return {
        "id": f"support.{charge}_noether_braid_{generation}.{index}",
        "assemblyId": assembly_id,
...
def count_core_occurrences(occurrences: list[dict[str, Any]]) -> dict[str, int]:
    counts = {assembly_id: 0 for assembly_id in NOETHER_BRAID_SUCCESSOR}
    for occurrence in occurrences:
        assembly_id = normalize_text(occurrence.get("assemblyId"))
...
def compute_added_tri_binary_support_pair_count(
    ...
    for charge in ("pro", "anti"):
        remaining_required = {
            generation: int(product_core_counts[f"{charge}_noether_braid_{generation}"] or 0)
            for generation in ("I", "II", "III")
        }
        available_direct = {
            generation: int(reactant_core_counts[f"{charge}_noether_braid_{generation}"] or 0)
            for generation in ("I", "II", "III")
        }
        available_middle = {
            generation: int(middle_supply_counts[f"{charge}_noether_braid_{generation}"] or 0)
            for generation in ("I", "II", "III")
        }
...
def build_direct_core_supply_from_reactant_cores(
    ...
    *,
    ...
    available_core_pool: dict[str, list[dict[str, Any]]] = {
        assembly_id: [] for assembly_id in NOETHER_BRAID_SUCCESSOR
    }
    ...
    for charge in ("pro", "anti"):
        source_to_targets = (
            (f"{charge}_noether_braid_III", (f"{charge}_noether_braid_III",)),
            (f"{charge}_noether_braid_II", (f"{charge}_noether_braid_III", f"{charge}_noether_braid_II")),
            (
                f"{charge}_noether_braid_I",
                (
                    f"{charge}_noether_braid_III",
                    f"{charge}_noether_braid_II",
                    f"{charge}_noether_braid_I",
                ),
            ),
        )
...
def build_product_task(product: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if is_noether_braid_assembly(assembly_id):
        return {
            "product": clone_json(product),
...
def add_single_dissociation(
    ...
    *,
    ...
    core_assembly_id = normalize_text(target_core_assembly_id) or get_noether_braid_for_assembly(assembly_id)
    if core_assembly_id is None:
        return None
    ...
    reactant_operator_choices.append(
        build_operator_choice(
            ...
            law_id=(
                ...
                if core_assembly_id == get_noether_braid_for_assembly(assembly_id) and get_dissociation_law(assembly_id) is not None
...
def build_exact_family(problem: dict[str, Any]) -> dict[str, Any] | None:
    ...
    remaining_reactant_cores = [
        ...
        if is_noether_braid_assembly(normalize_text(occurrence.get("assemblyId")))
    ]
    remaining_non_core_reactants = [
        clone_json(occurrence)
        for occurrence in remaining_reactants
        if not is_noether_braid_assembly(normalize_text(occurrence.get("assemblyId")))
    ]
    core_product_tasks = [
        clone_json(task)
    ...
    product_core_counts = {
        assembly_id: 0 for assembly_id in NOETHER_BRAID_SUCCESSOR
    }
    ...
    for index in range(1, support_pair_count + 1):
        ...
        reactant_core_counts["pro_noether_braid_I"] += 1
        reactant_core_counts["anti_noether_braid_I"] += 1
