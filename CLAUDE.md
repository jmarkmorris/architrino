...
- **Explanation standard.** [reference/op/operator-explanation-standard.md](reference/op/operator-explanation-standard.md) is the sole authority for operator-facing explanation and this floor does not restate it. Until it is read: the operator is an expert in the theory and a non-specialist in whatever external framework is being imported, so assume $\mathbb{A}\mathbb{A}\mathbb{A}$ and define every imported term in place; write technical prose plainly in the first instance rather than densely with a gloss appended; and treat response length as unconstrained, which outranks any client-level brevity setting.
- **Close substantive responses with a concise `Closure goal:` line,** and open every generated or recommended prompt with one.
- **Ask necessary questions one at a time,** with fixed choices in ranked order, preferred choice first, ending in an explicit option prompt such as `(y/n)` or `(a/b)`. Do not address the agent by name; when a role label is useful, use `Principal Proof Architect & Integrator`. End every substantive response with an `Open items:` block carrying forward each decision awaiting the operator, or `Open items: none.` These are floor restatements; the explanation standard above owns the full rules.
