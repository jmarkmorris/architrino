...
- Use the installed Browser skill and its documented APIs. Follow its browser selection, permission, and safety rules; stop if the requested action is blocked. This skill does not authorize an alternate browser or permission bypass. Do not navigate to local file URLs; use the helper's loopback URL for the deliberately limited preview. The dedicated port is intentional because the artifact is outside the web-app tree. Do not restart the shared app server for this workflow.
