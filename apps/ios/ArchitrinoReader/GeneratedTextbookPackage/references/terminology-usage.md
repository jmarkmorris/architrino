...
Use `Noether braid` as the canonical reader-facing term for a neutral braided assembly scaffold. The base, three-axis circular-chart, and common-axis three-binary-chart inventories contain six architrinos in three neutral binaries; two-component circular charts contains twelve architrinos in six neutral binaries. The term names the retained top-level branch and its causal-return ledger before an exact family member or coordinate record has been certified.
...
| a three-axis circular-chart or common-axis three-binary-chart defining inventory | `six architrino worldlines in three neutral binaries` | `core orbitals` or an unspecified six-site inventory | "The B1 record declares six architrino worldlines in three neutral binaries." |
| a two-component circular-chart defining inventory | `twelve architrino worldlines in six neutral binaries` | `twelve-core`, `dual core`, or treating the two ordered index subsets as B1 components without declaring that constraint | "The C2 record declares twelve coaxial architrino worldlines in six neutral binaries." |
...
| the common-midpoint, coincident-axis, common-frequency one-braid member | `B1` or `common-axis three-binary-chart member B1` | old shape-based family aliases | "B1 has coincident binary axes and one common frequency." |
| the fully symmetric three-axis circular-chart member | `A2` or `three-axis circular-chart member A2` | a support-band name that hides the equal-coordinate conditions | "A2 has three cyclically equivalent binaries." |
| the zero-axial-offset three-axis circular-chart member or one of its indexed frequency variants | `A1`, `A1.3`, or the exact applicable member ID | `inner/middle/outer` naming or an old shape-based family alias | "A1.3 has the indexed frequency ratio $f_1:f_2:f_3=4:2:1$ and $h_a=0$." |
| the general axial-decomposition three-axis circular-chart member or one of its indexed variants | `A3`, `A3.3`, or the exact applicable member ID | treating nonzero endpoint-orbit-center offsets as A1 | "A3.3 has the indexed frequency ratio $f_1:f_2:f_3=4:2:1$." |
...
Use `Noether braid` for one complete braid record. A base, three-axis circular-chart, or common-axis three-binary-chart record has six architrinos; a two-component circular-chart record has twelve worldlines in one top-level record. Use `particle` mainly at the effective or comparative level.
...
Do not use `Noether Pair` as a canonical ontology term because the name does not say which of these relations is intended. Use `assembly of two Noether braids` when only multiplicity is known, `pro/anti-orientation Noether-braid composite` when $R_o$ alone is the defining relation, and `a braid and its polarity-conjugate braid` when $R_C$ is the defining relation. Use a two-component circular-chart identifier only for one shared twelve-worldline top-level record satisfying the applicable two-component circular-chart chart; do not infer two-component circular charts from two separate six-worldline records. Likewise, do not call the assembly a photon unless the photon-specific planarity, polarity-conjugation, circulation, propagation, helicity, and binding conditions are all declared.
