...
| Degree-action proportionality $\Delta L_k\propto N_k$ | Open (modeling ansatz, not yet a topological theorem) |
...
- and whether a direct variational or delay-map bifurcation argument can force $s=2$ without putting it in by hand.

## Post-Poincare Reduction Target

The strongest next move is no longer another ledger identity. It is a reduced delayed phase model that can test whether the dyadic lock is dynamically selected.
...
$$
Then construct a branch-coupled Poincare return map
$$
P:(\phi_1,\phi_2)_n \mapsto (\phi_1,\phi_2)_{n+1},
$$
sampled once per outer-cycle crossing. The forcing terms must retain the delayed causal structure and, in particular, the Jacobian weighting
$$
\frac{1}{|J|} = \frac{1}{\left|1-\hat{\mathbf{r}}\cdot \mathbf{v}/c_f\right|},
$$
because the near-separatrix amplification is the candidate mechanism that could widen the $1{:}2$ resonance tongue and suppress phase drift.

The concrete theorem target is:

- prove or numerically demonstrate that the $1{:}2$ fixed point in the reduced map is stable as the relevant branch approaches $v=c_f$,
- then determine whether the tri-binary extension yields a stable chained lock $(\phi_1^*,\phi_2^*)$ corresponding to $1{:}2{:}4$,
- and only after that promote the dyadic hierarchy from plausible closure to selected dynamics.
...
1.  **The Matching Condition:** Write the delay equation for the Middle binary forced by the wake of the Outer binary. Test whether the reduced phase map captures stable locking near $f_M = 2f_O$ when $r_M \approx r_O/2$.
...
This 1:2:4 lock remains a plausible working candidate for a Planck-scale assembly "ground-state-like" configuration. The current note supports that claim kinematically and topologically only in a conditional sense; the next stage must show dynamical selection in the reduced delayed map.
