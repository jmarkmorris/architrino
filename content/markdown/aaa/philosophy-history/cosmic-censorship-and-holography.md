...
The same distinction applies to [Ryu-Takayanagi entropy](https://arxiv.org/abs/hep-th/0603001), which relates boundary entanglement entropy to homologous bulk minimal surfaces on a static slice at leading classical-gravity order in its AdS/CFT setting, and to [island and replica-wormhole calculations](https://arxiv.org/abs/1911.12333), which recover Page-curve behavior of fine-grained radiation entropy in controlled gravitational models; the cited calculation uses two-dimensional Jackiw–Teitelboim gravity coupled to matter and a nongravitating radiation region. These are comparison results, not derivations of an architrino horizon. Their useful question is which entropy and encoding relations a derived native interface can reproduce in the corresponding effective regime.
...
| Which bulk observables admit boundary encoding? | A proposed terminal-alignment interface may support a reduced description of selected assembly and medium records | The encoded observables, retained information, and reconstruction domain |
...
The table states comparison questions, not three identified physical regions. A native self-hit regime requires actual intersections of an architrino with its own earlier wake, with the same root-completeness and singular-event treatment required elsewhere. Speed above the primitive causal-wake speed $c_f$ alone is not a self-hit certificate, and equality to $c_f$ alone does not establish an event horizon. The primitive wake speed is not thereby a universal terminal constituent speed.
...
Claim grade: guessed. The alignment-to-horizon identification is a candidate mechanism, not a derived interface or an AdS/CFT duality. For a declared candidate family that claims alignment is sufficient, a retained solution within that family satisfying its alignment and preparation assumptions but failing its predicted signal-access or observer-export conditions falsifies that sufficiency claim. The comparison conditions are specified in [Black Holes](../spacetime/black-holes.md). No retained solution satisfying those conditions is supplied here.
...
**What Is Reclassified:** Under $\mathbb{A}\mathbb{A}\mathbb{A}$, geometric and metric quantities are intended to arise as coarse-grained summaries of Noether sea response, clock-rate variation, and signal structure in an underlying Euclidean substrate. This is the proposed level assignment; a particular metric or holographic dictionary still requires derivation.
