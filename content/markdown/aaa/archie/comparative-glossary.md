...
| Assembly | No exact single standard term; overlaps partially with particle, bound state, composite, or excitation depending on context. | Canonical AAA ontological term for a localized architrino configuration, from a single architrino up through bound hierarchical structures such as binaries, tri-binaries, and higher composites. Use `assembly` when the text means the object itself rather than only its observer-level phenomenology. |
...
| Field (when used as a default AAA substrate term) | Fundamental or effective dynamical object in classical field theory / QFT. | Avoid as the default ontological term for emitted architrino influence. Prefer `wake` or `causal wake` for substrate-level AAA prose. Keep `field` for QFT comparison, effective continuum description, or observer-level translation when that is the real topic. |
...
| Particle | Localized excitation, detector event, or asymptotic object in standard quantum and particle-physics language. | Keep mainly as an observer-level, experimental, or Standard Model comparative term. In native AAA ontology, what is often called a particle is usually an assembly together with its emitted wake structure. |
| Particle-Wave Duality | Standard quantum tension in which the same object exhibits particle-like localization and wave-like interference. | Not treated as a fundamental ontological paradox. AAA responds with `assembly and wake`: the assembly is the localized source-organized object, while the wake is the distributed causal structure that propagates, overlaps, and interferes. |
...
| Polar Site | No direct standard-model equivalent. | One of the six local attachment/dwelling positions occupied by the axial architrinos. Use `polar` for site geometry and `axial` for the six-site system-level organization. |
...
| Potential Fluctuation | Typically treated as field/vacuum fluctuations in QFT language. | Context-dependent $\mathbb{A}\mathbb{A}\mathbb{A}$ term for transient, local potential-pattern variation in the architrino/Noether-sea system; use when "virtual particle" language would be misleading. |
...
| Wave | Distributed oscillatory or amplitude-carrying structure in classical or quantum language; often part of wavefunction or field descriptions. | Use with care. In AAA substrate ontology, the preferred term for emitted causal structure is `wake`, not `wave`. Keep `wave` for standard QM comparison, observer-level phenomenology, or explicitly effective descriptions where that is the intended level. |
...
| field line (in AAA substrate prose) | line in a continuum field picture | line of action; radial line of action | The canonical AAA interaction law is radial and wake-based rather than line-of-force based. |
...
| active triad | weak-coupling triad (interim alias) | Weak-Coupling Triad | Canonical $\mathbb{A}\mathbb{A}\mathbb{A}$ weak-interaction term. |
...
| particle-wave duality (as native ontology) | standard quantum paradox language | assembly and wake | In AAA the localized object and the distributed propagation structure are complementary components of one causal picture, not mutually exclusive identities. |
