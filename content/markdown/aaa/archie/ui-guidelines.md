...
| Close behavior | Search closes on `Escape`, outside pointerdown, or focus leaving the panel; detail closes with an explicit close button; composer menus close on outside pointerdown | The markdown panel currently has no dedicated `Close` button in the shipped HTML despite the Archie doc describing one |
...
| Keyboard navigation affordances | Support `Cmd/Ctrl + K`, `Enter`, and `Escape` for search; support wrapped arrow-key element navigation when search is closed and no text input is focused | Composer also binds `Space` to playback toggle, and Hyde periodic hotspots expose their own arrow-key navigation model |
...
| Icon style | Prefer simple monoline SVG icons with `currentColor` stroke/fill, paired with compact uppercase labels when text is needed | Composer docs uses an emoji icon, element navigation uses pure triangles, Hyde overlay uses text/glyph keycaps |
