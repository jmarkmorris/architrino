...
| Sphere ring action signal | Use purple glow rings only for actionable spheres: child-scene navigation, markdown/file opens, image-gallery opens, and animator-panel opens. Non-clickable spheres that render a ring use a black ring. |  |
...
| Global navigation buttons   | Include `Back`, `Forward`, `Home`, `Search`, `Info`, and `Notes`, using 32x32 circular dark-shell icon buttons in the main scene chrome                                                   | animator action controls use text buttons instead of 32x32 circular icon buttons                                 |
