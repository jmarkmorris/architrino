# Legal Terms

This page gives a plain-language orientation to the legal terms for using Architrino materials. If this page conflicts with the repository license or a specific third-party attribution, the controlling license or attribution governs.

The simple version is that the project is open for inspection and improvement, but the material is not guaranteed, not professional advice, and not a substitute for checking the controlling license, attribution, or source status.

Basic terms: Architrino is open source, published in good faith, and offered for inspection and improvement without a warranty. Use the material at your own risk, keep required license and attribution notices with copied material, and do not treat explanations, apps, simulations, generated media, or diagrams as professional advice or proof by themselves.

## License

Project-authored Architrino code and documents are open source under the [MIT License](../../../../LICENSE), Copyright (c) 2026 J Mark Morris. Bundled third-party libraries, datasets, images, and derivatives retain their own licenses and attribution requirements; see [Licenses, Attribution & Source Use](licenses-attributions.md).

In broad terms, the MIT License allows use, copying, modification, merging, publication, distribution, sublicensing, and sale of copies of the software, as long as the copyright notice and permission notice are included in copies or substantial portions of the software.

The MIT License also states that the software is provided "as is," without warranty of any kind.

## Good-Faith Publication

Architrino publishes research materials, software, generated media, app surfaces, and supporting documentation in good faith for inspection, study, discussion, and improvement.

The material may contain errors, incomplete derivations, open proof burdens, stale surfaces, experimental apps, candidate mechanisms, or claims that are explicitly marked as priority-only, speculative, comparative, diagnostic, or unsupported.

Readers and users should inspect source status, claim labels, validation notes, and open burdens before relying on any material.

## Use At Your Own Risk

Use Architrino materials at your own risk.

The project does not warrant that its software, documents, simulations, generated media, explanations, diagrams, or app outputs are accurate, complete, current, secure, fit for a particular purpose, or suitable for a specific use.

The material is educational and research-oriented. It is not personal medical, legal, financial, safety, engineering, or operational advice.

## Third-Party Material

Some assets, data, libraries, and references used by the project have their own licenses, permissions, or attribution requirements. See [Licenses, Attribution & Source Use](licenses-attributions.md) for the public attribution inventory and source-use boundaries.

When a third-party license, attribution, or source-specific term applies, users are responsible for complying with that term.

## Generated Media

Generated text, audio, images, diagrams, storyboards, captions, transcripts, and future media are explanatory artifacts, not evidence by themselves.

Generated media must preserve source status, claim labels, and public-suitability standards. It should not be treated as proof, validation, endorsement, professional advice, or a guarantee of correctness.

## Public Conduct

Use the public project surfaces in a way that supports inspection, critique, and constructive improvement.

Do not use Architrino materials to harass, mislead, impersonate, violate privacy, violate rights, or present unsupported claims as established results.
