...
The document is not only a usage sheet in the narrow copyediting sense. It records why certain terms are preferred, what level-boundary errors those preferences prevent, and how $\mathbb{A}\mathbb{A}\mathbb{A}$ native wording relates to older Standard Model or relativistic vocabulary. It is therefore the canon-and-rationale reference for durable terminology decisions.
...
| an orthogonal-axis three-binary or coincident-axis three-binary defining inventory | `six architrino worldlines in three neutral binaries` | `core orbitals` or an unspecified six-site inventory | "The coincident-axis three-binary braid record declares six architrino worldlines in three neutral binaries." |
| a two-component circular defining inventory | `twelve architrino worldlines in six neutral binaries` | `twelve-core`, `dual core`, or treating the two ordered index subsets as coincident-axis three-binary braid components without declaring that constraint | "The counter-rotating coincident-center record declares twelve coaxial architrino worldlines in six neutral binaries." |
...
| the common-midpoint, coincident-axis, common-frequency one-braid member | `coincident-axis three-binary braid` or `coincident-axis three-binary member coincident-axis three-binary braid` | old shape-based family aliases | "coincident-axis three-binary braid has coincident binary axes and one common frequency." |
| the fully symmetric orthogonal-axis three-binary member | `phase-compensated equal-geometry orthogonal-axis braid` or `orthogonal-axis three-binary member phase-compensated equal-geometry orthogonal-axis braid` | a support-band name that hides the equal-coordinate conditions | "phase-compensated equal-geometry orthogonal-axis braid has three cyclically equivalent binaries." |
| the zero-axial-offset orthogonal-axis three-binary member or one of its indexed frequency variants | `coincident-midpoint orthogonal-axis braid`, `coincident-midpoint orthogonal-axis 4:2:1 braid`, or the exact applicable member ID | `inner/middle/outer` naming or an old shape-based family alias | "coincident-midpoint orthogonal-axis 4:2:1 braid has the indexed frequency ratio $f_1:f_2:f_3=4:2:1$ and $h_a=0$." |
| the general axial-decomposition orthogonal-axis three-binary member or one of its indexed variants | `axially separated orthogonal-axis braid`, `axially separated orthogonal-axis 4:2:1 braid`, or the exact applicable member ID | treating nonzero endpoint-orbit-center offsets as coincident-midpoint orthogonal-axis braid | "axially separated orthogonal-axis 4:2:1 braid has the indexed frequency ratio $f_1:f_2:f_3=4:2:1$." |
...
Use `Noether braid` for one complete braid record. A base, orthogonal-axis three-binary, or coincident-axis three-binary record has six architrinos; a two-component circular record has twelve worldlines in one top-level record. Use `particle` mainly at the effective or comparative level.
