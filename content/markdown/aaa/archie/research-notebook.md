...
The runtime moved the app beyond a hand-drawn demonstration. The standalone page defaulted to a live `pairInteraction` replay path when its engine was available, with the representative mock replay preserved as an explicit `?replay=mock` review mode. The live path advanced the electrino and positrino together from declared initial positions and velocities, publishes frame samples and path-history data, and leaves the canvas runtime to derive the two live wake series by back-solving emission points against the current reception points. Both live and mock paths preserve a solver-shaped dataset contract: frames, retained path-history samples, delayed-hit diagnostics, rejected or unresolved root diagnostics, and compact contribution summaries.
