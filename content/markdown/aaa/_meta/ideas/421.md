...
The next subsection records an action-partition hypothesis: each outer 1 Hz step transacts a total of $h$ across the tri-binary, with the inner increment fixed at twice the outer increment.

---

**Geometric Derivation of Inner-Weighted Action Partition**

This hypothesis targets the **symplectic structure** of the master equation by introducing a constrained action partition across branches.

From a geometric-analysis perspective, this is treated as a consequence of **causal root counting**.

The following breakdown motivates an inner-weighted action partition in the self-hit regime.
...
*   **Result:** The phase-space area doubles.
    $$
    \Delta L_{\text{tot}} = \Delta L_O + \Delta L_M + \Delta L_I = h,\qquad \Delta L_I = 2\Delta L_O
    $$
...
If the inner branch receives twice the outer increment while the total increment remains fixed at $h$, the Inner Binary is **mechanically stiffer** (higher effective bulk modulus).
*   **Outer/Middle (fractional $h$ shares):** softer response.
*   **Inner (largest share):** harder response under fixed total action.
...
$$
r_{\text{Inner}} \propto \frac{1}{\text{Stiffness}}
$$
...
This inner-weighted partition has implications for particle identity.
...
**Hypothesis:** The Inner Binary, operating as the highest-share branch under the global-$h$ ledger, may be a geometric origin of **graviton-like behavior** or tensor perturbations. While the total assembly (Fermion) may sum to spin-1/2 due to vector cancellations (the $120^\circ$ phase), the core energy-storage channel couples strongly to metric deformation.

**Conclusion:** In this framing, the field-speed horizon $v=c_f$ acts as a **frequency-to-action partition interface**.
*   Input: one outer-step transacts total action $h$.
*   Partition rule: $\Delta L_I = 2\Delta L_O$ with $\Delta L_O+\Delta L_M+\Delta L_I=h$.
*   Effect: inner branch receives the largest share while preserving global conservation.
...
This corresponds to $\beta(f) \propto \sqrt{f}$ on the self-hit branch (up to normalization). It is a robust geometric prediction: if the Inner Binary is driven to higher frequencies, its radius shrinks until it reaches the **MCB floor** (the universal max-curvature limit), after which it cannot contract further.
...
An analytic derivation is available. In this construction, the profile (high-velocity decay toward a tangent of 1 followed by secular acceleration) is obtained from the **Tri-Binary Radius Evolution Equation** by summing Inner (AdS) and Outer (CFT) contributions.
...
We are not just "made of" tri-binaries; we are made of micro-black-holes stabilized by quantized action partition in the self-hit channel.
...
*   **architrino:** Posits the Inner Binary relaxation term $v_I(t) \propto e^{-\lambda t}$.
    ...
    *   **Difference:** $\Lambda\mathrm{CDM}$ requires a "Reheating" event to stop inflation. In architrino theory, "Reheating" is geometric—it is the natural decay of the self-hit velocity to $c_f$. The stop is automatic.
...
## Minimal Model (Chosen Baseline)

This section resolves the branch-choice ambiguity by taking the **variable Outer branch** as default:
- Outer: sub-field at low $f_O$, rising monotonically to $c_f$ at $f_O=f_P$.
- Middle: pinned at $c_f$ for the full range.
- Inner: super-field from the floor, increasing and saturating at the universal MCB cap.

Define
$$
x \equiv \frac{f_O}{f_P}\in(0,1],\qquad
v_O=c_f B_O(x),\quad
v_M=c_f,\quad
v_I=c_f B_I(x).
$$

Use a compact parameter family $(B_{O,\min},p,\beta_{I,0},q,\beta_{\text{MCB}})$ with
$$
\frac{1}{2}<B_{O,\min}<1,\qquad p>0,\qquad 1<\beta_{I,0}<\beta_{\text{MCB}},\qquad q\ge 1.
$$

### Piecewise velocity factors

Outer (sub-field, monotone to horizon lock, and kept above the middle-radius crossover):
$$
B_O(x)=B_{O,\min}+\left(1-B_{O,\min}\right)x^p,\qquad 0<x\le1.
$$
So $B_O(1)=1$ and $v_O<c_f$ for $x<1$.

Inner (super-field with MCB saturation):
$$
B_I(x)=
\begin{cases}
\beta_{I,0}+\left(\beta_{\text{MCB}}-\beta_{I,0}\right)\left(\dfrac{x}{x_t}\right)^q, & 0<x\le x_t,\\[6pt]
\beta_{\text{MCB}}, & x_t<x\le1,
\end{cases}
$$
with threshold $x_t\in(0,1]$ where MCB saturation begins.

### Piecewise warpage factor

Use $\Gamma$ only for inner-radius compression in the self-hit branch:
$$
\Gamma(x)=
\begin{cases}
1, & B_I(x)\le 1+\delta,\\
1+\eta\!\left(B_I(x)-1-\delta\right), & 1+\delta < B_I(x)<\beta_{\text{MCB}},\\
\Gamma_{\text{MCB}}, & B_I(x)=\beta_{\text{MCB}},
\end{cases}
$$
with $\delta\ge0$, $\eta>0$, and $\Gamma_{\text{MCB}}\ge1$.

### Radii (with 1:2:4 lock)

Using $f_M=2f_O$, $f_I=4f_O$:
$$
r_O=\frac{c_f B_O(x)}{2\pi f_O},\qquad
r_M=\frac{c_f}{4\pi f_O},\qquad
r_I=\frac{c_f B_I(x)}{8\pi f_O\,\Gamma(x)}.
$$

### 3-regime summary table

| Regime | Condition | $B_O$ | $B_I$ | $\Gamma$ | Velocity picture | Radius picture |
| --- | --- | --- | --- | --- | --- | --- |
| Sub-field | $0<x<x_t$, early branch | $B_{O,\min}\le B_O<1$ | $\beta_{I,0}\le B_I<\beta_{\text{MCB}}$ | $1$ (or near 1) | $v_O<c_f,\ v_M=c_f,\ v_I>c_f$ | $r_O$ largest branch, $r_M\propto 1/f_O$, $r_I$ compact |
| Threshold | $B_I=1+\delta$ onward | increasing | increasing | turns on above 1 | self-hit stiffness activates | inner slope steepens relative to $1/f_O$ |
| MCB-saturation | $x\ge x_t$ and $B_I=\beta_{\text{MCB}}$ | approaches 1 (equals 1 at $x=1$) | fixed at cap | fixed at $\Gamma_{\text{MCB}}$ | inner at universal cap; outer hits $c_f$ only at $f_P$ | capped inner branch, horizon outer branch |

### Concrete numeric calibration (example)

Use:
$$
f_P \approx 1.85487\times10^{43}\ \text{Hz},\quad
B_{O,\min}=0.6,\ p=1,\ \beta_{I,0}=1.1,\ q=2,\ x_t=0.25,\ \beta_{\text{MCB}}=2,\ \delta=0.2,\ \eta=2,\ \Gamma_{\text{MCB}}=3.
$$

For compact checking, define normalized radii:
$$
\hat r_O \equiv \frac{2\pi f_O r_O}{c_f}=B_O,\qquad
\hat r_M \equiv \frac{2\pi f_O r_M}{c_f}=\frac{1}{2},\qquad
\hat r_I \equiv \frac{2\pi f_O r_I}{c_f}=\frac{B_I}{4\Gamma}.
$$
Conversion to SI meters:
$$
r_k=\hat r_k\frac{c_f}{2\pi f_O},\qquad k\in\{O,M,I\}.
$$

| $f_O$ | $x=f_O/f_P$ | $B_O$ | $B_I$ | $\Gamma$ | $v_O/c_f$ | $v_M/c_f$ | $v_I/c_f$ | $r_O$ [m] | $r_M$ [m] | $r_I$ [m] |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| $10^{30}\ \text{Hz}$ | $5.391\times10^{-14}$ | $0.600000$ | $1.100000$ | $1.000000$ | $0.600000$ | $1$ | $1.100000$ | $2.863\times10^{-23}$ | $2.386\times10^{-23}$ | $1.312\times10^{-23}$ |
| $10^{40}\ \text{Hz}$ | $5.391\times10^{-4}$ | $0.600216$ | $1.100004$ | $1.000000$ | $0.600216$ | $1$ | $1.100004$ | $2.864\times10^{-33}$ | $2.386\times10^{-33}$ | $1.312\times10^{-33}$ |
| $0.1f_P$ | $0.1$ | $0.640000$ | $1.244000$ | $1.088000$ | $0.640000$ | $1$ | $1.244000$ | $1.646\times10^{-35}$ | $1.286\times10^{-35}$ | $7.353\times10^{-36}$ |
| $0.25f_P$ | $0.25$ | $0.700000$ | $2.000000$ | $3.000000$ | $0.700000$ | $1$ | $2.000000$ | $7.203\times10^{-36}$ | $5.145\times10^{-36}$ | $1.715\times10^{-36}$ |
| $0.5f_P$ | $0.5$ | $0.800000$ | $2.000000$ | $3.000000$ | $0.800000$ | $1$ | $2.000000$ | $4.116\times10^{-36}$ | $2.572\times10^{-36}$ | $8.574\times10^{-37}$ |
| $0.9f_P$ | $0.9$ | $0.960000$ | $2.000000$ | $3.000000$ | $0.960000$ | $1$ | $2.000000$ | $2.744\times10^{-36}$ | $1.429\times10^{-36}$ | $4.764\times10^{-37}$ |
| $0.99f_P$ | $0.99$ | $0.996000$ | $2.000000$ | $3.000000$ | $0.996000$ | $1$ | $2.000000$ | $2.588\times10^{-36}$ | $1.299\times10^{-36}$ | $4.331\times10^{-37}$ |
| $f_P$ | $1$ | $1$ | $2.000000$ | $3.000000$ | $1$ | $1$ | $2.000000$ | $2.572\times10^{-36}$ | $1.286\times10^{-36}$ | $4.287\times10^{-37}$ |

Sanity checks from this calibration:
- Outer remains sub-field for $f_O<f_P$ and reaches $c_f$ only at $f_P$.
- Middle is exactly field-locked over the entire range.
- Inner is super-field and saturates at a universal cap from $x_t=0.25$ onward.

### Quantized-action postulate table (global 1 Hz bump rule)

Impose as a separate postulate (independent of the kinematic fit):
$$
\Delta L_O+\Delta L_M+\Delta L_I=h
$$
for each outer-frequency step $\Delta f_O=+1\ \text{Hz}$, with
$h=6.62607015\times10^{-34}\ \text{J}\cdot\text{s}$.

Under the 1:2:4 lock:
$$
\Delta f_M=+2\ \text{Hz},\qquad \Delta f_I=+4\ \text{Hz}.
$$

Parameterize the action split by fixed shares and enforce the design rule
$$
w_O,w_M,w_I\ge0,\qquad w_O+w_M+w_I=1,\qquad w_I=2w_O.
$$
so that for each outer-step:
$$
\Delta L_O=w_O h,\qquad \Delta L_M=w_M h,\qquad \Delta L_I=w_I h.
$$

| Binary | Frequency bump when $\Delta f_O=1$ Hz | Action increment per outer-step | Constant across range? | Implied branch form |
| --- | --- | --- | --- | --- |
| Outer ($O$) | $\Delta f_O=1$ | $\Delta L_O=w_O h$ | Yes (if $w_O$ fixed) | $L_O(f_O)=L_{O,\mathrm{ref}}+w_O h\,(f_O-f_{O,\mathrm{ref}})$ |
| Middle ($M$) | $\Delta f_M=2$ | $\Delta L_M=w_M h$ | Yes (if $w_M$ fixed) | $L_M(f_O)=L_{M,\mathrm{ref}}+w_M h\,(f_O-f_{O,\mathrm{ref}})$ |
| Inner ($I$) | $\Delta f_I=4$ | $\Delta L_I=w_I h$ | Yes (if $w_I$ fixed) | $L_I(f_O)=L_{I,\mathrm{ref}}+w_I h\,(f_O-f_{O,\mathrm{ref}})$ |
| Total | coupled 1:2:4 bump | $\Delta L_{\mathrm{tot}}=h$ | Yes (postulate) | $L_{\mathrm{tot}}(f_O)=L_{\mathrm{tot,ref}}+h\,(f_O-f_{O,\mathrm{ref}})$ |

### Radii table under the global rule ($q^2$ closure; calibrated example)

At tri-binary level, replace mass by a charge-coupling inertia functional:
$$
\mathcal I_k(f_O)=\kappa_k q_k^2\,S(x),\qquad x\equiv f_O/f_P,\qquad S(x)=x^2,
$$
so
$$
r_O=\frac{L_O}{\mathcal I_O c_f B_O},\qquad
r_M=\frac{L_M}{\mathcal I_M c_f},\qquad
r_I=\frac{L_I}{\mathcal I_I c_f B_I\Gamma}.
$$

For this example we use fixed action shares consistent with $w_I=2w_O$:
$$
w_O=0.25,\qquad w_M=0.25,\qquad w_I=0.5,
$$
and set $L_k(f_O)=w_k h f_O$.

Calibrate the base coefficients at $f_O=f_P$ (where $S=1$) to match the prior meter-scale anchor, and keep $r_I<r_M$ across the listed range:
$$
\kappa_O q_O^2=3.984386\times10^{36},\quad
\kappa_M q_M^2=7.968773\times10^{36},\quad
\kappa_I q_I^2=1.625305\times10^{37}.
$$

| $f_O$ | $x=f_O/f_P$ | $B_O$ | $B_I$ | $\Gamma$ | $r_O$ [m] | $r_M$ [m] | $r_I$ [m] |
| --- | --- | --- | --- | --- | --- | --- | --- |
| $10^{30}\ \text{Hz}$ | $5.391\times10^{-14}$ | $0.600000$ | $1.100000$ | $1.000000$ | $7.952\times10^{-23}$ | $2.386\times10^{-23}$ | $2.127\times10^{-23}$ |
| $10^{40}\ \text{Hz}$ | $5.391\times10^{-4}$ | $0.600216$ | $1.100004$ | $1.000000$ | $7.949\times10^{-33}$ | $2.386\times10^{-33}$ | $2.127\times10^{-33}$ |
| $0.1f_P$ | $0.1$ | $0.640000$ | $1.244000$ | $1.088000$ | $4.019\times10^{-35}$ | $1.286\times10^{-35}$ | $9.318\times10^{-36}$ |
| $0.25f_P$ | $0.25$ | $0.700000$ | $2.000000$ | $3.000000$ | $1.470\times10^{-35}$ | $5.145\times10^{-36}$ | $8.408\times10^{-37}$ |
| $0.5f_P$ | $0.5$ | $0.800000$ | $2.000000$ | $3.000000$ | $6.431\times10^{-36}$ | $2.572\times10^{-36}$ | $4.204\times10^{-37}$ |
| $0.9f_P$ | $0.9$ | $0.960000$ | $2.000000$ | $3.000000$ | $2.977\times10^{-36}$ | $1.429\times10^{-36}$ | $2.336\times10^{-37}$ |
| $0.99f_P$ | $0.99$ | $0.996000$ | $2.000000$ | $3.000000$ | $2.609\times10^{-36}$ | $1.299\times10^{-36}$ | $2.123\times10^{-37}$ |
| $f_P$ | $1$ | $1.000000$ | $2.000000$ | $3.000000$ | $2.572\times10^{-36}$ | $1.286\times10^{-36}$ | $2.102\times10^{-37}$ |

## Perspective on this Exercise

This document is to some extent intuition fitting.

What’s solid:

- defined explicit constraints and written them mathematically.
- can test internal consistency and scaling behavior.

What is not yet solid:

- Several closure choices are underdetermined (B_O, B_I, Gamma, action shares, I(q) sea factor).
- We adjusted forms to satisfy desired behavior, which is model tuning, not derivation.
- No unique result yet that forces itself from first principles.

What would count as “snaps into place”:

1. A minimal axiom set that uniquely implies your functional forms (or a very narrow family).
2. A no-free-lunch consistency check that most alternatives fail.
3. At least one nontrivial quantitative prediction not used in calibration.
4. A potential falsifier with clear pass/fail.
