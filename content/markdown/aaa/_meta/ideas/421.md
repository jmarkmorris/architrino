...
Boundary continuity requires a smooth matching map between double-wrapped and single-wrapped domains. The only continuous resonant bridge is a factor of $2$ at the separatrix, giving the unique frequency lock:
$$
f_I:f_M:f_O = 4:2:1.
$$
...
This baseline removes piecewise tuning parameters and uses only geometric/topological constraints from the delay flow.
