...
Use it as the top orientation document before reading the more specialized [Simulation Run Protocols](run-protocols.md), convergence checks, action-energy notes, the [$A_0$ branch certificate protocol](a0-branch-certificate-protocol.md), the [$A_0$ Tier 0 result interpretation](a0-tier0-result-interpretation.md), the [A1 Action-Increment Protocol](nested-shell-braid-action-increment-protocol.md), the [Retuning-Map Toy Model](retuning-map-toy-model.md), the [Cosmology Shared Residual Fit Protocol](cosmology-shared-residual-fit.md), the [Redshift-Budget Toy Model](redshift-budget-toy-model.md), the [Static Response Vector Toy Model](static-response-vector-toy-model.md), and the [Hydrogen $\Gamma_N$ Spectral Coefficient Row Toy Scan](hydrogen-gamma-n-spectral-row-toy-scan.md).
