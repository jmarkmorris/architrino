...
- Output channels:
  ...
  - Causal wake surface provenance/event tags: for each received contribution at $(\mathbf X_k,T)$, record `emitter_id` together with $T_t$, satisfying $\| \mathbf X_k - \mathbf X_{\text{emitter}}(T_t)\| = c_f (T - T_t)$
...
- Provenance tables: `receiver_id`, $T$, `emitter_id`, $T_t$, `contribution_strength`
