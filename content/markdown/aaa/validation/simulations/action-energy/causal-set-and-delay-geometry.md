...
\mathcal{C}_o(T)
=
\big\{\,T_t<T\mid \|\mathbf X_{o'}(T)-\mathbf X_o(T_t)\|=T-T_t\,\big\}
$$
For $\|\mathbf V_o(T_t)\|<1$ locally, $\mathcal{C}_o(T)$ is generically a singleton; for $\|\mathbf V_o\|>1$, it may contain multiple solutions, including self-hits when $o'=o$.

Clarification: "Multi-valued" means that, for a fixed observation time $T$, there can be more than one emission time $T_t$ that satisfies the causal-distance condition; i.e., $\mathcal{C}_o(T)$ may contain multiple causal roots when $\|\mathbf V_o\|>1$ or when same-transmitter roots exist for $o'=o$. This multiplicity can occur only if the transmitter/source has exceeded field speed at least once; if $\|\mathbf V_o\|<1$ everywhere, $F(T_t;T)$ is strictly increasing in $T_t$ and the causal root is unique.
...
- Root condition as an expanding causal isochron intersection:
  - Define $F(T_t; T) \equiv \|\mathbf X_{o'}(T) - \mathbf X_o(T_t)\| - (T - T_t)$ (with $v=1$ units). Causal roots satisfy $F(T_t; T)=0$ with $T_t < T$ and $H(T-T_t)$.
- Geometrically: the source point $\mathbf X_o(T_t)$ must lie on the causal wake surface (isochron) of radius $\Delta = T - T_t$ centered at the receiver’s current position $\mathbf X_{o'}(T)$.
...
- Conventions at singular cases:
  - We adopt $H(0)=0$ so the instantaneous emission at $T_t=T$ does not produce an immediate self-kick.
  - No $r=0$ causal roots beyond $\Delta=0$: because $r = v(T - T_t)$, $r=0$ implies $\Delta=0$; the $\Delta=0$ case is excluded by $H(0)=0$. Under mollification, the symmetric limit as $r\to 0$ yields zero net push.
