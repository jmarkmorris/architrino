...
**Advancement (reduced-model theorem, proved under stated assumptions):**
Treat a moving tri-binary as a phase-locked two-leg causal loop with center speed $v$ through the Noether Sea and signal speed $c_f$.
Let $\beta=v/c_f$, $L_0$ be the rest bond length, and
$$
C(v)\equiv \frac{L_\parallel(v)}{L_0}.
$$
Parallel-leg round-trip delay:
$$
T_\parallel(v)=\frac{L_\parallel}{c_f-v}+\frac{L_\parallel}{c_f+v}
=\frac{2L_0}{c_f}\frac{C(v)}{1-\beta^2}.
$$
Transverse-leg round-trip delay from
$c_f^2\tau^2=L_0^2+v^2\tau^2$:
$$
T_\perp(v)=2\tau=\frac{2L_0}{c_f}\frac{1}{\sqrt{1-\beta^2}}.
$$
Impose the operational no-leak condition (clock period independent of internal orientation):
$$
...
$$
So Lorentz-FitzGerald contraction and time dilation are not inserted by hand in this reduced delay model; they are forced by causal-delay geometry plus isotropy of internal phase locking.

**Immediate corollary (low-$\beta$ coefficient fixed):**
$$
\frac{L_\parallel(v)}{L_0}=1-\frac{\beta^2}{2}+O(\beta^4),\qquad
\frac{T(v)}{T_0}=1+\frac{\beta^2}{2}+O(\beta^4).
$$
This fixes the first nontrivial suppression coefficient to $1/2$.

**Scope:** This is a controlled reduced result, not yet the full NFDE proof for all tri-binary modes. The full theorem still requires proving conjugacy of the full boosted-relaxed flow.
