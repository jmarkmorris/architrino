...
A candidate mechanism, at hypothesis level, says that the geometry of an accessory record helps set the size of this exposure. The [Thomson Dressing Mechanism](../noether-braid/braid-mathematics.md#thomson-dressing-mechanism) orders same-polarity accessory configurations by their first nonvanishing polarity-signed moment; for example, a six-site octahedron has no dipole, quadrupole, or octupole moment. Applying that ordering to the current charged-fermion architecture requires the actual six-site mixed-polarity axial record, its strain on the braid core, and its retained far-field ledger. The older shortcut that represented quarks by isolated four-site and two-site same-polarity dressings is not the current quark inventory. Thus the relationship between $\mathcal{Z}_{\mathrm{tf}}^{ab}(A)$, $\zeta(A)$, and accessory geometry remains a routing hypothesis rather than a computed extraction.
...
3. **Why do neutrinos have mass at all?** If a [neutrino](./fermions/neutrinos.md) is a near-photon polarity-conjugate braid pair with nearly perfect shielding ($\zeta \sim 10^{-12}$), which residual internal-binary exposure breaks exact photon-like cancellation?
