...
3. **Frozen rigid octahedral chart.** The rigid zero-offset octahedral carrier at fixed speed is conjectured, from earlier single-instrument screening, to carry a nonzero tangential residual rejecting the narrow fixed-speed branch chart; this is unverified pending re-screening on the validated engine, and the reading discipline for that chart is recorded in [Neutral Braid](neutral-braid.md).
