...
For the broader cross-axis classifier, see [Noether Braid Configuration Space](noether-braid-configuration-space.md). The base family chapters are:
...
| [**shell braid**](braid-families.md) | A neutral braid whose six architrino paths remain in a controlled radial support band. | Radial support control, with near-antipodality only as an optional constraint. |
| [**nested shell braid**](braid-families.md) | A shell braid with three ordered radial support bands. | Ordered support-band structure; exact binaries are an additional proof assumption, not a separate braid type. |
...
Among candidate realizations of this case structure, one named family with distinctive exact structure is the **spindle braid**, the rigid uniaxial iso-frequency family defined in [Spindle Braid](spindle-braid.md) and situated among the alternatives in [Noether Braid Configuration Space](noether-braid-configuration-space.md#a-named-candidate-the-spindle-braid). Its interest is structural — tilt-decoupled kinematics and an inherited discrete-symmetry scaffold — and it remains a named candidate rather than a retained branch. No family ranking is asserted; comparative adjudication among the families is an open obligation of the theory.
...
Canonical reader-facing prose uses **Noether braid** for the assembly class, **neutral braid** for the base case, and **spindle braid** for that named candidate realization; **shell braid** and **nested shell braid** remain in use as the comparison-family vocabulary of [Braid Families](braid-families.md), reserved for claims that depend on those families' specific structure. Durable symbols and internal runtime identifiers may still contain `NS`, `noether_braid`, or `nested-shell-braid`; those strings are stable implementation identifiers, not a second taxonomy. The braid's dynamic envelope geometry is developed separately in [Braid Envelope Geometry](braid-envelope-geometry.md), while metric-level translation belongs to [Emergent Metric](../spacetime/emergent-metric.md).
...
| Family ladder | [Braid Families](braid-families.md) | The base neutral braid (six-body inventory and all-pairs ledger), the one-band symmetric shell family, and the three-band nested shell family, with their closure labels and the dressing and generation hypotheses. |
...
| Named candidate | [Spindle Braid](spindle-braid.md) | The rigid uniaxial iso-frequency family: definition, tilt-decoupled geometry, the harmonic-matching hypothesis, and discrete-symmetry structure. |
