...
This question is the crux of the theory. The Noether sea, the particle architecture, the mass-response program, and the effective-metric recovery all rest on a retained six-body branch, so the retained-branch question is the central open obligation of this scene. The chapters here define the braid families, carry the shared mathematics, and state the requirements a retained branch must satisfy, each at its stated claim level. Search status, campaign results, and candidate rankings are not textbook content; they live with the active research program and enter these chapters only when a result is certified at corpus grade.
...
| [**shell braid**](explored-braid-geometries.md) | A neutral braid whose six architrino paths remain in a controlled radial support band. | Radial support control, with near-antipodality only as an optional constraint. |
| [**nested shell braid**](explored-braid-geometries.md) | A shell braid with three ordered radial support bands. | Ordered support-band structure; exact binaries are an additional proof assumption, not a separate braid type. |
...
Among candidate realizations of this case structure, one named family with distinctive exact structure is the **spindle braid**, the rigid uniaxial iso-frequency family defined in [Spindle Braid](spindle-braid.md) and situated among the alternatives in [Noether Braid Taxonomy](noether-braid-taxonomy.md#a-named-candidate-the-spindle-braid). Its interest is structural — tilt-decoupled kinematics and an inherited discrete-symmetry scaffold — and it remains a named candidate rather than a retained branch. No family ranking is asserted; comparative screening and evolution on the validated engine are open obligations of the research program.
...
| Family ladder | [Explored Braid Geometries](explored-braid-geometries.md) | The comparison population: the one-band symmetric shell family and the three-band nested shell family, with their closure labels and the dressing and generation hypotheses. |
...
| Mechanism | [Noether Braid Doubling-Frequency Resonance Lock](doubling-frequency-lock.md) | The `NSH-421` candidate frequency family. |
