...
All certified braids must reproduce Lorentz deformation of space and time at the observer-export layer when they are tested in a homogeneous moving-branch regime. The same retained record must then export a ruler row whose longitudinal-to-transverse deformation approaches $\xi=R_{\parallel}/R_{\perp}\to1/\gamma$, a clock row whose extracted rate approaches $d\tau/dt\to1/\gamma$, and bounded preferred-frame leakage through the declared $\epsilon_{\text{LV}}$ or two-way anisotropy diagnostic. The current rest-branch ideal-braid qualification has group velocity $0$, so no moving-envelope deformation is present there; Lorentz deformation is a separate moving-export certification obligation, not a burden carried only by that test object.
...
This table is a proof map, not only a taxonomy decoder. A proof ID names a specific effort and does not assert success. The base families are:

| Code | Base family | Meaning |
| --- | --- | --- |
...
| `PL` | planar lower-rank braid | Candidate or boundary family whose angular-momentum rows fail to supply a rank-three volumetric frame. |
...
| `NSH-0` | nested shell braid | Three ordered support bands in a rest branch. | $\mathbf{V}_{\mathrm{grp}}=0$ | Three support bands. | Must be solved: rank-three or lower-rank is not assumed. | Optional axis-neutral or axis-polarized chart. | Not fixed. | Not fixed. | Energy/action rows $E_I,E_M,E_O$ or unordered $E_a$ must close if claimed. | Not tested in the rest qualification. | Retain the nested support, role map, causal-root, energy, frame, and stability rows. A former ideal-braid rest fixture belongs here only if nested support is declared. |
...
| `PL-0` | planar lower-rank braid | Rest-branch comparison family with no rank-three frame. | $\mathbf{V}_{\mathrm{grp}}=0$ | Shell count must be stated: none, one-band, or nested support. | Lower-rank: $D_{\mathrm{plane}}=0$ or below the rank-three floor. | Optional. | Optional. | Optional. | Energy rows may be diagnostic unless retained. | Not tested. | Decide whether planar/lower-rank behavior is a retained branch, a boundary of another family, or only a diagnostic toy. |
| `PL-L` | planar lower-rank braid | Moving lower-rank continuation, only if `PL-0` retains. | $\mathbf{V}_{\mathrm{grp}}>0$ | Inherited from `PL-0`. | Lower-rank. | Optional. | Inherited if claimed. | Inherited if claimed. | Moving energy/action export required if retained. | Conditional: required only if the planar branch is promoted toward certification. | Test whether a lower-rank retained branch can export observer rows without masquerading as rank-three Lorentz closure. |
