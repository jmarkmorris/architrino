...
| Generation behavior | The branch supplies a controlled path from generation I to generation II and III behavior, with the current candidate route being destabilization of one binary row for generation II and two binary rows for generation III. | The braid cannot be only a rest object; it must explain why the same broad architecture can support the observed fermion family pattern. |
...
| Support geometry | Where the six paths live geometrically | shell braid, nested shell braid, oblate envelope |
