...
with $\varrho$ the brake-to-pump ratio and $\Theta$ the field-speed switch: below the rail the channel does not exist and the pump drives the speed up; above the rail the brake drives it back down. The field-speed edge is therefore a **two-sided dynamic attractor** — a pin — precisely when $\varrho>1$, a condition the declared coincidence stratum satisfies with wide margin ($\varrho\approx50$); the balance bounds the stratum scale from above rather than leaving it free (compare the near-field scale discussion in [Particle Masses](../assemblies/particle-masses.md)). Whether a released middle binary actually converges on the rail — the pin's dynamical content — is open, and is a target for direct evolution. Claim level: derivation plus measurement for the sign structure; the attractor reading follows from the reduced dynamics and is not confirmed by an evolved trajectory; the pin does not by itself confine radius.
