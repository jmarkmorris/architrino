...
For agreed changes to dispatch behavior, select `npm test -- tests/agent-dispatch.test.mjs tests/agent-dispatch-session.test.mjs`. These suites are explicit-use checks, outside routine local and GitHub Content Integrity. Test execution validates code, not delivery of a particular message. The [testing process](testing-regime.md) owns execution scope; the [incident and validation record](../priorities/development-process-review/analysis/agent-dispatch-integrity.md) retains measured coverage, live evidence and the remaining host boundary.
