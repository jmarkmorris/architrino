...
1. Prepare the first PR with the workflow, build/setup tools, tests, and all retained files. Keep the repository Actions variable `ARCHITRINO_PAGES_DEPLOY_ENABLED` unset or `false`, and confirm no organization-level variable enables it. Refresh required tracked generated drift during final PR preparation. Require the exact-state local gate and both remote checks: Content Integrity and the Pages build. PR builds must not deploy. The Pages build includes a source-only reconstruction test that excludes every retained runtime file.
2. The operator merges the first PR. This still triggers the existing branch publisher; the new workflow builds and tests but skips its entire deployment job while the variable is off. Verify the merged identity, successful `main` checks, and completion of the old Pages deployment before changing publishing settings. During the controlled cutover, keep the variable off while switching the Pages build type to GitHub Actions, preserving the domain and HTTPS settings. Then set the repository Actions variable `ARCHITRINO_PAGES_DEPLOY_ENABLED` to `true` under Settings → Secrets and variables → Actions → Variables and start a fresh manual workflow run on the verified `main` state. Do not enable an environment-only variable, rerun an old pre-cutover run, publish a working branch, or merge another PR during the cutover.
...
The deployment job requires the explicit enable variable, `main`, a push or manual-dispatch event, and a successful build. A preflight read of the Pages API must return `build_type: workflow` before either configure or deploy actions execute; legacy mode, missing metadata, and API errors fail closed. Missing variables evaluate to empty strings in GitHub Actions and therefore leave deployment disabled. See [GitHub's variables context](https://docs.github.com/en/actions/reference/workflows-and-actions/contexts#vars-context). Builds and tests remain enabled regardless of this switch. After live acceptance, leave the repository variable enabled for ordinary automatic deployments from `main`.

If the first Actions deployment fails, leave the file-removal step blocked. Set the repository variable to `false` and cancel or wait for any in-flight custom Pages deployments before changing the publishing source; changing the variable does not cancel an already-running deployment. While the retained files remain on `main`, the publishing source can then be returned to the verified `main` root branch source and its legacy deployment checked. After the second PR removes those files, switching the setting alone is not a valid rollback: the known-good publisher inputs must first be restored through an explicitly approved repair PR. Never rewrite repository history as part of this transition.
