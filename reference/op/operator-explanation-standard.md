...
- **Do not surface branch/PR-process work during ordinary development.** Staging, commits, pushes, generator `--write` regeneration, pull requests, merges, branch deletion, and rollover belong to an operator-invoked [codex-pr-branch.md](git/codex-pr-branch.md) run, which resolves them within its own procedure. Generated drift in a target that the procedure regenerates from a source the current task edited is an expected consequence of editing, not a finding. Record it where that procedure will consume it and continue. Do not report it as an operator-facing result and do not offer it as a next possible action, unless it blocks the current task or the operator has invoked the procedure.
