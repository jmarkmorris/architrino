...
3. Moons in our solar system. Before cutting the first moon branch, add or update a committed moon registry so the exact sequence is frozen for branch use. Default ordering should be parent body from the Sun outward, then increasing mean orbital distance from that parent. Use IAU-recognized natural-satellite names in lowercase hyphenated form.
4. Minerals and gemstones. Before cutting the first mineral or gemstone branch, add or update a committed registry. Default ordering should be alphabetical by the common mineral or gemstone name in lowercase hyphenated form. Avoid commercial or trademark-only names unless explicitly approved.
5. Space missions and probes. Before cutting the first mission or probe branch, add or update a committed registry. Default ordering should be launch date, then canonical mission or probe name in lowercase hyphenated form. Avoid duplicate variants of the same mission name.
6. Dog or cat breeds. Before cutting the first breed branch, add or update a committed registry. Default ordering should be one combined alphabetical list of recognized dog and cat breed names in lowercase hyphenated form, unless the operator/developer explicitly chooses separate dog-first or cat-first lists.
