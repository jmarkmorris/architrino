...
Run writes only after an explicit regeneration or fix-drift request, or during the [final branch/PR process](git/pr-lifecycle.md). Ordinary edits use check mode and record drift for that process under the AGENTS.md generated-artifact rule, rather than raising it with the operator.
