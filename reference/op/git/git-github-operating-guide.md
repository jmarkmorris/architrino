...
We need an arrangement in which each task knows which checkout it is using, which files it owns, which resources it shares, and how to coordinate changes that affect others. A successor must be able to recover the task's decisions, unfinished work, actual checks, and next action. The publication runner must assemble the intended candidate, validate it in the appropriate environments, and follow the operator-selected review pause and merge authority in the live PR lifecycle. This is a problem of shared development operations as well as information management.
...
| [Standard repository process](pr-lifecycle.md) | Operator-invoked publication, shared-checkout coordination, validation, review handoff, merge verification, and branch rollover | Procedural authority for the designated publication runner. Repository policy permits either Claude or Codex to be designated. |
...
| [Content Integrity workflow](../../../.github/workflows/content-integrity.yml) and [PR Pages build](../../../.github/workflows/pages.yml) and [main Pages deployment](../../../.github/workflows/pages-deploy.yml) | GitHub-hosted checks and site build/deployment | These implement the remote automation. Their success has the scope of the selected checks; it is not a substitute for every research acceptance test. |
...
**Decision: rejected, not deferred.** The operator rejects temporary/task-managed linked worktrees, permanent linked worktrees and teaching/evaluation experiments, including a disposable toy-repository experiment undertaken for this project. This supersedes the earlier evaluation permission and proposed adoption comparison. Use the existing local checkout by default; do not launch an experiment to revisit the choice without a new explicit operator decision.
