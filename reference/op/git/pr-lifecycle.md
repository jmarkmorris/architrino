...
The exact-state receipt invokes Content Integrity with `--profile=local`; GitHub invokes it with `--profile=github`. Both profiles currently select the same required checks from `scripts/check-content-integrity.mjs`. The local receipt additionally runs foundational-impact reevaluation and Animator runtime wiring. GitHub installs locked JavaScript dependencies and ripgrep, audits production dependencies in its Content Integrity workflow, and separately checks fresh-checkout runtime reconstruction and the Pages payload. Python admissions removed with Option A/B enforcement retirement are not implicit publication requirements. Relevant Python and C++ behavior tests remain the responsibility of the affected development work; use the approved shared venv for Python and never substitute system Python. The broad Node sweep is opt-in and reporting-only inside Content Integrity; maintenance checks are separately opt-in. A passing publication gate establishes its named checks, not a complete Node, Python, C++ or scientific test run, and local validation does not replace the required GitHub checks.
