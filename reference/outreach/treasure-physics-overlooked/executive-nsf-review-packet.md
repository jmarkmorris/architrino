...
**Declared burdens:** Lorentz recovery, Born/Bell recovery, and compact-object bridging are now stated as explicit acceptance conditions. They are not yet discharged by derivations from a concrete Noether sea response, basin measure, or dense-matter source-to-effective map.

This asymmetry is not a defect if it is made visible. It is the roadmap.
...
3. **Redshift transport artifact.** Convert the written redshift energy ledger into a transport generator that commutes with global frequency rescaling, preserves image sharpness and blackbody shape, avoids long-baseline dispersion, and uses one constitutive response for distance ladder, CMB, lensing, and growth.

4. **Compact-object bridge artifact.** Write a source-to-effective map from dense Noether braid retuning, source/release history, and Noether sea response to exterior strong-field observables: mass-radius behavior, gravitational-wave records, horizon thermodynamics, and recycling.
...
The program loses force if each domain requires its own unrelated rescue mechanism. It also loses force if one Noether sea response cannot simultaneously hide the preferred frame, carry redshift/cosmology transport, and match gravitational and photon channels to modern bounds; if the basin measure cannot recover Born/Bell/no-signaling constraints together; or if compact-object recycling cannot close the same energy economy invoked by the redshift ledger.
