...
def shade_height_field(z: np.ndarray) -> Image.Image:
    ...
    normal = np.dstack((-1.45 * dz_dx, -1.45 * dz_dy, np.ones_like(z)))
    normal /= np.linalg.norm(normal, axis=2, keepdims=True)
    light = np.array([-0.35, -0.72, 1.28])
    light /= np.linalg.norm(light)

    diffuse = np.clip(normal @ light, 0.0, 1.0)
    shade = 0.75 + 0.32 * diffuse
    base = np.array([242, 241, 247], dtype=np.float32)
    rgb = np.clip(base * shade[..., None], 0, 255).astype(np.uint8)

    image = Image.fromarray(rgb, "RGB").filter(ImageFilter.GaussianBlur(radius=0.28)).convert("RGBA")
