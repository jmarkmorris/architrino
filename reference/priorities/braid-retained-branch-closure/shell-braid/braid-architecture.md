# Mathematical Foundations of the Braid Architecture

Terminology status: `quarantined-priority-history`. This architecture draft is inherited source, not terminology authority. Its disputed labels may remain here for git history and mathematical source mining, but they are not accepted reader-facing taxonomy. Route any reuse through [Braid](../braid-retained-branch-closure.md), whose active reader-facing triad is `neutral braid`, `shell braid`, and `nested shell braid`.

This priority draft states the shell braid architecture as a theorem-target program, not as a completed proof. After [Neutral Braid Model](../neutral-braid/neutral-braid-model.md) and [Nested Shell Braid Model Card](../nested-shell-braid/nested-shell-braid-model-card.md), the shell braid architecture should be read as one mathematical case of a broader neutral braid family rather than as an automatic replacement for the nested shell braid model. It proposes deformable, phase-locked spatiotemporal choreographies that may occupy a shell braid case with a common support band, but it does not promote any downstream claim until a retained-branch certificate supplies active causal roots, positive Jacobian floors, finite memory depth, closure residuals, central inventory accounting, energy/action ledgers, and stability data on one live ledger. Observer-export claims remain blocked until the corresponding export rows are explicitly statused.

Promotion status: `priority-only` for branch-retention and broad architecture claims. Accepted terminology, notation compatibility notes, and detailed validation-method material may be promoted earlier only through the scoped promotion policy in [Braid](../braid-retained-branch-closure.md).

Development posture: build the priority-side neutral braid, shell braid, and nested shell braid architecture documents in this directory until the user-facing corpus merge gate passes. Do not treat this draft as authority for broad architecture migration into `content/markdown/aaa`, simulations, scene assets, or app copy. Scoped promotion may extract accepted terminology or validation-method material when it is reader-ready, self-contained, and does not claim a retained branch. The broad merge decision requires a retained-branch certificate packet with live-ledger rows closed, a central-inventory replacement for the former polar-charge layer, an accepted binary-partition notation policy, and an explicit comparison against the nested shell braid source packet set preserved in [nested-shell-braid-causal-closure](../../braid-nested-shell-causal-closure/braid-nested-shell-causal-closure.md).

---

## 1. Claim Scope

The shell braid hypothesis is that stable Noether braid and fermion assemblies may not require three permanently radially nested binaries. The broader neutral braid generalization also removes mandatory binary grouping: a neutral braid may be six labeled architrinos, three positive and three negative, on closed free-support bounded-speed paths. Binaries, a binary partition, exact antipodality, and common-radius behavior then become case rows. The shell braid architecture is attractive because it may replace some former special roles of `inner`, `middle`, and `outer` layers with symmetric phase offsets, central inventory, and topological branch data. The nested shell braid model card keeps the stronger merge possibility open: the former labels may survive as geometric-order, continuation-history, or weak-stress-role labels when the retained radial row supports them.

The current mathematical status is:

| Claim family | Status in this draft | Required closure object |
| --- | --- | --- |
| DDE well-posedness | theorem target | shell braid branch certificate with finite active roots, positive Jacobian floors, finite memory depth, and either fixed-speed tangent closure or bounded-speed speed-ODE/support closure |
...
The shell braid architecture now has the following priority-side companion packets in this directory:

| Packet | Role |
| --- | --- |
| [Neutral Braid Model](../neutral-braid/neutral-braid-model.md) | General neutral braid branch model with no required binary grouping, support-band distribution rows, topology ledger, all-pairs roots, and optional case reductions. |
| [Neutral Braid Base Certificate](../neutral-braid/neutral-braid-base-certificate.md) | Irreducible six-site neutral braid certificate tuple before binary partition, shell support, nested radial ordering, retention, or observer export. |
| [all-pairs-root-ledger.md](../neutral-braid/all-pairs-root-ledger.md) | Ordered $30$ cross-site causal-root and Jacobian ledger that every paired, partitioned, shell, or nested reduction must explicitly reduce from. |
| [Neutral Braid Finite Mode Search](../neutral-braid/neutral-braid-finite-mode-search.md) | First executable finite-mode search specification for free-support neutral braids with hollow support, bounded speed factors, all-pairs roots, residual rows, and first-failure statuses. |
| [Nested Shell Braid Model Card](../nested-shell-braid/nested-shell-braid-model-card.md) | Merge card treating nested shell braid, shell braid, and transition geometries as cases of one bounded-speed Noether braid branch family. |
| [Nested Shell Braid Radial Support Functional](../nested-shell-braid/nested-shell-braid-radial-support-functional.md) | Radial support functional extracting $R_a$ from the free-support descriptor and emitting spread, gap, transition, and derivative rows for shell cases. |
| [Shell Braid Reduction Row](shell-braid-reduction-row.md) | Case-reduction row treating shell braid as the common support-band case of a neutral braid branch, with support work and occupancy statuses but no retained-branch claim. |
| [Nested Shell Braid Reduction Row](../nested-shell-braid/nested-shell-braid-reduction-row.md) | Stricter case-reduction row requiring binary partition, radial functional, ordered support bands, label-use ledger, and transition status before nested-shell conclusions. |
...
| [Assembly Braid Fallback Investigation](../assembly-braid-fallback-investigation.md) | Low-priority fallback investigation for one enlarged $N>6$ Noether braid ledger only after six-site, shell, nested, or central-inventory routes reach a documented hard obstruction. |
| [retained-branch-promotion-theorem.md](retained-branch-promotion-theorem.md) | Composite promotion theorem stating the full certificate tuple required before a shell braid branch can leave priority-only status. |
...
| [Shell Braid Branch Mathematics](shell-braid-branch-mathematics.md) | Shell braid branch chart, admissible history space, causal-root ledger, Jacobian floor, finite-memory, regularized DDE, weak-limit, and tangential-residual theorem targets. |
...
| [binary-specific-deformation-search-results.md](binary-specific-deformation-search-results.md) | Binary-specific radial and speed-coupled phase screen showing better speed/root behavior but persistent tangential and support residuals. |
...
| [free-support-bounded-speed-dynamics.md](free-support-bounded-speed-dynamics.md) | Free-support dynamics packet clarifying that closed arclength curves in $\mathbb{R}^3$ with support-band rows are the base model, while zero variation is an idealized shell case. |
...
| [bounded-speed-factor-master-retention-theorem.md](bounded-speed-factor-master-retention-theorem.md) | Master certificate for retaining a bounded-speed shell braid branch after root, tail, action, Krawczyk, Noether, event, and stability rows are recomputed. |
...
| [period-closure-and-winding-targets.md](period-closure-and-winding-targets.md) | Equal-length and rational-winding theorem targets needed before force-improving intrinsic curves can be retained as shell braid branches. |
...
| [adaptive-memory-action-row.md](adaptive-memory-action-row.md) | Adaptive finite-memory theorem target tying $\eta_{\mathrm{mem}}$ to radius, active-root completeness, history action, and the $\Gamma_K$ ledger. |
...
| [finite-mode-branch-convergence-theorem.md](finite-mode-branch-convergence-theorem.md) | Refinement theorem target promoting uniformly certified finite-mode rows to a curve-level shell braid dynamics branch. |
...
| [central-inventory-hollow-support.md](../neutral-braid/central-inventory-hollow-support.md) | Hollow support and central-inventory representative statuses for charged-branch bookkeeping without unresolved point-charge placement or hidden force channels. |
...
### 2.2 Noether Braid Inventory

A neutral Noether braid remains a six-architrino branch with
...
The architecture needs a replacement for the nested branch machinery. For a candidate branch class $q$ over a time window $W$, define a shell braid certificate
...
| $\Phi_q$ | shell braid phase offsets, winding data, and closure labels |
...
and must vanish within tolerance on the retained branch. This is the replacement for assuming that shell braid motion automatically remains on a closed carrier.
...
Antipodal binary grouping can bound the mutual partner delay, but it does not by itself prove well-posedness for the full non-Markovian system. The retained causal-root set must be explicit:
...
Operational proper time is not fundamental at the substrate level. A shell braid branch may define a candidate clock ratio by comparing retained branch periods:
...
The shell braid architecture may give a cleaner strong-field continuation than the nested shell braid model, but the claim is not established here. A compact strong-field region $\Omega$ must satisfy a finite-boundary continuation gate:
...
A shell braid simulation packet must emit the fields below even when some are `not_computed`:
...
The shell braid architecture becomes migration-ready only if a review packet can answer `yes` to all rows below:
...
| Branch existence | at least one retained shell braid branch packet with finite active roots, positive Jacobian floors, finite memory depth, no unresolved near-zero self roots, and either fixed-speed tangent/curvature closure or bounded-speed speed-band, speed-ODE, support descriptor, coupled fixed-point, action, and event closure |
...
| Nested shell braid comparison | explicit comparison against the restored nested shell braid causal-closure packet set and the nested reduction row, naming which assumptions are replaced, preserved, rejected, or left as fallback source material |
| Migration batch plan | reviewable sequence for corpus, simulation, and asset migration after the architecture decision, promotion-policy compliance, and rollback route through git history and the restored nested shell braid source bucket |
...
The shell braid architecture is a promising candidate replacement for the nested shell braid model because it attacks the old architecture's special-layer assumptions, fragile polar-charge placement, and anisotropy burden. The mathematical upgrade is conditional: it becomes stronger than the nested corpus only after the shell braid branch certificate replaces the old nested shell braid machinery rather than bypassing it.

The immediate deliverable is therefore one retained shell braid branch packet and a populated architecture decision gate, not a broad rewrite of the corpus. Until those objects exist, this document should be used as a disciplined architecture and certificate plan.
