...
Promotion status: `priority-only`. This packet refines [root-ledger-floquet-stability-certificate.md](root-ledger-floquet-stability-certificate.md) by writing the actual variational equation for perturbations of a retained shell braid branch candidate whose delayed roots move with the perturbation. A stability multiplier computed with frozen roots is not a retained-branch stability certificate.
