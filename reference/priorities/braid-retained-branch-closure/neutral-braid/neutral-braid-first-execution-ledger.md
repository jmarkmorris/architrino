...
```bash
node scripts/neutral-braid/finite-mode-artifact.mjs --out /tmp/neutral-braid-finite-mode-artifact.json --pretty
node scripts/neutral-braid/finite-mode-artifact.mjs --validate /tmp/neutral-braid-finite-mode-artifact.json --pretty
```
...
```bash
node scripts/neutral-braid/octahedral-root-ledger.mjs --out /tmp/neutral-braid-octahedral-root-ledger.json --pretty
node scripts/neutral-braid/octahedral-root-ledger.mjs --validate /tmp/neutral-braid-octahedral-root-ledger.json --pretty
```

This second artifact instantiates the rigid octahedral carrier from [Octahedral Carrier Worked Example](../shell-braid/octahedral-carrier-worked-example.md), samples every ordered distinct source pair on the declared phase mesh, and emits the analytic all-pairs root certificate for the six antipodal-partner rows and twenty-four cross-binary rows. Its useful verdict is:
...
```bash
node scripts/neutral-braid/octahedral-speed-ode-diagnostic.mjs --out /tmp/neutral-braid-octahedral-speed-ode.json --pretty
node scripts/neutral-braid/octahedral-speed-ode-diagnostic.mjs --validate /tmp/neutral-braid-octahedral-speed-ode.json --pretty
```
...
The successor priority target is [Bounded Speed Factor Speed Ode Zero Mean Correction Target](../shell-braid/bounded-speed-factor-speed-ode-zero-mean-correction-target.md). The bridge intake certificate consumes `neutral-braid-octahedral-speed-ode-diagnostic/v1` and emits $\mathbf{m}_{\mathrm{frz}}=m_*\mathbf{1}_6$ plus the range-condition target for bounded-speed shell braid work. It is not another neutral first-execution success criterion; it packages the source vector for the live shell-braid equation that would have to turn the frozen mean obstruction into $\mathcal{M}_i^\nu(z)=0$ with bounded-speed clock, root, Jacobian, force, support, action, and event derivative columns on one ledger.
...
```bash
node scripts/neutral-braid/octahedral-same-source-witness.mjs --out /tmp/neutral-braid-octahedral-same-source-witness.json --pretty
node scripts/neutral-braid/octahedral-same-source-witness.mjs --validate /tmp/neutral-braid-octahedral-same-source-witness.json --pretty
```
...
```bash
node scripts/neutral-braid/octahedral-inventory-closure-witness.mjs --out /tmp/neutral-braid-octahedral-inventory-closure-witness.json --pretty
node scripts/neutral-braid/octahedral-inventory-closure-witness.mjs --validate /tmp/neutral-braid-octahedral-inventory-closure-witness.json --pretty
```
...
```bash
node scripts/neutral-braid/octahedral-phase-offset-retention-witness.mjs --out /tmp/neutral-braid-phase-offset-retention-witness.json --pretty
node scripts/neutral-braid/octahedral-phase-offset-retention-witness.mjs --validate /tmp/neutral-braid-phase-offset-retention-witness.json --pretty
```
...
```bash
node scripts/neutral-braid/octahedral-polarity-phase-retention-witness.mjs --out /tmp/neutral-braid-octahedral-polarity-phase-retention-witness.json --pretty
node scripts/neutral-braid/octahedral-polarity-phase-retention-witness.mjs --validate /tmp/neutral-braid-octahedral-polarity-phase-retention-witness.json --pretty
```
...
The first-failure ladder is inherited from [Neutral Braid Master Retention Theorem](neutral-braid-master-retention-theorem.md). For the first minimal audit artifact, the expected result is usually:
