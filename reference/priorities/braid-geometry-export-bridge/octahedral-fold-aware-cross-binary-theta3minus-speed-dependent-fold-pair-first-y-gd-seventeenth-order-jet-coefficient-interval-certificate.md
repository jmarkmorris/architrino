# Octahedral Fold-Aware Cross-Binary Theta3minus Speed-Dependent Fold-Pair First-Y G/D Seventeenth-Order Jet Coefficient Interval Certificate
...
This packet continues [octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sixteenth-order-jet-coefficient-interval-certificate](octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-sixteenth-order-jet-coefficient-interval-certificate.md). The predecessor interval-certifies the constant through sixteenth-order first-y quotient coefficients. This packet extends the zero-cell quotient jet by one more coefficient:

$$
Q_G(y,\nu)=\sum_{k=0}^{17}Q_{G,k}(\nu)y^k+O(y^{18}),
\qquad
Q_D(y,\nu)=\sum_{k=0}^{17}Q_{D,k}(\nu)y^k+O(y^{18}).
$$

It is not a full first-y-cell remainder enclosure and it is not full $\theta_{3-}^{-}$ collar closure. It closes the seventeenth-order coefficient row only. The remaining fold-pair blocker is now a finite eighteenth-order quotient tail bound after the constant through seventeenth-order coefficients.
...
\delta_\varepsilon
=
d+\varepsilon\beta y+\gamma y^2+h_{0,\varepsilon}y^3+h_{1,\varepsilon}y^4+h_{2,\varepsilon}y^5+h_{3,\varepsilon}y^6+h_{4,\varepsilon}y^7+h_{5,\varepsilon}y^8+h_{6,\varepsilon}y^9+h_{7,\varepsilon}y^{10}+h_{8,\varepsilon}y^{11}+h_{9,\varepsilon}y^{12}+h_{10,\varepsilon}y^{13}+h_{11,\varepsilon}y^{14}+h_{12,\varepsilon}y^{15}+h_{13,\varepsilon}y^{16}+h_{14,\varepsilon}y^{17}+h_{15,\varepsilon}y^{18}+h_{16,\varepsilon}y^{19}+h_{17,\varepsilon}y^{20}+O(y^{21}),
$$
...
The certificate solves $h_{0,\varepsilon}$ through $h_{17,\varepsilon}$ from the $y^4$ through $y^{21}$ source-equation coefficients. The directed-rounded slope clearance remains uniform:
...
| all source-equation coefficients through $y^{21}$ contain zero | `true` |
| maximum absolute source-equation coefficient interval through $y^{21}$ | $241723400.584$ |
| $h_{15}$ interval hull | $[-2822625.21719,2425285.72046]$ |
| $h_{16}$ interval hull | $[-17271814.3689,17271814.3689]$ |
| $h_{17}$ interval hull | $[-92883974.2428,93984211.6088]$ |
| $Q_{G,15}$ coefficient hull | $[-2235611.16533,2235611.16533]$ |
| $Q_{D,15}$ coefficient hull | $[-35769778.6453,35769778.6453]$ |
| $Q_{G,16}$ coefficient hull | $[-12656271.9888,14024649.6342]$ |
| $Q_{D,16}$ coefficient hull | $[-238419043.782,215156623.809]$ |
| $Q_{G,17}$ coefficient hull | $[-79605963.3697,79605963.3697]$ |
| $Q_{D,17}$ coefficient hull | $[-1432907340.66,1432907340.66]$ |
| maximum $Q_G$ sixteenth-order coefficient loss on $0\le y\le0.001796875$ | $1.65644828804\times10^{-37}$ |
| maximum $Q_D$ sixteenth-order coefficient loss on $0\le y\le0.001796875$ | $2.81596208966\times10^{-36}$ |
| maximum $Q_G$ seventeenth-order coefficient loss on $0\le y\le0.001796875$ | $1.68946551204\times10^{-39}$ |
| maximum $Q_D$ seventeenth-order coefficient loss on $0\le y\le0.001796875$ | $3.04103792167\times10^{-38}$ |
...
| maximum absolute $Q_{D,16}+17Q_{G,16}$ identity interval | $453575667.591$ |
| maximum absolute $Q_{D,17}+18Q_{G,17}$ identity interval | $2865814681.31$ |
...
$$
\texttt{certifies\_directed\_rounded\_first\_y\_GD\_seventeenth\_order\_jet\_coefficient\_enclosure=true}.
$$

It also imports the predecessor constant, linear, quadratic, cubic, quartic, quintic, sextic, septic, octic, nonic, decic, undecic, duodecic, tridecic, fourteenth-order, fifteenth-order, and sixteenth-order coefficient claims.
...
| `theta3minus.fold-pair-first-y-GD-eighteenth-order-tail-bound` | directed-rounded open |
| `theta3minus.fold-pair-first-y-GD-directed-rounded-enclosure` | blocked by eighteenth-order tail bound |
| `theta3minus.fold-pair-scaled-remainder-continuous-collar` | blocked by directed-rounded first-y $G,D$ enclosure |

## Tail Target

The seventeenth-order coefficient row proves the leading coefficient of the post-sixteenth-order tail, but it does not bound the continuous finite remainder. After this packet, the exact quotient-tail quantities are

$$
T_G^{(18)}
=
\frac{Q_G-A_{G,17}}{y^{18}}
=
\frac{P-L-y^2A_{G,17}}{y^{20}},
\qquad
T_D^{(18)}
=
\frac{Q_D-A_{D,17}}{y^{18}}
=
\frac{D_{\mathrm{pair}}-L-y^2A_{D,17}}{y^{20}},
$$

where $A_{G,17}$ and $A_{D,17}$ are the certified seventeenth-order quotient jets. A full first-y enclosure requires directed-rounded bounds on $T_G^{(18)}$ and $T_D^{(18)}$ with no raw division by a zero-touching $y$ interval.
...
h_\varepsilon(y,\nu)
=
h_{\varepsilon,\le17}(y,\nu)+y^{18}E_\varepsilon(y,\nu)
$$

over the same first-y cell and speed cells. The post-seventeenth-order proof target is

$$
\mathcal H_{\varepsilon,18}(y,E,\nu)
=
\frac{R_{\varepsilon,E}(y,\nu)}{y^{22}}
=
\frac{H_\varepsilon(y,h_{\varepsilon,\le17}+y^{18}E,\nu)}{y^{18}},
$$

interpreted by Taylor cancellation rather than raw division. This is the next missing zero-safe evaluator; the seventeenth-order packet closes one more coefficient row but does not close this finite tail tube.

The exact pair-tail identity is

$$
T_D^{(18)}=-19T_G^{(18)}-y\partial_yT_G^{(18)}.
$$

Thus a sufficient proof may certify $T_D^{(18)}$ directly or certify a correlated $T_G^{(18)}$ derivative bound:

$$
\sup|T_G^{(18)}| < B_{G,18},
\qquad
19\sup|T_G^{(18)}|+\sup|y\partial_yT_G^{(18)}| < B_{D,18}.
$$

The global budget floors emitted by this packet are

$$
B_{G,18}^{\min}=2.25311393279\times10^{48},
\qquad
B_{D,18}^{\min}=2.25296492143\times10^{48}.
$$

## Executable Artifact

The executable packet is [octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-seventeenth-order-jet-coefficient-interval-certificate.mjs](../../../scripts/neutral-braid/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-seventeenth-order-jet-coefficient-interval-certificate.mjs). It emits:

- directed-rounded interval series rows over the $128$ speed cells;
- branch $h_{0,\varepsilon}$ through $h_{17,\varepsilon}$ interval rows;
- source-equation coefficient containment through $y^{21}$;
- $Q_G(0)$, $Q_D(0)$, $Q_{G,1}$ through $Q_{G,17}$, and $Q_{D,1}$ through $Q_{D,17}$ coefficient intervals;
- linear-loss through seventeenth-order-loss and eighteenth-order-tail-budget rows on the first y cell;
- explicit open finite-tail, full quotient, scaled-remainder, `I1`, and retention rows.

The companion test [neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-seventeenth-order-jet-coefficient-interval-certificate.test.js](../../../tests/neutral-braid-octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-fold-pair-first-y-gd-seventeenth-order-jet-coefficient-interval-certificate.test.js) validates schema, no-fixed-speed-window discipline, $h_{17}$ solve rows, coefficient intervals, remaining post-seventeenth-order budget, overclaim rejection, and CLI write/validate behavior.
