...
=
\left(
\mathcal P_{\mathfrak B},
\mathcal P_{C(\mathfrak B)},
\mathcal W_\gamma,
\mathcal J_\gamma,
\mathcal Q_\gamma,
\lambda_{\mathrm{hel}},
...
Here $\mathcal P_{\mathfrak B}$ and $\mathcal P_{C(\mathfrak B)}$ are the planar ledgers of one braid record and its fixed-worldline polarity conjugate, $\mathcal W_\gamma$ is the retained photon wake-history record, $\mathcal J_\gamma$ is the photon-side angular-momentum ledger, $\mathcal Q_\gamma$ is the static exposure and polarity-balance record, $\lambda_{\mathrm{hel}}\in\{+1,-1\}$ is the target helicity sign, and $\Theta_\gamma$ collects transverse phase data.
...
s_{\mu}^{\mathrm{plane}},
q_{\mu}^{\mathrm{eff}},
\mathcal R_{\mu}^{\mathrm{act}}
\right),
\qquad
\mu\in\{\mathfrak B,C(\mathfrak B)\}.
$$

The contra-rotating polarity-conjugate structure is encoded by the substrate signs

$$
s_{\mathfrak B}^{\mathrm{plane}}
=
-s_{C(\mathfrak B)}^{\mathrm{plane}},
\qquad
...
| `neutral_but_dark_fail` | Static exposure cancels but $\Delta_{\mathrm{surv}}^\gamma=1$. | The polarity-conjugate cancellation erased the free oscillatory photon ledger; no free photon branch is certified. |
