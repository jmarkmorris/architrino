...
The direct candidate claim is that the three binaries may share one frequency while their effective lever arms and speed relations differ because the retained branch energy places each binary differently inside the velocity-deforming Noether braid branch state:
...
The current branch-certificate ranking cannot be used as the sole disposition rule for this candidate. Its primary proxy $n_I-n_M=2$ is designed around the role-assigned $(f+2,f,f-1)$ family, so $(I,M,O)=(f,f,f)$ gives zero by construction. The runner now emits separate iso-frequency rows for common-frequency closure, effective lever-arm speed placement, triadic $120^\circ$ phase profile, and transaction-frequency collapse. The remaining rows still need retained Noether braid geometry, root-multiplicity or branch-incidence support for the doubled inner burden, and retained angular-momentum / wake-energy routing on the same event or positive-width retained domain. The diagnostic `index-ratio` policy should not reject this family, because that policy maps speed ratios to frequency-index ratios while the iso-frequency hypothesis assigns speed ratios through retained lever arms, envelope deformation, and branch energy.
...
First calculation. Insert the role-assigned frequency-index families and the iso-frequency family into the existing action-angle energy condition. In the clean no-wake, no-root-energy approximation, compare whether
