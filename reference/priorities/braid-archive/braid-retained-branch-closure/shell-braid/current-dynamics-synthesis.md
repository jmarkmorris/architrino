...
The bounded-speed rows are coupled rather than sequential once they become a certificate. The speed ODE changes $\chi_i$, $\Lambda_i$, causal roots, source-normal root charts, receiver-normal branch strengths, delayed directions, support multipliers, action scale, and event surfaces. Thus the scalar speed row and the normal reconstruction row are diagnostic unless they are joined in the coupled residual $\mathcal{R}_{\mathrm{cpl}}^\nu$ of [bounded-speed-factor-coupled-fixed-point-theorem.md](bounded-speed-factor-coupled-fixed-point-theorem.md).
...
| [binary-specific-deformation-search-results.md](binary-specific-deformation-search-results.md) | binary-specific radial/phase coupling improves speed RMS to about $0.0689$ and keeps $J_{\min}\approx0.3663$ | tangential RMS remains about $0.8209$ |
...
| [delayed-force-lipschitz-envelope.md](delayed-force-lipschitz-envelope.md) | derives per-root force derivative bounds from $\eta_0$, $J_0$, receiver-normal first-derivative rows, and curve-variation constants | current $M=3$ packets do not emit the derivative envelope needed for trust, refinement, Krawczyk, or curl bounds |
...
The tangent sensitivity equations now give the derivative backbone for that continuation theorem. Along a branch tangent $\tau$, the derivative chain runs through inverse arclength phase, delayed root motion, source-normal root-chart motion, receiver-normal per-root force motion, residual motion, fitted/action scale motion, and event-margin motion. This is the row future solvers must emit if they claim a Krawczyk, event, or Floquet certificate rather than a finite-difference screen.
...
The delayed-force Lipschitz row supplies the missing constants behind those certificates. It bounds root motion, line-of-action direction motion, source-normal root-chart variation, and the receiver-normal inverse-square branch weight on a root-regular chart. Without those constants, the trust-radius, Krawczyk, collocation, and curl rows remain formal rather than executable.
