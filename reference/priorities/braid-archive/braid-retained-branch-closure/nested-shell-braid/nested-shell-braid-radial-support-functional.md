...
Promotion status: `priority-only`. This packet closes the local definition gap in [Nested Shell Braid Model Card](nested-shell-braid-model-card.md): the shorthand shell radius $R_a(t)$ is not a primitive object of the base branch. It must be extracted from the free-support descriptor and the closed arclength curves of [free-support-bounded-speed-dynamics.md](../shell-braid/free-support-bounded-speed-dynamics.md).
...
| `status` | first failed status or `nested-shell-braid-radial-support-certified` |
...
9. `nested-shell-braid-radial-support-certified`
