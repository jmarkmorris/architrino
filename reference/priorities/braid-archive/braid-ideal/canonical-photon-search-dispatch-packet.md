...
**Date:** 2026-07-15
**Status:** EXECUTED 2026-07-15 — **scoped negative for the declared ranges**: 75,876 rows scored, zero reach force balance at any drift (best $\epsilon_{\rm bind}=0.833$ at $u=0.99$ vs the $0.03$ gate; ladder-converged margins $>20\times$); no rest branch; H-$\pi$ found degenerate with $\Delta=0$ (antipodal identity); no flattening trend measured; luminal endpoint via direct fail-closed attempt + extrapolation ($0.83071 \pm 2.4\times10^{-6}$). eom acceptance gate verified green before scoring; no row survived to coupled release, so no temporal claim exists. Full coverage statement, exact counts, and falsifiers: [canonical-photon-search-spec.md](canonical-photon-search-spec.md). §93/§95 control reproduction environment-blocked in the executing sandbox (45 s ceiling) — rerun commands in the spec. **Engine:** the new `src/eom` coupled integrator (see the [independent inspection](../../app-solver/eom-engine-independent-inspection-2026-07-13.md)); dynamical/stability claims require the eom acceptance gate green. The legacy central solver (prescribed-orbit evaluator, being retired as apps migrate to eom) must not carry any new claim (see the [2026-07-12 quarantine worklist](../../app-solver/claims-triage-ledger-2026-07-12.md)).
...
Runner + fixture + owner test under `scripts/braid-ideal/` and `tests/`; a spec under `reference/priorities/braid-archive/braid-ideal/` with the coverage statement and the $d_i(u)$ flattening diagnostic; the pinned anchor/control regressions; answers to operator decisions (a)–(d) recorded before scoring; and a plain-language verdict. Report thread state, authority used, files changed, and validation status.
