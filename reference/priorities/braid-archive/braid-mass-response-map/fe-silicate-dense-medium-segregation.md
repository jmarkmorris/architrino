...
- **Main claim:** existing iron-rich metallic assemblies may have lower relative chemical and medium-response cost than silicate assemblies as normalized Noether braid density increases, after ordinary pressure, temperature, phase, and gravity terms are separated.
...
- [Noether Braid Scaling and Packing Scaffold](../braid-doubling-frequency-lock/noether-braid-scaling-and-packing.md) supplies the current priority-side model for packing-limited center density and oblate-envelope support-function contact.
...
| Speculation | Metallic iron may be especially compatible with high normalized Noether braid density because its packing, bonding, and exposed mass response lower relative medium cost compared with silicate phases. |
...
This condition says that the iron-rich metallic branch becomes relatively cheaper as normalized Noether braid density $n=\rho_{\text{NS}}/\rho_{\text{NS},0}$ rises. It does not say that $n$ creates iron.
...
| $\Psi$ | convex penalty as local Noether braid density approaches the phase-specific packing ceiling |
...
The packing side should now be treated as a support-function lattice-cell calculation, not as two scalar declarations. For phase $X$, start with an oblate Noether braid envelope
...
| `n_steps` | normalized Noether braid density values or increments |
...
1. **Branch-speed certification.** Replace the finite-$\eta$ branch assumptions $\beta_{\mathrm{Fe}}=2/3$ and $\beta_{\mathrm{sil}}=1/2$ with a Noether braid return-map derivation or with certified intervals. If only intervals are available, propagate
