# $A_0$ Corpus Change Recommendations
...
The current priority-side result is a controlled negative result:

1. the compact finite-coordinate test remains a no-go after scalar, quotient-class, raw-root-key, and two-bin `I` observation-phase residual-balance ledgers, with best relative residual

```text
0.3500173344435869 > 0.02
```

2. the branch-chart revision checker rejects the default source declaration as `rejected_hidden_fit_split`;
3. the same checker rejects a permissive `prefit_branch_chart` declaration as `overfit_holdout_fail`, with maximum held-out relative residual

```text
2.4537879974811028 > 0.02
```

This is a derivation / closure negative result. It is not an $A_0$ attractor, not a physical branch-coordinate discovery, and not a mass-map falsification.

## Recommended Corpus Changes

### 1. $A_0$ Branch Certificate Protocol

- Target: [A0 Branch Certificate Protocol](../../../../content/markdown/aaa/validation/simulations/a0-branch-certificate-protocol.md)
- Priority: high
...
- Target: [Particle Masses](../../../../content/markdown/aaa/assemblies/particle-masses.md)
- Priority: high
- Claim bucket: derivation / closure target
- Risk: high
...
- Target: [Nested Shell Braid Dynamics](../../../../content/markdown/aaa/noether-braid/braid-families.md#nested-shell-braid-dynamics)
- Priority: medium
- Claim bucket: derivation / closure target
- Risk: medium
...
- Target: [Convergence Tests](../../../../content/markdown/aaa/validation/simulations/convergence-tests.md)
- Priority: medium
...
### 5. $A_0$ Tier 0 Result Interpretation

- Target: [A0 Tier 0 Result Interpretation](../../../../content/markdown/aaa/validation/simulations/a0-tier0-result-interpretation.md)
- Priority: medium
...
- Target: [Energy](../../../../content/markdown/aaa/dynamics/energy.md)
- Priority: medium
...
## Unsafe Corpus Changes

Do not edit authored AAA prose to claim any of the following:

- $\mathcal{H}_I$ is accepted branch geometry;
- modes $\{4,5,7\}$, guard mode `6`, or Nyquist mode `8` are physical coordinates;
- a corrected one-period rerun is authorized;
- downstream Tier 2 or Tier 3 energy, shielding, or medium-response outputs exist for $A_0$;
...
The validation-facing batch, Particle Masses Reference Attractor Gate, Nested Shell Braid Dynamics carrier caveat, and Energy handoff caveat now carry the fail-closed branch-chart revision semantics.
