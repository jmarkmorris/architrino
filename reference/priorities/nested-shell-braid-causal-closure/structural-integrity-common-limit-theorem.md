...
| --- | --- |
| [Absolute Timespace](../../../content/markdown/aaa/foundations/absolute-timespace.md#causal-wake-geometry) | The structural-integrity target requires a common matter/photon/calibration limit and Lorentzian deformation from one causal-root ledger, Noether sea dressing map, and assembly deformation law. |
...
| [Shared Branch Certificate Contract](nested-shell-braid-causal-closure.md#shared-branch-certificate-contract) | The accepted branch package already carries $c_f$, $c_{\text{eff}}$, $c_\gamma$, $c_0$, active-root ledger, stability gaps, Lorentz residual, photon rows, medium response, and event ledger. |
...
2. Extract the moving material deformation map $v\mapsto(a_{\parallel,q},a_{\perp,q},T_q)$ from the same return section, and prove the $R_{\xi}$ and $R_T$ bounds before any photon or PPN row is used.
3. Define $c_{\mathrm{mat},q}^{\mathrm{lim}}$ from the persistence boundary of the same retained moving branch: the branch limit is where the stability gap, inactive-root gap, or ledger transversality floor fails, not where a fitted Lorentz formula is declared singular.
