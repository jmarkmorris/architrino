...
4. Do not assign physical mass to architrino primitives. The T3 native/bridge contract names its numerical integration scalar `integrationWeight` / `integrationWeights`; any remaining `mass` or mass-like field outside that T3 contract remains a comparison-kernel parameter or legacy/toy interaction coefficient.
...
17. Use the simulation-envelope wake row rule: every candidate wake row inside the declared envelope must become a resolved wake row, an aggregated wake-noise/background row, or a fail-closed failure row; silent truncation is forbidden.
18. Use `t3-wake-history-row.v1` for resolved wake rows, with explicit `windingLabel`; separate `local-pre-wrap-evidence` from `global-periodic-wrap-evidence` so zero-winding local rows are not mistaken for periodic-wrap evidence.
...
10. `simulation_envelope_wake_row_rule` — Define the simulation-envelope rule that separates resolved wake rows, aggregated wake-noise/background rows, and forbidden silent truncation. Status: `complete`; source: [requirements-and-design](requirements-and-design.md).
11. `wake_history_row_winding_evidence_split` — Define `t3-wake-history-row.v1` with explicit `windingLabel`, then separate local pre-wrap evidence from global periodic-wrap evidence. Status: `complete`; source: [requirements-and-design](requirements-and-design.md).
12. `t3_integration_weight_contract` — Rename or quarantine T3 mass fields as `integrationWeight` / `integrationWeights` in the T3 native structs, C ABI, app bridge request/response rows, T3 state, initial conditions, serialization, statistics, and tests. Status: `complete`; source: [native-bridge-audit-and-first-screen](native-bridge-audit-and-first-screen.md).
13. `dataset_manifest_contract` — Define `t3-app-dataset-manifest.v1` with fields for topology, simulation envelope, initial conditions, current state frames, trajectory/path-history sources, native bridge status, diagnostic status, value authority, explicit gap rows for wake history, winding labels, outbound-face replay, error budget, candidate/resolved/aggregated/failure wake row conservation, and local-pre-wrap versus global-periodic-wrap evidence status. Status: `pending`; depends on: `native_solver_capability_gap`, `smallest_elegant_first_screen`, `outbound_face_wake_replay_schema`, `diagnostic_status_vocabulary`, `simulation_envelope_wake_row_rule`, `wake_history_row_winding_evidence_split`, `logarithmic_ui_prototype_rules`, `first_screen_control_layout`, `first_screen_layer_control_layout`, and `t3_integration_weight_contract`.
14. `first_native_backed_fixture` — Select the smallest native-backed T3 run that exercises scale, image offsets, path-history replay, and at least one wake-history diagnostic without making proof-level claims. Status: `pending`; depends on: `dataset_manifest_contract`.
