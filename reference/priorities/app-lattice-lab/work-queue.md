...
- **Status:** Verified
- **Verified result:** Every repeat cell uses a half-open fundamental-domain ownership record with exactly one owned representation per actual site. The panel uses one plain translation sentence plus the three declared vectors and no jargon legend. The miniature displays the complete nearest-neighbor incidence set for both owned sites, including immediate periodic images, as one tiled pattern. `Highlight repeat cell` is off by default and affects only the main canvas: ordinary relationship lines are hidden while the thick established light-purple highlighted set is shown; no diagonal frame, ghost trace, sphere resizing, or miniature mutation remains.
- **Independent check:** A separately written enumeration reconstructs every main-crop site pair and every repeat-cell translated site pair, then requires exact equality with the displayed relationship sets and checks every displayed segment against the one case-specific nearest-neighbor distance. The uncompressed checkerboard has `12` owned-site incidences rendered as `11` unique physical links; a negative test rejects diagonal $\sqrt{2}d$ bridges. Under X compression at $\lambda=0.4$, only the `4` incidences at the current shortest transformed distance `0.4d` are highlighted (`3` unique physical links), while the `8` longer deformed reference incidences at `d` are explicitly excluded rather than mislabeled nearest neighbors.
