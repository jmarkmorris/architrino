...
This packet turns the $A_0$ reference-attractor posture into a concrete reduced proof and simulation target. It does not replace the full six-worldline $A_0$ output schema in [priorities.md](priorities.md). Its job is narrower: define the first finite branch-search certificate that can decide whether a coupled multi-scale Noether braid lock is worth promoting into shielding extraction, energy-ledger work, and mass-map comparison.
...
Work in the homogeneous Noether sea rest cell used by [priorities.md](priorities.md):
$$
u^i_{\text{sea}}=0,
\qquad
G_{\text{grad}}=0,
...
The operational contract for answering those questions is now [the $A_0$ Branch-Chart Revision Contract](a0-branch-chart-revision-contract.md). It selects the non-root-key inner-layer harmonic deformation coordinate $\mathcal{H}_I$ as the first Type B candidate for $z_\Lambda^\star$, with primary modes $\{4,5,7\}$, guard mode $\{6\}$, a Nyquist guard for `m=8`, held-out residual checks, locked-key exclusion, and benchmark exclusion before another corrected one-period rerun is admissible. The executable checker rejects the current artifact: the residual-surface source is `rejected_hidden_fit_split`, and the branch-state-facing declarations `prefit_branch_chart`, `active_roots`, `root_times`, and `corrected_carrier_state` all fail as `overfit_holdout_fail` with held-out relative residual about `2.454`. A scan over all one-, two-, and three-mode subsets of modes `1..7` also fails; its best simple row is mode `6`, with held-out residual about `1.094`. The first root-transport source record has now been emitted and checked as an `I`-receiver inter-layer $J$ / delay shear coordinate using cyclic finite differences of $\log |J|$ and $\log \tau$ by root key. Its source-declared quotient passes the source, coordinate-construction, and degrees-of-freedom guards, but still fails held-out residual with $R_{\mathrm{xval}}\approx1.712369148202459$. Diagnostic signed-polarity and mixed M-Jacobian quotients are not source-declared and also fail held-out residual at about `1.616` and `1.945`. A feature-span scanner now rules out eight small fixed source-record feature families; the best diagnostic-only family still has held-out residual about `1.247`. The follow-on [root-transport residual spectrum](a0-root-transport-residual-spectrum.md) confirms that the remaining `I` residual is a broad high cyclic-mode packet: modes `4..7` carry about `0.7552232385377363` of the one-sided residual forcing energy, with dominant mode `6` at about `0.207`. The [mode-band source eligibility packet](a0-mode-band-source-eligibility.md) then finds a lawful pre-fit source direction in reciprocal inter-layer transport: `transport:M:inter_layer:I:mean_D_J` carries mode-band fraction about `0.994`, and the corrected carrier state's `I` relative velocity also carries the band above `0.91`. The [reciprocal inter-layer branch-equation checker](a0-reciprocal-interlayer-branch-equation-checker.md) tests that lead as a three-feature vector equation projected along the normalized `I` relative velocity. It passes degrees-of-freedom controls with `48` equations, `3` coefficients, and feature rank `3`, but fails held-out residual with maximum relative residual `1.4057625588588099`. The [carrier-frame residual spectrum](a0-carrier-frame-residual-spectrum.md) then localizes the same forcing mostly radially in the corrected `I` carrier frame under declared linear time alignment, with radial energy fraction `0.5823726218116948` and radial mode-band fraction `0.7984257865887138`. The [carrier-frame branch-coordinate checker](a0-carrier-frame-branch-coordinate-checker.md) rules out the direct corrected-carrier deformation ladder: `delta_radius * e_I,r` fails with maximum held-out residual `1.0492394121933206`, and adding radial rate or tangential projection worsens holdout. The [reciprocal carrier-frame projection checker](a0-reciprocal-carrier-frame-projection-checker.md) then rules out the same reciprocal scalars projected through $\widehat{\mathbf e}_{I,r}$, $\widehat{\mathbf e}_{I,\theta}$, or both; the best held-out row is tangential at `1.0546122909019986`, and the six-feature radial/tangential row still fails at `1.2113507567372126`. The [root-loop branch-coordinate checker](a0-root-loop-branch-coordinate-checker.md) also rules out the immediate two-edge active-root loop holonomy candidate: its best row, `tangential/im_loop_curl`, still has maximum held-out residual `1.705996205813595`. The [delayed source-direction branch-coordinate checker](a0-delayed-source-direction-branch-coordinate-checker.md) rules out the root-specific delayed line-of-action vector lead: after receiver-polarity signing for the relative-`I` target, the best held-out row is `im_delayed_direction` at `1.0408163198841647`, and the weighted/pair variants either fail held-out residual or the leverage guard. The root-transport source now labels `transport_id` as a single-artifact local slot rather than a refinement-stable identity, and $R_{\mathrm{transport}}$ remains pending as `root-transport-identity-not-refinement-stable` with phase-origin covariance uncertified. A standalone certificate script now compares two source records by root key and cyclic order without using `transport_id`, and the [phase-origin variant certificate packet](a0-root-transport-phase-origin-variant-certificate.md) records the first declared bucket-shift source-record pass. The separate [root-ledger refinement stability certificate packet](a0-root-ledger-refinement-stability-certificate.md) rejects that same phase-origin sidecar as `phase-origin-variant-not-root-ledger-refinement`, and it records one carrier-replay continuation source as non-phase-origin certificate-only root-ledger evidence with explicit tolerance `1e-6`. Checker consumption records this evidence but does not turn it into rerun authority while the source row remains false. This is therefore a stronger no-go than another low-degree harmonic relabeling, another projection of the same reciprocal source scalars, a two-edge active-root loop coordinate, or the immediate delayed source-direction vector coordinate, but still not a rerun authorization.
