...
2. `central_solver_runtime_switch` - The query-gated page path can now execute the central bridge through the browser-side WASM loader when the built solver artifact exists. Remaining work is to replace the mock-seeded app-playback payload with solver-produced frame samples, retained path-history samples, causal-root rows, delayed-hit rows, and diagnostics, then decide when central replay becomes the default instead of query-gated. Status: `active`.
3. `drag_to_solver_loop` - Make source, Virtual Observer, velocity arrow, and history-depth handles update setup state, show only a temporary `draft preview` during dragging, and rerun the solver on release. Status: `pending`.
