...
- The current central pair-interaction lane can run through the optional native C/WASM C ABI when the loaded solver artifact exports it, and falls back to a JavaScript bridge integrator for stale artifacts. Retained path constraints now enter the pair-integration loop as finite-time target constraints, non-step-aligned retained times are emitted as real frame samples instead of being painted onto the path after integration, non-start retained constraint hits reseed their velocity from neighboring retained knots, and intermediate constrained samples target the retained-knot Hermite boundary path instead of aiming only at the next retained point. Residual diagnostics compare the constrained integrated path against the current pair-interaction law, boundary-residual diagnostics compare retained-knot finite-difference acceleration against the pair law at the retained knots, and guidance diagnostics report the extra acceleration injected to follow the retained-knot Hermite boundary path. Constrained central replays surface as `solver guided replay` in the source chip when that guidance is present. Remaining solver work is graduating the display-space force law and retained-knot Hermite boundary guidance into the intended full physical pair-interaction boundary-value solver behind the same `pairInteraction` contract.
...
- Explicit central review URLs still work. `?replay=central&solverReplay=pair-interaction` selects the default central pair replay, `?replay=central&solverReplay=motion` selects central motion-seed replay directly, and `?replay=central&solverReplay=app-playback` keeps the bridge app-playback review path available. Motion-policy review URLs can pass `motionPolicy=pair_segmented_attraction_seed` or `motionPolicy=pair_initial_attraction_seed`, plus optional `pairSegmentCount` and `pairAccelerationScale` values. Pair-interaction review URLs can pass `pairInteractionLaw=inverse_distance_pair_attraction_v1` to run the existing $1/r$-attenuated pair law through the central `pairInteraction` request without changing the visual default.
...
6. After the central solver bridge is available, dragging path-history point `1`, the velocity arrow, or the Virtual Observer should submit the setup to the solver or app-worker adapter.
...
- `constraint_hermite_boundary_guidance` - For sample times between retained points, guide the constrained pair replay toward the cubic Hermite boundary path defined by retained positions and retained-knot tangents, then keep pair-law residuals and guidance acceleration as diagnostics until the full physical boundary-value solver replaces this guided path.
...
- `pair_interaction_law_review_option` - Add a query/request option for `pairInteractionLaw` so review URLs can run `inverse_distance_pair_attraction_v1` through the central `pairInteraction` path while the current display-law default remains stable for visual comparison.
...
- Use path-history point `1` as the initial-position handle; submit interior and final retained-point drags as retained path constraints through the central pair replay contract. Current solver behavior uses retained-knot Hermite boundary guidance inside pair integration while the full physical path-constraint boundary-value solver is still pending.
