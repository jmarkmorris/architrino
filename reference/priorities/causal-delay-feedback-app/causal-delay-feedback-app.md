...
- `node scripts/check-solver-app-bridge.mjs` passes for the current bridge, including causal-delay app-playback, native-C-ABI pair-interaction, resolver-created no-WASM JavaScript-fallback pair-interaction, native and no-WASM JavaScript-fallback retained-knot subinterval frame-refinement runs, finite boundary-relaxation residual and accepted-step tolerance runs, fail-closed boundary-residual, retained-position residual, and guidance-acceleration tolerance gates when constrained audit samples are missing, full finite-difference relaxation candidate-family mask coverage for constrained native and JavaScript-fallback pair runs, physical-boundary-solver pending metadata on constrained pair replays, motion-simulation, and delayed-hit smoke runs. `tests/solver-app-bridge-native-pair-status.test.js` also proves the central bridge downgrades a stale native `converged` boundary-relaxation code to `no_relaxable_samples` when residual samples are absent, keeping the replay in the guided diagnostic claim class while still reporting the physical boundary solver as pending. The remaining causal-delay solver work is the intended full physical pair-interaction and path-constraint model rather than repairing the generic bridge.
- `node scripts/check-solver-contract-fixtures.mjs` passes for the pair-interaction contract surface, including retained-knot frame-refinement count, finite-difference candidate-family telemetry, retained-knot boundary-residual acceptance status, retained-position residual acceptance status, guidance-acceleration acceptance tolerance/status fields, and physical-boundary-solver pending status fields in both the JSON schema and TypeScript declaration surface.
