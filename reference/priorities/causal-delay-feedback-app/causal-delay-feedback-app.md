...
- The capture script starts or reuses the local dev server, opens a Chromium-family browser through the Chrome DevTools Protocol, pins the replay time and selected wake row, verifies central proofs use `central_solver_bridge_replay`, verifies the retained-edit, point-`1` initial-position, final-endpoint, context-menu reception-insertion, and explicit boundary-relaxation proofs return `solver boundary replay` with `discrete_boundary_value_converged` and `finite_difference_pair_boundary_value_solve_converged`, verifies those boundary proofs meet a finite post-relaxation residual threshold and residual-ratio threshold, verifies the initial-velocity proof returns `solver pair replay`, and writes central desktop/portrait, central retained-edit desktop, central boundary-retained-edit desktop, central initial-position desktop, central final-endpoint desktop, central reception-insertion desktop, central initial-velocity desktop, plus representative mock desktop/portrait PNG proofs.
- [central-pair-purple-1920x1080.png](browser-qa/central-pair-purple-1920x1080.png) - 1920x1080 runtime capture of the default central `pairInteraction` replay with the iOS Purple canvas, selected solver-hit diagnostics, and the settings panel closed.
- [central-pair-purple-390x844.png](browser-qa/central-pair-purple-390x844.png) - 390x844 portrait runtime capture of the default central `pairInteraction` replay at 2x device scale, proving the central solver replay path renders in the phone-shaped layout instead of only the representative mock portrait proof.
- [central-retained-edit-purple-1920x1080.png](browser-qa/central-retained-edit-purple-1920x1080.png) - 1920x1080 runtime capture after a scripted retained-point drag and central replay rerun, proving the direct-manipulation path returns a converged discrete boundary replay by default with `maxPathConstraintBoundaryRelaxationResidualAfter <= 1` instead of staying in mock, draft, or merely guided state.
- [central-boundary-retained-edit-purple-1920x1080.png](browser-qa/central-boundary-retained-edit-purple-1920x1080.png) - 1920x1080 runtime capture after the same retained-point drag with explicit finite relaxation settings, proving the central replay can surface `solver boundary replay` with `discrete_boundary_value_converged` and `finite_difference_pair_boundary_value_solve_converged` when the finite-difference boundary relaxation meets the requested tolerance. The proof currently requires `pathConstraintBoundaryRelaxationTolerance=10`, `pathConstraintBoundaryRelaxationIterationCount=64`, `maxPathConstraintBoundaryRelaxationResidualAfter <= 10`, and `pathConstraintBoundaryRelaxationResidualRatio <= 0.02`.
- [central-initial-position-edit-purple-1920x1080.png](browser-qa/central-initial-position-edit-purple-1920x1080.png) - 1920x1080 runtime capture after a scripted path-history point `1` drag and central replay rerun, proving the source-position handle is the first numbered path-history marker and returns a converged discrete boundary replay with the edited initial condition preserved and `maxPathConstraintBoundaryRelaxationResidualAfter <= 1`.
- [central-final-position-edit-purple-1920x1080.png](browser-qa/central-final-position-edit-purple-1920x1080.png) - 1920x1080 runtime capture after a scripted final path-history marker drag and central replay rerun, proving the ending marker is the final numbered path-history point and returns a converged discrete boundary replay with the edited endpoint preserved and `maxPathConstraintBoundaryRelaxationResidualAfter <= 1`.
...
| `pathConstraintGuidanceAccelerationTolerance` | Optional maximum retained-knot guidance acceleration tolerance requested by review URL or injected request options. |
...
| `pathConstraintSolverStatus` | Machine-readable constrained-path status: `constraint_snap_only`, `guided_constraint_path`, `discrete_boundary_value_converged`, or `unconstrained`. |
| `pathConstraintSolverClaim` | Claim scope for the constrained path. Guided runs use `diagnostic_constraint_replay_not_boundary_value_solve`; tolerance-converged discrete finite-difference solves use `finite_difference_pair_boundary_value_solve_converged`. The app adapter only derives this finite-difference claim from missing solver status fields when residual samples exist, the requested tolerance is satisfied, and max/mean/RMS residuals are all non-worsening. |
...
- `central_browser_screenshot_proof` - Capture a 1920x1080 browser proof of the default central `pairInteraction` replay and fail the proof if the runtime silently falls back to `representative_mock_solver_replay`.
- `central_portrait_browser_screenshot_proof` - Capture a 390x844 phone-shaped browser proof of the default central `pairInteraction` replay and fail the proof if the runtime silently falls back to `representative_mock_solver_replay`, so the future iPhone integration path has central-replay evidence rather than only mock portrait evidence.
- `central_direct_edit_browser_screenshot_proof` - Capture a 1920x1080 browser proof after a scripted retained-point drag, wait for the central replay rerun, and fail the proof unless the returned dataset is `central_solver_bridge_replay` with `pathConstraintSolverStatus=discrete_boundary_value_converged`, `pathConstraintSolverClaim=finite_difference_pair_boundary_value_solve_converged`, the edited retained point preserved, retained-position residual evidence covering every submitted path-history marker, and the source chip reading `solver boundary replay` with `posRows` and `posStatus=within_tolerance`.
- `central_boundary_relaxation_browser_screenshot_proof` - Capture a 1920x1080 browser proof after a scripted retained-point drag with finite `pathConstraintBoundaryRelaxationTolerance` and `pathConstraintBoundaryRelaxationIterationCount` settings, wait for the central replay rerun, and fail the proof unless the returned dataset is `central_solver_bridge_replay` with `pathConstraintSolverStatus=discrete_boundary_value_converged`, `pathConstraintSolverClaim=finite_difference_pair_boundary_value_solve_converged`, `maxPathConstraintBoundaryRelaxationResidualAfter` at or below the requested proof threshold, `pathConstraintBoundaryRelaxationResidualRatio` at or below the requested proof threshold, retained-position residual evidence covering every submitted path-history marker, and the source chip reads `solver boundary replay` with `posRows` and `posStatus=within_tolerance`.
- `central_initial_position_browser_screenshot_proof` - Capture a 1920x1080 browser proof after a scripted path-history point `1` drag, wait for the central replay rerun, and fail the proof unless the returned dataset is `central_solver_bridge_replay` with `pathConstraintSolverStatus=discrete_boundary_value_converged`, `pathConstraintSolverClaim=finite_difference_pair_boundary_value_solve_converged`, the edited initial condition preserved by the first numbered path-history marker, retained-position residual evidence covering every submitted path-history marker, and the source chip reading `solver boundary replay` with `posRows` and `posStatus=within_tolerance`.
- `central_final_position_browser_screenshot_proof` - Capture a 1920x1080 browser proof after a scripted final path-history marker drag, wait for the central replay rerun, and fail the proof unless the returned dataset is `central_solver_bridge_replay` with `pathConstraintSolverStatus=discrete_boundary_value_converged`, `pathConstraintSolverClaim=finite_difference_pair_boundary_value_solve_converged`, the edited endpoint preserved as the final numbered path-history point, retained-position residual evidence covering every submitted path-history marker, and the source chip reading `solver boundary replay` with `posRows` and `posStatus=within_tolerance`.
- `central_reception_insert_browser_screenshot_proof` - Capture a 1920x1080 browser proof after a scripted canvas context-menu reception-point insertion, wait for the central replay rerun, and fail the proof unless the returned dataset is `central_solver_bridge_replay` with `pathConstraintSolverStatus=discrete_boundary_value_converged`, `pathConstraintSolverClaim=finite_difference_pair_boundary_value_solve_converged`, both histories containing seven renumbered points, rebuilt wake links following `source n -> receiver n+1`, retained-position residual evidence covering every submitted path-history marker, and the source chip reading `solver boundary replay` with `posRows` and `posStatus=within_tolerance`.
- `central_initial_velocity_browser_screenshot_proof` - Capture a 1920x1080 browser proof after a scripted initial-velocity handle drag, wait for the central replay rerun, and fail the proof unless the returned dataset is `central_solver_bridge_replay` with the source chip reading `solver pair replay`.
