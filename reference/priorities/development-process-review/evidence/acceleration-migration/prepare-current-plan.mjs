// Known-case-tested metadata inventory; no numerical stage is launched.
...
const original=JSON.parse(readFileSync('reference/priorities/braid-program/evidence/2026-08-27-f6c-continuous-reception-acceleration-launch.v1.json'));
const recovered=JSON.parse(readFileSync('reference/priorities/development-process-review/evidence/next-caller-inputs/source-identities.json')).find(r=>r.path===E.DECLARATION);
const declarationInput={originalPath:E.DECLARATION,...bind(recovered.archive)};
...
const plan={...original,historicalInputs,priorCoverClosure:{...original.priorCoverClosure,ownerSha256:E.FIXED[9][2]},schema:'braid-program/f6c-continuous-reception-acceleration-launch.v2',executionBridge:bind(E.BRIDGE),declarationInput,
 consumer:bind(E.CONSUMER),controls:bind(E.CONSUMER_TESTS),rangeVerifier:bind(E.CHECKER),declaration:{path:E.DECLARATION,sha256:declarationInput.sha256,bytes:declarationInput.bytes},
 runtimeBindings:[...new Set([...inventory.files,git,path.resolve(python,'../../pyvenv.cfg')])].sort().map(bind),
 operationalBindings:[E.BRIDGE,E.BRIDGE_TESTS,E.ENTRY,E.LAUNCHER,E.TESTS,E.PROCESS_TESTS,E.HELPERS,E.OUTER,E.CHECKER_TESTS,'/bin/ps','/usr/bin/memory_pressure',realpathSync(process.execPath)].map(bind)};
...
const destination=path.join(out,'acceleration-launch.v2.json');writeFileSync(destination,JSON.stringify(plan,null,2)+'\n');
const results=[];
