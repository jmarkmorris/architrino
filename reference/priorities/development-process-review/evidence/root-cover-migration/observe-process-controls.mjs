...
writeFileSync(wrapper,`import{writeFileSync}from'node:fs';import{spawn}from'node:child_process';writeFileSync(${JSON.stringify(pidFile)},String(process.pid));const c=spawn(process.execPath,['--test','--test-concurrency=1','tests/f6c-root-cover-pilot-process.test.js','tests/f6c-cached-root-cover-pilot-process.test.js'],{stdio:'inherit'});c.once('error',e=>{throw e});c.once('exit',(code,signal)=>{process.exitCode=signal?1:code});\n`);
