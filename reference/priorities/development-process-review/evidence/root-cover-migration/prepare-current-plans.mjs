...
for(const cached of ['', '-cached']) {
 const R=await import(pathToFileURL(path.join(root,`scripts/eom/run-f6c${cached}-root-cover-pilot.mjs`)));
 ...
 const old=JSON.parse(readFileSync(`reference/priorities/braid-program/evidence/2026-08-27-f6c${cached}-root-cover-pilot-launch.v1.json`));
 const plan={...old,schema:`braid-program/f6c${cached}-root-cover-pilot-launch.v2`,python,pythonRealPath:realpathSync(python),node:realpathSync(process.execPath),git:realpathSync('/usr/bin/git')};
