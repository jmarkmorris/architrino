...
import {decode, validate} from '../equation-mapping/current-source-manifest.mjs';
...
export function beginBorgRuntimeAdmission({root, selection=decode(fs.readFileSync(path.join(root,BORG_RUNTIME_SELECTION)))}={}) {
