...
function envelope(lease,joint){
 ...
 assert.equal(joint.candidateId,'coincident-midpoint-common-frequency');assert.equal(joint.rung,8);
...
function controls(){
 ...
 const p={closed:true,exitObserved:true,exitCode:0,exitSignal:null},o={closed:true,failure:null,probes:[p,p]},lease={exitCode:0,processGroupClosed:true,elapsedWallSeconds:1},joint={schema:'circular-current-rung-admission.v1',accepted:true,h3EvidenceEligible:false,laterLadderAuthorized:false,candidateId:'coincident-midpoint-common-frequency',rung:8,process:{accepted:true,processesClosed:true,guardClosed:true,admission:{accepted:true,h3EvidenceEligible:false}},observations:o,memory:o,resourceObservations:[{accepted:true,h3EvidenceEligible:false},{accepted:true,h3EvidenceEligible:false}]};envelope(lease,joint);
 for(const mutate of [x=>x.lease.exitCode=1,x=>x.lease.processGroupClosed=false,x=>x.lease.elapsedWallSeconds=1800,x=>x.joint.rung=32,x=>x.joint.process.guardClosed=false,x=>x.joint.memory.failure='failure',x=>x.joint.observations.probes[0].closed=false,x=>x.joint.resourceObservations[0].accepted=false]){const pair=structuredClone({lease,joint});mutate(pair);assert.throws(()=>envelope(pair.lease,pair.joint));}
...
if(process.argv[2]!=='--controls'){
 assert.equal(process.argv.length,4,'review-current-rung.mjs --controls | RUN_ID ADMISSION_PATH');
 ...
 const profile=json(joint.profile.path);bind(joint.profile.path,joint.profile.sha256);assert.equal(profile.candidateId,joint.candidateId);assert.equal(profile.rung,joint.rung);assert.equal(profile.limitMs,1800000);
 ...
 const summary=json(rechecked.summary.path);assert.equal(summary.phaseCount,8);assert.equal(summary.rowCount,288);
 ...
 writeFileSync('reference/priorities/development-process-review/evidence/circular-current-execution/current-rung-independent-review.json',JSON.stringify(result,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify({accepted:true,phaseCount:8,rowCount:288,checkedUniqueBindings:bindings.size,laterLadderAuthorized:false}));
}
