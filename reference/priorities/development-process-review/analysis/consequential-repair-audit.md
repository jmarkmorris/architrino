...
**Subsequent recovery, 2026-09-07:** the source recovery and binding repair (retired migration record) recovers the full entry and the intermediate preparation version exactly, inventories the complete historical source set, and repairs the reviewed current composition. It supersedes the unrecovered-preparation status above. Historical authentication remains open; the new archive manifest names its exact remaining byte and contract gaps.
