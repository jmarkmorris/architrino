...
- **XTM-006 — `thermal_statistical_radiative_qed`.** Discipline statistical emergence, radiation Gate C, regularization, and nonlinear electromagnetic response with thermal/radiative benchmarks. The focused [Strong-Field Electromagnetic Response](benchmarks/strong-field-electromagnetic-response.md) packet requires photon--photon scattering, vacuum birefringence, amplitude-dependent polarization and propagation, electron--positron pair production, and pair backreaction to pass on one shared record before that sector is covered. **Native carrier requirement:** a finite-window statistical carrier, photon/radiation event ledger, regularized wake/material boundary, Noether sea thermal and polarization-resolved constitutive records, retained conjugate pair basins, exact architrino identity provenance, and a continuing source/sea/product/remnant/boundary account. It has no XTM-005 scheduling dependency.
...
- **XTM-008 — `spin_bell_measurement`.** Apply Zeeman, Malus, Stern-Gerlach, and Bell hard gates only after their case-specific carriers exist. **Native carrier requirement:** every case needs a retained ordered-frame or spin-response kernel. Zeeman additionally needs a retained atomic spectral carrier, effective magnetic-state projection, photon-polarization ledger, and analyzer record; Stern-Gerlach needs an apparatus impulse ledger and detector basin measure; Bell needs pair provenance and detector basin measures. It may consume an accepted XTM-007 phase or measure carrier only where the same record is explicit.
