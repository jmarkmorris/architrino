...
6. `structural_integrity_common_limit` — Populate the common matter-limit, Noether sea dressed clock/ruler speed, photon-channel speed, clock/ruler deformation, two-way signal residual, and $\mathfrak{S}_{\gamma A}^{(q)}(W)$ from one retained branch record. Status: `same-branch-gate-a-row-defined; branch-population-blocked`. Depends on: `shared_branch_certificate_contract`, `photon_gate_a_kinematics`, `lorentz_residual_packet`, `lorentz_gr_bridge`.
