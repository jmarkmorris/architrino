...
| Exact first executable blockers | `EQ-24` retained-attempt checker stops at `missing_accepted_theta_sea_rho_NS`; `EQ-25` thermodynamic record checker stops at `missing_accepted_theta_therm`; `EQ-06` still needs the density/cadence projection lemma or an executable refinement-family proof. |
