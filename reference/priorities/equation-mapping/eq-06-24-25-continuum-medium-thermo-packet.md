...
| Fail-closed method | Numeric agreement remains score-neutral unless accepted retained rows exist; priority packets, authored prose, generated output, attempt/mock fixtures, and negative controls do not count as retained evidence sources. |
...
This table is priority-only. It maps the continuum, medium, and thermodynamic comparison terms to retained $\Theta_{\mathrm{sea}}^{(\ell,W)}$ geometry, required carriers, same-record bindings, fail-closed controls, and smallest accepted evidence objects.
