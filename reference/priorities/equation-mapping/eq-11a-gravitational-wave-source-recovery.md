...
| Deepest current probe | [eq11a-gravitational-wave-source-chirp-mass-row-source-contract-attempt.v1.json](../../../scripts/equation-mapping/eq11a-gravitational-wave-source-chirp-mass-row-source-contract-attempt.v1.json) accepts the top carrier, `gw_source_carrier`, `theta_sea`, `effective_metric_tensor_channel`, `source_event_ledger`, `quadrupole_source_row`, and a row-specific `chirp_mass_row` source-contract boundary, then stops at `missing_accepted_peters_decay_row`. |
...
| Fail-closed controls already exercised | Coordination-source and probe-source controls keep accepted-looking `gw_source_carrier` rows blocked when sourced only to priority/probe artifacts; the `theta_sea` controls keep authored Noether sea prose or row-mismatched durable source contracts from satisfying retained `theta_sea` evidence; the tensor, source-event-ledger, quadrupole, and chirp-mass row-mismatch controls keep wrong-row support from satisfying `effective_metric_tensor_channel`, `source_event_ledger`, `quadrupole_source_row`, or `chirp_mass_row`; the checker now blocks all-source-contract shells at `source_contract_boundary_not_retained_evidence`. |
| Next safe implementation target | A score-neutral `peters_decay_row` source-contract attempt for the GW150914 carrier; leave the score at `2` unless a full accepted retained source packet lands and the checker reports populated. |
