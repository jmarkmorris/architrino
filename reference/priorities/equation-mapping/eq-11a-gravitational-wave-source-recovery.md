...
| Deepest current probe | [eq11a-gravitational-wave-source-effective-metric-tensor-source-contract-attempt.v1.json](../../../scripts/equation-mapping/eq11a-gravitational-wave-source-effective-metric-tensor-source-contract-attempt.v1.json) accepts the top carrier, `gw_source_carrier`, `theta_sea`, and a row-specific `effective_metric_tensor_channel` source-contract boundary, then stops at `missing_accepted_source_event_ledger`. |
...
| Fail-closed controls already exercised | Coordination-source and probe-source controls keep accepted-looking `gw_source_carrier` rows blocked when sourced only to priority/probe artifacts; the `theta_sea` controls keep authored Noether sea prose or row-mismatched durable source contracts from satisfying retained `theta_sea` evidence; the tensor row-mismatch control keeps wrong-row support from satisfying `effective_metric_tensor_channel`. |
| Next safe implementation target | A score-neutral source-event-ledger source-contract attempt for the GW150914 carrier; leave the score at `2` unless a full accepted retained source packet lands and the checker reports populated. |
