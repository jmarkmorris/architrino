...
Expected result: `blocked_missing_rows`, `nextBlocker=missing_accepted_theta_gamma_packet`, `carrierAccepted=true` (checker-internal accepted-looking parent only; not retained evidence), `scoreDecision=no_score_increase`, solver residuals passing, and all six negative controls passing. The same command with `--require-populated` must exit nonzero.
...
Expected result: `blocked_missing_rows`, `nextBlocker=missing_accepted_theta_gamma_packet`, `rowStatuses.theta_gamma_packet.reason=accepted_without_evidence_source`, `scoreDecision=no_score_increase`, solver residuals passing, and all six negative controls passing. The same command with `--require-populated` must exit nonzero.
