...
- Related high-energy routing: [High-Energy Astrophysics](../high-energy-astrophysics/high-energy-astrophysics.md#equation-examination-capture)
- Assigned ID: `EQ-07C`
...
- Schwarzschild/Kerr horizon and photon-path comparison scales;
...
| $\Pi_{\mathrm{ext}}$ | Exterior effective-metric projection. | Lensing, clock, ruler, photon-path, EHT-scale, and no-hair readouts from the same carrier. |
...
| --- | --- |
| $\mathcal{R}_{\mathrm{ext}}$ | Checks that $M$, $\mathbf J$, $Q_{\mathrm{eff}}$, $r_s$, $r_+$, $r_{\mathrm{ph}}$, $r_{\mathrm{ISCO}}$, and exterior clock/ruler/photon-path readouts project from the same strong-field record. |
...
The residual may consume accretion or jet rows as boundary data only through declared imports from `EQ-07B`. Those rows cannot replace $\mathcal{R}_{F_H}$, $\mathcal{R}_{\mathrm{int}}$, $\mathcal{R}_{A_H}$, or $\mathcal{R}_{S_H}$.
...
- one no-hidden-retune witness across exterior scale, spin, horizon area, entropy, interior continuation, ringdown/remnant, and any imported release-channel rows.
...
| Closure driver | Recover black-hole-proper exterior mass/radius/area/spin scales, horizon-interface boundary condition, finite interior continuation, entropy-area comparison, and ringdown/remnant labels from one strong-field carrier. |
| Primary carrier | $\Theta_{\mathrm{BH}}^{07C}(\Omega,W)$ with compact region $\Omega$ and absolute-time window $W$. |
| Smallest accepted evidence object | Accepted source-backed `black_hole_horizon_interface_carrier` plus accepted bindings for exterior scale, spin, horizon radius/area, $F_H$, finite interior continuation, horizon label ensemble, event ledger, source provenance, and no-hidden-retune witness. |
...
| Direct Geometry Layer | Present in this packet; it binds exterior mass/radius/area/spin, horizon-interface condition, finite interior continuation, entropy-area comparison, remnant/ringdown labels, event ledger, provenance, and no-hidden-retune witness to one black-hole-proper source identity. |
...
- Promote now: no.
- Defer with blocker: accepted black-hole horizon-interface carrier, source-backed exterior scale/spin rows, finite interior continuation, horizon label ensemble, and no-hidden-retune witness.
