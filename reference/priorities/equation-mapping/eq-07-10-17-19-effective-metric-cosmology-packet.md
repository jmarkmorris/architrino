...
- Exact first blockers: the weak-field checker blocks at `missing_accepted_theta_W`; the signed-transfer checker blocks at `missing_accepted_theta_transfer`; the effective-FRW checker blocks at `missing_accepted_theta_cos`; the pressure/$\Lambda_{\mathrm{eff}}$ checker blocks at `missing_accepted_theta_sea_rho_NS`.
