...
- iso-frequency-energy-radius-candidate.md (legacy-braid ref: `braid-angular-momentum-spin/iso-frequency-energy-radius-candidate.md`): the iso-frequency candidate supplies current-proxy evidence for `(f,f,f)` with phase offsets and different effective lever-arm / speed relations, but retained row-set acceptance is still missing.
