...
- Exact first blocker: the refined [EQ-21 Through EQ-23 And EQ-32 Shared Observation Residual Packet](eq-21-23-32-shared-observation-residual-packet.md) makes this executable and blocks at `missing_accepted_theta_obs`; the upstream `theta_cos` handoff is accepted for the provider-backed `EQ-20` route but still must be consumed by `theta_obs`; `EQ-32` additionally needs a same-window `delta_a_star` projection from accepted density evidence.
