...
| Candidate breakthrough angle | Use one native event record from the synchrotron lane: a charged assembly path segment with $\Gamma_{e^\pm}(t)$, causal-root/Jacobian data (same-record source-normal $D_s$, receiver-normal $D_T$, and branch strength $W^{\mathrm{rec}}$), $\mathcal V_{\mathrm{NS}}$, $G_{\text{grad}}$, wake-strain threshold status, photon Gate A/B output, and source-depletion identity. |
