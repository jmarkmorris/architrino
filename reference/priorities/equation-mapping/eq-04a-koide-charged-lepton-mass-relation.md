...
The accepted-source guard now rejects priority packets, authored AAA prose, generated paths, attempts, toys, probes, mocks, negative controls, and temporary paths as accepted evidence for `EQ-04A`. [eq04a-koide-residual-priority-source-negative-control.v1.json](../../../scripts/equation-mapping/eq04a-koide-residual-priority-source-negative-control.v1.json) keeps accepted-looking priority/source-map rows blocked at the inherited first blocker with `carrierReason=accepted_without_evidence_source` and `scoreDecision=no_score_increase`.
...
The expected boundary run reports `status: blocked_missing_accepted_charged_lepton_mass_map`, `nextBlocker: koide.accepted_mass_map_missing`, `carrierAccepted: true`, and `scoreDecision: no_score_increase`. This does not land retained evidence. It only proves that, once the inherited carrier shell is source-backed, the checker advances to the charged-lepton mass-map blocker instead of counting the near-Koide numerical residual.
