...
3. `lorentz_envelope_closure` - Land the source-backed positive-width `S_eq` retained-domain fixture whose first row is `raw_labeled_rows_preserved_on_retained_history`; then define the minimum source-backed transport comparison behind $W_{\mathrm{hol}}$. The owning objects are [EQ-02 Through EQ-04 Translating Binary Shared-Record Instantiation](eq-02-04-translating-binary-shared-record-instantiation.md) and [EQ-02 Through EQ-04 S_eq Retained-Domain Evidence Object](eq-02-04-s-eq-retained-domain-evidence-object.md). Status: `next`. Depends on: `score_five_closure_ladder`.
4. `effective_metric_constitutive_map` - Complete the remaining CMB transfer/thermalization/recombination/acoustic, BBN source-window, and score-review consumers from the accepted shared constitutive chain without hidden retune. The populated `EQ-11`, `EQ-20`, `EQ-21`, `EQ-24`, and `EQ-32` child history is recorded in [work-log.md](work-log.md). Status: `started`. Depends on: `score_five_closure_ladder`.
...
- Cross-workstream consumers: Equation-Map Bearing On Braid Configuration Search, [Cross-Theory Mapping Priority Candidates](../cross-theory-mapping/priorities.md), [Cosmology Transfer-Function Closure](../cosmology-closure/priorities.md), Noether-Core Stability and First Mass Map, Braid, and Geometry Bridge.
