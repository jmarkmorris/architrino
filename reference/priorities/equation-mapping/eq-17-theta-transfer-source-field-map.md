...
| Fail-closed negative control | `signed_frequency_transfer_priority_source_negative_control` blocks priority prose with `coordination_source_path`; `signed_frequency_transfer_source_contract_negative_control` blocks the source-contract shell with `source_contract_path`. Both remain `scoreDecision: no_score_increase`. |
