...
| Deepest current executable blockers | `EQ-12`: `missing_accepted_theta_gamma_packet`; `EQ-12A`: `missing_accepted_retained_orbit_reduction_row` with shared photon-support blocker `missing_accepted_theta_gamma_packet`; `EQ-14`: `missing_accepted_W`; `EQ-15`/`EQ-27`: `missing_accepted_ordered_frame_loop`; `EQ-16`: muon projection lane now blocks at `missing_accepted_weak_quotient` while the ledger-only lane still blocks at `missing_accepted_weak_projection` and the baseline skeleton still blocks at `missing_accepted_weak_visible_branch_ledger`; `EQ-16A`: `missing_accepted_neutral_lepton_retained_branch` with inherited `S_eq` blocker `missing_accepted_raw_labeled_rows_preserved_on_retained_history`. |
...
The priority-only [EQ-16 Weak-Visible Branch Ledger Source-Field Map](eq-16-weak-visible-branch-ledger-source-field-map.md) now has one durable muon-decay `weak_visible_branch_ledger` evidence row and one same-domain `weak_projection` row. [weak-gauge-exposure-domain-muon-projection-evidence.v1.json](../../../scripts/equation-mapping/weak-gauge-exposure-domain-muon-projection-evidence.v1.json) reports accepted `weak_visible_branch_ledger` and `weak_projection`, `scoreDecision: no_score_increase`, and next blocker `missing_accepted_weak_quotient`; it does not populate quotient, exposure, `V-A`, CKM, PMNS, provenance, covariance, reaction-event, or Noether sea rows.
