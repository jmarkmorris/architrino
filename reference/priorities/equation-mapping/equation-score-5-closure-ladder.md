...
| `EQ-15` | `2` | Spinor, Dirac/Klein-Gordon, spin-statistics, and exchange rows derived from ordered-frame, angular-momentum, and same-record sector exposure. | Use the retained-row status map in [eq-12-16a-photon-quantum-gauge-neutrino-packet.md](eq-12-16a-photon-quantum-gauge-neutrino-packet.md) to populate the non-coplanar path, gauge-control, quotient, conservation, and exchange rows before importing relativistic wave equations. |
