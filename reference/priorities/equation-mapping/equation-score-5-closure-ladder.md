...
| `EQ-17` | `4` | Signed frequency-transfer ledger closes gravitational, Doppler, source, and path-history redshift as one source-path-receiver record. | Use [signed-frequency-transfer-ledger.mjs](../../../scripts/equation-mapping/signed-frequency-transfer-ledger.mjs) to populate one clean spectral family and the same endpoint cadence/path record as `EQ-07` through `EQ-09`. The current attempt fixture [signed-frequency-transfer-attempt.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-attempt.v1.json) blocks first at `missing_accepted_theta_transfer`. |
...
| `EQ-21` | `3` | Structure growth, matter power, weak lensing/RSD, CMB lensing, and halo/cluster residuals computed from one Noether sea and assembly record. | Populate $\mathcal R_{\mathrm{obs}}(\Theta_{\mathrm{obs}})$ from [eq-21-23-32-shared-observation-residual-packet.md](eq-21-23-32-shared-observation-residual-packet.md) and [shared-observation-residual.mjs](../../../scripts/equation-mapping/shared-observation-residual.mjs); the current attempt blocks at `missing_accepted_theta_obs`. |
| `EQ-22` | `3` | CMB transfer, blackbody, acoustic, lensing, and frame rows derived from shared thermal/provenance, source, path, and growth records. | Reuse the same $\Theta_{\mathrm{obs}}$ as `EQ-21` and add photon-loading and blackbody residual rows; the shared-observation checker remains score-neutral until the CMB projection row is accepted. |
| `EQ-23` | `3` | BBN freezeout, light-element yields, $\eta$, $N_{\text{eff}}$, photon loading, and neutrino rows derived from one source-window thermal record. | Reuse $\Theta_{\mathrm{src}}$ and $\Theta_{\mathrm{therm/prov}}$ inside $\Theta_{\mathrm{obs}}$; the shared-observation checker forbids independent source-zone parameters until accepted retained rows exist. |
...
| `EQ-31` | `2` | Resonance widths, lifetimes, and branching fractions derived from metastable branch stability and admissible escape corridors. | Use [finite-window-statistical-carrier.mjs](../../../scripts/equation-mapping/finite-window-statistical-carrier.mjs) to compute escape measures $\gamma_k$, $\Gamma$, $\tau$, branching fractions, first-exit corridor semantics, null-separatrix mass, and refinement compatibility, then replace the toy input with an accepted retained carrier. |
| `EQ-32` | `3` | RAR/BTFR and low-acceleration galaxy regularities derived from the same Noether sea constitutive response that preserves local gravity, CMB, BBN, growth, and cluster constraints. | Derive $a_\star^\theta(E)$ inside $\Theta_{\mathrm{obs}}$ and prove it does not spoil the large-scale and high-acceleration rows; [shared-observation-residual.mjs](../../../scripts/equation-mapping/shared-observation-residual.mjs) is the current fail-closed shared-record guardrail. |
...
7. **Cosmology shared-observation residual:** `EQ-18`, `EQ-19`, `EQ-21`, `EQ-22`, `EQ-23`, and `EQ-32`. These should advance after the same Noether sea constitutive record has at least one local coefficient extraction; [shared-observation-residual.mjs](../../../scripts/equation-mapping/shared-observation-residual.mjs) currently reports the first blocker as `missing_accepted_theta_obs`.
