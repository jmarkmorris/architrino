...
The next implementation should either add an `EQ-30` detected-class-measures source-evidence probe or draft the retained top-carrier source contract for `EQ-31`. The `EQ-31` parent-carrier ladder now advances through `C`; an accepted-looking top carrier sourced only to this map fails at `accepted_without_evidence_source`. The `EQ-30` ladder advances through `Phi_in` and stops at `missing_accepted_detected_class_measures`. No retained finite-window carrier is closed until all parent carrier rows, the top carrier, and row-specific projection rows are accepted from durable evidence sources.
