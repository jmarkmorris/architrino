...
The next implementation should either add the `EQ-31` `B` source-evidence probe or the `EQ-30` `Q` source-evidence probe. The `EQ-31` parent-carrier ladder currently advances through `K_det` and stops at `missing_accepted_B`; the `EQ-30` parent-carrier ladder advances through `mu_star_T` and stops at `missing_accepted_Q`. No retained finite-window carrier is closed until all parent carrier rows and row-specific projection rows are accepted from durable evidence sources.
