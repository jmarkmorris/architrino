...
The next implementation should either add an `EQ-30` cross-section-comparisons source-evidence probe or harden the retained top-carrier source contract for `EQ-31` before any accepted top-carrier fixture exists. The `EQ-31` parent-carrier ladder now advances through `C`; an accepted-looking top carrier sourced only to this map fails at `accepted_without_evidence_source`, and a generic durable source should also fail until it declares top-carrier support. The `EQ-30` ladder advances through detected class measures and stops at `missing_accepted_cross_section_comparisons`. No retained finite-window carrier is closed until all parent carrier rows, the top carrier, and row-specific projection rows are accepted from durable evidence sources.
