...
- Source-audit basis: the consolidated high-energy survey separated compact-star support into `EQ-07A`, assigned the initial score `2`, and identified `missing_accepted_compact_region_carrier` as the first blocker; the survey itself supplied no accepted retained evidence.
...
| $M_{\mathrm{Ch}}\propto Y_e^2M_\odot$ and $R\propto M^{-1/3}$ | Composition and mass-radius support readout from the same compact-region support ledger. | `composition_row`, `compact_region_record`, `eos_projection` | $Y_e$, $\mu_e$, $M$, $R$, and the EOS projection remain bound to one compact-region record. | Chandrasekhar controls reject composition or mass-radius rows that pass numerically but split from the carrier. | Accepted composition, compact-region, and EOS projection rows with durable source support. |
| TOV pressure-gradient and mass-continuity rows | Observer-level radial support readout through $\Theta_{\mathrm{NS}}(r)$ and compactness response. | `neutron_star_radial_support_row`, `metric_compliance_row`, `metric_projection` | $P(r)$, $\epsilon(r)$, $m(r)$, compactness, radial support, and metric projection share one compact-region carrier. | TOV controls reject radial-support samples that pass arithmetic but are not carrier-bound. | Accepted neutron-star radial-support row plus metric-compliance and metric-projection rows. |
| Electron-capture, photodisintegration, neutrino, heat, and remnant updates | Collapse/reaction readout through one compact-region conservation ledger. | `reaction_inventory_row`, `compact_region_conservation_ledger`, `compact_region_record` | Reaction inventory, neutrino/heat output, medium update, remnant row, and support loss cite the same compact region and window. | Ledger and hidden-retune controls reject reaction rows imported after support or metric rows are fixed. | Accepted reaction inventory and compact-region conservation ledger bound to the same carrier. |
| $\lambda_A$, $\mathcal S_{\mathrm{mat}}$, and $\mathcal S_{\mathrm{metric}}$ | Material-scale compression and effective spatial-compliance readout from the same retained record. | `material_scale_compression_row`, `metric_compliance_row`, `metric_projection` | Material compression, metric compliance, lapse/signal-delay comparison, and horizon-interface activation use the same carrier. | Scale/metric and level-separation controls reject substituting material scale for Fermi spacing or retuning metric response separately. | Accepted material-scale-compression and metric-compliance rows with one no-hidden-retune witness. |
| $\mathcal S_{\mathrm{retune}}$ and source provenance | Retained-record identity and no-hidden-retune witness for the whole compact-star packet. | `source_provenance`, `no_hidden_retune_witness`, all required rows | Every required row cites durable non-priority evidence with the same carrier id, compact region, window, and source-support metadata. | Coordination-source, metadata-missing, and probe-source controls reject priority packets, durable-but-unsupported paths, and probe artifacts as evidence. | A retained $\Theta_{\mathrm{cs}}^{07A}(\Omega,W)$ packet whose required rows are all accepted, source-backed, same-record bound, and checker consumable. |
