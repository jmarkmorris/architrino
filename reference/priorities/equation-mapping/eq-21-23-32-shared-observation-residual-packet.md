...
- `EQ-21` child status: [eq21-growth-transfer-child-residual.mjs](../../../scripts/equation-mapping/eq21-growth-transfer-child-residual.mjs) accepts the first predictive child, [eq21-matter-power-transfer-child-residual.mjs](../../../scripts/equation-mapping/eq21-matter-power-transfer-child-residual.mjs) accepts the matter-power child, [eq21-lensing-transfer-child-residual.mjs](../../../scripts/equation-mapping/eq21-lensing-transfer-child-residual.mjs) accepts the CMB-lensing child, and [eq21-shear-rsd-transfer-child-residual.mjs](../../../scripts/equation-mapping/eq21-shear-rsd-transfer-child-residual.mjs) accepts the shear/RSD child. The accepted inputs consume the same shared-observation parent; the matter-power child consumes the accepted $f\sigma_8$ child; the lensing child consumes the accepted parent, accepted $f\sigma_8$ child, and accepted normalized $P(k,z)$ child; the shear/RSD child consumes that accepted parent/growth/matter/lensing chain; inherited shared keys do not retune. The accepted runs report $f\sigma_8=0.4165634684945517$, `matterPowerGridNormalizedResidual=0` with `sampleCount=6`, `cmbLensingGridNormalizedResidual=0` with `sampleCount=3`, and `shearRsdGridNormalizedResidual=0` with `sampleCount=3`.
- Remaining blocker boundary: the parent observation carrier, first $f\sigma_8$ child, normalized $P(k,z)$ child, normalized CMB-lensing child, and shear/RSD child are score-neutral and populated; they do not yet derive halo/cluster, nonlinear growth, score-review observational transfer, CMB transfer, Planck blackbody, recombination/acoustic, BBN source-window, or galaxy-response child physics.
- Smallest next artifact: a halo/cluster, nonlinear, or score-review observational transfer child, blackbody/acoustic transfer, a BBN source-window row, or a RAR/BTFR galaxy-response law, consuming the accepted shared-observation evidence without replacing it.
...
| --- | --- | --- | --- | --- | --- |
| Structure growth, matter power, $f\sigma_8$, $S_8$, CMB lensing, and shear/RSD | Linear-growth and matter-loading projections from the same Noether sea, baryon, neutral-assembly, effective-coupling, and readout rows. | Accepted $\Theta_{\mathrm{obs}}$ parent with accepted `growth_projection` and $\Theta_{\mathrm{read}}$ rows. | Growth, lensing, shear/RSD, equality scale, free-streaming, and BAO rows must share $\rho_{\text{NS}}$, $\rho_{\mathrm{bar}}$, $\rho_A$, $G_{\mathrm{eff}}^\theta$, $a_{\mathrm{eff}}^\theta$, and $H_{\mathrm{eff}}^\theta$. | Hidden retune between growth, CMB lensing, shear/RSD, and FRW readout rows; failure to consume the accepted upstream `theta_cos` handoff. | [shared-observation-provider-backed-consumer-evidence.v1.json](../../../scripts/equation-mapping/shared-observation-provider-backed-consumer-evidence.v1.json) for the accepted parent and growth projection, [eq21-growth-transfer-child-evidence.v1.json](../../../scripts/equation-mapping/eq21-growth-transfer-child-evidence.v1.json) for the accepted score-neutral $f\sigma_8$ child, [eq21-matter-power-transfer-child-evidence.v1.json](../../../scripts/equation-mapping/eq21-matter-power-transfer-child-evidence.v1.json) for the accepted score-neutral normalized $P(k,z)$ child, [eq21-lensing-transfer-child-evidence.v1.json](../../../scripts/equation-mapping/eq21-lensing-transfer-child-evidence.v1.json) for the accepted score-neutral normalized $C_L^{\phi\phi}$ child, and [eq21-shear-rsd-transfer-child-evidence.v1.json](../../../scripts/equation-mapping/eq21-shear-rsd-transfer-child-evidence.v1.json) for the accepted score-neutral shear/RSD child; BAO, halo/cluster, nonlinear, and score-review transfer remain downstream. |
...
This child computes shear-band and RSD growth readouts from the accepted $\Theta_{\mathrm{obs}}$ parent, accepted $f\sigma_8$ child, accepted normalized $P(k,z)$ child, and accepted normalized $C_L^{\phi\phi}$ child. It remains score-neutral and does not replace the need for halo/cluster, nonlinear, or score-review observational transfer.
