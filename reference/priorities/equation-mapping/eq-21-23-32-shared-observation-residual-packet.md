...
- Remaining blocker boundary: the parent observation carrier is score-neutral and populated; it does not yet derive predictive growth transfer, CMB transfer, Planck blackbody, recombination/acoustic, BBN source-window, or galaxy-response child physics.
- Smallest next artifact: one predictive child consumer, preferably $P(k,z)$ or $f\sigma_8$, blackbody/acoustic transfer, a BBN source-window row, or a RAR/BTFR galaxy-response law, consuming the accepted shared-observation evidence without replacing it.
...
Defer with blocker: promotion should wait until at least one predictive child consumer derives growth transfer, CMB transfer or blackbody/acoustic rows, BBN source-window physics, or the galaxy-response law from the accepted $\Theta_{\mathrm{obs}}$ parent without changing the shared record.
