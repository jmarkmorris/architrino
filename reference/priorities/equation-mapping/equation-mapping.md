...
Priority-only method: each packet should include a direct geometry layer when the equation has identifiable terms. The layer maps each comparison term to its $\mathbb{A}\mathbb{A}\mathbb{A}$ geometric readout, required Noether braid, Noether sea, event-ledger, retained-record, or finite-window carrier, same-record binding, and one fail-closed negative control. This geometry layer complements the accepted-evidence and checker path; it does not move scores by itself. Its job is to expose whether an effective term is only a consumer of native geometry, whether an inverse form reveals a missing native carrier, and whether multiple rows should share one geometric object before any coefficient comparison is accepted.
