...
3. `lorentz_envelope_closure` - Drive $\xi$, $\Gamma_N$, moving-clock, moving-ruler, and energy-momentum maps into one retained Noether braid branch ledger. The translating-binary same-record instantiation is staged in [EQ-02 Through EQ-04 Translating Binary Shared-Record Instantiation](eq-02-04-translating-binary-shared-record-instantiation.md). Passes U-Z now harden the source-backed retained-domain and gamma-free coframe producer path; the current next boundary is the active Cartan $W_{\mathrm{hol}}$ response over that retained-domain support before adding connection transport-step fields, connection-id stability, holonomy-step bounds, or score-moving no-retune claims. Status: `next`. Depends on: `score_five_closure_ladder`.
