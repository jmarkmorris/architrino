...
4. `effective_metric_constitutive_map` - Build the shared Noether sea constitutive record that projects into lapse, spatial compliance, drift, signal delay, PPN rows, weak-field GR observables, and compact-object scale-compression rows. Status: `started`; the density-compression route now has an accepted provider-backed `EQ-24` slice, and the provider-backed `EQ-20` route consumes it through a retained `delta_P_eff` pressure report plus an accepted score-neutral `theta_cos` handoff. `theta_W`, `theta_11_20`, metric/coupling projections, shared local/cosmology coupling checks, and `delta_a_star` remain open downstream. Depends on: `score_five_closure_ladder`.
