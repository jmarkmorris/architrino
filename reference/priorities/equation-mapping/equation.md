...
The current map is explicit as a comparison interface. The first score-neutral growth child now computes $f\sigma_8$ from accepted shared-observation keys, while full predictive $P(k,z)$, CMB-lensing, halo/cluster, and nonlinear transfer remain open.
