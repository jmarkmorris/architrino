...
The current map is explicit as a comparison interface. The first score-neutral growth child now computes $f\sigma_8$ from accepted shared-observation keys, the matter-power child computes a normalized $P(k,z)$ grid from the same accepted parent and growth child, the CMB-lensing child computes a normalized $C_L^{\phi\phi}$ grid from that accepted chain, the shear/RSD child computes same-chain shear-band and RSD growth readouts, and the halo/cluster child checks same-chain lensing/shear/RSD inversion back to the inherited $P(k,z)$ samples. Nonlinear transfer and score-review observational transfer remain open.
