...
Do not attempt full GR closure first. Carry the accepted `theta_11_20` weak-gravity slice into `theta_W`, the `EQ-24` weak-gravity output rows `delta_N`, `delta_gamma_ij`, and `delta_G_eff`, and downstream weak-gravity observables without changing the accepted density, pressure, or `theta_cos` records. Until those consumers pass on the same record, the Poisson-limit handoff remains score-neutral.
...
Carry the populated shared `EQ-11`/`EQ-20` residual into `theta_W`, `EQ-32` `delta_a_star`, growth, CMB, and low-acceleration consumers. Until the same $G_{\mathrm{eff}}$, pressure, relaxation, and effective-$\Lambda$ rows remain no-retune through those consumers, the dark-energy row remains score-neutral.
