...
### 2026-09-02 — Priority owner parked

- At the operator's direction, moved the complete `mapping-nuclear-atomic-molecular` directory under `dormant-deferred`.
- Preserved all three unresolved queue rows, focused packets, retained-evidence records, and historical completion entries. The rows are non-executable while parked.
- The move changes active routing and attention ranking only; it completes no scientific object and advances no nuclear, atomic, or molecular claim.
...
- Kept the merged [Electron Orbitals](../electron-orbitals/priorities.md) note in `dormant-deferred` as historical input. Its private `EOC` compression and digit patterns are definition-driven and noncanonical; they were not promoted into the corpus.
