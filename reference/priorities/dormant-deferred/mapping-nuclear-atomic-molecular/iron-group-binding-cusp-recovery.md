...
- Primary corpus destination: [Nuclear Binding](../../../../content/markdown/aaa/nuclear-atomic/nuclear-binding.md) after the recovery target has a derived or constrained model that passes the rows with a Not advanced disposition below.
...
- the first not advanced row;
- comparison rows for deuteron, diproton, saturation, and a representative heavy split.

The paired focused test is [iron-group-binding-cusp-toy-sweep.test.js](../../../../tests/iron-group-binding-cusp-toy-sweep.test.js). The default coefficient set is a shared global toy set; the script deliberately does not advance for deuteron loss, diproton overbinding, missing saturation, wrong cusp region, hidden coefficient scope, ledger loss, shielded-energy leakage, and missing source-binding promotion readiness.
...
| `confinement_functional` | Corridor scale, surface loss, large-$A$ packing, shell/readout envelope, and finite saturation. | Blocked; [confinement-functional-source-target.v1.json](../../../../scripts/nuclear-atomic/confinement-functional-source-target.v1.json) is durable and parseable, accepts the retained [K-perp-transverse-stiffness-functional-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/K-perp-transverse-stiffness-functional-retained-evidence.v1.json), [V-exc-excitation-potential-functional-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/V-exc-excitation-potential-functional-retained-evidence.v1.json), [rho-NS-confinement-domain-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/rho-NS-confinement-domain-retained-evidence.v1.json), [chi-sea-confinement-delay-factor-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/chi-sea-confinement-delay-factor-retained-evidence.v1.json), [axis-exceptionality-charge-confinement-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/axis-exceptionality-charge-confinement-retained-evidence.v1.json), [same-record-noether-sea-response-confinement-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/same-record-noether-sea-response-confinement-retained-evidence.v1.json), [sigma-eff-extraction-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/sigma-eff-extraction-retained-evidence.v1.json), local open-color projection-limit support, `asymptotic_field_audit`, `local_color_singlet_closure_support`, `local_color_singlet_nucleon_envelope_support`, `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, and `no_free_color_asymptotic_state` source-acquisition rows. The source-acquisition report validates the upstream target rows and names `accepted_proton_color_singlet_envelope` as the first missing upstream accepted row; the accepted no-free-color row is now an input to the top proton/neutron envelope certificates, not envelope, color-singlet closure, branch-interface, no-open-color, or promotion evidence. The remaining blocker packets keep the top branch-interface bundle, color-singlet nucleon envelope bundle, same-domain residual derivation, top color-singlet closure, same-record branch interface, same-record audit, no-open-color, and downstream promotion rows unaccepted. | `missing_accepted_color_singlet_nucleon_envelope` |
| `weak_channel` | Beta-stable band, asymmetry pressure, weak reaction provenance, and weak/noether coupling consistency. | Blocked; retained muon ledger, weak projection, weak quotient, and weak exposure record are present, but downstream weak rows remain attempt-level. The source-acquisition report accepts those four targets, validates downstream target component shape, names `va_chirality_gate` as the first missing upstream accepted row, and now carries packets required before advancement for the toy-bound `reaction_event_ledger` and weak-channel `noether_sea_response` update row. | `missing_accepted_va_chirality_gate` |
...
The projection/surface request now has accepted retained evidence in [open-color-projection-surface-support-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/open-color-projection-surface-support-retained-evidence.v1.json). The accepted rows are narrow: `bounded_open_color_projection_operator_norm` follows from the idempotent $\Pi_{\mathrm{open}}$ witness on the finite retained audit domain with $\kappa_{\mathrm{open}}=1$, and `finite_open_color_surface_measure` follows from the normalized compact audit surface with $\Omega_{\mathrm{open}}=1$. This makes the same-event ledger request smaller, but `K_open_finite`, `same_event_ledger`, `same_record_no_open_color_audit`, `no_open_color_far_field`, and all branch-interface promotion rows remain blocked.

The same-record branch-interface/confinement-functional support join now has accepted retained evidence in [same-record-branch-interface-confinement-functional-audit-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/same-record-branch-interface-confinement-functional-audit-retained-evidence.v1.json). It accepts only `same_record_branch_interface_confinement_functional_audit`, tying the accepted branch-interface ledgers, same-record $E,\mathbf p,\mathbf J$ accounting, accepted $\sigma_{\mathrm{eff}}$ extraction, the same-record $\sigma_{\mathrm{eff}}$/color-singlet/branch-interface/Noether-sea tail support join, and the open-color projection support rows into one support domain. This removes that support join from the no-open audit blocker, but `same_event_ledger`, `K_open_finite`, `N_open_R_le_K_open_T_NN_R_squared`, `same_record_no_open_color_audit`, `no_open_color_far_field`, and branch-interface promotion rows remain unaccepted.
...
That check currently reports `structuralPass: true` for the $\sigma_{\mathrm{eff}}$, color-singlet envelope, $\Delta E_{\mathrm{corr}}^{NN}$, no-open-color, and toy-binding dependency chain but keeps `--require-accepted` blocked until `sigma_eff_extraction`, `color_singlet_nucleon_envelope`, `delta_E_corr_NN`, and `no_open_color_far_field` become accepted source rows. Its source-acquisition check accepts the retained $K_{\perp}$ transverse-stiffness source row, the retained $V_{\mathrm{exc}}$ excitation-potential source row, the retained $\rho_{\text{NS}}$ confinement-domain source row, the retained $\chi_{\text{sea}}$ delay-factor source row, the retained axis-exceptionality charge source row, the retained same-record Noether sea response source row, the retained $\sigma_{\mathrm{eff}}$ extraction row, local color-singlet closure support, local color-singlet nucleon-envelope support, finite-range residual, and no-free-color asymptotic state. The $\sigma_{\mathrm{eff}}$ row now has all declared source-acquisition rows accepted, but the top row itself remains target-only; broader confinement acquisition still reports `sourceAcquisitionPass: false` with `sourceAcquisitionFirstMissingObject: missing_accepted_accepted_proton_color_singlet_envelope`. The checker now emits `sourceAcquisitionBlockerMap`; its first active blocker is `accepted_proton_color_singlet_envelope`, which holds `color_singlet_nucleon_envelope` and directly blocks `alphaSurf`, `alphaPair`, `alphaShell`, `boundaryDegreeLoss`, and `bounded_degree_surface_depleted_corridor_estimator` until accepted proton color-singlet closure, finite envelope boundary, accepted no-free-color asymptotic-state input, and same-record branch-interface evidence exist in the same confinement-functional record. The accepted-$\sigma_{\mathrm{eff}}$, same-record branch-interface, accepted branch-interface row bundle, proton, neutron, accepted color-singlet nucleon envelope, color-singlet closure, no-free-color, accepted-$\Delta E_{\mathrm{corr}}^{NN}$, finite-range residual, same-record no-open-color audit, no-open-color, and finite-overlap blocker packets point to the exact acceptance condition for the color-singlet and $\Delta E_{\mathrm{corr}}^{NN}$ bundle: accepted $\sigma_{\mathrm{eff}}$ extraction, accepted same-record branch-interface bundle, accepted top-level branch-interface rows, accepted color-singlet nucleon envelope bundle, accepted color-singlet closure, same-domain residual derivation, finite envelope boundary, finite residual tail, same-record no-open-color audit, no-open-color far-field closure, and no-free-color asymptotic-state audit in the same retained confinement-functional domain. The finite-tail controls (`alphaPack`, `dSat`, `maxDegree`, `packSoftA`, and `finite_tail_saturation_check`) now name `finite_range_residual` as a required row before graph-rule promotion, and that route now consumes `local_color_singlet_nucleon_envelope_support` instead of requiring the downstream top color-singlet envelope first.

The first proton-envelope source request now has a precise certificate shape: `proton_projection_boundary_no_free_color_implies_envelope_support_0001`. The source object must keep the accepted proton branch-interface ledger, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, same-record branch-interface, $\Pi_{\mathrm{singlet},X}$, $\Pi_{\mathrm{open},X}$, $W_{\mathrm{locked},pX}$, $\epsilon_{pX}$, $\Delta_{\mathrm{color},pX}$, $E_{\mathrm{color},pX}\le\Delta_{\mathrm{color},pX}$, finite $R_p$, refinement window, and accepted no-free-color asymptotic-state row in one retained event record. The local support already supplies `proton_color_singlet_closure` and `finite_envelope_boundary`; the top `accepted_proton_color_singlet_envelope` row remains blocked until same-record branch-interface and envelope-certificate evidence are accepted in the same domain.

That route now has accepted retained support for `proton_color_singlet_closure` and `finite_envelope_boundary` in [proton-color-singlet-envelope-support-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/proton-color-singlet-envelope-support-retained-evidence.v1.json). The retained support row fixes the local proton open-color exposure ratio at $E_{\mathrm{color},pX}=0\le\Delta_{\mathrm{color},pX}=0$ and records finite normalized envelope radii $R_p=1$ and $R_n=1$. This is not accepted proton-envelope evidence: `accepted_proton_color_singlet_envelope`, the neutron envelope, color-singlet closure, same-record branch interface, finite residual corridor overlap, and no-open-color rows remain downstream blockers.

The same local-envelope route now has accepted retained support for `neutron_color_singlet_closure` in [neutron-color-singlet-envelope-support-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/neutron-color-singlet-envelope-support-retained-evidence.v1.json). The retained support row fixes the normalized neutron open-color exposure ratio at $E_{\mathrm{color},nX}=0\le\Delta_{\mathrm{color},nX}=0$ without A-, Z-, isotope-, element-, or Fe/Ni-indexed tuning. This is not accepted neutron-envelope evidence: `accepted_neutron_color_singlet_envelope`, color-singlet closure, same-record branch interface, same-record no-open audit, and no-open-color rows remain downstream blockers.

The no-free-color asymptotic-state route now has accepted retained evidence in [no-free-color-asymptotic-state-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/no-free-color-asymptotic-state-retained-evidence.v1.json). It consumes accepted local proton and neutron color-singlet closure support, finite $R_p$ and $R_n$, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, the same-record branch-interface/confinement-functional support audit, both open-color projection limits $\lim_{R\to\infty}\sup_X E_{\mathrm{color},pX}(R)=0$ and $\lim_{R\to\infty}\sup_X E_{\mathrm{color},nX}(R)=0$, and accepted `finite_range_residual` in one retained support domain. It accepts only `no_free_color_asymptotic_state`; accepted proton/neutron envelope rows, `color_singlet_closure`, same-record branch interface, and no-open-color remain downstream blockers.

The local color-singlet support join now has accepted retained evidence in [local-color-singlet-closure-support-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/local-color-singlet-closure-support-retained-evidence.v1.json). It accepts only `local_color_singlet_closure_support`, joining accepted local proton and neutron color-singlet closure support, finite envelope boundary, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, accepted `asymptotic_field_audit`, and the same-record branch-interface/confinement-functional support audit. It now feeds the accepted no-free-color evidence together with `finite_range_residual`. The top color-singlet closure, accepted proton/neutron envelopes, same-record branch interface, and no-open-color rows remain downstream.

The open-color projection zero-limit subrequest now has accepted retained support in [open-color-asymptotic-projection-limits-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/open-color-asymptotic-projection-limits-retained-evidence.v1.json). It accepts only the local support rows `lim_R_to_infty_sup_X_E_color_pX_R_eq_0` and `lim_R_to_infty_sup_X_E_color_nX_R_eq_0`, derived from the accepted local proton/neutron closure support, finite envelope boundary, and same $\sigma_{\mathrm{eff}}$ domain. Those limit rows feed the accepted local `asymptotic_field_audit` support object, but they do not accept the top no-free row; accepted proton/neutron envelopes, same-record branch interface, top color-singlet closure, and no-open-color rows remain downstream.
...
The finite-range residual blocker now carries a priority-only proof scaffold for the prior tail step: $\mathcal T_{NN}(R)=\sup_{r\ge R}|\Delta E_{\mathrm{corr}}^{NN}(r;\Gamma_{N_1},\Gamma_{N_2},\sigma_{\mathrm{eff}},\rho_{\text{NS}},\chi_{\text{sea}})|$ with target $\lim_{R\to\infty}\mathcal T_{NN}(R)=0$. The dedicated [delta-E-corr-NN-tail-limit-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-limit-retained-evidence.v1.json) object now accepts only `Delta_E_corr_NN_tail_limit` from the accepted local nucleon-envelope support, accepted $\sigma_{\mathrm{eff}}$, branch-interface ledgers, same-record Noether sea response, and tail-calculus rows. The immediate finite-range source request is now same-domain retained evidence that joins the accepted tail-limit row with `bounded_residual_overlap` and `large_r_zero_limit`.

The tail-calculus support rows now have accepted retained evidence in [delta-E-corr-NN-tail-calculus-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-calculus-retained-evidence.v1.json). With the retained support constants $R_0=1$, $C=1$, $\lambda=1$, $C_w=1$, and $\eta=0$, the accepted support supplies `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail` without A-, Z-, isotope-, element-, or Fe/Ni-indexed tuning. The separate tail-limit retained evidence now consumes those support rows to accept `Delta_E_corr_NN_tail_limit`; it still does not accept finite-range residual, same-record no-open-color, no-open-color far-field, no-free-color, or promotion rows.

The same-record support join named by the tail-limit blocker now has accepted retained evidence in [same-record-sigma-eff-color-singlet-branch-interface-noether-sea-response-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/same-record-sigma-eff-color-singlet-branch-interface-noether-sea-response-retained-evidence.v1.json). It accepts only `same_record_sigma_eff_color_singlet_branch_interface_noether_sea_response`, joining accepted $\sigma_{\mathrm{eff}}$, local proton/neutron color-singlet closure support, finite envelope boundary, branch-interface ledgers, same-record $E,\mathbf p,\mathbf J$ accounting, same-record Noether sea response, and the accepted tail-calculus support rows in one retained support domain. The local nucleon-envelope support row and tail-limit evidence build on that support domain, but they still do not accept finite-range residual or no-open-color rows.

The same packet now spells out the uniform exponential-tail certificate. The retained object carries one retained event id, $\Delta E_{\mathrm{corr}}^{NN}$, $\mathcal T_{NN}(R)$, $R_0=1$, $C=1$, $\lambda=1$, corridor weight $w_{\mathrm{corr}}$, $C_w=1$, $\eta=0$, an $\eta<\lambda$ witness, accepted $\sigma_{\mathrm{eff}}$ extraction, the local nucleon-envelope support ref, branch-interface ledger refs, same-record Noether sea response, and coefficient exclusion. It accepts only `Delta_E_corr_NN_tail_limit`.

The same tail route now has accepted retained support for the corridor-weight and coefficient-exclusion prerequisites in [delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json). It fixes the normalized corridor weight $w_{\mathrm{corr}}(r)=1$ with $C_w=1$ and $\eta=0$, so the accepted exponential tail with $\lambda=1$ has $0\le\eta<\lambda$. It also accepts the audit that $R_0$, $C$, $\lambda$, $C_w$, $\eta$, $\mathcal T_{NN}$, $\mathcal O_{NN}$, and $w_{\mathrm{corr}}$ are not A-, Z-, isotope-, element-, or Fe/Ni-indexed. This support file is still not finite-range residual evidence.

The tail route also now has accepted retained support for the uniform large-$R$ bound prerequisite in [delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json). It supplies the normalized support envelope with $R_0=1$, $C=1$, and $\lambda=1$ and keeps the coefficient-exclusion audit intact. This accepts only `uniform_large_r_bound_C_lambda_R0`; the separate tail-limit evidence consumes it, while finite-range residual, same-record no-open-color audit, and no-open-color far-field rows remain blocked.

The finite-range residual blocker now has accepted finite-tail support aliases in [finite-range-tail-support-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/finite-range-tail-support-retained-evidence.v1.json). The retained evidence consumes accepted `lim_R_to_infty_T_NN_R_eq_0`, accepted `O_NN_finite`, and accepted `exists_R0_C_lambda_exp_decay_tail` in the normalized tail-support domain to supply only `large_r_zero_limit` and `bounded_residual_overlap`. With `Delta_E_corr_NN_tail_limit` accepted separately, [finite-range-residual-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/finite-range-residual-retained-evidence.v1.json) binds those support aliases to accepted $\sigma_{\mathrm{eff}}$, local nucleon-envelope support, branch-interface, same-record Noether sea response, and tail-limit rows to accept only `finite_range_residual`; [no-free-color-asymptotic-state-retained-evidence.v1.json](../../../../scripts/nuclear-atomic/no-free-color-asymptotic-state-retained-evidence.v1.json) then consumes that row to accept only `no_free_color_asymptotic_state`. Neither evidence object accepts `finite_residual_corridor_overlap`, `accepted_delta_E_corr_NN`, `same_record_no_open_color_audit`, or `no_open_color_far_field`.
