# Transactions, Eligibility, and Measurement Records
...
Ruth E. Kastner's transactional interpretation provides an external comparison for these distinctions. The available local account describes the Possibilist Transactional Interpretation, abbreviated PTI, and its relativistic development, abbreviated RTI. Its central contrast is between precursor possibilities and an actualized transaction. The comparison with $\mathbb{A}\mathbb{A}\mathbb{A}$ concerns what earns an outcome-bearing record and how outcome weights acquire physical meaning. It does not identify the two frameworks' underlying entities.
...
These distinctions matter even when a theory proposes one process that answers several questions together. Conservation can be a necessary condition for a transaction without uniquely selecting the transaction. An account can explicitly postulate irreducible indeterminism while quantifying rates. A persistent spacetime event and a detector state stable over an observation window also express different notions of persistence. Comparing them requires identifying the observable record and its time dependence, rather than assuming that the common word already establishes equivalence.
...
The captured replies sharpen the account. Kastner describes absorber responses as precursors to one actualized transaction; photon transfer establishes an emission event, an absorption event, and their connection. She makes satisfaction of conservation laws a prerequisite for a transaction, identifies persistence with the invariant status of the resulting spacetime event, and also describes the durable record as the new spatial interval created in the transaction. She explicitly treats the transition from many possibilities to one outcome as intrinsically indeterministic, quantified through decay rates. The local source contains no derivation of those rates.
...
The retained companion-paper summary says that real photons are both emitted and absorbed, have finite lifetimes, and permit an observer-independent account of the measurement transition without modifying the quantum equations. The interview likewise distinguishes ordinary scattering or correlation from an actual source–absorber transaction. These are reported mechanism claims awaiting equation-level examination. An abstract, chapter summary, or interview assertion cannot establish that the claimed mechanism has met every conservation, probability, and signaling requirement.
...
The $\mathbb{A}\mathbb{A}\mathbb{A}$ comparison starts from architrinos, delayed path-history interaction, Euclidean void, and absolute time. Quantum descriptions and observer-level spacetime behavior are recovery targets. A shared interest in emergent spacetime does not identify RTI's possibility domain with the void, a causal wake, or an assembly. An event graph can organize records at an effective level without replacing the substrate that must generate them.

The intake describes time-symmetric mathematical solutions but also records Kastner's objection to treating offers and confirmations as small waves literally traveling through spacetime, or interpreting the account as controllable travel into the past. That distinction prevents a simplified handshake picture from becoming a literal substrate mechanism. The claimed direct-action origin of projection-like operators and Born probabilities remains a primary-source comparison target; it is not established here by naming those mathematical objects.
...
The proposed record predicate is
...
where $\mathsf C_\theta$ tests detector coupling, $\mathsf L_\theta$ tests an energy/momentum ledger condition, $\mathsf P_\theta$ tests record persistence, and $\mathsf N_\theta$ names the no-signaling and context-consistency condition. The formula is a proposed organization of obligations. It is not a completed derivation of any of its factors from delayed assembly dynamics.

For the product to be an eligibility predicate, each factor must take values in $\{0,1\}$. The product then equals one exactly when all four conditions hold. This logical equivalence is derived directly from Boolean multiplication and does not require the factors to be statistically independent. If the factors were real-valued scores, the same product would instead be a score; interpreting its level set at one as physical eligibility would require another stated rule.
...
Assume a finite or countable outcome family, measurable basins and eligible set, and a finite positive total eligible weight. The source's record-normalized assignment is
...
Write the denominator as $Z(T;\theta)$. Nonnegativity of the measure and $0<Z(T;\theta)<\infty$ imply $p_i^{\mathrm{rec}}\geq0$ and $\sum_i p_i^{\mathrm{rec}}=1$. These are conditional mathematical consequences of normalization. If the basins are pairwise disjoint and cover the eligible set up to a set of measure zero, countable additivity gives $Z=\mu_{*,T}(E_\theta(T))$. In that case the expression is the conditional probability of a mutually exclusive detector outcome given eligibility. Overlapping basins can still yield normalized numbers, but a history counted in two basins no longer represents an exclusive record without an additional outcome-assignment rule.
...
A simple derived probability example shows why this matters. Let a binary variable $X$ be uniformly distributed on $\{0,1\}$, let a setting $b$ take either value, and keep only histories satisfying $R_b=1$ when $X=b$. Before filtering, $P(X=1)=1/2$ for either setting. The retained fraction is also $P(R_b=1)=1/2$ for either setting, yet

$$
P(X=1\mid R_b=1)=b.
$$

Thus an unchanged unfiltered distribution and equal retained fractions do not imply unchanged conditioned outcome distributions. This is an abstract conditioning example, not a physical signaling construction: it specifies neither separated apparatuses nor local access to the retained subset. It proves only that an eligibility filter cannot acquire no-signaling merely by normalizing its output. A physical assessment must specify which records are locally accessible, how selection is implemented, and which marginals are compared.

Kastner's captured reply ties no-signaling to the light-speed connection between emission and absorption. That states her offered explanation. The saved packet does not contain a complete proof of the required operational marginal independence, so it neither verifies that proof nor establishes that RTI lacks one. The same evidential restraint applies to the assembly proposal: a factor bearing the name of a constraint is not evidence that the dynamics satisfies the constraint.
...
The six-part correspondence mapping is useful at this limited level. Absorber responses compare with candidate detector histories; conservation admissibility compares with coupling and ledger conditions; an actualized source–absorber connection compares with a realized recorded event; invariant-event persistence compares with stability over an apparatus record window; the claimed absence of signaling compares with cross-context statistical consistency; irreducible actualization compares with the still-open question of final selection. Each comparison identifies a question shared by the descriptions. None establishes equal ontology, equal equations, or equal explanatory success.
...
The useful inferred lesson is chronological and evidential. Preparation establishes the initial experimental situation. A nondestructive confirmation can constrain it without being the final outcome. Post-selection restricts the retained cases. A durable detector record supplies the final recorded event under the declared channel. Calling each of these a which-path measurement without distinguishing what is known, when it is known, and from which retained cases invites an inference the evidence does not license.
...
A meaningful comparison would identify a precise result, its assumptions, and the quantity an existing detector or record account must reproduce. The claim that direct-action physics generates projection-like operators or Born probabilities is a suitable subject for equation-level primary reading. It is not converted into an assembly derivation by replacing transactional terminology with basin terminology. Likewise, a measure chosen to produce desired statistics is evidence about that choice until its origin in the dynamics is established.
...
Claims about a deeper realm of possibilities, a denial that gravity is a quantum field, gravity or dark-matter-like rotation-curve mechanisms, and connections to consciousness or volition remain external interpretation or speculation in this packet. They do not enter the assembly account as premises. Even the correspondence's explicit appeal to volition characterizes its author's position; it supplies no physical test of that position here.

The preserved primary-source leads make a later comparison possible while the current evidence boundary remains precise. A complete primary derivation could change the assessment of what RTI establishes; a derived assembly measure and detector rule could change what the conditional record formula establishes. Until those objects are supplied, the productive comparison concerns the structure of the questions and the assumptions each proposed answer must make explicit.
