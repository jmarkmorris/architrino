...
## Complete Insight And Open-Issue Coverage Audit — 2026-08-05

This audit compared the original seven-part Bell closure request, the
2026 EPR lecture packet, the 2022 Bell-foundations interview packet, the
primary-source experiment discussion, and the live corpus decision
reconstruction against this lane's four canonical files. The inventory below
is the durable completeness check for the discussion to date.

### Retained Insight Inventory

| Insight | Grade | Durable disposition |
| --- | --- | --- |
| Bell's early program is EPR-sympathetic: it rehabilitates deeper variables, then finds the obstruction for the local branch. | Historical inference from Bell 1964 and 1966 | Retain the garden-path refinement; do not call the published theorem accidental. |
| “Realism,” determinism, free will, and counterfactual definiteness are not substitutes for Bell's mathematical assumptions. | Derived scope clarification | State factorization, measurement independence, and trial assumptions separately. |
| A passive record fixed at entanglement remains inside the complete past state $\lambda$. | Derived | Retain as the Bell-local negative control under measurement independence and valid sampling. |
| One-particle predecision does not entail a valid two-wing answer table over independently chosen settings. | Derived scope distinction | Require the full setting-indexed joint law before using “decided at entanglement” in a Bell claim. |
| Definite substrate states and one real detector record do not derive the two-wing Bell correlation. | Derived limitation | Keep measurement ontology and Bell correlation closure as separate obligations. |
| A global conservation or pair-provenance ledger constrains the source but does not supply local response functions. | Derived limitation | Separate source constraint, locally available variables, apparatus response, live influence, and joint distribution. |
| Operational no-signaling is weaker than Bell factorization. | Derived | A nonfactorizable model may preserve setting-independent one-wing marginals; both checks remain mandatory. |
| Counts, correlations, and the Bell statistic are arithmetic after a trial table is fixed, while mapping hardware into that table is a physical and statistical inference. | Derived distinction | Keep trial boundaries, pairing, detection, spacelike timing, memory, nonstationarity, setting predictability, and stopping rules explicit. |
| Enlarging $\lambda$ with braid complexity, $q\mathbf V$, device history, path history, or Noether sea state does not alter CHSH while locality and independence remain. | Derived | More source memory is not a Bell escape by itself. |
| Detector inefficiency matters through conditional inclusion, not through apparatus complexity alone. | Derived model possibility | Require explicit inclusion variables and efficiency, threshold, or timing-window predictions. |
| A strictly wing-local detector or setting response remains inside $A(x,\lambda)$ or $B(y,\lambda)$. | Derived | Local apparatus dynamics cannot exceed the Bell bound under measurement independence and valid sampling. |
| Deterministic dynamics does not imply measurement dependence. | Derived distinction | A deterministic setting generator can remain statistically independent of the prepared pair. |
| A shared Noether sea does not by itself establish measurement dependence. | Derived limitation | Require a quantitatively adequate $P(\lambda,x,y)$ and a setting-source or sea-state discriminator. |
| The 2015 loophole-controlled experiments used laboratory setting generators; stellar and quasar setting controls arrived in 2017 and 2018. | Measured chronology | Never merge the loophole-free and cosmic-setting claims or apparatus assumptions. |
| Cosmic settings constrain the spacetime region of a possible pair--setting common cause but do not logically eliminate superdeterminism. | Measured result plus scoped inference | Preserve fair-sampling, coincidence, astrophysical, and cosmological assumptions. |
| No experiment is strongest in every category. | Evidence-audit conclusion | Compare loophole closure, statistical strength, cosmic provenance, baseline, speed bounds, and multipartite relevance separately. |
| A live finite-speed channel can violate Bell factorization in a preferred frame but is constrained by event timing and multipartite theorems. | Inferred route plus derived constraints | Keep as a conditional comparator only; EPRB-003 and EPRB-007 own its burdens. |
| Finite-influence-speed bounds are conditional lower bounds, not universal measured speeds or measurements of $c_f$. | Measured result with model boundary | State baseline, frame, simultaneity uncertainty, velocity assumption, and sampling conditions for every bound. |
| Bancal excludes a specified finite-speed hidden-influence class, not every conceivable medium. | Derived theorem, scope-limited | Any shielding proposal must name the changed screening, independence, marginal, or no-signaling premise. |
| Loss of live-channel reach does not imply $S\to2$. | Derived limitation | Declare the unreachable fallback law, reach fractions, and timing mixture. |
| A hierarchy between a hypothetical Bell influence and photon propagation does not imply dispersion. | Derived limitation | Frequency dependence requires a separate constitutive equation. |
| Under present absolute-time ontology, live forward coordination and common-past measurement dependence are structurally admissible comparisons; retrocausality is not, and a global or instantaneous law changes the primitive inventory. | Ontology compatibility map | Treat excluded-ontology routes only as comparisons until an explicit ontology decision changes their status. |
| GHZ supplies an all-or-nothing multipartite comparison in its ideal form. | Primary-source lead | Keep inside the existing Quantum Closure Bell gate; use primary theorem and experiment sources for exact claims. |
| PBR is scoped by ontic overlap, preparation independence, and retained quantum predictions. | Derived theorem boundary | It does not prove that the wavefunction is a primitive field or select an EPR--Bell route. |
| The measurement triad identifies which quantum claim a completion changes but does not perform the missing Born, linear-regime, or record derivation. | Established taxonomy | Quantum Closure owns those derivations. |
| Pilot-wave theory demonstrates that definite, deterministic ontology can be explicitly nonlocal. | Comparison model | Do not import its configuration-space wavefunction or guidance law as Architrino ontology. |
| Observers and measurements should be physical systems and interactions, not undefined primitives. | Methodological requirement | Already owned by measurement ontology; no duplicate Bell gate. |
| Coordinate labels are not intrinsic geometry, entropy does not generate time, and Lorentzian causal reconstruction is not evidence for absolute time. | Adjacent comparison insight | Existing geometry, entropy, and Lorentz-recovery owners retain these points; no EPR--Bell task. |
| The live corpus has a provisional finite-$c_f$ preference but no valid settled decision and contains a detector-transfer contradiction. | Live-corpus audit | EPRB-002 owns the decision and reconciliation after EPRB-001. |

Plainly: every useful result from the discussion and both videos is either a
Bell-lane rule, a blocked action, an explicitly routed adjacent obligation, or
a rejected overstatement. None is being used to select a route silently.

### Open-Issue And Action Coverage

| Open issue | Canonical action | Current blocker |
| --- | --- | --- |
| Exact meaning of “decided at entanglement” | EPRB-001 | Operator must state whether local response and later-setting independence are preserved. |
| Actual route status and contradictory owning-page language | EPRB-002 | EPRB-001 and a scoped operator decision. |
| Finite-speed multipartite viability and Bancal shielding | EPRB-003 | Live channel must remain under comparison after EPRB-001. |
| Detector, setting, timing, selection, memory, and shared-sea mechanisms | EPRB-004 | Selected route plus accepted detector variables from Quantum Closure. |
| New strongest experiments and precise primary-source values | EPRB-005 | Refresh only when a material result or route decision makes it decision-relevant. |
| Quantitative forward-causal measurement dependence | EPRB-006 | Measurement dependence must remain an active comparison route. |
| Finite-$c_f$ reach, fallback, crossover, speed identification, and dispersion separation | EPRB-007 | EPRB-003 and retention of the live-channel route. |
| Negative control and computed Bell-gate handoff | EPRB-008 | Quantum Closure `QC-001`, `QC-005`, and `QC-006`. |
| PBR, Born measure, effective linearity, one-record formation, GHZ, Hardy, and Tsirelson calculations | Quantum Closure and no-go-theorem owners | Not duplicated in this lane; tracked through the cross-lane dependency table. |

Plainly: the first row is the only decision currently executable. Every other
open issue has an owner, prerequisite, and completion boundary rather than an
unassigned “future work” note.
...
Plainly: the spin-singlet correlation is the negative cosine of the angle
between the two spin-measurement axes.

Optimal CHSH axes give

$$
|S|=2\sqrt2\approx2.828.
$$

Plainly: this quantum angular correlation cannot be reproduced by one
...
## What $E(\theta)$ Means And Why The Singlet Gives $-\cos\theta$

The symbol $E$ here means correlation, not energy. In one trial, Alice and
Bob record binary outcomes $a,b\in\{-1,+1\}$. Their product is $+1$ when the
labels agree and $-1$ when they disagree. For fixed detector directions,

$$
E(\theta)
=\langle ab\rangle
=\sum_{a,b=\pm1}ab\,P(a,b\mid\theta).
$$

Plainly: $E$ is the average agreement score. It is $+1$ for outcomes that
always match, $-1$ for outcomes that are always opposite, and $0$ when
agreement and disagreement balance.

For the spin-$\tfrac12$ singlet comparison, the pair state is
...
Plainly: the state has total spin zero and gives opposite labels whenever
both stations measure along the same axis.

Let $\hat{\mathbf x}$ and $\hat{\mathbf y}$ be the unit vectors defining the
two spin-measurement axes, and let $\theta$ be the angle between them. The
binary quantum observables use the Pauli-matrix vector
$\boldsymbol{\sigma}$:

$$
A=\hat{\mathbf x}\cdot\boldsymbol{\sigma},
\qquad
B=\hat{\mathbf y}\cdot\boldsymbol{\sigma},
$$

Plainly: each operator asks whether the local spin lies with or against the
chosen axis and assigns the answer $+1$ or $-1$.

The predicted product average is

$$
E(\hat{\mathbf x},\hat{\mathbf y})
=
\langle\psi^-|
(\hat{\mathbf x}\cdot\boldsymbol{\sigma})
\otimes
(\hat{\mathbf y}\cdot\boldsymbol{\sigma})
|\psi^-\rangle.
$$

Plainly: this is the quantum rule for averaging the product of the two local
binary records over repeated preparations of the same pair state.

Because the singlet has no preferred spatial direction, rotational symmetry
requires its correlation tensor to have the form
...
Plainly: a directionless pair cannot assign different correlation strengths
to the $x$, $y$, and $z$ axes; only matching components can survive, with one
common coefficient $C$.

Same-axis measurements are always opposite, so that common coefficient is
$C=-1$. The singlet correlation tensor is therefore
...
Plainly: matching spatial components are perfectly anticorrelated, while
orthogonal components have zero product average.
...
Plainly: parallel axes give $E=-1$, perpendicular axes give $E=0$, and axes
pointing in opposite directions give $E=+1$.

The same result can be expressed through agreement probabilities:

$$
P(\mathrm{same}\mid\theta)=\sin^2\frac{\theta}{2},
\qquad
P(\mathrm{opposite}\mid\theta)=\cos^2\frac{\theta}{2}.
$$

Plainly: the singlet increasingly changes from opposite labels to matching
labels as one axis rotates from parallel to antiparallel with the other.

Subtracting the two probabilities gives

$$
E(\theta)
=P(\mathrm{same}\mid\theta)-P(\mathrm{opposite}\mid\theta)
=-\cos\theta.
$$

Plainly: the cosine is not an arbitrary curve fitted to Bell data. It follows
from the singlet's rotational symmetry, the geometry of two axes, and perfect
same-axis anticorrelation within the quantum comparison model.

For linear-polarization Bell tests, the physical analyzer axis is unchanged
by a $180^\circ$ rotation. With the polarization singlet and the usual binary
labeling, the corresponding comparison law is below; some Bell states or
outcome conventions change its overall sign.

$$
E(\alpha,\beta)=-\cos\!\bigl(2(\alpha-\beta)\bigr),
$$

Plainly: photon-polarization plots commonly contain twice the analyzer-angle
difference. That is a property of polarization geometry and labeling, not a
contradiction with the spin-singlet formula.

This calculation is an observer-level quantum recovery target. It is not an
Architrino-level premise. A completed $\mathbb{A}\mathbb{A}\mathbb{A}$ account
must derive the same setting-indexed probabilities from its source measure,
pair histories, apparatus-response kernels, and trial-selection rule.

Plainly: reproducing the words “rotational symmetry” or “singlet” does not
close the theory. The substrate model must generate the actual cosine-shaped
record table.

## Prediction-To-Assembly Recovery Map — 2026-08-05

**Claim level:** derivation target with an observer-level benchmark. This map
does not choose a Bell route and does not import the quantum state or Pauli
operators into Architrino ontology. It states what an assembly calculation
must emit before any route can claim success.

For a spin-singlet preparation, all four binary joint probabilities can be
written as one target table:

$$
P_{\mathrm{target}}(a,b\mid\hat{\mathbf m}_A,\hat{\mathbf m}_B)
=
\frac14
\left(
...
Plainly: the target is not merely the curve $E=-\cos\theta$. It specifies how
often every pair of detector records must occur for every pair of detector
orientations.

The corresponding assembly interface begins with a prepared source history
$\gamma_{\mathrm{src}}$ and exports a retained pair-provenance record:

$$
\Pi_{AB}=C_{\mathrm{pair}}(\gamma_{\mathrm{src}}),
\qquad
\rho_{\mathrm{src}}
=C_{\mathrm{pair}*}\mu_{\mathrm{src}}.
$$

Plainly: the source assembly must generate a distribution of actual pair
records—angular-momentum balance, ordered braid frames, phases, causal-wake
history, and relational data—rather than selecting probabilities from the
desired Bell table.

Let $\zeta_A$ and $\zeta_B$ denote unresolved initial states of the two
detector assemblies, and let
$B_{ab}^{\hat{\mathbf m}_A,\hat{\mathbf m}_B}$ denote the basin whose completed
apparatus records are $(a,b)$. A route-neutral assembly prediction has the
pushforward form

$$
P_{\mathbb{A}\mathbb{A}\mathbb{A}}
(a,b\mid\hat{\mathbf m}_A,\hat{\mathbf m}_B)
=
\int
\mathbf 1_{B_{ab}^{\hat{\mathbf m}_A,\hat{\mathbf m}_B}}
\!\left(
\Phi_{\hat{\mathbf m}_A,\hat{\mathbf m}_B}
...
Plainly: evolve the prepared pair and two physical detector assemblies,
classify the two persistent records, and measure the histories landing in
each joint record basin. The target probabilities must emerge from that
calculation.

The function $\Phi_{\hat{\mathbf m}_A,\hat{\mathbf m}_B}$ is intentionally
route-neutral. Bell-local response makes it reducible to two screened local
responses. Live coordination gives it a preferred-time cross-wing input.
Measurement dependence changes the source measure. A selection mechanism
changes which completed basins enter the reported trial table.

Plainly: one prediction interface can compare the candidate routes without
silently building a preferred route into the benchmark.

The assembly-generated correlation is

$$
E_{\mathbb{A}\mathbb{A}\mathbb{A}}
...
Plainly: $E$ is computed after the assembly dynamics emits its four joint
record frequencies; it is not supplied to the dynamics as an interaction
law.

A compact spin-singlet recovery residual is

$$
\Delta_{\mathrm{singlet}}
=
...
Plainly: a successful ideal-limit assembly model recovers the entire angular
curve, not merely four hand-picked CHSH points.

| Observer-level prediction | Required assembly object | Failure diagnostic |
| --- | --- | --- |
| Two binary outcomes | Stable detector record basins produced by local material and Noether-sea dynamics | No persistent record, ambiguous sign, or outcome assigned before a basin forms |
| Same-axis anticorrelation | Pair-provenance ledger and matched-axis detector response | Excess matched-axis same-record events beyond declared apparatus error |
| $E(\theta)=-\cos\theta$ | Source measure plus orientation-dependent record-basin pushforward | Nonzero $\Delta_{\mathrm{singlet}}$ or a curve fitted only at selected settings |
| Unbiased local outcomes | Sign-symmetric one-wing basin marginals | Marginal drift from one half or dependence on the remote setting |
| CHSH value | Four correlations from one source law and declared setting schedule | Context-fitted tables, product-screened local answers, or a subquantum deficit |
| No signaling | Remote-setting invariance after marginalizing the joint assembly table | Operational remote-setting dependence in either local frequency |
| Ordering independence | One observer-export table for $A\prec_tB$ and $B\prec_tA$ preferred-time sectors | Undeclared preferred-order leakage |
| Tsirelson ceiling | The same generated tables satisfy $|S|\le2\sqrt2$ | A superquantum table, which is a failure rather than extra success |
| Photon-polarization curve | Photon assembly, transverse analyzer, material return map, and doubled-angle response | Reusing the spin kernel or inserting Malus' law as a detector rule |

Plainly: each quantum prediction becomes a concrete question about source
assemblies, detector assemblies, basin measures, and exported records. This
is the level at which the empirical predictions constrain
...
chosen to reproduce the singlet table, and completed local records may not
product-screen.

Plainly: the architecture of the calculation exists. The missing evidence is
a source-generated pair record and detector-generated basin measure from
accepted assembly histories.

**Assumptions and proof burden:** the source preparation, local detector state
spaces, record windows, and exported trial rule must be declared; their
measures must be derived rather than fitted; and any successful
nonfactorizable table must identify the exact Bell assumption changed.

**Concrete promotion target:** after an assembly calculation exists, its
result may support the Bell bridge, entanglement bridge, measurement ontology,
and Bell-family validation chapter. The target map alone warrants no corpus
promotion.

**Next artifact:** produce one `spin_singlet_assembly_export` record family
from accepted source histories and accepted Stern--Gerlach detector kernels,
then compute the full angular curve, marginals, CHSH value, product-screening
residual, measurement-independence residual, and ordering residual from those
same records. This executes Quantum Closure QC-005 and QC-006; it does not
create a new gate.
