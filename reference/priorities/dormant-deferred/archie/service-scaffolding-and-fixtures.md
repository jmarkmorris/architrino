...
| MCP tool-contract checker | [validate-mcp-tool-contracts.mjs](../../../../scripts/archie-service/validate-mcp-tool-contracts.mjs) | Checks eight complete request/response pairs, nine cases with a Not advanced disposition, and first-page cursor continuation across `search`, `read`, `topics`, `neighbors`, and `walk` without starting a server. |
...
| Local fixture MCP smoke | [check-fixture-mcp-server.mjs](../../../../scripts/archie-service/check-fixture-mcp-server.mjs) | Spawns the launcher and checks initialization, all five tool calls, structured/text parity, missing-source behavior, unknown-tool rejection, ping, and no-write/no-network boundaries. |
...
| `scripts/archie-service/validate-mcp-tool-contracts.mjs --check` | Present. Recomputes eight bounded tool responses, nine cases with a Not advanced disposition, and cursor continuation for all five tools without starting a server. |
...
| `scripts/archie-service/check-fixture-mcp-server.mjs --check` | Present. Spawns the local stdio adapter and checks initialization, static tool discovery, all five calls, missing-source and unknown-tool errors, ping, snapshot immutability, and static no-network/no-write boundaries. |
...
| `tests/archie-service/fixtures/mcp/` | Present. Eight complete bounded request/response pairs, nine not advanced request/snapshot mutations, one subprocess stdio transcript covering initialization, discovery, all five calls, errors, and ping, and a 24-case remote deployment negative suite. |
