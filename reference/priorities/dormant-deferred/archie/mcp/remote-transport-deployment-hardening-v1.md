...
This contract defines the boundary a future remote Archie-service MCP adapter must satisfy before staging or authorized remote use. MCP is a software-client protocol boundary, not a user-facing application, website route, or static browser surface. The separate [Loopback Streamable HTTP Adapter](loopback-streamable-http-adapter.md) implements the local test boundary; this contract still does not select a host, create remote credentials, change a persistent client connection, or deploy anything.

The plain-language rule is: the local retrieval engine may be wrapped in a remote transport only when the wrapper preserves the same read-only snapshot boundary and independently passes its network, authorization, capability, load, health, logging, and rollback gates.
...
| [Contract validator](../../../../../src/archie-service/mcp/remote-deployment-contract-v1.mjs) | Applies cross-field service-classification, transport, security, limit, snapshot, rollback, and capability invariants. |
...
| [Focused tests](../../../../../tests/archie-service-mcp-remote-deployment-contract.test.js) | Check fixture-only state, service classification, all negative cases, forbidden exposure surfaces, and candidate snapshot identity enforcement. |
...
POST /mcp
    |
    v
existing five-tool adapter core
    |
    v
...
The current adapter makes no model calls and has no provider payload, but keeping those fields forbidden prevents a broader Archie service log schema from silently expanding the MCP boundary later.
...
The request path performs no repository scan, filesystem write, model call, local-tool call, credential access, live repository-state access, or external action. Remote publication additionally requires a snapshot tied to accepted `main`. The current full-corpus artifact identifies a local source state, so `candidateIsAcceptedMain` remains false and blocks readiness.
...
It exposes no resources or prompts and permits no model call, repository write, external action, durable user state, local-tool access, live repository-state access, or credential access. It is not eligible for the static-site build or the user-facing application inventory. A host adapter may reject, meter, authenticate, and route these calls; it may not enlarge the tool semantics or source authority defined by the existing snapshot and tool contracts.

## Verification Fixtures Required for Advancement

The negative suite covers 24 unsafe mutations:

- public-deployment authority, user-facing application or static-browser classification, or premature remote readiness;
...
- optional rollback;
- a write tool, model call, local-tool access, repository-state access, or credential access;
