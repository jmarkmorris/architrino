# Archie Service MCP Adapter Architecture

## Status

- Kind: `priority-service-protocol-adapter-architecture`
- Claim level: `priority-only`
- Status: `starting-design`
- Parent tracker: [Archie Service MCP Adapter](priorities.md)
...
Expose the Architrino knowledge system as a structured, queryable source for authorized compatible software clients through an Archie-service protocol adapter.

The repository remains the single source of truth. The public website, source-index snapshots, and MCP adapter all derive from or identify one accepted repository `main` commit. The human interface remains the GitHub Pages site. MCP is not an application or website surface; any remote endpoint is a separately deployed, authenticated, capability-limited Archie-service boundary that consumes the same accepted source state.
...
The website and MCP adapter are separate delivery surfaces. The adapter must not be included in the user-facing application inventory or emitted as a static browser route. Neither surface becomes a second editing authority.
...
| Local MCP adapter | Shared request core plus fixture and full-corpus launchers under `scripts/archie-service/` | Runnable public-corpus-scope stdio adapters over one startup-loaded immutable snapshot; full-corpus SDK and Codex conformance pass locally. The scope label is a source-visibility class, not public network authority. |
| Local versioned index output | `content/generated/source-index/local-full-corpus-snapshot.v1.json` | Archie-service-owned local immutable snapshot; remote artifact storage and accepted-`main` publication remain open. |

The MCP boundary is a protocol adapter over the shared source index. It does not fork source classes, visibility rules, source authority, System Card routing, or deployment boundaries already owned by the Archie service packets. It exposes no local tools, live repository state, credentials, provider calls, prompts, resources, or durable user state.
...
The four views should be published as one source-index snapshot with:
