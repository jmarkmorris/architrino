...
MCP-001 named-client conformance and MCP-002 bounded higher-order graph traversal are complete. The local stdio and loopback HTTP surfaces expose deterministic `walk` over declared edges with explicit depth, node, page, cycle, provenance, visibility, and cursor rules. MCP-003 remains deferred while the parent Archie service is parked. Remote readiness separately remains false because TLS ingress, a trusted proxy, exact deployment origins, an authorization server, an accepted-`main` snapshot, and a real prior rollback snapshot do not exist yet.
