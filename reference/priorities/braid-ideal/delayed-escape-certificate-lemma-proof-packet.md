...
Step labels: (C), (WP), (S), (D$_{\mathrm{op}}$), (P), (M), (W), and (H) are hypothesis rows, each checkable or monitorable on one recorded state-plus-history row; Lemmas 1-4, Lemma A, the Theorem, and the Corollary are derivations under those hypotheses; the anti-damping remark consumes a sampled diagnostic (the planar tangential screen) and derives only its sign routing; the interval-certification section states a certification target with its burdens, not a certified result; the persistence routes in the remaining obligations are labeled speculation.
...
**Anti-damping remark.** The known tangential anti-damping pump ($\Phi_{\mathrm{tan}}\approx2.9\beta$ on the planar screen; third-chart consilience in the work log) is transverse: it feeds $\|\mathbf v_\perp\|^2$, whose sign is favorable for escape in Lemma 1. Its threat is exclusively to hypothesis (S) — it drives speeds toward $c_f$ — which is a monitored failure event, not a return channel. This incorporates the anti-damping driving term into the certificate with the correct sign discipline: it can end certification, it cannot produce a return turn while certification is in force.
...
This packet is the home for upgrading the planar tangential anti-damping screen from a sampled diagnostic to a certified inequality. The current input is the first-hunt result of the [Axis-Neutral Rotating-Wave Spectrum Packet](axis-neutral-rotating-wave-spectrum-packet.md): tangential residual strictly positive at every sample over $\beta\in[0.02,0.985]$ with growth $\Phi_{\mathrm{tan}}\approx2.9\beta$, evaluated by [axis-neutral-rotating-wave-residual-scan.mjs](../../../scripts/braid-ideal/axis-neutral-rotating-wave-residual-scan.mjs) on the exact planar rotating channel with the declared kernel row ($c_f=1$, $\kappa=1$, zero softening, receiver-normal over floored source-normal branch weight, partner-wake only).

Target statement (certification target, not a certified result): certified constants $0<c_1\le c_2$ and a declared interval $[\beta_{\mathrm{lo}},\beta_{\mathrm{hi}}]\subset(0,1)$ with

$$
c_1\,\beta\;\le\;\Phi_{\mathrm{tan}}(\beta)\;\le\;c_2\,\beta
\qquad\text{for all }\beta\in[\beta_{\mathrm{lo}},\beta_{\mathrm{hi}}].
$$

What an interval certificate needs, with burdens labeled:

1. **Certified constant-lag enclosures (derivation burden).** Per directed pair, an enclosure $[\tau^-,\tau^+]$ for the constant lag, witnessed by an outward-rounded sign change of the root residual $F_{ij}$ at the endpoints plus a positive lower bound on $|F_{ij}'|$ over the enclosure. The transversality floor already exists: on the sub-field-speed channel the root residual derivative is bounded away from zero by the same $c_f(1-\beta)$ mechanism as (WP)/A3.
2. **Certified root-count completeness (derivation, discharged in this packet).** By Lemma 5 and Theorem M, on the sub-field-speed channel each directed pair has exactly one transversal causal root with residual-derivative floor $(1-\beta)c_f$, so the root-topology class is pinned at $m=1$ and no unenclosed root can contribute to the sum.
3. **Outward-rounded residual evaluation (computation burden).** $\Phi_{\mathrm{tan}}$ is a finite sum of kernel rows evaluated at the enclosed lags; interval evaluation needs derivative or monotonicity bounds for each row in $(\beta,\tau)$ over each box, with directed rounding throughout.
4. **Finite subdivision with endpoint discipline (computation burden).** A finite subdivision of $[\beta_{\mathrm{lo}},\beta_{\mathrm{hi}}]$ whose per-box lower bounds are positive. Near $\beta=0$ the screen vanishes linearly, so the certified object must be the ratio bound $\Phi_{\mathrm{tan}}(\beta)/\beta\ge c_1>0$, not bare positivity.
5. **Same-record discipline (hypothesis).** Every constant comes from the declared kernel row; no cross-chart imports (in particular no frozen-octahedral ledger values), matching non-claim 4 above.

Consumption inside this certificate. A certified band upgrades the anti-damping remark's driving term from sampled to certified, and the routing is the dominate-or-exploit split with both branches already sign-correct. Nothing in the radial envelope $-K_{\mathrm{sgn}}/R^2$ needs to dominate $\Phi_{\mathrm{tan}}$: the pump is transverse and orthogonal to the radial projection in Lemma 1, which is the exploit branch, and it is exact. The domination question moves entirely into hypothesis (S): under the transfer hypothesis that the screen residual tracks the tangential wake force on near-channel released rows (hypothesis — the screen is evaluated on the rigid ansatz, released rows deviate from it), a certified $c_1>0$ converts (S)-persistence into a quantitative clock, driving $\beta$ toward $1$ at a certified minimum rate unless an internal or environmental absorber (breathing exchange, hinge clicks, Noether sea response; see `internal_tangent_authority_derivation`) removes the pumped tangential action. Whether a certified $c_1>0$ actually forbids (S)-persistence for the isolated rotating channel — forcing $t_{\mathrm{viol}}<\infty$ without an absorber — is labeled speculation pending the persistence obligation below.

No new validator, schema, or gate is required: the certificate is a finite directed-rounding computation over the existing residual functions of the rotating-wave scan, and its output is a constant pair $(c_1,c_2)$ with its $\beta$-interval, recorded in this packet and consumed by the anti-damping remark and the persistence obligation.
...
4. Executing the interval certification of the planar tangential screen per the requirements stated above; the framework and burdens are in place, the directed-rounding computation is not.
...
- Priority status: conditional theorem proved at priority level with hypotheses checkable on sweep output rows; all eleven current witness rows fail-closed (no certified window); the signed envelope halves the margin requirement; interval certification and hypothesis persistence remain open targets.
- Next proofing consumers: the hypothesis-persistence lemma, the interval-certification computation, and `sh0sea_dipole_wake_sum`, which should compare its computed sea response against this packet's margin inequality through the owner checker.
