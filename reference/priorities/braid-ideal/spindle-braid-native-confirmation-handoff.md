...
## Candidate Row 3: Responsive-Sea-Dressed Release of Support-Candidate v1 (executed and REJECTED 2026-07-09)

Row status: `executed-candidate-rejected-2026-07-09` — run on the native machinery at both declared bookings and REJECTED by the operator in the run thread, first blocker `sea_anti_confines_at_exact_causal_delays`; run record [spec Section 44](fold-crossing-chart-spec.md#44-candidate-row-3-native-run-certificate-rejected--exact-causal-delays-flip-the-dressed-budget-the-sea-anti-confines-at-the-named-spacing-the-confining-band-moves-inward-2026-07-09) (owner script `--responsive-sea` mode, tests 11 passing). All five named release diagnostics were delivered: dispersal clock unmoved (0.855/0.8525 vs 0.83/0.80); the sea deepens the inner brake (τ_I −0.243 dressed vs −0.218 bare) instead of lifting it; the metabolism ledger runs backward (net sea torque −0.082 vs +0.11 estimated); the radial ledger opens rather than closes (sea supply −13.7%/+7.2%/−38.0% vs +6.6% estimated); the sea order parameter holds ≈0.32 with no secular growth. Root cause isolated: exact per-pair causal delays flip the dressed budget — the estimate's confinement lived in the uniform 2R/c loop-delay idealization (anti-confining phase slip, the packet's named failure mode, measured at the seed record); the exact-delay confining band sits at R≈3.25–3.5 (forward inner torque up to 52% of need), not at the named 4.25. The gate stays fail-closed. The row below is retained as the executed scope contract.

Status when tabled: TABLED (operator decision, 2026-07-09, braid-ideal evaluator thread). Same-thread acceptance rule applies in the run thread. Motivated by the Row 2 verdict (inner tangential brake -0.22 sets the dispersal clock; bare-void candidates cannot pass the release gate on the tangential clock) and the dressed-budget estimates (spec sections, cite by title: "The Dressed Budget..." and "The Escapement-Return Study...").

Braid seed: support-candidate v1 exactly as Row 2 (parameters in the Row 2 block above).
...
Decisive diagnostics (report regardless of outcome):
1. Dispersal clock vs Rows 1 (0.83) and 2 (0.80): the dressed tangential budget covers ~20% at estimate grade; the release measures whether self-consistency (near-field, domain response, dressed transfer reduction, phase-pinned escapement return) finds the rest. Any clock movement is signal; clock unmoved = the tangential ledger still rules and the estimate-grade sources are insufficient.
2. Inner tangential row along the release: does tau_I lift from -0.22 toward 0?
3. Net sea torque on the braid and per-layer sea rows (the metabolism ledger, entry 32): sign and magnitude vs the estimate (+0.11 net, forward-inner, zero-middle).
...
Row status: `executed-candidate-rejected-2026-07-09` — run on the native machinery at both declared bookings and REJECTED by the operator in the run thread, first blocker `radial_surplus_lag_collapse_middle_starvation`; run record [spec Section 45](fold-crossing-chart-spec.md#45-candidate-row-4-native-run-certificate-rejected--the-sea-breaks-the-inner-brake-invariant-but-the-relaxation-lag-collapses-the-radial-surplus-the-blocker-moves-from-tangential-to-radial-allocation-2026-07-09) (owner script `--rsea-spacing` tabled selection, tests 12 passing). All five diagnostics delivered: the dispersal clock MOVED for the first time (earlier, 0.725/0.7725 vs 0.83/0.80/0.855); τ_I lifted to the expected −0.10 and HELD (the inner-brake invariant is breakable by environment — first native demonstration; sea feed steady +0.117); net sea torque POSITIVE as predicted (+0.0285 seed, growing); but the tabled +10% radial surplus is a fast-limit quantity — the declared γ=2ω relaxation collapses the radial channel at this spacing (native mean +1.2%; M starves to 0.783 dressed while O drowns at 1.166) and the M-negative allocation set the arc's earliest clock. The failure mode moved from tangential to radial-allocation. Gate stays fail-closed; the named-constant re-declaration question stays open. The row below is retained as the executed scope contract.

Status when tabled: TABLED (operator decision, 2026-07-09, braid-ideal evaluator thread; middle-path ruling — the named sea-spacing constant is NOT re-declared by this tabling; the re-declaration decision is explicitly deferred pending this row's verdict). Same-thread acceptance rule applies in the run thread.

Identical to Candidate Row 3 in braid seed (support-candidate v1), machinery, regulators, booking, and acceptance discipline, with exactly ONE change and one validation obligation:

- Sea shell spacing: a = 3.40 (the exact-delay supply=deficit region; deliberate mild surplus side of the fixed point — under-supply is the known death mode, and the escapement regulates inward surplus). FCC first coordination shell, 12 sites; p0 = 2.20; gamma = 2*omega_braid; orientation-only dynamics.
- Validation obligation (Row 3 lesson, binding): the environment build and ALL seed expectations below are exact-per-pair-delay quantities; no uniform loop-delay shortcut anywhere in the build or the cross-checks.

Seed expectations (evaluator, exact delays, Nt=16, 6-shell scaled x2 for FCC-12 — the Row 3 cross-check confirmed true-geometry doubling at this scaling):
- mean radial sea margin ~ +10% of centripetal need (mild surplus vs the 6% deficit); per-layer rows land unevenly (O-heavy, M slightly negative) — report raw rows, allocation is free;
- inner tangential sea torque ~ +0.12 (~55% of the 0.218 need), middle ~ small;
- braid seed rows identical to Row 2/3 (closure 0.3241; support 0.96/0.88/0.98 bare).

Decisive diagnostics (report regardless of outcome; deltas vs Rows 1/2/3):
1. Dispersal clock vs 0.83 / 0.80 / 0.855: ANY movement is signal; this is the first row with a computed reason for it to move.
2. Inner tangential row tau_I along the release: bare -0.218, Row 3 dressed DEEPENED it to -0.243; expectation here is a lift toward ~ -0.10.
3. Metabolism ledger: net sea torque expected POSITIVE this time (Row 3 measured -0.082 at the wrong phase); per-layer sea rows vs the expectations above.
4. Support rows along the release (radial expected in surplus; watch for inward drift regulated by the middle escapement — the first native test of surplus-side behavior).
...
## Candidate Row 7: Bare Release of Self-Equilibrated Candidate V5 (tabled 2026-07-09; executed and REJECTED 2026-07-10)

Row status: `executed-candidate-rejected-2026-07-10` — run on the native machinery and REJECTED by the operator in the run thread, first blocker `rail_pump_unabsorbed_coherent_expansion`; run record [spec Section 60](fold-crossing-chart-spec.md#60-candidate-row-7-native-run-certificate-rejected--the-radial-and-tilt-gates-hold-natively-at-the-self-equilibrated-seed-but-the-un-absorbed-rail-pump-drives-a-new-failure-mode-coherent-expansion-2026-07-10) (owner script `--row=7` tabled selection, tests 23 passing). Both binding obligations met: the full Sections 57-59 gate re-derived in-build and PASSED whole (radial residual 4e-6, basin, tau ~0, tilt global null + restoring quotient, all witnessed), and the frozen-kappa discipline yielded the gauge-invariant release coupling kappa_eq = kappa_fit/lambda = 1/R_M(eq) = 0.28623 (R_M(eq) = 3.4937 kappa*eps^2/c_f^2 from both the certificate gauge and the in-build fit). All five decisive diagnostics delivered: the radial ledger closed natively at the seed for the first time (support 1.0000/0.9996/1.0000 by self-adjustment, bare) and every gate row held at release — but the +0.227 rail pump went un-absorbed (zero rail re-crossings at the canonical booking), beta_M rode the rail upward to 1.167, and the size pin inverted into a NEW failure mode: coherent expansion (tube loss 0.113 rot, dt-witnessed; coherent to halt at 1.94 rotations, the arc's longest — no cap fall-in, no ejection, no scatter). Size mode: one restoring dip to R_M = 0.9855 before rail detachment (the size pin is conditional on rail residence). Escapement regime map regulator-complete: pointwise never engages; chart at rho_c=0.01 over-brakes destructively; chart at rho_c=0.05 engages (91 clicks, 12 crossings, hunts below rail) but over-absorbs — rail-holding needs the marginal brake=pump stratum, the open d0 magnitude. Axis dynamics: flutter is REAL (nutation 7.8 deg by t=1.0, preceding shape loss — the quasi-static tilt gate does not govern the gyroscopic-circulatory native dynamics). Twin rows: no returning kick at any size (separations linear in kick, riding the pump drift). The gate stays fail-closed; items 21/23 stay blocked. The row below is retained as the executed scope contract.
...
## Candidate Row 8: Marginal-Stratum Release of V5 — the d0-From-Survival Row (tabled 2026-07-10; executed and REJECTED 2026-07-10)

Row status: `executed-candidate-rejected-2026-07-10` — run on the native machinery and REJECTED by the operator in the run thread, first blocker `no_clean_marginal_stratum_in_declared_span`; run record [spec Section 62](fold-crossing-chart-spec.md#62-candidate-row-8-native-run-certificate-rejected-at-the-pre-release-stratum-map--no-clean-marginal-brake-equals-pump-stratum-exists-at-the-chart-booking-the-absorbed-fraction-jumps-from-zero-to-over-absorption-and-is-nowhere-regulator-converged-no-release-occurred-2026-07-10) (owner script `--row=8 --stratum-map`, tests 26 passing). **NO release occurred**: the binding pre-release stratum map fired its scoped-negative branch — the full Sections 57-59 gate re-derived in-build and PASSED whole (kappa_eq = 0.28623 reproduced; in-build pump 0.22736 cross-checks the 0.2274 constant), but EVERY cell in the declared span [0.005, 0.08] over-absorbs (fractions 2.0-24, no crossing of 1), the regulator-honesty witnesses fail almost everywhere (dt-halving moves the absorbed fraction 22-114% vs the declared <=10%; substep spread 50-77% below rho_c ~ 0.013), and the labeled out-of-span probe at the extrapolated crossing rho_c ~ 0.112 still books 2.84 (receding crossing). The d0 statement booked per the contract: no clean marginal brake=pump stratum exists at this booking — the candidate marginal regime sits at rho_c >~ 0.11 R_M, ~5x the Section 12 pin-stability marginal stratum 0.022, and the absorbed fraction is nowhere dt-converged at the declared regulators. Durable positives: the two-sided escapement hunt EXISTS natively and tightens monotonically with rho_c (0.93-1.07 at 0.08-0.112; dt/2 probe 0.94-1.04 with 66% in-band residence, tightest in the arc); the clicker is never starved (164-216 clicks/rotation vs the kinematic requirement ~18 — per-click impulse calibration, not click count, is the binding constraint); the dt/2 rows uniformly move TOWARD the marginal regime (the named follow-up: a dt-converged impulse-resolved click booking). The gate stays fail-closed. The row below is retained as the executed scope contract.

Status when tabled: TABLED (operator decision (a), 2026-07-10, braid-ideal evaluator thread). Same-thread acceptance rule applies in the run thread. Motivated by the Row 7 verdict (spec Section 60 by title): the radial program is COMPLETE (first native bare closure at the self-equilibrated seed) and the sole blocker is the un-absorbed +0.2274 rail pump; the escapement regime map brackets survival at the MARGINAL brake-equals-pump stratum between the destructive push at rho_c=0.01 and the over-absorbing cell at rho_c=0.05. This row makes that bracket an instrument: SURVIVAL NAMES d0.

THE CLAIM CONTRACT (declared up front):
- Primary claim: RAIL RESIDENCE by marginal absorption — a cell where the escapement holds beta_M on the rail with absorbed pump fraction ~= 1 converts the coincidence stratum from a declared regulator into a DERIVED persistence requirement (report the implied d0 against the declared d0 = R_MCB and against 1/kappa_eq = R_M(eq)).
- Full prize: shape survival (dispersal clock beaten) at the marginal cell.
- Partial positive (still bookable): rail held but flutter kills — the d0 row STANDS (pump absorbed natively) and the tilt/gyroscopic program opens as the next named killer. Flutter and expansion have separable signatures (nutation angle vs radius runaway); report both clocks independently.

Braid seed: identical to Row 7 (SELF_EQUILIBRATED_V5, re-derived in-build equilibrium, frozen gauge-invariant kappa_eq = 1/R_M(eq) = 0.28623, full Sections 57-59 gate re-check). No environment.

Binding pre-release obligation — the stratum map: on short holds/releases at the V5 seed, sweep the chart-booking coincidence stratum rho_c over a declared grid spanning [0.005, 0.08] (log-spaced, >= 6 cells, plus the canonical soft booking as the zero-absorption reference); book per cell the absorbed pump fraction per rotation (click-ledger tangential sum / 0.2274 per rotation), rail residence statistics, and the two regulator-honesty witnesses that Row 7 flagged: substep witness spread (the 7.3% coincidence-push artifact at 0.01) and dt-halving on the absorbed fraction. The MARGINAL CELL rho_c* is where absorbed fraction crosses 1 with witnesses clean; if the crossing is regulator-fragile (witness spread swamps the crossing), book the scoped negative honestly — that too is a d0 statement (no clean marginal stratum exists at this booking).

Release at rho_c*: full Row 7 protocol and diagnostics, plus:
1. Dispersal clock AND flutter clock reported as separable verdict channels (radius/tube vs nutation angle; Row 7 references: tube 0.113 rot, nutation 7.8 deg at t=1.0).
2. Rail residence: crossings, hunting band, beta_M excursions (Row 7 rho_c=0.05 reference: hunting 0.78-0.99 = over-absorption; the marginal cell should hunt tight around 1.0).
3. Size mode: R_M about its equilibrium WITH the rail held — the size pin's first native test in its valid regime.
4. Twin rows at several kicks (the basin prediction gets its fair test only with the pump absorbed).
5. h_act ledger statistics at the marginal cell (clicks per rotation vs the kinematic bound; the discreteness-as-confinement hypothesis's quantitative row).

Failure modes to report honestly: regulator-fragility of the marginal cell (the claim dies with dirty witnesses — say so); flutter preceding rail loss (partial positive per the contract); middle radius creep under sustained clicking (the rho_c=0.05 contraction mode at reduced strength); delay-memory modes outside the quasi-static gate.
