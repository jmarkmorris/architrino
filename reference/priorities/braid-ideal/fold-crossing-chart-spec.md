...
**Result 1 — the apoapsis seed IMPROVES native unified closure.** Seed-record residual at frozen $\kappa^\*$: $0.4721$ (circular) $\to0.4531$ ($e_M=0.05$) $\to0.4464$ ($e_M=0.1$) — the $e_M=0.05$ value reproducing the Section 23 descent-refined score ($0.4531$) from an orthogonal direction. The Section 17 rejection of the $m{=}2$-at-constant-rate mode does not cover this family: the a.m.-matched apoapsis start is a speed-modulated (Kepler-flavored) variant measured on the native record, and it is a genuine closure-improvement direction — a new hunt axis for the next candidate row.
