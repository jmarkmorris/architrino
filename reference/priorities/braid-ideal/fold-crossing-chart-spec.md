...
Instrument: `scripts/braid-ideal/responsive-sea-margin-feasibility-estimate.mjs` (+ 3 tests). The Section 37 question at linear-response estimate grade: polarizable point-dipoles on a shell at $R_{\mathrm{sea}}$, responding to the braid's retarded field and acting back with retarded dipole fields — the loop carries the causal double delay $\sim2R_{\mathrm{sea}}/c_f$.
