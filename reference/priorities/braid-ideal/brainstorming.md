...
1. **Charge quantum as a symmetry orbit (promoted, hypothesis).** [architrino.md](../../../content/markdown/aaa/foundations/architrino.md) names $|e|=6\epsilon$ an input parameter with the protected-site target $|G_{\mathrm{ax}}\text{-orbit}|=6$. The Thomson-octahedral accessory dressing supplies a candidate: pure inversion is a symmetry of a same-polarity dressing (unlike the core's $\iota$), and $\langle\varrho,-\mathbb I\rangle$ has order six, acting on the six octahedral accessory sites as a single free orbit. Charge arrives in $6\epsilon$ units because dressing comes in whole orbits of the braid's rotoinversion symmetry. Promoted into the accessory-dressing subsection of nested-shell-braid.md; owning program remains quantum-number mapping.
