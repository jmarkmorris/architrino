...
### Phases 0–2 — Completed (2026-07-08, branch `codex/calcite`)
...
- **Phase 2 — Lane hygiene (verification).** Done. Verified file-role separation: every lane already carries a `work-log.md`, and all work-logs are disciplined dated-status logs, so the work-log → brainstorming migration had no targets. The one live sub-action — trimming explanation-first material out of the large `priorities.md` files — is folded into Phase 3 below (its destination is the unified `brainstorming.md`), excluding the OP-2-deferred `braid-geometry-export-bridge`. See the dated Phase 2 note in `work-log.md`.

### Phase 3 — Brainstorm merge + priorities trim (OP-3)
Inputs: every lane's `brainstorming.md`; plus explanation-first material currently embedded in the bloated `priorities.md` files (architecture notes, equation explanations, comparison matrices, draft prose — as opposed to the concise ranked queue), notably `braid-angular-momentum-spin` (124 KB) and `braid-mass-response-map` (81 KB). Action: physically merge all brainstorm-grade content into one `braid-ideal/brainstorming.md`. Give the merged file a dated table-of-contents header and group entries under clear headings; where a lane of origin matters, keep a short inline `(from: <lane>)` reference on the entry rather than a separate index. De-duplicate near-identical ideas, keeping the strongest statement. Mark any idea that has matured into an executable object for promotion to a packet or the queue. Reduce each emptied source `brainstorming.md` to a one-line stub — `Merged into ../braid-ideal/brainstorming.md (2026-07-08).` — so every lane still carries its three role files. As part of the same pass, de-bloat each large `priorities.md` by **routing each block to its correct home by content type** (operator decision 2026-07-08), not everything to brainstorming: doctrine / conceptual framing → the merged `brainstorming.md`; per-item dated status and blocker narrative embedded in queue items → that lane's `work-log.md`; reference surfaces and detailed proof-program specs (e.g. PDG benchmark tables, the `$A_0$` deliverable spec, CERN reconstruction-signal tables) → focused sibling files. Leave each `priorities.md` a concise ranked queue that references those homes — **except** `braid-geometry-export-bridge/priorities.md`, which is OP-2-deferred (re-index only, no extraction). Guardrail: do not delete ideas in the merge (only de-duplicate); do not weaken any claim level in a move (text travels with its labels); downgrade exuberant wording only when promoting, not in place; because `device_bash` cannot `rm`, the stub-overwrite approach keeps the source files present-and-pointing; leave certificate/packet files untouched. Done when `braid-ideal/brainstorming.md` holds all provisional braid insight, every source brainstorm is a resolving stub, and each trimmed `priorities.md` (except the OP-2-deferred lane) is a concise ranked queue.
