...
- **Promotion target:** remain in `reference/priorities/field-speed-ceiling`; no Master Equation or reader-facing promotion without a separate operator decision.
- **Current disposition and follow-up:** the proposed independent review is now incorporated through the [complete-lobe and returning-event review](analysis/independent-complete-lobe-returning-event-review-2026-09-02.md) and the [operator-selected multivalued exact-mirror continuation decision](decisions/continuation-selection-operator-decision-2026-09-02.md). At current FSC authority, continuation remains a multivalued relation containing the straight trace and prescribed positive-onset braking branches. The later decision supplies a law-relative future-equivalence quotient and a quotient-state two-cycle for equal prescribed onsets; it supplies neither an autonomous onset nor periodicity of the literal all-past record. The earlier construction and failed cap-duration trigger above retain their conditional scope.

The remaining recorded follow-up is the queue's [Routed Reproducibility Gap](work-queue.md#routed-reproducibility-gap): recover independently runnable instruments for the review's numerical calibration, validate them on known cases before reproduction, and record the evidence and the FSC-014 ranking decision. That queue item reports the missing instruments; this summary does not claim a fresh reproduction or reopen the completed selection decision. No existing FSC wake or action account defines $\Phi$; a future selector remains a separate proposal requiring an independent derivation and operator authorization. The scope summary would need revision if the owning queue closes that reproducibility gap or a later accepted decision supplies a selector.
...
## 2026-09-15 — Why the speed ceiling does not settle mirror passage

**Theory level:** conditional derivation within the proposed FSC response. **Evidence grade:** derived consequences of the declared projection, with the continuation diagnosis taken from the [selector analysis](analysis/exact-mirror-continuation-selection-analysis.md). No new law or selector is adopted.

The operator's forward-acceleration intuition agrees with the proposed regular response: at field speed, remove the positive component of the complete finite ordinary acceleration ledger along the velocity. Preserve backward and transverse components. A speed ceiling therefore prevents further speed increase but does not require zero acceleration in every direction or forbid departure below field speed. It is not equivalent to capping a scalar potential; the latter would require a separately defined potential and a derivation of its relationship to this velocity-dependent delayed response.

In the isolated incoming mirror cap segment, the ordinary net contribution is forward, so the projected acceleration is zero and the pair continues toward coincidence at constant speed. At coincidence the ordinary expression is outside its domain; a rule acting on a finite ordinary ledger does not itself define a nonordinary event. The proposed zero-impulse event completion admits straight outgoing motion. Thus the ceiling does support a consistent straight passage after that separate event convention is supplied.

The remaining failure is uniqueness in the declared absolutely continuous, almost-everywhere solution class. The delayed-braking constructions have a backward contribution once their partner channel activates, which the ceiling permits. They share the straight preceding history yet depart at a freely supplied positive time. Their existence does not supply a cause of first activation. The speed restriction alone cannot discriminate them because both straight and braking traces obey it. This is a limitation of the present extension, not an exclusion theorem against every possible capped model.

**Falsifier and next artifact:** inspect the first-admission boundary in the existing delayed-ignition proof for an already admitted causal-root or solution requirement that excludes its onset construction. Such a requirement would overturn the current nonuniqueness diagnosis without adding a selector. If none exists, record precisely the additional admission or solution condition needed; do not infer a capped potential or adopt a preferred trace. The existing decision and theorem owners remain the promotion targets for any substantiated correction.
...
## 2026-09-15 — First partner acceleration at crossover and accumulated cap fronts

**Question:** how large is the first braking contribution when the partner crossover emission and the family emitted since first reaching field speed are both retained? **Authority:** only the speed cap may modify the Master Equation. **Grade:** derived incidence and direction calculations on the prescribed mirror geometry; the finite event response remains unresolved.

Put crossover at $t=0$, use $c_f=1$, and write the incoming capped paths as $X_1(s)=s$, $X_2(s)=-s$ for $-L\le s\le0$. A partner emission at $s=-\tau$ has its center at $+\tau$ and sphere radius $\tau$ when receiver 1 reaches the origin. Its local surface density per emission-time interval has the canonical inverse-square factor $1/\tau^2$. Every such front meets the origin at the same reception time, giving $D_t=0$ throughout the cap interval. These are a nonisolated causal family, so assigning each an ordinary acceleration row and summing is not a valid use of the simple-root formula.

Their direction also matters: for receiver 1 at crossover, the old partner emission centers are ahead at $+\tau$. Opposite-polarity attraction points toward them, in the incoming forward direction, not the outgoing braking direction. This directional statement does not define the singular event or authorize applying the finite-ledger projection to a divergent family. On the prescribed straight outgoing path, the older cap fronts cease to intersect the receiver; only the partner crossover emission remains a root. Its center is now behind the receiver, and its ordinary contribution is backward with magnitude $K/(2t^2)$. Thus the old cap family and the outgoing crossover-root contribution cannot be counted as one persistent stack of backward ordinary rows.

The emitted signed weight $q\,ds$ is source measure, not a separate acceleration term to add to the ordinary kernel. The local regular row already includes source and receiver polarity magnitudes through $K$, the inverse-square range, and the transmitter Jacobian. The existing packet's cutoff expression $K\int_\varepsilon^L d\tau/(2\tau^2)$ is a divergent diagnostic of an attempted source-swept extension, explicitly not an authorized finite impulse at the degenerate event. It supplies no finite first velocity change. Likewise, integrating the straight outgoing partner expression gives $K(1/\varepsilon-1/t)/2$, which diverges as $\varepsilon\downarrow0$; it is a trial-path obstruction, not the actual velocity change of an evolved solution.

**Next artifact and falsifier:** construct the complete cap-only event/near-event receiver measure, preserving directions and every source label, and prove or refute a finite continuous outgoing solution. A valid canonical event evaluation or a complete self-consistent outgoing path would resolve this open response question. The source surface integral alone does not. Supporting owners are the [inherited-family geometry](analysis/inherited-partner-characteristic-family-disposition.md) and [endpoint reanalysis](analysis/capped-collinear-endpoint-reanalysis.md); no cancellation, cutoff, frozen-front suppression, or new impulse law is adopted.
