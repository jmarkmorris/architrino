...
- Source-contract shell: [signed-frequency-transfer-theta-transfer-source-contract.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-theta-transfer-source-contract.v1.json)
- Source-contract control: [signed-frequency-transfer-source-contract-negative-control.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-source-contract-negative-control.v1.json)
- Row served: `EQ-17`
...
This packet narrows `EQ-17` to one signed source-path-receiver transfer carrier. It does not treat a redshift fit, a priority packet, an authored prose explanation, a source-contract shell, or a path-history analogy as accepted retained evidence. No score changes.
...
| Existing scripts/fixtures/packets found | [signed-frequency-transfer-ledger.mjs](../../../scripts/equation-mapping/signed-frequency-transfer-ledger.mjs), [signed-frequency-transfer-attempt.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-attempt.v1.json), [signed-frequency-transfer-priority-source-negative-control.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-priority-source-negative-control.v1.json), [signed-frequency-transfer-theta-transfer-source-contract.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-theta-transfer-source-contract.v1.json), [signed-frequency-transfer-source-contract-negative-control.v1.json](../../../scripts/equation-mapping/signed-frequency-transfer-source-contract-negative-control.v1.json), and the broad parent DGL in [EQ-07 Through EQ-10 And EQ-17 Through EQ-19 Effective Metric / Cosmology Packet](eq-07-10-17-19-effective-metric-cosmology-packet.md#direct-geometry-layer). |
...
| Negative control required for advancement | `signed_frequency_transfer_priority_source_negative_control` blocks priority prose with `coordination_source_path`; `signed_frequency_transfer_source_contract_negative_control` blocks the source-contract shell with `source_contract_path`. Both remain `scoreDecision: no_score_increase`. |
| Smaller next action | Replace the priority-source and source-contract controls with a real durable `theta_transfer` source report, then populate child rows one at a time under `--require-populated` until the checker advances beyond `missing_accepted_theta_transfer`. |
...
| --- | --- | --- | --- | --- | --- |
| $1+z_X\approx\Gamma_{N,E}\mathcal P_{E\to R}/(\Gamma_{N,R}B_XD_v)$ | One signed source-path-receiver transfer budget with endpoint cadence, source branch, launch geometry, and path-history factors kept as separate readouts. | `theta_transfer`, `endpoint_cadence_emitter`, `endpoint_cadence_receiver`, `source_branch_factor`, `launch_geometry_factor`, `path_history_propagation`. | Same `event_id`, `source_branch_id`, `receiver_branch_id`, path window, and factor values across endpoint, source, launch, and path rows. | Priority-source false positive; source-contract shell; redshift factor collapse into one fitted scalar. | Durable source-backed `theta_transfer` parent row with all factor rows accepted on the same transfer. |
...
```sh
node scripts/equation-mapping/signed-frequency-transfer-ledger.mjs \
  --input scripts/equation-mapping/signed-frequency-transfer-source-contract-negative-control.v1.json \
  --summary --pretty
```

Expected score-neutral result: `status: blocked_missing_rows`, `scoreDecision: no_score_increase`, `nextBlocker: missing_accepted_theta_transfer`, and `rows.theta_transfer.reason: source_contract_path`.

## Next Action

Create a durable source-backed `theta_transfer` source report for one clean spectral family. The report must make endpoint cadence, source-branch factor, launch geometry, path-history propagation, photon-channel record, event ledger, energy-exchange segments, path-quality constraints, and no-hidden-retune witness concrete under one transfer id. Until those rows exist as accepted retained evidence outside priority prose, source contracts, generated files, attempts, mocks, probes, toys, and negative controls, the correct disposition is no score change.
