...
- Review synthesis: [Planck Action Period, Blackbody, and Fine-Structure Coupling](../../../office-of-research/research-history/review-packets/planck-action-period-blackbody-alpha-reconciliation-2026-07-29.md)
- Row served: `EQ-12A`
...
| Row | `EQ-12A` |
| Current score | `2` |
| Carrier | Retained action-period carrier: $\gamma_0\to\mathcal U\to h_\vartheta$, refined by locked iso-frequency `S_eq` three-binary evidence. |
...
| Smallest accepted evidence object | One source-backed retained iso-frequency Noether braid branch with integer winding plateau, same-branch four-readout coincidence, and positive Floquet margin. |
...
| Iso-frequency row | $(f_1,f_2,f_3)=(f,f,f)$ and $\omega_1=\omega_2=\omega_3=\omega_f$ on the same branch. |
...
| Four readouts | $h_E=E_\gamma/\nu$, $h_\Phi=\int p\,dq$, $h_p=2\pi \lvert p\rvert/\lvert k\rvert$, and $h_J=2\pi J/n$, all equal inside the plateau. |
...
| Iso-frequency three-binary $(f_1,f_2,f_3)=(f,f,f)$ | Same-branch retained reduction and locked iso-frequency support for the action-period carrier. | `retained_orbit_reduction_row`, `constant_delay_self_hit_model_row`, `hopf_retained_orbit_birth_row` | Iso-frequency identity, retained reduction, constant-delay self-hit model, and Hopf birth row share one `S_eq` branch id. | [constant-delay-retained-orbit-retained-reduction-metadata-missing-negative-control.v1.json](../../../../scripts/equation-mapping/constant-delay-retained-orbit-retained-reduction-metadata-missing-negative-control.v1.json) and [constant-delay-retained-orbit-self-hit-model-metadata-missing-negative-control.v1.json](../../../../scripts/equation-mapping/constant-delay-retained-orbit-self-hit-model-metadata-missing-negative-control.v1.json) reject durable source paths without `EQ-12A`, row, carrier, and `S_eq` support metadata. | Accepted retained-reduction row with explicit `S_eq` iso-frequency support metadata. |
...
The probe marks only `retained_orbit_reduction_row` accepted-looking with explicit `EQ-12A`, `retained_orbit_reduction_row`, retained action-period carrier, and `S_eq` iso-frequency support metadata. It remains score-neutral and advances only to `nextBlocker: missing_accepted_constant_delay_self_hit_model_row`; the `--require-populated` form must exit nonzero.
