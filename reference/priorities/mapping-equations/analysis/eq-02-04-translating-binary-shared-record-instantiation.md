...
This is a branch-validity row rather than an extra Lorentz fit. If the moving binary loses the forward causal roots needed by its own clock, envelope, or acceleration rows, a small Lorentz-looking residual is not eligible for promotion.
...
3. one declared equal-time rule selects the transverse boundary events;
4. the center record fixes the axes;
