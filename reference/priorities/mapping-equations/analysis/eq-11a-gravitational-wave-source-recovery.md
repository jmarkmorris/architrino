...
- Claim level: observer-level gravitational-wave source benchmark, native carrier dictionary, and residual required for advancement target
...
| Deepest current probe | [eq11a-gravitational-wave-source-no-hidden-retune-witness-source-contract-attempt.v1.json](../../../../scripts/equation-mapping/eq11a-gravitational-wave-source-no-hidden-retune-witness-source-contract-attempt.v1.json) marks every required row accepted-looking with row-specific source-contract boundaries, then does not advance at `source_contract_boundary_not_retained_evidence`. |
| Selected source-window shard | [EQ-11A GW150914 Source-Field Map](eq-11a-gw150914-source-field-map.md), which maps `GW150914-v3` to the Direct Geometry Layer and controls required for advancement without accepting retained evidence. |
| Smallest accepted evidence object | A retained $\Theta_{\mathrm{GWsrc}}(W,P)$ packet whose source carrier, Noether sea/effective-metric rows, source-event ledger, detector strain record, radiated-output ledger, ringdown label, source provenance, and no-hidden-retune witness are accepted, durable-source-backed, and same-record bound. |
| Smallest next artifact | Fill the GW150914 retained-evidence intake with local event, strain, parameter-estimation, waveform, calibration, extraction, path, and hash artifacts, then bind the first accepted `gw_source_carrier` to that artifact set. |
| Controls required for advancement already exercised | Coordination-source and probe-source controls keep accepted-looking `gw_source_carrier` rows blocked when sourced only to priority/probe artifacts; the `theta_sea` controls keep authored Noether sea prose or row-mismatched durable source contracts from satisfying retained `theta_sea` evidence; the tensor, source-event-ledger, quadrupole, chirp-mass, Peters-decay, strain-flux, ringdown-label, detector-strain, source-provenance, and no-hidden-retune row-mismatch controls keep wrong-row support from satisfying `effective_metric_tensor_channel`, `source_event_ledger`, `quadrupole_source_row`, `chirp_mass_row`, `peters_decay_row`, `strain_flux_row`, `ringdown_label_row`, `detector_strain_record`, `source_provenance`, or `no_hidden_retune_witness`; the checker blocks all source-contract and retained-evidence-contract shells at `source_contract_boundary_not_retained_evidence` and blocks durable source summaries without row-specific local artifact paths and artifact-family hashes at `source_artifact_hashes_missing`. |
| Next safe implementation target | Fill the GW150914 retained-evidence intake template with local event, strain, parameter-estimation, waveform, calibration, extraction, path, and hash artifacts; only after that, replace the source-contract shell with the smallest accepted retained $\Theta_{\mathrm{GWsrc}}(W,P)$ evidence object. Leave the score at `2` until that object is durable-source-backed and the checker reports populated without source-contract-boundary or artifact-hash blockers. |
...
The next accepted-retained-evidence artifact is not another normalized fixture. It is a source-backed $\Theta_{\mathrm{GWsrc}}(W,P)$ carrier whose row bindings are accepted and whose source ledger keeps the quadrupole, chirp, orbital decay, detector strain, radiated energy/angular momentum, and ringdown labels on one record. The immediate score-neutral precursor is the GW150914 retained-evidence intake template: it must be filled with local source artifact paths and hashes before an accepted-looking retained packet is safe to test.
