...
- Owned output file: `reference/priorities/mapping-equations/analysis/eq-07-10-17-19-effective-metric-cosmology-packet.md`
...
- Smallest next artifact: one durable weak-window `theta_W` source object with lapse, drift, spatial-compliance, signal-delay, cadence, redshift, Shapiro/lensing, PPN, null/eikonal, action-to-acceleration, provenance, and no-hidden-retune rows on one carrier.
- Safe pass action: priority packet refinement only. Numeric passes remain candidate evidence until the accepted retained carrier and row-level provenance object exist.
...
| Effective FRW line element | Observer-level $a_{\mathrm{eff}}$, $H_{\mathrm{eff}}$, $\tau_c$, $k$, and distance rows from the fixed-void Noether sea record. | Accepted `theta_cos` carrier with FRW projection and fixed-void rows. | `theta_cos` must inherit from the same Noether sea and source-provenance record used by $\rho_{\mathrm{eff}}$, $P_{\mathrm{eff}}$, and $\Lambda_{\mathrm{eff}}$. | `cosmology.void_expansion_level_collapse` and hidden source-term controls in `effective-frw-handoff-residual.mjs`. | Retained effective-FRW source packet proving the scale factor and distance rows are observer projections, not Euclidean-void expansion. |
| Friedmann and continuity comparison | $R_H$, $R_\rho$, $\mathcal{S}_{\mathrm{eff}}$, $\rho_{\mathrm{eff}}$, $P_{\mathrm{eff}}$, $G_{\mathrm{eff}}$, and $\Lambda_{\mathrm{eff}}$ as effective observer variables. | Accepted `theta_cos` plus accepted source-provenance rows for assembly association, dissociation, recycling, transport, or Noether sea exchange. | The source term $\mathcal{S}_{\mathrm{eff}}$ must point back to the same absolute record $S(T)$ used for the FRW projection. | `cosmology.source_without_provenance` and pressure/$\Lambda_{\mathrm{eff}}$ source negatives. | Retained source-provenance row proving effective density and pressure terms are generated by declared carrier rows. |
| Friedmann source-provenance residual | $H_{\mathrm{eff}}$, $\rho_{\mathrm{eff}}$, $P_{\mathrm{eff}}$, $k$, and $\Lambda_{\mathrm{eff}}$ reconstructed from one source-bearing Noether sea record rather than assigned as independent fit variables. | Accepted `theta_cos` with a source ledger $S(T)$ and declared component rows for release, capture, recycling, transport, and thermalization. | The same record must generate the Friedmann comparison residual, continuity residual, redshift transfer row, and CMB thermalization row. | `cosmology.friedmann_source_split`, `cosmology.lambda_without_pressure_source`, and per-component hidden retunes. | One retained cosmology packet in which changing the source ledger changes all dependent effective variables through declared maps, not copied summaries. |
| Pressure/$\Lambda_{\mathrm{eff}}$ derivation | Noether sea pressure, tension, relaxation, and density-compression rows that feed effective pressure and dark-energy comparison variables. | Accepted `theta_sea_rho_NS` carrier feeding the pressure/$\Lambda_{\mathrm{eff}}$ checker and downstream `theta_cos`. | `theta_sea_rho_NS` must bind density compression, pressure, tension, relaxation, and cosmology handoff ids without copied numeric summaries. | `accepted_without_evidence_source`, hidden `theta_cos_id` retune, and source-without-provenance negatives. | Retained Noether sea density/pressure source packet accepted by the pressure/$\Lambda_{\mathrm{eff}}$ checker before any score review. |
...
The derivation pass should populate four rows before any score review:
...
The `EQ-18`/`EQ-19` effective-FRW handoff slice is also executable in [effective-frw-handoff-residual.mjs](../../../../scripts/equation-mapping/effective-frw-handoff-residual.mjs). Its attempt fixture [effective-frw-handoff-attempt.v1.json](../../../../scripts/equation-mapping/effective-frw-handoff-attempt.v1.json) blocks at `missing_accepted_theta_cos` while $H_{\mathrm{eff}}$, $R_H$, $R_\rho$, source-provenance, fixed-void, shared-key, and no-hidden-retune diagnostics pass on one coherent attempt carrier. The accepted input [effective-frw-handoff-theta-cos-accepted.v1.json](../../../../scripts/equation-mapping/effective-frw-handoff-theta-cos-accepted.v1.json) consumes [effective-frw-theta-cos-handoff.v1.json](../../../../scripts/equation-mapping/effective-frw-theta-cos-handoff.v1.json) and reports `status=populated` with `nextBlocker=null` for the provider-backed `EQ-20` route. Broader redshift, distance-ladder, growth, CMB, and no-retune consumers still need to consume that handoff without private records.

[EQ-18 And EQ-19 Theta-Cos Source-Field Map](eq-18-19-theta-cos-source-field-map.md) records the accepted handoff object and the remaining consumer burden. It is priority-only and leaves `EQ-18` and `EQ-19` at score `3`.
...
```sh
node scripts/equation-mapping/effective-metric-weak-field-residual.mjs --input scripts/equation-mapping/effective-metric-weak-field-attempt.v1.json --summary --pretty
...
node scripts/equation-mapping/signed-frequency-transfer-ledger.mjs --input scripts/equation-mapping/signed-frequency-transfer-attempt.v1.json --summary --pretty
node scripts/equation-mapping/signed-frequency-transfer-ledger.mjs --input scripts/equation-mapping/signed-frequency-transfer-priority-source-negative-control.v1.json --summary --pretty
...
node scripts/equation-mapping/eq20-pressure-effective-lambda-residual.mjs --input scripts/equation-mapping/eq20-pressure-effective-lambda-attempt.v1.json --summary --pretty
node scripts/equation-mapping/eq20-pressure-effective-lambda-residual.mjs --input scripts/equation-mapping/eq20-pressure-effective-lambda-priority-source-negative-control.v1.json --summary --pretty
```

These routes remain score-neutral. They must continue to report `scoreDecision=no_score_increase` until accepted source-backed `theta_W`, `theta_transfer`, `theta_cos`, or `theta_sea_rho_NS` rows exist.
