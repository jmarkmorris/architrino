...
- Review synthesis: [EQ-02 Through EQ-04A Retained Point, Coframe, No-Retune, and Koide](../../../office-of-research/research-history/review-packets/eq02-04-retained-point-coframe-no-retune-koide-reconciliation-2026-07-29.md)
...
| Field | Current packet status |
| --- | --- |
| Current score | `1` |
| Closure driver | Treat Koide as a frozen post-prediction charged-lepton mass-root residual after `EQ-04` supplies a retained mass-shell carrier and one charged-lepton generation-by-shielding mass map. |
| Exact first blocker | `missing_accepted_raw_labeled_rows_preserved_on_retained_history`, inherited from the `EQ-02` through `EQ-04` retained-domain carrier. |
...
| Safe implementation target for this packet | The carrier-shell source-contract boundary now does not advance as non-evidence for the inherited `S_eq` mass-shell carrier. The next safe target is a real source-backed charged-lepton mass-map row template that still preserves `no_score_increase` until generation rows are accepted on the same record. |
...
This table is priority-only. It maps the Koide comparison terms to the $\mathbb{A}\mathbb{A}\mathbb{A}$ geometric readouts they must consume before the residual can count as evidence. It does not make Koide an invariant, does not add a new score claim, and does not bypass the inherited `EQ-04` retained mass-shell blocker.
...
| --- | --- | --- | --- | --- | --- |
| $m_e,m_\mu,m_\tau$ or $M_{\ell,g}$ | Exposed charged-lepton mass readouts from one generation-by-shielding family, with mass as the quadratic norm of a branch/root readout. | $\Theta_{\ell}^{04A}$ consuming the inherited `S_eq` carrier, `mass_shell_row`, `rest_mass_row`, `medium_response_row`, `massMapId`, and generation rows $A_{\ell,0}$, $A_{\ell,1}$, $A_{\ell,2}$. | All three masses share `commonCarrierId`, `domainId`, `supportId`, `massMapId`, `exposureRowId`, `shieldingRowId`, `NoetherSeaResponseRowId`, and `fixedParameterHash`. | `koide.split_generation_map` if one generation uses a private exposure, shielding, or Noether sea response row; [eq04a-koide-residual-priority-source-negative-control.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-priority-source-negative-control.v1.json) if accepted-looking rows point only to priority prose. | Accepted `S_eq` retained-domain row identity plus accepted charged-lepton mass-map and generation rows using the same carrier ids. |
| $\sqrt{m_e},\sqrt{m_\mu},\sqrt{m_\tau}$ or $\mathbf R_{\ell}$ | Mass-root section: linear root/coframe lengths whose quadratic norms are the exposed masses. | Frozen root section derived from $\Pi_M[\mathcal E_{\ell}(A_{\ell,g}),\mathcal C_{\mathrm{sea}}]$ after `EQ-04` mass-shell acceptance. | The square-root coordinates are computed from the already-fixed mass readouts; no separate root-vector fit, angle handle, or post-Koide rescaling is allowed. | `koide.direct_fit` if the root section or mass-map parameters are chosen by minimizing the Koide residual. | Source-backed frozen mass-root section with a pre-Koide `fixedParameterHash` and durable source evidence for each mass readout. |
| $\hat{\mathbf d}=(1,1,1)/\sqrt3$ | Democratic generation-axis readout for the ordered charged-lepton generation map, not a fitted physical axis. | Generation-order row binding $g=0,1,2$ to the same retained charged-lepton branch family. | The generation index, branch ids, and mass-map ids are fixed before the angle is computed. | `koide.invariant_overclaim` if the axis is described as a derived invariant before the generation map proves it. | Accepted generation-order/source row for $A_{\ell,0}$, $A_{\ell,1}$, and $A_{\ell,2}$ on the same retained support. |
| $Q_{\ell}$ and the inverse ratio | Observer-level scalar residual computed after the mass triplet is fixed. | `eq04a-koide-residual.mjs` residual rows `RKoide` and `RInv`. | The residual consumes the same mass triplet and mass convention as the generation rows. | `koide.direct_fit` if the residual is part of the fit objective; `koide.mass_scheme_ambiguous` if pole/rest/running conventions are mixed. | Accepted mass convention plus frozen mass-map source evidence, followed by a score-neutral residual run. |
| $\cos^2\theta_{\ell}$ | Projection of the frozen mass-root section onto the democratic generation axis. | Residual row `RAngle` plus propagated `cos2ThetaInterval`. | The angle uses the same $\mathbf R_{\ell}$ and uncertainty band as the scalar Koide residual. | `koide.error_band_too_wide` if the propagated band admits broad unrelated mass triples. | Accepted mass-readout uncertainty rows narrow enough to make the angle residual falsifiable. |
| $\mathcal J_K=\lVert\mathbf R_{\mathrm{tr}}\rVert^2-\lvert R_d\rvert^2$ | Moment-map/equipartition diagnostic on the root section. | Residual rows `J_K`, `normalizedJ_K`, `R_d_abs2`, and `R_tr_norm2`. | The trace/traceless split is computed from the same frozen root section; it cannot be an independent target. | `koide.invariant_overclaim` if one frozen residual is called a theorem-level generation invariant. | Accepted frozen root section plus a family-level proof or repeated fixed-branch statistic before any invariant language. |
| $\mathcal S_{04A}^{\mathrm{retune}}$ | Hidden-retune witness for electron, muon, tau, and Koide readouts. | Shared `massMapId`, `exposureRowId`, `shieldingRowId`, `NoetherSeaResponseRowId`, and no-Koide objective metadata. | The electron, muon, tau, and Koide residual are evaluated under the same fixed mass-map parameters. | Split-generation negative control [eq04a-koide-residual-split-generation-map-negative-control.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-split-generation-map-negative-control.v1.json) must continue to fail at `koide.split_generation_map`. | Accepted no-retune witness on the charged-lepton mass map after the inherited `EQ-04` carrier is accepted. |
...
Executable status: [eq04a-koide-residual.mjs](../../../../scripts/equation-mapping/eq04a-koide-residual.mjs) evaluates the score-neutral Koide, angle, and moment-map diagnostics:

```bash
node scripts/equation-mapping/eq04a-koide-residual.mjs --summary --pretty
```

The current attempt fixture [eq04a-koide-residual-attempt.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-attempt.v1.json) reports `status: blocked_inherited_carrier`, `scoreDecision: no_score_increase`, and `nextBlocker: missing_accepted_raw_labeled_rows_preserved_on_retained_history`. It computes the observed charged-lepton diagnostic as `RAngle = 0.000004616434583493145` and `normalizedJ_K = -0.000009232869166904428`, but that is not evidence because the inherited `EQ-04` mass-shell carrier, charged-lepton mass map, and generation rows remain unaccepted.

The accepted-source guard now rejects priority packets, authored AAA prose, generated paths, attempts, toys, probes, source-contract shells, mocks, negative controls, and temporary paths as accepted evidence for `EQ-04A`. [eq04a-koide-residual-priority-source-negative-control.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-priority-source-negative-control.v1.json) keeps accepted-looking priority/source-map rows blocked at the inherited first blocker with `carrierReason=accepted_without_evidence_source` and `scoreDecision=no_score_increase`.

The carrier-shell source-contract boundary is staged at [eq04a-koide-retained-carrier-shell-source-contract.v1.json](../../../../scripts/equation-mapping/eq04a-koide-retained-carrier-shell-source-contract.v1.json) and exercised by [eq04a-koide-retained-carrier-shell-source-contract-attempt.v1.json](../../../../scripts/equation-mapping/eq04a-koide-retained-carrier-shell-source-contract-attempt.v1.json):

```bash
node scripts/equation-mapping/eq04a-koide-residual.mjs --input scripts/equation-mapping/eq04a-koide-retained-carrier-shell-source-contract-attempt.v1.json --summary --pretty
```

The expected boundary run reports `status: blocked_inherited_carrier`, `nextBlocker: missing_accepted_raw_labeled_rows_preserved_on_retained_history`, `carrierAccepted: false`, `carrierReason: accepted_without_evidence_source`, and `scoreDecision: no_score_increase`. This does not land retained evidence. It proves the source-contract shell itself cannot stand in for the inherited mass-shell carrier. Once a real source-backed inherited carrier replaces the contract shell, the next expected child blocker is the charged-lepton mass-map row.
...
Build a small `EQ-04A` residual packet required for advancement that consumes three charged-lepton masses from the same generation-by-shielding record, computes $\mathcal R_{04A}^{\mathrm{Koide}}$ and $\mathcal R_{04A}^{\mathrm{angle}}$, propagates the mass-readout uncertainty to $\cos^2\theta_{\ell}$, and always reports `no_score_increase`. Any later score review requires a separate score review after the masses come from a declared non-Koide mass map with an accepted `EQ-04` mass-shell carrier.
...
The score-neutral checker is [eq04a-koide-residual.mjs](../../../../scripts/equation-mapping/eq04a-koide-residual.mjs). Its default attempt fixture [eq04a-koide-residual-attempt.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-attempt.v1.json) computes the Koide, angle, and moment-map diagnostics while inheriting the current upstream blocker `missing_accepted_raw_labeled_rows_preserved_on_retained_history`. The direct-fit negative control [eq04a-koide-residual-direct-fit-negative-control.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-direct-fit-negative-control.v1.json) must block at `koide.direct_fit` even when all carrier-looking rows are marked accepted. The split-generation negative control [eq04a-koide-residual-split-generation-map-negative-control.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-split-generation-map-negative-control.v1.json) keeps the same near-Koide mass triplet but gives one charged-lepton row a private shielding id; it must block at `koide.split_generation_map`. The priority-source negative control [eq04a-koide-residual-priority-source-negative-control.v1.json](../../../../scripts/equation-mapping/eq04a-koide-residual-priority-source-negative-control.v1.json) marks the carrier, mass map, and generation rows accepted-looking but sources them only to priority prose; it must remain blocked with `accepted_without_evidence_source`. These fixtures change no scores; they exist to prevent a future mass-map comparison from being counted unless it is source-backed, frozen before the Koide check, and bound to the inherited mass-shell carrier.
