...
- Do not promote this packet into reader-facing prose until the missing coefficient and proof rows are supplied as accepted retained evidence and any resulting score movement is recorded in [equation.md](equation.md).
...
| Method with verification required for advancement | Numeric agreement remains score-neutral unless accepted retained rows exist; priority packets, authored prose, generated output, attempt/mock fixtures, and negative controls do not count as retained evidence sources. |
...
This table is priority-only. It maps the continuum, medium, and thermodynamic comparison terms to retained $\Theta_{\mathrm{sea}}^{(\ell,W)}$ geometry, required carriers, same-record bindings, controls required for advancement, and smallest accepted evidence objects.
...
| --- | --- | --- | --- | --- | --- |
| Noether sea density $\rho_{\text{NS}}$ and continuity | Low-moment density/current projection from retained Noether braid centers, group velocities, cadence states, causal wakes, and boundary records. | `density_row`, `cadence_row`, `wake_boundary_row`, `moment_refinement_row` | Same $\Theta_{\mathrm{sea}}^{(\ell,W)}$, same window $W_\ell$, same retained braid inventory, same boundary wake record, and same event ledger. | Refinement failure, hidden source term, cadence-row split, or separate $\rho_{\text{NS}}$ / $\Gamma_N$ records block the continuum row. | Accepted density/cadence projection lemma or executable refinement packet whose residuals decrease from source-backed retained braid population data. |
| Acoustic metric and perturbation speed | Perturbation-speed readout from the same density-compression coefficient row, not a fitted acoustic analogy. | `theta_sea_rho_NS`, `speed_row`, `channel_declaration`, `delayed_support_row` | $\rho_{\text{NS}}$, $n$, $\mathbf u_{\mathrm{sea}}$, $e_{\mathrm{sea}}$, channel declaration, delayed support, and speed coefficient share one event ledger. | `missing_accepted_theta_sea_rho_NS`, non-durable source controls, and delayed-support failures block acoustic residuals before score review. | Source-backed retained density-compression bundle with accepted $\rho_{\text{NS}}$ and perturbation-speed coefficient rows. |
| Stress-strain / elastic response | Coefficient row for $\delta\sigma_{ij}=C_{ij}{}^{kl}\delta\epsilon_{kl}+\cdots$ read from the same Noether sea response window. | `stress_strain_row`, `metric_embedding_row` or accepted explicit missing-output row, `acoustic_elastic_agreement` | Stress/strain, metric compliance, response kernel, speed row, and retune witness stay on the same $\Theta_{\mathrm{sea}}^{(\ell,W)}$. | `attempt_numeric_passed` remains score-neutral unless the agreement row is accepted and source-backed; split stress/metric records fail. | Accepted same-window acoustic/elastic agreement row plus speed and stress/strain or metric-compliance coefficients. |
| Thermodynamic state count and deterministic pushforward | Finite-window statistical record over a declared state space, measure, coarse-graining, and deterministic branch pushforward. | `theta_therm`, state-space row, deterministic-pushforward row, collision/projection row | State space, measure, pushforward, collision/projection, event ledger, and Noether sea row share one finite-window carrier. | `missing_accepted_theta_therm` and source-window split controls block thermodynamic residuals before numeric entropy checks count. | Accepted `theta_therm` carrier with source-backed state-space, measure, pushforward, collision/projection, and event-ledger rows. |
| Entropy balance and fluctuation rows | Entropy-production and fluctuation readouts from the same finite-window statistical measure and boundary flux record. | `entropy_balance`, `thermalization_depth`, `fluctuation`, `source_provenance`, `no_hidden_retune_witness` | Entropy, thermalization depth, fluctuation, source provenance, shared Noether sea row, and retune witness use one source window and carrier. | Hidden thermo retune, shared-key split, and authored/prose source controls block entropy/thermalization rows that use private records. | Accepted thermodynamic record whose entropy, thermalization, fluctuation, provenance, and retune rows are source-backed and same-record bound. |
| Cross-row medium/thermo handoff | Shared Noether sea response that supports medium, radiation, measurement, CMB, or transport consumers without private coefficients. | `source_provenance`, `no_hidden_retune_witness`, downstream consumer-readiness rows | Downstream `EQ-24`, `EQ-25`, `EQ-11`, `EQ-20`, and `EQ-32` consumers cite the same retained window or explicitly declare missing outputs. | Numeric/checker pass is score-neutral when consumer rows are attempts or declared missing outputs. | One accepted source-backed retained-window bundle whose projected consumer rows are accepted or explicitly not advanced without hidden retune. |
...
- Braid center $\mathbf X_k(t)$ and group velocity $\dot{\mathbf X}_k(t)$.
...
The score-neutral runner is [noether-sea-density-compression-surface-slice.mjs](../../../../scripts/spacetime/noether-sea-density-compression-surface-slice.mjs), with a deliberately incomplete mock input in [noether-sea-density-compression-surface-slice-mock.json](../../../../scripts/spacetime/noether-sea-density-compression-surface-slice-mock.json), a retained-attempt skeleton in [noether-sea-density-compression-surface-slice-retained-attempt.v1.json](../../../../scripts/spacetime/noether-sea-density-compression-surface-slice-retained-attempt.v1.json), a focused `rho_NS` source-attempt route in [noether-sea-density-compression-rho-ns-source-attempt.v1.json](../../../../scripts/spacetime/noether-sea-density-compression-rho-ns-source-attempt.v1.json), and an `EQ-32` projected-output probe in [noether-sea-density-compression-eq32-delta-a-star-source-attempt.v1.json](../../../../scripts/spacetime/noether-sea-density-compression-eq32-delta-a-star-source-attempt.v1.json). The two source-attempt fixtures still block at `missing_accepted_theta_sea_rho_NS`; they are runnable artifacts, not retained evidence.
...
The first accepted `theta_sea_rho_NS` / `rho_NS` row cannot be produced by changing the retained-attempt fixture's status field or by pointing that row back at the attempt fixture, this packet, or a prior audit note. Those files are durable coordination records, but they are not retained coefficient evidence for the density row. The consolidated retained-evidence audit found no existing repo evidence that populates accepted retained rows for the Noether sea density-compression surface-slice reducer: every $\Theta_{\mathrm{sea}}$ row, required response row, and retune witness in the retained-attempt skeleton remained attempt-level.
