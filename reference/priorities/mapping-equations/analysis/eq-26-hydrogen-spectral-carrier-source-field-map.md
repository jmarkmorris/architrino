...
| Negative control required for advancement | `observer_rydberg_import_without_hydrogen_carrier`: two observed Rydberg lines match $\Lambda_{ab}$ but the packet lacks source-backed envelope gaps, $\mathbf g_{N,\mathrm H}^{(\ell)}$, static response, and provenance. The carrier-shell source-contract boundary must also not advance at `accepted_without_evidence_source`. |
