...
- Dated score snapshot: the `6/23` ledger records `EQ-26` at `3`, `EQ-26A` at `2`, `EQ-27` at `2`, `EQ-28` at `3`, `EQ-29` at `3`, `EQ-30` at `2`, and `EQ-31` at `2`. This packet preserves the assessment rationale but does not update [equation.md](equation.md), the sole current score authority.
...
| --- | --- | --- |
| `EQ-26` | `3` | The Rydberg coefficient-row residual and hydrogen spectral scaffold are explicit, but the native envelope-gap derivation and spin-sensitive residuals are still open. |
| `EQ-26A` | `2` | The fine-structure coupling and running row is now explicit, but charge exposure, action-unit, photon-speed, gauge/coupling, and scale-threshold rows are not accepted. |
| `EQ-27` | `2` | The angular-momentum and measurement-response ledgers name the right consumers, but no retained branch yet derives $g_\ell$, $a_\ell$, or precession frequencies. |
| `EQ-28` | `3` | The event-ledger grammar is strong; the Compton shift, photoelectric threshold, and pair-threshold cases still need one compact replayable event derivation. |
| `EQ-29` | `3` | Radiation, bremsstrahlung, and synchrotron documents define the residual/event rows, but emitted power, spectrum, and polarization recovery are not yet derived from one source ledger. |
| `EQ-30` | `2` | Cross sections and form factors have a plausible branch-statistics carrier, but specific amplitudes, detector kernels, and exposure distributions are not yet connected to Noether braid dynamics. |
| `EQ-31` | `2` | Resonance widths and branching fractions have the right metastability language, but no branch-stability calculation currently produces $\Gamma$, $\tau$, or $B_k$. |

## Shared Ledger Contract

For the assigned precision rows, the common closure object is a finite event or observation record
...
| Magnetic moment, Larmor/cyclotron precession, and $g-2$ (`EQ-27`) | Ordered-frame loop, spin lift, angular-momentum ledger, exposure-current moment-map, covering-degree row, apparatus record, and exposure-fiber residual. | Accepted `ordered_frame_loop` followed by accepted `moment_map_magnetic` on the same retained branch. | $\mathcal J_{\mathrm{core}}$, exposed mass response, charge/polarity, apparatus, moment-map current, and covering-degree witness must share one `sameRecordId`. | `eq27.assigned_spin_label`, `record_split`, priority-source evidence, and non-gauge spin-lift source-contract failures. | Durable source-backed ordered-frame loop proving nontrivial holonomy and $4\pi$ restoration, then a same-record moment-map magnetic row. |
| Compton, photoelectric, pair-threshold, and recoil rows (`EQ-28`) | Incoming/outgoing photon records, target branch, recoil branch, medium/remnant support, Gate A/B/C rows, and energy-momentum-angular-momentum ledger. | Accepted `photon_gate_A_input_output` on $\mathsf e_{\gamma e}^{0}$, followed by accepted Gate B/C and target/recoil rows. | Gate A source, `EQ-26` shared rows, photon constants, target mass, recoil convention, medium/remnant rows, and event id must bind to the same event ledger. | Attempt-level photon packet import, shared-row anti-retune failure, and `missing_accepted_photon_gate_A_input_output`. | One event-bound Gate A source row proving photon input/output evidence for `eventId: e_gamma_e_0`, not merely a comparison replay. |
| Radiation source power, spectrum, cooling, and polarization (`EQ-29`) | Mechanism-declared source ledger separating carrier/channel family, source mechanism, source branch, Noether sea magnetic state, Gate A/B output, depletion, recoil/wake/remnant, and cooling rows. | Accepted `radiation_source_carrier` with accepted channel-family and source-mechanism rows. | Radiation carrier, source branch, photon channel, Noether sea magnetic state, event ledger, source provenance, and no-retune witness must stay on one source-window identity. | Carrier/channel collapse, source-mechanism collapse, unrelated durable source, priority-source, probe-source, and non-synchrotron mechanism controls. | Retained radiation source carrier with metadata proving `EQ-29` radiation-source support and the declared source mechanism before source-ledger numerics count. |
| Scattering cross sections and form factors (`EQ-30`) | Finite-window carrier, transition map, measure, quotient, detector kernel, outcome partition, prepared flux, detected classes, cross-section rows, exposure distribution, form-factor samples, and elastic-regime row. | Accepted finite-window `W`, then accepted $\Phi_T$, $\mu_{\star,T}$, $Q$, $K_{\mathrm{det}}$, $B$, $S_{\mathrm{retune}}$, and `EQ-30` projection rows. | Parent carrier rows, detector/readout kernel, form-factor samples, elastic-regime declaration, and top carrier must share one `carrierId`. | Toy import, priority-source accepted-looking rows, generic durable source, row-name-only durable source, and `eq30_same_record_binding_mismatch`. | Source-backed finite-window top carrier whose parent rows and `EQ-30` projection rows are accepted from durable evidence on one carrier identity. |
| Resonance widths, lifetimes, and branching fractions (`EQ-31`) | Metastable branch window, corridor family, escape/dephasing measure, first-exit corridor semantics, null-separatrix behavior, refinement compatibility, width, lifetime, branching, and line-shape rows. | Accepted finite-window `W`, accepted corridor family `C`, and accepted retained top carrier for the metastable branch. | Width $\Gamma$, lifetime $\tau$, branching fractions $B_k$, detector classification, corridor preimages, and line-shape residuals must consume the same top finite-window carrier. | Toy evidence, priority-source top carrier, generic durable source, row-name-only durable source, and `blocked_eq31_same_record_binding`. | Retained metastable branch/corridor source packet proving parent finite-window rows, top carrier, and row-specific projection rows on one shared `carrierId`. |
...
The dated `6/23` assessment records `3`. The variable map and residual grammar are explicit, but the native electron-envelope calculation and spin/radiation corrections remain derivation targets. This assessment is not retained evidence and does not change the current score in [equation.md](equation.md).
...
The dated `6/23` assessment records `2`. The closure dependencies are clear, but current material still blocks proof on ordered-frame spinor closure and concrete magnetic-response apparatus dynamics. This assessment is not retained evidence and does not change the current score in [equation.md](equation.md).
...
The dated `6/23` assessment records `3`. The row has a strong event-ledger carrier and local corpus support, but no closed calculation yet derives the standard $\Delta\lambda$ form from the retained photon and target records. This assessment is not retained evidence and does not change the current score in [equation.md](equation.md).
...
The compact replay summary now also reports `nextBlockerDetails`. For the native attempt, the first blocker detail is `id=photon_gate_A_input_output`, `status=attempt`, `rowId=photon-gate-A-input-output-attempt`, `sourcePath=reference/priorities/mapping-equations/analysis/eq-26-31-observation-first-precision-packet.md`, `sourceReferenceExists=true`, `eventId=e_gamma_e_0`, and `eventIdMatches=true`. This confirms that the carrier wiring is present but still not score evidence: the first row must be promoted from `attempt` to an accepted retained row by source-backed Gate A evidence, not by the comparison replay.
...
The dated `6/23` assessment records `3`. The event grammar is developed enough for a strong partial map, but standard $P_L$, Lienard, bremsstrahlung emissivity, synchrotron $\gamma^2B$, and blackbody limits still need derivation from one mechanism-declared ledger. This assessment is not retained evidence and does not change the current score in [equation.md](equation.md).
...
The dated `6/23` assessment records `2`. The native branch-stability interpretation is clear, but no current row computes a width, lifetime, or branching fraction from a retained Noether braid branch. This assessment is not retained evidence and does not change the current score in [equation.md](equation.md).
