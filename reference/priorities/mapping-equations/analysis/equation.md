...
This file is the sole current maturity-score ledger for Equation Mapping. The `6/23` column records the retained 2026-06-23 team-agent disposition. Sibling closure packets may preserve the reasoning behind that dated assessment, but they cannot set or change a score.

Any later score movement requires an explicit edit to this ledger supported by accepted retained evidence, same-record binding, source provenance, the declared residual, and the applicable no-hidden-retune controls. A numerical pass, executable checker, source-attempt object, priority document, authored prose, generated output, probe, mock, or negative control does not by itself authorize score movement.
...
The completed [EQM-010 Solved-Wave Inverse-Clue Audit](../evidence/solved-wave-inverse-clue-audit.md) retains only the six families that already have a named row and falsifier. It adds no score or carrier evidence; its worked Green-function example fixes the exact root, provenance, transversality, and support information that a later `EQ-13` coarse-graining must export.
...
No score change follows from that result. `EQ-02`, `EQ-03`, and `EQ-04` first need one validated finite-memory relative-periodic point in a declared validation box. The operator acts on the square group-reduced residual $F_{N,u}(x)=P_{N,u}(x)-g\mathbin{\cdot}x$:
...
Build the first `EQ-04A` residual packet by connecting the charged-lepton generation-by-shielding row to the `EQ-04` mass map. The first useful artifact is not a high-precision Koide fit; it is a residual required for advancement that reports whether the same predicted lepton masses land near the Koide surface without hidden retuning.
...
Executable status: [effective-metric-weak-field-residual.mjs](../../../../scripts/equation-mapping/effective-metric-weak-field-residual.mjs) now evaluates the weak-field effective-metric residual for `EQ-07` through `EQ-10`. The attempt fixture [effective-metric-weak-field-attempt.v1.json](../../../../scripts/equation-mapping/effective-metric-weak-field-attempt.v1.json) still reports `blocked_missing_rows`, `scoreDecision=no_score_increase`, and first blocker `missing_accepted_theta_W`. The accepted input [effective-metric-weak-field-theta-w-accepted.v1.json](../../../../scripts/equation-mapping/effective-metric-weak-field-theta-w-accepted.v1.json) consumes [effective-metric-theta-w-evidence.v1.json](../../../../scripts/equation-mapping/effective-metric-theta-w-evidence.v1.json) and reports `status=populated`, `nextBlocker=null`, all weak-field rows accepted, and zero weak-field numeric and negative-control residuals. The checker now requires accepted `theta_W` evidence for accepted-looking weak-field rows; the coordination-source control [effective-metric-weak-field-coordination-source-negative-control.v1.json](../../../../scripts/equation-mapping/effective-metric-weak-field-coordination-source-negative-control.v1.json) and generic durable sources remain rejected. This is a score-neutral accepted slice, not an equation score increase.
...
- a locked iso-frequency Noether braid branch with middle-binary speed pinning, integer winding plateau, four-readout coincidence, and positive Floquet margin;
...
## EQ-16A: Neutrino Oscillation Phase Gaps And Equal-Frequency Three-Binary Candidate
...
The iso-frequency Noether braid candidate in iso-frequency-energy-radius-candidate.md is therefore a good match only if it has the form
...
Claim level: derivation-closure target. This is not a corpus claim that $(f,f,f)$ already explains PMNS data. It is a high-value equation-level benchmark because it tests exactly the structure the iso-frequency idea needs: one hidden shared clock plus residual splittings.
...
The first falsifier is sharp: if exact three-binary cancellation also forces $\delta H_{3B}=0$, then iso-frequency three-binary structure cannot explain neutrino oscillation. A second falsifier is loss of the additive beat relation $\Delta\omega_{31}=\Delta\omega_{32}+\Delta\omega_{21}$ after the $\mathbb{A}\mathbb{A}\mathbb{A}$ operator is diagonalized. A third falsifier is a residual operator that naturally produces all-zero or evenly spaced residual eigenvalues when the data require one small solar gap and one much larger atmospheric gap.

### Agent Target

Derive the smallest retained neutral-lepton phase-operator packet: start from the iso-frequency $S_{\mathrm{eq}}$ row set, define $\delta H_{3B}$ from phase offsets, effective lever arms, energy-radius rows, wake/coupling transfer, Noether sea matter response, and weak-coupling exposure, then check whether the resulting eigenvalue gaps can reproduce one small solar-scale gap and one much larger atmospheric-scale gap without fitted flavor terms.
...
The score-neutral executable route is now [effective-frw-handoff-residual.mjs](../../../../scripts/equation-mapping/effective-frw-handoff-residual.mjs). The attempt fixture [effective-frw-handoff-attempt.v1.json](../../../../scripts/equation-mapping/effective-frw-handoff-attempt.v1.json) reports `blocked_missing_rows`, `scoreDecision: no_score_increase`, and first blocker `missing_accepted_theta_cos`. Its numeric handoff checks pass for $H_{\mathrm{eff}}$, $R_H$, $R_\rho$, source provenance, fixed-void discipline, shared keys, and hidden-retune residuals, but those passes are not score evidence because the retained cosmology rows remain `attempt`. The accepted input [effective-frw-handoff-theta-cos-accepted.v1.json](../../../../scripts/equation-mapping/effective-frw-handoff-theta-cos-accepted.v1.json) now consumes [effective-frw-theta-cos-handoff.v1.json](../../../../scripts/equation-mapping/effective-frw-theta-cos-handoff.v1.json) and reports `status=populated`, `nextBlocker=null`, and `sourceEvidenceFailureCount=0`. The [EQ-18 And EQ-19 Theta-Cos Source-Field Map](eq-18-19-theta-cos-source-field-map.md) carries the Direct Geometry Layer binding the homogeneous `theta_cos` carrier, FRW handoff rows, Friedmann rows, continuity/source term, fixed-void witness, shared keys, and no-hidden-retune witness to one source-backed cosmology record.
...
Use the accepted `theta_cos` homogeneous-window record as the fixed-void cosmology handoff for downstream redshift, distance-ladder, growth, CMB, and shared-coupling consumers. The score remains `3` until those consumers preserve the same $a_{\mathrm{eff}}$, $H_{\mathrm{eff}}$, $\rho_{\mathrm{eff}}$, $P_{\mathrm{eff}}$, $\Lambda_{\mathrm{eff}}$, and $\mathcal S_{\mathrm{eff}}$ rows without hidden retuning.
...
The current map is explicit as a comparison interface. The first score-neutral growth child now computes $f\sigma_8$ from accepted shared-observation keys, the matter-power child computes a normalized $P(k,z)$ grid from the same accepted parent and growth child, the CMB-lensing child computes a normalized $C_L^{\phi\phi}$ grid from that accepted chain, the shear/RSD child computes same-chain shear-band and RSD growth readouts, the halo/cluster child checks same-chain lensing/shear/RSD inversion back to the inherited $P(k,z)$ samples, and the nonlinear child computes a deterministic dimensionless-power nonlinear readout from the accepted halo/cluster parent. Score-review observational transfer remains open.
...
Executable status: [shared-observation-residual.mjs](../../../../scripts/equation-mapping/shared-observation-residual.mjs) now evaluates the shared `EQ-21`/`EQ-22`/`EQ-23`/`EQ-32` residual family. The attempt fixture [shared-observation-residual-attempt.v1.json](../../../../scripts/equation-mapping/shared-observation-residual-attempt.v1.json) still reports `blocked_missing_rows`, with first blocker `missing_accepted_theta_obs`. The accepted input [shared-observation-provider-backed-consumer-accepted.v1.json](../../../../scripts/equation-mapping/shared-observation-provider-backed-consumer-accepted.v1.json) consumes [shared-observation-provider-backed-consumer-evidence.v1.json](../../../../scripts/equation-mapping/shared-observation-provider-backed-consumer-evidence.v1.json), which cites the populated shared `EQ-11`/`EQ-20` residual, accepted `theta_W` evidence, accepted output-projection evidence, and accepted `theta_cos` handoff. It reports `status=populated`, `nextBlocker=null`, no missing projection families, no missing shared keys, and `hiddenRetuneNumericPass=true` across `BBN`, `CMB`, `growth`, and `RAR` consumers. On top of that parent, [eq21-growth-transfer-child-residual.mjs](../../../../scripts/equation-mapping/eq21-growth-transfer-child-residual.mjs) evaluates the first predictive child. The accepted input [eq21-growth-transfer-child-accepted.v1.json](../../../../scripts/equation-mapping/eq21-growth-transfer-child-accepted.v1.json) consumes [eq21-growth-transfer-child-evidence.v1.json](../../../../scripts/equation-mapping/eq21-growth-transfer-child-evidence.v1.json), inherits the accepted shared-observation path and growth keys, computes $f\sigma_8=0.4165634684945517$, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `hiddenRetuneNumericPass=true`, `modelDerivedPass=true`, `fSigma8ResidualPass=true`, and `scoreDecision=no_score_increase`. The matter-power child [eq21-matter-power-transfer-child-residual.mjs](../../../../scripts/equation-mapping/eq21-matter-power-transfer-child-residual.mjs) consumes both the accepted shared-observation parent and the accepted $f\sigma_8$ child. Its accepted input [eq21-matter-power-transfer-child-accepted.v1.json](../../../../scripts/equation-mapping/eq21-matter-power-transfer-child-accepted.v1.json) consumes [eq21-matter-power-transfer-child-evidence.v1.json](../../../../scripts/equation-mapping/eq21-matter-power-transfer-child-evidence.v1.json), computes a six-sample normalized $P(k,z)$ grid, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `parentGrowthTransferAccepted=true`, `hiddenRetuneNumericPass=true`, `modelDerivedPass=true`, `matterPowerGridResidualPass=true`, `sampleCount=6`, `matterPowerGridNormalizedResidual=0`, and `scoreDecision=no_score_increase`. The CMB-lensing child [eq21-lensing-transfer-child-residual.mjs](../../../../scripts/equation-mapping/eq21-lensing-transfer-child-residual.mjs) consumes the accepted shared-observation parent, accepted $f\sigma_8$ child, and accepted normalized $P(k,z)$ child. Its accepted input [eq21-lensing-transfer-child-accepted.v1.json](../../../../scripts/equation-mapping/eq21-lensing-transfer-child-accepted.v1.json) consumes [eq21-lensing-transfer-child-evidence.v1.json](../../../../scripts/equation-mapping/eq21-lensing-transfer-child-evidence.v1.json), computes a three-sample normalized $C_L^{\phi\phi}$ grid, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `parentMatterPowerTransferAccepted=true`, `hiddenRetuneNumericPass=true`, `modelDerivedPass=true`, `cmbLensingGridResidualPass=true`, `sampleCount=3`, `cmbLensingGridNormalizedResidual=0`, and `scoreDecision=no_score_increase`. The shear/RSD child [eq21-shear-rsd-transfer-child-residual.mjs](../../../../scripts/equation-mapping/eq21-shear-rsd-transfer-child-residual.mjs) consumes the accepted shared-observation parent, accepted $f\sigma_8$ child, accepted normalized $P(k,z)$ child, and accepted CMB-lensing child. Its accepted input [eq21-shear-rsd-transfer-child-accepted.v1.json](../../../../scripts/equation-mapping/eq21-shear-rsd-transfer-child-accepted.v1.json) consumes [eq21-shear-rsd-transfer-child-evidence.v1.json](../../../../scripts/equation-mapping/eq21-shear-rsd-transfer-child-evidence.v1.json), computes a three-sample shear/RSD grid, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `parentLensingTransferAccepted=true`, `hiddenRetuneNumericPass=true`, `modelDerivedPass=true`, `shearRsdGridResidualPass=true`, `sampleCount=3`, `shearRsdGridNormalizedResidual=0`, and `scoreDecision=no_score_increase`. The halo/cluster child [eq21-halo-cluster-transfer-child-residual.mjs](../../../../scripts/equation-mapping/eq21-halo-cluster-transfer-child-residual.mjs) consumes the accepted shared-observation parent, accepted $f\sigma_8$ child, accepted normalized $P(k,z)$ child, accepted CMB-lensing child, and accepted shear/RSD child. Its accepted input [eq21-halo-cluster-transfer-child-accepted.v1.json](../../../../scripts/equation-mapping/eq21-halo-cluster-transfer-child-accepted.v1.json) consumes [eq21-halo-cluster-transfer-child-evidence.v1.json](../../../../scripts/equation-mapping/eq21-halo-cluster-transfer-child-evidence.v1.json), computes a three-sample halo/cluster inversion grid, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `parentShearRsdTransferAccepted=true`, `hiddenRetuneNumericPass=true`, `modelDerivedPass=true`, `haloClusterGridResidualPass=true`, `sampleCount=3`, `haloClusterGridNormalizedResidual=3.0204931705456123e-16`, and `scoreDecision=no_score_increase`. The nonlinear child [eq21-nonlinear-transfer-child-residual.mjs](../../../../scripts/equation-mapping/eq21-nonlinear-transfer-child-residual.mjs) consumes the same accepted chain plus the accepted halo/cluster child. Its accepted input [eq21-nonlinear-transfer-child-accepted.v1.json](../../../../scripts/equation-mapping/eq21-nonlinear-transfer-child-accepted.v1.json) consumes [eq21-nonlinear-transfer-child-evidence.v1.json](../../../../scripts/equation-mapping/eq21-nonlinear-transfer-child-evidence.v1.json), computes a three-sample deterministic dimensionless-power nonlinear readout, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `parentHaloClusterTransferAccepted=true`, `hiddenRetuneNumericPass=true`, `modelDerivedPass=true`, `nonlinearGridResidualPass=true`, `sampleCount=3`, `nonlinearGridNormalizedResidual=3.0204931705456123e-16`, and `scoreDecision=no_score_increase`. The `EQ-32` galaxy-response child [eq32-galaxy-response-child-residual.mjs](../../../../scripts/equation-mapping/eq32-galaxy-response-child-residual.mjs) consumes the accepted shared-observation parent and accepted `delta_a_star` output projection. Its accepted input [eq32-galaxy-response-child-accepted.v1.json](../../../../scripts/equation-mapping/eq32-galaxy-response-child-accepted.v1.json) consumes [eq32-galaxy-response-child-evidence.v1.json](../../../../scripts/equation-mapping/eq32-galaxy-response-child-evidence.v1.json), computes a three-sample RAR/BTFR/high-acceleration grid, and reports `status=populated`, `nextBlocker=null`, `sourceEvidenceFailureCount=0`, `parentSharedObservationAccepted=true`, `parentOutputProjectionAccepted=true`, `hiddenRetuneNumericPass=true`, `rarGridResidualPass=true`, `btfrLowAccelerationPass=true`, `highAccelerationRecoveryPass=true`, `lensingDynamicsSplitPass=true`, and `scoreDecision=no_score_increase`. These are score-neutral child-consumer successes, not score-review closure.

### Agent Target

Extend the populated nonlinear and galaxy-response children into score-review observational transfer, or route the same accepted shared-observation evidence into CMB transfer/blackbody/acoustic or BBN source-window child physics while preserving the no-private-retune witness.
...
The focused integration and derivation program is the [Fine-Structure Coupling Map](../../mapping-electromagnetism/analysis/fine-structure.md). This section retains the scored equation row and compact closure contract.
...
This is not a direct three-binary particle row. It is an observation-first constitutive-response benchmark for the Noether sea around baryonic assemblies. The useful question is whether a shared Noether sea response law can make the apparent acceleration depart from the baryonic Newtonian estimate in the low-acceleration regime while preserving lensing, structure growth, CMB, BBN, and local gravity constraints.
...
Broaden the populated galaxy-response child into score-review observational transfer and broader compatibility checks without changing the accepted shared-observation record.
...
- Start with `reference/priorities/mapping-equations/priorities.md` and `reference/priorities/mapping-equations/analysis/equation.md`.
...
5. Each worker should produce a bidirectional closure packet with:
   ...
   - an Equation Attack Card with current score, closure driver, primary carrier, smallest accepted evidence object, exact first blocker, existing scripts/fixtures/packets, negative control required for advancement, and smaller next action;
   - a Direct Geometry Layer table mapping each comparison term to its $\mathbb{A}\mathbb{A}\mathbb{A}$ geometric readout, required carrier or row, same-record binding, negative control required for advancement, and smallest accepted evidence object;
...
Scope:
- Edit authority: direct edits allowed under `reference/priorities/mapping-equations/`.
