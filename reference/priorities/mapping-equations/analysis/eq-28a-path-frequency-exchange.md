...
| Deepest current boundary | The carrier-shell source-contract attempt marks only the top carrier and `path_frequency_exchange_carrier` row as accepted-looking for boundary testing, but the checker now rejects the source-contract shell itself as non-evidence and remains blocked at `missing_accepted_path_frequency_exchange_carrier` with `carrierReason=accepted_without_evidence_source`. |
| Child-row source guard | [eq28a-path-frequency-exchange-theta-gamma-source-negative-control.v1.json](../../../../scripts/equation-mapping/eq28a-path-frequency-exchange-theta-gamma-source-negative-control.v1.json) marks `theta_gamma_packet` accepted-looking while sourcing it only to a source-attempt fixture; after source-contract hardening, this child-row guard is masked until the parent carrier uses a real retained source rather than the contract shell. |
...
The carrier-shell source-contract boundary is staged at [eq28a-path-frequency-exchange-carrier-shell-source-contract.v1.json](../../../../scripts/equation-mapping/eq28a-path-frequency-exchange-carrier-shell-source-contract.v1.json) and exercised by [eq28a-path-frequency-exchange-carrier-shell-source-contract-attempt.v1.json](../../../../scripts/equation-mapping/eq28a-path-frequency-exchange-carrier-shell-source-contract-attempt.v1.json). That boundary fixture marks only the top `carrier` and `path_frequency_exchange_carrier` row as accepted-looking against the contract source. The hardened checker now rejects the contract shell as non-evidence, so the expected result is `status: blocked_missing_accepted_path_frequency_exchange_carrier`, `nextBlocker: missing_accepted_path_frequency_exchange_carrier`, `carrierAccepted: false`, `carrierReason: accepted_without_evidence_source`, and `scoreDecision: no_score_increase`. The next child-row boundary is only meaningful after a real retained parent-carrier source replaces the contract shell.
