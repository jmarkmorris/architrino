...
- Source fixtures:
  ...
  - [eq15-27-ordered-frame-loop-unrelated-durable-source-negative-control.v1.json](../../../scripts/equation-mapping/eq15-27-ordered-frame-loop-unrelated-durable-source-negative-control.v1.json)
...
The fixture `eq15-27-ordered-frame-loop-unrelated-durable-source-negative-control.v1.json` isolates the unrelated durable-source branch: all rows are accepted-looking and numerically pass same-record, gauge, angular-momentum, nonzero moment-map, and $g_{\mathrm{lead}}=2$ checks, but every row points to a durable source shell whose metadata belongs to the `S_eq` same-branch retained-domain lane, not the `EQ-15`/`EQ-27` `ordered_frame_loop` carrier. The checker must return `blocked_source_evidence`, `nextBlocker: source_lacks_eq15_eq27_ordered_frame_loop_support`, and `scoreDecision: no_score_increase`.
