...
| Candidate breakthrough angle | Use one EOM event record from the synchrotron lane: a charged assembly path segment with $\Gamma_{e^\pm}(t)$, causal-root/Jacobian data (same-record transmitter factor $D_t$, acceleration weight $W_{\mathrm{acc}}=c_f/\lvert D_t\rvert$, and separate signed playback $D_r/D_t$), $\mathcal V_{\mathrm{NS}}$, $G_{\text{grad}}$, wake-strain threshold status, photon Gate A/B output, and transmitter-depletion identity. |
