...
| doubling-frequency-lock (legacy-braid ref: `braid-doubling-frequency-lock/priorities.md`) | Finite-$\eta$ reduced phase-amplitude map and stable `1:2` / `1:2:4` fixed-point diagnostics. | Provides a concrete reduced-map laboratory for transfer-operator stability, invariant or metastable measures, and basin gaps. |
