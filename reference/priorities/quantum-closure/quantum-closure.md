...
10. **No-signaling and Tsirelson checks.** The same calculations must show setting-independent marginals at each detector and no violation of the Tsirelson bound $|S|\leq2\sqrt{2}$. The no-signaling row should be derived as local-setting relabeling invariance of the joint basin measure, so one-wing marginalization is independent of the far setting rather than being imposed as a post-fit cancellation. A superquantum result is not a success state; it is a failure diagnostic.
...
- The correlation calculations recover the spin-singlet curve, the photon-polarization curve, the CHSH values, no-signaling marginals, the Tsirelson ceiling, and the preferred-frame leakage handoff from the corresponding derived kernels and observer-export records, with no-signaling supplied by local-setting relabeling invariance of the joint basin measure.
