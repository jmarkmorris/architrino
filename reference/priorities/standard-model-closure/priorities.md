...
This workstream owns the remaining Standard Model-facing closure tasks that are not already carried by mass-map (legacy-braid ref: `braid-mass-response-map/priorities.md`), angular-momentum-spin (legacy-braid ref: `braid-angular-momentum-spin/priorities.md`), or [quantum-closure](../quantum-closure/priorities.md).
...
The focused source-mining capture for QCD confinement, hadronization, jet-fragmentation, and lattice observables is [QCD Confinement And Hadronization Recovery Targets](qcd-confinement-hadronization-recovery-targets.md). It keeps parton-level terms, hadron-level observables, detector-level objects, and finite-assembly recovery targets separated while adding benchmark rows for static tension, color-singlet closure, fragmentation functions, event shapes, reconstructed jets, and the downstream residual nuclear potential. The first reduced $p+n$/$p+p$ channel evaluation for that downstream potential is [NN Corridor Overlap First Evaluation](../nuclear-atomic-molecular-closure/nn-corridor-overlap-first-evaluation.md).
...
| `confinement_energetics` | [geometry-first-program.md](geometry-first-program.md) and [qcd-confinement-hadronization-recovery-targets.md](qcd-confinement-hadronization-recovery-targets.md) | [color-charge-su3](../../../content/markdown/aaa/assemblies/fermions/color-charge-su3.md) and [gluons](../../../content/markdown/aaa/assemblies/bosons/gluons.md) | Confinement-scale behavior derived from topological or strain energetics with a color-singlet bound-state check and a native $\sigma_{\mathrm{eff}}$ extraction row. |
