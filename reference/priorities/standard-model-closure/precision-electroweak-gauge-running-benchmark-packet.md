...
| --- | --- | --- | --- | --- |
| $G_F$ | Muon lifetime normalization | Derived from lifetime plus radiative-correction convention; not a primitive weak charge. | `weak_exposure_record`; low-energy weak-current normalization. | `missing_accepted_weak_visible_branch_ledger`; later `reaction_event_ledger`. |
...
| $\bar{s}_{\ell}^{\,2}$, $\sin^2\theta_{\mathrm{eff}}^{\ell}$ | Effective leptonic weak mixing angle | Effective fit/extraction from asymmetries; collider versions depend on PDFs and angular modelling. | Weak-exposure angular readout; shared covariance residual. | No accepted use until `weak_projection`, `weak_quotient`, and `effective_gauge_covariance_witness` share one carrier. |
| CKM rows | Flavor overlap | Fit output under unitary and decay-model assumptions; first-row tension is a residual, not a retune license. | `ckm_overlap_readout` in the same weak-exposure domain. | `missing_accepted_weak_visible_branch_ledger`. |
| PMNS rows | Neutrino mixing and mass-difference surface | Fit output with ordering, matter-effect, source/detector, and absolute-mass-bound conventions. | `pmns_overlap_readout`; neutrino weak-exposure consistency row. | `missing_accepted_weak_visible_branch_ledger`; neutrino carrier rows remain separate. |
...
| Require same-domain weak exposure before scoring CKM, PMNS, `V-A`, W/Z, and weak-angle rows. | The current first blocker is one accepted `weak_visible_branch_ledger`, not another benchmark table. |

## Priority Closure Targets

1. Populate one source-backed `weak_visible_branch_ledger` for `EQ-16`. The row must bind weak projection, quotient, weak exposure, `V-A` chirality, CKM/PMNS overlap, weak-corridor provenance, effective gauge covariance, reaction event ledger, and Noether sea response on one weak-visible retained domain. Current first blocker: `missing_accepted_weak_visible_branch_ledger`.
