...
- **Next artifact:** calculate the full covariance response of $(R_b,A_b,A_{\mathrm{FB}}^b)$ to one predeclared axial-frame exposure rotation and test whether a single $\phi_b$ improves the asymmetry without worsening the rate or other neutral-current rows.
