...
The current scalar-action route already identifies one such architrino-native candidate: a recoil contribution distinct from the rejected receiver playback multiplier. It is not included in the proposed equation because the action is intrinsically two-sided: its complete variation needs both past emissions received now and present emissions received later. The [coincident-birth recoil asymptotic](analysis-causal-recoil-coincident-birth-asymptotic.md) shows that the incoming sharp contribution is order $t^{-4}$, while its time-transposed companion is advanced; it also records that an even finite-width profile cancels the leading recoil impulse on the diagnostic newborn partition, so the sharp and finite-width integration limits must not be conflated.

The [minimal causal wake-state model](analysis-minimal-causal-wake-state-model.md) replaces future dependence with a present scalar wake state and derives explicit conserved totals, but it fails a more basic physical gate: one real scalar wake cannot give both like-polarity repulsion and positive wake energy. A stable causal wake state or different conservation mechanism remains a promotion obligation. The current state of every gate is summarized in the [Master Equation Promotion Readiness Matrix](master-equation-promotion-readiness-matrix.md).
