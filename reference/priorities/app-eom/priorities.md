...
3. `precision_convergence_and_failure_policy` — Propagate error budgets through interpolation, roots, branch evaluation, accumulation, integration, reductions, and output. Add split absolute time and retain MPFR escalation for difficult rows. Require the timestep, history-depth, sampling, regulator, perturbation, precision, and worker-count ladders used by each claim; fail closed when certification fails. Amendment 2 and both complete preset records are ratified; V8 carries per-axis state radii, shared segment endpoints, complete provenance, the UI selector, unchanged-Decimal-oracle containment, deliberate under-budget rejection, and certified-prefix display after a later rejected candidate. Borg now avoids the optional far-field enclosure width at six paths, contracts sharp-root brackets only inside acceleration reconstruction, and publishes a recertified four-quarter candidate under the unchanged allocation. The resulting seed 0–3 matrix passes `T=1.2` in both presets but still halts after `4.13` to `45.47` wall seconds on encounter-dependent root-state amplification or an uncertified caustic transit. A new root-time budget is no longer the next remedy; the open target is a certified richer joint-state/cross-path correlation or separately validated reconditioned integrator. Status: `active-regulator-event-foundation; fixed-budget-ten-minute-acceptance-blocked-on-joint-state-representation`. Depends on: `coupled_retained_history_integrator`.
