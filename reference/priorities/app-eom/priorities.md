...
7. `eom_application_surface` — Complete the bounded-population run/inspect surface with retained-history import, duration and step controls, precision display, progress, cancellation, checkpointing, convergence, failure diagnostics, and provenance. Status: `active-borg-run-control-surface`. Depends on: the bounded-population portions of items 2–5.
8. `borg_shadow_run_and_first_migration` — Evolve Borg from a certified artificial retained history and preserve the conditional provenance on every published EOM frame. The initial-history interval is computed from the selected positions and source speeds rather than evolved as startup work. Status: `completed-accepted-initial-history-promotion`; the strict eight-path ladder passed with maximum state delta `6.30e-14` and byte-identical one-thread/four-thread output. Ordinary Borg startup is idle, and explicit Start begins EOM evolution at $T=0$. General EOM validation remains in the solver-validation workstream. Evidence: [eom-borg-eight-path-promotion-attempt-apple-m3-2026-07-16.md](evidence/eom-borg-eight-path-promotion-attempt-apple-m3-2026-07-16.md). Depends on: `eom_application_surface`, `eom_sole_engine`.
9. `consumer_migration` — Every consumer runs on EOM, conditional prescribed-history analysis, or visualization-only playback of recorded EOM datasets. Status: `complete`. Depends on: `eom_sole_engine`.
...
- no result claims EOM authority when the run used a prescribed analytic path, a constrained replay, T3, or a display interpolation.
