...
1. `coupled_retained_history_integrator` — Complete the production history-to-roots-to-acceleration-to-history kernel for bounded-population long runs: enumerate every admitted sharp root for every ordered pair including self, route certified near-multiple strata through the bound finite-width law, evaluate the Master EOM, and advance coupled histories atomically. Certified regulator-limit common-domain matching now supports the first seed-0 finite-width transit; post-transit horizon and broader refinement acceptance remain open. Borg now forks a permanent, explicitly `display-only` point-evolution suffix from the last accepted boundary when certification fails; that restores exploratory motion but does not satisfy or reduce this claim-ready kernel obligation. Status: `active-production-kernel`. Depends on: completed native root, acceleration, and atomic-evolution layers.
