...
3. `precision_convergence_and_failure_policy` — Propagate error budgets through interpolation, roots, branch evaluation, accumulation, integration, reductions, and output. Add split absolute time and retain MPFR escalation for difficult rows. Require the timestep, history-depth, sampling, regulator, perturbation, precision, and worker-count ladders used by each claim; fail closed when certification fails. Amendment 2 and both complete preset records are ratified; V8 carries per-axis state radii, shared segment endpoints, complete provenance, the UI selector, unchanged-Decimal-oracle containment, deliberate under-budget rejection, and certified-prefix display after a later rejected candidate. Borg avoids optional far-field width at six paths, contracts sharp-root brackets only inside acceleration reconstruction, and publishes a recertified four-quarter candidate. MPFR retained-history reconditioning now propagates differentiable error through every shared join and has an analytic forced-precision control; it narrows Research seed 0's difficult residual substantially but still leaves the root image about `1%` over the fixed ceiling. Publication-count, multirate, embedded-intersection, and unpreconditioned affine-solve ablations are insufficient. The open target is a certified cross-path joint-state representation or theorem-backed preconditioned interval corrector under the unchanged preset hashes. Status: `active-regulator-event-foundation; fixed-budget-ten-minute-acceptance-blocked-on-cross-path-joint-state-representation`. Depends on: `coupled_retained_history_integrator`.
