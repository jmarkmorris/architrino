...
1. `coupled_retained_history_integrator` — Complete the production history-to-roots-to-acceleration-to-history kernel for bounded-population long runs: enumerate every admitted sharp root for every ordered pair including self, route certified near-multiple strata through the bound finite-width law, evaluate the Master EOM, and advance coupled histories atomically. Certified regulator-limit common-domain matching now supports the first seed-0 finite-width transit; post-transit horizon and broader refinement acceptance remain open. Borg exposes two fixed run grades instead of an automatic crossover: `Display grade` starts directly from point-projected input history and retains the input-plus-computed point history append-only for exploratory motion, while `Claim grade` runs certification and stops on failure. Display motion does not satisfy or reduce this claim-grade kernel obligation. Status: `active-production-kernel`. Depends on: completed native root, acceleration, and atomic-evolution layers.
2. `persistent_long_run_checkpoint_and_campaign_driver` — Keep retained histories resident across accepted windows, and retire an older segment only after a solver-owned causal-support test proves that no future requested receiver event can reach it; provide checkpoint/resume, deterministic campaign manifests, convergence ladders, perturbation batches, progress, cancellation, claim observables, and reusable ensemble execution without resending complete histories for every chunk. Adopt the checkpointed attractor-search harness and measured cost instrument as EOM capabilities while leaving seed choice, persistence criteria, fate classification, and collapse adjudication with the consuming scientific program. Status: `active-disk-backed-exact-history-complete-checkpointing-open`; Display suffix transport, bounded browser and server relay ownership, host-aware admission, and a solver-owned fixed-spherical-envelope clearance certificate exist. The persistent worker stores full immutable 64-segment blocks as exact decimal-token temporary files under a one-TiB ceiling, pages them through a bounded per-thread cache, removes the prior temporary run before a replacement run, and releases the active store when the run ends. A path outside the clearance envelope continues with exact disk-backed history and receives no retirement certificate; it is not dropped or halted by retention policy. Only the solver-owned clearance certificate can retire a causally unreachable prefix during a live run. Checkpoint/resume, crash-surviving manifests, and general campaign orchestration remain open. Depends on: `coupled_retained_history_integrator`.
3. `precision_convergence_and_failure_policy` — Propagate error budgets through interpolation, roots, branch evaluation, accumulation, integration, reductions, and output. Add split absolute time and retain MPFR escalation for difficult rows. Require the timestep, history-depth, sampling, regulator, perturbation, precision, and worker-count ladders used by each claim; do not advance when certification fails. Amendment 2 and both complete preset records are ratified; V8 carries per-axis state radii, shared segment endpoints, complete provenance, the UI selector, unchanged-Decimal-oracle containment, deliberate under-budget rejection, and certified-prefix display after a later rejected candidate. Borg avoids optional far-field width at six paths, contracts sharp-root brackets only inside acceleration reconstruction, and publishes a recertified four-quarter candidate. MPFR reconditioning and the prior representation ablations do not close the fixed-budget endurance gate. The root-time theorem, shared-symbol consumer, fallback-dominance gate, delayed-root and sharp-acceleration interval derivatives, centered certified sharp-row map, generic Krawczyk certificate, strict joint root bracket, exact-pair difficult-cell bridge, and retained joint-cubic evaluator now exist. The baseline shadow fails ordinary-radius dominance by `23%`–`29%`; suppressing only redundant root-bracket symbols reduces that to at most `15.64%`, while the deliberately incomplete propagated-only row passes dominance with root-time width `1.83501e-4`. The open target is live snapshot-row summation, the actual 18-by-18 endpoint-corrector Krawczyk enclosure, coefficient/remainder append and incremental-cache retention, and the unchanged-hash 300-second adjudicator. Status: `active-regulator-event-foundation; certified-joint-sharp-row-derived-and-controlled; fixed-budget-ten-minute-acceptance-blocked-on-snapshot-sum-and-corrector-publication`. Depends on: `coupled_retained_history_integrator`.
...
5. `first_binary_and_claim_outcome_gates` — Apply the base EOM contract to binary and claim outcomes with complete histories, all partner and self-root families, event routes, adaptive long-horizon evolution, convergence, and perturbation stability. Status: `pending-claim-campaign`; source: [evolution-contract-v1.md](evolution-contract-v1.md). Depends on: items 1–4, but not the million-path scale profile.
...
5. `million_path_performance_and_acceptance_gate` — Pass `eom_evolution_contract/v1/amendment-1` with complete compact accounting of all $10^{12}$ ordered relationships, exhaustive nested controls, heterogeneous parity, distributed restart, and dense-workload evidence required before advancement.
