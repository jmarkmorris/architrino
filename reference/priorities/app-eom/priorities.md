...
The pre-EOM evaluator was removed 2026-07-16 after consumer-by-consumer migration (removed 2026-07-16 — see [pre-eom-evaluator-removal](../operations/pre-eom-evaluator-removal.md)). EOM is the only engine; no other engine may be introduced, resurrected, or cited.

This operator decision supersedes the earlier implementation assumption that every missing dynamical capability must be added to the pre-EOM engine of the time. EOM is endorsed now, while canonical authority for any particular output remains gated by the frozen EOM contract, independent validation, and consumer migration requirements.
...
2. (Discharged 2026-07-16.) The migration-era compatibility window is closed; the pre-EOM evaluator and all its outputs are removed.
...
1. `legacy_solver_containment_and_prediction_provenance` — Enforce EOM as the only forward solver target; inventory every existing consumer of prescribed paths, current-solver motion, or `canonical_eom_evidence`; prohibit any non-EOM engine; separate conditional path analysis from evolved paths; and invalidate false dynamical authority. Status: `complete-by-removal` — the pre-EOM evaluator, its outputs, and its paper trail were removed 2026-07-16 (removed 2026-07-16 — see [pre-eom-evaluator-removal](../operations/pre-eom-evaluator-removal.md)). Depends on: none.
...
9. `borg_shadow_run_and_first_migration` — Evolve Borg from a certified artificial retained history and preserve the conditional provenance on every published EOM frame. The initial-history interval is computed from the selected positions and source speeds rather than evolved as startup work. Status: `completed-accepted-initial-history-promotion`; the strict eight-path ladder passed with maximum state delta `6.30e-14` and byte-identical one-thread/four-thread output. Ordinary Borg startup is idle, explicit Start begins EOM evolution at $T=0$, and the pre-EOM runtime is removed under operator authorization. General EOM validation remains in the solver-validation workstream. Evidence: [eom-borg-eight-path-promotion-attempt-apple-m3-2026-07-16.md](evidence/eom-borg-eight-path-promotion-attempt-apple-m3-2026-07-16.md). Depends on: `eom_application_surface`, `legacy_solver_containment_and_prediction_provenance`.
10. `consumer_by_consumer_eom_migration_and_legacy_retirement` — Migrate every eligible existing consumer to EOM individually; classify the remainder as conditional prescribed-history analysis, geometry/root analysis, visualization-only playback, separate model, quarantine, or retirement. No new work may enter the legacy side while this queue is open. Status: `active-after-borg-migration`; source: [migration-plan.md](migration-plan.md). Depends on: `legacy_solver_containment_and_prediction_provenance`.
