# OpenAlex Source Baseline for Simulations
...
| [collinear-breather](../../../content/markdown/aaa/proof-programs/collinear-breather.md) | Fold-layer and branch-chart material already has the right proof vocabulary; use this source baseline if a simulation packet starts consuming those rows. | Follow-up only if a run is selected. |
| [master-equation-closure baseline](../master-equation-closure/openalex-source-baseline.md) | Rank-1 baseline already captured state-dependent delay compatibility. This simulations baseline is the numerical/regularization counterpart. | Cross-workstream linkage established. |
