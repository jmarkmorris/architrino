...
The [focused review](office-document-standards-review.md#focused-review-of-the-technical-brief-and-shared-instructions) and its authorized refinements are complete. The brief now requires the full vector acceleration residual, distinguishes it from radial, causal-root, and playback diagnostics, and explains retained branches, causal-return records, and certificates where needed. Moving the preserved shared-prompt history to an archival companion remains optional and deferred; the historical material is already explicitly separated from current instructions.
...
Diagnostic details are now optional and collapsed by default. The GitHub form also permits an empty diagnostic field; readers write and submit their report there. The [intake account](privacy-safe-feedback-intake.md#accepted-path) explains the complete flow and its public-submission boundary.
