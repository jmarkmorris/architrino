...
OPS-013 is awaiting remote and live verification under the dated [production byte-identity audit](pages-borg-record-byte-identity-audit-2026-09-02.md). The deployed `main` registry and record payload disagree for 143 of 145 Borg records. The candidate emitter now uses a duration-scaled $2\times10^{-11}$ position grid with conservatively enlarged residual bounds; it collapses all 145 historical macOS/Ubuntu record pairs to byte identity. Current regeneration and a source-only reconstruction each pass the new 145-of-145 registry/collection verifier, which now runs in Content Integrity and immediately before the Pages artifact upload. Closure still requires current-branch Ubuntu execution, a verified `main` deployment, 145-of-145 public hash agreement, and representative Borg loading.
