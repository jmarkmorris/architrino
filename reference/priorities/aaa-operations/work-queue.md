...
- **Evidence / blocker:** The fourteen-application count, the one release profile, and the two performance profiles are `measured` on 2026-09-06. That the thirteen unwatched pages carry equivalent release risk is `guessed`; no page-by-page assessment has been made, and that assessment is the task. That a stale profile halts the suite is `measured`, by observing `check-content-integrity.mjs` stop at its sixth step. No blocker; this needs an operator decision rather than a capability. Update, 2026-09-06: the page-by-page assessment is now made above, and the equivalent-risk guess is withdrawn in favour of the per-page reasons recorded there; the remaining blocker is operator acceptance of the two recommendations.
