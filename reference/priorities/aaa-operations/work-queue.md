...
- **Evidence / blocker:** Awaiting one run of the real aggregate on the operator's machine to confirm the summary reads well over the true thirty-three steps; this sandbox cannot run the full aggregate inside its call limit. The router fingerprints the runner, so expected startup-router drift accompanies the edit into the branch/PR process.
...
- **Evidence / blocker:** Sandbox estimate for the swept set is about 54 s at three workers; the Mac figure is unmeasured and is the number to record next. Promotion to blocking waits on a green Mac sweep, which depends on OPS-018, OPS-019, and the two routed breakage rows.
