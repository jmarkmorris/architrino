...
6. `release_gate_profile_coverage` — [OPS-020](#ops-020--release-gate-profile-coverage). Status: `Queued`, questions 1 and 2 decided, question 3 open.
7. `content_integrity_runner_report_and_continue` — [OPS-021](#ops-021--content-integrity-runner-report-and-continue). Status: `Queued`.
...
- **Evidence / blocker:** No blocker. The router fingerprints the runner at 175 lines, so the edit produces expected startup-router drift for the branch/PR process.
