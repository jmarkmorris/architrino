...
3. `layout_pass_digest_pin_attribution` — [OPS-018](#ops-018--layout-pass-digest-pin-attribution). Status: `Awaiting verification`; all nine Node digest failures carry verdicts, none attributable to the layout pass; only the Python suites remain unrun for want of the shared venv.
4. `release_gate_profile_coverage` — [OPS-020](#ops-020--release-gate-profile-coverage). Status: `Queued`, all three questions decided.
5. `agent_guidance_surface_consolidation` — [OPS-014](#ops-014--agent-guidance-surface-consolidation). Status: `Queued`.
6. `reference_equation_mapping_surface` — [OPS-016](#ops-016--reference-equation-mapping-surface). Status: `Queued`.
7. `archie_mcp_tool_contract_fixture_drift` — [OPS-023](#ops-023--archie-mcp-tool-contract-fixture-drift). Status: `Queued`.
