...
| Large-object lifecycle | A required public object exceeds the selected host's per-file limit | Accepted need to publish large runtime objects independently of the site release | Named consumer, cache/integrity contract, rollback behavior, and pilot on the proposed object path. |
