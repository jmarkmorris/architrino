...
The finite-range residual packet now makes the prior tail step explicit: define $\mathcal T_{NN}(R)=\sup_{r\ge R}|\Delta E_{\mathrm{corr}}^{NN}(r;\Gamma_{N_1},\Gamma_{N_2},\sigma_{\mathrm{eff}},\rho_{\text{NS}},\chi_{\text{sea}})|$ and require $\lim_{R\to\infty}\mathcal T_{NN}(R)=0$ in the same $\sigma_{\mathrm{eff}}$, color-singlet, branch-interface, and Noether sea response record. The dedicated [delta-E-corr-NN-tail-limit-source-acquisition-blocker.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-limit-source-acquisition-blocker.v1.json) packet now records the exact accepted-row request: `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail` must be supplied by durable non-fixture evidence before `Delta_E_corr_NN_tail_limit` can be accepted. Its conditional lemma is the first proof route: if $|\Delta E_{\mathrm{corr}}^{NN}(r)|\le C e^{-\lambda(r-R_0)}$ and $w_{\mathrm{corr}}(r)\le C_w e^{\eta(r-R_0)}$ with $0\le\eta<\lambda$ in the same retained record, then $\mathcal T_{NN}(R)\le C e^{-\lambda(R-R_0)}$ and $\mathcal O_{NN}\le C C_w/(\lambda-\eta)$, so the zero-limit and finite-overlap rows follow. This is a priority-only proof scaffold; it still waits on accepted $\sigma_{\mathrm{eff}}$, accepted color-singlet envelope, and same-record domain rows, and it feeds `finite_range_residual`, `same_record_no_open_color_audit`, `no_open_color_far_field`, and `no_free_color_asymptotic_state` after acceptance rather than assuming those rows first.

The tail-limit packet now has a concrete uniform exponential-tail certificate request. The required source object must carry one retained event id, $\Delta E_{\mathrm{corr}}^{NN}$, $\mathcal T_{NN}(R)$, $R_0$, $C$, $\lambda$, corridor weight $w_{\mathrm{corr}}$, $C_w$, $\eta$, an $\eta<\lambda$ witness, accepted branch-interface ledger refs, same-record Noether sea response, and coefficient exclusion. If accepted, that object would supply only `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail`; it still leaves `Delta_E_corr_NN_tail_limit` unaccepted until accepted $\sigma_{\mathrm{eff}}$ extraction and accepted color-singlet nucleon envelope rows join the same record.

The tail certificate now has accepted retained support for two non-promotional rows in [delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json). The corridor-weight row fixes $w_{\mathrm{corr}}(r)=1$, $C_w=1$, and $\eta=0$, so any future accepted exponential tail with $\lambda>0$ satisfies $0\le\eta<\lambda$. The coefficient-exclusion row audits $R_0$, $C$, $\lambda$, $C_w$, $\eta$, $\mathcal T_{NN}$, $\mathcal O_{NN}$, and $w_{\mathrm{corr}}$ against A-, Z-, isotope-, element-, and Fe/Ni-indexed tuning. This removes `corridor_weight_growth_eta_lt_lambda` and `coefficient_exclusion_audit` from the tail certificate's missing list, but `uniform_large_r_bound_C_lambda_R0`, accepted $\sigma_{\mathrm{eff}}$, accepted color-singlet envelope, `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, and no-open-color remain blocked.
...
The tail-calculus step now has accepted retained evidence in [delta-E-corr-NN-tail-calculus-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-calculus-retained-evidence.v1.json). It combines the accepted uniform exponential bound with the accepted corridor-weight row $w_{\mathrm{corr}}(r)=1$, $C_w=1$, and $\eta=0$ to accept only `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail`: $\mathcal T_{NN}(R)\le e^{-(R-1)}$ gives the zero limit, and $\mathcal O_{NN}\le 1$ gives the finite-overlap support row in the normalized retained tail-support domain. This still does not accept `Delta_E_corr_NN_tail_limit`; the remaining blocker is the same-record join to accepted $\sigma_{\mathrm{eff}}$ extraction, accepted color-singlet nucleon envelope rows, branch-interface ledgers, and same-record Noether sea response.

The finite-range residual packet now also records `tail_limit_overlap_zero_rows_imply_finite_range_support_0001`, a priority-only bridge from accepted tail-limit row names to the finite-range support row names. If the same retained record carries accepted `lim_R_to_infty_T_NN_R_eq_0`, accepted `O_NN_finite`, and accepted `exists_R0_C_lambda_exp_decay_tail`, the bridge would supply `large_r_zero_limit` and `bounded_residual_overlap` for the finite-range audit. It still does not accept `finite_range_residual`: accepted $\sigma_{\mathrm{eff}}$, accepted color-singlet envelope, branch-interface ledgers, same-record Noether sea response, and coefficient exclusion remain required before the finite-range row can feed no-open-color.
