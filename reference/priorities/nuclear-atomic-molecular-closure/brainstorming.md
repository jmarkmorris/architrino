...
The finite-range residual row is now accepted over the accepted `Delta_E_corr_NN_tail_limit` and local nucleon-envelope support domain; it is not no-free-color, not no-open-color, and not a promotion row. The accepted local rows now cover `proton_color_singlet_closure`, `neutron_color_singlet_closure`, `finite_envelope_boundary`, `same_sigma_eff_domain`, `asymptotic_field_audit`, `local_color_singlet_closure_support`, `local_color_singlet_nucleon_envelope_support`, accepted $\sigma_{\mathrm{eff}}$ extraction, accepted branch-interface ledgers, same-record $E,\mathbf p,\mathbf J$ accounting, same-record Noether sea response, `Delta_E_corr_NN_tail_limit`, `bounded_residual_overlap`, `large_r_zero_limit`, and `finite_range_residual`. They do not accept `accepted_color_singlet_nucleon_envelope`, `no_free_color_asymptotic_state`, `finite_residual_corridor_overlap`, `accepted_delta_E_corr_NN`, `same_record_no_open_color_audit`, or `no_open_color_far_field`. The next priority-only proof-order target is a same-domain `no_free_color_asymptotic_state` retained evidence object over the already accepted `local_color_singlet_closure_support`, `asymptotic_field_audit`, and `finite_range_residual` rows; `K_open_finite`, `N_open_R_le_K_open_T_NN_R_squared`, `same_event_ledger`, `same_record_no_open_color_audit`, and `no_open_color_far_field` stay downstream and unaccepted.
...
The neutron-envelope route now has matching accepted retained support for the narrow `neutron_color_singlet_closure` row in [neutron-color-singlet-envelope-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/neutron-color-singlet-envelope-support-retained-evidence.v1.json). It accepts only the normalized local neutron projection ratio $E_{\mathrm{color},nX}=0\le\Delta_{\mathrm{color},nX}=0$ as `neutron_color_singlet_closure`; the already accepted `finite_envelope_boundary` remains supplied by the proton-envelope support packet. This does not accept `accepted_neutron_color_singlet_envelope`, `accepted_proton_color_singlet_envelope`, `no_free_color_asymptotic_state`, `color_singlet_closure`, `same_record_branch_interface`, `finite_range_residual`, `same_record_no_open_color_audit`, or `no_open_color_far_field`. The downstream no-open-color path still waits on finite residual, top color-singlet closure, same-record no-open audit, and the top proton/neutron envelope plus no-free-color chain that feeds color-singlet closure.

The no-free-color asymptotic-state blocker now has accepted retained support for the intermediate `asymptotic_field_audit` row in [asymptotic-field-audit-retained-evidence.v1.json](../../../scripts/nuclear-atomic/asymptotic-field-audit-retained-evidence.v1.json). The accepted support keeps local proton and neutron color-singlet closure support, finite $R_p$ and $R_n$, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, the same-record branch-interface/confinement-functional support audit, and the two open-color projection limits $\lim_{R\to\infty}\sup_X E_{\mathrm{color},pX}(R)=0$ and $\lim_{R\to\infty}\sup_X E_{\mathrm{color},nX}(R)=0$ in one retained support domain. This is still only an intermediate support row: it does not accept `no_free_color_asymptotic_state`, the top proton/neutron envelope rows, `color_singlet_closure`, `finite_range_residual`, same-record branch interface, or no-open-color.

The local proton/neutron color-singlet support join now has accepted retained evidence in [local-color-singlet-closure-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/local-color-singlet-closure-support-retained-evidence.v1.json). It accepts only `local_color_singlet_closure_support` by joining the accepted local proton and neutron color-singlet closure support rows, finite envelope boundary, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, accepted `asymptotic_field_audit`, and the same-record branch-interface/confinement-functional support audit. This removes the circular need for top `color_singlet_closure` before `no_free_color_asymptotic_state`; the no-free row now waits on `finite_range_residual` in the same support domain. It still does not accept top `color_singlet_closure`, accepted proton/neutron envelope rows, `no_free_color_asymptotic_state`, finite residual, same-record branch interface, or no-open-color.
