...
Here $\Theta_X$ is the source-row bundle for the assembly or event, $\mathcal L_{E\mathbf p\mathbf J}$ is the conservation/event ledger, $\mathcal F_X$ is the recovery predicate for the ordinary phenomenon, and $\mathcal N_X$ is the negative-control set that must not advance. The Noether sea and Noether braid question for each item is then concrete: which coefficient, graph rule, event row, or response term in $\Theta_X$ actually consumes $\rho_{\text{NS}}$, $\theta_{\mathrm{sea}}$, $\chi_{\text{sea}}$, branch-interface rows, or retained braid geometry? If no such row is consumed, the item should stay a non-Noether recovery target rather than being linked by analogy.
...
The immediate no-open-color proof scaffold is now the priority-only limit $\lim_{R\to\infty}\mathcal N_{\mathrm{open}}(R)=0$ for a same-record far-field diagnostic built from $\Delta E_{\mathrm{corr}}^{NN}$, the two color-singlet nucleon envelopes, and the $p+n$/$p+p$ branch-interface record. The blocker packet now carries the same bridge as the audit target: $\mathcal N_{\mathrm{open}}(R)\le K_{\mathrm{open}}\mathcal T_{NN}(R)^2$, so $\lim_{R\to\infty}\mathcal T_{NN}(R)=0$ with finite $K_{\mathrm{open}}$ becomes the analytic route from finite residual tail to no-open-color far field. It remains non-promotional until `finite_range_residual`, `color_singlet_closure`, `same_record_no_open_color_audit`, the accepted proton and neutron branch-interface ledgers, and the same-record conservation ledger are accepted in the same retained record; top `accepted_branch_interface_rows` stay downstream of no-open-color rather than prerequisites for it.

The same-record audit scaffold packages that condition as $\mathcal A_{\mathrm{no\ open}}=(L_{E\mathbf p\mathbf J},B_{pn},B_{pp},\Gamma_{N_1},\Gamma_{N_2},\Delta E_{\mathrm{corr}}^{NN},\mathcal N_{\mathrm{open}})$. Its sufficient-condition target is `finite_range_residual` plus `color_singlet_closure` plus $\lim_{R\to\infty}\mathcal N_{\mathrm{open}}(R)=0$ implying `no_open_color_far_field` in the same event record; `finite_tail_saturation_check` and `bounded_degree_surface_depleted_corridor_estimator` stay locked until that audit is accepted. The audit packet now records the conditional lemma `finite_tail_color_singlet_event_ledger_implies_same_record_no_open_color_audit_0001`: accepted `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, `color_singlet_closure`, same-event ledger, branch-interface ledgers, and finite $K_{\mathrm{open}}$ in one retained event record would supply the same-record audit row. The downstream proof bridge is the priority-only bound $\mathcal N_{\mathrm{open}}(R)\le K_{\mathrm{open}}\mathcal T_{NN}(R)^2$: if $\lim_{R\to\infty}\mathcal T_{NN}(R)=0$ and $K_{\mathrm{open}}$ is finite in the same $\mathcal B_{\mathrm{br}}$ record, the open-color far-field norm closes without retuning Fe/Ni coefficients. The no-open-color blocker records this downstream step as `finite_tail_same_record_audit_implies_no_open_color_far_field_0001`; it would derive `lim_R_to_infty_N_open_R_eq_0` and `no_open_color_far_field` only after the accepted finite residual, color-singlet closure, same-record audit, tail-limit, bounded-overlap, large-$R$ zero-limit, and finite-$K_{\mathrm{open}}$ rows exist in the same retained record.

The same-event ledger subrequest is now explicit as `branch_noether_residual_color_projection_event_lock_implies_same_event_ledger_0001`. The accepted branch ledgers, same-record $E,\mathbf p,\mathbf J$ ledger, and same-record Noether sea response are already available inputs, but they do not by themselves accept the no-open audit's `same_event_ledger`. The missing request is one retained event id that also binds `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, `color_singlet_closure`, `bounded_open_color_projection_operator_norm`, and `finite_open_color_surface_measure`; otherwise the audit would splice a branch ledger, residual tail, color-singlet closure, and projection norm from different records.

The finite-$K_{\mathrm{open}}$ substep is now isolated as `same_record_bounded_open_projection_implies_finite_K_open_0001`. Its proof route is priority-only: bound the open-color projection $\Pi_{\mathrm{open}}$ over the same residual corridor by $\|\Pi_{\mathrm{open}}\delta\mathcal C\|\le\kappa_{\mathrm{open}}|\Delta E_{\mathrm{corr}}^{NN}|$, integrate over the same finite open-color surface measure, and set $K_{\mathrm{open}}=\Omega_{\mathrm{open}}\kappa_{\mathrm{open}}^2$. The normalized `bounded_residual_overlap` support alias is now accepted, but finite-$K_{\mathrm{open}}$ still requires that alias to be joined with `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, `color_singlet_closure`, and the same-event ledger in one retained record; it cannot use A-, Z-, isotope-, element-, or Fe/Ni-indexed projection coefficients.

The bounded projection and surface-measure subrequest is now explicit as `bounded_open_projection_surface_measure_implies_projection_support_0001`. Its acceptance target is narrower than finite-$K_{\mathrm{open}}$: one retained event must define $\Pi_{\mathrm{open}}$, $\kappa_{\mathrm{open}}$, $\Omega_{\mathrm{open}}$, $\mathcal N_{\mathrm{open}}(R)$, and $\mathcal T_{NN}(R)$ on the same projection domain as the branch-interface and residual-tail record. If accepted, this would supply only `bounded_open_color_projection_operator_norm` and `finite_open_color_surface_measure`; it would feed the same-event ledger and finite-$K_{\mathrm{open}}$ route, but it would not itself prove `same_record_no_open_color_audit` or `no_open_color_far_field`.

The projection-support subrequest now has accepted retained evidence in [open-color-projection-surface-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/open-color-projection-surface-support-retained-evidence.v1.json). The accepted rows are only `bounded_open_color_projection_operator_norm` and `finite_open_color_surface_measure`: $\Pi_{\mathrm{open}}$ is treated as a bounded idempotent projection on the finite retained audit domain with $\kappa_{\mathrm{open}}=1$, and the normalized compact audit surface gives $\Omega_{\mathrm{open}}=1$. This removes those two support rows from the same-event ledger's missing list, but it still does not accept `K_open_finite`, `same_event_ledger`, `same_record_no_open_color_audit`, `no_open_color_far_field`, or any branch-interface promotion row.

The same-record branch-interface/confinement-functional support audit now has accepted retained evidence in [same-record-branch-interface-confinement-functional-audit-retained-evidence.v1.json](../../../scripts/nuclear-atomic/same-record-branch-interface-confinement-functional-audit-retained-evidence.v1.json). It accepts only `same_record_branch_interface_confinement_functional_audit` by joining the accepted proton and neutron branch ledgers, same-record $E,\mathbf p,\mathbf J$ ledger, accepted $\sigma_{\mathrm{eff}}$ extraction, same-record $\sigma_{\mathrm{eff}}$/color-singlet/branch-interface/Noether-sea tail support join, bounded open-color projection norm, finite open-color surface measure, and coefficient-exclusion audit into one support domain. This removes that support-domain join from the no-open audit and no-open-color far-field missing lists, but it still does not accept `same_event_ledger`, `K_open_finite`, `N_open_R_le_K_open_T_NN_R_squared`, `same_record_no_open_color_audit`, or `no_open_color_far_field`.

The no-free-color asymptotic-state row is now accepted over the accepted `local_color_singlet_closure_support`, `asymptotic_field_audit`, and `finite_range_residual` domain; it is not top proton-envelope evidence, not top neutron-envelope evidence, not top color-singlet closure, not same-record branch-interface, not no-open-color, and not a promotion row. The accepted local rows now cover `proton_color_singlet_closure`, `neutron_color_singlet_closure`, `finite_envelope_boundary`, `same_sigma_eff_domain`, `asymptotic_field_audit`, `local_color_singlet_closure_support`, `local_color_singlet_nucleon_envelope_support`, accepted $\sigma_{\mathrm{eff}}$ extraction, accepted branch-interface ledgers, same-record $E,\mathbf p,\mathbf J$ accounting, same-record Noether sea response, `Delta_E_corr_NN_tail_limit`, `bounded_residual_overlap`, `large_r_zero_limit`, `finite_range_residual`, and `no_free_color_asymptotic_state`. They do not accept `accepted_proton_color_singlet_envelope`, `accepted_neutron_color_singlet_envelope`, `accepted_color_singlet_nucleon_envelope`, `color_singlet_closure`, `same_record_branch_interface`, `finite_residual_corridor_overlap`, `accepted_delta_E_corr_NN`, `same_record_no_open_color_audit`, or `no_open_color_far_field`. The next priority-only proof-order target is a same-record proton/neutron envelope certificate that consumes accepted no-free-color without treating it as envelope, color-singlet closure, branch-interface, or no-open-color evidence; `K_open_finite`, `N_open_R_le_K_open_T_NN_R_squared`, `same_event_ledger`, `same_record_no_open_color_audit`, and `no_open_color_far_field` stay downstream and unaccepted.

The finite-range residual packet now makes the prior tail step explicit: define $\mathcal T_{NN}(R)=\sup_{r\ge R}|\Delta E_{\mathrm{corr}}^{NN}(r;\Gamma_{N_1},\Gamma_{N_2},\sigma_{\mathrm{eff}},\rho_{\text{NS}},\chi_{\text{sea}})|$ and require $\lim_{R\to\infty}\mathcal T_{NN}(R)=0$ in the same $\sigma_{\mathrm{eff}}$, local nucleon-envelope support, branch-interface, and Noether sea response record. The dedicated [delta-E-corr-NN-tail-limit-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-limit-retained-evidence.v1.json) object accepts only `Delta_E_corr_NN_tail_limit` by joining the accepted local support row and accepted tail-calculus rows in that domain. The separate [finite-range-residual-retained-evidence.v1.json](../../../scripts/nuclear-atomic/finite-range-residual-retained-evidence.v1.json) object now accepts only `finite_range_residual` by joining that accepted tail-limit row with `bounded_residual_overlap` and `large_r_zero_limit` in the same retained support domain. This finite residual can feed `no_free_color_asymptotic_state`, `finite_residual_corridor_overlap`, `same_record_no_open_color_audit`, and `no_open_color_far_field`, but it does not accept any of those downstream rows.

The tail-limit packet has a concrete uniform exponential-tail certificate. The retained source object carries one retained event id, $\Delta E_{\mathrm{corr}}^{NN}$, $\mathcal T_{NN}(R)$, $R_0=1$, $C=1$, $\lambda=1$, corridor weight $w_{\mathrm{corr}}$, $C_w=1$, $\eta=0$, an $\eta<\lambda$ witness, accepted $\sigma_{\mathrm{eff}}$ extraction, the local nucleon-envelope support ref, accepted branch-interface ledger refs, same-record Noether sea response, and coefficient exclusion. That object accepts only the top `Delta_E_corr_NN_tail_limit` row; it does not accept `finite_range_residual`, `accepted_delta_E_corr_NN`, `same_record_no_open_color_audit`, or no-open-color rows.

The tail certificate has accepted retained support for two non-promotional rows in [delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json). The corridor-weight row fixes $w_{\mathrm{corr}}(r)=1$, $C_w=1$, and $\eta=0$, so the accepted exponential tail with $\lambda=1$ satisfies $0\le\eta<\lambda$. The coefficient-exclusion row audits $R_0$, $C$, $\lambda$, $C_w$, $\eta$, $\mathcal T_{NN}$, $\mathcal O_{NN}$, and $w_{\mathrm{corr}}$ against A-, Z-, isotope-, element-, and Fe/Ni-indexed tuning. The corridor-weight, coefficient-exclusion, uniform large-$R$ bound, tail-calculus rows, accepted $\sigma_{\mathrm{eff}}$ extraction, local nucleon-envelope support row, and accepted `Delta_E_corr_NN_tail_limit` are now available support inputs, but `finite_range_residual` and no-open-color remain blocked until finite-range residual evidence exists.

The same tail route also has accepted retained support for the narrow `uniform_large_r_bound_C_lambda_R0` row in [delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json). The support row supplies $R_0=1$, $C=1$, and $\lambda=1$ for the normalized retained tail-support envelope and preserves the coefficient-exclusion audit, but it does not by itself accept `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, `same_record_no_open_color_audit`, or `no_open_color_far_field`.

The tail-calculus step now has accepted retained evidence in [delta-E-corr-NN-tail-calculus-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-calculus-retained-evidence.v1.json). It combines the accepted uniform exponential bound with the accepted corridor-weight row $w_{\mathrm{corr}}(r)=1$, $C_w=1$, and $\eta=0$ to accept only `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail`: $\mathcal T_{NN}(R)\le e^{-(R-1)}$ gives the zero limit, and $\mathcal O_{NN}\le 1$ gives the finite-overlap support row in the normalized retained tail-support domain. The separate tail-limit retained evidence now consumes those support rows to accept `Delta_E_corr_NN_tail_limit`; the tail-calculus support file itself still does not accept finite-range residual or no-open-color rows.

The same-record support join now has accepted retained evidence in [same-record-sigma-eff-color-singlet-branch-interface-noether-sea-response-retained-evidence.v1.json](../../../scripts/nuclear-atomic/same-record-sigma-eff-color-singlet-branch-interface-noether-sea-response-retained-evidence.v1.json). It accepts only `same_record_sigma_eff_color_singlet_branch_interface_noether_sea_response` by tying accepted $\sigma_{\mathrm{eff}}$ extraction, local proton/neutron color-singlet closure support, finite envelope boundary, accepted branch-interface ledgers, same-record $E,\mathbf p,\mathbf J$ ledger, same-record Noether sea response, and normalized tail-calculus rows to one support domain. That join feeds the accepted `Delta_E_corr_NN_tail_limit` evidence, but it still does not accept `accepted_color_singlet_nucleon_envelope`, `finite_range_residual`, `same_record_no_open_color_audit`, or `no_open_color_far_field`.

The local nucleon-envelope support row now has accepted retained evidence in [local-color-singlet-nucleon-envelope-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/local-color-singlet-nucleon-envelope-support-retained-evidence.v1.json). It accepts only `local_color_singlet_nucleon_envelope_support` by joining local color-singlet closure support, proton/neutron local closure rows, finite envelope boundary, same $\sigma_{\mathrm{eff}}$ domain, accepted $\sigma_{\mathrm{eff}}$ extraction, accepted branch-interface ledgers, same-record $E,\mathbf p,\mathbf J$ accounting, same-record Noether sea response, and the branch-interface/confinement support audit. It feeds the accepted `Delta_E_corr_NN_tail_limit` row, but it does not accept the top `accepted_color_singlet_nucleon_envelope`, `finite_range_residual`, or no-open-color rows.

The finite-range residual packet has accepted retained support for the two finite-tail aliases in [finite-range-tail-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/finite-range-tail-support-retained-evidence.v1.json). The evidence consumes the accepted normalized tail-calculus rows `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail` to accept only `large_r_zero_limit` and `bounded_residual_overlap`. With `Delta_E_corr_NN_tail_limit` accepted separately, [finite-range-residual-retained-evidence.v1.json](../../../scripts/nuclear-atomic/finite-range-residual-retained-evidence.v1.json) now binds accepted $\sigma_{\mathrm{eff}}$, local nucleon-envelope support, branch-interface ledgers, same-record Noether sea response, tail-limit, bounded-overlap, and large-$R$ zero-limit rows to accept only `finite_range_residual`. The next same-domain row is `no_free_color_asymptotic_state`; no-open-color still needs color-singlet closure, same-record no-open audit, and the branch-interface conservation rows.
...
| Deuteron bound while ordinary diproton is unbound. | $p+n$ and $p+p$ branch-interface rows differ through orientation, Coulomb cost, and corridor mismatch rather than an arbitrary nuclear-force label. | Promote only after the branch-interface extraction supplies accepted $p+n$/$p+p$ source rows and the toy graph keeps `deuteron_unbound` and `diproton_overbound` cases not advanced. |
...
The first proton-envelope source request is now explicit as `proton_projection_boundary_no_free_color_implies_envelope_support_0001`. A future accepted proton color-singlet envelope certificate must keep the accepted proton branch-interface ledger, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, same-record branch-interface, $\Pi_{\mathrm{singlet},X}$, $\Pi_{\mathrm{open},X}$, $W_{\mathrm{locked},pX}$, $\epsilon_{pX}$, $\Delta_{\mathrm{color},pX}$, $E_{\mathrm{color},pX}\le\Delta_{\mathrm{color},pX}$, finite $R_p$, refinement window, and accepted no-free-color asymptotic-state row in one retained event record. If accepted, this supplies only `proton_color_singlet_closure` and `finite_envelope_boundary`; `accepted_proton_color_singlet_envelope` remains blocked until the accepted no-free-color audit and same-domain requirements exist.

The proton-envelope route now has accepted retained support for those two narrow rows in [proton-color-singlet-envelope-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/proton-color-singlet-envelope-support-retained-evidence.v1.json). It accepts `proton_color_singlet_closure` through a normalized local projection ratio $E_{\mathrm{color},pX}=0\le\Delta_{\mathrm{color},pX}=0$ and accepts `finite_envelope_boundary` through finite normalized support radii $R_p=1$ and $R_n=1$. This does not accept `accepted_proton_color_singlet_envelope`, `accepted_neutron_color_singlet_envelope`, `color_singlet_closure`, `same_record_branch_interface`, `finite_residual_corridor_overlap`, or `no_open_color_far_field`; the first confinement acquisition blocker remains the top proton envelope row because the accepted no-free-color audit still must be joined with same-record branch-interface and envelope-certificate evidence.

The neutron-envelope route now has matching accepted retained support for the narrow `neutron_color_singlet_closure` row in [neutron-color-singlet-envelope-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/neutron-color-singlet-envelope-support-retained-evidence.v1.json). It accepts only the normalized local neutron projection ratio $E_{\mathrm{color},nX}=0\le\Delta_{\mathrm{color},nX}=0$ as `neutron_color_singlet_closure`; the already accepted `finite_envelope_boundary` remains supplied by the proton-envelope support packet, and the no-free-color audit is now accepted separately. This does not accept `accepted_neutron_color_singlet_envelope`, `accepted_proton_color_singlet_envelope`, `color_singlet_closure`, `same_record_branch_interface`, `same_record_no_open_color_audit`, or `no_open_color_far_field`. The downstream no-open-color path still waits on top color-singlet closure, same-record no-open audit, and the top proton/neutron envelope plus same-record branch-interface chain that feeds color-singlet closure.

The no-free-color asymptotic-state route now has accepted retained evidence in [no-free-color-asymptotic-state-retained-evidence.v1.json](../../../scripts/nuclear-atomic/no-free-color-asymptotic-state-retained-evidence.v1.json). It consumes accepted local color-singlet closure support, accepted `asymptotic_field_audit`, and accepted `finite_range_residual` in one confinement-functional support domain, plus the proton and neutron open-color projection zero limits, and accepts only `no_free_color_asymptotic_state`. It does not accept the top proton/neutron envelope rows, `color_singlet_closure`, same-record branch interface, same-record no-open audit, or no-open-color.

The local proton/neutron color-singlet support join now has accepted retained evidence in [local-color-singlet-closure-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/local-color-singlet-closure-support-retained-evidence.v1.json). It accepts only `local_color_singlet_closure_support` by joining the accepted local proton and neutron color-singlet closure support rows, finite envelope boundary, same $\sigma_{\mathrm{eff}}$ domain, same-record Noether sea response, accepted `asymptotic_field_audit`, and the same-record branch-interface/confinement-functional support audit. Together with [finite-range-residual-retained-evidence.v1.json](../../../scripts/nuclear-atomic/finite-range-residual-retained-evidence.v1.json), it now feeds [no-free-color-asymptotic-state-retained-evidence.v1.json](../../../scripts/nuclear-atomic/no-free-color-asymptotic-state-retained-evidence.v1.json), which accepts only `no_free_color_asymptotic_state`. Top `color_singlet_closure`, accepted proton/neutron envelope rows, same-record branch interface, and no-open-color remain unaccepted.

The open-color projection zero-limit support rows now have accepted retained evidence in [open-color-asymptotic-projection-limits-retained-evidence.v1.json](../../../scripts/nuclear-atomic/open-color-asymptotic-projection-limits-retained-evidence.v1.json). The support object accepts only `lim_R_to_infty_sup_X_E_color_pX_R_eq_0` and `lim_R_to_infty_sup_X_E_color_nX_R_eq_0` from the accepted local proton/neutron color-singlet closure rows, finite envelope boundary, and same $\sigma_{\mathrm{eff}}$ domain. Those two limit rows now feed the accepted local `asymptotic_field_audit` support object, but they still do not accept `no_free_color_asymptotic_state`, the top proton/neutron envelope rows, `finite_range_residual`, `same_record_branch_interface`, or `no_open_color_far_field`.

The accepted-$\sigma_{\mathrm{eff}}$ route now has retained evidence in [sigma-eff-extraction-retained-evidence.v1.json](../../../scripts/nuclear-atomic/sigma-eff-extraction-retained-evidence.v1.json). The evidence supplies a normalized same-domain certificate for $\sigma_{\mathrm{eff}}(Q;\rho_{\text{NS}},\chi_{\text{sea}})=\inf_{a,f}\mathcal S_{\sigma}[a,f;Q,\rho_{\text{NS}},\chi_{\text{sea}}]$, with accepted $K_{\perp}$, $V_{\mathrm{exc}}$, $\rho_{\text{NS}}$, $\chi_{\text{sea}}$, axis-exceptionality, and same-record Noether sea response inputs, minimizer data, zero stationarity residual, and zero refinement residual in the retained support domain. This accepts only `sigma_eff_extraction` and `accepted_sigma_eff_extraction`; it does not accept `color_singlet_nucleon_envelope`, `accepted_color_singlet_nucleon_envelope`, `delta_E_corr_NN`, `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, `same_record_no_open_color_audit`, `no_open_color_far_field`, or any Fe/Ni promotion row.

The same $\sigma_{\mathrm{eff}}$ domain lock now has accepted retained support in [same-sigma-eff-domain-retained-evidence.v1.json](../../../scripts/nuclear-atomic/same-sigma-eff-domain-retained-evidence.v1.json). It accepts only `same_sigma_eff_domain` by tying the accepted axis-exceptionality charge, accepted $\sigma_{\mathrm{eff}}$ extraction, finite envelope-boundary support, same-record Noether sea response input, and source-path extraction row to the same normalized confinement-functional domain. This removes `same_sigma_eff_domain` from the missing lists inside the proton-envelope, neutron-envelope, and no-free-color audit blocker packets, but it does not accept `accepted_proton_color_singlet_envelope`, `accepted_neutron_color_singlet_envelope`, `no_free_color_asymptotic_state`, `finite_range_residual`, `same_record_branch_interface`, or `no_open_color_far_field`.

Weak-channel success marker. [weak-channel-source-target-check.mjs](../../../scripts/nuclear-atomic/weak-channel-source-target-check.mjs) verifies that the current muon weak-channel source accepts `weak_visible_branch_ledger`, `weak_projection`, `weak_quotient`, and `weak_exposure_record`, keeps the weak rows in one retained domain and branch record, preserves zero weak residuals, and ties the Fe/Ni beta-stability, asymmetry, and sea-imbalance toy rows to weak source rows. It does not advance `--require-accepted` until `va_chirality_gate`, `reaction_event_ledger`, the weak-channel `noether_sea_response` update row, and the remaining downstream weak rows become accepted.

Weak source-acquisition marker. The weak-channel checker now also validates `sourceAcquisitionTargets.requiredLedgerComponents`. The current source-acquisition report accepts the retained muon `weak_visible_branch_ledger`, `weak_projection`, `weak_quotient`, and `weak_exposure_record` targets, but its first downstream acquisition blocker remains `missing_accepted_va_chirality_gate`. The blocker packet recording a Verification incomplete outcome [va-chirality-gate-source-acquisition-blocker.v1.json](../../../scripts/equation-mapping/va-chirality-gate-source-acquisition-blocker.v1.json) records why the retained weak quotient, weak exposure row, and priority-only `V-A` benchmark table are not enough: the missing evidence is an accepted same-domain `V-A` chirality gate over `D_weak_visible_attempt_0001` / `A_weak_attempt_0001` with left-channel charged-current selection, right-channel charged-current suppression, and Michel-parameter binding. The toy-bound downstream blocker packets [weak-reaction-event-ledger-source-acquisition-blocker.v1.json](../../../scripts/equation-mapping/weak-reaction-event-ledger-source-acquisition-blocker.v1.json) and [weak-noether-sea-response-source-acquisition-blocker.v1.json](../../../scripts/equation-mapping/weak-noether-sea-response-source-acquisition-blocker.v1.json) separately keep `alphaAsym`, `betaValleySlope`, `beta_stable_band_center`, and `seaImbalancePenalty` from promoting on retained quotient/exposure rows or the accepted retained-window Noether sea provider alone. The weak target now carries `acceptedSourceRowProofTargets.va_chirality_gate`, `acceptedSourceRowProofTargets.reaction_event_ledger`, and `acceptedSourceRowProofTargets.noether_sea_response`: the V-A proof target must retain the weak branch ledger, projection, quotient, exposure row, chirality row, same-domain rows, charged-current left-channel selection, right-channel charged-current suppression, and Michel-parameter binding; the event ledger must retain same-domain weak branch, projection, quotient, exposure, accepted $V-A$ gate, energy, momentum, angular momentum, emitted-product, recoil accounting, and one same-record event balance $L_{\mathrm{weak}}^{\mathrm{in}}+L_{\mathrm{sea}}^{\mathrm{in}}=L_{\mathrm{emitted}}+L_{\mathrm{recoil}}+L_{\mathrm{heat/radiation}}+\Delta L_{\mathrm{sea}}^{\mathrm{weak}}$; and the weak-event Noether sea row must retain the reaction event ledger, same-domain Noether sea update row, and the distinction from the retained-window Noether sea response provider. This makes the next mathematical target the chirality gate itself, followed by the same-domain weak reaction event ledger and weak-event Noether sea update, not another Fe/Ni beta-valley coefficient adjustment.
...
| `confinement_functional` | Corridor scale, surface loss, large-$A$ packing behavior, shell/readout envelope, and finite saturation. | `missing_accepted_color_singlet_nucleon_envelope` |
...
Condition required for advancements:
...
- triage [dormant-deferred/electron-orbitals](../dormant-deferred/electron-orbitals/priorities.md) into a priority packet that separates standard orbital notation from $\mathbb{A}\mathbb{A}\mathbb{A}$ localized-assembly interpretation;
