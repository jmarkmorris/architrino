...
The finite-range residual packet now has accepted retained support for the two finite-tail aliases in [finite-range-tail-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/finite-range-tail-support-retained-evidence.v1.json). The evidence consumes the accepted normalized tail-calculus rows `lim_R_to_infty_T_NN_R_eq_0`, `O_NN_finite`, and `exists_R0_C_lambda_exp_decay_tail` to accept only `large_r_zero_limit` and `bounded_residual_overlap`. It still does not accept `Delta_E_corr_NN_tail_limit` or `finite_range_residual`: accepted $\sigma_{\mathrm{eff}}$, accepted color-singlet envelope, branch-interface ledgers, same-record Noether sea response, and the same-domain tail-limit join remain required before the finite-range row can feed no-open-color.
