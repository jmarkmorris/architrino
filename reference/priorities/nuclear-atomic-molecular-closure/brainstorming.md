...
The same tail route now also has accepted retained support for the narrow `uniform_large_r_bound_C_lambda_R0` row in [delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json). The support row supplies $R_0=1$, $C=1$, and $\lambda=1$ for the normalized retained tail-support envelope and preserves the coefficient-exclusion audit, but it does not accept $\lim_{R\to\infty}\mathcal T_{NN}(R)=0$, $\mathcal O_{NN}$ finite, `exists_R0_C_lambda_exp_decay_tail`, `Delta_E_corr_NN_tail_limit`, `finite_range_residual`, `same_record_no_open_color_audit`, or `no_open_color_far_field`. The remaining tail-limit blocker is now the same-record join to accepted $\sigma_{\mathrm{eff}}$ extraction and accepted color-singlet nucleon envelope rows.
