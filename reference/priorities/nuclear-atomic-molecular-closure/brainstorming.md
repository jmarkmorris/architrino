...
For the current Fe/Ni packet, the first decisive blocker is the branch-interface family. The next useful branch-interface capture is not another fit: it is an accepted proton/neutron branch-interface ledger bundle that keeps $p+n$ and $p+p$ orientation rows, same-record $E$, $\mathbf p$, and $\mathbf J$ accounting, Coulomb separation, and the no-open-color far-field condition in one source-acquisition trail. Only after that bundle exists should the toy row-shape pass be used as evidence for the nuclear-binding cusp route.
...
| Beta-stability valley and neutron excess in heavy nuclei. | Weak-channel provenance plus Noether sea response penalties constrain proton-neutron imbalance. | Require accepted weak exposure, reaction-event, lepton/neutrino, recoil, and Noether sea update rows before using beta-stability terms as promotion evidence. |
...
Weak-channel success marker. [weak-channel-source-target-check.mjs](../../../scripts/nuclear-atomic/weak-channel-source-target-check.mjs) verifies that the current muon weak-channel source accepts `weak_visible_branch_ledger`, `weak_projection`, and `weak_quotient`, keeps the weak rows in one retained domain and branch record, preserves zero weak residuals, and ties the Fe/Ni beta-stability, asymmetry, and sea-imbalance toy rows to weak source rows. It keeps `--require-accepted` fail-closed until `weak_exposure_record`, `reaction_event_ledger`, `noether_sea_response`, and the remaining downstream weak rows become accepted.

Weak source-acquisition marker. The weak-channel checker now also validates `sourceAcquisitionTargets.requiredLedgerComponents`. The current source-acquisition report accepts the retained muon `weak_visible_branch_ledger`, `weak_projection`, and `weak_quotient` targets, but its first downstream acquisition blocker remains `missing_accepted_weak_exposure_record`. This makes the next mathematical target an accepted same-domain weak exposure record over `D_weak_visible_attempt_0001` / `A_weak_attempt_0001`, not another Fe/Ni beta-valley coefficient adjustment.
...
| `weak_channel` | Beta-stable band, asymmetry pressure, beta-decay provenance, and weak-channel consistency with the Noether sea response row. | `missing_accepted_weak_exposure_record` |
