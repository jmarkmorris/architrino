...
That check currently reports `algebraicPass: true` for the $p+n$/$p+p$ orientation extraction and `sourceAcquisitionPass: false` with `sourceAcquisitionFirstMissingObject: missing_no_open_color_far_field`. It keeps `--require-accepted` blocked until `nucleon_branch_interface_ledgers`, `pn_orientation_count`, and `pp_orientation_count` become accepted source rows and the upstream `sourceAcquisitionTargets` for no-open-color carry both the required ledger-component shape and accepted durable non-fixture evidence. The accepted proton, neutron, and same-record source-acquisition rows are partial upstream success markers, not promotion evidence by themselves.
