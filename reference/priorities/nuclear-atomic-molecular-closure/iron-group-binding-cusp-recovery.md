...
The same tail route now has accepted retained support for the corridor-weight and coefficient-exclusion prerequisites in [delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-tail-corridor-weight-retained-evidence.v1.json). It fixes the normalized corridor weight $w_{\mathrm{corr}}(r)=1$ with $C_w=1$ and $\eta=0$, so a future accepted exponential tail with positive $\lambda$ automatically has $0\le\eta<\lambda$. It also accepts the audit that $R_0$, $C$, $\lambda$, $C_w$, $\eta$, $\mathcal T_{NN}$, $\mathcal O_{NN}$, and $w_{\mathrm{corr}}$ are not A-, Z-, isotope-, element-, or Fe/Ni-indexed. This is still not the uniform large-$R$ bound, not `Delta_E_corr_NN_tail_limit`, and not finite-range residual evidence.

The tail route also now has accepted retained support for the uniform large-$R$ bound prerequisite in [delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json](../../../scripts/nuclear-atomic/delta-E-corr-NN-uniform-tail-bound-retained-evidence.v1.json). It supplies the normalized support envelope with $R_0=1$, $C=1$, and $\lambda=1$ and keeps the coefficient-exclusion audit intact. This accepts only `uniform_large_r_bound_C_lambda_R0`: the zero-limit row, finite-overlap row, exponential-tail row name, top `Delta_E_corr_NN_tail_limit`, finite-range residual, same-record no-open-color audit, and no-open-color far-field rows remain blocked until accepted $\sigma_{\mathrm{eff}}$ extraction and accepted color-singlet nucleon envelope rows are joined in the same retained record.

The finite-range residual blocker now has accepted finite-tail support aliases in [finite-range-tail-support-retained-evidence.v1.json](../../../scripts/nuclear-atomic/finite-range-tail-support-retained-evidence.v1.json). The retained evidence consumes accepted `lim_R_to_infty_T_NN_R_eq_0`, accepted `O_NN_finite`, and accepted `exists_R0_C_lambda_exp_decay_tail` in the normalized tail-support domain to supply only `large_r_zero_limit` and `bounded_residual_overlap`; it does not accept `Delta_E_corr_NN_tail_limit` or `finite_range_residual`, because the accepted $\sigma_{\mathrm{eff}}$, color-singlet envelope, branch-interface, same-record Noether sea response, and same-domain tail-limit join still have to hold together.
