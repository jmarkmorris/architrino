...
The four rows are the decimal B1 catalog members B1.1 through B1.4. They retain the parent B1 common axis $(0,0,1)$, radii $(0.22,0.32,0.44)$, frequencies $(0.25,0.25,0.25)$, phases $(0,2\pi/3,4\pi/3)$, and the exact prior endpoint identifiers and paths. Their respective $(h_1,h_2,h_3)$ and $(\rho_1,\rho_2,\rho_3)$ rows are interior reference, high-axial interior, all-equatorial, and all-axial as recorded in the candidate analysis methodology. The compatibility names remain only in stable IDs, filenames, record URLs, endpoint IDs, the compatibility wrapper path, and this migration ledger.
