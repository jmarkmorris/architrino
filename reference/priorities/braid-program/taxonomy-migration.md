...
The catalog also contains the constrained Family-C variants C1.1 and C2.1. Each uses two coaxial B1.3 all-equatorial components separated along their common oriented axis by the positive coordinate $d_C$. C1.1 inherits the C1 same-circulation relation, while C2.1 inherits the C2 opposite-circulation relation. Their representative selects $d_C=1.10$, centers $(0,0,-0.55)$ and $(0,0,0.55)$, common z-axis frames, equal frequencies, and relative phase $\pi/3$. The axial offset $d_C$ remains an independently sampled Monte Carlo coordinate rather than a fixed family constant. These are new source and record identities; they do not replace or invalidate C1, C2, or B1.3.
