...
Current disposition note (2026-07-23): `B1.4` is no longer an active taxonomy or Borg-catalog member. Its all-axial equation remains a B1 coordinate boundary, and its stable specification id, source file, generated record, and historical hashes remain unchanged as a deprecated axial-limit null control. The migration rows below preserve the historical identity assignment and are not an active-candidate inventory.
...
| --- | --- | --- | --- | --- | --- |
| `illustrative-spindle-chart-hypothesis-v0` | `illustrative-spindle-chart-hypothesis.v0.json` | `illustrative-spindle-chart-hypothesis.assembly-view-record.v0.json` | Illustrative spindle chart hypothesis | Illustrative spindle prescribed geometry | `B1.1 — interior reference` |
| `illustrative-extreme-cap-tilt-spindle-variant-v0` | `illustrative-extreme-cap-tilt-spindle-variant.v0.json` | `illustrative-extreme-cap-tilt-spindle-variant.assembly-view-record.v0.json` | Illustrative extreme cap-tilt spindle variant | Spindle variant — extreme cap tilt | `B1.2 — high-axial interior` |
| `illustrative-planar-tri-binary-spindle-boundary-v0` | `illustrative-planar-tri-binary-spindle-boundary.v0.json` | `illustrative-planar-tri-binary-spindle-boundary.assembly-view-record.v0.json` | Illustrative planar tri-binary spindle boundary | Spindle boundary — planar tri-binary | `B1.3 — all-equatorial boundary` |
| `illustrative-full-cap-axial-spindle-boundary-v0` | `illustrative-full-cap-axial-spindle-boundary.v0.json` | `illustrative-full-cap-axial-spindle-boundary.assembly-view-record.v0.json` | Illustrative full-cap axial spindle boundary | Not previously catalogued | `B1.4 — all-axial boundary` |

At assignment time, the four rows were the decimal B1 catalog members B1.1 through B1.4. They retain the parent B1 common axis $(0,0,1)$, radii $(0.22,0.32,0.44)$, frequencies $(0.25,0.25,0.25)$, phases $(0,2\pi/3,4\pi/3)$, and the exact prior endpoint identifiers and paths. Their respective $(h_1,h_2,h_3)$ and $(\rho_1,\rho_2,\rho_3)$ rows are interior reference, high-axial interior, all-equatorial, and all-axial. B1.1 through B1.3 remain active; B1.4 is the preserved deprecated control described above. The compatibility names remain only in stable IDs, filenames, record URLs, endpoint IDs, the compatibility wrapper path, and this migration ledger.

Family C now contains the general coaxial twelve-architrino chart. C1 is its general co-rotating member and C2 is its general counter-rotating member; neither requires decomposition into two B1 components. The prior C1/C2 two-B1 representatives are constrained loci renumbered C3/C4 and made coaxial by placing their centers at $(0,0,-0.55)$ and $(0,0,0.55)$ on the common z axis. The prior C1.1/C2.1 all-equatorial representatives are renumbered C5/C6. C5/C6 retain two coaxial B1.3 components separated by the positive coordinate $d_C$, with C5 co-rotating and C6 counter-rotating. Stable legacy filenames and `specId` values remain compatibility interfaces; taxonomy fields, visible labels, and current analytical routing use C3 through C6.
