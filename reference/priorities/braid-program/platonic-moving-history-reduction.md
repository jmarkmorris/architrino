...
The next exact dynamics task remains a nine-channel rigid stratum, but both the `+++---` difference-edge orbit and the antipodal-alternating sum-edge orbit are now excluded. The next finite high-symmetry representative is a mixed-face axis such as $\hat{\mathbf n}=(\mathbf e_x+\mathbf e_y-\mathbf e_z)/\sqrt3$. A certified zero requires every scalar channel, the thirty-root ledger, the positive $D_{uv}$ floor, and the inferred positive scale $R=\kappa\lambda/\beta^2$ to agree. A positive lower bound over the complete open speed interval would exclude the selected stratum. The eighteen-channel axes, continuous generic-axis charts, and non-rigid histories remain separate programs rather than hidden members of this finite calculation.

Plainly: the two smaller special-axis systems and two full nine-channel systems are now excluded. The next unresolved finite high-symmetry calculation is an antipodal-alternating mixed-face axis; the lattice colorings currently in the app do not supply its balanced octahedral word automatically.

## Closure Boundary

This packet closes eight items: the exact calculation object, the stationary balanced-octahedron chart, the coloured rigid-axis symmetry census, the complete partner-root criterion for every rigid sub-field-speed octahedral history, every rigid vertex-axis octahedral history, the five-channel antipodal-alternating difference-edge-axis stratum, the nine-channel `+++---` difference-edge-axis stratum, and the nine-channel antipodal-alternating sum-edge-axis stratum below field speed. It does not close the mixed-face or continuous generic-axis nine-channel systems, the eighteen-channel edge-, face-, and generic-axis systems, non-rigid octahedral motion, shell-complete dynamics, retained EOM-solver evolution, or stability. Release under the EOM solver is admissible only after one prescribed residual actually vanishes with its root ledger certified. Linear stability is admissible only after that balance or a genuine periodic solution exists.

Plainly: the bookkeeping and reduction are complete, stationary and vertex-axis rigid octahedra are ruled out, and the five-channel difference-edge plus two selected nine-channel systems fail. The next unresolved finite high-symmetry question is whether the antipodal-alternating mixed-face system has a real zero.
