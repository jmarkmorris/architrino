...
This equation stack closes the residual definition and complete-root census for rigid sub-field-speed octahedral histories. It does not establish that any remaining channel system has a zero. The monochromatic-face axis is excluded for every ordinary simple-root rigid speed, every vertex-axis orbit is excluded by the fixed-receiver projections, and the five-channel difference-edge-axis stratum is excluded by the tangential lower bound. The [complete outward-rounded difference-edge certificate](../evidence/2026-09-01-octahedral-nine-channel-edge-axis-exclusion.md) excludes the nine-channel `+++---` stratum with $\hat{\mathbf n}=(\mathbf e_x-\mathbf e_z)/\sqrt2$. The [sum-edge certificate](../evidence/2026-09-01-octahedral-antipodal-sum-edge-exclusion.md) excludes the antipodal-alternating orbit with $\hat{\mathbf n}=(\mathbf e_x+\mathbf e_z)/\sqrt2$. The mixed-face axis, face diagonal, all four exact generic-axis quotient vertices, and [all five closed generic-axis quotient edges](../evidence/2026-09-02-octahedral-antipodal-generic-axis-edge-exclusion.md) are also excluded throughout $0\leq\beta<1$. The two generic-axis simplex interiors and the remaining eighteen-channel systems remain to be solved or bounded.
...
The next exact dynamics task is the interior of each antipodal-alternating generic-axis simplex. The complete edge certificate leaves no boundary survivor: every one of 9,078 outward-rounded edge-speed boxes preserves all thirty directed partner roots and rejects common balance through at least one of the nine full-vector residual channels. A certified interior zero requires every scalar channel, the thirty-root ledger, the positive $D_{uv}$ floor, and the inferred positive scale $R=\kappa\lambda/\beta^2$ to agree. The eighteen-channel axes and non-rigid histories remain separate programs rather than hidden members of this calculation.

Plainly: no balanced antipodal-alternating history occurs on the boundary of the exact generic-axis quotient. Only the two triangular interiors remain in that nine-channel chart, and the lattice colorings currently in the app still do not supply this balanced octahedral word automatically.

## Closure Boundary

This packet closes the exact calculation object, the stationary balanced-octahedron chart, the coloured rigid-axis symmetry census, the complete partner-root criterion for every rigid sub-field-speed octahedral history, every rigid vertex-axis octahedral history, the five-channel antipodal-alternating difference-edge-axis stratum, the nine-channel `+++---` difference-edge-axis stratum, the nine-channel antipodal-alternating sum-edge-axis stratum, the antipodal mixed-face and face-diagonal axes, all four generic-axis quotient vertices, and all five generic-axis quotient edges below field speed. It does not close either generic-axis simplex interior, the eighteen-channel edge-, face-, and generic-axis systems, non-rigid octahedral motion, shell-complete dynamics, retained EOM-solver evolution, or stability. Release under the EOM solver is admissible only after one prescribed residual actually vanishes with its root ledger certified. Linear stability is admissible only after that balance or a genuine periodic solution exists.

Plainly: the bookkeeping and reduction are complete, and the full boundary of the antipodal generic-axis quotient is ruled out. The next unresolved question is whether either exact triangular interior contains a real common zero.
