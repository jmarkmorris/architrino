...
Owner: [Braid Program](../priorities.md). General requirement inference remains with [Equation Mapping](../../mapping-equations/analysis/inferring-braid-requirements.md); this document owns the application to candidate records.

Status: CURRENT EVIDENCE-GRADED MATRIX, reconciled 2026-09-02 with the accepted phase-varying display representative enclosed-root restart, the independently integrated factual-configuration prescribed-history evidence, and the exact stella-octangula catalog-endpoint balance exclusion. This document applies the consolidated requirements inferred in [Inferring Braid Requirements](../../mapping-equations/analysis/inferring-braid-requirements.md) to every admitted or exploratory row in the [Braid Candidate Registry](../configurations/candidate-registry.md) and retains named scoped-negative realizations when their adjudication prevents a rejected row from silently re-entering search. The registry remains authoritative for names, inclusion, concise status, and next action. This matrix owns requirement dispositions and their evidence grades.
...
The canonical velocity domain is unbounded: finite member speeds below, at, and above $c_f$ are admitted. The [EOM evolution contract](../../app-solver/contracts/evolution-contract-v1.md#ordered-pair-and-self-history-rule) explicitly includes all three regimes and states that equality alone is not a singularity. The [field-speed-ceiling compatibility decision](../../field-speed-ceiling/decisions/field-speed-ceiling-compatibility-decision.md#decision-question) concerns a separately versioned alternative with no canonical adoption. This domain statement does not assert that every admitted history is regular, that every instrument supports it, or that any such history is retained.
...
| `axially separated orthogonal-axis three-binary configuration`, measured five-coordinate slice | `P[D]` | `P[M]` | `P[M]` | `P[M]` | `U` | One pair-conjugate five-coordinate slice completed the [guarded release](../evidence/three-binary-five-coordinate-bounded-eom-comparison.md) through $T=0.15$ with certified noncoincidence, complete roots, and speed margin; the pass does not cover the full axially separated configuration chart. |
...
| `centered five-coordinate representative` | `P[D]` | `P[M]` | `P[M]` | `P[M]` | `U` | The metric-matched five-coordinate row completed the [guarded $T=0.15$ release](../evidence/three-binary-five-coordinate-bounded-eom-comparison.md) and preserved its cyclic slice, but did not return. |
...
| `phase-varying display representative` common-cadence circular realization | `P[D]` | `F[D]` | `U` | `U` | `U` | The declared ordinary two-ring circular realization has an [exact incompatibility](../../mapping-equations/analysis/inferring-braid-requirements.md#joint-projection-audit-and-a-two-ring-no-go) among everywhere-noncoincident member geometry, stationary centroid, and instantaneous dipole null; the revised row is materially different and does not erase this scoped negative. |
...
| `scoped-negative circular control` (demoted realization) | `P[D]` | `P[D]` | `P[D]` | `F[M]` | `U` | Exact continuous noncoincidence and complete simple roots pass on the prescribed circular history, but its [measured member-acceleration residual](../../mapping-equations/analysis/inferring-braid-requirements.md#scoped-negative-circular-control-root-ledger-and-member-acceleration-screen) rejects that isolated history as an EOM realization. |
| `octahedron antipodal-alternating sum-edge rigid history` | `P[D]` | `P[D]` | `P[D/M]` | `F[D/M]` | `N/A` | The [complete strict sub-field root ledger and interval certificate](../evidence/2026-09-01-octahedral-antipodal-sum-edge-exclusion.md) prove that the `+-+-+-` word about $(\mathbf e_x+\mathbf e_z)/\sqrt{2}$ requires zero $x$ acceleration at the $+\mathbf e_y$ receiver while the causal contribution remains strictly negative for $0\le\beta<1$. This excludes only that exact rigid word-axis stratum. |
| `static stella-octangula catalog history` | `P[D]` | `P[D]` | `P[D/M]` | `F[D/M]` | `N/A` | The [exact catalog-endpoint root and acceleration packet](../evidence/2026-09-02-stella-octangula-catalog-endpoint-balance.md) certifies all $56$ retained roots at $T=1$ and matches an unchanged independent pair-sum oracle, while every member has nonzero inward acceleration. This excludes only the exact prescribed static $[0,1]$ history. |
| `asymmetric counter-breathing representative` | `P[D]` | `P[M]` | `P[M]` | `P[M]` | `U` | Bounded releases in the [asymmetric counter-breathing representative geometry program](../configurations/f6c-geometry.md) preserve the exact symmetry surface and achieve partial turns, but no root-valid nondegenerate complete return exists. |
...
The accessory rows inherit the unresolved `H5` status of their underlying braid. They also carry six accessory-specific requirements from [Accessory Placement Relative To The Braid](../../mapping-equations/analysis/inferring-braid-requirements.md#accessory-placement-relative-to-the-braid).
...
No admitted or exploratory candidate currently passes `H5`, so this matrix selects no physical braid. The separately governed allocation percentages in the [candidate registry](../configurations/candidate-registry.md) do not alter that conclusion. The immediately discriminating rows are:
