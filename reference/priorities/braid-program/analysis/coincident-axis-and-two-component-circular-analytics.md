# Coincident-Axis Three-Binary And Two-Component Circular Analytics

Status: the coincident-axis three-binary and two-component circular configuration descriptions are implemented. The two-component circular description contains the general coaxial twelve-architrino geometry, with or without a separately declared Accessory Configuration. The bounded prescribed-path pilot measured on 2026-07-22 is historical diagnostic evidence only: it instantiated $c_f=4$, whereas current workspace policy requires $c_f=1$. Its twelve rejected rows characterize those twelve protocol rows and do not close, rank, or negatively adjudicate the declared configuration space.

Claim grade: the coordinate, dimension-extension, taxonomy, and causal-delay statements are derived from the declared exact paths and operator disposition. The numerical rows are measured historical diagnostics from the prescribed-path evaluator. Comparisons below apply only to that bounded legacy sample and cannot support a current coincident-axis three-binary or two-component circular recommendation before a $c_f=1$ rerun.
...
- the coincident-axis three-binary locus remains three neutral binaries about one common midpoint;
- the six-architrino train is a generalized coincident-axis three-binary coordinate chart whose common-midpoint sublocus is the coincident-axis three-binary locus;
- every twelve-architrino geometry in this lane belongs to two-component circular configurations when all twelve defining architrino worldlines share one axis;
- the co-rotating and counter-rotating coincident-center two-component circular configurations are the general circulation charts;
- the co-rotating and counter-rotating coaxial-separated two-component circular configurations each comprise two complete coincident-axis three-binary components on one common axis;
- the co-rotating and counter-rotating coaxial-separated two-planar-braid configurations are the all-equatorial restrictions of those coaxial-separated component pairs; and
- six additional architrino worldlines form an Accessory Configuration only when that inventory and every path are declared explicitly. The Accessory Configuration is not part of the twelve defining two-component circular worldlines and does not change the two-component circular configuration identity.
...
## Coincident-Axis Three-Binary Submanifold And Midpoint-Dimension Extension
...
When the two paths also obey the coincident-axis three-binary polarity-conjugate, antipodal, common-frequency, and common-circulation relations, they may be written
...
The coincident-axis three-binary submanifold is the exact locus

$$
b_1=b_2=b_3=0.
$$

If $b_1=b_2=b_3=b$, the common value is absorbed into the braid center by $\mathbf C(T)\mapsto\mathbf C(T)+b\hat{\mathbf n}_B$, so this also represents the same intrinsic coincident-axis geometry. After quotienting out that common translation, allowing independent binary midpoints adds only two intrinsic continuous coordinates, for example
...
Equivalently, six ordered axial coordinates modulo one common shift contain five intrinsic axial coordinates: the three coincident-axis half-separations $(h_1,h_2,h_3)$ plus two relative-midpoint coordinates $(\beta_1,\beta_2)$. This nearest train generalization is therefore a dimension extension of the coincident-axis three-binary submanifold, not evidence for a new topology.

For the four coaxial-separated two-component circular and two-planar-braid configurations with fixed internal pairings, the same decomposition applies independently to each six-worldline coincident-axis three-binary component. Their relative axial displacement belongs to the two-component circular configuration; relaxing the three common-midpoint equalities inside either component adds two intrinsic midpoint coordinates. An Accessory Configuration is different: adding it changes the declared source inventory and is not merely a dimension extension of the twelve defining worldlines.
...
The counter-rotating coincident-center two-component circular reference uses the ordered index subsets
...
The pilot orders $\mathcal B^{(1)}$ before $\mathcal B^{(2)}$, keeps counterpart pairs internal to each subset, uses equal frequencies, and makes the two subsets counter-rotate with relative phase $\pi$. These subsets declare the counter-rotating circulation relation; they are not assumed to be complete coincident-axis three-binary components. The co-rotating coincident-center configuration, rational-frequency, reflection-conjugate, and deliberately mismatched strata remain declared Monte Carlo coordinates rather than inferred properties of the drawing.
...
The pilot midpoint stratum uses $\epsilon_k=a_k=b_k=0$, six additional radii $\rho_{Y,k}=0.07$, common return frequency, and alternating polarity so the Accessory Configuration is neutral. Each accessory-worldline record declares polarity, offsets, radius, frequency, phase, circulation, ordered-subset association, adjacent slot, and reflection relation. The complete source-inspired assembly contains twelve defining two-component circular architrino worldlines plus six Accessory Configuration worldlines, for eighteen declared worldlines.
...
- geometry class: six-architrino coincident-axis three-binary extension or twelve-architrino two-component circular configurations;
- additional-worldline count: zero or six in the measured two-component circular pilot, with zero or three available for the coincident-axis three-binary scaling controls;
...
Instrument: `evaluatePrescribedRecordAnalysis({ sourceRecord, protocol })` through the historical generalized-coincident-axis three-binary complete-cycle campaign reducer. Grade: measured prescribed-path analytics.
...
| counter-rotating coincident-center two-component circular, central envelope, no accessory | 7.6049 | 7.5035 | 2.7771 | 0.009352 | 0.067036 | fail |
| counter-rotating coincident-center two-component circular, central envelope, six-worldline accessory | 27.0090 | 23.3130 | 21.1213 | 0.007991 | 0.059656 | fail |
| counter-rotating coincident-center two-component circular, equal radius, no accessory | 7.2499 | 5.0437 | 3.2437 | 0.010308 | 0.070041 | fail |
| counter-rotating coincident-center two-component circular, reflection broken | 6.2323 | 8.3596 | 2.6098 | 0.009408 | 0.066972 | fail |
| counter-rotating coincident-center two-component circular, crossed pairing, six-worldline accessory | 27.0090 | 23.3130 | 21.1213 | 0.007991 | 0.059656 | fail |
...
For the exact six-worldline Accessory Configuration minus the no-accessory counter-rotating coincident-center two-component circular pair,
...
- Inferred from measured rows: the six-worldline central-envelope reference has smaller axial, radial, and tangential residual RMS than the matching counter-rotating coincident-center two-component circular central-envelope reference, but worse exterior ratios.
...
Do not use the legacy pilot to accept or reject the common-axis idea or rank its configuration classes. No measured pilot row passed its historical protocol, but that local result is not a negative coincident-axis three-binary or two-component circular finding.

Retain these configuration classes as regression anchors inside a much larger $c_f=1$ search:

1. the six-architrino coincident-axis three-binary central-envelope reference, because its residual projections and its seeded neighbor reproduce under time refinement;
2. the twelve-architrino counter-rotating coincident-center two-component circular equal-radius no-accessory control, because it defeats the central envelope on axial and radial residual RMS and therefore remains a necessary falsifier;
3. the twelve-architrino counter-rotating coincident-center two-component circular central-envelope no-accessory reference, as the matched baseline for future spacing and accessory studies;
4. a co-rotating coincident-center two-component circular central-envelope reference, so common circulation is tested rather than inferred from the counter-rotating rows; and
5. co-rotating and counter-rotating coaxial-separated two-component circular controls together with their all-equatorial two-planar-braid restrictions, so each constrained geometry and circulation relation remains directly tested.
...
The next analytical search should first establish the coincident-axis three-binary submanifold embedding at $c_f=1$, then open the two relative-midpoint dimensions $(\beta_1,\beta_2)$ while retaining the existing coincident-axis coordinates $(h_a,\rho_a,\phi_a,\omega_a)$. Only a predeclared bounded domain with adequate space-filling coverage, held-out samples, co-translating exterior probes, and refinement may close that searched domain negative.
...
- **Action or angular-momentum organization:** reject if an independently authored reducer disagrees, held-out samples erase the pattern, or sampling-measure changes move the apparent organization.

## Promotion Routing

The exact common-axis source and delay equations and the general two-component circular configuration are promoted into the reader-facing taxonomy and analysis methodology. The coincident-axis three-binary submanifold and midpoint-dimension reduction remains priority-only as the precise relation between the six-worldline extension and the common-midpoint locus. The legacy measured campaign remains historical priority evidence and cannot support a current candidate verdict.
