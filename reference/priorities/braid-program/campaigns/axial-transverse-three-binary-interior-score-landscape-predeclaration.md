# Axial-Transverse Coincident-Axis Three-Binary Interior Score-Landscape Predeclaration

Status: coordinator-reviewed priority-only design. The obsolete score-free compatibility packet was withdrawn during the current-only exact-identity migration because it bound superseded family/member metadata rather than current exact configuration identities. It had evaluated zero causal roots and computed zero scores. A new machine manifest, protocol, runner, and independent dense-root audit must be frozen from current exact configurations before any analytical execution, which remains unauthorized. This file does not reopen or modify the sealed 2026-07-25 nine-configuration campaign.
...
The only input result is the write-once [nine-configuration Monte Carlo-to-basin diagnostic](../evidence/2026-07-25-bc-monte-carlo-basin-diagnostic.md). Its predeclared handoff ceiling remains a refined full-cycle maximum pointwise member residual of `6`, with primary/refined relative-or-absolute change at most `0.05`. The old 576 draws, protocol, receipt, database rows, and zero-row handoff ledger remain unchanged.

This packet proposes a new, separately hashed Axial-transverse coincident-axis three-binary interior configuration diagnostic population. It does not authorize another Monte Carlo campaign, a directed descent, or post-score point creation.

Plainly: the old campaign remains closed. This proposal asks how the Axial-transverse coincident-axis three-binary interior configuration score changes around its lowest retained row without changing the old pass line or calling unavailable scores failures.
...
3. bind the canonical Axial-transverse coincident-axis three-binary interior configuration source bytes, the complete materialized center specification, and the center hashes stated below;
...
- planar common-center three-binary constraint sample 11: named surface event-convergence failure.
- planar common-center three-binary constraint sample 43: compact surface minimum-separation failure.
- Coaxial-separated two-planar-braid counter-rotating sample 36: named surface event-convergence failure.

They retain null scores and are unresolved numerical rows, not candidate failures.

## Lowest retained Axial-transverse coincident-axis three-binary interior configuration region

The Axial-transverse coincident-axis three-binary interior configuration applicable population contains 37 scores. Its refined peak distribution has minimum `59.29861867019956`, first quartile `110.33829223921393`, median `140.21549054100748`, third quartile `186.4041371318672`, and maximum `422.39619893906456`.
...
For every applicable row, retain the uncollapsed lexicographic objective

$$
P=\max_{a,s,k}
\left\|
\mathbf A^{\mathrm{prescribed}}_{a,s}(T_k)
-
\mathbf A^{\mathrm{declared\ inventory}}_{a,s}(T_k)
\right\|,
$$
...
## Exact Axial-transverse coincident-axis three-binary interior configuration chart

The anchor is sealed Axial-transverse coincident-axis three-binary interior configuration sample 5:
...
Hold the declared common midpoint, common axis, common frequency, common circulation, source order, return period `4`, spherical envelope `0.5`, common frequency `0.75`, circulation `-1`, and polarity assignments `(+1,-1,-1)` fixed.
...
| Coordinate | Anchor | Declared range |
| --- | ---: | --- |
| $R_1$ | 0.1013708390621178 | [0.09215530823828891, 0.11150792296832959] |
| $R_2$ | 0.12599123487820238 | [0.11453748625291124, 0.13859035836602263] |
| $R_3$ | 0.19797831306779531 | [0.17998028460708662, 0.21777614437457488] |
| $u_1$ | 0.5607114957052897 | [0.4607114957052897, 0.6607114957052897] |
| $u_2$ | 0.19973938313738124 | [0.1, 0.2994787662747625] |
| $u_3$ | 0.7813938908936988 | [0.6813938908936988, 0.8813938908936988] |
| $\theta_1$ | 6.250463126085282 | anchor $\pm\pi/8$, periodic |
| $\theta_2$ | 1.2831301703861584 | anchor $\pm\pi/8$, periodic |
| $\theta_3$ | 3.9301873448540365 | anchor $\pm\pi/8$, periodic |
| $\kappa$ | 0.017682417404789597 | [-0.1323175825952104, 0.1676824174047896] |
...
Plainly: this is a bounded twelve-coordinate Axial-transverse coincident-axis three-binary interior configuration chart around the retained anchor. It fixes every old discrete label so a frequency, circulation, or polarity change cannot masquerade as a local geometric effect.
...
The prior seed strings named the retired member code and therefore do not define a current manifest. A replacement freeze must declare new factual seed strings before it materializes any row; it must preserve the coordinate construction below and may not inspect a score before freezing those strings.
...
Only the first two dispositions carry scores. `Inapplicable`, `unknown`, and `invalid` rows remain null-score rows and do not count as candidate failures. An `applicable-threshold-noncrossing` is only a failure to cross this diagnostic handoff line; it is not a configuration-wide or physical negative.
...
- A lower row would show only that the fixed Axial-transverse coincident-axis three-binary interior configuration chart contains a lower prescribed-path score. It would not establish a basin, stability, retention, binding, energy closure, particle identity, or physical realization.
...
Closure goal: freeze a separately hashed Axial-transverse coincident-axis three-binary interior configuration score-landscape protocol that maps the retained low row without rerunning the old campaign, changing the handoff ceiling, or upgrading prescribed-path diagnostics into physical evidence.
