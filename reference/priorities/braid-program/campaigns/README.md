...
Machine-readable pre-evaluation freezes may also live here when a reviewed
campaign design requires exact row and protocol identities before execution.
The B1.1 local-landscape freeze is
[`b1-1-score-landscape-manifest.v1.json`](b1-1-score-landscape-manifest.v1.json)
plus its
[`complete-cycle protocol`](b1-1-score-landscape-complete-cycle-protocol.v1.json).
These files declare inputs only; their presence does not authorize analytical
execution.
