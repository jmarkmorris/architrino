...
The archived [SQLite benchmark](../evidence/analytical-campaign-database-benchmark.md) supports application-side gzip for V1 as a storage-mechanics result; the next complete generation must revalidate it. Cross-packet compression remains a sizing observation only; V1 preserves one independently addressable packet artifact per distinct content hash.
