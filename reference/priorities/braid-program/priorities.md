...
5. Campaign: undirected ensemble release (operator-directed, 2026-07-16). The undirected complement to the ladder: seed randomized populations through the checkpoint-chunked ensemble harness ([eom-attractor-search](../eom-attractor-search/priorities.md) owns the harness and cost model; this program owns the campaign), evolve, and classify what assembles — census rows, a declared persistence criterion, 2:2 neighborhoods first in coordination with rung 3. Gated on the instrument gate's booking rules. Exact native `assembly-view-record.v0` emission is accepted; the sampled record converter remains display-only and cannot satisfy this campaign's booking duty. Prerequisite infrastructure (dispatched separately, ungated): the $N=12$ host-control transfer factor and display-route confirmation through Borg `?eomRecord=`.
