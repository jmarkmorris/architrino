...
Plainly: this is exactly the creative geometry the operator approved for display and exactly the scientific row that produced the promising nominal pilot. Only its authority metadata differs from the frozen pilot fixture.
...
Plainly: the first half proves how far the stored cubic history can be from the exact analytic path. Keeping it separate prevents the root adapter from certifying its own approximation.

## Enclosed root adapter and ladder

The second implementation batch may call the production `certify_exact_pair_batch` API but may not modify that API, the frozen independent enclosure, or the prescribed-worldline conformance oracle. It must independently reconstruct the twelve approved histories, apply the frozen position/velocity widths to every segment, and preserve the exact constituent identities, source order, polarities, and retained interval.

The reception ladder is 8, 32, then 128 uniform phases on $[0,P)$. Every rung evaluates all 144 ordered source--target rows, including self histories, for totals of 1,152, 4,608, and 18,432 rows. The fixed controls are root tolerance `1e-8`, root depth `192`, cell limit `300000`, MPFR ladder `128`--`512` bits, and eight workers. Each output records `analyticInterpolationErrorBounded=true`, both source hashes, the enclosure-record hash, implementation hashes, toolchain identity, elapsed wall time, and every row-level certificate.

A rung passes only if every row is `certified_complete`, owns a root-free complement, avoids memory-boundary contact, every partner row has at least one positive-delay ordinary root with positive transmitter- and receiver-factor margins, every self row owns the coincident-endpoint exclusion without an unclassified ordinary event, and the complete output passes an independent ledger reducer. Execution stops on the first failed row, identity/order mismatch, enclosure escape, solver resource limit, or projected final-rung wall time above thirty minutes.

Plainly: success must reproduce the nominal pilot's favorable root structure while accounting for interpolation uncertainty. A nominally similar answer is insufficient if the wider intervals make even one row undecidable.
