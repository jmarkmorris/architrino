# Sub-Field Circular-Root H3 Pilot Predeclaration
...
Plainly: the independent theorem tells us what roots must exist; the raw solver ledger must account for them without omissions. A failed numerical certificate is an instrument or representation result until its cause is determined, not a rejection of the prescribed configuration.

Artifacts belong in new candidate/run directories under `.local-data/braid-analysis/subfield-circular-root-pilot-20260827-v1/`. Reserve output paths create-exclusively, emit flushed progress every fifteen seconds, retain raw certificates and history bytes, and permit no worker database writes. Only the coordinator may integrate accepted receipts. No old report or output directory may be overwritten.
...
Closure goal: bind and independently accept the catalog circular-history carrier, measure the two-phase EOM pilot, and only then scale the fixed sixteen-candidate root ladder.
