...
Status: closed diagnostic prescribed-path campaign, 2026-07-25. This packet records a bounded reconnaissance run over declared coordinates. It does not establish exhaustive family coverage, a global optimum, a dynamical basin, stability, retention, binding, energy closure, particle identity, independent acceptance, or physical realization.

## Configuration and predeclaration

The campaign used the live active members B1.1, B1.2, B1.3, and C1 through C6. It drew 64 cases per member, 576 total, from `constraint-preserving-full-taxonomy/sha256-counter-v1` with seed `braid-bc-monte-carlo-basin-20260725-v1`. Every numerical evaluation used $c_f=1$. The finite coordinate measure, exact member inventory, six-process cap, disposition rules, and descent protocol are recorded before the run in `.local-data/braid-analysis/monte-carlo-basin/bc-initial-20260725-v1/predeclared-protocol.json`; its SHA-256 is `f24bb346816ec644faae0c6881bfe5888f35d1b499ac8d207efefbec7c108a4f`.
...
The three unknown rows were B1.3 samples 11 and 43 and C6 sample 36. Samples 11 and 36 failed named event-convergence gates; B1.3 sample 43 failed the compact surface minimum-separation gate. Their exact coordinates, hashes, error messages, and null-score dispositions remain in the ordinary campaign rows. They are unresolved numerical cases, not negative candidate evidence.

The measured sum of the nine member-process wall times was `10412.046761915` seconds. The run used the predeclared bounded pool, with at most six member processes active. This number is measured process time and is not a claim about future throughput.
...
Two repeated exports of the 64-row B1.1 campaign were byte-identical at SHA-256 `31e16ce70c799431846ad02cd5f177660bfb1878766670faef9a7c5797dc003b`. Two exports of the empty handoff campaign were also byte-identical at SHA-256 `ba7a4d84b46259152fd7d48e68cbf5e7bf2615653a01c37968ec964d687a043d`.
...
Run each of the nine members separately with the common seed, quota, sampler, and coverage resolution:
...
Replace `B1.1` and the output suffix in turn with B1.2, B1.3, and C1 through C6. Then pass the nine comma-separated output paths to:
...
- A rerun with the exact seed, member, source inventory, protocol, and implementation identity that changes any campaign or case hash falsifies deterministic reproducibility.
