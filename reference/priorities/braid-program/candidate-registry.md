...
| `B1.3` | All-equatorial B1 boundary with $h_a=0$ and $\rho_a=R_a$. | Admitted taxonomy; Borg signed-off. | No retained braid; current-source `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
