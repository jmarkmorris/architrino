...
| `F6c` | `STASIS` | no eligible return; radial-frequency coordinate exhausted below its gate | verify and implement the [same-generation normalized member-acceleration predeclaration](evidence/2026-08-26-f6c-normalized-member-acceleration-predeclaration.md), or predeclare a materially new coordinated-turn/full-root packet |
...
| `F5` common-cadence circular realization | `DEMOTED` | exact `H2 F[D]` common-cadence circular obstruction | nonobstructed chart or regulator-independent coordinate-coincidence continuation |
...
Plainly: F5's approved creative geometry now passes the declared complete-root test with independently checked history uncertainty. The next question is how the EOM-generated future behaves from that prehistory; the prescribed path's built-in return does not answer it.

### 2026-08-26 Parallel A/B/C Admission Overlay

The later [parallel A/B/C H1/H2 census](evidence/2026-08-26-parallel-abc-h1-h2-census.md) supersedes the missing-H2 and stale-current-identity blockers in the historical campaign table above without changing any `STASIS` disposition or score. All nineteen frozen A/B/C child rows pass `H1` and continuous prescribed-history `H2`. `A3.1` and `A3.2` carry `H2 P[D/M]`; the other seventeen carry `H2 P[D]`. `A1`, `A1.3`, and `A3.3` lie outside this strict sub-field H3 method because $v_{\max}\ge1$; their general `H3` status remains `U`. The remaining sixteen are strictly sub-field. Their separately reviewed [pilot, candidate-specific ladder declaration, and resource plan](evidence/2026-08-27-braid-search-launch-readiness.md#parallel-abc-scope) are accepted, and the four-worker `8/32/128` machine run is complete. The [independently accepted complete ladders](evidence/2026-08-27-braid-search-launch-readiness.md#independently-accepted-complete-ladders) close scoped prescribed `H3 P[M]` for `A1.1`, `A1.2`, `A1.4`, `A2`, `A3.1`, `A3.2`, `A3.4`, `B1.1`, `B1.2`, `B1.3`, `C1`, `C2`, `C3`, `C4`, `C5`, and `C6`; all sixteen declared ladders and the final dispatcher are independently integrated.

The current-identity binding is therefore closed for `B1.1`, and the current-source analytical rebuild is closed for `C1`--`C6`. `BP-009` still governs the separate B1.1 center-pilot decision. `BP-007` remains a separate A1.3 calibration route and is no longer a direct H2 dependency for `C5`.
...
### 2026-08-29 B1.3 Regular-Locus Acceleration-Balance Overlay

The [planar co-rotating $N{:}N$ circular-balance packet](evidence/2026-08-29-planar-co-rotating-n-n-circular-balance.md) establishes one exact B1.3 sublocus under the default uncapped Master Equation with $c_f=1$: three equal-radius antipodal neutral binaries, binary phases $0$, $2\pi/3$, and $4\pi/3$, alternating endpoint polarities around the common circle, $\beta_f=2.974307176117306$, and $R/R_*=0.5617317000713459$. The complete 72-root directed ledger has maximum full prescribed-acceleration residual $7.62510728881475\times10^{-14}$ at the primary tolerance and $4.500069559058899\times10^{-12}$ after root-tolerance tightening. An unchanged generic prescribed-history evaluator independently checks every cross-transmitter root and acceleration contribution, and the unchanged circular-binary instrument independently checks the nontrivial same-transmitter roots.

Plainly: B1.3 now contains one independently checked circular path whose Master-Equation acceleration exactly supplies its prescribed circular acceleration to numerical tolerance. The unequal-phase equal-radius chart and the unequal-radius B1.3 chart remain unresolved, and this result establishes no retention, binding, stability, release survival, physical identity, score increase, or scientific acceptance.
...
| `A1.1` | A1 with one common binary frequency. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` and scoped prescribed `H3 P[M]` on the frozen prescribed history. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `A1.2` | A1 with equal radii, common frequency, and phases separated by $120^\circ$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` and scoped prescribed `H3 P[M]` on the frozen prescribed history. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `A1.3` | A1 with indexed frequency ratio $4{:}2{:}1$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` on the frozen prescribed history; `H3 U` outside the current sub-field method. | 0.00% | — | Validate a root method for the unchanged history, or separately freeze a slower history and reconfirm H2 to reuse the sub-field method; `BP-007` remains a separate optional calibration route. |
| `A1.4` | A1 with indexed frequency ratio $3{:}2{:}1$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` and scoped prescribed `H3 P[M]` on the frozen prescribed history with a narrow sub-field speed margin. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. Preserve the recorded speed margin. |
| `A2` | Fully symmetric three-binary Family-A locus with equal geometric scales and cadence, common circulation, and $120^\circ$ phases. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` and scoped prescribed `H3 P[M]` on the frozen prescribed history. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. A2 remains a parent geometry for exploratory F-series seeds. |
| `A3` | General three-binary Family-A geometry with a common braid center and independent axial, transverse, phase, and cadence coordinates. | Admitted taxonomy; Borg signed-off. | No retained braid; one bounded five-coordinate A3 slice has been evolved, without return or family coverage. | 14.17% | 76.67% | A full-A3 claim requires a separately predeclared covering campaign; none is queued. |
| `A3.1` | A3 with one common binary frequency. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1 P[D]`, `H2 P[D/M]`, and scoped prescribed `H3 P[M]` on the frozen prescribed history. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `A3.2` | A3 with equal radii, common frequency, and $120^\circ$ phases while axial/transverse decompositions remain distinct. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1 P[D]`, `H2 P[D/M]`, and scoped prescribed `H3 P[M]` on the current frozen prescribed history. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls; preserve the tight continuous clearance bound. |
| `A3.3` | A3 with indexed frequency ratio $4{:}2{:}1$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` on the frozen prescribed history; `H3 U` outside the current sub-field method. | 0.00% | — | Validate a root method for the unchanged history, or separately freeze a slower history and reconfirm H2 to reuse the sub-field method. |
| `A3.4` | A3 with indexed frequency ratio $3{:}2{:}1$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` and scoped prescribed `H3 P[M]` on the frozen prescribed history. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
...
| `B1.1` | Interior B1 locus with positive axial and transverse coordinates for every binary. | Admitted taxonomy; Borg signed-off. | No retained braid; current-identity `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls; `BP-009` remains a different center-pilot authorization decision. |
| `B1.2` | High-axial interior B1 locus with $h_a>\rho_a>0$. | Admitted taxonomy; Borg signed-off. | No retained braid; current-source `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `B1.3` | All-equatorial B1 boundary with $h_a=0$ and $\rho_a=R_a$. | Admitted taxonomy; Borg signed-off; the equal-radius phase-symmetric alternating locus has independently checked complete-root prescribed acceleration balance. | No retained braid; current-source `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed; the broader unequal-phase and unequal-radius B1.3 domains remain unresolved. | 0.00% | — | Predeclare ordinary EOM evolution from the balanced regular locus with explicit coupling, retained history, and numerical controls; a broader B1.3 claim separately requires phase-and-radius coverage. |
| `C1` | General twelve-member coaxial geometry with one circulation relation. | Admitted taxonomy; current prescribed display inspection complete. | No retained braid; current-identity `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `C2` | General twelve-member coaxial geometry with opposite circulation on two declared subsets. | Admitted taxonomy; current prescribed display inspection complete. | No retained braid; current-identity `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `C3` | Co-rotating coaxial pair of B1 components inside C1. | Admitted taxonomy; current prescribed display inspection complete. | No retained braid; current-identity `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls. |
| `C4` | Counter-rotating coaxial pair of B1 components inside C2. | Admitted taxonomy; current prescribed display inspection complete. | No retained braid; current-identity `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls; preserve the recorded history-depth margin. |
| `C5` | Co-rotating coaxial pair of all-equatorial B1.3 components. | Admitted taxonomy; current prescribed display inspection complete. | No retained braid; current-identity `H1/H2 P[D]` and scoped prescribed `H3 P[M]` are closed. | 0.00% | — | Separately predeclare ordinary EOM evolution with explicit coupling and numerical controls; `BP-007` remains only a distinct optional calibration route. |
