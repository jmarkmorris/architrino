...
| `F6c` | Eight-member, six-coordinate polarity-resolved breathing tetrahedral geometry. | Admitted candidate and active geometry program with an exact symmetry-invariant configuration surface. | No retained braid; bounded EOM solver releases preserve the surface and show partial turns, but the stored census and completed guarded continuations contain no root-valid complete return. | 18.47% | 77.25% | Complete the [source-bound family root-box coverage and residual/quadrature consumer](evidence/2026-08-27-braid-search-launch-readiness.md#bp-010-reference-blocker): provenance, named reference repairs, export, reconstructed-family containment, uniform guards and the conditional root-box library are reviewed. The family diagnostic requires no replacement-history selection; historical-score attribution and three-rung measurement remain separate. No further radial-frequency refinement is justified. See [F6c Geometry](../mapping-electromagnetism/f6c-geometry.md). |
