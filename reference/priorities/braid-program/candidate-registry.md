...
| Geometric and causal admissibility | Field-speed margin | 4 | Smallest value of $1-\lVert\mathbf V_i(T)\rVert/c_f$ over all members and the claimed history in normalized $c_f=1$ units. Higher is better; a nonpositive value leaves the strict guarded domain and requires a separately declared boundary treatment. |
...
- Field-speed and root-transversality margins remain separate because one is a member-speed boundary and the other is a causal-root geometry boundary. On a strict sub-field-speed simple-root chart the speed margin lower-bounds $D_t/c_f$, so their combined weight remains limited to 9 rather than counting them as independent major evidence groups.
...
The 25 points for geometric and causal admissibility protect the minimum conditions under which later numbers have a defined referent. Coincidence-limit closure receives 8 of those points because an unresolved zero-separation event can leave the outgoing history undefined, not because small separation is physically unfavorable. Root completeness receives 8 because one omitted causal branch can change the complete acceleration ledger. The remaining 9 points are split between the global field-speed margin and the root-local transversality margin because they report different boundaries; their limited combined weight recognizes the one-way lower-bound relation between them on the strict sub-field-speed chart.
...
The later [parallel A/B/C H1/H2 census](evidence/2026-08-26-parallel-abc-h1-h2-census.md) supersedes the missing-H2 and stale-current-identity blockers in the historical campaign table above without changing any `STASIS` disposition or score. All nineteen frozen A/B/C child rows pass `H1` and continuous prescribed-history `H2`. `A3.1` and `A3.2` carry `H2 P[D/M]`; the other seventeen carry `H2 P[D]`. `A1`, `A1.3`, and `A3.3` are blocked before H3 by $v_{\max}\ge1$. The remaining sixteen are strictly sub-field. Their separately reviewed [pilot, candidate-specific ladder declaration, and resource plan](evidence/2026-08-27-braid-search-launch-readiness.md#parallel-abc-scope) are accepted, and the four-worker `8/32/128` machine run is complete. The [independently accepted complete ladders](evidence/2026-08-27-braid-search-launch-readiness.md#independently-accepted-complete-ladders) close scoped prescribed `H3 P[M]` for `A1.1`, `A1.2`, `A1.4`, `A2`, `A3.1`, `A3.2`, `A3.4`, `B1.1`, `B1.2`, `B1.3`, `C1`, `C2`, `C3`, `C4`, `C5`, and `C6`; all sixteen declared ladders and the final dispatcher are independently integrated.
...
| --- | --- | --- | --- | ---: | ---: | --- |
| `A1` | Three neutral binaries with a common braid center and zero axial half-separation; each pair uses one center-crossing circle. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` on the frozen prescribed history, but its speed blocks H3 admission. | 0.00% | — | Freeze a slower-cadence history and reconfirm H2 before any H3 packet. |
...
| `A1.3` | A1 with indexed frequency ratio $4{:}2{:}1$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` on the frozen prescribed history, but its speed blocks H3 admission. | 0.00% | — | Freeze a slower-cadence history and reconfirm H2; `BP-007` remains a separate optional calibration route. |
...
| `A3.3` | A3 with indexed frequency ratio $4{:}2{:}1$. | Admitted taxonomy; Borg signed-off. | No retained braid; `H1/H2 P[D]` on the frozen prescribed history, but its speed blocks H3 admission. | 0.00% | — | Freeze a slower-cadence history and reconfirm H2 before any H3 packet. |
