...
**Dimension-extension relation (grade: derived coordinate identity).** For binary $a$, any paired axial coordinates decompose as $\xi_{a,\pm}=b_a\pm h_a$. Canonical B1 is the common-midpoint locus $b_1=b_2=b_3$, with the common value absorbed into the braid center. Modulo that common translation, the nearest six-orbital train generalization adds only two relative-midpoint coordinates, such as $(b_2-b_1,b_3-b_1)$, to the existing B1 half-separations. A fixed-component twelve-core train inherits this extension componentwise. Payload worldlines change the inventory and are not merely extra coordinates.
