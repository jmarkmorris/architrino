...
## Entry 4 — accessory quietness applications require the current six-site inventory (2026-07-21, taxonomy migration audit)

**The retained idea (grade: idea, unproven lead).** The Thomson dressing mechanism orders same-polarity accessory configurations by the lowest polarity-signed multipole that survives their symmetry. A six-site octahedron is quiet through octupole order; a four-site tetrahedron first leaks at octupole order; a two-site axial pair leaks at quadrupole order. This geometry may help order exposed mass response, resilience, and near-field coupling.

**Conflict found during migration.** The older application called the four-site and two-site arrangements the up-type and down-type quark inventories. Current corpus architecture gives every charged fermion a six-site axial record, with quark charge and color encoded by mixed-polarity patterns on those six sites. The old shortcut therefore cannot be promoted as the current quark model. The electron octahedral case is geometrically compatible with a six-site axial record, but still lacks a retained confinement and far-field calculation. The neutrino Fourier-mode and Koide comparisons are separate speculative mappings and do not follow from Thomson geometry alone.

**What would confirm or kill it.** Build the actual six-site polarity-weighted moment tensor for the electron, up-type, and down-type axial patterns, including the braid-induced site deformation. The idea survives if the first nonzero moments order independently computed exposed wake ledgers and mass-response tensors. It fails if the moment order does not track those ledgers or if the required axial configurations are not retained.

**Promotion routing.** The configuration theorem remains in `braid-mathematics.md`. The particle-specific applications remain priority-only until a six-site component ledger, polarity-weighted moment calculation, and retained far-field comparison exist.
