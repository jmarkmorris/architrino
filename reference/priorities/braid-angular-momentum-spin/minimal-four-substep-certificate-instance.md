...
Status. Proof packet for the reduced `worked_three_layer_noether_transition` row in [braid-angular-momentum-spin.md](braid-angular-momentum-spin.md). This file instantiates the solved positive outer-coupled four-substep branch as far as the present symbolic scaffold allows. It is priority material, not AAA prose, and it does not prove the general Noether braid three-binary partition theorem.
