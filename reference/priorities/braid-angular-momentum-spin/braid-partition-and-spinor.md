...
This detailed priority file supports [Angular Momentum and Spin Closure](braid-angular-momentum-spin.md). It preserves the Noether braid three-binary partition theorem target, solved minimal transition, ordered Noether braid frame, spinor proof obligations, and component-resolved causal-writhe hypothesis.
...
5. **Phase-lock constraints.**
   The post-transaction branch must close the active causal roots over a full three-binary cycle:
   $$
...
The current search set also includes the older dyadic lock and a finite general integer-lock grid. In canonical order the dyadic control is $(I,M,O)=(4f,2f,f)$, with legacy outer-normalized translation $f_O:f_M:f_I=1:2:4$. The general controls sample $(I,M,O)=(nf,mf,f)$, with legacy outer-normalized translation $1:m:n$, over the declared finite `m,n` range in the runner. These rows are search candidates only; they do not promote the dyadic lock or the integer-lock lattice to retained branch acceptance.

Branch-certificate ranking 2026-06-20. The runner now separates the retained-branch relevance ranking from the span-only diagnostic. Its primary criterion is coverage of the `self_root_parity_index_proxy` target $n_I-n_M=2$, followed by phase-lock premise coverage, priority-candidate status, reduced-row pass coverage, and only then inner self-hit span as a tie-breaker. Under this ranking the middle-hinge family $(I,M,O)=(f+2,f,f-1)$ ranks first: all 14 sampled cases satisfy the self-root parity proxy, including all seven phase-lock cases. The adjacent integer-lock controls $(3f,2f,f)$, $(4f,3f,f)$, and $(5f,4f,f)$ only hit the parity proxy at $f=2$, giving 2 of 14 sampled cases each. The high-span $(6f,2f,f)$ control and the dyadic $(4f,2f,f)$ control have zero self-root parity proxy coverage, so their larger inner self-hit spans remain diagnostic rather than branch-certificate evidence. The retained branch claim remains false for every family because row-set identity, retained phase lock, torque consistency, tail-wake pullback, retained physical vector partition, energy routing, section stability, and retained competitor rows are still blocked.

Non-minimal competitor audit 2026-06-20. The same search now emits `nonMinimalCompetitorAudit`, a finite reduced competitor-family audit over the symmetric control, dyadic control, and integer-lock controls. This closes the easy absence claim: a finite reduced competitor set is populated. It does not close retained competitor exclusion, because all competitor families remain reduced controls with `retainedBranchClaim=false`; none carries the same retained row-set identity, retained phase lock, torque, wake, partition, energy-routing, section-stability, and branch-selection residual payload needed to defeat or certify the middle-hinge branch.

### Candidate 2026-06-21: Test $(I,M,O)=(f,f,f)$ As An Energy-Radius Hinge

Status: priority-only candidate under `tri_binary_partition_rule` and `worked_three_layer_noether_transition`. This is not a theorem row, not a canonized three-binary law, and not reader-facing corpus prose. It is tracked in [equal-frequency-energy-radius-candidate.md](equal-frequency-energy-radius-candidate.md) because it tests a different mechanism from the offset family.
...
The direct candidate claim is that the three binaries may share one frequency while their effective lever arms and speed relations differ because the retained branch energy places each binary differently inside the velocity-deforming Noether braid three-binary branch state:
...
Here $\rho_\ell$ is the retained effective lever-arm projection entering the local binary speed row, not automatically the global nested-shell coordinate or the radius of a common sphere. In a retained Noether braid three-binary replay, $\rho_\ell$ might be a low-drift small-circle lever arm $R_{\mathrm{sph}}\sin\theta_\ell$, a moving oblate-envelope incidence projection, a branch-selected shell radius, a planar-alignment support parameter, or another retained branch map. Three common-sphere great circles with equal $\omega_f$ would have equal speed magnitude in the simple kinematics, so that control cannot by itself provide the speed ordering. Thus the middle binary may remain on the field-speed hinge through an energy-radius, envelope-deformation, or incidence relation, while the inner self-hit burden is carried by causal-root multiplicity, phase offsets, branch incidence, wake routing, or action partition rather than by a frequency offset. The first high-priority phase profile is the triadic $120^\circ$ profile:

$$
\left(\phi_I,\phi_M,\phi_O\right)
=
\left(0,\frac{2\pi}{3},\frac{4\pi}{3}\right).
$$

The more general free phase data are
...
and must be sampled as branch data, not treated as decoration.

This candidate also sharpens the one-unit angular-momentum question. In the clean four-substep ledger the weighted transaction-frequency target is
...
For $(I,M,O)=(f,f,f)$ this collapses to $\omega_\ast=\omega_f$. If a retained branch can prove the matching energy-radius law, the angular-momentum unit currently written as $\hbar$ can remain the single conserved action increment, while the energy placement, effective lever arms, envelope deformation, speeds, phase offsets, and wake/recoil routing determine how that unit is exposed across the three binaries. This is the reason the candidate should be very high priority rather than a neutral control.

The current branch-certificate ranking cannot be used as the sole disposition rule for this candidate. Its primary proxy $n_I-n_M=2$ is designed around the offset hypothesis, so $(I,M,O)=(f,f,f)$ gives zero by construction. The runner now emits separate equal-frequency rows for common-frequency closure, effective lever-arm speed placement, triadic $120^\circ$ phase profile, and transaction-frequency collapse. The remaining rows still need retained Noether braid three-binary geometry, root-multiplicity or branch-incidence support for the doubled inner burden, and retained angular-momentum / wake-energy routing on the same event or positive-width retained domain. The diagnostic `index-ratio` policy should not reject this family, because that policy maps speed ratios to frequency-index ratios while the equal-frequency hypothesis assigns speed ratios through retained lever arms, envelope deformation, and branch energy.

Vector-partition payload update 2026-06-20. The runner now emits `aaa-tri-binary-retained-vector-partition-payload-target.v1` for the $\pi/2$ middle-hinge packet with status `retained_vector_partition_payload_unit_balance_and_carrier_assignment_populated_acceptance_blocked`. Its route-local unit transport/recoil vector balance is populated on both middle pair-map routes with maximum residual $0$, and the nested `aaa-tri-binary-physical-partition-carrier-assignment-target.v1` row is `physical_partition_carrier_assignment_candidate_populated_coefficients_missing`: two of two unit transport/recoil carrier-pair rows are populated with maximum carrier-balance residual $0$. The nested `aaa-tri-binary-partition-transport-recoil-coefficient-source-target.v1` row reports `partition_transport_recoil_coefficient_source_current_wake_coefficients_rejected`: the unit transport/recoil coefficient is exact but ineligible as a diagnostic convention, the accepted route-local wake coefficient rows do not match the partition transport vectors, and the best real rejected reuse is the normalized boundary-charge vector with residual $0.0011593636323863898$. The nested `aaa-tri-binary-partition-transport-recoil-coefficient-law-search-target.v1` row reports `partition_transport_recoil_coefficient_law_search_fitted_same_source_scale_formal_acceptance_blocked`: 12 current candidates were tested, four exact rows are formal/target-derived, including the route-geometry cross-product row $\Delta J_{\mathrm{partition}}^{\mathrm{geom}}=r_{\mathrm{mid}}\times\Delta p_{\mathrm{route}}^{\mathrm{unit}}$ and the fitted same-source scalar $0.09754069445241503$, eight independent route/wake scalar rows are rejected, and no candidate is accepted. The nested `aaa-tri-binary-partition-transport-recoil-coefficient-independent-source-exclusion-summary.v1` row reports `partition_transport_recoil_coefficient_independent_source_current_search_exhausted_new_law_required`, with two rejected source families, one formal-only family, zero incomplete current families, and retained-branch acceptance still false. The nested `aaa-tri-binary-partition-point-event-torque-coefficient-source-target.v1` row reports `partition_point_event_torque_coefficient_source_rejected_not_partition_transport_law`: the same-source row's point-torque delta is $0$ while the required partition transport is $z=0.09754069445241503$, so point-event torque difference is rejected as the coefficient source despite the same-receiver formal zero row matching to residual $5.551115123125783\times10^{-17}$. This makes `partition_payload` partial rather than missing in the compensated retained-chart inventory, discharges the route-local carrier-assignment blocker, and sharpens the remaining blocker to a new independent partition transport/recoil coefficient law or full-event rule plus retained force / torque / wake rows and accepted energy routing.
...
First calculation. Insert the offset families and the equal-frequency family into the existing action-angle energy condition. In the clean no-wake, no-root-energy approximation, compare whether
...
more naturally for the canonical $(I,M,O)=(f+2,f,f-1)$ family than for the symmetric control, or whether the equal-frequency family collapses the target to $\omega_\ast=\omega_f$ while shifting the proof burden to the retained energy-radius law. Equivalently, the offset comparison includes canonical $(I,M,O)=(f+2,f,f-1)$, while the equal-frequency comparison keeps canonical $(I,M,O)=(f,f,f)$ and varies retained Noether braid three-binary lever arms, envelope deformation, speeds, phase offsets, root multiplicity, and wake routing. If either comparison depends on undeclared wake or root-energy slack, the candidate remains blocked rather than promoted.

Reduced solver probe 2026-06-20. The first executable comparison now lives in [tri-binary-offset-family-runner.mjs](../../../scripts/angular-momentum/tri-binary-offset-family-runner.mjs). It uses the central solver bridge to sample circular source roots, delayed hits, root-ledger details, and circular self-hit spans for both the canonical $(I,M,O)=(f+2,f,f-1)$ candidate and the canonical $(I,M,O)=(f+1,f,f-1)$ control. The default report target is `.tmp/angular-momentum-spin/tri-binary-offset-family-solver-report.json`.

Equal-frequency runner addition 2026-06-21. The runner includes $(I,M,O)=(f,f,f)$ as the priority `equal-frequency-energy-radius` family and adds an `equal-frequency` policy rather than forcing the candidate through `index-ratio`. The sampled replay for $2\le f\le8$ now emits `aaa-tri-binary-frequency-candidate-solver-report.v22`, `aaa-tri-binary-frequency-triplet-search-summary.v23`, `aaa-tri-binary-frequency-triplet-candidate-set-review.v19`, `aaa-tri-binary-frequency-triplet-comparison-axis-review.v1`, and `equalFrequencyEnergyRadiusAudit` schema `aaa-equal-frequency-energy-radius-audit.v23` with status `equal_frequency_phase_catalog_populated_priority_triadic_pass_acceptance_blocked`: common angular frequency passes in the current reduced chart, retained effective-lever-arm projections populate in canonical `I:M:O` order as $\rho_I=1.25/f$, $\rho_M=1/f$, and $\rho_O=0.75/f$, speed ratios populate as $1.25:1:0.75$, the priority triadic phase profile populates as $\phi_I=0$, $\phi_M=1/3$, and $\phi_O=2/3$ turns, and $\omega_\ast$ collapses to the common frequency $\omega_f=f$. The v22 solver contract exposes `layers[].effectiveLeverArm` as the physics-facing speed-row quantity and `layers[].solverGeometry.effectiveLeverArmChartCoordinate` as the flattened-limit chart coordinate; `layers[].radius`, public circular-source radius fields, and lift-target geometry language are removed. The current executable chart is `flattened-limit-effective-lever-arm-projection-chart`, which remains a projection of the retained velocity-deforming Noether braid three-binary branch, not the branch model itself. The v19 candidate review compares equal-frequency, offset, dyadic, and finite $m:n$ candidates on frequency, radius/velocity, phase, and retained-acceptance evidence rather than letting the old offset proxy exclude another family. The v23 phase-deformation balance audit reports that triadic $120^\circ$ phase-only balance passes seven of seven priority rows, while effective-lever-arm weighted, unit-inertia action weighted, and target $2:1:1$ root/action weighted balance each pass zero of seven rows. The v23 phase-lattice audit scans 144 rows at $30^\circ$ spacing and separates two exact phase-only triadic rows from one exact $2:1:1$ weighted-action opposition row with zero overlap; the weighted-action control is `phase-lattice-12-i0-m6-o6`, with $\phi_M=\phi_O=1/2$ opposite the doubled inner row. The v23 return-period audit reports seven reduced-chart passes and zero retained return-period certificates; the v23 plane-sector audit reports seven coplanar planar-$\mathbb{Z}_3$ rows, zero near-orthogonal rows, and zero retained plane-sector certificates. The v23 retained frequency/phase packet adds seven $S_{\mathrm{eq}}$ current-proxy rows, one for each sampled priority $f$, with seven frequency-proxy passes, seven sector-proxy passes, seven triadic holonomy-proxy passes, and zero retained acceptances. The v23 retained row-set scaffold names $S_{\mathrm{eq}}$ as one finite retained branch chart for the same six-body polarity-neutral Noether braid, with current proxy evidence from six subaudits, zero retained acceptance, and 14 row-set blockers. The v23 retained event/domain lift target is populated current-proxy-only: accepted full point-event rule, positive-width retained domain, global row-set identity, binary-to-binary retained history rows, retained payload rows, retained energy routing, same-event momentum/angular-momentum rows, and Noether sea event/domain binding remain false. The v23 retained replay acceptance blueprint defines $R_{\mathrm{eq}}=(r_{\mathrm{rows}},r_{\mathrm{evt}},r_P,r_{\rho},r_E,r_D,r_W,r_J)$ on one retained row set $S_{\mathrm{eq}}$ and one accepted retained event or positive-width retained domain; it reports seven current-evidence conditions, zero accepted retained conditions, and eight blockers. The current dynamics/foundations refresh also prevents overreading the phase result: the unweighted $120^\circ$ identity is the planar $\mathbb{Z}_3$ cyclic cancellation unless retained plane normals, phase-bundle holonomy, and $D_{\mathrm{plane}}$ classify the row as coplanar cyclic, near-orthogonal three-binary, or another retained sector. The next replay should evaluate those rows in the primary retained Noether braid three-binary branch, with same retained row-set identity, accepted event/domain support, retained phase row-set identity, plane-sector classification, root-multiplicity or branch-incidence support for the inner burden, phase choice or reconciliation between unit-clock cyclic balance and $2:1:1$ weighted-action balance, and one-unit angular-momentum / wake-energy routing.

The reduced probe supports continued priority work but does not certify the branch. In the `phase-lock` policy, where offsets are treated as integer labels and retained effective lever-arm projections are retuned so the inner, middle, and outer speeds stay super-field, field-speed, and sub-field, both offset families pass the sampled reduced rows with no inner self-hit span separation. In the diagnostic `index-ratio` stress policy, where speed ratios are mapped to $n_{\ell}/n_M$, both offset families pass the sampled reduced rows and the canonical $(I,M,O)=(f+2,f,f-1)$ candidate has the larger inner self-hit span for $2\le f\le8$. This means the candidate remains live because the solver sees the expected stronger inner self-hit burden under the stress mapping, but promotion still requires a retained branch chart with phase, vector-ledger, energy-routing, wake, torque, and stability residuals. The current action-scale subpacket also keeps the middle-hinge route on the priority path: its best route-local scale candidate misses by $1.3999924176888445\times10^{-7}$, and the residual-correction diagnostic rules out quadrature convergence and the tested simple endpoint-leakage scales. The same diagnostic maps the miss to a required wake boundary-charge norm $0.09870019421579312$, gap $1.3613099170139975\times10^{-7}$, and its boundary-charge increment candidate audit reports `boundary_charge_increment_candidates_rejected_target_derived_comparison_only` with zero acceptance-eligible matches among 10 tested rows. The alternate-normalization diagnostic reports `alternate_normalization_shift_candidates_rejected_target_derived_comparison_only`: exactness would require changing the action-kernel margin ratio from $4$ to $3.9999944830507057$, shifting $\eta$ by $3.385162359106908\times10^{-8}$ or the minimum $h_+$ route width by $1.35406307597008\times10^{-7}$, but none of the 11 tested quadrature or endpoint-width candidate rows supplies an acceptance-eligible match. The retained-work / near-field diagnostic reports `retained_work_action_scale_candidates_rejected_target_derived_comparison_only`: 23 finite route-payload rows were tested, zero acceptance-eligible rows matched, and the closest independent route-payload row is `same_source_endpoint_pair_residual_half` at residual magnitude $1.3686468311394084\times10^{-4}$. The coefficient-provenance diagnostic reports `coefficient_provenance_candidates_rejected_target_derived_comparison_only`: exactness would require the `outer->middle` least-norm coefficient to shift by $5.599969670755378\times10^{-7}$, but accepted least-norm solve residuals, accepted row-amplitude residuals, and finite causal-gap Gaussian effects do not supply it; the best independent coefficient-unit row is only $9.470949326781743\times10^{-17}$. That leaves an independent source for the boundary-charge increment, a new normalization derivation beyond the rejected current rows, a new retained-work / near-field law beyond the tested formal route-payload rows, or different exact route-local action-scale law as the next proof target.

The same report now carries a branch-chart projection. It marks `root_chart_reduced`, `active_row_lineage_probe`, `torque_wake_same_row_diagnostic`, `outer_speed`, `middle_hinge`, `inner_self_hit`, `cycle_phase_closure_proxy`, `phase_at_hit_rows`, `self_root_parity_index_proxy`, and `energy_frequency_target` as populated reduced rows, while leaving the retained branch-selection residual blocked: `row_set_identity`, `phase_lock`, `torque_consistency`, `tail_wake_pullback`, `vector_partition_retained`, `energy_routing`, `section_stability`, and `non_minimal_retained_competitors` still lack a retained certificate payload. The selected `index-ratio:f2` canonical $(I,M,O)=(f+2,f,f-1)$ case remains a candidate-only retained-lineage / phase / torque-wake diagnostic payload: it has same-row force / partition / torque / wake diagnostic row IDs, finite instantaneous torque and diagnostic wake samples, a fixed-receiver time-window torque probe, binary-to-binary path-history roots/hits, replayed root-ledger detail rows, transition classification, inactive-gap margins, retained hit-time coverage, two boundary-only hinge-point candidates, point-event witnesses, point-event admissibility rows, layer-balanced branch-transport incidence rows, and a preferred $\pi/2$ topology branch-transport pair-map. `retainedBranchClaim=false` remains the solver verdict.

The priority-side `torque_wake_same_row_diagnostic_report/v0` carries `same_retained_active_row_certificate_contract/v0` for the top-six consumers. The current readout has `sampled_same_row_id_binding=true` but `same_retained_active_row_id_binding=false`, with `first_failure=branch_certificate_ref_missing_before_same_retained_active_row_ids`. This distinguishes sampled force / partition / torque / wake row alignment from the retained-row certificate needed by rank 2 `accepted_transition_source` intake and rank 6 `moving_retained_branch_certificate/v0`. The next provider must supply one `branch_certificate_ref`, `same_retained_active_row_ids`, `accepted_branch_chart_ref`, and `moving_retained_branch_certificate_ref` on that same retained row set; route-local or synthetic row labels remain non-authorizing.
...
The positive-width retained-domain lift target now makes the failure mode explicit. `aaa-tri-binary-positive-width-retained-domain-lift-target.v1` reports `positive_width_retained_domain_lift_blocked_all_pair_point_only_and_route_compensation`: both middle routes have positive route-restricted one-sided width, but the all-pair retained-domain side intervals fail because `inner->inner` and `middle->middle` are point-only and the left/right side coverage misses different pair sets. This preserves the hinge packet as a high-priority canonical $(I,M,O)=(f+2,f,f-1)$ candidate while ruling out immediate promotion from route-local width alone.

The full point-event rule lift target now tests the other side of that fork. `aaa-tri-binary-full-point-event-rule-lift-target.v1` reports `full_point_event_rule_lift_blocked_route_compensation_and_global_scope`: candidate point-event admissibility, branch-transport incidence, topology pair-map support, and route-authorized diagonal identity all pass at the $\pi/2$ hinge; the off-diagonal point-torque norm is $2.7755575615628914\times10^{-17}$; and the route root key is the same retained root key `2856731379702547500`. The target stays fail-closed because endpoint-provider-assisted geometry is not yet global retained transport, zero-slack branch transport is missing, one same-source route still requires compensation, global retained row-set identity and positive-width common retained time domain are missing, and retained force, torque, wake, phase, partition, stability, vector-ledger, and energy-routing rows are not certified on that same event.

The diagonal identity rule target now turns those point-only diagonal rows into an explicit route-authorized point-event result instead of a vague blocker. `aaa-tri-binary-full-point-event-diagonal-identity-rule-target.v1` reports `route_authorized_point_event_diagonal_identity_accepted_full_scope_blocked`: diagonal identity witnesses are populated for `inner->inner`, `middle->middle`, and `outer->outer`; the first two are point-only; the `outer->outer` row has positive side width; and `routeAuthorizedPointEventDiagonalIdentityPass=true` with no route-authorized acceptance blockers. This does not accept the full point-event rule. The remaining blockers are retained-transport and retained-payload blockers: endpoint-provider geometry is still route-authorized local evidence, zero-slack branch transport and global row-set identity are missing, the same-source route requires compensation, and retained payload / energy-routing / full-rule acceptance are absent.

The full point-event payload coverage target now sharpens that blocker. `aaa-tri-binary-full-point-event-payload-coverage-target.v1` reports `full_point_event_payload_coverage_partial_route_wake_energy_carriers_populated_acceptance_blocked`: candidate point-event force/torque evidence, local provider-assisted branch geometry, complete compensated route payload, route-authorized wake boundary-charge pullback, route-authorized diagonal identity, retained torque payload acceptance target rows populated at candidate scope, a route-local clock-to-phase payload law with residual $0$, route-local vector-partition unit balance with residual $0$, route-local physical-partition carrier assignment with residual $0$, rejected current partition coefficient sources, a formal-only partition coefficient-law search with no accepted independent candidate, an exhausted current finite partition coefficient-source family, rejected point-event torque difference as the partition coefficient source, and exact same-event energy carriers are all populated. The retained torque target is `retained_torque_payload_point_event_candidate_populated_acceptance_blocked`: all six off-diagonal point-event torque rows pass with maximum row torque norm $5.551115123125783\times10^{-17}$, but retained torque acceptance still needs retained transport, global row-set identity, a full point-event rule, and retained energy routing. The target remains fail-closed because accepted $\omega_{\text{tx}}$, derived $\sigma\hbar$ action scale, accepted wake-energy increment law, accepted retained transport for the phase law, retained torque acceptance, an accepted partition transport/recoil coefficient law, section stability, accepted retained energy routing, and an accepted full point-event rule are still missing.
...
3. The symmetric canonical $(I,M,O)=(f+1,f,f-1)$ control passes all residuals with lower or equal branch-selection cost.
...
The candidate ordered Noether braid frame is the history-lifted object
...
1. **Branch-certificate extraction:** for one stable separated-scale Noether braid branch, extract $P_a$, $\hat{\mathbf n}_a$, $\mathcal{G}_a$, $\mathcal{G}_{ab}$, $\chi_c$, phase offsets, and the total angular-momentum ledger over a common closure period.
...
The working result is that scalar $Wr_c[\gamma]$ is a strong chirality signal but is probably too compressed to carry all three labels by itself. It can plausibly detect handedness of a single causal self-interaction pattern, but a scalar total can alias or vanish when the three-binary has multiple role-labeled components, balanced pro/anti pairings, mixed planar rows, or a weak-coupling-triad exposure state that depends on wake geometry rather than only on intrinsic handedness.
...
1. Evaluate $Wr_c^a$ and $Wr_c^{ab}$ on controlled pro-braid and anti-braid nested shell braid trajectories to see whether the ordered `HML/HLM` distinction survives smooth deformation and flips only through causal-locus reconnection.
