...
more naturally for the canonical $(I,M,O)=(f+2,f,f-1)$ family than for the symmetric control, or whether the equal-frequency family collapses the target to $\omega_\ast=\omega_f$ while shifting the proof burden to the retained energy-radius law. Equivalently, the offset comparison includes canonical $(I,M,O)=(f+2,f,f-1)$, while the equal-frequency comparison keeps canonical $(I,M,O)=(f,f,f)$ and varies retained Noether braid lever arms, envelope deformation, speeds, phase offsets, root multiplicity, and wake routing. If either comparison depends on undeclared wake or root-energy slack, the candidate remains blocked rather than promoted.
