# Coordinating Research, Notation and Evidence
...
AAA Work Threads coordinates these relationships. Its subject is the allocation of attention, the identification of shared mathematical obligations and the preservation of meaning as work moves between documents and instruments. The coordination record is not another physical theory, a substitute scientific queue or an independent source of accepted results. Its proposed common structures remain conditional until their scientific owners supply the objects they name.
...
A top-level owner may be deferred or unranked while remaining an active address for future decisions. A directory under the dormant archive remains excluded from active ranking despite an old score or active label inside its preserved history. Retaining an address can protect a useful reopening condition without creating work. The former simulation directory is a compatibility route to the operations protocol index, not another execution engine. The shared headless platform supplies application infrastructure without becoming a visitor-facing application.
...
Completed work has no remaining value as an object of attention. It can reduce the prerequisites or effort of a successor, but its former score must not persist as prestige for the workstream. Closing an object therefore replaces its row with a newly assessed successor or removes the row. The executable queue, local winner and unified description must refer to the same object.

This rule distinguishes progress from percentage complete. A large nearly finished project may still require a decisive conceptual invention; a small negative result may remove a major ambiguity. The ranking asks what the next result could change and what remains necessary to obtain it. It does not measure mathematical truth, corpus readiness or scientific acceptance.
...
Using the existing field names, the calculation is

$$
\mathrm{Pressure}=1+\frac{\max(0,-\mathrm{MinDelta})}{100},
\qquad
\mathrm{EWeight}=0.50+0.10\,\mathrm{Eureka},
$$

$$
\mathrm{Value}=\mathrm{Base}\,\mathrm{Cascade}\,\mathrm{Pressure}\,\mathrm{Engine}\,\mathrm{EWeight}.
$$

Cascade bands distinguish mostly local effects at 1.00–1.15, useful cross-links at 1.20–1.35, several theory queues at 1.40–1.55 and broad multipliers at 1.60–1.75. Engine bands distinguish core equations at 1.40–1.55, core certification at 1.15–1.35, first physical payoff at 1.00–1.10, downstream sectors at 0.85–0.95 and dependent ledgers, gates, applications or coordination at 0.00–0.40. These bands implement the dependency rule: organizing an unresolved derivation does not itself outrank performing that derivation. A concrete result ready for examination can change the situation; a more elaborate plan alone cannot.

Remaining burden is divided into execution once the route is known, conceptual invention, cross-workstream dependencies and validation or no-go work. Each is scored from one to ten. The procedure first rounds the following weighted sum half up to one decimal, then divides Value by that rounded Cost:

$$
\mathrm{Cost}=\operatorname{round}_{\mathrm{half\ up},1}
\left(0.25\,\mathrm{Exec}+0.35\,\mathrm{Intuition}+0.20\,\mathrm{Deps}+0.20\,\mathrm{Valid}\right),
\qquad
\mathrm{ROI}=\frac{\mathrm{Value}}{\mathrm{Cost}}.
$$
...
The preserved table demonstrates why identity matters more than a rank number. It includes binary fate, an independently evolving causal wake, a transfer operator and measure, constitutive response, a positive-width equation-recovery domain, quark-mass recovery, exposure, strong-field predictions, preferred-frame leakage, an event ledger, a detector kernel and application teaching or execution surfaces. Those objects have different upstream requirements. A numerical position assigned to one cannot be carried unchanged to its successor merely because the directory name is unchanged.

Earlier inventories and dated join tables contain other populations and orders. They are historical observations, not competing current rankings. The retained documents also contain stale descriptions beside later dispositions. This synthesis preserves those differences in source coverage rather than silently reranking work. A live owner that no longer exposes the scored object, a failed prerequisite or a materially different measured burden would invalidate the corresponding attention judgment.
...
Instantaneous configuration and velocity must remain distinct from the required delayed history. Calling a configuration complete does not prove that it determines future evolution without that history. Likewise, a residual needs a specified argument, norm or comparison, threshold and interpretation. A scalar number with no declared state and instrument cannot stand in for a branch certificate.
...
Symmetry removal, finite active-root control and absence of secular rest-frame drift can be parts of a particular certificate. Their relevance and hypotheses remain with its owner. A prescribed geometry, a short numerical excursion, a completed root instrument or a replayed trajectory does not automatically establish a persistent branch. The historical binary workload and its inward or outward diagnostics remain bounded evidence; coordination does not book an accepted fate from them.
...
The coordination proposal does not require every consumer to show a nonzero medium contribution. A source response may be negligible within a declared bound. What matters is a common account that establishes the response or its bounded absence on the admitted domain. A private coefficient chosen for each observable would defeat that common-account obligation.
...
The event ledger joins the appropriate energy, momentum, angular momentum, polarity or charge provenance, recoil and medium account. At the primitive level these are derivation obligations, and at the effective level their definitions and conversion must be explicit. Listing familiar conserved quantities does not postulate their standard formulas for individual architrinos. A source may require pro/anti provenance or pair-corridor information without having derived every effective charge label.
...
$$
\mathcal{E}_S(A)=Q_S[\Pi_S\mathcal L_A].
$$

Here $A$ identifies the branch, $S$ the sector, $\mathcal L_A$ its ledger, $\Pi_S$ the declared projection and $Q_S$ the quotient; $\mathcal{E}_S(A)$ names the resulting sector exposure. The projected and quotiented record must retain its source identity and applicable domain. It must also bound the channels claimed to be absent or hidden: unwanted anisotropy, longitudinal response, gauge dependence or residual internal exposure cannot disappear merely because the selected scalar readout omits them.
...
Here $B_i$ is the declared basin, $\mu_*$ the invariant measure and $p_i$ its proposed outcome weight. That expression does not derive a physical probability rule by naming a measure. The measure must be supplied and independently examined on an accepted persistent assembly state; finite-time or metastable alternatives need their own stated windows and approximations. Coarse-graining must say what information it discards and why the resulting observable remains appropriate.
...
Several recurring clusters follow. Residuals and event accounts join reaction and release descriptions. Exposure joins internal geometry to allowed sector response. Basin measure joins persistent evolution to outcome weights. Medium response joins clock, signal, inertial and gravitational comparisons. Each cluster can simplify duplicated work only when it retains its owner's definitions, source identity, hypothesis domain and falsifier.

Application and service owners remain consumers of this structure. A visualization can display a declared subset; a source-retrieval service can return an authored claim; an archived validation schema can preserve a comparison format. None supplies a new scientific acceptance key.
...
Directory genre is not a maturity test. An application lane can contain a substantive mathematical certificate, while a theory-named folder can contain operational instructions. A developed argument may deserve a whole manuscript treatment rather than being atomized into tasks. A negative result and the proposal it limits should remain together so that a later reader does not revive the rejected implication.
...
A zero first derivative does not make every response zero. Consider the elementary function and its central differences:
...
\frac{z(h)-z(-h)}{2h}=0,
\qquad
\frac{z(h)+z(-h)-2z_0}{h^2}=2a.
$$

For nonzero $h$, the odd response vanishes, while the even second response is nonzero when $a$ is nonzero. This logical counterexample supplies no physical medium law or measured coefficient. Small-step finite differences alone do not measure periodic-drive response, averaged flux or frequency-dependent transport.

For the illustrative quadratic $q(p)=q_0+bp^2$, a sinusoidal argument $p(T)=a\cos(\omega T)$ has mean quadratic term $ba^2/2$ over complete nonzero-frequency cycles. The term is nonzero when amplitude and coefficient are nonzero. This deduces a property of the function. Physical transport additionally requires an accepted population state, defined drive and response, flux and boundary accounts, an averaging window and a derivation from the same update law.
...
The earlier selection of spinor and holonomy learning belongs to its own recorded scope. Other ideas remain provisional or dormant, with simpler existing owners preferred where adequate. Enjoyable mathematical learning can be worthwhile without altering the physical ranking or reopening a scientific queue.
...
The current notation scope permits only worthwhile independent corrections: a concrete reader problem, an already settled meaning, a destination requiring no new displacement chain and a bounded complete source-and-consumer change. The broad speed and xi migrations are deferred. The recorded disposition is thirty-nine retained and five deferred overlapping table entries, not forty-four distinct symbols or a completed occurrence census.
...
The numerical convention $c_f=1$ does not erase these distinctions. Two quantities can have equal numerical values in those units while their functional definitions differ. The same applies to an index: it may identify an architrino, binary, sample or coefficient. An indexed beta is not automatically an internal-speed ratio.
...
The 29,623 historical symbol-description records included 20,375 shared-context fallbacks. That measures an extraction category, not the number of mathematically undefined symbols. A nearby source-context sentence can also be the wrong definition. Both require reading the actual equation and local meaning.

The top-fifty table is therefore a search aid. Numeric powers, dimensions, inverse maps, particle labels, indices and repeated exposition can distort a normalized spelling count. The raw TeX and source location must remain available, with occurrence and deduplicated views distinguished. Neither frequency nor a concern label authorizes a rename.
...
The distinction is visible in $P_q[v(T),a(T)]$: the outer symbol is a period, while the argument still uses absolute time. An interval $[T_0,T_0+P_{\mathrm{ret}}]$ retains its epoch. Probability, parity and permutation collisions use the approved cycle qualifier rather than overwriting their meanings. A dimensionless period in rescaled time remains distinct from its dimensional counterpart.

Historical serialized names keep their recorded spelling with current translations. A recorded cadence ratio or clock-period identifier can retain its old T-based key while current prose explains the corresponding P-based period ratio. An epoch, emission time, persistence duration, temperature or interaction interval must not be renamed solely because it contains T.
...
Source/output identity alone was insufficient for visual acceptance. Two equations clipped in the actual application because the centered container's scroll width omitted part of the formula extending before the scroll origin. The shared repair measured complete formula parts per row, treated fixed padding separately and rounded fitted sizes downward. It applied to the renderer rather than inserting an equation-specific exception.
...
Four motion quantities remain distinct: whole-assembly group motion, total native constituent motion, internal motion relative to a declared center and tangential orbital motion. The kinematic identity is

$$
\mathbf V_i=\mathbf V_{\mathrm{grp}}+\mathbf v_i^{\mathrm{int}},
\qquad
\|\mathbf V_i\|^2=\|\mathbf V_{\mathrm{grp}}\|^2+\|\mathbf v_i^{\mathrm{int}}\|^2
+2\mathbf V_{\mathrm{grp}}\cdot\mathbf v_i^{\mathrm{int}}.
$$

Only an appropriate orthogonality hypothesis removes the cross term. Stationary circular geometry can make total and tangential speeds coincide; radial motion, a moving center or nested rotation can break that identification. A fixed total speed is an additional branch hypothesis. Exceeding wake speed alone does not prove a positive-delay self-hit; the history must admit the root.

The deferred assessment found 108 non-group beta-f math spans in four corpus sources, plus fourteen in a reference audit. These are dated lexical and contextual observations, not 108 independent mathematical decisions. The uniformly moving transmitter is not an orbital case. Signed one-dimensional velocity must not be replaced by a nonnegative norm, and a maximum over retained history must not become an instantaneous speed.
...
One proposed angle destination already has a lattice occupant: $\boldsymbol\xi_{\mathrm d}$ denotes a polarity-differential displacement with length units. Other candidate spellings passed only initial lexical probes. Font differences and empty exact searches cannot settle a shared meaning or all indexed variants.

The angle issue itself contains two coordinates. A full half-delay can be defined by $\omega\Delta/2$, whereas a local partner coordinate satisfies a branch relation such as

$$
2\pi m+2\sigma\xi=2s\cos\xi.
$$

In this source chart $m$ is the winding index, $\sigma$ the orientation sign and $s$ the declared circular-speed ratio; their domains must travel with the relation. Dividing by two does not identify the local coordinate globally with full positive half-delay. Winding, orientation, branch domain and possible absolute-cadence conventions must be preserved. A pure rename can retain the equation; a normalization change needs further mathematics.

Shape normalization is another separate issue. With positive radii and transverse scale defined relative to its reference value, the declared shape and scale definitions give the volume ratio of corresponding ellipsoidal envelopes:

$$
\frac{V(v)}{V(0)}=\lambda^3(v)\frac{\xi(v)}{\xi(0)}.
$$

Dropping the reference-shape denominator requires the additional assumption $\xi(0)=1$. This geometrical statement neither derives a Lorentz relation nor authorizes correcting an original source during a notation migration. Pre-gauge phase offsets, post-gauge zero-sum coordinates, horizon or wave-flux generators and causal-writhe records similarly retain their distinct domains and definition gaps.
...
The data-layout work separated six initially simpler moves from twenty-six with code, test, contract or receipt consumers. Its scope included structured configuration, classification and contract files, scientific receipts, platform workloads and source archives. A move can change a literal link, a dynamically constructed basename or an input path embedded in a receipt. A clean top-level Markdown listing does not account for JSON or other data.
...
Historical evidence must remain attributable. A commit subject does not establish its contents; last touch does not establish the cause of a failure. To assign causality one needs the relevant expectation and the transition from the last matching state to the first mismatching state, including changes in the checker itself. A clean working copy of a source directory cannot prove that an unrelated failure predates a filing operation.
...
The distinction between a source stage, a generated-output stage, a rendered application and a released artifact also remains essential. A generator check establishes agreement with its inputs, not mathematical correctness or delivery. A local package rebuild does not update an installed reader. iOS and review-PDF exports follow their explicit on-demand boundaries, even when an older procedural table mentions them generically.
...
The [closure join](analysis/closure-join-matrix.md), [notation assessment](analysis/notation.md) and [recovery pilot](evidence/research-insight-recovery-pilot-2026-09-09.md) retain the detailed arguments and their source-time boundaries. Their scientific content remains useful precisely when the distinction between a proposed common structure and an established result is maintained.
