...
**Status — decision-only review completed, 2026-08-28:** Every entry in the approved notation table in Section 3.2 has a review disposition: **39 retain, 3 decision needed, 2 defer**. These are 44 table entries, including overlapping families, not 44 distinct symbols or an occurrence census. The three decision entries form one first priority: assembly group velocity versus internal motion. The two deferred entries concern xi shape/angle clearance. The time/period transition and independent suppression-alias cleanup remain complete in Sections 4.4 and 4.6. That review authorized no implementation. The subsequent operator approval authorizes the global terminology change to group velocity/group speed, preserving mathematical symbols and meanings; the unresolved symbol migration remains planning-only.
...
**Next planning step:** complete the selected group velocity/internal-motion assessment and its actual displacement dependencies. Xi clearance and general qualifier shortening are deferred. Do not ask for a replacement spelling until the selected family's inventory and full decision assessment are available. Each subsequent discussion should state the current phase, the decision being considered, what remains unresolved, and the next prerequisite; explaining a choice or answering a question does not authorize implementation.
...
| 21 | $v$ | 249 | 119 | Assembly group speed | Concern: assembly group velocity and internal orbit. | Known conflict — group velocity/internal orbit; extent and replacement destinations unmeasured. | Decision needed — first priority: group velocity versus internal motion. |
...
| 26 | $\beta_f$ | 209 | 111 | Group-speed/wake-speed ratio | Concern: group speed or orbital numerator. | Known conflict — group-speed/orbital ratio; existing uses of candidate s must be checked. | Decision needed — first priority: identify whose speed is in the numerator. |
...
| $\beta_X=v/c_X$, $\gamma_X=(1-\beta_X^2)^{-1/2}$ | Channel group-speed ratio; Lorentz factor | Use a declared channel $X$; do not silently use internal orbital speed for $v$. | Group-speed/orbital ratios and metric/comparison forms; clearance unmeasured. | Decision needed — same first-priority group velocity/internal-motion issue, not a new family. |
...
| ---: | --- | --- |
| 1 | Use the completed Section 3.2 retain/defer/decision review to select the connected assessment. The current first priority is group velocity versus internal motion. | One selected family and its actual dependencies; retained and deferred families are not a mandatory preliminary migration. |
