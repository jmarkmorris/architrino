...
| A1 and A3.3 owners | [A1 Dynamics](../../../content/markdown/aaa/noether-braid/braid-a1-dynamics.md): $v_a,\beta_a$ in circular-action and pinning charts, $s_a$ in the field-speed index set. [A3.3](../../../content/markdown/aaa/noether-braid/braid-a3-3-doubling-frequency-lock.md): $v_a=2\pi f_a r_a=\beta_a c_f$, carrier $v_h^{\mathrm{car}}$, and returned speed-factor rows | Preserve binary identity, tangential carrier meaning, instantaneous fluctuations, and cycle-averaged carrier values. An index a is not automatically an individual architrino index i. |
| Center-relative versus combined motion | [Braid Mathematics](../../../content/markdown/aaa/noether-braid/braid-mathematics.md), [A2 Symmetry and Return Response](../../../content/markdown/aaa/noether-braid/braid-a2-symmetry-and-return-response.md), [Planck-Scale A1 Geometry](../../../content/markdown/aaa/philosophy-history/theory-bridges/mapping-planck-scale-a1-geometry.md), and [Photon priorities](../app-photon/priorities.md) | Keep group motion, internal vector, tangential component, and total speed distinct. Preserve $v_{\mathrm{eff}}$, $v_{k,\mathrm{abs}}$, and declared projections until their role is mapped; “eff” in that geometry note is not a propagation-speed channel. |
...
| Active operator consumers | [Field-Speed Mathematics](../field-speed-ceiling/mathematics-geometry-dynamical-system.md), [Independent Causal-Wake State Analysis](../master-equation-closure/analysis-independent-causal-wake-state.md), [Undirected Ensemble Search](../braid-program/undirected-ensemble-search.md), [Dark-Sector Photon-Like Mode](../dark-sector/dark-sector-photon-like-mode.md), [Simulation priorities](../app-simulation/simulations.md), and specialist guidance | Separate native total speed, orbital speed, signed speed, and conventional counterexamples. The helical construction's axial u and transverse v are different components. Do not relabel its total-speed budget as either component. |
...
**Definition gaps, not rename permission:** broad threshold summaries do not always specify total native speed versus tangential carrier speed. A3.3 explicitly distinguishes a cycle-averaged carrier value from microscopic crossings; the existing s-a threshold set in A1 compares a time-dependent binary speed against $c_f$. These cannot be equated by a choice of letter. Keep those entries unresolved until their owner definitions supply the intended center, component, and averaging.
...
| $v_{\mathrm{orb}}$ and an orbital-qualified beta-f | Six math spans already use $v_{\mathrm{orb}}^{\mathrm{tan}}$ in Planck-Scale A1 Geometry. No exact $\beta_{\mathrm{orb},f}$ or $\beta_{f,\mathrm{orb}}$ was found in the scanned corpus/reference math, including text/roman qualifier variants. | **Low lexical conflict; definition still required.** | The existing tangential qualifier and geometric assumptions must survive. This family cannot absorb straight-moving transmitters or arbitrary total-site speed. No secondary eviction identified for those exact ratios; this is not clearance of all possible qualifier spellings. |
...
**Deferred resumption procedure, not the next task:** only if the operator explicitly reopens this connected migration, prepare the definition decision for indexed binary speed using the A1, A3.3, total-site, and translating speed-budget owners. Separate the unambiguous stationary-circle subset from summaries requiring a definition decision before preparing any transition option. The current scope permits only worthwhile, independent corrections and does not require completing this assessment's remaining definition work.
