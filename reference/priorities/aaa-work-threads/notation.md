...
**Status — 2026-08-28:** The separately approved independent suppression-alias cleanup in Section 4.6 is complete, including source/output checks and app visual acceptance after the authorized shared fitting fix. Broader notation implementation remains paused at the operator's direction. The previously approved time/period transition and its Section 4.4 receipt remain intact; this reset does not authorize a rollback. Section 4.5 contains the expanded xi assessment, including competing destinations and uses outside app-equation, but is not ready for a broader migration. The mathematical guides remain the canonical definitions; this file records planning, decisions, scope, and transition evidence. Planning approval, notation approval, source-edit approval, and generated-output approval are distinct.
...
**Completed scoped exception:** the operator subsequently requested completion of one fully checked independent cleanup, following the specific source/output proposal in Section 4.6, and then explicitly approved the shared app equation-fitting fix needed for visual acceptance. That batch covers the stated chapter edit, its regression tests, affected web regeneration, and shared fitting correction. It is complete. No further symbol family or unrelated renderer change is authorized by that exception.
