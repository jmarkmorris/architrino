...
|   15 | Op queue      | [`chapter-authoring`](../chapter-authoring/chapter-authoring.md)                                  | Unified chapter authoring queue and source-material mining                                    |  4.0 |    1.10 |      -27 |     1.27 |   0.75 |  4.19 |    4 |         2 |    3 |     2 |  2.7 | 1.55 |
...
- Workstream `simulations`: moves into the top-three as the fastest way to discipline the equation engine. The campaign object and $A_0$ protocol surface now carry classified and event-locked self-root `fold-layer` diagnostics, accepted-history source coverage, a fail-closed validation observer, and a direct fold-layer-locked one-period runner. The May 18 compact-fixture run executed `963815` retained steps without trajectory abort, then failed state/root/phase/speed/drift/energy-like residual closure while preserving the lock ledger. The next simulation work is a corrected branch-equation attempt driven by the residual-balance ledger, not another replay or budget-reduction pass.
...
2. Close the current breather pre-ledger obstruction: the candidate packet, mesh, refined null-coordinate causal ledger, diagonal-exclusion subledger, kinematic fold-layer atlas, and pre-ledger interval report now exist, but branch-chart work is blocked until the four separator layers have finite dual-mollified fold impulse ceilings and the remaining parent-boundary rows are partitioned into accepted empty, simple-root, or fold-layer rows.
