...
- Workstream `angular-momentum-spin`: the May 20 packet set turns the workstream from broad derivation into certificate population. It now has a branch-chart angular-momentum evaluation object, a symbolic minimal four-substep certificate, a finite candidate-set and branch-selection law target, a null spinor-holonomy control table, causal-writhe gauge controls, photon Gate B substrate residuals, Stern-Gerlach-like apparatus diagnostics, pair-provenance source scaffolds, Bell residual handoff gates, and a priority-only $(O,M,I)=(f-1,f,f+2)$ middle-hinge offset candidate with a reduced solver comparison, branch-chart projection, sampled active-row lineage, phase-at-hit payload, same-row torque/wake diagnostic payload, selected fixed-receiver time-window torque probe, older work-integral wake reconstruction, selected binary-to-binary path-history roots/hits, and replayed binary-to-binary root-ledger-detail rows against the symmetric $(f-1,f,f+1)$ control inside the tri-binary partition rule. The next angular-momentum solver work is transition classification and retained row-set identity over the replayed binary-to-binary root-ledger-detail rows, then a retained active-row evaluation over $W$ that resolves the nonzero middle/inner fixed-receiver torque residuals, nonzero no-boundary wake reconstruction, inner active-root competitors, and normalized action-kernel wake boundary charge on those same rows, followed by one retained Noether swarm branch chart plus one native analyzer or apparatus model.
...
6. Advance the angular-momentum work by populating one retained branch-chart certificate and one native analyzer or apparatus model, carrying the reduced-solver $(O,M,I)=(f-1,f,f+2)$ middle-hinge offset comparison against the symmetric $(f-1,f,f+1)$ control into root, phase, wake, torque, stability, recoil, and event-ledger residual rows before promotion.
