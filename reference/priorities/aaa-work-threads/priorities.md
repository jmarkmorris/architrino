...
2. Close the current breather pre-ledger obstruction: produce the finite fold impulse ceilings $I^{\mathrm{fold}}_{\eta,\epsilon_c,\Sigma}$ for $\Sigma_1,\ldots,\Sigma_4$ from a dual-mollified acceleration bound $A_{\Sigma,\eta,\epsilon_c}$ and transversality constant $C_\Sigma$; then promote the six fold-adjacent parent-boundary leftovers into accepted rows, or reject the cosine template before branch-chart work.
