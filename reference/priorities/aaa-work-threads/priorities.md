...
## Historical Audit Scope — 2026-07-26
...
This review updates inferred attention scores from the live bucket queues and preserves the existing scientific acceptance boundaries. It does not rescore the theory closure scorecard. The pressure inputs remain the latest dated June 28 assessment in that owner; later editorial work is not a new scientific assessment.

**Master-Equation Closure is now rank 1; Braid Program is rank 2.** Braid's prior cost of 4.3 understated the current BP-001 blocker: its own queue still requires certified root completeness through close approach and the longer transverse path before the frozen campaign can classify a fate. Remaining burdens are now Exec 7, Intuition 7, Deps 8 and Valid 9, giving Cost 7.6 and ROI 7.27 instead of 12.86. Value remains 55.28: no completed instrument work is newly credited. These burden numbers are judgments, not measured runtime or probabilities. Reassess them when the certified full-path prerequisite is supplied or a shorter accepted fate route is demonstrated.

The Braid local winner remains BP-001 after reviewing its ranked queue: the breather and teaching-app rows retain their dependencies, prescribed circular and Platonic diagnostics do not themselves supply a retained assembly, and fresh-generation campaigns retain full-history acceptance burdens. This preserves its local order without treating blocked rows as executable. The Master-Equation local winner remains the causal update obligation; its accepted finite smooth-feedback and separation results narrow specific controls, but do not complete the general wake-state update or conserved accounts. No new worker is dispatched by this ranking.

The remaining twelve rows retain their component scores: this review found no accepted replacement of their named winner that warrants transferring completed value into a new score. Their retained conditions are summarized below. All derived columns are recomputed and tracker Rank, Value, Cost and ROI metadata are synchronized. Arithmetic is verified by the ranking validator; adequacy of the attention estimates remains an inference that the linked owner can overturn.

| Bucket or packet | Remaining condition behind the retained score |
| --- | --- |
| Transfer operator and basin measure | Requires a persistent accepted assembly state and an invariant measure; formal state-space definitions are not that state. |
| Pressure-dependent Noether sea response | Requires one accepted branch and same-record causal accounts; static diagnostic results do not supply the constitutive response. |
| Equation mapping | The Lorentz envelope still requires the score-5 ladder, retained history and same-record moving/rest comparison. |
| Standard Model mapping | First geometry-derived quark mass row remains open, without observed-mass fitting. |
| Exposure quotient | Requires the accepted branch-to-sector quotient and null-sector bound named in the scored object. |
| Strong-field mapping | Observer predictions still require an embedded horizon-interface record. |
| Benchmark mapping | The preferred-frame row remains blocked on a moving branch and clock/ruler export. |
| Residual routing | An actual EOM-evolved transition and closed event account remain required. |
| Quantum mapping | Detector recovery remains blocked on the assembly basin measure; the separate learning task is not physical recovery. |
| EOM application | The queued bounded-population application surface remains useful integration work; completed solver capabilities do not make it an upstream derivation. |
| Photon app | Local propagation mapping remains dependent on a source-carried Noether sea response. |
| Borg app | The optional taxonomy morph packet remains deferred; no scientific classification follows from teaching geometry. |
...
- **Research insight recovery, approved follow-up complete:** the [assessment and completed integration](evidence/research-insight-recovery-pilot-2026-09-09.md#approved-pressure-packet-integration) preserve developed documents and topic collections while clarifying the Lattice Lab cancellation result and FSC disposition. The two approved pressure-packet paragraphs now make explicit that the small static pressure test does not settle higher-order or periodically driven response, and an isolated diagnostic does not settle collective transport. The accepted retained-state blocker remains open; no scientific queue, rank, promotion, relocation, or consolidation changes.
- **Operator-selected learning:** [QC-013 — Hilbert space and braid mapping](../mapping-quantum/work-queue.md#qc-013--hilbert-space-and-effective-state-vector-contract) is in progress in [mapping-hilbert/](../mapping-quantum/analysis/hilbert-space-and-braid-mapping.md), with an academic synthesis maintained as dialogue develops. Bring it back at the next operator priority review or quantum learning session. Begin with ordinary vectors and build toward the braid mapping; no prior Hilbert-space understanding is assumed. This selected learning task does not change the physical-recovery ranking.
...
- **Measured/derived:** `braid-program` has a ratified charter and instrument gate, a fully constructed 243-row Campaign 1 workload, one diagnostic transverse inward-to-outward reversal, and no booked physics fate. **Inferred rank claim:** it ranks second for its direct assembly payoff, but its remaining cost now includes the unresolved certified-root-completeness work before the campaign can book a fate. **Falsifier:** demote it if Campaign 1 cannot run under a source-frozen EOM/instrument contract or another lane obtains a shorter accepted route to a persistent assembly.
