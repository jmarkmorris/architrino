...
| U3 | [`cross-theory-mapping`](../cross-theory-mapping/cross-theory-mapping.md) | Can remain benchmark mapping, requirement classification, and acceptance-predicate drafting until one case is promoted. |
| U4 | [`dark-sector`](../dark-sector/dark-sector.md) | Can remain a watchlist and hypothesis-routing registry until a release-channel or event-ledger packet becomes concrete. |
| U5 | [`strong-field hypothesis bank`](../strong-field-closure/hypothesis-bank.md) | Can stay as a watchlist under `strong-field-closure`, separate from quantitative closure. |
| U6 | `deferred archives` | Archive indexing and parking-lot cleanup are organizational. Legacy product/outlook notes now route through [`source-mining`](../source-mining/source-mining.md). |
| U7 | [`3x3`](../deferred/3x3/3x3.md) | Deferred binary-slot matrix notes can remain parked unless a target corpus destination is chosen. |
| U8 | [`phenomenological-heuristics`](../dyadic-lock/phenomenological-heuristics.md) | Archive/source material for dyadic lock can stay sibling reference material rather than an active reduced-map proof. |
...
| U4 | `dark-sector` | [dark-sector.md](../dark-sector/dark-sector.md) | Speculative dark-sector candidate assembly lane for release, transport, redshift, reaction, and visible-channel re-entry hypotheses. | Priority candidate under watchlist review; promote only packets with a boundary condition, event ledger, redshift/re-entry gate, or discriminating observable. |
| U5 | `strong-field hypothesis bank` | [hypothesis-bank.md](../strong-field-closure/hypothesis-bank.md) | Strong-field and Noether swarm hypothesis watchlist. | Merged under `strong-field-closure`; not a ranked top-level workstream. |
| U6 | `deferred archives` | [priorities-legacy.md](../deferred/priorities-legacy.md) and sibling deferred notes | Archive and parking lot for non-current material; legacy product/outlook notes now route through [source-mining](../source-mining/source-mining.md). | Needs a deferred index later. |
| U7 | `3x3` | [3x3.md](../deferred/3x3/3x3.md) | Deferred binary-slot matrix note. | Keep deferred unless a target $\mathbb{A}\mathbb{A}\mathbb{A}$ document is selected. |
| U8 | `phenomenological-heuristics` | [phenomenological-heuristics.md](../dyadic-lock/phenomenological-heuristics.md) | Dyadic resonance lock archive source. | Keep as sibling archive unless a shared archive convention is adopted. |
...
- Workstream `angular-momentum-spin`: the May 20 packet set turns the workstream from broad derivation into certificate population. It now has the priority-only $(O,M,I)=(f-1,f,f+2)$ middle-hinge offset candidate tracked against the symmetric $(f-1,f,f+1)$ control in the new solver path: reduced solver comparison, branch-chart projection, sampled active-row lineage, phase-at-hit payload, same-row torque/wake diagnostic payload, selected fixed-receiver time-window torque probe, binary-to-binary path-history roots/hits, root-ledger transition classification, retained hit-time coverage, boundary-only hinge-point witnesses, point-event admissibility rows, a $\pi/2$ topology branch-transport pair-map, middle field-speed hinge-capture diagnostic, zero-slack retained-chart feasibility diagnostic, compensated retained-chart payload inventory, hinge-event row-set identity, hinge-root interval enclosure, hinge-root branch-transport route feasibility, compensated route-payload certificate, middle emission-clock jump diagnostic, and partial retained chains.
  ...
  The next angular-momentum solver work is wake completion on the same route-authorized retained event: keep using the new solver path to declare and evaluate the transaction-frequency/action-scale route that could turn the accepted boundary charge into wake energy, or evaluate the action-boundary $\partial_{t_1}$ kernel directly on the accepted route rows. Partition, section stability, retained phase, and retained torque rows remain downstream of that wake completion. Alternately, an accepted retained point-event rule or a geometrically continuous branch-transport pair-map could bypass the current compensated-route blocker. A single-f retained scan over f=3 through f=8 still does not rescue the family: f=3 and f=5 through f=8 have no common retained hinge point, while f=4 is one-sided-count blocked.
...
6. Advance the angular-momentum work by using the complete reduced-solver $(O,M,I)=(f-1,f,f+2)$ middle-hinge route payload to close the `wake_energy_increment_target_populated_action_boundary_law_missing` row, or by replacing that path with a geometrically continuous retained branch chart or accepted retained point-event rule, then carry that chart against the symmetric $(f-1,f,f+1)$ control into root, phase, wake, torque, stability, recoil, and event-ledger residual rows before promotion.
