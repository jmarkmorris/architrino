...
6. Advance the angular-momentum work by populating one retained branch-chart certificate and one native analyzer or apparatus model, carrying the $(O,M,I)=(f-1,f,f+2)$ middle-hinge offset candidate as a priority comparison against the symmetric $(f-1,f,f+1)$ control, then promote only rows whose root, phase, wake, torque, stability, recoil, and event-ledger residuals pass together.
