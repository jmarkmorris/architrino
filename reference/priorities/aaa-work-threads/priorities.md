...
- Workstream `master-equation-closure`: now ranked first because the priority surface is explicitly equation-engine-first. The newly exposed action-kernel issue is the delayed-interior characteristic-tail Noether closure: endpoint normalization, receiver-gradient cancellation, and energy/momentum/angular-momentum boundary increments must close before downstream conservation ledgers can treat the kernel as more than a diagnostic scaffold.
...
- Workstream `simulations`: moves into the top-three as the fastest way to discipline the equation engine. The campaign object and $A_0$ protocol surface now carry classified and event-locked self-root `fold-layer` diagnostics plus accepted-history source coverage for the fold-layer-locked attempt; the next work is adaptive one-period Tier 1 continuation with explicit residual, drift, monodromy, and failure-code outputs.
- Workstream `mass-map`: the $A_0$ Tier 0 scaffold now emits quotient coordinates, residuals, blocked accepted-history output, weak-retained fail-closed behavior, weak-emitter fail-closed behavior, direct-root branch retention, branch surplus tracking, an event-local fold-layer lock, and a source-coverage-passing fold-layer-locked attempt packet. The active calculation is the adaptive fold-layer-locked one-period continuation, not surplus classification or a new coarse reduced scan.
...
1. Drive `master-equation-closure` first: close the delayed-interior characteristic-tail Noether boundary issue, then extend the tractable master-equation stack until Lorentz / GR bridge work, quantum closure, and first-principles mass work have equations rather than only routing language.
...
4. Use the source-coverage-passing fold-layer-locked $A_0$ one-period attempt packet to compute residuals, drift, monodromy, and eta-ladder persistence; the macro-stride attempt now plans all ready rows under the $10^6$ cap, but accepted-history remains blocked until those one-period validation predicates are actually computed.
