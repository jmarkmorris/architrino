...
4. Use the classified $A_0$ `fold-layer` rows to build a one-period adaptive direct-root Tier 1 continuation attempt with explicit step-budget control, residuals, drift, monodromy, and accepted-history blocking checks; the current fail-closed intake blocks because the estimated step burden remains about $2.93\times10^7$ to $4.62\times10^7$ steps per period.
