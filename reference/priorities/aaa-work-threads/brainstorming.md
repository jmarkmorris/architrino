...
| --- | --- | --- |
| [Spinors, rotations, and history](../mapping-quantum/spinors-rotations-and-history/brainstorming.md) | Work through one full rotation and two full rotations, then distinguish a mathematical sign from a physically detectable relative phase. | Mathematical learning and an existing recovery target; promote only a separately justified result to the angular-momentum/spin owner. |
| [Gauge, geometric phase, and holonomy](../mapping-standard-model/geometric-phase-and-holonomy/brainstorming.md) | Compare one change of phase convention with one closed transport cycle, identifying the invariant observable and the missing substrate carrier. | Effective comparison; the assembly gauge chapters retain mechanism and recovery ownership. |
| Information, entropy, and observer access; [entropy mapping](../mapping-benchmarks/benchmarks/entropy.md) | Compare a complete source-history description with a declared observer's accessible record and identify exactly what the projection loses. | Effective comparison or conditional mathematical example; no assumed invertibility, entropy-production law, or wake-account closure. |
