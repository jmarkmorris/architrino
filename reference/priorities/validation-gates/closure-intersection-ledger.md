...
| Benchmark-only fitting | A target benchmark in $\mathfrak{B}^{\mathrm{obs}}_S$ is used as an input to $\mathcal{L}_A$, $\Pi_S$, $Q_S$, $\mathcal{R}$, $\{B_i\}$, or $\mathcal{M}_{\mathrm{sea}}^{ab}$ rather than as an output of a replayable closure record. | [mass-map](../braid-mass-response-map/braid-mass-response-map.md), [quantum-closure](../quantum-closure/quantum-closure.md), [braid](../braid-retained-branch-closure/braid-retained-branch-closure.md) |
