...
The app should make the core geometry legible at a glance while leaving room for meters, orthogonal views, and modeling overlays. The page should avoid unnecessary jargon in both design and code. When a stable $\mathbb{A}\mathbb{A}\mathbb{A}$ term already exists, reuse it rather than inventing a new label.
...
- Show the Noether core inside that stage as a deformable ellipsoidal body rather than as a sphere.
...
- The Noether core is rendered inside that spherical stage and can be viewed from different directions without surrendering the central space to surrounding panels.
...
- In the default screen composition, keep the displayed major-axis diameter as the vertical span of the core view.
- Let only the minor-axis diameter shorten as translation velocity increases.
- Keep the major-axis radius fixed while the ellipsoid becomes more oblate.
- Do not make the central view read as if the whole core is simply getting smaller; let flattening carry the main geometric signal.
...
## Core Views And Interaction

- Allow the user to rotate the core freely to any view.
- Provide preset views for front, side, and top, plus a general orbit camera.
- Treat the front view as the view down the minor axis unless later geometry work changes that convention.
- Allow the user to toggle axis lines on and off.
- Support two or three orthogonal companion views when the screen has enough room.
- Animate the orbits, the path, and the architrinos continuously.
- Normalize the inner motion as needed so the central visual emphasizes changing shape, scale, and state rather than only six architrinos whipping around at raw literal speeds.
...
- Use that table for repeated binary-level quantities such as major radius, minor radius, orbit frequency, architrino velocity, and energy level.
- Render those table values in compact scientific notation so dense quantitative comparison remains readable.
- Show an effective occupied-volume readout derived from the current radii and group velocity.
- Show group velocity as an assembly-level readout associated with the binary table, not as a per-binary cell value.
- Use the surrounding readouts, graphs, and table values to communicate minor-axis change and the resulting volume change, since the central view is normalized to emphasize flattening.
...
- That includes view presets, axis visibility, orbit visibility, graph selection, equation-panel visibility, scale-reference selection, and test-point visibility.
...
- The default first-pass rows should include major radius, minor radius, orbit frequency, architrino velocity, and energy level.
...
- Use the left side band for always-visible summary readouts anchored by group velocity and occupied volume.
...
- Treat any above-threshold behavior as a separate geometry mode with its own controls and presentation rather than as a blind continuation of the sub-threshold slider.
...
- Show the main axis lines when requested.
- Use standard oblate-spheroid terminology for overlays and readouts: principal axes, equatorial radius, polar radius, equatorial diameter, polar diameter, equator, poles, and meridians.
...
- Treat low and moderate group velocity as visually close to spherical, with the strongest visible deformation concentrated near a significant fraction of the field-speed threshold.
- Treat the above-threshold regime as a distinct geometry domain rather than as an ordinary continuation of the sub-threshold deformation curve.
- In that distinct geometry domain, prioritize the visible shape transformation first.
- In both domains, prioritize flattening over apparent overall shrinkage in the main core view by holding the major-axis diameter fixed in the display while the minor axis changes.
...
- Keep a short geometry reference available either inline or in a companion markdown panel.
- That reference should cover the general ellipsoid, the special oblate-spheroid case, and the terminology for the axes.
- Use [ellipsoid.md](ellipsoid.md) as the companion note for the broader geometry and effective-metric context.

## Runtime Architecture

- Implement the main core model in Python.
- Use Python for the core state, geometry calculations, derived measures, and the quantitative data that feeds the readouts, graphs, and tables.
...
- Treat the Python side as the canonical source for the detailed binary table values, group velocity, occupied volume, and other derived quantitative outputs.
...
- Reuse the animator visual language from the JavaScript side where it helps, but keep the ellipsoidal core logic owned by the Python model.
...
- Should the middle and inner binary frequency changes be shown directly, or only implied through the changing geometry?
...
- Extend the combined system toward ellipsoidal deformation, axis overlays, companion views, and meter readouts in that order.
