...
| Source-normal denominators | Small $D_{s,ij}$ amplifies the receiver-normal branch factor $W_{ij}^{\mathrm{rec}}=\lvert D_{T,ij}/D_{s,ij}\rvert$ and magnifies numeric error. | Use small-Jacobian diagnostics, interval or extended precision paths, and explicit branch-weight authority. |
