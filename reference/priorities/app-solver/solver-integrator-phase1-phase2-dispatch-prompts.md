...
- **O4 Sub-$c_f$ circular falsification (theorem-anchored).** `binary-dynamics.md:682-717` proves a strictly sub-$c_f$ constant-speed circular orbit is impossible (partner row anti-damped). Seed the analytic circular binary; the oracle asserts the integrator **reproduces the predicted anti-damped spiral** with the correct tangential-work sign — a "held circle" is an automatic FAIL.
