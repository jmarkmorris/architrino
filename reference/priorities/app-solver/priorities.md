...
2. The native central solver is the production solver for architrino motion, causal roots, delayed hits, path histories, wake history, T3 stepping, and solver-owned geometry.
3. Do not create a new production solver, parallel solver stack, app-local solver, or alternate default engine. Missing capabilities must extend the central solver contract, native implementation, bridge schema, and validation fixtures instead of creating another solver.
...
8. Geometry calculations should be centralized around solver-owned simulation and geometry routines for Photon, Ideal Braid, Animator, and the central solver.
9. The solver must offer excellent precision across many orders of magnitude. Orbital speed and assembly speed may span many orders of magnitude, so dynamic range, numerical conditioning, precision paths, numeric charts, and explicit error budgets are core design requirements. The detailed policy lives in [precision.md](precision.md).
10. Every run should declare a [simulation envelope](simulation-envelope-contract.md): entity count, physical scale, simulated duration, time resolution, interaction density, branch complexity, precision claim, output detail, memory budget, storage budget, and expected latency. The solver should admit, simplify, batch, escalate, or reject a run based on that envelope instead of quietly accepting a request it cannot support accurately.
11. Error budgets must propagate through root solving, motion integration, stream encoding, readback, projection, and app-facing buffers so every consumer knows whether a value is authoritative, approximate, or display-only.
12. The solver must be high-speed and efficient enough to support interactive apps, batch simulations, and reusable diagnostics. The implementation language and runtime are therefore first-order design decisions, not afterthoughts.
13. The solver should be highly parallelizable where it improves performance and makes engineering sense. Threading must be benchmark-driven, deterministic where the result requires it, and optional where single-threaded execution is safer or sufficient. Even while GPU execution is deferred, work units, data layout, and reduction rules should be designed so future GPU kernels can map onto mainstream laptop, desktop, browser, and service GPU hardware.
14. Solver work and dense data should be organized into transport-ready packets where practical. A packet should be able to move across browser workers, native threads, native processes, network services, and future GPU or service backends without inventing a second data format.
15. Path histories can consume memory quickly. The solver needs per-path data streams that keep only a bounded active window in memory, spill high-speed path data to non-volatile storage, and read those records back at high speed.
16. Path-history streams need explicit indices and metadata: path id, time range, frame range, byte offsets, precision path, units, scale normalization, schema version, checksums, provenance, and diagnostic summaries. They should also capture inexpensive derived geometry and frame data when it is cheap to gather, because future solver speedups will likely come from fast spatial and causal queries over this recorded history.
17. Path-history indexing should be tiered. The hot active window should keep only the indices needed to continue precise action, root, and geometry modeling over the next simulation window. Aged-out records should spill to non-volatile storage, where deeper offline indices can be built later for long-term studies, huge simulations, replay, and research.
18. Hot-path data structures should use compact canonical encodings that are straightforward to implement and verify. Compression should usually happen after a run or during export; the solver loop should write efficient canonical chunks first and add hot-path compression only when benchmarks prove it helps.
19. Apps must be able to communicate with the solver through one shared app bridge. App code should call a stable JavaScript or TypeScript request/response API and should not need app-specific C++ or WebAssembly handling.
20. The repo needs an isolated baseline-comparison sandbox for the current app-facing solver paths. The sandbox must run fixed Photon, Ideal Braid, and Animator baseline cases with resource caps, no network access, fixed seeds, controlled working directories, artifact-only output, and no writes back into app source paths. Differences are classified as `baseline_within_tolerance`, `baseline_refined_result`, `baseline_model_boundary_difference`, or `baseline_investigation_required_mismatch`.
21. New solver validation must include analytic fixtures, manufactured solutions, and invariant checks in addition to existing-baseline comparisons.
22. Once the solver design and first implementation exist, the repo needs a migration plan for the current app-facing migration targets: Photon, Ideal Braid, and Animator.
23. The solver workstream needs retrospective impact audits for expensive proof and research paths when those paths depended on root solving, path histories, precision, or ledger diagnostics. The first audit target is H39: select representative cases, replay the solver-relevant parts through the central solver when available, and classify whether the central solver would likely have produced no material effect, the same result faster, a refined result, a changed bottleneck, or an investigation-required mismatch.
...
The active app-facing solver scope contains Photon, Ideal Braid, and Animator.
...
The cross-priority solver requirements come from known app priorities, simulation priorities, geometry bridge, braid, mass-map, photon closure, master-equation closure, and proof-program sources that request solver-like behavior.
...
- Add assembly topological charge reporting for branch searches that claim Noether braid assembly-sector evidence: $N_s$, $M_p$, signed root-complex side data, phase-bundle $c_1=(m,n)$ when a Noether braid phase chart exists, gluing status, and stability-gap status, following [Noether Braid Topological Charge](../../../content/markdown/aaa/noether-braid/noether-braid-topological-charge.md).
...
18. App adapters for Photon, Ideal Braid, and Animator during migration.
19. Test fixtures or benchmark scenarios that compare app bridge behavior against golden solver cases before app code is simplified.
...
| Run baseline comparisons | Isolated baseline sandbox runs for Photon, Ideal Braid, and Animator, with diff reports that classify differences as `baseline_within_tolerance`, `baseline_refined_result`, `baseline_model_boundary_difference`, or `baseline_investigation_required_mismatch`. Legacy notes may clarify the boundary, but they do not create migration parity gates. |
...
The detailed precision and dynamic-range policy now lives in [precision.md](precision.md). This includes the numeric-chart strategy, log-magnitude and signed-log chart discussion, standard floating-point limitations, error-budget propagation, escalation rules, validation replay, and the renormalization relationship.
...
This requirement does add a deliberate bridge layer. It should not add special handling in each app. The correct abstraction is one shared app bridge plus focused app adapters for Photon, Ideal Braid, and Animator.
...
| `cancelRun` | Cancel a pending or active run by `runId` or `requestId`, return a normalized cancellation status, mark stored run manifests as cancelled when available, and release stream handles only when explicitly requested. |
...
| `root_ledger.v1` | One row per retained, inactive, separator, or transition root row. | `receiverId`, `sourceId`, `rootId`, `rootKind`, `hitTime`, `emissionTime`, `delay`, `distance`, `residual`, `sourceNormalDenominator`, `receiverNormalNumerator`, `receiverNormalFactor`, `unsignedReceiverNormalFactor`, `branchWeight`, `jacobian`, `jacobianSign`, `receiverNormalStatusCode`, `statusCode`, `firstFailureCode`. | Photon diagnostics, validation replay, proof handoff, delayed-hit reconstruction. |
| `delayed_hit_events.v1` | One row per delayed-hit event. | `eventId`, `rootId`, `emitterId`, `receiverId`, emission point, receiver point, unit direction, `sourceNormalDenominator`, `receiverNormalNumerator`, `receiverNormalFactor`, `branchWeight`, `jacobian`, `jacobianSign`, `receiverNormalStatusCode`, `statusCode`. | Animator delayed-hit rendering, Photon hit sums, Ideal Braid potential views. |
| `field_shell_events.v1` | One row per field-shell emission event. | `eventId`, emission time, status code, sign, emission point, field speed, display radius, display time, cadence interval, cadence index, emitter hash, display/emission strength, lifetime. | Animator field-shell cadence streams, delayed-hit emission descriptors, durable stream readback. |
...
Numeric serialization rules are defined for every `numericType` in [numeric-serialization-contract](numeric-serialization-contract.md). The schema defines byte order, signedness, scale factors, exponent layout, limb order, interval endpoint convention, rounding mode, NaN and infinity policy where applicable, comparison semantics, and text export representation. `decimal128` and `mp_limb_block` are not acceptable as labels alone.
...
The completed survey and first-prototype benchmark evidence live in [spatiotemporal-query-algorithm-survey](spatiotemporal-query-algorithm-survey.md). Its selected first prototype is a hybrid of interval-tree time slabs, spatial-hash receiver cells, and emission-shell annulus rows, benchmarked as `emission-shell-broad-phase-v0` against brute-force chunk replay.
...
- lifecycle summaries with tier counts, action counts, bytes by tier, unsafe-to-age-out chunk ids, and deep-index work queues;
- lifecycle apply metadata that persists the latest retention plan into stream manifests, plus guarded full-stream cleanup when every chunk is planned for deletion;
...
| --- | --- |
| Browser app runtime | Package `solver.wasm`, an ES-module WebAssembly loader, `solver-worker.js`, shared JavaScript adapter, shared app-side client resolver, TypeScript declarations, schema files, and an artifact manifest. |
...
- circular and helical orbit geometry used by braid candidates;
...
- solver owns canonical numeric geometry used by Photon, Ideal Braid, and Animator migration targets;
...
| [Simulation envelope contract](simulation-envelope-contract.md) | Admission tests for entity count, volume, duration, time resolution, interaction density, branch complexity, output detail, memory budget, storage budget, latency target, simplification policy, and `simulation_envelope_exceeded` halts. |
...
Before app migration, build these focused fixtures and keep each one small enough to run in local CI. The detailed analytic and invariant validation plan lives in [analytic-and-invariant-validation](analytic-and-invariant-validation.md).
...
| `ideal_braid_potential_samples_bridge` | Ideal Braid `computePotentialSamplesWithSolverBridge` and `solveFlightTimeRowWithSolverBridge` cases. | Batched delayed-potential rows, flight-time delays, emission times, geometry status, and artifact hashes. | Bridge rows match declared geometry baselines and the app can refresh surface samples through the shared bridge. |
| `ideal_braid_circular_self_hit_span_bridge` | Ideal Braid `solveCircularSelfHitSpanRowsWithSolverBridge` cases. | Batched self-hit span rows, regime labels, root status, span geometry, and diagnostics. | Span endpoints, delay windows, and root status match declared geometry baselines, and path-profile views consume solver rows rather than direct self-hit solving. |
...
| `emission_shell_broad_phase_query` | Synthetic emitter/receiver paths replayed from path-history chunks, with path-count, time-slab, density, same-source, all-to-all, and speed-regime transition sweeps. | Interval-time-slab rows, spatial-hash receiver-cell rows, emission-shell annulus rows, indexed candidate pair lists, brute-force replay oracle, sampled narrow-phase roots, deterministic work-packet headers with `emission_shell_candidate.v1` outputs, and app-bridge `indexOptions` fixture coverage. | Implemented as `emission-shell-broad-phase-v0`: solver-owned indexed broad phase finds every brute-force oracle candidate, reports zero missed candidates, records candidate reduction, false-positive ratio, same-source candidates, speed-regime transition candidates, packet replay equality, and app-facing `native_c_abi_indexed_v0` execution. |
...
| Work packets | Solver work should be divisible into transport-ready packets with explicit ranges, buffer references, checksums, expected outputs, and deterministic merge keys. The same packet layout should serve workers, threads, services, and future GPU backends, as defined in [work-packet-transport-contract](work-packet-transport-contract.md). |
...
| Numeric serialization | Every numeric type has explicit byte order, scale, exponent or limb layout, interval convention, rounding mode, comparison semantics, and export representation, as defined in [numeric-serialization-contract](numeric-serialization-contract.md). |
| Storage | Logical per-path streams are backed by a run-level chunked binary store, manifest, event store, encoding dictionary, binary index sidecar, deep-index store, and summary record. Browser and native storage define retention, cleanup, quota pressure, failed-run cleanup, export, and deletion behavior, as defined in [storage-lifecycle-policy](storage-lifecycle-policy.md). |
...
6. Use [numeric-serialization-contract](numeric-serialization-contract.md) for every declared numeric type.
7. Use [storage-lifecycle-policy](storage-lifecycle-policy.md) for storage retention, cleanup, quota pressure, failed-run cleanup, export, deletion, active-window age-out, and deep-index policy.
8. Use [work-packet-transport-contract](work-packet-transport-contract.md) for packetized solver work and keep the round-trip fixture coverage green.
9. Prototype the selected emission-shell hybrid from [spatiotemporal-query-algorithm-survey](spatiotemporal-query-algorithm-survey.md) and benchmark it against the emission-shell broad-phase fixture.
...
The migration plan for Photon, Ideal Braid, and Animator follows these steps.
...
8. Build a minimal benchmarked C++/Clang solver core and verify Photon, Ideal Braid, and Animator bridge fixtures across ordinary and many-orders-of-magnitude orbital-speed and assembly-speed scale sweeps.
...
14. Run the baseline-comparison sandbox against fixed Photon, Ideal Braid, and Animator cases, and classify each difference as `baseline_within_tolerance`, `baseline_refined_result`, `baseline_model_boundary_difference`, or `baseline_investigation_required_mismatch`.
...
18. Keep Ideal Braid delayed-potential and self-hit calculations on shared geometry and causal-delay routines.
...
- `model_contract` - Closed as a model-contract closeout in [model-contract](model-contract.md). The app schema, TypeScript surface, native admission path, claim-level precision floor, precision compatibility failure, and run manifest hashes define the run-level model declaration.
- `precision_dynamic_range_contract` - Closed as a standalone priority contract in [precision-dynamic-range-contract](precision-dynamic-range-contract.md). Runtime validation is covered by completed [cpp-clang-runtime-validation](cpp-clang-runtime-validation.md); app bridge consumption is covered by completed [app-bridge-contract](app-bridge-contract.md). Future scoped implementation work may extend validation replay breadth, stricter arithmetic kernels, and migration parity evidence.
- `simulation_envelope_contract` - Closed as a standalone priority contract in [simulation-envelope-contract](simulation-envelope-contract.md). Stream implementation and validation are covered by completed [path-history-stream-contract](path-history-stream-contract.md), app bridge consumption is covered by completed [app-bridge-contract](app-bridge-contract.md), and future scoped work may broaden `simulation_envelope_admission` fixture coverage.
- `virtual_observer_path_record_contract` - Closed as a standalone priority contract in [virtual-observer-path-record-contract](virtual-observer-path-record-contract.md). Path-history schema coverage is captured in completed [path-history-stream-contract](path-history-stream-contract.md); future scoped work may add non-linear segment fixtures and app playback refinements.
- `temporal_assembly_graph_contract` - Closed as a standalone priority contract in [temporal-assembly-graph-contract](temporal-assembly-graph-contract.md). Future scoped work may broaden assembly graph fixtures, hierarchy replay validation, and app bridge surfaces that expose assembly state.
- `numeric_serialization_contract` - Closed as a standalone priority contract in [numeric-serialization-contract](numeric-serialization-contract.md). Stream manifests are governed by completed [path-history-stream-contract](path-history-stream-contract.md), app bridge validation is covered by completed [app-bridge-contract](app-bridge-contract.md), and future scoped work may broaden validation replay artifacts.
- `path_history_stream_contract` - Closed as a standalone priority contract in [path-history-stream-contract](path-history-stream-contract.md), with schema artifacts, all six stream fixture evidence rows, and dedicated native benchmark cases for spill, readback, deep-index build, and recovery detection. Storage lifecycle consumption, work-packet chunk references, runtime validation, and app bridge stream consumption are covered by their completed contract artifacts; future scoped work may add threshold or stress-scale performance acceptance.
- `storage_lifecycle_policy` - Closed as a standalone priority policy in [storage-lifecycle-policy](storage-lifecycle-policy.md). Storage lifecycle fixtures, browser/native retention behavior, and app bridge lifecycle consumption are covered by completed artifacts; future scoped work may extend lifecycle stress tests and selected emission-shell broad-phase benchmark paths.
- `work_packet_transport_contract` - Closed as a standalone priority contract in [work-packet-transport-contract](work-packet-transport-contract.md), with a focused audit in [work-packet-transport-contract-audit](../../../scripts/solver-audits/work-packet-transport-contract-audit.mjs). Future scoped work may add larger `work_packet_round_trip` fixture coverage across execution modes and packetized solver operations beyond the current emission-shell broad-phase and exact-root refinement path.
- `spatiotemporal_query_algorithm_survey` - Closed as a standalone priority survey in [spatiotemporal-query-algorithm-survey](spatiotemporal-query-algorithm-survey.md). The selected first prototype is `interval-tree time slabs + spatial-hash receiver cells + emission-shell annulus rows`, and the solver-owned `emission-shell-broad-phase-v0` path now compares the indexed candidate set against brute-force chunk replay and deterministic packet replay under the `check-solver-benchmark-thresholds` acceptance gate. Future scoped work may add larger stress-scale breadth, app-facing packet execution, and default-strategy promotion evidence.
...
- `baseline_comparison_sandbox` - Closed as a fixed-case sandbox and migration-parity harness in [baseline-comparison-sandbox](baseline-comparison-sandbox.md). The current manifest covers 17 cases across Animator, Photon, and Ideal Braid, all classified `baseline_within_tolerance`.
...
Lower-priority solver task-queue items:
...
- [app-bridge-contract](app-bridge-contract.md)
- [analytic-and-invariant-validation](analytic-and-invariant-validation.md)
- [animator-adapter](animator-adapter.md)
- [baseline-comparison-sandbox](baseline-comparison-sandbox.md)
...
- [photon-adapter](photon-adapter.md)
...
- [spatiotemporal-query-algorithm-survey](spatiotemporal-query-algorithm-survey.md)
- [solver-contract](solver-contract.md)
- [storage-lifecycle-policy](storage-lifecycle-policy.md)
- [temporal-assembly-graph-contract](temporal-assembly-graph-contract.md)
- [threading-execution-policy](threading-execution-policy.md)
...
- [animator simulation mode](../app-animator/simulation-mode.md)
- [app-simulation](../app-simulation/priorities.md)
- [app-photon](../app-photon/priorities.md)
