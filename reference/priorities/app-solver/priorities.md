...
8. Geometry calculations should be centralized around EOM-owned simulation and geometry routines for Photon, Ideal Braid, Animator, and future consumers.
...
19. Apps must communicate with EOM through one shared app bridge. App code should call a stable JavaScript or TypeScript request/response API and should not need app-specific C++ or WebAssembly handling.
...
22. Follow the EOM consumer-by-consumer migration plan for Photon, Ideal Braid, Animator, Borg, and every other current dependency; do not bulk-redirect consumers.
23. The solver workstream needs retrospective impact audits for expensive proof and research paths when those paths depended on root solving, path histories, precision, or ledger diagnostics. The first audit target is H39: select representative cases, replay the solver-relevant parts through EOM when available, and classify whether EOM produces no material effect, the same result faster, a refined result, a changed bottleneck, or an investigation-required mismatch.
