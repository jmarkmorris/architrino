...
| Class | Meaning |
| --- | --- |
| `zombie-solver contract` | Must define the first zombie-solver contract or its stable schema (historical: the retired bridge contract). |
...
- `work_packet_transport_contract` - Closed as a standalone priority contract in [work-packet-transport-contract](work-packet-transport-contract.md), with a focused audit in `scripts/solver-audits/work-packet-transport-contract-audit.mjs`. Future scoped work may add larger `work_packet_round_trip` fixture coverage across execution modes and packetized solver operations beyond the current emission-shell broad-phase and exact-root refinement path.
