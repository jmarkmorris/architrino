...
- Stage: `accepted-architecture`
- Implementation: `C++20 EOM solver active; production-conformance closure remains open`
- Authority: operator-selected EOM boundary
- Production-host decision: [eom-cpp-production-host.md](../../../architectural-decisions/eom-cpp-production-host.md)
...
The production host is the C++20 EOM solver under `src/eom`, as selected by [eom-cpp-production-host.md](../../../architectural-decisions/eom-cpp-production-host.md). It owns Master EOM evaluation, causal-root logic, integration, accepted-history evolution, and acceptance decisions. JavaScript may provide fixtures, independent comparison instruments, protocol adapters, and display arithmetic, but it is not a production solver and may not fill a missing production capability.

Application shells remain outside the compute authority. Borg initiates supported EOM runs and inspects live or recorded run state. Animator does not initiate, continue, or modify an EOM run; it displays accepted recorded output through `eom-recorded-playback-handoff.v1`. A handoff pins the exact record with canonical JSON SHA-256 and declares the EOM contract, model binding, engine identity and version, run identity, claim grade, evidence status, and completed status. Animator rejects the handoff when the pin is stale, any identity is incompatible, or the record has not reached an accepted completed state.
...
## Borg Run And Inspection Surface

Borg is the application surface for EOM execution and inspection. It should expose:
...
Visualization can inspect accepted output paths, root events, and diagnostics. It must not feed edited future geometry back into the EOM run. Animator is the separate recorded-playback consumer and cannot acquire run authority through this inspection path.
