...
| `partially migrated` | A zombie-solver row, stream, or ledger exists, but the app still reconstructs causal, path-dependent, or branch-local geometry needed for diagnostics or summaries. |
...
Ideal Braid builds solver requests by linearizing circular source histories into source segments. Photon builds circular-source request structures from app state. Animator builds a linear motion fallback request from app drift configuration, while delayed-hit scene code now passes document/dataset/cadence state to [AnimatorReceiverPathDescriptors.mjs](../../../src/apps/animator/display/AnimatorReceiverPathDescriptors.mjs), which emits `path_segment.v1` receiver descriptors instead of app-built receiver-track sample arrays.
...
Animator delayed-hit generation no longer computes shell/path intersections in the app delayed-hit runtime. [AnimatorFieldShellEventStream.mjs](../../../src/apps/animator/display/AnimatorFieldShellEventStream.mjs) packages field-shell emission cadence and `field_shell_events.v1` rows, [AnimatorReceiverPathDescriptors.mjs](../../../src/apps/animator/display/AnimatorReceiverPathDescriptors.mjs) packages receiver `path_segment.v1` descriptors from document/dataset/cadence state, and [AnimatorDelayedHitRows.mjs](../../../src/apps/animator/display/AnimatorDelayedHitRows.mjs) owns the path-history shell solve from `animator-delayed-hit-stream-descriptors.v1` requests and emits `delayed_hit_events.v1` rows consumed by the app visual mapper.
