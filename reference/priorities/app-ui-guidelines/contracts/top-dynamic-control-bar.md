...
| [ideal-braid.html](../../../../ideal-braid.html) | Runtime-built TOC, Back, Forward, Home, and global Search bar; markdown overlay owns Layout, Print, and Close. | Full — canonical runtime | UI-005 batch 9 removed the repeated static shell, kept braid controls in the app panels, and reserves compact top clearance for the bar. |
...
| [assembly-explorer.html](../../../../assembly-explorer.html) | Compatibility redirect to Borg Library. | Retired | No independent app chrome remains. |
...
| [borg.html](../../../../borg.html) | Runtime-built TOC, Back, Forward, Home, Search, and Diagnostics bar; viewport and timeline controls remain local. | Full — canonical runtime | UI-005 batch 9 makes Diagnostics the declared app-mode extension after Search and preserves its app-owned panel behavior. |
| [borg-library.html](../../../../borg-library.html) | Runtime-built TOC, Back, Forward, Home, and global Search bar beside the separate Workbench action; collection search remains local. | Full — canonical runtime | UI-005 batch 9 replaced the text Applications link while retaining collection search, filters, grouping, playback, and the Workbench route locally. |
...
| [braid-search.html](../../../../braid-search.html) | Runtime-built TOC, Back, Forward, Home, and global Search bar. | Full — canonical runtime | UI-005 batch 9 removed the copied runtime constructor and wiring; dashboard filters and views remain local. |
...
| [website-stats.html](../../../../website-stats.html) | Sticky operations-utility header with compact Operations and Archie actions plus analytics opt-out. | Lightweight header | Conforms to the lightweight utility policy; analytics controls remain local and the public static route is not represented as private. |
| [solver-gpu-harness.html](../../../../solver-gpu-harness.html) | Local developer-harness header with compact Applications action and live GPU status. | Internal only — Pages-excluded | The Pages builder excludes both the harness entry point and its runtime; the source remains available for local development. |
...
| --- | --- | --- | --- | --- |
| Public utility | Lightweight header | Show the page title and a compact Home/project action. Keep utility-specific controls local; omit Search, Documents, and Settings unless the utility actually implements them. | [feedback.html](../../../../feedback.html) | Adopt the full standard only if the utility becomes an interactive app or workbench with app-level navigation, search, documents, or settings. |
| Public static operations utility | Lightweight header | Show the operations title and compact Operations/Archie actions. Keep operational controls local, omit product-app Search and Home destinations, and state that static hosting provides no access control. | [website-stats.html](../../../../website-stats.html) | Add an authenticated service boundary before describing the surface as private; operational intent alone is not access control. |
...
| Developer harness | Internal only — Pages-excluded | In the local development environment, show the harness title/status and compact Home navigation. Keep benchmark and diagnostic controls local. | [solver-gpu-harness.html](../../../../solver-gpu-harness.html) | Reclassify the surface and remove its deployment exclusion only after an explicit decision to make the harness a public app. |
...
6. Braid Search and Borg Library: complete; both consume the canonical adapter while dashboard filters, catalog search, playback, and Workbench entry remain local. Evidence is bound by the [UI-005 batch-9 receipt](../evidence/ui-005-batch-9-borg-braid-browser-captures.2026-09-02.json).

### Phase 5: Page Policy

- Website Statistics: use the lightweight public static operations-utility header with compact Operations and Archie actions, page-local analytics controls, and an explicit no-access-control notice.
- Solver GPU Harness: keep as a local-only developer surface. The Pages builder excludes its entry point and runtime; its local lightweight-header evidence remains bound by the [UI-005 batch-4 receipt](../evidence/ui-005-batch-4-lightweight-headers-browser-captures.2026-09-02.json).
