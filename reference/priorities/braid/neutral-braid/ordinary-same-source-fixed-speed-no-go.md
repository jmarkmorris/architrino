...
Promotion status: `priority-only`. Closure status: `closed-rejected` only for the ordinary same-source positive-delay root hypothesis on the rigid fixed-speed circular carrier. This packet isolates the same-source row from the rigid octahedral carrier worked example in [Octahedral Carrier Worked Example](../shell-braid/octahedral-carrier-worked-example.md). It does not reject any broader neutral braid, shell braid, nested shell braid, event-ledger, bounded-speed, fold-layer, or deformed support-band route.
