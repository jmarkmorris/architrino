...
Terminology status: `quarantined-priority-history`. This gate map is inherited architecture source, not terminology authority. Its disputed labels may remain here for git history and mathematical source mining, but they are not accepted reader-facing taxonomy. Route any reuse through [Braid](../braid.md), whose active reader-facing triad is `neutral braid`, `shell braid`, and `nested shell braid`.
...
The merge decision now runs through [Neutral Braid Model](../neutral-braid/neutral-braid-model.md) and [Nested Shell Braid Model Card](../nested-shell-braid/nested-shell-braid-model-card.md). Shell braid geometry is therefore one mathematical case of a broader neutral braid branch family, while the nested shell braid packet set remains comparison and fallback source material until the user-facing corpus merge gate passes.
