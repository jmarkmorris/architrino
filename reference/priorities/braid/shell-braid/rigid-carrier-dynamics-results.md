...
The zero-offset fixed-speed row is now closed by rejection in [Rigid Octahedral Fixed Speed No Go](../neutral-braid/rigid-octahedral-fixed-speed-no-go.md). The deterministic witness at $(i,\theta)=((1,+),0)$ gives
...
```bash
node scripts/neutral-braid/octahedral-phase-offset-scan.mjs --out /tmp/neutral-braid-octahedral-phase-offset-scan.json --pretty
node scripts/neutral-braid/octahedral-phase-offset-scan.mjs --validate /tmp/neutral-braid-octahedral-phase-offset-scan.json --pretty
```

Its default schema is

$$
\texttt{neutral-braid-octahedral-phase-offset-scan/v1}.
$$
...
The retention overread is now closed in [Phase Offset Improvement Not Retention](../neutral-braid/phase-offset-improvement-not-retention.md). The executable witness `scripts/neutral-braid/octahedral-phase-offset-retention-witness.mjs` records `closed-rejected:sampled-phase-offset-improvement-implies-retention`: sampled RMS improvement is a useful search signal, but it is not a retained fixed-speed neutral braid branch.

---

## 4. Force-Balance Implication

[force-balance-reduction.md](force-balance-reduction.md) shows that a retained shell braid carrier must close both:
...
The ordinary same-source exclusion is now closed by [Ordinary Same Source Fixed Speed No Go](../neutral-braid/ordinary-same-source-fixed-speed-no-go.md). The executable witness `scripts/neutral-braid/octahedral-same-source-witness.mjs` records `closed-rejected:ordinary-same-source-positive-delay` for the rigid exact-$c_f$ circular convention. This does not close or reject controlled self-hit, bounded-speed, or fold-layer event policies.
