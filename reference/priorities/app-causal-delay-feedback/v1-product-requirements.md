...
1. `eom_replay_runtime` - The standalone page replays recorded EOM datasets (`eom_evolution_contract/v0`) through `CausalDelayFeedbackEomReplayAdapter.js`, which normalizes frames, retained path-history samples, delayed-hit rows, and compact diagnostics into the canvas runtime dataset shape and fails closed when any required rows are missing. `?replay=mock` remains the representative visual-review mode. Interactive constrained-replay solving (retained-constraint edits driving a boundary-value solve) is not available; reintroducing it is EOM contract work, tracked as a capability gap. Status: `active`.
