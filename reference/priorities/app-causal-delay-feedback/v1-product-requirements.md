...
- default the standalone page to the EOM solver bridge replay path;
...
- expose the stream target as `eom_native_path_history_stream`;
- use the EOM solver bridge for frame samples, path-history samples, delayed-hit rows, and compact diagnostics when a solver client is available.

Current replay inspection (updated 2026-07-16):

- `src/apps/causal-delay-feedback/CausalDelayFeedbackEomReplayAdapter.js` normalizes recorded EOM datasets (`eom_evolution_contract/v0`) into the canvas runtime dataset shape: frames, retained path-history samples, delayed-hit rows, and compact diagnostics. It does not advance when frame samples, retained path-history samples, or required diagnostics are missing.
- The runtime uses the EOM replay adapter by default, while `?replay=mock`, `?solver=mock`, or `?adapter=mock` forces the temporary representative replay adapter for visual review.
...
6. After the EOM solver bridge is available, endpoint-handle edits and scripted direct-edit paths may submit retained path constraints to the solver or app-worker adapter, while interior retained path-history samples remain hidden.
...
- The capture script starts or reuses the local dev server, opens a Chromium-family browser through the Chrome DevTools Protocol, pins the replay time and selected wake row, and writes the representative mock desktop and portrait PNG proofs. Capture proofs pinned to `eom_history_replay` through the EOM replay adapter are a queued build.
...
- `temporary_mock_solver_replay_adapter` - Create `representative_mock_solver_replay` data with frame samples, retained path-history points, diagnostic cross-path wake rows, official red/blue polarity colors, and the future EOM solver bridge target.
- `solver_adapter_boundary` - Isolate the temporary mock replay in a focused adapter module so the EOM solver bridge can replace the data source without rewriting the canvas renderer.
...
- `replay_contract_adapter` - Add a central-bridge replay request builder and response normalizer for causal-delay feedback datasets, with injected bridge-run support for tests and future worker wiring.
- `central_bridge_causal_delay_app_id` - Add `causal-delay-feedback` to the EOM solver bridge app-id contract and smoke it through built-in app playback.
...
- `default_live_replay` - Make the standalone page attempt central pair-interaction replay by default, keep the mock replay as the immediate fallback, and preserve `?replay=mock` for representative visual review.
...
- `drag_to_solver_loop` - Keep the solver submission loop for scripted retained constraints and velocity review hooks. Visible v1 interaction stays focused on replay, settings, wake selection, and keyboard/scrubber timing; the physical boundary-solver status contract is tracked by `eom_replay_runtime`.
...
- `boundary_residual_acceptance_gate` - Add optional `pathConstraintBoundaryResidualTolerance` request/query support so constrained pair replays return `not advanced` with `path_constraint_boundary_residual_exceeded` when retained-knot boundary residual diagnostics exceed an explicit tolerance; leave the default unset so normal visual replay remains available when the boundary residual is not being audited.
...
- `position_residual_acceptance_gate` - Add optional `pathConstraintPositionResidualTolerance` request/query support so constrained pair replays return `not advanced` with `path_constraint_position_residual_exceeded` or `path_constraint_position_residual_unresolved` when retained-position diagnostics do not prove that submitted start, interior, and final path-history points were preserved.
- `guidance_acceleration_acceptance_gate` - Add optional `pathConstraintGuidanceAccelerationTolerance` request/query support so constrained pair replays return `not advanced` with `path_constraint_guidance_acceleration_exceeded` when retained-knot guidance acceleration remains above an explicit tolerance; leave the default unset so ordinary visual replay remains available, and use the gate mainly for relaxation-disabled guidance diagnostics.
...
- `eom_replay_browser_screenshot_proof` - Capture a 1920x1080 browser proof of the default `eom_history_replay` dataset through the EOM replay adapter and fail the proof if the runtime silently falls back to `representative_mock_solver_replay` or `temporary_mock_adapter`. Capture proof is a queued build.
- `eom_replay_portrait_browser_screenshot_proof` - Capture a 390x844 phone-shaped browser proof of the default `eom_history_replay` dataset through the EOM replay adapter and fail the proof if the runtime silently falls back to `representative_mock_solver_replay` or `temporary_mock_adapter`, so the future iPhone integration path has EOM-replay evidence rather than only mock portrait evidence. Capture proof is a queued build.
- `boundary_tolerance_query_default_fix` - Treat an absent or blank `boundaryResidualTolerance`/`pathConstraintBoundaryResidualTolerance` URL parameter as unset instead of coercing `null` to zero, so ordinary direct-manipulation central reruns advance unless the operator explicitly requests a tolerance gate.
- `settings_reset_preset` - Add a compact `Reset preset` action to the settings popover that clears direct-manipulation draft edits and reloads the currently selected preset through the active replay adapter, including the EOM solver bridge when available.
...
- Keep the EOM solver bridge as the authoritative integration target.
