...
Let $q$ be a retained same-level branch class over a window $W$, and let $\mathbf{C}^{(\mathbf{v})}(t)$ be the branch center in a moving row with constant drift velocity
