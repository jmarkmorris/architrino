...
| [$A_0$ Branch-Chart Revision Contract](a0-branch-chart-revision-contract.md) | Pre-rerun contract and executable checker status for revising $z_\Lambda$ after the compact finite-coordinate no-go; declares the non-root-key inner-layer harmonic deformation coordinate $\mathcal{H}_I$, primary modes $\{4,5,7\}$, guard mode $\{6\}$, Nyquist guard, equality contract, and current `rejected_hidden_fit_split` / `overfit_holdout_fail` results before any further corrected one-period map. | None until a revised branch row passes; supports [$A_0$ Reduced Branch Certificate Packet](a0-reduced-branch-certificate.md) and [A0 Corpus Change Recommendations](a0-corpus-change-recommendations.md) |
