...
| [$A_0$ Branch-Chart Revision Contract](a0-branch-chart-revision-contract.md) | Pre-rerun contract for revising $z_\Lambda$ after the compact finite-coordinate no-go; declares the non-root-key inner-layer harmonic deformation coordinate $\mathcal{H}_I$, primary modes $\{4,5,7\}$, guard mode $\{6\}$, Nyquist guard, equality contract, and falsification conditions before any further corrected one-period map. | None until a revised branch row passes; supports [$A_0$ Reduced Branch Certificate Packet](a0-reduced-branch-certificate.md) |
...
5. Rerun the corrected fold-layer-locked one-period map only after [the branch-chart revision contract](a0-branch-chart-revision-contract.md) emits `revision_candidate_only` with the anti-overfit residual passed, rather than a no-go or rejected split, and require state return, root closure, phase closure, speed ordering, energy-like speed closure, center-drift closure, and $\mathcal{R}_{\text{lock}}$ to pass before monodromy.
