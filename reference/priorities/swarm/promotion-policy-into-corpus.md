...
Current decision: `priority-only`. This packet is itself a promotion policy and guardrail. Its mathematical advance is the predicate $\mathrm{Promote}_{\mathrm{corpus}}(M,D)$ and the claim-level/prerequisite ladder above. The next promotion pass should apply this policy to one concrete destination at a time rather than attempting a broad swarm corpus migration while retained-branch, validation, and observer-export rows remain open.
