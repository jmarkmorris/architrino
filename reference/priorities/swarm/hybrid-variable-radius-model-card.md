...
Promotion status: `priority-only`. This packet records the proposed relation between the nested shell swarm and shell swarm cases. It does not authorize migration into `content/markdown/aaa`, simulations, scene assets, or app copy. It defines a branch-family target in which those cases are mathematical sectors of one bounded-speed Noether swarm model with explicit shell rows. After [neutral-knot-cloud-branch-model.md](neutral-knot-cloud-branch-model.md), this packet is a binary-partition sector of the broader neutral swarm branch family. The partition-indexed radius shorthand $R_a$ is derived in [hybrid-support-radius-functional.md](hybrid-support-radius-functional.md).
