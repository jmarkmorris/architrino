...
The status is `neutral-swarm-finite-mode-candidate` only if:
...
The status is `neutral-swarm-finite-mode-rejected` only if the search proves a
...
The status is `neutral-swarm-finite-mode-open` when the search lacks a proof-quality
candidate or proof-quality rejection. The first-failure row should be the earliest
applicable status in this ordering:

1. `neutral-swarm-finite-mode-schema-open`
...
| --- | --- |
| `packet_id` | `neutral_swarm_finite_mode_search` |
