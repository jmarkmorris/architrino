...
| `jacobian-root-fold` | Some retained root has $|J_a|\le\epsilon_J$. A true fold has $G_a=0$, $J_a=0$, $\partial_{\eta\eta}G_a\ne0$, and a transverse continuation derivative. | Root sensitivities and force derivatives are no longer certified because the root-front law divides by $J_a$. |
