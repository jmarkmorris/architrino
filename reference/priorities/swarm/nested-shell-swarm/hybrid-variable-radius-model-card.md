...
Promotion status: `priority-only`. This packet records the proposed relation between the nested shell swarm and shell swarm cases. It does not authorize migration into `content/markdown/aaa`, simulations, scene assets, or app copy. It defines a branch-family target in which those cases are mathematical sectors of one bounded-speed Noether swarm model with explicit shell rows. After [neutral-knot-cloud-branch-model.md](../neutral-swarm/neutral-knot-cloud-branch-model.md), this packet is a binary-partition sector of the broader neutral swarm branch family. The partition-indexed radius shorthand $R_a$ is derived in [hybrid-support-radius-functional.md](hybrid-support-radius-functional.md).

The working decision is not to replace the old layer picture immediately. The safer mathematical move is to demote strict radial nesting and exact binaries from ontology to explicitly stated sector conditions. The general base branch is six labeled architrinos with three positive and three negative sites. The three-binary description remains available only when an optional partition into three binaries is declared and certified. In a nested shell swarm, each shell contains one binary, so the shell may also be called a binary when the nested shell context is clear. Their shell rows, speeds, causal-root ledgers, and event rows decide whether a retained branch lies in a nested shell swarm sector, a shell swarm sector, a transition sector, or outside the binary-partition sector altogether. The ideal case with variation set to zero is a simplifying limit inside the shell model, not a separate swarm type.

---

## 1. Merge Hypothesis

The general base branch uses site labels $i\in I=\{1,\ldots,6\}$ with $\sum_i\sigma_i=0$ and no required binary relation. This nested shell swarm sector adds a partition
...
Only after $\mathcal{P}$ is declared may one write $i=(a,\sigma)$, with $a\in\{1,2,3\}$ indexing the three binaries and $\sigma\in\{+,-\}$ indexing the two architrinos in each binary. In a nested shell swarm, these are the three binaries, one in each shell. The sector branch is a set of closed arclength curves in $\mathbb{R}^3$ with a declared support descriptor:
...
The nested sector is the retained region in which the three partition-indexed radii are persistently ordered:
...
### 2.2 Shell Swarm Sector

The shell swarm sector is the retained region in which
...
This sector is the mathematical home for the operator question: the old `I/M/O` or `I/M/L` labels may continue as branch-history labels while the geometry approaches a common support band. A transition-sector packet must declare whether the labels are:

| Label use | Meaning |
| --- | --- |
| `geometric-order` | binary labels are still radius ordered on $W$ |
| `continuation-history` | binary labels name the branch sector from which the current row was continued |
...
| --- | --- | --- |
| Hybrid branch definition | explicit variables for partition-indexed radii, speed factors, phase/topology data, inventory, causal roots, action, event rows, and observer exports | open |
| Sector map | nested shell swarm, shell swarm, and transition sectors stated as residual conditions using the radius functional, not competing ontologies | staged here |
...
| Shell swarm comparison | explicit comparison against the shell swarm and bounded-speed packets in this directory, naming which rows survive unchanged and which must be recomputed with variable radii | open |
...
2. Treat nested shell swarm, shell swarm, and transition geometries as sectors of one shell-swarm branch family.
