...
This priority draft states the shell swarm architecture as a theorem-target program, not as a completed proof. After [neutral-knot-cloud-branch-model.md](../neutral-swarm/neutral-knot-cloud-branch-model.md) and [hybrid-variable-radius-model-card.md](../nested-shell-swarm/hybrid-variable-radius-model-card.md), the shell swarm architecture should be read as one mathematical sector of a broader neutral swarm family rather than as an automatic replacement for the nested shell swarm model. It proposes deformable, phase-locked spatiotemporal choreographies that may occupy a shell swarm sector with a common support band, but it does not promote any downstream claim until a retained branch certificate supplies active causal roots, positive Jacobian floors, finite memory depth, closure residuals, central inventory accounting, energy/action ledgers, exposure maps, and stability data.

Promotion status: `priority-only` for branch-retention and broad architecture claims. Accepted terminology, notation compatibility notes, and detailed validation-method material may be promoted earlier only through the scoped promotion policy in [swarm.md](../swarm.md).

Development posture: build the priority-side neutral swarm, shell swarm, and nested shell swarm architecture documents in this directory until the user-facing corpus merge gate passes. Do not treat this draft as authority for broad architecture migration into `content/markdown/aaa`, simulations, scene assets, or app copy. Scoped promotion may extract accepted terminology or validation-method material when it is reader-ready, self-contained, and does not claim a retained branch. The broad merge decision requires a retained branch packet, a central-inventory replacement for the former polar-charge layer, an accepted binary-partition notation policy, and an explicit comparison against the nested shell swarm source packet set preserved in [tri-binary-causal-closure](../../tri-binary-causal-closure/tri-binary-causal-closure.md).

---

## 1. Claim Scope

The shell swarm hypothesis is that stable Noether swarm and fermion assemblies may not require three permanently radially nested binaries. The broader neutral swarm generalization also removes mandatory binary grouping: a neutral swarm may be six labeled architrinos, three positive and three negative, on closed free-support bounded-speed paths. Binaries, a binary partition, exact antipodality, and common-radius behavior then become sector rows. The shell swarm architecture is attractive because it may replace some former special roles of `inner`, `middle`, and `outer` layers with symmetric phase offsets, central inventory, and topological branch data. The hybrid model card keeps the stronger merge possibility open: the former labels may survive as geometric-order, continuation-history, or weak-stress-role labels when the retained radius-sector row supports them.
...
The shell swarm architecture now has the following priority-side companion packets in this directory:

| Packet | Role |
| --- | --- |
| [neutral-knot-cloud-branch-model.md](../neutral-swarm/neutral-knot-cloud-branch-model.md) | General neutral swarm branch model with no required binary grouping, support-band distribution rows, topology ledger, all-pairs roots, and optional sector reductions. |
| [hybrid-variable-radius-model-card.md](../nested-shell-swarm/hybrid-variable-radius-model-card.md) | Merge card treating nested shell swarm, shell swarm, and transition geometries as sectors of one bounded-speed Noether swarm branch family. |
| [hybrid-support-radius-functional.md](../nested-shell-swarm/hybrid-support-radius-functional.md) | Radius functional extracting $R_a$ from the free-support descriptor and emitting spread, gap, transition, and derivative rows for shell sectors. |
...
| [same-level-branch-mathematics.md](same-level-branch-mathematics.md) | Shell swarm branch chart, admissible history space, causal-root ledger, Jacobian floor, finite-memory, regularized DDE, weak-limit, and tangential-residual theorem targets. |
...
| [bounded-speed-factor-master-retention-theorem.md](bounded-speed-factor-master-retention-theorem.md) | Master certificate for retaining a bounded-speed shell swarm branch after root, tail, action, Krawczyk, Noether, event, and stability rows are recomputed. |
...
| [period-closure-and-winding-targets.md](period-closure-and-winding-targets.md) | Equal-length and rational-winding theorem targets needed before force-improving intrinsic curves can be retained as shell swarm branches. |
...
| [finite-mode-branch-convergence-theorem.md](finite-mode-branch-convergence-theorem.md) | Refinement theorem target promoting uniformly certified finite-mode rows to a curve-level shell swarm dynamics branch. |
...
### 2.2 Noether Swarm Inventory

A neutral Noether swarm remains a six-architrino branch with
...
The architecture needs a replacement for the nested branch machinery. For a candidate branch class $q$ over a time window $W$, define a shell swarm certificate
...
| $\Phi_q$ | shell swarm phase offsets, winding data, and closure labels |
...
A fixed-speed candidate branch declares the ideal shell swarm carrier constraints
...
and must vanish within tolerance on the retained branch. This is the replacement for assuming that shell swarm motion automatically remains on a closed carrier.
...
Operational proper time is not fundamental at the substrate level. A shell swarm branch may define a candidate clock ratio by comparing retained branch periods:
...
and it should be compared with the canonical photon requirement that a free photon is a coaxial contra-rotating pro/anti planar pair. The shell swarm choreography may supply a route into that branch, but it does not by itself define the photon.
...
The shell swarm architecture may give a cleaner strong-field continuation than the nested shell swarm model, but the claim is not established here. A compact strong-field region $\Omega$ must satisfy a finite-boundary continuation gate:
...
A shell swarm simulation packet must emit the fields below even when some are `not_computed`:
...
The shell swarm architecture becomes migration-ready only if a review packet can answer `yes` to all rows below:
...
The shell swarm architecture is a promising candidate replacement for the nested shell swarm model because it attacks the old architecture's special-layer assumptions, fragile polar-charge placement, and anisotropy burden. The mathematical upgrade is conditional: it becomes stronger than the nested corpus only after the shell swarm branch certificate replaces the old nested shell swarm machinery rather than bypassing it.

The immediate deliverable is therefore one retained shell swarm branch packet and a populated architecture decision gate, not a broad rewrite of the corpus. Until those objects exist, this document should be used as a disciplined architecture and certificate plan.
