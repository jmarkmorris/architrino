...
This packet refines the support geometry used by [bounded-speed-factor-center-time-dynamics.md](bounded-speed-factor-center-time-dynamics.md), [swarm-architecture.md](swarm-architecture.md), [hybrid-variable-radius-model-card.md](../nested-shell-swarm/hybrid-variable-radius-model-card.md), and [force-balance-reduction.md](force-balance-reduction.md). It does not retain a branch, compute a causal-root ledger, or authorize corpus migration.
...
The support-band row may be narrow, wide, fixed in center time, continued along a branch, or slot-dependent in a hybrid variable-radius sector. None of these choices makes the curve spherical unless the retained packet adds the stronger equality row
