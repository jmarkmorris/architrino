...
The master retained-branch decision is stated in [support-complete-m3-master-retention-theorem.md](support-complete-m3-master-retention-theorem.md). It is the only $M=3$ packet here that permits `retained-exact-antipodal-m3-branch-candidate`, and only after all component rows emit certified error envelopes on one live ledger.
