...
14. `ordered_core_recycling_decision` — Decide whether shedding plus ordered-core recycling should be rejected, retained as speculation, or promoted into a formal entropy-ledger theorem target after free-steady-particle radiative shedding was ruled out. Any authorized route must name the accelerated or gravitational entropy-export source and preserve supernova time dilation, Tolman brightness, CMB, BBN, and structure-formation constraints. Status: `discussion-scoped`. Depends on: none for the decision; implementation depends on `noether_sea_source_relaxation_balance` and a closed entropy ledger.
