...
The result is a branch-response row for the root ledger, not a retained dynamics branch. The rigid octahedral packet explicitly says its all-pairs root ledger is certified while dynamics, action, Noether, event, stability, observer-export, shell swarm, and nested shell swarm rows remain separate. This packet respects that boundary.
