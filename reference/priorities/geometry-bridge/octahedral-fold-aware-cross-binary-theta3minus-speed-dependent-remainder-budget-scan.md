...
This table is the current answer to how much work remains in the local collar row. One continuous directed-rounded remainder packet remains before the collar can compose into `I1` regular critical exhaustion:

1. a continuous directed-rounded scaled fold-pair $G,D$ remainder certificate for the coalescing roots, now preceded by sampled-stencil, directed-rounded sampled-node intervals, a directed-rounded root-tube cell cover, and a directed-rounded $h$ root-graph cell cover. The regular-root complement is already closed by the hybrid raw-$F$/fold-$p$ complement-slab packet.
...
The executable packet is [octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-remainder-budget-scan.mjs](../../../scripts/neutral-swarm/octahedral-fold-aware-cross-binary-theta3minus-speed-dependent-remainder-budget-scan.mjs). It emits:
