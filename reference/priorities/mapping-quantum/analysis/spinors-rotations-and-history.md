...
Let $\gamma_0(s)=I_3$ and $\gamma_1(s)=R_z(2\pi s)$ for $0\le s\le1$. They have identical initial and final orientations, yet their lifts from $1$ end at $1$ and $-1$. Any classifier depending only on the endpoint frame assigns the same answer to both. This exact counterexample extends the existing [endpoint-exchange exclusion](../../mapping-equations/analysis/inferring-braid-requirements.md#relational-geometry-exchange-and-measurement) to controlled rotation paths.
