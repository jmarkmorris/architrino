...
The retrieval/source side is currently checked by [build-source-index.mjs](../../../scripts/archie-service/build-source-index.mjs), which verifies the source-index dry-run route cases against current repo files, markdown anchors, scene-source routing, canonical parent expectations, priority exclusion, and missing-route behavior.
