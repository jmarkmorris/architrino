...
`spiral_turning_conditions` is also promoted into [master-equation](../../../content/markdown/aaa/dynamics/master-equation.md). The corpus now contains the variable-pitch extension, corrected partner Jacobian, self-branch Frenet analogue, radial-turn inequality, weighted tangential obstruction test, normalized radial-row clarification that separates the branch sum $B_r$ from the independent force ratio $\Gamma$, the fixed A1 benchmark as a tangential-pass/radial-blocked retained-chart example, and a roadmap line naming the independent force-ratio row among the remaining certificate obligations. The active priority is no longer to state those formulas or the A1 partial-certificate split; it is to supply an independent force-ratio closure or certify another concrete branch chart that consumes them.
