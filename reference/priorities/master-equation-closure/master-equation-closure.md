...
Interface. Mass-map closure may consume $\mathfrak{B}$ only as Tier 0 / Tier 1 branch-certificate input: finite active roots, inactive gaps, Jacobian floor, memory depth, returned-section residual, and stability margin. $\mathfrak{B}$ does not supply $E_{\text{internal}}(A_0)$, $\zeta(A_0)$, $\mathcal{L}_{\text{aniso}}$, $\mathcal{M}_{\text{sea}}^{ab}$, or a particle-facing mass comparison. Those remain downstream Tier 2 / Tier 3 extraction objects after a stable branch has passed.
