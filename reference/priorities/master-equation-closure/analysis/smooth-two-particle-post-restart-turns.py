...
L=M.L;I=M.I;B=M.B;OUT=ROOT/'.local-data/master-equation-closure/post-restart/check'
def known():
 t=np.arange(4225)/1024;y=np.zeros((len(t),1,3));v=y.copy();a=y.copy();y[:,0,2]=t+t*t/2;v[:,0,2]=1+t;a[:,0,2]=1
 h,w,e=L.sign_enclosures({'target_y':y,'target_v':v,'target_a':a},15/4,33/8,1/16,1/32)
 exact=F(33,8)+F(33,8)**2/2
 assert h.lo<=exact-F(1,16) and h.hi>=exact+F(1,16)
 assert w.lo<=F(19,4)-F(1,32) and w.lo>4
 assert e.lo<=F(41,8)-F(1,32) and e.hi>=F(41,8)+F(1,32)
 r=B.rise_ratio(I(1),I(2),I(3));assert r.lo<=2<=r.hi
 OUT.mkdir(parents=True,exist_ok=True);(OUT/'signs-known.json').write_text(json.dumps({'result':'PASS','source_sha256':M.digest(__file__),'controls':['exact rising quadratic on15/4..33/8 with nonzero errors','exact shared-trough ratio2']},indent=2)+'\n');print('KNOWN continuous signs PASS before target')
def target(a):
 known=json.loads((OUT/'signs-known.json').read_text());assert known['result']=='PASS'
 rp=OUT/'target-residual.json';r=json.loads(rp.read_text());assert r['time']==['15/4','33/8'] and r['g']==16 and r['c_f']==1
 ...
 h,w,e=L.sign_enclosures(np.load(archive),15/4,33/8,*errors)
 ip=ROOT/'.local-data/master-equation-closure/population-restart/check/signs.json';inherited=json.loads(ip.read_text());assert inherited['horizon']=='15/4' and inherited['actual_velocity_range'][0]>0
