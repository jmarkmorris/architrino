...
The nonclaims list must cover conservation, action, singular continuation, physical boundary values, retained branches, consumption of MEC-006 quarantined rows, and solver acceptance. Every `coverage_cells` leaf has exactly one class: `root_free`, `simple_root_tube`, `boundary_stratum`, or `unresolved_search_cell`. An unresolved cell remains inside the possible-zero cover and blocks the full-domain verdict; it is never converted to an empty cell.
...
| Complete receiver/self acceleration gradient | **MEC-006 complete at bounded regular-domain scope; singular derivative rows terminally quarantined.** [MEC-006](receiver-wake-gradient-closure.md) has an independent analytic audit and a separately structured three-dimensional numerical verifier for the fixed-reception regular partner-root tensor. Its positive-delay same-history specialization remains conditional on separation and transmitter-factor floors. No unique self-diagonal, fold, or coincident-birth derivative prescription follows from the regular mathematics. | The kinematic control may consume accepted regular rows only. Every quarantined derivative row remains unresolved and consumer-ineligible. Completed MEC-007 supplies the encounter's unchanged-law obstruction, but a two-body conservation interpretation still requires a separately selected boundary update with same-update account derivation. |
...
| Characteristic-tail action candidate | **Overturned for `CT-FH-1`.** The cross-pair receiver-gradient identity survives, but the frozen self-inclusive functional diverges and every self-only completion preserving the regular cross kernel retains the future-transmitter response. | It cannot supply the encounter's conserved account or boundary charge. A modified action is a new candidate. |
...
The terminal MEC-006 quarantine does not authorize a numerical MEC-005 diagonal semantics. Unless a separately derived MEC-002/MEC-003 boundary update is accepted, the ledger must retain:
...
Completed MEC-007 is the mirror close-approach boundary companion. It owns the stationary-mirror first-boundary theorem, admissible-history first-exit ordering, independently checked pre-boundary root and measure ledger, exact same-event exclusion, and unchanged-law post-threshold obstruction. It does not own root provenance, choose a boundary law, or provide an account map. MEC-005's symmetric two-body control may consume completed MEC-006 regular derivative rows, but it remains kinematic-only wherever a singular derivative is required and until a separately accepted MEC-002/MEC-003 boundary update and separate account maps are accepted for the exact rows consumed.
