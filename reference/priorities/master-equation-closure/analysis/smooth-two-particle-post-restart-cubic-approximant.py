"""Bounded population refresh to17/4 and target scout to5.
...
OUT=C.OUT;GRID=1024;START=F(15,4);END=F(17,4);TARGET_START=F(9,2);TARGET_END=F(5)
SOURCE=OUT/'population-h15-4.npz';BRIDGE=OUT/'target-h9-2.npz';POPULATION=OUT/'population-h17-4.npz'
...
def known():
    ...
    report={'result':'PASS','source_sha256':C.digest(__file__),'controls':['exact first-old shell examples','exact shell25 zero cut','known zero-cut candidate graph with self exclusion','exact quadratic Hermite reconstruction and derivatives','stationary zero history','future-source-query rejection','exact axis and diagonal cubic tensor values']}
    (OUT/'cubic-known.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report),flush=True)

def prepare():
    checked=json.loads((OUT/'cubic-known.json').read_text());assert checked['result']=='PASS'
...
def scout():
    checked=json.loads((OUT/'cubic-known.json').read_text());assert checked['result']=='PASS';P,A,N=C.dependencies()
    ...
    sol=solve_ivp(rhs,(float(TARGET_START),float(TARGET_END)),initial,method='DOP853',atol=1e-20,rtol=3e-13,max_step=1/(2*GRID),dense_output=True,events=event);assert sol.success
    tt=np.arange(int(TARGET_START*GRID)+1,int(TARGET_END*GRID)+1)/GRID;values=sol.sol(tt).T
