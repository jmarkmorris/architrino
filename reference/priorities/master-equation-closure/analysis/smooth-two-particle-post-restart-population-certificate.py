"""Continuous full-law residual for the population refresh through15/4.
...
R=M.R;N=M.N;I=M.I
OUT=ROOT/'.local-data/master-equation-closure/post-restart/population-check'
MANIFEST=ROOT/'.local-data/master-equation-closure/post-restart/approximant/population-h15-4.json'
START=F(13,4);END=F(15,4);GRID=1024
B=F(1,64);BS=F(1,2000)
...
def known():
 ...
 OUT.mkdir(parents=True,exist_ok=True);r={'result':'PASS','source_sha256':M.digest(__file__),'controls':['inherited exact causal rows, continuous polynomial residual and ordered scatter','independent exact cubic axis polynomial14(1/4+t)^3 and two Taylor derivatives']}
...
def residual(args):
 ...
 m=json.loads(MANIFEST.read_text());archive=Path(m['array_file']);assert M.digest(archive)==m['array_sha256'];data=np.load(archive)
 assert m['g']==16 and m['c_f']==1 and F(m['horizon'])==END and F(m['source_start'])==START
 incoming_archive=Path(m['incoming_source_archive']);assert M.digest(incoming_archive)==m['incoming_source_archive_sha256'];old=np.load(incoming_archive)
 ...
 assert len(points)==675 and len(env)==674 and len(incoming)==506
 assert M.bit_equal(points[:505],old['source_points']) and M.bit_equal(data['incoming_source_points'],incoming)
 for q in ['y','v','a']:
  assert M.bit_equal(data['source_'+q][:3329,:505],old['source_'+q])
  assert M.bit_equal(data['source_'+q][:,76:77],data['target_'+q]*np.array([-1.,1.,1.]))
  assert M.bit_equal(data['target_'+q][:3329],old['target_'+q])
 ...
 begun=time.perf_counter();profiles=M.path_norm_profiles(ps,[n*32 for n in range(104,121)]);per=profiles[ps.n];norms=np.max(per,axis=1);assert norms[0]<float(B)
 ...
 def rhs(t,values):
  ...
  row,roots=R.row_jet(tt,rv,offset,lambda ss,j,d:pb.values(ss,j,d),np.broadcast_to(es,tt.lo.shape),np.nextafter(float(BS),np.inf))
...
if __name__=='__main__':
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('mode',choices=['known','residual']);p.add_argument('--subdivisions',type=int,default=4,choices=[4,8,16]);p.add_argument('--batch',type=int,default=8);args=p.parse_args();known() if args.mode=='known' else residual(args)
