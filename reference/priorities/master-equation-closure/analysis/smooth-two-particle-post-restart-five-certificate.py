"""Full-law target comparison through5 with frozen source input.

Stationary coefficient interval and Legendre remainder come from the independently
frozen post-restart assessment, Section1; no equation is altered.
"""
...
s=importlib.util.spec_from_file_location('frozen_new_population',HERE/'smooth-two-particle-post-restart-cubic-population-certificate.py');P=importlib.util.module_from_spec(s);s.loader.exec_module(P)
M=P.M;R=P.R;N=P.N;I=P.I;OUT=ROOT/'.local-data/master-equation-closure/post-restart/five-check'
MANIFEST=ROOT/'.local-data/master-equation-closure/post-restart/approximant/target-h5.json'
START=F(9,2);END=F(5);CUT=F(17,4);GRID=1024
...
def known():
 ...
 q=F(1,256);c5=F(65,2)*(27/(1-q)+15*q/(1-q)**2+2*q*(1+q)/(1-q)**3);assert c5<900
 assert F(65,2)*27==F(1755,2)
 OUT.mkdir(parents=True,exist_ok=True);(OUT/'known.json').write_text(json.dumps({'result':'PASS','source_sha256':M.digest(__file__),'controls':['inherited continuous source/target residual controls','exact mixed-coordinate cubic(-259,-308,63)','exact zero-radius fifth-order remainder877.5 and rational C5(1/16)<900']},indent=2)+'\n');print('KNOWN later target controls PASS before target',flush=True)
...
def target(args):
 ...
 points=d['source_points'];assert len(points)==835 and M.bit_equal(points,old['source_points'])
 for q in ['y','v','a']:
  assert M.bit_equal(d['source_'+q],old['source_'+q]);assert M.bit_equal(d['target_'+q][:4609],prior['target_'+q])
 ...
 sources=N.selected(M.CENTERS[1],points,cuts,END,F(1,16),F(1,128));offset=M.CENTERS[1]-points[sources];signs=16*R.parity(M.CENTERS[1])*R.parity(points[sources]);receivers=np.zeros(len(sources),dtype=int);groups=M.scatter_groups(receivers)
 norms=N.polynomial_bounds(pt,np.array([0]));assert norms[0]<1/16
 source_norms=json.loads((P.OUT/'polynomial-bounds.json').read_text());assert source_norms['archive_sha256']==M.digest(source_archive) and source_norms['norms'][0]<1/128
 a_lo=P.rational(F(143016,10000));a_hi=P.rational(F(143271,10000));coefficient=I(a_lo.lo,a_hi.hi);rem_factor=P.rational(F(16*900))
 def rhs(t,values):
  ...
  row,roots=R.row_jet(tt,rv,offset,lambda ss,j,d:P.source_values(ps,ss,j,d),np.broadcast_to(sources,tt.lo.shape),np.nextafter(1/128,np.inf));assert np.max(roots.hi)<float(CUT)
