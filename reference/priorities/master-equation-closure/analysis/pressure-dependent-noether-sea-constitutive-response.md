...
The [vacuum-birefringence source map](../../source-mining/analysis/vacuum-birefringence-noether-sea-constitutive-map.md) adds a concrete photon-channel consumer without changing this packet's blocker or status. Project the same retained orientation or strain record into the transverse photon plane,

$$
P_{\perp}^{ij}=\delta^{ij}-\hat{k}^{i}\hat{k}^{j},
$$

and require the polarization-resolved susceptibility to be derived from the same $\mathcal C_{\mathrm{sea}}^{(\ell)}$ row:
