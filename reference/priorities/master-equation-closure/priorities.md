...
10. `dressed_electron_retained_contract_audit` — Audit the EOM solver and retained-history interfaces against the authorized [Dressed-Electron Global Angular-Momentum Drain Scope Decision](dressed-electron-global-drain-scope-decision.md), then specify the smallest EOM retained-run extension needed to compare the bare scaffold, the two seed-grade dressed nulls, one separately authorized self-consistent off-axis candidate, and its charge-conjugate control. Status: `scope-authorized; read-only contract audit authorized; implementation-not-authorized` (2026-07-11). Depends on: the retained-history runner; any new payload geometry or solver extension requires a separate operator decision.
...
$$
and defines the corresponding energy, momentum, and angular-momentum wake-history increments across a time cut. [effective-lagrangian](../../../content/markdown/aaa/dynamics/effective-lagrangian.md) and [nested-shell-braid-dynamics](../../../content/markdown/aaa/noether-braid/braid-families.md#nested-shell-braid-dynamics) now consume that kernel as the available action-level repair rather than as a missing Noether-boundary placeholder.
