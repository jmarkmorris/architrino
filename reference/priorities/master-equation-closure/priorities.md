...
MEC-008 is `In progress` and owns that reachability question. Its analytical owner is [coincide-or-not.md](coincide-or-not.md), which derives the leading secular sign, a conditional populated invariant tube, the local same-transmitter chord expansion, the regularization reparameterization obstruction, and a bounded three-architrino EOM arc.

The opposite-polarity mutual pair row increases $\lvert h\rvert$ and makes exact aim locally repelling; it does not create the proposed coincidence attractor. Under a positive pair-coefficient floor, bounded population remainder, and finite relative speed, the owner now proves that $\lVert\mathbf h\rVert^2\ge\beta d$ is forward invariant at sufficiently small separation and that every monotone approach outside the shrinking cone $\lVert\mathbf w_\perp\rVert=O(d^2)$ is captured by such a protected region. The chord expansion completes required control (4): sub-cap motion, one-sided cap arrival, and nonzero cap curvature exclude near-diagonal self roots at linear, quadratic, and cubic order respectively.

Neither MEC-008 acceptance condition is yet met because the exact retained-history update has not supplied those bounds uniformly over every populated history, deterministic rows could still hold a pair inside the shrinking cone, and no independently certified EOM-evolved counterexample reaches the boundary. The successful EOM arc covers only $0.135183$ turns; two longer adaptive attempts halted fail closed below one turn when root-completeness certification exhausted its precision budget, so the required full-turn control remains open. MEC-002 and MEC-003 may not cite domain exclusion as a solution.
...
| Near-field magnitude lemma, recorded in the [MEC-008](work-queue.md#mec-008--same-transmitter-coincidence-domain-reachability) progress block of 2026-09-03 | [Master Equation](../../../content/markdown/aaa/dynamics/master-equation.md) | An instrument authored separately from the lemma reproduces the leading-order magnitude on a chart with $D_t$ bounded away from zero, and the constant-radial-velocity case is confirmed in closed form by a separate author. The lemma's present same-author, same-sitting algebraic check is determinism, not evidence, and does not open this gate. Promotion additionally requires the lemma to be written reader-ready in its own document rather than extracted from [coincide-or-not.md](coincide-or-not.md), which is an adjudication packet and is packet furniture throughout. |
