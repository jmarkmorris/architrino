...
| [spiral-a1-drive-interval-target](spiral-a1-drive-interval-target.md) | Sidecar-consumed radial threshold and tangential interval row. | Reports $B_r(C_{\mathrm{A1}};0)\in[-0.005994791326773983,-0.005994715991872956]$ and $D_T(C_{\mathrm{A1}};I_\ast)\in[-0.0015572472070875527,-0.00023480430280344085]$, so the tangential row is strictly negative while radial turn remains blocked without an accepted $\Gamma$ interval. |
...
| [spiral-a1-current-interval-rows](spiral-a1-current-interval-rows.json) and [spiral-a1-interval-report](spiral-a1-interval-report.md) | Executable A1 sidecar and report. | Structural rows, root transport, dependency status, and tangential drive pass; theorem grade remains false with first nonpassing obligation `radial_turn`. |
| [spiral-a1-corpus-recommendation](spiral-a1-corpus-recommendation.md) and [spiral-a1-user-facing-doc-recommendations](spiral-a1-user-facing-doc-recommendations.md) | Corpus promotion and user-facing recommendation packets. | Promotes the generic Master EOM radial-normalization clarification and the scoped A1 tangential-pass/radial-blocked benchmark into `content/markdown/aaa`; it does not promote A1 as a closure result. |
