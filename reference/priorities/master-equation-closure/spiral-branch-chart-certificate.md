...
| [spiral-a1-drive-interval-target](spiral-a1-drive-interval-target.md) | Receiver-normal drive restart target. | Requires same-box $D_s$, $D_t$, $W^{\mathrm{rec}}$, radial, tangential, aggregation, and negative-control rows before any drive verdict is promoted. |
| [spiral-a1-gamma-force-ratio-decision](spiral-a1-gamma-force-ratio-decision.md) | Receiver-normal force-ratio decision target. | Requires a receiver-normal $B_r^{\mathrm{rec}}$ interval and an independently justified $\Gamma$ interval on the same retained record. |
| [spiral-a1-kinematic-gamma-closure](spiral-a1-kinematic-gamma-closure.md) | Receiver-normal kinematic-balance target. | Records the prescribed-history kinematic demand only; it cannot supply radial status before $B_r^{\mathrm{rec}}$ exists. |
| [spiral-a1-tangential-compatibility-no-go](spiral-a1-tangential-compatibility-no-go.md) | Receiver-normal tangential compatibility target. | Requires $T_0^{\mathrm{rec}}$ from same-box receiver-normal branch strengths before accepting or rejecting the prescribed history. |
| [spiral-a1-variable-rate-turn-target](spiral-a1-variable-rate-turn-target.md) | Receiver-normal variable-rate continuation target. | Requires $B_r^{\mathrm{rec}}$ and $T_0^{\mathrm{rec}}$ before any local angular-rate continuation condition is tested. |
| [spiral-a1-current-interval-rows](spiral-a1-current-interval-rows.json) and [spiral-a1-interval-report](spiral-a1-interval-report.md) | A1 retained topology sidecar and receiver-normal restart report. | Structural rows may survive as conditional topology inputs; force/action verdict rows are absent until same-box receiver-normal branch rows are emitted. |
| [spiral-a1-corpus-recommendation](spiral-a1-corpus-recommendation.md), [spiral-a1-user-facing-doc-recommendations](spiral-a1-user-facing-doc-recommendations.md), and [spiral-a1-post-benchmark-user-facing-doc-recommendations](spiral-a1-post-benchmark-user-facing-doc-recommendations.md) | Reset corpus recommendation packets. | Recommend no A1 pass/fail or prescribed-history verdict in reader-facing prose before receiver-normal branch rows exist. |
