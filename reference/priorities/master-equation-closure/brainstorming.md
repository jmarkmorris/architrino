...
Claim level: open theory question raised by a settled sub-result; **must be answered before any radiation statement enters the corpus.**

- **The settled sub-result.** A control-certified, doubly-audited far-field measurement (spindle-braid §82) found the rigid-circular neutral braid **bound / non-radiating on both channels** to $R=512$, with the deeper reason named: the master-equation force integrand is $\hat{\mathbf r}/r^2\,\delta(g)$ with retarded *positions and velocities only* — **no acceleration term** — so the field of a steady source falls as $1/r^2$.
- **Sharper framing (a single lone charge cannot cancel its own radiation).** The force law is $\propto\hat{\mathbf r}/r^2\times$(velocity factors) — the Liénard-Wiechert *velocity* field only, with the *acceleration/radiation* ($1/r$) term absent. So the law is **radiation-free at the single-charge level by construction**, not by cancellation; jh8's braid result is inherited from the law, not a multipole cancellation. A single-architrino far-field test is therefore nearly *confounded*: the law predetermines a $1/r^2$ answer. The real question is not "does a bare architrino radiate" but **"is the radiation-free interaction law correct given observed synchrotron/Larmor radiation, and if so where does observed radiation come from?"**
- **The candidate resolution (claim level: speculation, but coherent).** Radiation in AAA is plausibly **photon emission** — emission of bound wake-assemblies (the $\xi\to0$ photon objects) — a quantized, *assembly-level* process, not a classical single-charge field. Then: a steady braid (center at rest, constituents orbiting) is a **bound ground state** that does not radiate (jh8's §82 result) — this is the substrate version of the quantum resolution of classical atomic collapse (bound ground states don't radiate; radiation requires a transition). An *accelerated* braid (or an inter-state transition) **emits photon-braids** — recovering synchrotron/Larmor as many-photon coarse-graining. This makes the radiation-free classical law a *feature* (matter stability) and correctly predicts radiation is quantized.
- **Next artifact (revised).** Do NOT rely on the single-bare-architrino flux test (confounded). Instead: (1) test whether an *accelerated* braid (center accelerating, or a driven transition between braid states) emits a bound photon-assembly — the real synchrotron mechanism; and (2) develop the emergent-radiation account tying photon-emission to observed Larmor/synchrotron/bremsstrahlung as a hard closure target against tested physics. Owner: master-equation-closure + quantum (photon closure) + a cross-theory-mapping synchrotron consumer. Until resolved, keep any "AAA is non-radiating" statement OUT of the corpus — it is only true for the *steady bound* configuration and only in the *classical velocity-field* channel.

### Propagation Through The Sea — Prepared-Path Radiation And Transparency-As-Elastic-Parting (2026-07-11)

Claim level: **speculation / brainstorm** — a mechanism picture for how a Standard-Model assembly transits the Noether sea and why acceleration radiates. Not promotable to `content/markdown/aaa` until the prepared-path residual and the $\gamma$-scaling are made quantitative. Owner: master-equation-closure (radiation) + braid-ideal (global drain) + [Noether Sea](../../../content/markdown/aaa/spacetime/noether-sea.md) (transparency).
