...
| `framing_quadrupoles` | Matter framing quadrupole $Q_A^{ij}$, sea-response trace-free quadrupole or $\zeta_{ij}^{\mathrm{TF}}$, and $D_{\mathrm{plane}}$ or equivalent frame-conditioning row when a Noether braid three-binary branch supplies the clock or matter assembly. | Feeds orientation leakage, two-way photon anisotropy, Hughes-Drever matter anisotropy, and scalar-mass anisotropy as one $\ell=2$ obstruction family. | `lorentz.framing_quadrupole_missing` or `lorentz.frame_isotropy_failed` |
