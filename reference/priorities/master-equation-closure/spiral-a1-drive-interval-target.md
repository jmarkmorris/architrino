...
Priority-only for closure, sidecar-promoted for the branch-sum and weighted-drive rows. This packet has been consumed by [spiral-a1-current-interval-rows](spiral-a1-current-interval-rows.json) and [spiral-a1-interval-report](spiral-a1-interval-report.md). It is superseded as the final A1 verdict by [spiral-a1-kinematic-gamma-closure](spiral-a1-kinematic-gamma-closure.md) and [spiral-a1-tangential-compatibility-no-go](spiral-a1-tangential-compatibility-no-go.md): the prescribed constant-$\Omega$ A1 radial kinematics resolve the radial row, and the exact turn-center tangential residual rejects this prescribed isolated two-body history.
