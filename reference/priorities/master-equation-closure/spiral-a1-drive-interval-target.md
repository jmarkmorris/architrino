...
Claim level. A1 radial threshold interval report plus tangential interval-pass probe, not a repository theorem-grade certificate. The force-ratio row remains blocked: $\Gamma$ must not be used as a search knob and must not be inferred from $b_\ast$, a branch threshold, or the tangential sign. Repository theorem-grade status remains pending until the A1 rows are emitted by the runner or a typed sidecar and the root-transport and radial force-ratio rows are resolved.
...
5. Tangential pass certificate: emit the outward integral interval `D_T_interval=[-0.0015572472070875527,-0.00023480430280344085]`, root-boundary verification, positive active $|J|$ lower bound, nonnegative weight $w(\theta)=\cos^2(3\theta)$ on $I_\ast$, `epsilon_T=2.0e-4`, and the strict inequality $D_T^+\le-\epsilon_T$ through the runner or a typed sidecar.
...
Priority-only and sidecar-deferred. A1 now has a retained-root structural packet, a radial threshold interval, and a strict negative tangential-drive interval probe. It is still not ready for authored corpus promotion as an isolated spiral closure result because the runner or typed sidecar must consume the A1 rows, root transport must be emitted on the same chart, and the radial turn remains blocked without an independently derived $\Gamma$ interval. The next durable result should be an A1 sidecar-compatible interval packet that preserves the tangential pass row $D_T^+\le-\varepsilon_T<0$ while explicitly leaving radial turn blocked by missing $\Gamma$, or rejects the A1 row by structural failure, nonnegative tangential endpoint, or certified radial failure from an accepted force-ratio interval.
