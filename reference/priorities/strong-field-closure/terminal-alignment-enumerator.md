...
It enumerates reduced circular terminal labels, delayed inter-layer roots, branch Jacobian transversality, diagnostic branch-action rows, receiver-side and source-recoil ledger residuals, per-branch stationarity residuals, branch-summed action-variation residuals, cycle-residual adapters, observer-quotiented edge-map multisets, area-normalized finite-block coefficients, and the resulting transfer proxies.
...
- materialize intra-layer self-hit and partner-hit action rows so the branch-summed residual covers the full terminal branch set rather than sampled inter-layer branches only;
