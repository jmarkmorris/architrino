...
- **Flat catalog:** Family membership is no longer a catalog or navigation requirement. The [accepted repository decision](../../architectural-decisions/flat-assembly-catalog.md) defines a family-free routing catalog, a single ordered workbench list, and property-based discovery. Source constraints and sealed historical metadata remain separate; they cannot recreate family navigation. The source-schema migration remains part of this item's independently verified migration work, not a reason to retain catalog families.
- **Concrete-example names:** Operator-approved A1.0, A2.0, and A3.0 now name the existing concrete records as peers. [The naming contract](requirements-and-design.md#concrete-example-names) preserves sealed records, source IDs, URLs, hashes, old-label search, and saved selections. The shared catalog owns current display labels; mathematical A1/A2/A3 classes and historical source metadata retain their distinct meaning. A1.0 and A3.0 keep their indexed 1:2:3 frequencies, not the equal-frequency `.1` motion.
- **Naming propagation boundary:** Authored example tables and live Borg surfaces are updated and checked; generated web textbook copies await operator-authorized regeneration. Sealed records, historical evidence, and the on-demand iOS snapshot are intentionally unchanged. [Verification and refresh command](work-log.md#2026-08-30---concrete-example-names-a10-a20-and-a30).
- **Selector audit:** [All twenty-four catalog representatives across six dimensions](selector-assignment-audit.md), including SD3, F5, F6c, F6b, assigned values, and the remaining breathing/shape gaps. Speed policy is excluded from this audit by operator decision: it is a constraint applicable to any geometry, not a geometry classification; the live selector is unchanged. The A/B/C source audit identifies common-spherical-surface candidates A1.2, A2.0, A3.2. The spherical browse definition is `discussion-scoped`; no spherical assignments were added. Menus now omit `Unavailable`, retain shape `Unclassified`, and use `1D` for the line/point bucket. Internal missing data remains distinct from false.
...
- **Orbit-sharing column proposed:** The operator suggests iso-orbit/hetero-orbit. Recommend the clearer `Orbit sharing = {Shared, Dedicated, Mixed}`, independently of assembly-radius equality. A1.2/A2.0 and A1.0/B1.1 demonstrate all four radius/sharing combinations. Shared means multiply occupied tracks, not one orbit for the whole assembly; mixed composition must be representable. [Definitions, source boundaries, and next artifact](brainstorming.md#2026-08-30--orbit-sharing-as-an-independent-column). This remains discussion-scoped: no new selector or audit column is implemented yet.
- **Assembly-radius definition accepted and implemented:** The former nesting facet is replaced by `Assembly radii = {Iso-radii, Hetero-radii}`. Compare every architrino about the source-declared center of the whole assembly at equal times throughout the recorded window. B1.1/B1.2 are Hetero-radii, superseding earlier not-nested assignments. Source-formula comparison assigns four Iso-radii records (A1.2, A2.0, A3.2, F6b) and twenty Hetero-radii records, including all C records, SD3, F5, and F6c. [Definition and derivation](requirements-and-design.md#catalog-composition-classifications); [complete audit](selector-assignment-audit.md). Facet v5 and spindle-only classification v3 retire nesting values; old GUI filters are cleared with a notice and exact selections preserved, while old API queries are rejected. Source records, motion, and hashes remain unchanged.
- **F5/F6 radius clarification:** Compare all architrinos about the assembly center, not just corresponding modules. F5 repeats two unequal centered radii, so it is Hetero-radii. F6b has one fixed radius and is Iso-radii. F6c's polarity sectors have unequal centered radii away from the equal initial frame, so it is Hetero-radii and a Breather. Common breathing remains compatible with Iso-radii; the two facets are independent. [Source calculations](selector-assignment-audit.md#assembly-centered-radius-definition).
- **A2.0 spherical-surface discussion:** A2.0 is Iso-radii, while its circular paths lie on a common spherical surface. Recommend overlapping `Circular paths` and `Spherical surface` tags, with no claim of full-sphere coverage or precession. This label/definition remains proposed; if accepted, apply it consistently to all matching records, including source-supported A1.2 and A3.2, rather than adding an A2.0-only exception. [Source derivation and scope](selector-assignment-audit.md#spherical-geometry-finding).
- **Circular / precession discussion:** A1.0's current source declares fixed-center circular operators with fixed plane vectors; its circular-path tag describes that prescribed record, not a prohibition on precessing assemblies. A time-dependent rotation of a constant-radius binary preserves its spherical-surface radius but generally changes its complete path away from a fixed circle. Full spherical coverage does not follow from precession alone: axis tilt, orientation history, and orbital/precession phase relations matter. A1.0's unequal radii would correspond to three concentric spherical surfaces, not one common-radius surface. Keep orbital form, axis motion, and long-time coverage conceptually separate; whether to add an explicit precession facet or broaden shape semantics remains discussion-scoped. No new facet, source motion, or spherical assignment is approved by this clarification.
...
- **Composition implementation:** The [composition contract](requirements-and-design.md#catalog-composition-classifications) and [versioned spindle assignments](library-classifications.v3.json) retain spindle positives B1.1, B1.2, C1–C4 independently of the source-derived radius facet. The braid-count selector offers 1, 2, and 3 from source-declared component membership; C1–C6 each have two, including C1/C2 index subsets, without asserting independent binding or changing scientific taxonomy. Exact record pins keep mutable browse shape separate from sealed geometry.
- **Prompt:**
  ...
  Clean up the taxonomy even when this requires a one-time migration or recalculation. Represent classifications as versioned, independently assigned relations. Do not require family membership or a family/member tree for assembly identity, registration, or navigation. Taxonomy nodes receive stable, non-reused opaque identities separate from their primary names and codes; primary labels, synonyms, aliases, parentage, cross-memberships, merges, and supersessions remain revisable and historically traceable. A source-declared reference representative may anchor a name and migration, but it does not define the whole category or become the identity of every member. Separate the category concept from the evidence used to delimit or assign its members.
  ...
  Use the flat `Starting geometry` list for direct workbench selection and the visual-first, faceted **Assembly Library** for discovery at scale. Neither surface requires family navigation. Independently rotatable assembly-preview spheres are the primary discovery surface for humans. Descriptive names, historical codes, hashes, and other textual identifiers remain searchable and accessible but are secondary in the ordinary browse flow and may stay collapsed until a result is selected or copied. Provide orthogonal browse and filter facets at minimum for:
...
- **Completion:** Focused tests prove exact filter intersection and reset behavior, unavailable-versus-false handling, source-policy-versus-observed-speed separation without playback clamping, revision-specific facet filtering before grouping, deterministic order and pagination, total and facet counts, lazy preview loading, saved filter-state restoration, group descent, leaf resolution to exact identity, accessible machine hooks, no pixel-derived test identity, no zoom handlers or controls, rotation-invariant fixed framing, deterministic poster/reset orientation, source-only preview content, taxonomy-to-registry identity preservation, and fail-closed missing bounds. Browser QA at desktop and narrow widths performs the operator eye test across representative planar, spatial, breathing, non-breathing, Iso-radii, Hetero-radii, uncapped, capped-at-$c_f$, missing-speed-policy, multi-braid, boundary, visually similar/different-history, and same-shape/different-name entries: the grid matches canonical UI shell standards, the intended assembly can be reached without knowing its taxonomy name, each assembly remains fully visible through arbitrary rotations, wheel scrolling moves the grid rather than zooming a sphere, and no preview contains globe dots or any object other than architrinos and paths.

### BORG-016 — Polarity path color and half-turn fade

- **Status:** In progress — current catalog, workbench, and library styling implemented; broader live rotational-carrier acceptance remains open.
- **Source:** Operator direction on 2026-08-29, clarified 2026-08-30: shared two-architrino orbits have one red and one blue half; dedicated orbits have their full owner's color.
- **Request / acceptance:** Every base path uses exact electrino blue `#0000ff` or positrino red `#ff0000`. Shared antipodal opposite-polarity binaries get a trailing half-turn per owner, with the accepted fade; dedicated orbits get a solid full preceding turn or declared reconstruction cycle. Apply consistently across workbench, chart pose, library, inspection, and export. Never infer orbit sharing solely from binary membership, use an alternate palette, or manufacture missing past motion.
- **Implemented:** One [shared source-based trail policy](../../../src/apps/borg/BorgOrbitTrails.mjs) serves the library and prescribed scene. All 24 catalog records have tested dispositions. Live retained/compacted paths now match endpoint colors; prescribed palettes and unowned chart-orbit overlays are removed. Exact pin selection and recorded motion remain unchanged. [Renderer inventory and verification](work-log.md#2026-08-30---polarity-owned-shared-and-dedicated-orbit-trails).
- **Remaining acceptance:** EOM rows without source-carried rotational phase/ownership retain their ordinary past-history window without an orbital claim. A future live rotational carrier must use the same ownership rule; no phase is inferred from pixels or integrated in the app. Comparison remains source-contract blocked and has no independent path renderer. Validate these future carriers when available, and exercise export through its current-scene snapshot path.
- **Completion:** Exact colors, shared half-turn fade, dedicated full-turn/cycle visibility, clipping at fractional time boundaries, rewind, no future wrap, missing-carrier behavior, and applicable live/comparison/export scenes are checked at their actual source authority.
