...
18. `borg_candidate_adjudication_status_inspector` — [BORG-018](#borg-018--candidate-adjudication-status-inspector). Status: `Queued`.
...
- **Finding-count boundary:** `borg-library-finding-relations.2026-09-01.v1` links 114 exact configurations across 15 braid entries to active source-owned findings. Each exact configuration contributes at most one to its braid's displayed finding-bearing configuration count, even when several finding relations match. `No active findings indexed yet` is registry absence, not a scientific negative. The index performs no app-local physics and does not strengthen the source finding's claim grade.
- **Prompt:**
...
- **Demonstrator:** [Assembly Library](../../../borg-library.html) implements nine selectors, alternative facet counts, query URLs, group descent, lazy 12-result pages, independent pointer/keyboard rotation, fixed framing, playback/scrubbing, exact-record inspection, and accessible identity/facet/finding-count hooks. The default surface lists 45 unique Borg braid entries over 144 exact configurations. The initial active-finding relation index links 114 exact configurations across 15 braid entries and reports the separate per-braid count without treating an unindexed relation as a scientific negative. Source-formula radius comparisons classify 24 braid entries as Iso-radii and 21 as Hetero-radii; versioned operator assignments retain six spindle entries. Source membership supplies 21 one-braid and eight two-braid entries, with 16 unassigned; three is offered with zero matches. All current speed policies remain unavailable. General spindle/spherical descriptors, component isolation, complete taxonomy coverage, permanent braid identity, and model/occurrence identity remain BORG-014 work. [Identity and finding-count contract](assembly-identity-relation-contract.md); [radius contract](requirements-and-design.md#catalog-composition-classifications); [initial demonstrator verification](work-log.md#2026-08-30---visual-assembly-library-demonstrator).
...
- **Implemented:** One [source-based trail policy](../../../src/apps/borg/BorgOrbitTrails.mjs) serves the library and prescribed scene. All 144 exact catalog records, exposed through 45 default braid entries, have tested dispositions, including preceding phase-gap arcs for co-rotating multiply occupied circles with three or more occupants. Live retained/compacted paths now match endpoint colors; prescribed palettes and unowned chart-orbit overlays are removed. Exact pin selection and recorded motion remain unchanged. [Renderer inventory and verification](work-log.md#2026-08-30---polarity-owned-circle-trails).
...
- **Status:** Queued; depends on the BORG-014 registry/facet contract and the [BP-015](../braid-program/work-queue.md#bp-015--borg-assembly-source-inventory-handoff) scientific-source handoff.
...
- **Status:** Queued; depends on the BORG-014 exact-identity relation and the current Braid Program adjudication owner.
- **Source:** Operator acceptance on 2026-09-01 of an inspected-assembly panel that presents the current modeling, analytical, ordinary-release, and retained-branch evidence without promoting missing or bounded evidence.
- **Closure goal:** Make the scientific maturity of every inspected exact assembly legible through an identity-bound projection of the current [Braid Candidate Requirement Adjudication](../braid-program/braid-candidate-requirement-adjudication.md), while preserving the exact scope of every pass, failure, unknown, and unindexed relation.
- **Request / acceptance:** Add a dedicated scientific-status panel to the Assembly Library inspector and the corresponding exact-assembly inspection surface in the Borg workbench. Present the current `H1` closed-state, `H2` geometric/coincidence, `H3` causal-root, `H4` bounded ordinary-EOM release, and `H5` retained-branch states; organize the detailed evidence into plain-language Modeling, Analytical, Simulation, and Retention sections. The panel consumes source-owned evidence and performs no app-local dynamics, causal-root solving, stability calculation, or scientific adjudication.
- **Machine-readable authority:** Do not scrape Markdown prose or table layout at browser runtime. Extend or supersede the current versioned finding-relation projection through one canonical machine-readable adjudication schema rather than leaving parallel status paths. Each projected row must carry a stable opaque candidate/evidence relation id, exact `assemblyId + modelRevisionSha256` target where such a binding is justified, adjudication revision and source anchor, `H1`--`H5` state, claim grade, tested realization and domain, measured horizon when applicable, controlling instrument/evidence links, what the result establishes, what it does not establish, current blocker, and an operator-checkable falsifier. A build/check step must verify the projection against the current Braid Program owner; a display label, filename, rendered geometry, example representative, or prose search may not create a relation. If one adjudication row is broader than or only a slice of a Borg configuration, encode that scope explicitly rather than claiming exact equivalence.
- **Verdict contract:** Use `Retained branch established` only for an accepted `H5 P[...]` row. Use `No retained branch established yet` for `H5 U`; unknown is not falsification. Use `This exact realization failed bounded release` for an exact `H4 F[...]` result. Use `Retained-branch claim falsified within the tested scope` only for an actual `H5 F[...]` result. Report a bounded search with no success as `No solution found in the searched domain`, naming that domain. A whole candidate family may be called falsified only when a declared covering theorem, exhaustive certified search, or equivalent accepted argument reaches the whole named family and predicate. `No adjudication linked` and `Projection stale or invalid` are evidence-coverage states, not scientific negatives.
- **Panel content:** Show an overall status summary; an accessible `H1`--`H5` table with pass/fail/unknown/not-applicable state and claim grade; separate modeling, analytical, simulation, and retention details; tested horizon, realization, parameters, assumptions, and instrument where present; `Establishes`, `Does not establish`, `Current blocker`, and `Falsifier` fields; direct links to controlling evidence; and projection source, revision, and freshness. Explain technical terms immediately in plain language. Preserve the exact source status rather than reducing every result to one score or color.
- **Variants and groups:** Evaluate and bind evidence at the exact configuration/revision level before presentation grouping. An exact inspected leaf receives only its own evidence. A variant-set or group result may show aggregate coverage counts only when every count is computed from its exact members; it must separate pass, scoped fail, unknown, unindexed, and stale members and must never copy the example preview's verdict to the group. Opening a group remains the route to member-specific evidence.
- **Current baseline:** The live adjudication currently contains three candidate rows with bounded `H4 P[M]` ordinary-release evidence and no `H5` pass. Most `H5` cells are `U`, and current negatives are scoped to named realizations or requirements. This baseline is a validation target read from the live owner, not a number to hard-code into Borg.
- **Fail-closed and lifecycle behavior:** Exact-identity mismatch, changed model revision, missing source anchor, unsupported state token, broken evidence link, duplicate relation, stale adjudication revision, or incomplete group coverage must suppress the scientific verdict and expose the reason. Preserve active, superseded, and withdrawn evidence lifecycle without counting a withdrawn row as current. A presentation rename may preserve a valid exact binding; an identity-bearing scientific change requires a new binding and re-adjudication.
- **Evidence boundary:** Borg is a consumer and explainer. The panel does not turn a prescribed record into an EOM trajectory, a finite `H4` prefix into retention or stability, `H5 U` into failure, a scoped failure into a global theorem, or an indexed finding into physical identity or acceptance. A stability statement additionally requires the equilibrium/retained-neighborhood obligations named by the scientific owner; visual persistence and unperturbed playback are not stability tests.
- **Validation:** Add schema and projection validation; exact-identity positive and mismatch controls; label-rename and model-revision-change controls; `H5 U` wording and scoped `H4/H5 F` controls; missing/unindexed/stale/superseded/withdrawn controls; slice-versus-whole-candidate controls; variant aggregation and representative-noninheritance controls; saved exact-link and accessible-DOM coverage; and browser QA at desktop and narrow widths across prescribed-only, `H3` pass, bounded `H4` pass, scoped failure, `H5 U`, and unindexed examples. Verify evidence links, current source revision, zero browser warnings/errors, focused Borg tests, strict content validation, and `git diff --check`. Do not change scientific dispositions merely to make a UI test pass.
- **Completion:** One canonical versioned adjudication projection is accepted by the Braid Program owner and consumed consistently by both exact-inspection surfaces; every current exact Borg configuration reports a validated scientific-status panel or an explicit coverage reason; group summaries remain member-derived; claim language matches the verdict contract; focused and browser acceptance passes; and the work log records the exact coverage counts and remaining scientific blockers without calling unknown candidates falsified.
- **Implementation prompt:** [BORG-018 thread prompt](borg-018-candidate-adjudication-inspector-prompt.md).
...
- **Status:** Queued
- **Source:** [Borg code review of the prescribed six-worldline geometry](borg-code-review-2026-07-24.md)
