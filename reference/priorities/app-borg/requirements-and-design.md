...
Measured developer-test, 2026-07-16: `.tmp/eom-native-dev/eom_borg_shadow_cli` evolved this exact sixteen-path lattice at coupling `0.005` from `T=0` through `T=1` in twenty `0.05` chunks while emitting `0.01` display keyframes. The incremental-root build reported `12.236 s` of native chunk time in total. Every post-initial chunk reused its accepted boundary snapshot; all 256 ordered pairs carried a certified stable negative prefix, and 240 non-self pairs advanced that prefix incrementally. After the finite causal window filled, the final six chunks (`T=0.70` through `T=1.00`) stayed between `0.670 s` and `0.710 s`; their early/late median ratio was `1.038`, and root-batch time stayed between `0.560 s` and `0.588 s`. Claim grade: measured executable performance and behavior conditioned on the accepted exact-polynomial initial datum; cold-path parity checks confirm that cache reuse does not change the published histories, but this is not an independent proof that the implemented EOM is physically correct. Falsifier: a repeat of the declared profile that changes cached-versus-cold published histories, fails to reuse a continuous chunk boundary, or gives an early/late steady-window median ratio above `1.25` without a separately reported precision-escalation event.
