...
The Borg product has one discoverable doorway and two focused downstream workflows. Borg Library is the discoverable product entry and opens on the visual, property-based assembly catalogue. From there, the operator may open Braid Search for read-only campaign analysis, start a new random simulation, or select one exact registered configuration and hand its identity pins to Braid Search or Borg Workbench. Braid Search opens directly on the evaluation funnel for the selected active-candidate cohort and owns campaign denominators, gates, metrics, parameters, performance, and exact case diagnostics; it does not duplicate the Library's assembly overview or exact-identity summary. Borg Workbench opens directly on the working simulation or prescribed-replay surface rather than another landing page. Library-to-analysis and Library-to-Workbench navigation preserve a safe return to the Library's filtered or exact-selection URL; returning does not retarget or reinterpret the selected `assemblyId + modelRevisionSha256 + recordSha256` identity. The legacy Assembly Explorer URL remains only as a non-indexed compatibility redirect while inbound-reference and deployed-route evidence require it. All three surfaces should remain minimal, elegant, contemporary, and parsimonious by default, with restrained controls, clear hierarchy, and no decorative chrome that competes with assembly inspection, campaign analysis, or simulation state.
