...
The operator's 2026-08-30 positive classifications are carried in [library-classifications.v1.json](library-classifications.v1.json), separate from the sealed geometry records. The nesting intention is different binary layer radii within each braid, not merely multiple braids in one assembly. The confirmed nested records are A1.1, A1.3, A1.4, A3.1, A3.3, A3.4, B1.3, C5, and C6; C5 and C6 each contain two nested braids. The confirmed spindle records are B1.1, B1.2, C1, C2, C3, and C4. These are operator-assigned browse classifications, not measured binding, stability, or an independently validated geometric theorem. Shape tags remain nonexclusive: a spindle record can also carry circular-path descriptors.

Plainly: “two braids” counts groups; “nested” describes the binary radii inside a group; “spindle” describes the assigned visual form. These are separate questions, not competing family names.

Each positive assignment binds the full sealed-record digest rather than its alias. The inspector exposes the classification revision, source, and file digest. A renamed alias preserves the assignment; changed record bytes require a new explicit assignment. Absent assignments remain unavailable unless an explicit record declaration supplies the value. Updating classifications invalidates cached descriptors and query cursors without rewriting sealed records or changing their identities. This bounded demonstrator mechanism does not ratify the full model-identity or taxonomy-migration schema.
