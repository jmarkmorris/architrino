...
Borg must replace the flat prescribed-geometry selection treatment with a visual-first faceted selection canvas backed by the accepted assembly registry. Human discovery begins with filters and independently interactive spherical previews rather than taxonomy names. Descriptive names, taxonomy aliases, and exact identity remain searchable, accessible secondary metadata and become prominent when a result is selected or copied. The canvas uses a responsive grid and follows the canonical [UI Guidelines](../../../content/markdown/aaa/archie/ui-guidelines.md): dark navy or near-black shell surfaces, thin cool-gray borders, compact rounded geometry, `8px`-`12px` gaps, `8px`-`16px` panel padding, shared UI typography tokens, visible keyboard focus, and short functional transitions. Those shell rules do not constrain the scientific colors inside a preview sphere.

The filter rail exposes at minimum exact architrino count; braid count with `1`, `2`, and `3` offered even when a count has no current results; breather or non-breather motion classification; Iso-radii or Hetero-radii assembly-centered classification; Shared, Dedicated, or Mixed orbit sharing; uncapped or capped-at-$c_f$ speed policy; and `1D`, `2D · planar`, or `3D · spatial` dimensionality; and nonexclusive visual-form descriptors. The initial visual-form vocabulary is `circular paths`, `multiple circular path groups`, `spherical distribution`, and `spindle-like envelope`, but each descriptor must have a frozen definition before migration. A value must be source-carried, explicitly assigned in a versioned source classification, or computed by a versioned deterministic descriptor from source-declared inventory and geometry, with its owner/version and inspectable reason retained. `Any` leaves a selector unrestricted and includes missing assignments. `Unavailable` is not a menu option; shape retains `Unclassified`. The `1D` UI bucket includes lines and points without changing their mathematical rank. Internal missing values remain `unavailable`, never `false`, and the canvas does not infer any facet from rendered pixels. Because descriptors may overlap, visual form is a multi-select facet rather than a partition.

Plainly: the filter asks what the assembly contains and what its declared geometry does; it does not ask the user to remember a family code. If the source does not establish its breathing or assembly-radius classification, Borg reports `Not assigned` in the record details rather than placing it in the “no” bucket.
...
The query response exposes total matches and available facet counts before preview geometry loads. Results use deterministic ordering, cursor pagination or virtualization, and lazy preview loading; Borg never instantiates every match as a live sphere. Saved and shared URLs preserve exact filter state and the selected result identity. Each interactive result exposes through accessible DOM or an equivalent stable test contract its result kind, stable id, exact target id and revision for a leaf, facet values, member count, selected state, and unavailable reason. Automated and AI tests use those carriers rather than inferring identity or filter truth from screenshots.
...
Acceptance fixtures include at least one pair of visually similar models with different histories, one pair of differently named models with the same coarse shape, capped and uncapped models with the same sub-$c_f$ displayed speed, and a record with no declared speed policy. They also cover revision-specific facets under the same model representative. A user must be able to reach each exact leaf through filters and selected-result details without knowing its taxonomy alias, while the machine contract distinguishes the targets without pixel comparison. Identical-looking records must not require visual discrimination alone.
...
Plainly: compare everyone's distance from the same assembly center at the same time. B1.1 and B1.2 are Hetero-radii under this definition, regardless of their retired not-nested labels.
...
For a linear source the squared radius is $\lVert\mathbf d+\mathbf v t\rVert^2=\lVert\mathbf d\rVert^2+2\mathbf d\cdot\mathbf v\,t+\lVert\mathbf v\rVert^2t^2$. F5's orthogonal axial and transverse reconstruction gives $h^2+\rho_j^2$. F6c gives $h_\sigma(T)^2+\rho_\sigma(T)^2$; each breathing square uses $(b+a\sin\phi)^2=b^2+a^2/2+2ba\sin\phi-(a^2/2)\cos(2\phi)$. These are Euclidean identities for declared source paths, not EOM solver results.

Plainly: these formulas compare complete prescribed motion, not how one preview frame looks. F6c can start with equal radii and become unequal later.
...
The current catalog has four Iso-radii examples (A1.2, A2.0, A3.2, F6b) and twenty Hetero-radii examples, including B1.1/B1.2, all C records, SD3, F5, and F6c. [The audit](selector-assignment-audit.md) gives every assignment and the F5/F6 source calculations. This supersedes earlier nesting classifications, not source motion. No spherical shape assignments are inferred from radius equality.

Only spindle assignments remain in [library-classifications.v3.json](library-classifications.v3.json): B1.1, B1.2, C1, C2, C3, C4. Each boolean binds the full sealed-record digest, never its alias. The inspector exposes classification revision, source, and file digest. Renaming preserves the assignment; changed bytes require a new explicit pin. Missing spindle assignments stay unassigned. Shape tags remain nonexclusive, and operator spindle assignments are not measured binding or stability.
...
Classification v3 rejects retired nesting rows and radius overrides. Old browser URLs clear the obsolete nesting filter and cursor with a visible explanation while preserving exact selected-record identity and hash; the API rejects old nesting queries rather than translating their meaning. Descriptor/classification revisions invalidate cached results and cursors without rewriting sealed records. This demonstrator migration does not ratify the full model-identity or taxonomy schema.

`Braids in assembly` counts source-declared component groups only when their nonempty member lists partition the declared constituent inventory exactly once. It does not divide architrino count by six. C1–C6 each have two such groups; the other current seed records have one, including the twelve-architrino F5 record. For C1/C2, these are the source's component index subsets; the browse count does not assert that they are separately bound braids or change the scientific distinction between top-level and component braids. Missing, overlapping, or incomplete membership yields unavailable. The current seed set has eighteen one-braid records, six two-braid records, and no three-braid record.
...
`Orbit sharing = {Shared, Dedicated, Mixed}` classifies the occupants of complete source tracks. Shared requires every track to have at least two occupants; Dedicated requires exactly one per track; Mixed requires both kinds. An assembly may have multiple separate shared tracks. Compare tracks in one common whole-assembly frame: never independently align circles, merge instantaneous crossings, or equate congruent shapes with coincident tracks. Phase, angular speed, circulation, polarity, and declared binary membership do not by themselves change geometric sharing.

Plainly: radius equality asks how far architrinos are from the assembly center. Sharing asks whether they use the same track. A1.2 and A2.0 have equal assembly radii but different sharing assignments; A1.0 and B1.1 have unequal assembly radii and also differ in sharing.

[The geometry descriptor](../../../src/apps/borg/BorgOrbitGeometry.mjs) validates a one-to-one correspondence between source and recorded worldlines. Circular carriers compare center at a common epoch, common translation velocity, squared radius, and unoriented plane normal. A regular F5 reconstruction uses its actual offset circle, not its changing angular speed: the nondegenerate resultant ellipse winds once, and the regular triangle contributes a continuous periodic phase offset, so each member covers its circle. Circle coincidence uses a numerical tolerance of $10^{-10}$ on center coordinates, squared radii, and unit-normal alignment. Every pair in a group must agree; nontransitive tolerance chains remain unassigned. Relative track drift, degenerate stationary circles, and unsupported operators remain unassigned.

Plainly: equal circles in different locations are Dedicated, while several occupants of the same circle are Shared even when they run at different rates or in different directions. These are floating-point source-geometry classifications, not certified physical measurements.

For supported F6c harmonic paths, the source formula confines the axial projection to $\sigma h_0-|a_h|\leq (\mathbf x-\mathbf C)\cdot\mathbf n\leq\sigma h_0+|a_h|$. A point of another path outside this full range disproves track coincidence. Sixty-five evenly spaced source times in the record window seek such a witness, with only common translation removed. Failure to find a witness is unassigned, never proof of coincidence or dedication. This conservative comparison does not yet establish equality of arbitrary breathing or precessing tracks. F6c's current eight paths each have the required pairwise noncoincidence witnesses. The existing source evaluator is unchanged; hand-derived circle and separated-harmonic controls supply independent expectations.

Plainly: one point outside another track's possible region proves the tracks differ. Many matching samples cannot prove they are the same.

The current catalog has eight Shared records (A1.0–A1.4, B1.3, C5, C6), fifteen Dedicated records, and no Mixed record. SD3 is non-orbital and has no applicable assignment. Unknown and non-orbital values both display `Not assigned`, with distinct reasons, without adding an Unavailable menu choice. The audit now has seven geometry/composition columns; the live interface has eight selectors because speed policy remains an independent constraint. Facet v7 invalidates older cursors without changing source records, hashes, or exact selection links.

Geometric sharing does not by itself authorize a particular trail length. For three or more distinct co-rotating occupants of one circular track, complete source phases and equal signed angular rates authorize each preceding phase-gap arc. The trail renderer separately requires a source-declared antipodal, co-rotating, opposite-polarity pair before splitting one circle into red and blue half-turn tails. Dedicated supported tracks retain their full source turn or reconstruction cycle in the owner's color. Mixed classification does not invent new trail-ownership rules.

Plainly: the new selector changes how records are found, not their motion or polarity colors. Falsifier: displaced or crossing tracks merge, incomplete source data produces Dedicated, Mixed cannot represent both occupancy types, or a half-turn requirement is used as the geometric sharing definition.
...
### Flat Catalog And Selection

The accepted [flat assembly catalog decision](../../architectural-decisions/flat-assembly-catalog.md) removes family membership as a catalog and navigation requirement. Each catalog entry contains only `id`, `label`, and `recordUrl`; no family or parent field is admitted. The Starting geometry menu renders Random architrinos followed by every catalog record exactly once, in source order, with no family headings. A directly opened external record retains its explicit External prescribed geometry choice. The library remains property-filtered, with optional query groups rather than persistent family groups.

The workbench displays the current candidate name and its source-declared geometry constraints, not a Braid family row. Existing source schemas and analytical declarations keep their recorded family fields only for constraint validation and provenance; analytical consumers validate those fields against the source, not against the catalog. New registry design must not reintroduce a required family layer. The shared equations and sealed records are not rewritten by this organization change.

Plainly: one list contains all available assemblies, including assemblies that have no family metadata. Their geometry and evidence remain unchanged.
...
A source-confirmed opposite-polarity antipodal binary alone on one circle has two preceding half-turn trails: one red and one blue. Each fades linearly from full intensity at its architrino to zero at angular lag $\pi$. A dedicated orbit, traversed by just one architrino, uses a complete preceding turn in that architrino's solid standard color. Binary membership alone does not establish shared orbit ownership: axially separated partners have separate circles and each gets a full turn.

Plainly: one shared circle has a red half and a blue half. Two separate circles are each entirely their owner's color.

[The shared orbit-trail policy](../../../src/apps/borg/BorgOrbitTrails.mjs) compares source circle centers, velocities, radii, and planes, then checks declared neutral-pair membership and antipodal phase/circulation before assigning a shared half-turn. With three or more occupants on a circular track, equal signed angular rates and distinct source phases assign each member the preceding gap divided by its angular rate. Nonuniform gaps are supported; counter-rotation, unequal rates, or coincident phases do not establish fixed arc ownership. Dedicated F6c paths use one declared phase turn, including their recorded breathing. Dedicated F5 paths use one full reconstruction cycle; their nonlinear angular motion is not replaced by a constant orbital speed. Missing or ambiguous orbit/phase carriers remain unavailable. Non-orbital and unclassified EOM histories retain their explicit time/depth window, in exact polarity color, without asserting an orbital period.

Only recorded history at or before the displayed time is drawn. Fractional endpoints are clipped; scrubbing backwards removes later segments. At the beginning of coverage, a trail grows from the available past instead of borrowing future samples or fabricating a previous cycle. The history-depth control can shorten either span. Shared trails alone fade to zero at their assigned arc boundary; dedicated full-turn/cycle trails remain solid.

Plainly: a full dedicated orbit appears once enough past motion is available. The display never invents missing history to fill a circle.
...
Claim grade: source-carried geometry determines presentation, and tests measure rendering behavior; this establishes no dynamics or physical acceptance. Falsifier: a base path has a different color from its owner, an axially separated binary is limited to half an orbit, a shared antipodal pair is painted as two overlapping full circles, or any displayed segment lies outside the available past.
...
18. a path shows future history or an alternate base color; a shared antipodal binary trail extends beyond its preceding half-turn or fails to fade there; or a dedicated orbit is incorrectly restricted to a half-turn.
