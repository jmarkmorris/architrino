...
The filter rail exposes at minimum exact architrino count; breathing, non-breathing, or unavailable motion classification; nested, non-nested, or unavailable composition classification; uncapped, capped-at-$c_f$, or unavailable speed policy; planar, spatial, mixed, or degenerate/boundary dimensionality; and nonexclusive visual-form descriptors. The initial visual-form vocabulary is `circular paths`, `multiple circular path groups`, `spherical distribution`, and `spindle-like envelope`, but each descriptor must have a frozen definition before migration. A value must be source-carried or computed by a versioned deterministic descriptor from source-declared inventory and geometry, with its evaluator version and inspectable reason retained. Missing is `unavailable`, never `false`, and the canvas does not infer any facet from rendered pixels. Because descriptors may overlap, visual form is a multi-select facet rather than a partition.
