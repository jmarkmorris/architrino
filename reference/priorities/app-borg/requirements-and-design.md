...
The Borg product has two named workflows. Borg Library is the discoverable product entry and opens on the visual, property-based assembly catalogue. From there, the operator may start a new random simulation or select one exact registered configuration and hand its identity pins to Borg Workbench. Borg Workbench opens directly on the working simulation or prescribed-replay surface rather than another landing page. Library-to-Workbench navigation preserves the Library's filtered and exact-selection URL; returning does not retarget or reinterpret the selected `assemblyId + modelRevisionSha256 + recordSha256` identity. The legacy Assembly Explorer URL remains only as a non-indexed compatibility redirect while inbound-reference and deployed-route evidence require it. Both workflows should remain minimal, elegant, contemporary, and parsimonious by default, with restrained controls, clear hierarchy, and no decorative chrome that competes with assembly inspection or simulation state.
