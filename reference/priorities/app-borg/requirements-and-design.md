...
The simulation workspace exposes one explicit `Open prescribed geometry workspace` link instead of presenting prescribed records as starting-geometry choices for a live run. The link uses the immutable catalog's stable id and sealed-record URL and navigates through `borg.html?eomRecord=<encoded-url>`. The initial prescribed workspace is the illustrative spindle chart hypothesis emitted from its source specification with `engineId: prescribed-geometry`. Pressing Play from chart pose switches to prescribed-path playback and starts the timeline. Each source-carried binary axis is rendered as a dashed guide; coincident axes remain coincident. Chart-pose orbit curves use light purple. The desktop left rail is `620px`; the narrow layout retains its single-column breakpoint. The link is not a geometry source and does not solve the deferred external packet/manifest carrier.
