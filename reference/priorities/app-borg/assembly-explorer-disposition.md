...
Retire the standalone Assembly Explorer runtime and its `assembly-configuration-explorer.dataset.v2` contract. Route the legacy `assembly-explorer` scene token and scene path directly to `borg-library.html`; retain `assembly-explorer.html` only as a user-facing redirect for old direct URLs.
...
- the Assembly Library provides scalable source search, exact identity inspection, record-hash lookup, and exact workbench handoff.
