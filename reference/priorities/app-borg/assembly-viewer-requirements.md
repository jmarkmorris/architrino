...
The instrument gate has adopted this schema (2026-07-16): every booked campaign run and collapse-protocol seed emits a record, and the shared display adapter (`src/apps/shared/EomHistoryDataset.mjs`) ingests it. Borg opens current catalog records through the exact `assemblyId + modelRevisionSha256` route; a matching `recordSha256` may additionally pin the emitted bytes.
...
- `src/apps/borg/BorgBootstrap.js` owns initial source selection. `borg.html?assemblyId=<assembly-id>&modelRevisionSha256=<model-revision-sha256>` selects an exact current catalog configuration on the same workbench while retaining a lazily constructed random-simulation configuration; an optional matching `recordSha256` pins the emitted bytes. The live EOM client is not constructed during record-only replay.
- `src/apps/borg/BorgAssemblyRecordCatalog.js` owns the immutable built-in configuration navigation list. An entry contains exactly the assembly ID, model-revision hash, descriptive label, and record URL. It contains no app-authored geometry, physics, claim, or provenance fields; the runtime fetches and validates that URL in place when the entry is selected.
