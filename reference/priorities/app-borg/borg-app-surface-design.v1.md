...
The `path-history` layer renders adjacent native path rows as straight line segments with `smoothingPolicy = none`. Trail color, thickness, and visible age are app-facing visualization only; the path authority still comes from the native path-history rows. In the simulation workspace, the visible trail age defaults to 30 units of simulation time $T$ and is selectable as 30, 60, 90, 180, or 360. Changing it redraws the current trail without resetting or restarting the run. It does not trim EOM solver input history. The EOM protocol can retire a leading Display segment when an outward-enclosed causal-wake test proves that the segment's complete wavefront has passed the complete fixed spherical receiver envelope; it rejects a candidate receiver segment that crosses that envelope before publication. Ordinary Borg runs leave this retention policy inactive until a boundary lifecycle removes outbound identities and supplies separately identified inbound boundary histories. Prescribed replay instead uses its source-carried `displayTrailPeriods`. The first static page must not spline, tube-smooth, or otherwise curve native rows beyond the solver-emitted frame sequence.
