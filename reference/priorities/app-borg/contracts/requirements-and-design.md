...
The radius descriptor evaluates every current exact record directly from its source paths. [The audit](../evidence/selector-assignment-audit.md) gives the current assignment method and representative source calculations. No spherical-shape assignment is inferred from radius equality.
...
The Borg product has one discoverable doorway and two focused downstream workflows. Borg Library is the discoverable product entry and opens on the visual, property-based assembly catalogue. From there, the operator may open Braid Search for read-only evidence analysis, start a new random simulation, or select one exact registered configuration and hand its identity pins to Braid Search or Borg Workbench. Braid Search opens on the current evidence record for the selected `assemblyId + modelRevisionSha256`: the source and sealed-record lane, exact H1–H5 adjudication, broader or slice-only findings, current compact-cohort coverage, and exact compact rows when a terminal local export exists. Every current Borg identity must remain visible even when it has no exact adjudication or compact rows; these absences are explicit coverage states, not failures. Historical compact rows that predate exact Borg identity pins remain visible as an unbound archive and are never silently reassigned. Compact campaign views retain their own denominators, gates, metrics, parameters, performance, and exact case diagnostics, but their local export is optional and cannot gate access to the other evidence lanes. Braid Search repeats only the minimum identity pins required to bind evidence and does not duplicate the Library's geometry overview or preview. Borg Workbench opens directly on the working simulation or prescribed-replay surface rather than another landing page. Library-to-analysis and Library-to-Workbench navigation preserve a safe return to the Library's filtered or exact-selection URL; returning does not retarget or reinterpret the selected identity. All three surfaces should remain minimal, elegant, contemporary, and parsimonious by default, with restrained controls, clear hierarchy, and no decorative chrome that competes with assembly inspection, evidence analysis, or simulation state.
