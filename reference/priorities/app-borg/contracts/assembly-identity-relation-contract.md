...
The canonical `braid-candidate-adjudication-projection.2026-09-02.v2` contains 28 Braid Program adjudication rows and seven finding-context relations. It links all 145 current exact configurations across all 46 braid entries to at least one active exact or contextual relation. Twenty-five current exact configurations carry exact adjudication bindings; the withdrawn centered-five-coordinate display identity retains its exact scientific binding off-catalog. The replacement stella-octangula record now has its own independently produced exact catalog-endpoint balance exclusion rather than inherited evidence. The generic rotating octahedron carries one slice-only pointer to the separately registered sum-edge obstruction but remains `No adjudication linked`. Other current configurations without an exact row likewise report `No adjudication linked`, while their contextual relation states the concrete evidence boundary or blocker.
...
| The display-withdrawn centered five-coordinate identity and its stella-octangula replacement | The former retains its exact evidence binding off-catalog; the replacement has a separately produced exact endpoint-balance exclusion | Neither identity inherits the other's `H1`--`H5` state or verdict. |
...
Closure goal: Preserve the accepted 46-entry/145-configuration migration and fail-closed typed identity boundaries as new models, occurrences, taxonomy relations, and evidence records enter the indexed registry.
