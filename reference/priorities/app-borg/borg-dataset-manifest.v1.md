...
The design-owned pieces live in `src/apps/borg/BorgAppManifest.js`: the spherical simulation envelope, the population sizing rule, the seeded initial-condition policy (`initialLinePolicy = seeded-random-central-ball`, `polaritySignConvention = positrino-positive-electrino-negative`, `velocityPolicy = zero-initial-velocity`), canonical normalized `fieldSpeed = 1`, and the fail-closed gap-row vocabulary for retained wake rows, boundary-shell summaries, `borg-boundary-shell-influence-model.v1`, `borg-boundary-shell-noise-policy.v1`, velocity sampling, and `R_boundary->central`.
