...
- Gamma-N geometry extraction row: $\ln\Gamma_N=\mathbf{b}_N\cdot\mathbf{g}_N+\mathcal{R}_{\Gamma}$, with $\mathbf{g}_N=(\ln n,\ln\chi_{\text{sea}},\ln\lambda,-\ln\xi,\ln(R_{\text{core}}/R_{\text{core},0}))^T$ and weak-field constraint $\mathbf{b}_N\cdot\nabla\mathbf{g}_N\approx-\nabla\Phi_N/c_0^2$.
