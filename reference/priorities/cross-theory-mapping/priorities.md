...
| Lorentz, Sagnac, and Fizeau | braid (legacy-braid ref: `braid-retained-branch-closure/priorities.md`), [master-equation-closure](../master-equation-closure/priorities.md) | Two-way light speed, time dilation, length contraction, rotation, and moving-medium transport recover observer-level formulas while bounding preferred-frame leakage. |
| Weak-field metric | [validation-gates](../validation-gates/priorities.md), mass-map (legacy-braid ref: `braid-mass-response-map/priorities.md`) | One $\mathcal{M}_{\mathrm{sea}}^{ab}$ maps to redshift, Shapiro delay, lensing, precession, and PPN coefficients. |
...
| Topological and statistical emergence | [validation-gates](../validation-gates/priorities.md), mass-map (legacy-braid ref: `braid-mass-response-map/priorities.md`) | Robust integer/fractional invariants and fluctuation-dissipation limits emerge from stable basins and transport maps. |
