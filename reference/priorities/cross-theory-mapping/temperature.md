...
3. `kelvin_kb_bridge` - Route the Kelvin and $k_B$ discussion through Noether sea ensemble dynamics without treating a single Noether braid as thermodynamically hot. Status: `draft`. Depends on: `ensemble_temperature_definition`, mass-map (legacy-braid ref: `braid-mass-response-map/priorities.md`).
4. `blackbody_temperature_gate` - Tie Planck-spectrum recovery to Gate A, Gate B, Gate C, detailed balance, and thermalization depth. Status: `draft`. Depends on: braid (legacy-braid ref: `braid-retained-branch-closure/priorities.md`).
...
| `kelvin_kb_bridge` | [architrino-si-base-units](../../../content/markdown/aaa/validation/architrino-si-base-units.md), mass-map (legacy-braid ref: `braid-mass-response-map/priorities.md`), and [Noether sea](../../../content/markdown/aaa/spacetime/noether-sea.md) | $k_B$ is routed through Noether sea thermal equilibrium, effective assembly mass, and velocity-distribution width. |
