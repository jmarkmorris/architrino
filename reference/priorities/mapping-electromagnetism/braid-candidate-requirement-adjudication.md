...
No scalar candidate score is authorized by this matrix. The universal requirements are noncompensable: favorable symmetry cannot offset collision, incomplete causal roots, loss of identity, a $c_f=1$ boundary crossing, or failure to obtain an ordinary retained branch. Functional rows classify what a geometry can presently attempt; they do not establish particle identity. A later weighted score may rank the expected information value of experiments only after hard failures and unknowns remain separately visible.
...
| `H2` | Geometric admissibility | At least one nondegenerate realization has continuous positive member clearance while preserving every exact centroid, polarity, dipole, or symmetry condition claimed for that realization. | Forced collision, degeneracy, or an exact incompatibility among the claimed geometric conditions. |
| `H3` | Causal admissibility | On one declared complete history in normalized $c_f=1$ units, every required causal root is certified complete with positive separation and transmitter-factor margin, and every member remains below field speed. | Missing or unresolved required roots, collision on a delayed channel, unclassified fold, or loss of the speed margin. |
...
| `H5` | Retained branch | Refined ordinary evolution establishes a root-complete positive-width retained or metastable basin with a declared nondegenerate return action, member permutation, clearance, and recovery margin. | No qualifying return, zero-width fine tuning, degenerate scalar recrossing, or branch loss under declared refinement or neighborhood perturbation. |
...
| `A3` | `P[D]` | `P[M]` | `P[M]` | `P[M]` | `U` | One pair-conjugate five-coordinate slice completed the [guarded release](three-binary-five-coordinate-bounded-eom-comparison.md) through $T=0.15$ with positive clearance, complete roots, and speed margin; the pass does not cover full A3. |
...
| `F5` | `P[D]` | `F[D]` | `U` | `U` | `U` | The declared two-ring circular realization has an [exact incompatibility](inferring-braid-requirements.md#joint-projection-audit-and-a-two-ring-no-go) among collision freedom, stationary centroid, and instantaneous dipole null; sampled clearance alone does not pass `H2`. |
| `F6` | `P[D]` | `U` | `U` | `U` | `U` | Exact tetrahedral parent geometry; F6b and F6c own its concrete continuations. |
| `F6b` (demoted realization) | `P[D]` | `P[D]` | `P[D]` | `F[M]` | `U` | Exact continuous clearance and complete simple roots pass on the prescribed circular history, but its [measured member-acceleration residual](inferring-braid-requirements.md#f6b-root-ledger-and-member-acceleration-screen) rejects that isolated history as an EOM realization. |
...
| `ACC2` | Interior, boundary, just-exterior, and remote-exterior site families and polarity decorations have declared continuous clearance and complete histories. |
...
| `ACC5` | Full braid-plus-six ordinary evolution passes causal, clearance, identity, ledger, and retained-basin requirements with backreaction. |
