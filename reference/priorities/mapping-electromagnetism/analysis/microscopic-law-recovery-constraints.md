...
The collinear case gives an exact comparison at every sub-propagation speed, not only at first order. Let the source move along the delayed line of sight, $\boldsymbol\beta=s\beta\mathbf n$ with $s=+1$ for approach and $s=-1$ for recession, so that $d=1-s\beta$ and $\mathbf n-\boldsymbol\beta=(1-s\beta)\mathbf n$. The velocity field then has radial magnitude $R^2|\mathbf E_{\mathrm{vel}}|/(C_{\mathrm E}Q)=(1-\beta^2)/(1-s\beta)^2=(1+s\beta)/(1-s\beta)$, while the canonical single-hit weight is $1/(1-s\beta)$ under the same formula-level identification of propagation conventions. Their ratio is exactly $1+s\beta$. At $\beta=1/2$ the target is $3$ against a hit weight of $2$ on approach, and $1/3$ against $2/3$ on recession. Both expressions share the same $(1-\beta)^{-1}$ divergence on approach; they differ there by a bounded factor that tends to $2$. Claim grade: derived algebraic identity between the two formulas at equal delayed distance; it asserts nothing about assembly recovery.
