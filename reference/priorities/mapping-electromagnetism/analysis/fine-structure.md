...
The exact checker order remains owned by the [EQ-26A source-field map](../../mapping-equations/analysis/eq-26a-theta-alpha-source-field-map.md); the current global first blocker is `missing_accepted_theta_gamma_packet`, and the first local source-field object is `charge_exposure_row`.
