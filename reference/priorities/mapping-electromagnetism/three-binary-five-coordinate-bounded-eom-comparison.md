...
The direct falsifiers are a rerun that loses accepted root completeness, clearance, or the $c_f=1$ speed margin before $T=0.15$; a common-locus mismatch above $10^{-12}$; or a half-chunk result that removes or reverses the reported normal-leakage distinction. A full-A3 conclusion would additionally require a predeclared covering campaign over the A3 coordinate family and is not authorized by this artifact.

Plainly: the result can be overturned by a failed guarded rerun or failed refinement. Generalizing it requires a new experiment with explicit family coverage.

Closure goal: retain this result as a bounded measurement on one declared A3 slice and its matched sector-differential row, with no promotion to a full-family, retention, stability, or physical-identity claim.
