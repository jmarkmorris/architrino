...
- **Status:** active seed document; rigid two-history moving background rejected under the tested finite boundaries; adaptive-background closure open
- **Created:** 2026-08-24
...
| the selected history is retained and stable | rejected for the first eight-site evolution and for release symmetry throughout the $N=2,4,6$ finite ladder; `periodic_exact` case open | complete periodic EOM solution and response record | the finite-crop symmetry failures reject those runs; a periodic case still requires its own complete return record |
...
No EOM-solver record currently establishes an orbiting cubic background, stiffness, perturbative stability, propagation, reversible matter loading, energy closure, effective isotropy, physical Noether sea identity, electric or magnetic field recovery, gravity recovery, photon or gravitational-wave transport, Lorentz recovery, matter identity, or cosmological selection. The first finite open-crop evolution and the controlled finite release ladder instead reject their backgrounds before the one-period return stage.

Plainly: the document closes the geometry and defines the experiment. It does not close the proposed medium.

The rigid global-plane, two-sublattice history is no longer the next moving-medium candidate. The finite runs reject it under their declared boundaries, and its two shared histories exclude the local bending, orientation transport, and certified neighbor changes now under test. The [Adaptive Cubic-Medium Kinematics And Ledger Contract](adaptive-cubic-medium-kinematics-and-ledger-contract.md) supplies the successor representation and the directional decision rule. It does not supply an accepted adaptive background, an exact periodic or controlled exterior-tail boundary, or retained Physical Observer records. No new response campaign is admissible until those prerequisites close.
...
Closure goal: produce one root-complete adaptive-background existence record with an exact periodic-image or controlled exterior-tail boundary, one-period EOM history return, and a retained Physical Observer candidate before running matched directional response rows.
