...
- Added a display-only Contour visibility slider with a legible $75\%$ default. It continuously applies global opacity, canonical-white mix, and CSS-consistent width gain to exact analytic circles. Inner circles retain the accepted strong treatment; outer circles use one uniform clamped inverse-square prominence with a documented readability floor. The control leaves field color, contour geometry, source, states, and provider values unchanged.
