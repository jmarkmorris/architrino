...
| **Topo 15 — Orbiting-binary central chunk/block artifact** | **Failed user testing — active renderer-strategy diagnosis** | The unified 180-wide live and 240-wide paused grid with exact-endpoint path chaining did not meet the visible smoothness contract in the user's actual maximum-$\beta$ frame; worker topology and component agreement is insufficient evidence. Noncircular high-$\beta$ shape may be genuine because these are equal-value contours of the combined signed wake, not prescribed circles or asserted equipotential surfaces, but the visible rendering remains unacceptable. Stop incremental grid and patch tuning. Compare the exact visible binary output against the accepted smooth rendering paths used by other Topo scenes and determine whether binary contours require a different rendering strategy rather than another small resolution increase. Preserve real combined-wake values, thresholds, masks, shape, topology, field shading, histories, anchors, routing, playback, and other scenes. | In the actual maximum-$\beta$ binary frame, confirm user-visible contour continuity and smoothness with no block, gap, staircase, cap, segment, or renderer-handoff artifact while preserving every genuine noncircular shape and combined-wake topology. |
| **Topo 16 — Halve electrino display radius in all scenes** | **Planned / not implemented** | Audit every electrino rendering path and establish one shared canonical display-radius source, then reduce the visible electrino marker/body radius to one-half its current value, $X/2$, in every Topo scene. Treat this as display-size only unless the implementation audit proves the radius enters a physical calculation. Preserve source positions, trajectories, wake and field values, contour and shading geometry, masks, scenario state, interaction hit targets unless separately justified, all positrino rendering, and every user-passed behavior. | Inspect every scene at minimum, $1\times$, and maximum Display scale and at representative playback phases. Confirm every electrino visible radius is exactly half its prior visual radius, remains centered, aligned, and usable, while positrino rendering and all field, contour, and physics behavior remain unchanged. |
