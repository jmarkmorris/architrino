...
- An `Asinh` default may preserve detail near neutral while compressing the inverse-distance singular range more gracefully than signed log2. The comparison remains open until the same raw map is rendered both ways.
- A raw-value probe on hover can show coordinates, ordinary-root status, emission time, radius, untransformed value, transformed display value, and map-product identity.
...
- Later scenarios might include two sources, a binary snapshot, or imported fixed-time products. None should enter the menu until it has a declared scientific product and authority.
