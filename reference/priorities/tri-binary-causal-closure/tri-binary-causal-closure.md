...
Field-speed boundary note, 2026-05-20: the collinear breather packet now contains a head-on $x_L=-1$, $x_R=+1$, $|v|=c_f$ in-flight wake audit. It shows that exact affine field-speed inbound history is not ordinary simple-root initial data: the partner wake is still in flight at the audit time, all affine partner source times co-arrive at the origin caustic, and same-source roots form a continuum with $J=0$. Tri-binary middle-layer or horizon-interface uses of $v=c_f$ should therefore require finite-width, dephased, curved, held-release, or branch-certified history preparation before they are allowed to feed a clock, ruler, photon, or mass-response row.
