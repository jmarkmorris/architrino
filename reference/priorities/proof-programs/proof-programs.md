...
4. `regular_boundary_topology_ownership_handoff` — Store the breather proof regular-boundary topology/no-double-counting blocker as a proof-program follow-up. Status: `handoff-needed`; proof-interval-v10 in the breather proof records that residual regular-boundary cores cannot be consumed until a proof packet supplies an exact core-to-separator assignment plus an accepted topology/no-double-counting ownership convention with branch-reuse exclusions, or an accepted replacement route. Any downstream reuse must keep this as a same-ledger ownership rule: topology rows classify downstream structure only after required branch rows close, and they cannot rescue an open root, dynamics, action/Noether, event, stability, convergence, inclusion, domination, or non-core complement row. Depends on: `breather_certificate`, `proof_program_handoff`.
