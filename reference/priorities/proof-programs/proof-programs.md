...
4. `regular_boundary_topology_ownership_handoff` — Store the breather proof regular-boundary topology/no-double-counting blocker as a proof-program follow-up. Status: `strict-gap-route-selected`; [regular-boundary topology ownership certificate target](./breather-proof/certificate/regular_boundary_topology_ownership_certificate_target.md) reduces the proof-interval-v10 blocker to a finite residual-core certificate $T(C)$, [regular-boundary route decision](./breather-proof/certificate/regular_boundary_route_decision.md) records the pivot away from fresh domination, and [candidate repair strict-gap closure target](./breather-proof/certificate/candidate_repair_strict_gap_closure_target.md) now gives the 10-collar strict-gap pass/fail target for the selected repair route. Any downstream reuse must keep topology as a same-ledger ownership rule: topology rows classify downstream structure only after required branch rows close, and they cannot rescue an open root, dynamics, action/Noether, event, stability, convergence, inclusion, domination, or non-core complement row. Depends on: `breather_certificate`, `proof_program_handoff`.
