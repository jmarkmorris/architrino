...
| `next_candidate_solver_target.md` | `accepted` as next solver target | `d9b2d49e1b644e3ec7754a9919221869d772ce9048fdcc576923ebdc6185ccfd` | Defines the next executable target as a fresh fold-adapted collocation candidate with the null-coordinate pre-ledger as the first acceptance row and a tangent-space gap-opening criterion for parent complements, now with diagnostic and live local scanner surfaces. |
| `fresh_fold_collocation_solver_surface.md` | `accepted` as solver-surface audit | `efc3a072cc5bebb5f3bd72f6562a7a29ecc43191ba8c7bf11fbc69fdc95c5cfc` | Records that no runnable breather-specific fresh-candidate solver or pre-ledger generator exists yet and specifies the minimum lawful same-packet successor artifacts plus the finite gap-opening linearization, diagnostic scanner surface, and live local fold-shear matrix. |
| `null_coordinate_separation_direction_lemma.md` | `accepted` as constructive next-candidate lemma | `d2efe84aff8f0160000d56409bb3b41d128c5a43483d29acfafe39b7d45a2127` | Converts parent-complement repair into a finite-dimensional separation-direction and tangent-space feasibility problem for fresh collocation candidates; the finite criterion now has a fail-closed scanner and a live half-period fold-shear witness. |
...
1. Use `fresh_fold_collocation_solver_surface.md`, `next_candidate_solver_target.md`, `null_coordinate_separation_direction_lemma.md`, and `gap_opening_live_fold_shear_report.seed_cosine_residuals.v0.md` to implement or manually instantiate a fresh fold-adapted collocation candidate for `doubled_four_arc_generic`; seed the shape variables with the live fold-shear witness and do not attempt another mesh-only rescue of the rejected cosine packet.
...
6. `fixed_history_strict_collar_persistence_lemma.md` explains the fixed-cosine obstruction, while `null_coordinate_separation_direction_lemma.md` and the live fold-shear witness give the constructive fresh-candidate route: use a symmetry-preserving candidate-history shear before spending work on branch-chart rows.
