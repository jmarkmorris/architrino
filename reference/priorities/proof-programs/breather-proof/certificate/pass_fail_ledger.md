...
| `fold_parent_endpoint_exclusion_legality.md` | `rejected` proof-policy attempt | `8ad0da4ed790ee06ba04b21967e208baf1803b7ab2f823dcf2b2e19b7a361ed7` | Shows endpoint-touching parent strips are not accepted under the current two-alternative parent-complement contract; adding them would require a third contract alternative. |
...
| `fold_parent_contract_decision_packet.md` | `discussion-required` proof-policy packet | `c347dd405aad71fdf4652c0cf1273eb38f76908bafd2f851a866fd0da2bff1dd` | Summarizes the fork: add a third endpoint-exclusion alternative, add a regular-boundary fold-coverage theorem, or keep the contract unchanged and reject the current packet. |
...
| Seed-chart gate 2: null-coordinate pre-ledger | `rejected` | Live `causal_ledger.json`, `causal_preledger_interval_report.md`; 140 empty rows, 6 strict simple-root subrows, 0 accepted fold-layer rows, 22 `split_required` rows. `fold_full_interval_constants_certificate.json` now supplies accepted fixed-parameter fold ceilings for the 16 fold rows without editing the live ledger. The parent-complement attempts identify the remaining six-row blocker and reject both currently allowed routes for the 20 named strips. | The fold-ceiling part is fold-ceiling-ready only under the coarse fixed-parameter fallback. The current blocker is proof-policy: either add a third endpoint-exclusion alternative, prove a regular-boundary fold-coverage theorem, or keep the current contract and reject the packet. No live ledger rewrite is authorized before that decision. |
...
These parent rows have accepted simple-root interiors, but their boundary leftovers are adjacent to active fold layers. The fixed-parameter fold-layer alternative is now available for the 16 fold rows, but `fold_parent_complement_partition_attempt.md` shows that the natural boundary strips have zero/touching null-coordinate gaps. `fold_parent_endpoint_exclusion_legality.md` rejects endpoint-aware exclusion under the existing contract, and `fold_parent_fold_family_membership_attempt.md` rejects exact membership in the accepted fold-layer row rectangles. The remaining issue is the proof-policy decision recorded in `fold_parent_contract_decision_packet.md`.
...
1. Resolve the proof-policy fork in `fold_parent_contract_decision_packet.md`: add a third endpoint-exclusion alternative, add a regular-boundary fold-coverage theorem, or keep the current parent-complement contract unchanged and reject this packet.
2. If a new parent-complement route is approved, implement it in a contract update and then attempt a closure certificate for all 20 named strips.
3. If all six parent complements close, produce a separate live pre-ledger update that rewrites the 16 fold-touching rows as bounded fixed-parameter `fold_layer` rows, rewrites the six parent rows as accepted decompositions, and leaves no `split_required` rows.
4. Only after every pre-ledger row is `empty`, `simple_root`, or bounded `fold_layer` may `branch_chart.json` be produced.
