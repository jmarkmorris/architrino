...
| `next_candidate_solver_target.md` | `accepted` as next solver target | `36b8e99a0179257f2f1c363950dc8d5a90e3103a1783a3c3bf16cf8d893dfd95` | Defines the next executable target as a fresh fold-adapted collocation candidate with the null-coordinate pre-ledger as the first acceptance row and a tangent-space gap-opening criterion for parent complements, now with an executable scanner surface. |
| `fresh_fold_collocation_solver_surface.md` | `accepted` as solver-surface audit | `c72eba3baded25b762ac1a462d980ce827b72772a56fda4c020a942c1650debe` | Records that no runnable breather-specific fresh-candidate solver or pre-ledger generator exists yet and specifies the minimum lawful same-packet successor artifacts plus the finite gap-opening linearization and diagnostic scanner surface. |
| `null_coordinate_separation_direction_lemma.md` | `accepted` as constructive next-candidate lemma | `768da876e5eb50ca1ea47eb0c75f8468e039bbd3f09c59c506e18a20f9ce9184` | Converts parent-complement repair into a finite-dimensional separation-direction and tangent-space feasibility problem for fresh collocation candidates; the finite criterion now has a fail-closed scanner. |
...
1. Use `fresh_fold_collocation_solver_surface.md`, `next_candidate_solver_target.md`, `null_coordinate_separation_direction_lemma.md`, and `gap_opening_feasibility_report.seed_cosine_diagnostic_demo.v0.md` to implement or manually instantiate a fresh fold-adapted collocation candidate for `doubled_four_arc_generic`; do not attempt another mesh-only rescue of the rejected cosine packet.
...
6. `fixed_history_strict_collar_persistence_lemma.md` explains the fixed-cosine obstruction, while `null_coordinate_separation_direction_lemma.md` and the scanner diagnostic give the constructive fresh-candidate route: solve the tangent-space gap-opening feasibility problem before spending work on branch-chart rows.
