...
| `fold_parent_contract_decision_packet.md` | `accepted` as decision record | `22bd0594bff2a58837540efc83044a60eabad42654e603e45515b61f35ce01ae` | Records the fork that led to the approved Route A endpoint-exclusion alternative and now points to regular-boundary fold coverage or packet rejection. |
...
| Seed-chart gate 2: null-coordinate pre-ledger | `rejected` | Live `causal_ledger.json`, `causal_preledger_interval_report.md`; 140 empty rows, 6 strict simple-root subrows, 0 accepted fold-layer rows, 22 `split_required` rows. `fold_full_interval_constants_certificate.json` now supplies accepted fixed-parameter fold ceilings for the 16 fold rows without editing the live ledger. Route A endpoint-exclusion is now a contract alternative and closes seven singleton-contact strips locally. Finer `w` and `u` positive-overlap subdivisions are also rejected as parent-row consumption certificates. | The fold-ceiling part is fold-ceiling-ready only under the coarse fixed-parameter fallback. The current blocker is residual positive-width null-coordinate overlap or uncertified endpoint-scale separation after threshold subdivision. Endpoint policy and finer subdivision do not consume any parent row, so no live ledger rewrite is authorized. |
...
These parent rows have accepted simple-root interiors, but their boundary leftovers are adjacent to active fold layers. The fixed-parameter fold-layer alternative is now available for the 16 fold rows, and `fold_parent_endpoint_contract_extension.md` adds endpoint-excluded complements as a third contract alternative. The endpoint-closure attempts locally accept seven singleton-contact strips, and the finer positive-overlap subdivision attempts name diagnostic empty wings by inverse thresholds. The six parent rows remain blocked because residual positive-width null-coordinate overlaps or uncertified endpoint-scale gaps remain after those splits.
...
1. Decide whether to add a regular-boundary fold-coverage theorem for the residual equality cores, or reject this cosine packet before branch-chart certification.
2. If a regular-boundary coverage route closes all six parent complements, produce a separate live pre-ledger update that rewrites the 16 fold-touching rows as bounded fixed-parameter `fold_layer` rows, rewrites the six parent rows as accepted decompositions, and leaves no `split_required` rows.
3. Only after every pre-ledger row is `empty`, `simple_root`, or bounded `fold_layer` may `branch_chart.json` be produced.

If the regular-boundary route is not adopted or cannot close these residual equality cores, the current cosine template remains rejected before branch-chart certification even though the finite fixed-parameter fold ceiling is now available.
