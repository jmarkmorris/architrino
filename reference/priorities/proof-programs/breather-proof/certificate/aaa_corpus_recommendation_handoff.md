# AAA Corpus Recommendation Handoff

## Scope

This packet records the team-agent recommendation state for possible reader-facing updates in `content/markdown/aaa/proof-programs/`.

It is priority-only as a handoff artifact. It does not edit AAA prose, does not promote the rejected cosine packet as a theorem, and does not authorize branch-chart construction.

## Verdict

The current proof-program state supports a narrow corpus recommendation:

> The null-coordinate pre-ledger should be stated as a real candidate-falsification gate. A candidate cycle may not advance to branch-chart certification unless every ordered block is consumed as `empty`, `simple_root`, or `fold_layer`; fold-adjacent parent complements left after accepted subrows must themselves satisfy an accepted complement alternative.

The rejected cosine packet is not a reader-facing theorem. It is useful as evidence that mesh refinement of a fixed candidate does not remove intrinsic positive-width null-coordinate overlap. That detailed failure table should remain in `reference/priorities` unless the AAA corpus later needs a worked failure example.
...
| `content/markdown/aaa/proof-programs/collinear-breather.md`, `### Residual Scope Boundaries` | Add a scope boundary for candidate-packet falsification: rejected packets may preserve diagnostics, but they cannot be promoted into branch-chart, corridor, monodromy, returned-sample, topology, or Schauder rows. | Low | Promote now if this section still exists unchanged in the edit batch. |
| `content/markdown/aaa/proof-programs/master-equation-breather.md`, capstone theorem-program chain | Add one general sentence: the collinear pre-ledger is a falsification gate, not bookkeeping; finite null-coordinate parent-complement coverage precedes any branch chart. | Low | Promote now as abstract architecture prose. |

## Material Not To Promote

Do not promote the following into `content/markdown/aaa` in the next edit batch:

- artifact hashes;
- full JSON row inventories;
- detailed `w` and `u` parent-strip tables;
- full endpoint/subdivision/regular-boundary attempt history;
- branch-chart authorization for the rejected cosine packet;
- seed-chart, corridor, monodromy, returned-sample, topology, or Schauder progress not yet certified;
- a new enlarged regular-boundary coverage theorem.
