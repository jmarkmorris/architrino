...
2. `null_coordinate_preledger` — Produce `certificate/causal_ledger.json` and `certificate/causal_preledger_interval_report.md`, proving the named `Null-Coordinate Causal Pre-Ledger` theorem target or rejecting the candidate/itinerary before branch-chart certification. Status: `fresh-successor-preledger-rejected-fail-closed`.
   ...
   `regular_boundary_topology_ownership_certificate_target.md` sharpens the v10 route into a finite residual-core certificate target $T(C)$ with six required field classes, but it consumes no rows. `regular_boundary_route_decision.md` records the resulting decision point: the v8 domination audit has only historical fold ceilings, no fresh same-packet $I^{\mathrm{reg\text{-}bdry}}$ bound, no enlarged fresh ceiling, and no non-core complement closure. The next proof strategy must either build fresh same-packet domination data for the $T(C)$ route or pivot to candidate repair / strict-gap closure. Depends on: `candidate_cycle_packet`.
...
- Use `certificate/regular_boundary_route_decision.md` as the current stopping point for the regular-boundary route. It records that continuing the $T(C)$ route requires a fresh same-packet domination packet, while the lower-risk alternative is candidate repair / strict-gap closure.
