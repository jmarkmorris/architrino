...
- `fresh_v10_itinerary_structural_decision_packet` produced `certificate/fresh_v10_itinerary_structural_decision_packet.md`, selecting proof-grade intervalization of the Hermite dual obstruction as the immediate route and now recording that route as closed for the declared finite sampled row system before any continuous-in-collar lift, different same-itinerary structural ansatz, or higher-fold itinerary rebuild.
...
- Use `scripts/proof-programs/fresh-v10-hermite-active-row-interval-backend.mjs`, `certificate/fresh_v10_hermite_active_row_interval_backend.v0.json`, and `certificate/fresh_v10_hermite_active_row_interval_backend.v0.md` as the proof-grade active-row interval backend for the finite sampled Hermite dual obstruction. It closes the declared sampled row system with adjusted upper interval $\gamma\le -2.20499517531647\times 10^{-10}$, but it does not prove continuous-in-collar inequalities between samples or authorize a live pre-ledger pass.
- Use `certificate/fresh_v10_itinerary_structural_decision_packet.md` as the proof-strategy decision after the same-itinerary screens. It records proof-grade dual intervalization as closed for the finite sampled row system; the next decision boundary is whether to prove a continuous-in-collar lift or move to a genuinely different same-itinerary structural ansatz or higher-fold itinerary rebuild.
...
residual cap. The next structural solver burden is therefore no longer another
tangent ray, small subarc split, generic Hermite enrichment, multiplier
rationalization, or active-row intervalization. The remaining boundary is a
continuous-in-collar lift of the sampled obstruction; without that lift, a
constructive continuation must use a genuinely different same-itinerary
structural ansatz or explicitly rebuild around a higher-fold itinerary before
rerunning the proof-interval pre-ledger.
