...
Ideal Swarm builds solver requests by linearizing circular source histories into source segments. Photon builds circular-source request structures from app state. Animator builds a linear motion fallback request from app drift configuration, while delayed-hit scene code now passes document/dataset/cadence state to [AnimatorReceiverPathDescriptors.mjs](../../../src/solver/app/AnimatorReceiverPathDescriptors.mjs), which emits `path_segment.v1` receiver descriptors instead of app-built receiver-track sample arrays.
