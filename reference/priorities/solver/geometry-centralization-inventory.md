...
| Animator | Field-shell expansion, emission cadence, and path sampling | `partially migrated` | [AnimatorFieldShellEventStream.mjs](../../../src/solver/app/AnimatorFieldShellEventStream.mjs) owns cadence times, `field_shell_events.v1` event rows, stream/manifest metadata, render shell rows, and delayed-hit emission descriptors. | Scene runtime still samples assembly/core/member paths to supply emitter positions and still renders shell radius/opacity from packaged rows. The `field_shell_events.v1` package is not yet a core ABI/native-file binary stream layout. | Keep radius and opacity calculation for rendering. Move emitter position history and any native durable `field_shell_events.v1` storage behind the solver bridge before using those rows as validation-grade diagnostics. |
...
5. Animator first safe removal: complete for the app-local shell/path solver, scene-built receiver-track/sample-array boundary, and field-shell event/cadence packaging. The remaining Animator cleanup is source-history authority and optional ABI-backed native durable storage for `field_shell_events.v1`, not local hit solving.
...
- Do not claim full Animator geometry centralization while emitter position history and native durable `field_shell_events.v1` storage remain outside the core bridge. Delayed-hit shell/path solving and field-shell event/cadence packaging now run through solver-owned descriptors.
...
The remaining Animator boundary is source-history authority and optional
ABI-backed native durable storage for `field_shell_events.v1`, not local hit
solving.
