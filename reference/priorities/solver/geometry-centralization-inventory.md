...
| Animator | Field-shell expansion, emission cadence, and path sampling | `partially migrated` | [AnimatorFieldShellEventStream.mjs](../../../src/solver/app/AnimatorFieldShellEventStream.mjs) owns cadence times, `field_shell_events.v1` event rows, stream/manifest metadata, durable native-file chunks, stream-index sidecars, render shell rows, and delayed-hit emission descriptors. | Scene runtime still samples assembly/core/member paths to supply emitter positions and still renders shell radius/opacity from packaged rows. The `field_shell_events.v1` row layout is bridge-catalogued and native-file durable, but emitter histories are still app-supplied and not yet a native C++ producer output. | Keep radius and opacity calculation for rendering. Move emitter position history behind the solver bridge before using those rows as validation-grade diagnostics. |
...
Animator delayed-hit generation no longer computes shell/path intersections in the app delayed-hit runtime. [AnimatorFieldShellEventStream.mjs](../../../src/solver/app/AnimatorFieldShellEventStream.mjs) packages field-shell emission cadence and `field_shell_events.v1` rows before [AnimatorDelayedHitRows.mjs](../../../src/solver/app/AnimatorDelayedHitRows.mjs) owns the path-history shell solve from `animator-delayed-hit-stream-descriptors.v1` requests and emits `delayed_hit_events.v1` rows consumed by the app visual mapper.
...
| Field-shell radius, emission center, and emission cadence | [AnimatorFieldShellEventStream.mjs](../../../src/solver/app/AnimatorFieldShellEventStream.mjs), [AnimatorFieldShellRuntime.js](../../../src/apps/animator/AnimatorFieldShellRuntime.js), [ArchitrinoSceneAppRuntime.js](../../../src/apps/architrino/ArchitrinoSceneAppRuntime.js) | Animator delayed-hit shell intersection and solver shared-geometry sphere/point requests. | `partially migrated` | Cadence, event packaging, and durable `field_shell_events.v1` stream storage are solver-owned. Keep radius and opacity for rendering. Move emitter position history behind the solver bridge before using those rows as validation-grade diagnostics. |
...
5. Animator first safe removal: complete for the app-local shell/path solver, scene-built receiver-track/sample-array boundary, and field-shell event/cadence packaging. The remaining Animator cleanup is source-history authority and any future native C++ `field_shell_events.v1` producer, not local hit solving or durable JS-side stream storage.
...
- Do not claim full Animator geometry centralization while emitter position history remains outside the core bridge. Delayed-hit shell/path solving, field-shell event/cadence packaging, and durable `field_shell_events.v1` stream storage now run through solver-owned descriptors.
...
by solver-owned delayed-hit rows fed by stream-backed descriptors, and
field-shell event/cadence packaging now emits solver-owned stream-package rows.
The remaining Animator boundary is source-history authority and any future
native C++ producer for `field_shell_events.v1`, not local hit solving or
durable JS-side stream storage.
