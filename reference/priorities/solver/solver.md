...
Current implementation note: the native runtime benchmark reports strategy labels and named metrics for emission-shell broad-phase, the solver-owned `query_emission_shell_broad_phase_indexed_v0` interval-time-slab/spatial-hash/emission-shell-annulus path, and combined spacetime-cell index workloads. The v0 fixture compares indexed candidates against brute-force path-history chunk replay and currently reports zero missed oracle candidates, broad-phase recall `1.0`, candidate count reduction `0.999727`, and indexed pair-test reduction `0.983029` on the local 16/64/256/1024 path-count sweep. It also validates 16 deterministic work packets with `emission_shell_candidate.v1` outputs, zero packet missing candidates, zero packet extra candidates, and zero merge-order mismatches. The benchmark also reports deterministic single-worker versus bounded-worker causal-root batch measurements with worker counts, elapsed times, speedup ratio, and checksum delta. The benchmark runner writes `solver-benchmark-report.v1` JSON under `.tmp/solver-build/benchmark/` by default, records runtime/platform/CPU/memory/`EM_CACHE` context, and validates that the parsed report matches the console case count. These are benchmark observations, not machine-dependent acceptance thresholds.
...
| `emission_shell_broad_phase_query` | Synthetic emitter/receiver paths replayed from path-history chunks, with path-count, time-slab, density, same-source, all-to-all, and speed-regime transition sweeps. | Interval-time-slab rows, spatial-hash receiver-cell rows, emission-shell annulus rows, indexed candidate pair lists, brute-force replay oracle, sampled narrow-phase roots, and deterministic work-packet headers with `emission_shell_candidate.v1` outputs. | Implemented as `emission-shell-broad-phase-v0`: solver-owned indexed broad phase finds every brute-force oracle candidate, reports zero missed candidates, records candidate reduction, false-positive ratio, same-source candidates, speed-regime transition candidates, and packet replay equality. |
