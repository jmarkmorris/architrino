...
Current implementation note: the native runtime benchmark reports strategy labels and named metrics for emission-shell broad-phase and combined spacetime-cell index workloads, including tested pairs, rejected pairs, candidate rate, rejection rate, index rows, query matches, and overflow entries. It also reports deterministic single-worker versus bounded-worker causal-root batch measurements with worker counts, elapsed times, speedup ratio, and checksum delta. The benchmark runner writes `solver-benchmark-report.v1` JSON under `.tmp/solver-build/benchmark/` by default, records runtime/platform/CPU/memory/`EM_CACHE` context, and validates that the parsed report matches the console case count. These are benchmark observations, not machine-dependent acceptance thresholds.
...
- `simulation_envelope_contract` - Closed as a standalone priority contract in [simulation-envelope-contract](simulation-envelope-contract.md). Downstream implementation work remains in `solver_contract`, stream implementation and validation from [path-history-stream-contract](path-history-stream-contract.md), `app_bridge_contract`, and `simulation_envelope_admission` fixture coverage.
- `numeric_serialization_contract` - Closed as a standalone priority contract in [numeric-serialization-contract](numeric-serialization-contract.md). Downstream implementation work remains in stream manifests governed by [path-history-stream-contract](path-history-stream-contract.md), `solver_contract`, app bridge validation, and validation replay artifacts.
- `path_history_stream_contract` - Closed as a standalone priority contract in [path-history-stream-contract](path-history-stream-contract.md), with schema artifacts and all six stream fixture evidence rows validated by [check-solver-contract-fixtures](../../../scripts/check-solver-contract-fixtures.mjs). Storage lifecycle consumption and work-packet chunk references are covered by their completed contract artifacts; downstream implementation work remains in `app_bridge_contract`, `solver_contract`, and budgeted spill/readback/index/recovery benchmark cases.
- `storage_lifecycle_policy` - Closed as a standalone priority policy in [storage-lifecycle-policy](storage-lifecycle-policy.md). Downstream implementation work remains in `spatiotemporal_query_algorithm_survey`, `app_bridge_contract`, `solver_contract`, storage lifecycle fixtures, and browser/native retention implementation.
- `work_packet_transport_contract` - Closed as a standalone priority contract in [work-packet-transport-contract](work-packet-transport-contract.md), with a focused audit in [work-packet-transport-contract-audit](../../../scripts/solver-audits/work-packet-transport-contract-audit.mjs). Downstream implementation work remains in `spatiotemporal_query_algorithm_survey`, `threading_execution_policy`, `app_bridge_contract`, `solver_contract`, and `work_packet_round_trip` fixture coverage.

## Task Queue

1. `precision_dynamic_range_contract` - Convert [precision.md](precision.md)'s `auto` precision-path selector, scale-aware numeric charts, strict escalation rule, validation replay rule, stage-level error budgets, and error propagation metadata into the first implementation contract. Status: `active-root-forensics`. Depends on: none.
2. `virtual_observer_path_record_contract` - Define the minimal segment-level path record, interpolation policy, error-bound metadata, normalized assembly-membership references, membership-change events, dynamic replay attachment, and potential audit attachment. Status: `active`. Depends on: `precision_dynamic_range_contract`, `simulation_envelope_contract`.
3. `temporal_assembly_graph_contract` - Define assembly-state history, membership intervals, parent-child hierarchy intervals, assembly events, split/merge events, ambiguous membership records, threshold/self-action events, stable ids, graph indices, and hierarchy replay rules. Status: `active`. Depends on: `virtual_observer_path_record_contract`.
4. `spatiotemporal_query_algorithm_survey` - Prototype and benchmark broad-phase / narrow-phase query approaches for space blocks, time blocks, combined spacetime blocks, path-vs-emission-shell, path-vs-path, all-to-all, same-source, speed-regime transition, and candidate causal-root searches. Candidate families include bounding-volume hierarchies, interval trees, spatial hashing, k-d trees, R-trees, sweep-and-prune, and special-purpose emission-shell indices. Status: `active`. Depends on: `simulation_envelope_contract`. Uses completed [path-history-stream-contract](path-history-stream-contract.md), [storage-lifecycle-policy](storage-lifecycle-policy.md), and [work-packet-transport-contract](work-packet-transport-contract.md).
5. `app_bridge_contract` - Convert the chosen shared JavaScript adapter with TypeScript declarations and WebAssembly worker into a typed request/response, cancellation, stream-handle, diagnostics, normalized-error, model-contract, and error-budget contract. Status: `active-schema-validated-bridge`. Depends on: `precision_dynamic_range_contract`, `simulation_envelope_contract`. Uses completed [path-history-stream-contract](path-history-stream-contract.md) and [work-packet-transport-contract](work-packet-transport-contract.md).
6. `threading_execution_policy` - Implement the chosen native bounded task pool, browser worker baseline, deterministic mode, WebAssembly-thread gating, thread-count controls, diagnostics, and GPU-ready work partitioning where it also benefits CPU execution. Status: `active-first-plan-benchmark`. Depends on: `app_bridge_contract`. Uses completed [work-packet-transport-contract](work-packet-transport-contract.md).
7. `cpp_clang_runtime_validation` - Build and benchmark the selected C++20/Clang path against representative causal-root, source-history, precision, dynamic-range, simulation-envelope, virtual-observer path-record, temporal-assembly-graph, streaming-write, indexed-read, app-bridge, and thread-scaling workloads. Status: `active-runtime-benchmarking`. Depends on: `threading_execution_policy`.
8. `solver_contract` - Implement the central solver inputs, outputs, dataset schema, model schema, simulation-envelope schema, virtual-observer path-record schema, assembly-state schema, assembly-membership schema, assembly-hierarchy schema, path-history stream schema, numeric serialization schema, app bridge schema, threading metadata, diagnostics, halt statuses, precision-path metadata, storage metadata, API boundaries, root-ledger completeness rows, phase-at-hit metadata, failure-code taxonomy, and provenance artifacts. Status: `active`. Depends on: `cpp_clang_runtime_validation`.
9. `analytic_and_invariant_validation` - Define manufactured causal-root cases, closed-form geometry cases, root-count invariants, residual checks, conservation or bounded-drift checks where the model provides them, and stream replay invariants. Status: `active`. Depends on: `solver_contract`.
10. `baseline_comparison_sandbox` - Define isolated baseline harnesses for Photon, Ideal Swarm, and Animator, fixed input cases, resource caps, no-network execution, fixed seeds, controlled working directories, artifact-only output, tolerance files, provenance records, divergence reports, and ordered migration parity reporting. Status: `active-parity-report`. Depends on: `app_bridge_contract`, `solver_contract`, `analytic_and_invariant_validation`.
11. `gpu_acceleration_deferral` - Keep Metal, WebGPU, service GPU, and other GPU compute paths out of the first solver core and migration plan; preserve GPU-ready data layout and work partitioning, then reconsider GPU execution only after CPU benchmarks identify a suitable regular hotspot. Status: `settled-deferred`. Depends on: `cpp_clang_runtime_validation`.
12. `geometry_centralization_inventory` - Identify duplicated or app-side solver geometry in Photon, Ideal Swarm, and Animator. Status: `active-first-inventory-check`. Depends on: `solver_contract`.
13. `minimal_causal_root_core` - Implement or extract the first central causal-root core with source histories, branch diagnostics, precision diagnostics, simulation-envelope diagnostics, virtual-observer path-record output, temporal-assembly-graph output, streaming output, app-bridge output, threading diagnostics, and benchmark hooks. Status: `active-first-core-smoke`. Depends on: `solver_contract`.
14. `animator_adapter` - Route Animator simulation runs through the central solver contract while preserving the existing dataset playback surface. Status: `active-authoring-bridge-opt-in`. Depends on: `minimal_causal_root_core`, `baseline_comparison_sandbox`.
15. `photon_adapter` - Replace Photon-local causal-root diagnostics with shared source-history and causal-root calls. Status: `active-app-owned-bridge-migration`. Depends on: `minimal_causal_root_core`, `baseline_comparison_sandbox`.
16. `ideal_swarm_adapter` - Replace Ideal Swarm delayed-potential and self-hit calculations with shared solver geometry. Status: `migrated-app-owned-bridge`. Depends on: `minimal_causal_root_core`, `baseline_comparison_sandbox`.
17. `h39_solver_impact_audit` - Audit whether the central solver would likely have changed the H39 solution path by selecting representative H39 root, path-history, precision, and ledger bottleneck cases; building replay fixtures; comparing historical script artifacts with central-solver diagnostics; and classifying correctness, runtime, manual-effort, and proof-path impact. Audit note: [h39-solver-impact-audit](h39-solver-impact-audit.md). Status: `active-terminal-graph-replay-open`. Depends on: `minimal_causal_root_core`, `precision_dynamic_range_contract`, `analytic_and_invariant_validation`. Uses completed [path-history-stream-contract](path-history-stream-contract.md).
