...
Current inventory note: [geometry-centralization-inventory](geometry-centralization-inventory.md) is the closed inventory for Photon, Ideal Swarm, and Animator app-side geometry. Animator delayed-hit shell/path intersections now route through stream-backed descriptor requests and solver-owned delayed-hit rows in [AnimatorDelayedHitRows.mjs](../../../src/solver/app/AnimatorDelayedHitRows.mjs), and field-shell emission cadence now routes through solver-owned stream-package rows in [AnimatorFieldShellEventStream.mjs](../../../src/solver/app/AnimatorFieldShellEventStream.mjs). The remaining Animator boundary is emitter source-history authority and optional ABI-backed native durable storage for `field_shell_events.v1` rather than receiver-track sample arrays or local cadence rows. Photon delayed-emission contribution and observer-field reconstruction, plus Ideal Swarm source-history construction and fallback span profiles, remain app-local cleanup surfaces. Those removals require solver-owned rows or adapter descriptors first; renderer-only display geometry remains app-owned.
...
- `animator_adapter` - Closed as a motion dataset and playback bridge adapter in [animator-adapter](animator-adapter.md). Animator delayed-hit shell/path intersections now use stream-backed solver descriptors and solver-owned delayed-hit rows; field-shell event/cadence packaging now uses solver-owned stream-package rows. Emitter source-history authority and optional ABI-backed native storage for `field_shell_events.v1` remain future adapter work.
