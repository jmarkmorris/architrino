...
| --- | --- | --- | --- |
| `static_static_single_root` | Fixed source and fixed receiver with distance $d$ and hit time $T$. | One root at $t_e = T - d / c_f$ when $t_e$ lies inside the source support. | Root count 1, delay $d / c_f$, distance $d$, residual 0 within tolerance, positive delay, finite Jacobian, branch weight $1 / \lvert J \rvert$. |
...
| `small_jacobian_tangent` | Source speed and geometry tuned so the root is tangent or nearly tangent to the signal cone. | Root is rejected or downgraded unless the selected precision path can certify it. | Status `small_jacobian` or `transversality_floor_failed`; no ordinary branch weight is trusted when $\lvert J \rvert$ is below floor. |
...
| `delayed_potential_flight_time` | Ideal Swarm-style flight-time rows with fixed source/receiver geometry. | Emission time, delay, distance, potential row status, and shared-geometry artifact hash match reference. |
...
| Branch weight | For ordinary roots, branch weight or hit strength is `1 / abs(J)` inside branch-weight tolerance. | `validation_replay_mismatch`; `small_jacobian` when $\lvert J \rvert$ is below floor |
...
| Small Jacobian | `small_jacobian` | $\lvert J \rvert$ is below the branch-weight authority floor; ordinary inverse-Jacobian weighting is not trusted. |
...
| `closed_form_geometry` | Displacement/distance/direction, path bounds, constant acceleration, sphere membership, bounds overlap, circular self-hit span, delayed-potential flight time, phase-at-hit cycle. | Shared geometry, motion, phase, and Ideal Swarm bridge rows already exist. | Geometry row outputs match closed forms or high-precision references and preserve authority labels. |
...
| `root_hit_invariant` | Finite fields, time order, delay/distance consistency, residual tolerance, inverse-Jacobian branch weight, unit direction. | Native invariant checks and app bridge `checkRootHitInvariantsF64` exist. | Valid analytic rows pass and each deliberate corruption fails with the expected status. |
