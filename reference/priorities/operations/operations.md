...
3. `deployment_budget_contract` — Define a repo-wide deployment budget schema for webapps: bundle size, static asset transfer, GitHub Pages bandwidth estimate, browser heap budget, GPU memory budget, browser storage budget, Actions artifact budget, generated-output size, and native solver throughput. Status: `pending`; first consumer: [Borg app](../borg-app/borg-app.md).
