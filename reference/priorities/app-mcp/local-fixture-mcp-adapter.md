...
The four tools declare read-only, non-destructive, idempotent, closed-world annotations and explicitly forbid task-augmented execution.
...
4. `tools/list` returns exactly the four V1 tools in stable order with read-only annotations;
5. all four tools cross the stdio boundary and return typed structured results;
...
- any of the four tools fails to cross the stdio boundary;
