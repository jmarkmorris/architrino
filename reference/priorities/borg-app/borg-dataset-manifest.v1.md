...
The next build burden is `native_master_equation_fixed_parameter_fixture`: extend the native central solver contract, native implementation, bridge schema, Borg fixture generator, and validation fixture so the manifest can replace `nativeMasterEquationStatus = native-fixture-capability-missing` with authoritative fixed-parameter many-body acceleration, path-history, and wake/interaction evidence. Browser surface-budget measurement remains required later, but it must not precede the native solver evidence needed for Borg physics interpretation.
