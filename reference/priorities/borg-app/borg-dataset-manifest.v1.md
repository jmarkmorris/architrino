...
The next build burden is `native_master_equation_fixed_parameter_fixture`: implement the native solver behind `architrino_solver_integrate_master_equation_motion_f64` so the manifest can replace `nativeMasterEquationStatus = native-fixture-solver-pending` with authoritative fixed-parameter many-body acceleration, path-history, and wake/interaction evidence. Browser surface-budget measurement remains required later, but it must not precede the native solver evidence needed for Borg physics interpretation.
