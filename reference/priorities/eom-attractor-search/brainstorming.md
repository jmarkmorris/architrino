...
- **Phase 0 — workload characterization.** Profile the Borg-style release
  ...
  empirically. Existing engine optimizations targeted the pinned-fold MPFR
...
- **Phase 1 — single performance enhancement (SUPERSEDED as a decision:
  ...
  future change: byte-identical trajectories (SHA-256) on the standing
  engine fixtures, native↔oracle parity suites green, A-B-A paired timing, no
  certificate-rule or tolerance change, never in the same diff as a
  comparison instrument.
- **Phase 2 — headless ensemble harness:** declared seed distributions
  for neutral populations; endpoint-matched prehistory discipline built in (materially
...
- **Pinned/self-fold cost wall:** seeds at exactly $v = c_f$ (or total
  speed pinned to $c_f$) pay a perpetual near-singularity cost.
...
- What is the right persistence horizon $k$ given measured escape
  timescales?
- Static-observer pulse waveform of a passing held assembly (an E-B
  recovery probe); repeat inside a modeled sea once the constitutive
  operator exists.
