...
1. `fundamental_angular_momentum_ledger` — Promote the branch-resolved total-angular-momentum scaffold into a validated conserved functional for a Noether core whose binary frequencies change under momentum transfer, including self-hit branches, wake history, and the delayed-interior characteristic-tail Noether boundary terms once the Master-Equation action-kernel closure is accepted. Status: `scaffolded`; action derivation and validation pending. Depends on: `characteristic_tail_noether_closure` in [master-equation-closure](../master-equation-closure/master-equation-closure.md).
