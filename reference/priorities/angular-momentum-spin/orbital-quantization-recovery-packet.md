...
Status. Proof packet for `orbital_quantization_recovery` in [angular-momentum-spin.md](angular-momentum-spin.md). This packet is priority material. It does not close internal Noether-core spin, spin-statistics, atomic spin-orbit coupling, hyperfine structure, or molecular singlet/triplet behavior.
...
where $\Pi_{\mathrm{env}}$ is the declared projection from the internal Noether-core angular-momentum ledger into the external envelope chart. For a clean orbital recovery, $\Delta_{\mathrm{int}}$ must stay below tolerance unless the calculation is explicitly a spin-orbit or hyperfine coupling calculation.
...
| Row | Orbital quantization | Internal Noether-core spin |
| --- | --- | --- |
| Object | Effective external envelope $\Psi_{\mathrm{env}}$ | Ordered Noether-core branch history $\mathfrak H_B$ |
...
| `internal_spin_conflation_fail` | The calculation uses internal Noether-core spinor data to force an orbital label, or uses orbital labels to prove spin. | Keep orbital recovery and spinor closure separated. |
