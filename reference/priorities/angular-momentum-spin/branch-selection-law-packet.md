...
This packet supplies the deterministic residual law target, not the proof that delayed Noether-core dynamics realizes it. The next proof burden is to compute at least one nontrivial finite candidate set $\mathcal A(B^-,\Gamma_{\text{coupl}},W)$ from retained branch-chart data, evaluate $\mathcal R_{\mathrm{sel}}$ on every candidate, and show either:
