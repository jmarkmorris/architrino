...
The direct candidate claim is that the three binaries may share one frequency while their effective lever arms and speed relations differ because the retained branch energy places each binary differently inside the velocity-deforming tri-binary Noether swarm:
...
more naturally for the canonical $(I,M,O)=(f+2,f,f-1)$ family than for the symmetric control, or whether the equal-frequency family collapses the target to $\omega_\ast=\omega_f$ while shifting the proof burden to the retained energy-radius law. Equivalently, the offset comparison includes canonical $(I,M,O)=(f+2,f,f-1)$, while the equal-frequency comparison keeps canonical $(I,M,O)=(f,f,f)$ and varies retained tri-binary Noether swarm lever arms, envelope deformation, speeds, phase offsets, root multiplicity, and wake routing. If either comparison depends on undeclared wake or root-energy slack, the candidate remains blocked rather than promoted.
...
Equal-frequency runner addition 2026-06-21. The runner now includes $(I,M,O)=(f,f,f)$ as the priority `equal-frequency-energy-radius` family and adds an `equal-frequency` policy rather than forcing the candidate through `index-ratio`. The sampled replay for $2\le f\le8$ now emits `aaa-tri-binary-frequency-candidate-solver-report.v11`, `aaa-tri-binary-frequency-triplet-search-summary.v12`, and `equalFrequencyEnergyRadiusAudit` schema `aaa-equal-frequency-energy-radius-audit.v13` with status `equal_frequency_phase_catalog_populated_priority_triadic_pass_acceptance_blocked`: common angular frequency passes, retained effective lever-arm projections populate in canonical `I:M:O` order as $\rho_I=1.25/f$, $\rho_M=1/f$, and $\rho_O=0.75/f$, speed ratios populate as $1.25:1:0.75$, the priority triadic phase profile populates as $\phi_I=0$, $\phi_M=1/3$, and $\phi_O=2/3$ turns, and $\omega_\ast$ collapses to the common frequency $\omega_f=f$. The v11 solver contract intentionally removes the ambiguous public field `layers[].radius` and the misleading `geometryLiftTarget` / `retainedLiftTarget` geometry language; report consumers should read `layers[].effectiveLeverArm` for the speed-row quantity, `layers[].solverGeometry.circularSourceRadius` only for the flattened-limit circular-source chart parameter, and `retainedBranchModel` for the primary retained tri-binary Noether swarm branch. The base geometry model is `tri-binary-noether-swarm-velocity-deformation`; the primary branch model is `retained-tri-binary-noether-swarm-deformation-branch`; and the current executable chart is the flattened-limit projection view of `tri-binary-noether-swarm-deformation-projection-chart-family`, not the retained geometry itself. The equal-frequency phase catalog now has 28 cases: seven triadic-120 priority cases and 21 controls across `aligned-0`, `quadrature-90`, and `opposed-middle-180`. The triadic-120 profile ranks first by equal-spacing residual, with maximum residual $5.551115123125783\times10^{-17}$; `quadrature-90` has residual $1/6$, and the aligned/opposed controls have residual $1/3$. The v13 phase-response discriminator reports `phase_profiles_share_root_signature_but_phase_delta_means_differ`: all four phase profiles share one root, one active root detail, and one phase diagnostic row per layer, while the profile means differ in the middle phase-delta row by $0.21208071888628424$ and in the outer phase-delta row by $0.11371534893878049$. The v12 search summary's v8 candidate-set review keeps $(f,f,f)$, $(f+2,f,f-1)$, $(f+1,f,f-1)$, $(4f,2f,f)$, and finite $(nf,mf,f)$ controls in canonical `I:M:O` order and marks `O:M:I` only as legacy translation metadata. The v13 role-assignment audit populates all six $S_3$ role maps for the triadic $120^\circ$ profile: three preserve the forward `I:M:O` $+120^\circ$ orientation, three reverse it to $-120^\circ$, and all collapse to one analysis-only quotient-sector key. The v13 deformation-regime audit populates seven of seven low-drift spherical-orbit support rows, seven of seven moving oblate-envelope incidence projection candidates, and seven of seven flattened-limit projection rows; the common-sphere great-circle control passes zero of seven rows because equal great-circle lever arms force equal speed magnitude at common $\omega_f$. The v13 deformation-continuation audit emits `aaa-equal-frequency-deformation-continuation-audit.v1`: seven of seven priority rows have a restricted spherical endpoint, current oblate slice, and flattened-projection agreement; for $f=2$ it uses $\xi_{\mathrm{current}}=0.6$ and $q_I:q_M:q_O=1:7/16:0$, so retained dynamics must now derive the general continuation target $ho_a( i)=R_{perp}sqrt{ i^2u_a^2+(1- i^2)q_a}$ with spherical-orbit coordinate `u_a` and flattened/incidence coordinate `q_a`, rather than merely populate the restricted kinematic witness. The v13 action-ledger audit adds the first reduced $h$-unit diagnostic: under the unit-inertia circular proxy, $J_I:J_M:J_O=25:16:9$, so the inner-tagged row carries exactly one half of the proxy action in seven of seven priority rows; the full $2:1:1$ four-substep partition passes zero of seven unit-inertia rows, while seven of seven rows admit a positive effective-inertia solution such as $\mu_I:\mu_M:\mu_O=1.28:1:1.7777777777777777$ normalized to the middle row. The v13 effective-inertia law scan reports `no_independent_simple_law_exact_target_weighted_inverse_square_law_exact`: no independent kinematic law in the scan is exact; inverse-square lever-arm inertia matches the outer row exactly and misses only the inner doubled factor; and root-weighted inverse-square lever-arm inertia is exact in seven of seven rows as a target-derived proof route. The equal-frequency audit also emits `aaa-equal-frequency-retained-tri-binary-replay-target.v8`, whose missing rows are retained energy-radius stationarity, velocity-deformation branch-continuation map and deformation-regime selection, root-multiplicity weighted inverse-square effective-inertia law or wake/coupling action partition, same-event one-unit angular-momentum ledger, and binary-to-binary phase row-set identity. This is the flattened-limit projection view within the velocity-deforming tri-binary Noether swarm projection family, not the retained geometry itself. The report makes the geometry verdict explicit: $\rho_I>\rho_M>\rho_O$ passes as effective-lever-arm order, while the nested-shell-radius reading $\rho_I<\rho_M<\rho_O$ fails unless a retained replay derives a projection, envelope-deformation map, incidence map, or branch-radius map. The next replay should evaluate those rows in the primary retained tri-binary Noether swarm branch, with retained phase row-set identity, root-multiplicity or branch-incidence support for the inner burden, and one-unit angular-momentum / wake-energy routing.
