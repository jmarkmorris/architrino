...
with $\theta_{\mathrm{rec}}$ and $\zeta_{\mathrm{rec}}$ supplied by the record-cycle section below. The effective spinor coordinate $\psi(Z)$ is not part of this input unless a separate spinor-closure artifact has derived it from the ordered Noether-core history.
...
| Effective spinor coordinate | $\psi(Z)$, $p_+(Z;\hat{\mathbf m})$ | not supplied | Ordered Noether-core spinor closure is still pending. |
