...
Claim level. Reduced toy model / defer with blocker. The model shows how the record-cycle partition row can reproduce a half-angle basin measure once $p_{+}$ is supplied. It does not derive $p_{+}$ from a Noether-core ledger or a concrete apparatus impulse.
