...
Use one natural work packet per thread, but do not create an artificial stop just because the first pass produced a next action. If the current prompt grants broad advancement, integration, self-running, or team-agent authority, finish the first pass and continue into the highest-value direct follow-on task while it remains safe, scoped to the same workstream, and does not require Op discussion or a theory/canon/terminology decision. Do not ask Op to approve safe edits that Cody is already confident are low-risk and canon-conforming. Package only work outside current authority, risky edits, major scope expansions, new theory leaps, canon/terminology decisions, blocked tasks, or separate long-running packets as detailed prompt options. Ask for approval in the current thread only when the decision is required to complete the current task.
...
1. [High] Prompt: short title
   Use this prompt:
   ```text
   Cody, use the AAA corpus advancement skill in <mode>.
