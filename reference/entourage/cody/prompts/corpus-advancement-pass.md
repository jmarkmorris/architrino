...
## Goal-First Prompt And Forward Goal Standard

Every prompt generated by this protocol must begin with a `Goal:` statement before any command, context, task, scope, constraints, or expected output. The goal should state one concrete objective Codex goal mode can optimize for. Detailed work instructions follow after the goal.

Every numbered Priority Action Menu item, continuation prompt, discussion prompt, approval prompt, team-agent shard prompt, and new-thread prompt must follow this goal-first shape. Do not make Op reconstruct the objective from context.

Every pause, approval request, discussion request, or final response must include a concise `Forward goal:` line near the end. Use it to state the next concrete objective Op can paste, select, or refine. Use `Forward goal: none required` only when no useful continuation remains.
...
- Every actionable leap of intuition must also appear as a detailed, paste-ready prompt whose first line is `Goal:` in the end-of-response Priority Action Menu unless it was already discussed and authorized in the current task. Use `Recommendation: discuss first` when canonization is not yet safe.
...
At every pause, approval request, discussion request, or final response, make the thread state explicit at the end of the response after the report and any Priority Action Menu. The last thing Op sees should be the current status, the forward goal, and the exact next action:
...
Forward goal: none required | concise next objective for Op to reuse or refine
...
3. A ranked edit plan for work outside the completed scope; every numbered option must use the goal-first detailed prompt format above.
4. Leaps of intuition to discuss with Op before canonizing or editing into AAA prose; every actionable leap must also appear as a goal-first detailed prompt option.
...
7. The Priority Action Menu, if any, followed by the thread state handoff block with `Forward goal:` as the final item.
```
...
Final response:
Briefly list the files touched, summarize the main cleanup, mention generated artifacts if refreshed, report validation status, include `Forward goal:`, then end with any Priority Action Menu and the thread state handoff block.
```
...
11. Detailed continuation prompt options and Priority Action Menu:
   ...
   - each numbered next-action option written as a detailed, paste-ready prompt using the format above, with `Goal:` as the first line,
...
12. Thread state handoff block with `Forward goal:` as the final item. In this variant, `Authority used` should distinguish `claim-card-only`, `report-only`, and `edited files`, and `Files changed` should say whether any AAA prose changed.
...
```text
Goal:
Coordinate a non-overlapping team-agent corpus advancement pass that extracts concrete mathematical advances, resolves findings against canon, and returns goal-first follow-up prompts for any work outside current edit authority.
...
3. A ranked edit plan; every numbered option must use the goal-first detailed prompt format above.
4. Leaps of intuition to discuss with Op before canonizing or editing into AAA prose; every actionable leap must also appear as a goal-first detailed prompt option.
5. Concrete priority or infrastructure action items, only where they have enough detail to execute.
6. Any questions that must be answered before editing.
7. The Priority Action Menu, if any, followed by the thread state handoff block with `Forward goal:` as the final item.
