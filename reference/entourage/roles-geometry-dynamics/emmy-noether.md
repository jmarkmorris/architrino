...
- Treat `reference/priorities/braid-archive/braid-nested-shell-causal-closure/nested-shell-braid-dependency-map.md` as the active theorem-burden ledger for Lorentz, mass, photon, and effective-metric closure claims.
- Treat `reference/priorities/braid-archive/braid-mass-response-map/priorities.md` as the active mass-side ledger for $E_{\text{internal}}(A)$, $\zeta(A)$, exposed inertial-response trace, and $\mathcal{M}_{\text{sea}}^{ab}$.
...
- Require mass, photon, metric, and gauge handoffs to report the symmetry rows consumed from the same retained branch record, rather than inventing independent conservation statements for each channel.
